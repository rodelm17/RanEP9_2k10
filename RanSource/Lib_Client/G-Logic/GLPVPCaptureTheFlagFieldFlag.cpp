...
void GLPVPCaptureTheFlagField::FlagToHold( DWORD dwCharID, DWORD dwGlobID )
{
	...
	if ( pcrow->m_pCrowData->m_sBasic.m_emCTFType != EMNPC_CTF_FLAG_TYPE_A && pcrow->m_pCrowData->m_sBasic.m_emCTFType != EMNPC_CTF_FLAG_TYPE_B )	
		return;

	if ( pcrow->m_pCrowData->m_sBasic.m_emCTFType == EMNPC_CTF_FLAG_TYPE_A  && pplayer_data->wInfoTeam != CAPTURE_THE_FLAG_TEAM_B )
		return;

	if ( pcrow->m_pCrowData->m_sBasic.m_emCTFType == EMNPC_CTF_FLAG_TYPE_B  &&  pplayer_data->wInfoTeam != CAPTURE_THE_FLAG_TEAM_A )
		return;

	if ( m_bFlagHold[pplayer_data->wInfoTeam] )	return;

	if ( pcrow->m_pCrowData->m_sBasic.m_emCTFType == EMNPC_CTF_FLAG_TYPE_A )
	{
	...
	else if ( pcrow->m_pCrowData->m_sBasic.m_emCTFType == EMNPC_CTF_FLAG_TYPE_B )
	{
