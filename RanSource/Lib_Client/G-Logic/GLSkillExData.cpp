...
namespace SKILL
{
	...
	void SEXT_DATA::Assign ( const SEXT_DATA_108 &value )
	{
		...
		unknown_var0 = value.unknown_var0;
	}
	...
	void SEXT_DATA::Assign ( const SEXT_DATA_110 &value )
	{
		emANIMTYPE = value.emANIMTYPE;
		emANISTYPE = value.emANISTYPE;
	...
	void SEXT_DATA::Assign ( const SEXT_DATA_111 &value )
	{
		emANIMTYPE = value.emANIMTYPE;
		emANISTYPE = value.emANISTYPE;
		...
		unknown_var0	= value.unknown_var0;
		unknown_var1	= value.unknown_var1;
	}

	void SEXT_DATA::Assign ( const SEXT_DATA_112 &value )
	{
		emANIMTYPE = value.emANIMTYPE;
		emANISTYPE = value.emANISTYPE;
	...
	BOOL SEXT_DATA_110::LOAD ( basestream &SFile )
	{
		DWORD dwDATA(0);
		SFile >> dwDATA;	emANIMTYPE = EMANI_MAINTYPE(dwDATA);
		SFile >> dwDATA;	emANISTYPE = EMANI_SUBTYPE(dwDATA);
	...
	BOOL SEXT_DATA_111::LOAD ( basestream &SFile )
	{
		DWORD dwDATA(0);
		SFile >> dwDATA;	emANIMTYPE = EMANI_MAINTYPE(dwDATA);
		SFile >> dwDATA;	emANISTYPE = EMANI_SUBTYPE(dwDATA);
	...
	BOOL SEXT_DATA_112::LOAD ( basestream &SFile )
	{
		DWORD dwDATA(0);
		SFile >> dwDATA;	emANIMTYPE = EMANI_MAINTYPE(dwDATA);
		SFile >> dwDATA;	emANISTYPE = EMANI_SUBTYPE(dwDATA);
	...
	VOID SEXT_DATA::LoadCsv ( CStringArray &StrArray, int &iCsvCur )
	{
		...
		nMultiTatgetEffectFrame = (int)atoi( StrArray[ iCsvCur++ ] );

		unknown_var0 = (DWORD)atoi( StrArray[ iCsvCur++ ] );
		unknown_var1 = (DWORD)atoi( StrArray[ iCsvCur++ ] );
	}

	void SEXT_DATA::ReadStringStream ( std::stringstream &sStream )
	{
		sStream << emANIMTYPE << ",";
		sStream << emANISTYPE << ",";

		sStream << fDELAY4DAMAGE << ",";
		sStream << emTARG << ",";
		sStream << emTARG_POSA << ",";
		sStream << emTARG_POSB << ",";
		sStream << bTARG_ONE << ",";
		...
		sStream << emSELFZONE01 << ",";
		sStream << emSELFZONE01_POS << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strSELFZONE01[i] << ",";

		sStream << emSELFZONE02 << ",";
		sStream << emSELFZONE02_POS << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strSELFZONE02[i] << ",";

		sStream << emSELFZONE03 << ",";
		sStream << emSELFZONE03_POS << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strSELFZONE03[i] << ",";

		sStream << emTARGZONE01 << ",";
		sStream << emTARGZONE01_POS << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strTARGZONE01[i] << ",";

		sStream << emTARGZONE02 << ",";
		sStream << emTARGZONE02_POS << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strTARGZONE02[i] << ",";

		sStream << emTARGZONE03 << ",";
		sStream << emTARGZONE03_POS << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strTARGZONE03[i] << ",";

		sStream << emSELFBODY << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strSELFBODY[i] << ",";

		sStream << emTARGBODY01 << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strTARGBODY01[i] << ",";

		sStream << emTARGBODY02 << ",";
		for(int i=0; i<EMELEMENT_MAXNUM; ++i )
			sStream << strTARGBODY02[i] << ",";

		sStream << strHOLDOUT << ",";

		sStream << sICONINDEX.wMainID << ",";
		sStream << sICONINDEX.wSubID << ",";
		sStream << strICONFILE << ",";

		sStream << strCOMMENTS << ",";

		/*multi skill Juver, 2018/09/04 */
		sStream << idMultiTargetSkill.wMainID << ",";
		sStream << idMultiTargetSkill.wSubID << ",";
		sStream << nMultiTatgetEffectFrame << ",";

		sStream << unknown_var0 << ",";
		sStream << unknown_var1 << ",";
	}
};
