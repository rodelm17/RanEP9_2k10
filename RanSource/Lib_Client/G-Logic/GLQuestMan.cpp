...
GLQuestMan::QUESTGEN_RANGE* GLQuestMan::FindStartFromMapReach ( SNATIVEID nidMAP )
{
	static GLQuestMan::QUESTGEN_RANGE sRANGE;

	sRANGE = m_mapStartMapReach.equal_range ( nidMAP.dwID );
	if ( sRANGE.first==sRANGE.second )	return NULL;
...
void GLQuestMan::Insert ( DWORD dwID, SNODE sNODE )
{
	if ( sNODE.pQUEST )
	{
		...
		/*quest auto start in map, Juver, 2018/07/16 */
		if ( sSTART.nidMAP != SNATIVEID(false) )
			m_mapStartMapReach.insert ( std::make_pair(sSTART.nidMAP.dwID,dwID) );
	}
