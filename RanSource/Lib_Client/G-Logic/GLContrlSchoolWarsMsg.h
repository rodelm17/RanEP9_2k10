...
namespace GLMSG
{
	...
	struct SNETPC_SCHOOLWARS_C2A_REGISTER_REQ
	{
		...
		TCHAR	szHASH[SCHOOLWARS_PLAYER_HASH_SIZE];
		...
		{
			/* PVPSchoolWars hash check, Juver, 2021/07/17 */
			SecureZeroMemory( szHASH, sizeof(TCHAR) * (SCHOOLWARS_PLAYER_HASH_SIZE) );
	...
	struct SNETPC_SCHOOLWARS_F2A_MOB_KILL_UPDATE
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID	id_crow;
		DWORD		id_char;

		SNETPC_SCHOOLWARS_F2A_MOB_KILL_UPDATE ()
			: id_crow(false)
			, id_char(SCHOOLWARS_PLAYER_NULL)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SCHOOLWARS_F2A_MOB_KILL_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_SCHOOLWARS_A2F_MOB_KILL_UPDATE
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID	id_crow;
		DWORD		id_char;
		...
		SNETPC_SCHOOLWARS_A2F_MOB_KILL_UPDATE ()
			: id_crow(false)
			, id_char(SCHOOLWARS_PLAYER_NULL)
			, score_crow(0)
			, double_point(FALSE)
			, char_school(SCHOOLWARS_SCHOOL_SIZE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SCHOOLWARS_A2F_MOB_KILL_NOTICE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_SCHOOLWARS_F2C_MOB_KILL_UPDATE
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID	id_crow;
		DWORD		id_char;
		...
		SNETPC_SCHOOLWARS_F2C_MOB_KILL_UPDATE ()
			: id_crow(false)
			, id_char(SCHOOLWARS_PLAYER_NULL)
			, score_crow(0)
			, double_point(FALSE)
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SCHOOLWARS_F2C_MOB_KILL_NOTICE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
