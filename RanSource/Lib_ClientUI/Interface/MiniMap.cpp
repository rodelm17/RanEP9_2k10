...
#include "../../Lib_Client/G-Logic/SClubBattleTime.h"
...
CMiniMap::CMiniMap()
	...
	, m_nMin_X(1)
	, m_nMin_Y(1)
	, m_fMiniMapTimerUpdate(0.0f)

	, m_bTEST( true )
	, m_wAmmoCount( 0 )
	, m_sidAmmo(NATIVEID_NULL())
...
{	
	...
	if ( GLGaeaClient::GetInstance().GetCharacter()->VALID_SLOT_ITEM(emLHand) )
	{
			...
			emItemType == ITEM_BULLET )
		{
			...
			if ( sItemCustom.wTurnNum != m_wAmmoCount || m_sidAmmo != sItemCustom.sNativeID )
			{
				CString strCombine;
				...
				m_sidAmmo = sItemCustom.sNativeID;
			}
	...
	bool bFULL_SCREEN_ON_WINDOW = ROPTION::bScrWindowed && ROPTION::bScrWndHalfSize;
	...
	m_fMiniMapTimerUpdate += fElapsedTime;
	if ( m_fMiniMapTimerUpdate > 1.0f )
	{
		BattleUpdate();
		m_fMiniMapTimerUpdate = 0.0f;
	}
...
void CMiniMap::BattleUpdate()
{
	GLGaeaClient& pGaeaClient = GLGaeaClient::GetInstance();
	CLUB_BATTLE_TIMER_MAP& mapClubWar = pGaeaClient.m_mapCLUBBATTLETimer;

	BOOL bShowDisplay = FALSE;

	if ( bShowDisplay == FALSE )
	{
		CLUB_BATTLE_TIMER_MAP_ITER it = mapClubWar.begin();

		for( ; it != mapClubWar.end(); ++it )
		{
			SCLUBWAR_BATTLE_TIMER& sTimer = (*it).second;
			if( sTimer.bBattle && sTimer.fTime > 0.0f )
			{
				bShowDisplay = TRUE;
				break;
			}
	...
	if ( bShowDisplay == FALSE )
	{
		if ( pGaeaClient.m_bClubDeathMatch && pGaeaClient.m_fClubDeathMatchTimer > 0.0f )
			bShowDisplay = TRUE;
	}
	
	if ( bShowDisplay == FALSE )
	{
		if ( pGaeaClient.m_bFreePKMatch && pGaeaClient.m_fFreePKMatchTimer > 0.0f )
			bShowDisplay = TRUE;
	}

	if ( m_pClubTimeText )
	{
		m_pClubTimeText->SetVisibleSingle( bShowDisplay );
		...
		if ( bShowDisplay )
		{
			...
			for( ; it != mapClubWar.end(); ++it )
			{
				...
				{
					CString strCombine;
					strCombine.Format ( "%s %s %02d:%02d",  sTimer.szName, ID2GAMEWORD( "CLUB_BATTLE_TIME", 0 ),(DWORD)sTimer.fTime/60, (DWORD)sTimer.fTime%60 );
					m_pClubTimeText->AddText( strCombine, NS_UITEXTCOLOR::RED );
				}
			}
			...
			{
				CString strCombine;
				strCombine.Format ( "%s %02d:%02d", ID2GAMEWORD( "CLUB_BATTLE_TIME", 1 ), (DWORD)pGaeaClient.m_fClubDeathMatchTimer/60, (DWORD)pGaeaClient.m_fClubDeathMatchTimer%60 );
				m_pClubTimeText->AddText( strCombine, NS_UITEXTCOLOR::RED );
			}
			...
			{
				CString strCombine;
				strCombine.Format ( "%s %02d:%02d", ID2GAMEWORD( "CLUB_BATTLE_TIME", 2 ), (DWORD)pGaeaClient.m_fFreePKMatchTimer/60, (DWORD)pGaeaClient.m_fFreePKMatchTimer%60 );
				m_pClubTimeText->AddText( strCombine, NS_UITEXTCOLOR::RED );
			}
		}
