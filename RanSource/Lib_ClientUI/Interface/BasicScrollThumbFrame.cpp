...
const int CBasicScrollThumbFrame::nSCROLL_MOVE_LINE_DEFAULT = 3;
...
CBasicScrollThumbFrame::CBasicScrollThumbFrame () :
    ...
    m_nNumScrollMoveLine( nSCROLL_MOVE_LINE_DEFAULT ),
	m_pThumb ( NULL ),
	m_bMoreOnePage ( FALSE ),
	m_pUseParent(NULL),
    m_bScrollParent( false ),
...
void CBasicScrollThumbFrame::SetScrollParent( CUIControl* pControl )
{
    if ( !pControl ) 
        return;
    
    m_bScrollParent = true;
    m_pScrollParent = pControl;
}

void CBasicScrollThumbFrame::SetScrollMoveLine( int nScrollMoveLine )
{
    if ( nScrollMoveLine < 1 )
        return;

    m_nNumScrollMoveLine = nScrollMoveLine;
}

int CBasicScrollThumbFrame::GetPosition()
{	
	UINT nTotal = m_nMAXPOS;
	int nVisibleLine = m_nViewPerPage;

	const int nMovableLine = nTotal - nVisibleLine;
	float fPercent = GetPercent();
	int nViewPos = (int)floor( fPercent * nMovableLine );

// 	( nViewPos%2 != 0 ? nViewPos++ : nViewPos );
// 
// 	if( nViewPos < 0 )
// 		nViewPos = 0;
