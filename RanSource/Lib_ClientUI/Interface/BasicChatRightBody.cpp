...
CBasicTextBoxEx*	CBasicChatRightBody::CreateTextBoxEx( CD3DFontPar* pFont, UIGUID uID )
{
	...
	pTextBox->SetChat( true );/* Chat Color, Mhundz 02/22/25 */
...
void	CBasicChatRightBody::ADD_CHAT_PAGE_TEXTBOX ( CBasicTextBox::STEXTCHAT& strTemp, const D3DCOLOR& dwColor, CBasicTextBoxEx* pTextBox, CBasicScrollBarEx* pScrollBar, 
		SITEMLINK* pItemLink ) 
{
	...
	/*item link, Juver, 2017/07/31 */
	/* Chat Color, Mhundz 02/22/25 */
	pTextBox->AddTextChat ( strTemp, pItemLink, dwColor );
		...
		case CHAT_PRIVATE:
			{
				...
				{
					m_pEditBox->SetTextColor ( NS_UITEXTCOLOR::YELLOW );

					 /*item link, Juver, 2017/07/31 */
					 /* Chat Color, Mhundz 02/22/25 */
					AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_PRIVATE_MODE"), NS_UITEXTCOLOR::YELLOW,true, NULL );
				}
			}
			break;
		...
		case CHAT_REGIONAL:
			{
				...
				if ( RPARAM::bRegionalChatColor )
					dwColor = D3DCOLOR_ARGB( 0xFF, RPARAM::nRegionalChatColorR, RPARAM::nRegionalChatColorG, RPARAM::nRegionalChatColorB );

				m_pEditBox->SetTextColor ( dwColor );

				/*item link, Juver, 2017/07/31 */
				/* Chat Color, Mhundz 02/22/25 */
				AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_REGIONAL_MODE"), dwColor,true, NULL );
			}break;
...
void CBasicChatRightBody::AddChatMacro(CString strMsg)
{
	...
	bool bincmd = dxincommand::command ( strMsg.GetString (), &m_sItemLink ); 
	if ( !bincmd )				
...
/*item link, Juver, 2017/07/31 */
//juvs -- hope this wont break item link
void CBasicChatRightBody::CheckItemLink( CString strText )
{
	...
	CString strFilter = m_sItemLink.GetFilter();
	if ( strFilter.IsEmpty() )
		return;

	int n = strFind.Find ( strFilter );
	bool bFind = (n >= 0);
...
void CBasicChatRightBody::CheckItemLink( CString strText, SITEMLINK sItemLink )
{
	if ( sItemLink.bUSE )
	{
		...
		CString strFilter = sItemLink.GetFilter();
		if ( strFilter.IsEmpty() )
			return;
...
void CBasicChatRightBody::CheckShowMaxRVItemLink( CString strText, SITEMLINK sItemLink )
{
	if ( sItemLink.bUSE )
	{
		CString strFind = strText.GetString();
		...
		{
			SITEMCUSTOM sTemp;
			sTemp.Assign(sItemLink);
			CInnerInterface::GetInstance().ReqMaxRVShow( sTemp );
		}
	}
}