...
void CUIGroup::SetDiffuseAlpha( WORD wAlpha )
{
	...
	for ( ; citer != citer_end; ++citer )
	{
		...
		pControl->SetDiffuseAlpha ( wAlpha );
	}
}


void CUIGroup::SetNonBoundaryCheck( const bool bEnable )
{
	CUIControl::SetNonBoundaryCheck( bEnable );
	...
	for ( ; citer != citer_end; ++citer )
	{
		...
		pControl->SetNonBoundaryCheck ( bEnable );
	}
}


void CUIGroup::SetAlignedGlobalPos(const UIRECT& Pos)
{
	CUIControl::SetAlignedGlobalPos( Pos );
	...
	for ( ; citer != citer_end; ++citer )
	{
		...
		pControl->SetAlignedGlobalPos( rcNewChildPos );
	}
}
...
void CUIGroup::AddPosition ( const D3DXVECTOR2& vPos )
{
	CUIControl::AddPosition ( vPos );
...
{
	//original code
	//restored to check performance impact
	return m_bAllExclusiveControl;


	if( m_bAllExclusiveControl )	return TRUE;
