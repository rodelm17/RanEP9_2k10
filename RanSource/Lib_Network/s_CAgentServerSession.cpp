...
{	
	...
	case NET_MSG_IPFILTER_SESSION_TO_SERVER_BLOCK_ADD:
		{
			NET_IPFILTER_SESSION_TO_SERVER_BLOCK_ADD* netMsg = reinterpret_cast < NET_IPFILTER_SESSION_TO_SERVER_BLOCK_ADD* > (nmg);
			IPFilterSessionBlockAdd( std::string( netMsg->szIP ));
		}break;

	case NET_MSG_IPFILTER_SESSION_TO_SERVER_BLOCK_DEL:
		{
			NET_IPFILTER_SESSION_TO_SERVER_BLOCK_DEL* netMsg = reinterpret_cast < NET_IPFILTER_SESSION_TO_SERVER_BLOCK_DEL* > (nmg);
			IPFilterSessionBlockDel( std::string( netMsg->szIP ));
		}break;

	case NET_MSG_IPFILTER_SESSION_TO_SERVER_KNOWN_ADD:
		{
			NET_IPFILTER_SESSION_TO_SERVER_KNOWN_ADD* netMsg = reinterpret_cast < NET_IPFILTER_SESSION_TO_SERVER_KNOWN_ADD* > (nmg);
			IPFilterSessionKnownAdd( std::string( netMsg->szIP ));
		}break;

	case NET_MSG_IPFILTER_SESSION_TO_SERVER_KNOWN_DEL:
		{
			NET_IPFILTER_SESSION_TO_SERVER_KNOWN_DEL* netMsg = reinterpret_cast < NET_IPFILTER_SESSION_TO_SERVER_KNOWN_DEL* > (nmg);
			IPFilterSessionKnownDel( std::string( netMsg->szIP ));
		}break;

	case NET_MSG_WRITE_IP_SESSION_TO_SERVER:
		{
			NET_WRITE_IP_SESSION_TO_SERVER* netMsg = reinterpret_cast < NET_WRITE_IP_SESSION_TO_SERVER* > (nmg);
			//RLOG::LogFileNoDate( "login_ip.txt", "%s", netMsg->szIP );
		}
		break;

		/*dmk14 whitelist*/
	case NET_MSG_IPFILTER_WHITELIST_ADD_SERVERS:
		{
			NET_IPFILTER_WHITELIST_ADD_SERVERS* netMsg = reinterpret_cast < NET_IPFILTER_WHITELIST_ADD_SERVERS* > (nmg);
			m_IPFilter.AddWhiteList( std::string( netMsg->szIP ));
		}break;

	case NET_MSG_IPFILTER_WHITELIST_DEL_SERVERS:
		{
			NET_IPFILTER_WHITELIST_DEL_SERVERS* netMsg = reinterpret_cast < NET_IPFILTER_WHITELIST_DEL_SERVERS* > (nmg);
...
{	
	...
	StringCchCopy( nsi.gsi.szServerIP, MAX_IP_LENGTH+1, CCfg::GetInstance()->GetProxyIp(m_szAddress) );
