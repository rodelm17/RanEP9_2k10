...
protected :
	...
	TCHAR m_client_hash1[CLIENT_HASH_SIZE];
	TCHAR m_client_hash2[CLIENT_HASH_SIZE];

#ifdef USE_MSHIELD
	TCHAR m_szMShieldDLLHash1[CLIENT_HASH_SIZE];
	TCHAR m_szMShieldDLLHash2[CLIENT_HASH_SIZE];
#endif // USE_MSHIELD
...
public:
	...
	*/
	...
	void MsgPVPClubDeathMatchReBirthOutFb ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID );

	void MsgPVPPBGReBirthOutFb ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID );
...
public:
	...
	void UserAccountPageInit( MSG_LIST* pMsg );

	/* login device check, Juver, 2020/11/06 */
	void UserAccountPageLoginDeviceCheckSet( MSG_LIST* pMsg );
	void UserAccountPageLoginDeviceCheckSetFeedback( NET_USER_ACCOUNT_PAGE_LOGIN_DEVICE_CHECK_SET_FB2* fb2 );

	/* login device check, Juver, 2020/11/07 */
	void UserLoginDeviceCheckVerifyRequestInit( MSG_LIST* pMsg );
	void UserLoginDeviceCheckVerifyRequestAction( MSG_LIST* pMsg );
	void UserLoginDeviceCheckVerifyRequestActionFeedback( NET_USER_LOGIN_DEVICE_CHECK_VERIFY_REQUEST_ACTION_FB2* fb2 );
	
	/*dmk14 ingame web*/
	void MsgForgotPass					( MSG_LIST* pMsg );
	void MsgForgotPassBack				( NET_FORGOTPASS_FEEDBACK_DATA2* nlfd2 );
	void MsgForgotChangePass			( MSG_LIST* pMsg );
	void MsgForgotChangePassBack		( NET_CHANGEPASS_FEEDBACK_DATA2* nlfd2 );
	void MsgSelcharChangePass			( MSG_LIST* pMsg );
	void MsgSelcharChangePassBack		( NET_SELCHAR_CHANGEPASS_FEEDBACK2* nlfd2 );
	void MsgSelcharChangePin			( MSG_LIST* pMsg );
	void MsgSelcharChangePinBack		( NET_SELCHAR_CHANGEPIN_FEEDBACK2* nlfd2 );
	void MsgReqGameTimeConvert			( MSG_LIST* pMsg );
	void MsgReqGameTimeConvertBack		( NET_REQ_GAMETIME_CONVERT_FEEDBACK2* nlfd2);
	void MsgSelcharResetPin				( MSG_LIST* pMsg );
	void MsgSelcharResetPinBack			( NET_SELCHAR_RESETPIN_FEEDBACK2* nlfd2 );
	void MsgSelcharTopUp				( MSG_LIST* pMsg );
	void MsgSelcharTopUpBack			( NET_SELCHAR_TOPUP_FEEDBACK2* nlfd2 );
	void MsgSelcharChangeMail			( MSG_LIST* pMsg );
	void MsgSelcharChangeMailBack		( NET_SELCHAR_CHANGEMAIL_FEEDBACK2* nlfd2 );
...
public:
	void IPFilterBlockAdd( std::string strIP );
	void IPFilterBlockDel( std::string strIP );
	void IPFilterKnownAdd( std::string strIP );
	void IPFilterKnownDel( std::string strIP );

	void IPFilterSessionBlockAdd( std::string strIP );
	void IPFilterSessionBlockDel( std::string strIP );
	void IPFilterSessionKnownAdd( std::string strIP );
	void IPFilterSessionKnownDel( std::string strIP );

	void IPFilterSave();
	void IPFilterCleanIP( std::string strIP );

	/*client hash check, EJCode, 2018/11/26 */
	void client_check_get_client_hash( std::string app_path );
	void client_check_get_mshield_hash( std::string app_path );

	void client_check_rehash();
};
