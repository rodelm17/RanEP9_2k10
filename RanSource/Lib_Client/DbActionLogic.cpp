...
CSetChatBlockTime::CSetChatBlockTime(int nUserNum, const char* szUserID, __time64_t tBlockTime, const char* szReason, int nGMUserNum, const char* szGMUserID)
	: m_nUserNum(nUserNum)
	...
	, m_nGMUserNum(nGMUserNum)
	, m_strGMUserID(szGMUserID)
{
}

int CSetChatBlockTime::Execute(CServer* pServer)
{
	return m_pDbManager->SetChatBlockTime( m_nUserNum, m_strUserID, m_tBlockTime, m_strReason, m_nGMUserNum, m_strGMUserID );
}
...
CWTFAction::CWTFAction()
{
}

/* wtf, Juver, 2021/06/24 */
int CWTFAction::Execute(CServer* pServer)
{
	return m_pDbManager->SetWTFAction();
}
