...
// #endif
	...
	CBasicButton* pSettingButton = new CBasicButton;
	pSettingButton->CreateSub ( this, "QUICK_POTION_TRAY_SETTING_BUTTON", UI_FLAG_DEFAULT, QUICK_POTION_TRAY_SETTING_BUTTON );
	pSettingButton->CreateFlip ( "QUICK_POTION_TRAY_SETTING_BUTTON_F", CBasicButton::MOUSEIN_FLIP );
	pSettingButton->SetUseGlobalAction ( TRUE );
	pSettingButton->SetControlNameEx ( "퀵슬롯트레이 클로즈버튼" );
	RegisterControl ( pSettingButton );
}
...
void CBasicPotionTray::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
			...
			{	
				...
				if ( sSlot.sNID != NATIVEID_NULL () )
				{
					...
					if ( pItem )
					{
						...
						CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine ( pItem->GetName(), COMMENT::ITEMCOLOR[dwLevel] );
					}
				}
			}
		}
	...
	case QUICK_POTION_TRAY_SETTING_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( CHECK_LB_UP_LIKE ( dwMsg ) )
				{
					AddMessageEx ( UIMSG_TRAY_SETTING_LBUP );
				}
			}
		}
		break;
