...
DWORD DXLANDEFF::VERSION = 0x0103;
DWORD DXLANDEFF::SIZE = sizeof(float) + 
						sizeof(float) +
...
DXLANDEFF::DXLANDEFF(void) :
	...
	m_fVal2(1.0f)
{
	StringCchCopy( m_szName, LANDEFF_MAXSZ, "No Name" );
...
HRESULT DXLANDEFF::Load ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "DXLANDEFF::Load", SFile.GetFileName(), dwVer );
...
HRESULT DXLANDEFF::LOAD_0100 ( basestream &SFile )
{
	HRESULT hr(S_OK);
	...
	{
		CDebugSet::ToFile ( "LoadSet.log", "[%s/%s] Failure to load Effect File.", m_szName, m_szFileName );
		hr = E_FAIL;
	}
...
HRESULT DXLANDEFF::LOAD_0101 ( basestream &SFile )
{
	HRESULT hr(S_OK);
...
HRESULT DXLANDEFF::LOAD_0102 ( basestream &SFile )
{
	HRESULT hr(S_OK);
...
HRESULT DXLANDEFF::LOAD_0103 ( basestream &SFile )
{
	HRESULT hr(S_OK);
