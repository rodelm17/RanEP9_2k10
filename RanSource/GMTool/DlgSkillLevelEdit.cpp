...
BOOL CDlgSkillLevelEdit::OnInitDialog()
{
	...
	SetWin_Combo_Init( this, IDC_SKILLLEVELEDIT_COMBO_SKILLLEVEL, COMMENT::SKILL_LEVEL, SKILL::MAX_SKILL_LEVEL );
...
BOOL CDlgSkillLevelEdit::DataSave()
{
	...
	if( wSkillLevel >= SKILL::MAX_SKILL_LEVEL ){
		CDebugSet::MsgBox( GetSafeHwnd(), "Skill Level must be between %d ~ %d only", 1, SKILL::MAX_SKILL_LEVEL -1 );
		return FALSE;
	}
