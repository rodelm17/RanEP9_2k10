...
HRESULT DxCharPart::Render ( LPDIRECT3DDEVICEQ pd3dDevice, DxSkeleton* pSkeleton, SKINEFFDATA& sSKINEFFDATA, BOOL bShadow, BOOL bEff )
{
	...
	if ( !m_pmcMesh->pSkinInfo )
	{
		...
		case EMPEACE_WEAPON_TC_STICK_SLOT:
			m_pSlotHeld = pSkeleton->FindBone ( SKINTYPE_STRING::m_szWHEREBACKTYPE[m_emWeaponWhereBack] );
			break;
		};

		if ( m_emWeaponWhereBack == EMPEACE_WEAPON_USER_SLOT )
		{
			//use data from piece file
		}
...
void DxCharPart::RenderEff( LPDIRECT3DDEVICEQ pd3dDevice, DxSkeleton* pSkeleton, const float fScale )
{
	...
	else pMatrix = &pSkeleton->pBoneRoot->matCombined; /*ABL system, Juver, 2017/06/04 */
