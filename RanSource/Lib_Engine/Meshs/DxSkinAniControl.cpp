...
void DxSkinAniControl::ResetAnimationCore()
{
...
void DxSkinAniControl::ClassifyAnimation ()
{
	...
	{
		int nMType = m_vecAnim[n]->GetMType();
		int nSType = m_vecAnim[n]->GetSType();

		if ( nMType >= AN_TYPE_SIZE ||  nSType >= AN_SUB_00_SIZE )
		{
			CDebugSet::ErrorCode( "DxSkinAniControl::ClassifyAnimation", "Animation Key not supported Name:%s Main:%d~Sub:%d", m_vecAnim[n]->pAnimCont->m_szName, nMType, nSType );
			continue;
		}

		m_vecAniType[nMType][nSType].insert(m_vecAnim[n]);
	}
}
