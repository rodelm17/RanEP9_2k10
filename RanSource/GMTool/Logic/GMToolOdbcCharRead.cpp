...
int CGMToolOdbcBase::CharDataGet( int nUserNumber, int nChaNumber, SGMToolCharData* pData )
{
	if ( !pData ){
		return DB_ERROR;
	}

	SCHARDATA2* pCharData2 = pData->pCharData2;
	...
	_snprintf( szTemp, 1024, 
		...
		"ChaPKDeath, ChaWarChips, ChaGamePoints, ChaPlayTime, ChaPlayPoint, "
		"ChaReborn "
	...
	while (true)
	{
		...
		{		
			...
			/*contribution point, Juver, 2017/08/23 */
			::SQLGetData(pConn->hStmt,42, SQL_C_LONG, &nChaContributionPoint, 0, &cbChaContributionPoint);	
			...
			::SQLGetData(pConn->hStmt,47, SQL_C_LONG, &nChaWarChips, 0, &cbChaWarChips );	
			::SQLGetData(pConn->hStmt,48, SQL_C_LONG, &nChaGamePoints, 0, &cbChaGamePoints );	

			/* play time system, Juver, 2021/01/26 */
			::SQLGetData(pConn->hStmt,49, SQL_C_SBIGINT, &llChaPlayTime, 0, &cbChaPlayTime );	
			::SQLGetData(pConn->hStmt,50, SQL_C_SBIGINT, &llChaPlayPoint, 0, &cbChaPlayPoint );	
			...
			/*contribution point, Juver, 2017/08/23 */
			pCharData2->m_dwContributionPoint = (DWORD)nChaContributionPoint;
			...
			pCharData2->m_dwWarChips = (DWORD)nChaWarChips;
			pCharData2->m_dwGamePoints = (DWORD)nChaGamePoints;

			/* play time system, Juver, 2021/01/26 */
			pCharData2->m_llPlayTime = llChaPlayTime;
			pCharData2->m_llPlayPoint = llChaPlayPoint;

			/* reborn system, Juver, 2021/09/17 */
			pCharData2->m_dwReborn	= (DWORD) nChaReborn;    
	...
	sReturn = m_pGameDB->ReadImage("ChaInfo.ChaCodex", nChaNumber, ByteStream);
	if (sReturn == DB_ERROR)
		return DB_ERROR;
	else
		pCharData2->SETCODEX_BYBUF(ByteStream);
	...
	PersonalLockValidate( pCharData2 );
	PersonalLockRead( pCharData2 );

	/* pk combo, Juver, 2021/02/11 */
	PKComboCheck( pCharData2 );
	PKComboRead( pCharData2 );

	/* award title, Juver, 2021/02/13 */
	ChaAwardTitleGet( pCharData2->m_dwCharID, pData->mapAwardTitle );

	CombatRecordCheck( pCharData2 );

	CombatRecordRead( pCharData2 );
