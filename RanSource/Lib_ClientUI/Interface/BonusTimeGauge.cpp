...
void CBonusTimeGauge::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( CHECK_MOUSE_IN ( GetMessageEx () ) )
	{
		if ( CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineBegin () )
		{
			...
			//dmk14 btg info
			CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert( strMsg, NS_UITEXTCOLOR::WHITE  );
			CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert( ID2GAMEINTEXT("BONUS_TIME_MESSAGE",0), NS_UITEXTCOLOR::WHITE  );
			CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert( ID2GAMEINTEXT("BONUS_TIME_MESSAGE",1), NS_UITEXTCOLOR::WHITE  );
