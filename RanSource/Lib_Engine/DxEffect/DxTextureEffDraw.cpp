	...
	for( ; iter!=m_mapTexEff.end(); ++iter )
	{
		...
		while( pEff )
		{
			...
			case DEF_TEXEFF_VISUALMATERIAL:
				((DxTexEffVisualMaterial*)pEff)->FrameMove( fElapsedTime );
				break;
	...
	if( pMeshs->m_pAttrib )
	{
		...
		for ( DWORD i=0; i<pMeshs->m_dwAttrib; ++i )
		{
			...
			while( pEff )
			{
				...
				case DEF_TEXEFF_VISUALMATERIAL:
					{
						DxTexEffVisualMaterial* pEffVM = (DxTexEffVisualMaterial*)pEff;
						if ( pEffVM )
						{
							if( pEffVM->IsUseSpec() )
								pEffVM->RenderSpec( pd3dDevice, pLocalMesh, dwAttribID );

							if( pEffVM->IsUseGlow() )
								pEffVM->RenderGlow( pd3dDevice, pLocalMesh, dwAttribID );

							if(pEffVM->IsUseFlow() )
								pEffVM->RenderFlow( pd3dDevice, pMeshs->m_pEffVB, pMeshs->m_pEffIB, pMeshs->m_pAttrib, i, pMeshs->m_pVertSrc, pMeshs->m_dwVert );

							if(pEffVM->IsUseRotate())
								pEffVM->RenderRotate( pd3dDevice, pMeshs->m_pEffVB, pMeshs->m_pEffIB, pMeshs->m_pAttrib, i, pMeshs->m_pVertSrc, pMeshs->m_dwVert );
						}
					}break;
				};
				...
				case DEF_TEXEFF_VISUALMATERIAL:
					{
						...
						{
							if( pEffVM->IsUseSpec() ) 
								pEffVM->RenderSpec( pd3dDevice, pLocalMesh, i );

							if( pEffVM->IsUseGlow() )	
								pEffVM->RenderGlow( pd3dDevice, pLocalMesh, i );

							if( pEffVM->IsUseFlow() || pEffVM->IsUseRotate() )
								if( !pMeshs->m_pEffVB )	pMeshs->MakeEffectMeshs( pd3dDevice, VERTEX::FVF );
						}
					}break;
				};
