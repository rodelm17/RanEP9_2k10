...
DWORD		DxEffCharNoAlpha::VERSION			= 0x0102;
char		DxEffCharNoAlpha::NAME[MAX_PATH]	= "DxEffCharNoAlpha";
...
HRESULT	DxEffCharNoAlpha::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if ( dwVer == 0x0101 )
	{
		//	Note : 이팩트의 Property 를 저장.
		//
		EFFCHAR_PROPERTY_NOALPHA_101	sProperty;
		SFile.ReadBuffer ( &sProperty, sizeof(EFFCHAR_PROPERTY_NOALPHA_101) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharNoAlpha::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharNoAlpha::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
