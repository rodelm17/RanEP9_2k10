	...
	case HWOPTION_AUDIO_SFX_PROGRESS:
		{
			...
			if ( lVolume < 1 )	ROPTION::nSndSfx = DSBVOLUME_MIN;
			else				ROPTION::nSndSfx = lVolume - lCONTROLVOLUME;
	...
	case HWOPTION_AUDIO_ENVIR_PROGRESS:
		{
			...
			if ( lVolume < 1 )	ROPTION::nSndMap = DSBVOLUME_MIN;
			else				ROPTION::nSndMap = lVolume - lCONTROLVOLUME;
	...
	case HWOPTION_AUDIO_BGM_PROGRESS:
		{
			...
			if ( lVolume < 1 )	ROPTION::nSndMusic = DSBVOLUME_MIN;
			else				ROPTION::nSndMusic = lVolume - lCONTROLVOLUME;
...
void CBasicAudioOption::LoadCurrentOption ()
{
	...
	//	SFX	
	if (ROPTION::nSndSfx == DSBVOLUME_MIN )
		lVolume = 0;
	else
		lVolume = ROPTION::nSndSfx + lCONTROLVOLUME;
    fPercent = float(lVolume) / float(lCONTROLVOLUME);
	m_pSfxProgressBar->SetPercent ( fPercent );

	//	Environment
	if (ROPTION::nSndMap == DSBVOLUME_MIN )
		lVolume = 0;
	else
		lVolume = ROPTION::nSndMap + lCONTROLVOLUME;
    fPercent = float(lVolume) / float(lCONTROLVOLUME);
	m_pEnvirProgressBar->SetPercent ( fPercent );

	//	BGM
	if (ROPTION::nSndMusic == DSBVOLUME_MIN )
		lVolume = 0;
	else
		lVolume = ROPTION::nSndMusic + lCONTROLVOLUME;
    fPercent = float(lVolume) / float(lCONTROLVOLUME);
	m_pBgmProgressBar->SetPercent ( fPercent );

	//	소리 설정
	m_bSndMuteSfx = !ROPTION::bSndMuteSfx;
	m_pSfxButton->SetFlip( m_bSndMuteSfx );

	m_bSndMuteMap = !ROPTION::bSndMuteMap;
	m_pEnvirButton->SetFlip( m_bSndMuteMap );

	m_bSndMuteMusic = !ROPTION::bSndMuteMusic;
	m_pBgmButton->SetFlip( m_bSndMuteMusic );
}
