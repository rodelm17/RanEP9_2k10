...
CNameDisplay* CNameDisplayMan::MAKE_NAMEDISPLAY ()
{
	...
	if( !pNameDisplay->IsMemPool() )
	{
		...
		{
			pNameDisplay->CreateSub ( this, "NAME_DISPLAY_EP9", UI_FLAG_XSIZE | UI_FLAG_YSIZE, m_CONTROL_NEWID );
		}
		...
		if ( pDISP->DIFFERENT(sDISP_NAME) )
		{
			...
			//	Note : 새로 생성.
			{
				...
				if( sDISP_NAME.m_szBADGE[0] != _T('\0') )
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_bRankMark, sDISP_NAME.m_szBADGE, sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY, sDISP_NAME.m_wCaptureTheFlagTeam,sDISP_NAME.m_dwMMR );
				else
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_bRankMark, "", sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY, sDISP_NAME.m_wCaptureTheFlagTeam,sDISP_NAME.m_dwMMR );
...
void CNameDisplayMan::UpdateHold ( DWORD _dwCOUNT )
{
	...
	while ( pos!=end )
	{
		...
		if ( emCROW==CROW_ZONE_NAME )						bVISIBLE = FALSE;
		if ( emCROW==CROW_GATE_NAME )						bVISIBLE = FALSE;
