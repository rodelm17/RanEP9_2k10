...
HRESULT EFF_PROPERTY::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	BOOL bValid = FALSE;
	...
	if ( bValid )
	{
		...
		pNewProp->R2LoadFile ( SFile, pd3dDevice );
		m_pSibling = pNewProp;
	}
...
HRESULT EFF_PROPGROUP::LOAD_0201 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::R2LOAD_0100 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
	...
	if ( bValid )
	{
		...
		hr = pNewProp->R2LoadFile ( SFile, pd3dDevice );
		if ( FAILED(hr) )
		{
			SAFE_DELETE(pNewProp);
			return hr;
...
HRESULT EFF_PROPGROUP::R2LOAD_0101 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::R2LOAD_0102 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::R2LOAD_0103 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::R2LOAD_0104 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::R2LOAD_0105 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::R2LOAD_0106 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT EFF_PROPGROUP::SaveFile ( const char* szFileName )
{
	...
	if ( SFile.OpenFile(FOT_WRITE,m_strFileName.c_str()) )
	{
		...
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_EFFECT_GROUP_V1 );
...
HRESULT EFF_PROPGROUP::LoadFile ( const char* szFileName, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer>=VERSION_ENCODE )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_EFFECT_EGP_VER_1 );

	if ( dwVer>=ENCODE_VER_BYTE_CRYPT_2021_EFFECT_GROUP_V1 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_EFFECT_GROUP_V1 );

	if ( dwVer == EFF_PROPGROUP::VERSION )
	{
		hr = LOAD_0201( SFile, pd3dDevice );
	}
...
HRESULT EFF_PROPGROUP::R2LoadFile ( const char* szFileName, LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( !szFileName )		return S_FALSE;
	...
	{
		if( strlen( szFileName ) > 0 )
			CDebugSet::ErrorFile( "EFF_PROPGROUP::R2LoadFile", szFileName );
	...
	if ( dwFileVer > EFF_PROPGROUP::FILEVERSION )
	{
		CDebugSet::ErrorVersion( "EFF_PROPGROUP::R2LoadFile", dwFileVer );
		return E_FAIL;
	}
	...
	if ( dwVer == 0x0106 )
	{
		hr = R2LOAD_0106( SFile, pd3dDevice );
	}
	else if ( dwVer == 0x0105 )
	{
		hr = R2LOAD_0105( SFile, pd3dDevice );
	}
	else if ( dwVer == 0x0104 )
	{
		hr = R2LOAD_0104( SFile, pd3dDevice );
	}
	else if ( dwVer == 0x0103 )
	{
		hr = R2LOAD_0103( SFile, pd3dDevice );
	}
	else if ( dwVer == 0x0102 )
	{
		hr = R2LOAD_0102( SFile, pd3dDevice );
	}
	else if ( dwVer == 0x0101 )
	{
		hr = R2LOAD_0101( SFile, pd3dDevice );
	}
	else if ( dwVer == 0x0100 )
	{
		hr = R2LOAD_0100( SFile, pd3dDevice );
	}
	else
	{
		CDebugSet::ErrorVersion( "EFF_PROPGROUP::R2LoadFile", dwVer );
		return E_FAIL;
	}
