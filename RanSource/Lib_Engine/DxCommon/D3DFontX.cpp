...
HRESULT CD3DFontX::GetTextExtent( const TCHAR* strText, SIZE &Size )
{
	...
	// In Thailand 98, the GetTextExtentPoint function malfunctions, so it is calculated separately.
	if( m_bWindows98 )
	{
...
BOOL CD3DFontX::ConvWideAndTextExtent( const TCHAR* strText, INT iText, WCHAR* wstrText, INT &iWText, SIZE &Size, BOOL bConv )
{
	...
	if( iWText > 0 )
	{
		...
		{
			// [shhan][2015.02.16] An error occurred, but running ConvWideAndTextExtent98 seems to have a problem.
