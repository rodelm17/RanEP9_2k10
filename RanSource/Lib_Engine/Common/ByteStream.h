...
inline BOOL CByteStream::operator << ( D3DXVECTOR2 Value )
{
	m_Buffer.insert ( m_Buffer.end(), LPBYTE(&Value), LPBYTE(&Value) + sizeof(Value) );
	return TRUE;
}

inline BOOL CByteStream::operator << ( D3DXVECTOR3 Value )
{
	m_Buffer.insert ( m_Buffer.end(), LPBYTE(&Value), LPBYTE(&Value) + sizeof(Value) );
	return TRUE;
}

inline BOOL CByteStream::operator << ( D3DXVECTOR4 Value )
{
	m_Buffer.insert ( m_Buffer.end(), LPBYTE(&Value), LPBYTE(&Value) + sizeof(Value) );
	return TRUE;
}

inline BOOL CByteStream::operator << ( D3DXMATRIX Value )
{
	m_Buffer.insert ( m_Buffer.end(), LPBYTE(&Value), LPBYTE(&Value) + sizeof(Value) );
	return TRUE;
}
