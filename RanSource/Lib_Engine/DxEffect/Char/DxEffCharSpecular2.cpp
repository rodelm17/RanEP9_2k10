...
DWORD		DxEffCharSpecular2::VERSION			= 0x0200;
char		DxEffCharSpecular2::NAME[MAX_PATH]	= "DxEffCharSpecular2";
...
HRESULT DxEffCharSpecular2::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	TextureManager::LoadCubeTexture( m_szCubeTexture, pd3dDevice, m_pCubeTex, 0, 0 );
...
HRESULT DxEffCharSpecular2::DeleteDeviceObjects ()
{
	...
	TextureManager::ReleaseTexture ( m_szCubeTexture, m_pCubeTex );
...
void DxEffCharSpecular2::Render_Cube( LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bPieceRender )
{
	...
	if ( m_bCubeTexture && m_pCubeTex )
	{
		pd3dDevice->SetTexture( 1, m_pCubeTex );
	}else{
...
HRESULT	DxEffCharSpecular2::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if ( dwVer == 0x0106 )
	{
		//	Note : 이팩트의 Property 를 저장.
		//
		EFFCHAR_PROPERTY_SPECULAR2_106 sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_SPECULAR2_106) );

		m_Property.m_dwFlag			= sProp.m_dwFlag;
		m_Property.m_cSpecular		= sProp.m_cSpecular;
		m_Property.m_dwMaterials	= sProp.m_dwMaterials;
		m_Property.m_dwColorOP		= sProp.m_dwColorOP;
		m_Property.m_bCubeTexture	= FALSE;
	...
	else if ( dwVer == 0x0105 )
	{
		//	Note : 이팩트의 Property 를 저장.
		//
		EFFCHAR_PROPERTY_SPECULAR2_104_105 sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_SPECULAR2_104_105) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharSpecular2::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharSpecular2::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharSpecular2::R2LoadFile", SFile.GetFileName(), dwVer );
