	...
	m_bTracingUser(FALSE)
{
	...
	for( int i=0;i<13; ++i)
	{
		m_cInvenItemShop[i].SetState ( 1, 1000 );
	}
...
void SCHARDATA2::Assign ( SCHARDATA2 &sDATA )
{
	...
	m_mapCodexProg = sDATA.m_mapCodexProg;
	m_mapCodexDone = sDATA.m_mapCodexDone;

	/* personal lock system, Juver, 2019/12/06 */
	for ( int i=0; i<EMPERSONAL_LOCK_SIZE; ++i )
		m_sPersonalLock[i]	= sDATA.m_sPersonalLock[i];

	/* pk combo, Juver, 2021/02/09 */
	m_sPKComboCount			= sDATA.m_sPKComboCount;

	m_sCombatRecord			= sDATA.m_sCombatRecord;
	/*12-9-14, Battle Pass - CNDev*/
	m_mapBattlePassProg = sDATA.m_mapBattlePassProg;
	m_mapBattlePassDone = sDATA.m_mapBattlePassDone;
	m_mapBattlePassReward = sDATA.m_mapBattlePassReward;
	
	/*12-16-24, daily login - CNDev*/
	m_bDailyLogin = sDATA.m_bDailyLogin;
	m_mapDailyLogin = sDATA.m_mapDailyLogin;

	/*dmk14 send notif less data*/
	m_bRecMsgNotif = sDATA.m_bRecMsgNotif;
...
BOOL SCHARDATA2::SETEXPSKILLS_BYBUF ( CByteStream &ByteStream )
{
	...
	for ( DWORD i=0; i<dwNum; i++ )
	{
		if ( dwVersion==0x0100 )
		{
			SCHARSKILL_100 sCharSkillOld;
			GASSERT(dwSize==sizeof(sCharSkillOld));
			ByteStream.ReadBuffer ( (LPBYTE)&sCharSkillOld, sizeof(sCharSkillOld) );

			sCharSkill.Assign ( sCharSkillOld );
			m_ExpSkills.insert ( std::make_pair(sCharSkill.sNativeID.dwID,sCharSkill) );
		}
		else if( dwVersion==SCHARSKILL::VERSION )
		{
			GASSERT(dwSize==sizeof(SCHARSKILL));
		...
		{
			CDebugSet::ErrorVersion( "SETEXPSKILLS_BYBUF", dwVersion );
		}
	}
...
BOOL SCHARDATA2::GETEXPSKILLS_BYBUF ( CByteStream &ByteStream ) const
{
	...
	ByteStream << (DWORD)sizeof(SCHARSKILL);
...
void SCHARSKILL::Assign( const SCHARSKILL_100 &sOld )
{
...
/* Add ItemShop, Ssodomain, 11-09-2023, Start */
BOOL SCHARDATA2::ADDITEMSHOP ( const char* szPurKey, SNATIVEID nidITEM , const WORD wPrice , const WORD wStock , const WORD wItemCtg , const WORD wCurrency  )
{
	SITEMCUSTOM sCUSTOM(nidITEM);
	...
	sTemp.sID = nidITEM;
	sTemp.wItemStock  = wStock;
	sTemp.wItemPrice  = wPrice;
	sTemp.wItemCtg  = wItemCtg;
	sTemp.wItemCurrency = wCurrency;
	m_mapItemShop.insert ( std::make_pair(sTemp.strItemNum,sTemp) );
