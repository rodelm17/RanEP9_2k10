...
void CPVPCaptureTheFlagPage::CreateSubControl ()
{
	...
	m_pTextMapName->AddText( ID2GAMEWORD( "PVP_COMPETITION_BUTTON", 2 ), NS_UITEXTCOLOR::ORANGE );
	...
	m_pRewardButton->CreateSub( this, "COMPETITION_CAPTURE_THE_FLAG_PAGE_REWARD_INFO_BUTTON", UI_FLAG_DEFAULT, CAPTURE_THE_FLAG_PAGE_INFO_REWARD_BUTTON );
	m_pRewardButton->CreateFlip( "COMPETITION_CAPTURE_THE_FLAG_PAGE_REWARD_INFO_BUTTON_FLIP", CBasicButton::MOUSEIN_FLIP );
...
void CPVPCaptureTheFlagPage::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	switch( ControlID )
	{
	case CAPTURE_THE_FLAG_PAGE_INFO_REWARD_BUTTON:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE(dwMsg) )
			{
				/*pvp reward info, Juver, 2018/07/25 */
				CInnerInterface::GetInstance().PVPCaptureTheFlagShowRewardInfo();
			}
		}break;
	};
}
...
void CPVPCaptureTheFlagPage::UpdateInfo()
{
	...
	if ( m_pTextInfoDetail )
	{
		...
		strCombine.Format ( ID2GAMEWORD( "PVP_CAPTURE_THE_FLAG_INFO_TEXT", 10 ), GLPVPCaptureTheFlagClient::GetInstance().m_dwContributionReq );
		m_pTextInfoDetail->AddText( strCombine.GetString(), NS_UITEXTCOLOR::WHITE );
	...
	if ( m_pTextInfoReward )
	{
		...
		if ( fContriMultiplier > 1.0f )
		{
			...
			m_pTextInfoReward->AddString( nIndex, ",", NS_UITEXTCOLOR::GREENYELLOW ); 
			m_pTextInfoReward->AddString( nIndex, ID2GAMEWORD( "PVP_CAPTURE_THE_FLAG_REWARD_TEXT", 2 ), NS_UITEXTCOLOR::GREENYELLOW );
			strCombine.Format( "(%g x)", fContriMultiplier );
			m_pTextInfoReward->AddString( nIndex, strCombine.GetString(), NS_UITEXTCOLOR::ORANGE ); 
			...
			m_pTextInfoReward->AddString( nIndex, ",", NS_UITEXTCOLOR::RED ); 
			m_pTextInfoReward->AddString( nIndex, ID2GAMEWORD( "PVP_CAPTURE_THE_FLAG_REWARD_TEXT", 2 ), NS_UITEXTCOLOR::RED );
			strCombine.Format( "(%g x)", fContriMultiplier );
			m_pTextInfoReward->AddString( nIndex, strCombine.GetString(), NS_UITEXTCOLOR::ORANGE ); 
		}
		else
...
void CPVPCaptureTheFlagPage::SetCompetitionJoin( bool bEnable )
{
	if ( m_pInfoBattle )
