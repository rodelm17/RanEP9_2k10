...
void CPVPTyrannyTowerCapture::CreateSubControl ()
{
	...
	std::string strclubstatus[TYRANNY_TOWER_SIZE] = 
	{
		"TYRANNY_TOWER_CLUB_IMAGE_01",
		"TYRANNY_TOWER_CLUB_IMAGE_02",
		"TYRANNY_TOWER_CLUB_IMAGE_03",
	};

	for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )
	{
		m_pClubStatusTower[i] = new CUIControl;
		m_pClubStatusTower[i]->CreateSub ( this, strclubstatus[i].c_str(), UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pClubStatusTower[i]->SetUseRender(TRUE);
		m_pClubStatusTower[i]->SetVisibleSingle( FALSE );
		RegisterControl ( m_pClubStatusTower[i] );
	}
}
...
HRESULT CPVPTyrannyTowerCapture::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT( pd3dDevice );
	...
	for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )		m_pClubStatusTower[i]->SetVisibleSingle ( FALSE );
	...
	for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )
	{
		int nID = static_cast<int>( GLPVPTyrannyClient::GetInstance().m_dwGuiID[i] );
		if( nID == CLUB_NULL )	continue;

		int nServer = static_cast<int>(GLGaeaClient::GetInstance().GetCharacter()->m_dwServerID);
		DWORD dwVer = GLPVPTyrannyClient::GetInstance().m_dwMarkVer[i];

		DxClubMan::DXDATA& sMarkData = DxClubMan::GetInstance().GetClubData ( pd3dDevice, nServer, nID, dwVer );

		m_pClubStatusTower[i]->SetTexturePos ( 0, sMarkData.vTex_1_LU );
		m_pClubStatusTower[i]->SetTexturePos ( 1, sMarkData.vTex_2_RU );
		m_pClubStatusTower[i]->SetTexturePos ( 3, sMarkData.vTex_3_LD );
		m_pClubStatusTower[i]->SetTexturePos ( 2, sMarkData.vTex_4_RD );

		m_pClubStatusTower[i]->SetVisibleSingle ( TRUE );
		m_pClubStatusTower[i]->SetTexture ( sMarkData.pddsTexture );
		{
			hr = m_pClubStatusTower[i]->Render ( pd3dDevice );
			if( FAILED ( hr ) ) return hr;
		}
	}
