...
void	NavigationMesh::Save ( CSerialFile &SFile )
{	
	SFile << m_nNaviVertex;
	if ( m_nNaviVertex )
	{
...
void	NavigationMesh::Load_100 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_nNaviVertex;
	if ( m_nNaviVertex )
	{
...
BOOL	NavigationMesh::Load_File ( const TCHAR* path_name, LPDIRECT3DDEVICEQ pd3dDevice/*=NULL*/ )
{
	...
	CSerialFile SFile;
	if ( !SFile.OpenFile ( FOT_READ, path_name ) )	return FALSE;
	...
	if ( ver == 0x0100 )
	{
		Load_100( SFile, pd3dDevice );	
	}
	else
	{
		CDebugSet::ErrorVersion( "NavigationMesh::Load_File", SFile.GetFileName(), ver );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet( dwCur+size );
	}

	return TRUE;
}

BOOL	NavigationMesh::Save_File ( const TCHAR* path_name )
{
	if ( path_name == NULL )	return FALSE;

	CSerialFile SFile;
	if ( !SFile.OpenFile ( FOT_WRITE, path_name ) )	return FALSE;
