...
GLLevelFile::GLLevelFile(void):
	...
	m_bTradeRestricted(false),

	m_bAllowHideDead(false),

	m_bShowPkMark(false),

	m_bUseAutoPotion(false),
	m_bAutoPotionManual(false),

	m_bUseAutoPilot(false),

	m_bPKMapCount(false),

	m_bUseCaptcha(false),

	m_bBlockGeneralChat(false),

	m_bResetSkillDelayOnDie(false),
...
//	m_bPKZone			= rvalue.m_bPKZone;
	...
	m_bCPResetDeath		= rvalue.m_bCPResetDeath;
	...
	/*map party setting, Juver, 2018/06/29 */
	m_bBlockParty			= rvalue.m_bBlockParty;

	/*force revive, Juver, 2018/07/09 */
	m_bForceRevive			= rvalue.m_bForceRevive;

	/* open pk function, Juver, 2019/12/23 */
	m_bOpenPK				= rvalue.m_bOpenPK;

	/* death hide, Juver, 2020/02/02 */
	m_bDeathHide			= rvalue.m_bDeathHide;

	/* map entry user verified, Juver, 2020/02/27 */
	m_bUserVerifiedMapEntry = rvalue.m_bUserVerifiedMapEntry;

	/* costume combine map restriction, Juver, 2020/09/11 */
	m_bCostumeCombineRestrictMoveSpeed		= rvalue.m_bCostumeCombineRestrictMoveSpeed;
	m_bCostumeCombineRestrictAttackSpeed	= rvalue.m_bCostumeCombineRestrictAttackSpeed;

	m_bAllowHideDead		= rvalue.m_bAllowHideDead;

	m_bShowPkMark			= rvalue.m_bShowPkMark;

	m_bUseAutoPotion		= rvalue.m_bUseAutoPotion;
	m_bAutoPotionManual		= rvalue.m_bAutoPotionManual;

	m_bUseAutoPilot			= rvalue.m_bUseAutoPilot;

	m_bPKMapCount			= rvalue.m_bPKMapCount;

	m_bUseCaptcha			= rvalue.m_bUseCaptcha;

	m_bBlockGeneralChat		= rvalue.m_bBlockGeneralChat;

	m_bResetSkillDelayOnDie = rvalue.m_bResetSkillDelayOnDie;
	...
	m_MobSchManClient	= rvalue.m_MobSchManClient;
...
void GLLevelFile::SetNaviFileName ( const char* szFile )
{
	m_sLevelHead.m_strNaviFile = szFile;
}

const char* GLLevelFile::GetNaviFileName () const
{
	return m_sLevelHead.m_strNaviFile.c_str();
}
...
void GLLevelFile::SetObjRotate90()
{
	...
	/*dmk14 large map window ep9*/
	m_MobSchManClient.SetObjRotate90();
}
/*
	D3DXMATRIX matRotate;
	D3DXMatrixRotationY( &matRotate, D3DX_PI*0.5f );
