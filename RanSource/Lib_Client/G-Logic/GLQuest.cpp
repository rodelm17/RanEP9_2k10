...
GLQUEST& GLQUEST::operator= ( const GLQUEST &rVALUE )
{
	...
	m_dwGiftContributionPoints	= rVALUE.m_dwGiftContributionPoints;
	...
	m_AttendanceOption		= rVALUE.m_AttendanceOption;
...
bool GLQUEST::LOAD_0020 ( basestream &SFile )
{
...
bool GLQUEST::LOAD_0021 ( basestream &SFile )
{
	typedef std::vector<SITEMCUSTOM_QUEST_117>	INVENQARRAY_OLD;
	typedef INVENQARRAY_OLD::iterator		INVENQARRAY_OLD_ITER;
...
bool GLQUEST::LOAD_0022 ( basestream &SFile )
{
	typedef std::vector<SITEMCUSTOM_QUEST_117>	INVENQARRAY_OLD;
	typedef INVENQARRAY_OLD::iterator		INVENQARRAY_OLD_ITER;
...
{
	typedef std::vector<SITEMCUSTOM_200>	INVENQARRAY_OLD;
	typedef INVENQARRAY_OLD::iterator		INVENQARRAY_OLD_ITER;
...
bool GLQUEST::LOAD_0203 ( basestream &SFile )
{
	typedef std::vector<SITEMCUSTOM_200>	INVENQARRAY_OLD;
	typedef INVENQARRAY_OLD::iterator		INVENQARRAY_OLD_ITER;
...
bool GLQUEST::LOAD_0204 ( basestream &SFile )
{
	typedef std::vector<SITEMCUSTOM_201>	INVENQARRAY_OLD;
	typedef INVENQARRAY_OLD::iterator		INVENQARRAY_OLD_ITER;
...
bool GLQUEST::LOAD_0205 ( basestream &SFile )
{
	DWORD dwSize(0);
	DWORD dwData(0);
...
bool GLQUEST::LOAD_0206 ( basestream &SFile )
{
	DWORD dwSize(0);
	DWORD dwData(0);
	...
	SFile >> m_dwGiftMONEY;
	SFile >> m_dwGiftContributionPoints;
	SFile.ReadBuffer( &m_llGiftEXP, sizeof( m_llGiftEXP ));
	SFile >> m_dwGiftSKILLPOINT;
	SFile >> m_dwGiftSTATSPOINT;
	SFile >> m_dwGiftLIFEPOINT;
...
bool GLQUEST::LOAD ( const char *szFile )
{
	...
	case 0x0203:	LOAD_0203(SFile);	break;
	case 0x0204:	LOAD_0204(SFile);	break;
	case 0x0205:	LOAD_0205(SFile);	break;
	case VERSION:	LOAD_0206(SFile);	break;
...
void GLQUEST::LoadCsvBaseData( CStringArray &StrArray )
{
	...
	m_dwGiftContributionPoints = (DWORD)atoi( StrArray[15] );
	m_dwGiftSKILLPOINT = (DWORD)atoi( StrArray[16] );
	m_dwGiftELEMENT = (DWORD)atoi( StrArray[17] );
	m_dwGiftSTATSPOINT = (DWORD)atoi( StrArray[18] );
	m_dwGiftLIFEPOINT = (DWORD)atoi( StrArray[19] );
	m_dwGiftQUEST = (DWORD)atoi( StrArray[20] );
	...
	{
		if( StrArray[21+(i*10)] == " " ) break;

		sItemCustom.sNativeID.wMainID = (WORD)atoi( StrArray[21+(i*10) ] );
		sItemCustom.sNativeID.wSubID = (WORD)atoi( StrArray[22+(i*10) ] );
		sItemCustom.cDAMAGE = (BYTE) atoi( StrArray[23+(i*10) ] );
		sItemCustom.cDEFENSE = (BYTE)atoi( StrArray[24+(i*10) ] );
		sItemCustom.wTurnNum = (WORD)atoi( StrArray[25+(i*10) ] );
		sItemCustom.cRESIST_ELEC = (BYTE)atoi( StrArray[26+(i*10) ] );
		sItemCustom.cRESIST_FIRE = (BYTE)atoi( StrArray[27+(i*10) ] );
		sItemCustom.cRESIST_ICE = (BYTE)atoi( StrArray[28+(i*10) ] );
		sItemCustom.cRESIST_POISON = (BYTE)atoi( StrArray[29+(i*10) ] );
		sItemCustom.cRESIST_SPIRIT = (BYTE)atoi( StrArray[30+(i*10) ] );
	...
	{
		if (StrArray[121+(i*10)] == " " ) break;
		
		sNativeID.wMainID = (WORD)atoi( StrArray[121+(i*10) ] );
		sNativeID.wSubID = (WORD)atoi( StrArray[122+(i*10) ] );
...
bool GLQUEST::SAVE ( const char *szFile )
{
	...
	CSerialFile SFile;
	BOOL bOPEN = SFile.OpenFile ( FOT_WRITE, strLIST_FILE.c_str(), EMRCRYPT_BYTE_TYPE_OLD );
	if ( !bOPEN )					
		return false;
...
void GLQUEST::SaveCsvBaseHead( std::fstream &SFile )
{
	...
	SFile << "CompensationContributionPoint" << ",";
...
BOOL GLQUESTPROG::GET_BYBUFFER ( CByteStream &ByteStream )
{
	...
	case VERSION:	LOAD_200( ByteStream ); break;
	default:	break;
	}
...
BOOL GLQUESTPROG::LOAD_200 ( CByteStream &ByteStream )
{
	ByteStream >> m_sNID.dwID;
	ByteStream >> m_dwEDITVER;
...
bool GLQUESTPROG::DoPROGRESS ( bool &_bModifyInven, GLQUEST_PROGRESS_MAP &stepMoveMap )
{
	...
	if ( m_sSTEP_PROG.IsCOMPLETE() )
	{
		...
		if( pSTEP->m_stepMoveMap.nidMAP.wMainID  != USHRT_MAX &&
			pSTEP->m_stepMoveMap.nidMAP.wSubID   != USHRT_MAX &&
			(pSTEP->m_stepMoveMap.wPosX != USHRT_MAX &&
			 pSTEP->m_stepMoveMap.wPosY != USHRT_MAX ||
