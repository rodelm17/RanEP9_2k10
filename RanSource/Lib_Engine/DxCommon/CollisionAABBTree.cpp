...
namespace COLLISION
{
	...
	void FrameMove( const float fTime, const float fElapsedTime, OBJAABBNode* pNode )
	{
		if ( !pNode )	return;

		if ( pNode->pObject )	pNode->pObject->FrameMoveAABB( fTime, fElapsedTime );

		FrameMove( fTime, fElapsedTime, pNode->pLeftChild );
		FrameMove( fTime, fElapsedTime, pNode->pRightChild );
	}
