...
BOOL DxLandMan::SaveFile ( const char *szFile )
{
	...
	if ( hr = SFile.OpenFile ( FOT_WRITE, szFile ) )
	{
		...
		{
			m_pStaticMaterialMesh->Save_File( szFile );		// 자체 저장
		}
		...
		if ( m_pshadowmesh )	
		{
			m_pshadowmesh->Save_File( szFile );		// 자체 저장
		}
...
BOOL DxLandMan::LoadFile ( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad )
{
	...
	if ( hr = m_SFileMap.OpenFile ( FOT_READ, szPathName ) )
	{
		...
		if ( dwVersion >= VERSION_ENCODE )
			m_SFileMap.SetEncodeType( EMRCRYPT_BYTE_TYPE_WLD_1_VER_1 );

		if ( dwVersion >= VERSION_ENCODE_2 )
			m_SFileMap.SetEncodeType( EMRCRYPT_BYTE_TYPE_WLD_1_VER_2 );

		if ( dwVersion >= VERSION_ENCODE_3 )
			m_SFileMap.SetEncodeType( EMRCRYPT_BYTE_TYPE_WLD_1_VER_3 );

		if ( dwVersion >= VERSION_ENCODE_4 )
			m_SFileMap.SetEncodeType( EMRCRYPT_BYTE_TYPE_WLD_1_VER_4 );

		if ( dwVersion >= DxLandMan::ENCODE_VER_BYTE_CRYPT_2021_WLD1_V1 )
			m_SFileMap.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_WLD1_V1 );

		if ( dwVersion == VERSION_WLD )
		{
			return LoadFile_VER206 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0205 )
		{
			return LoadFile_VER205 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0204 )
		{
			return LoadFile_VER204 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0203 )
		{
			return LoadFile_VER203 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0202 )
		{
			return LoadFile_VER202 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0201 )
		{
			return LoadFile_VER201 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
...
#ifdef _DEBUG
		else if ( dwVersion == 0x0123 && g_bWORLD_TOOL )
		{
			return LoadFile_VER123 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0122 && g_bWORLD_TOOL )
		{
			return LoadFile_VER122 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
		else if ( dwVersion == 0x0121 && g_bWORLD_TOOL )
		{
			return LoadFile_VER121 ( szFile, pd3dDevice, bUseDynamicLoad, szPathName );
		}
#endif
		...
		else if ( dwVersion == 0x0111 || dwVersion == 0x0112 )
		{
...
BOOL DxLandMan::LoadFile_VER204( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
	...
	if ( bExist )
	{
		...
		m_pNaviMesh->Load_100 ( m_SFileMap, pd3dDevice );
	}
	...
	if ( bExist )
	{
		m_pStaticMesh = new DxMultiStaticMesh;
		m_pStaticMesh->Load_File_120( pd3dDevice, szPathName );
		m_pStaticMesh->StartThread( pd3dDevice );

		//wld1
		m_pTexEffect = new DxTexEffect;
		m_pTexEffect->Load_File( pd3dDevice, szPathName );
	}
	else return TRUE;
	...
	if ( bExist )
	{
		m_pStaticMaterialMesh = new DxMultiStaticMaterialMesh;
		m_pStaticMaterialMesh->Load_File( pd3dDevice, szPathName );
		m_pStaticMaterialMesh->StartThread( pd3dDevice );
	}
	else return TRUE;
	...
	if ( bExist )
	{
		m_pshadowmesh = new DxShadowMesh;
		m_pshadowmesh->Load_File( pd3dDevice, szPathName );
	}
	else return TRUE;
	...
	if ( bExist )
	{
		m_PieceManager.LoadWLD_STABLE( pd3dDevice, m_SFileMap );
	}
	else return TRUE;
...
BOOL DxLandMan::LoadFile_VER206( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
...
BOOL DxLandMan::LoadFile_VER205( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
...
BOOL DxLandMan::LoadFile_VER203( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
...
BOOL DxLandMan::LoadFile_VER202( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
...
BOOL DxLandMan::LoadFile_VER201( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
	...
	m_sbasic.m_blegacy_shadow = false;
	m_sbasic.m_fVS_1_1_ColorMulti = 0.75f;
	m_sbasic.set_shadow_type();
...
BOOL DxLandMan::LoadFile_VER123( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
	...
	{
		m_pAniManHead = new DxAnimationMan;
		m_pAniManHead->LoadFile_BETA ( m_SFileMap, pd3dDevice );
	}
...
BOOL DxLandMan::LoadFile_VER122( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	BOOL	hr = FALSE;
	BOOL	bExist = FALSE;
	...
	m_sbasic.m_blegacy_shadow = false;
	m_sbasic.m_fVS_1_1_ColorMulti = 0.50f;
	m_sbasic.set_shadow_type();
...
BOOL DxLandMan::LoadFile_VER120( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	...
	m_sbasic.m_blegacy_shadow = true;
	m_sbasic.m_fVS_1_1_ColorMulti = 0.50f;
	m_sbasic.set_shadow_type();
...
BOOL DxLandMan::LoadFile_VER119( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	...
	{
		m_pStaticMesh = new DxMultiStaticMesh;
		m_pStaticMesh->Load_File_109( pd3dDevice, szPathName );
		m_pStaticMesh->StartThread( pd3dDevice );
...
BOOL DxLandMan::LoadFile_VER116( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bUseDynamicLoad, const char* szPathName )
{
	...
	if ( bExist )
	{
		m_PieceManager.LoadWLD_OLD( pd3dDevice, m_SFileMap );
	}
	else return TRUE;
...
BOOL DxLandMan::ReplaceNagivationMesh(std::string strFilePath, LPDIRECT3DDEVICEQ pd3dDevice)
{
	SAFE_DELETE(m_pNaviMesh);

	m_pNaviMesh = new NavigationMesh;
	return m_pNaviMesh->Load_File(strFilePath.c_str(), pd3dDevice);
}
