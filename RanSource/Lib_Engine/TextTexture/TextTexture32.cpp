			...
			for( LONG y=pPart->m_rtTBound.top; y<pPart->m_rtTBound.bottom; ++y )
			{
                ...
                else
                {
                    ...
                    if( SUCCEEDED( m_pTexture->LockRect( 0, &rect, NULL, 0 ) ) )
                    {
                        memmove
                            (
                            &((DWORD*)rect.pBits)[ pPart->m_rtTBound.left + y * m_iTextureSize ],
                            &((DWORD*)rect.pBits)[ pPart->m_rtTBound.left + iDeleteWidth + y * m_iTextureSize ],
                            sizeof( DWORD ) * pPart->m_iWidth
                            );
...
VOID CTextTexture32::CalcTexData( CTextPart* pPart )
{
    ...
    if( m_bFailed == FALSE && m_pTexture && SUCCEEDED( m_pTexture->LockRect( 0, &rect, NULL, 0 ) ) )
    {
        ...
        if (FreetypeLib::CopyBitmap(FTWstr, pPart->m_pFont->GetFontHeight(), rect.pBits, 4, pPart->m_rtTBound, m_iTextureSize, dwFlags) == true)
        {
            m_pTexture->UnlockRect( 0 );
