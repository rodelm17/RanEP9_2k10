		...
		if ( bOnline )
		{
			...
			{	
				...
				else
				{
					...
					if( strMap.IsEmpty() && sClubMember.nidMAP.wSubID != 0 )
					{
						SNATIVEID sMapID = sClubMember.nidMAP;
						sMapID.wSubID = 0;

						SMAPNODE *pMapNode = GLGaeaClient::GetInstance().FindMapNode ( sMapID );
						if ( pMapNode && pMapNode->bInstantMap )
						{
							strMap = pMapNode->strMapName.c_str();
						}
					}
...
void CClubMember::LoadClubMember( const CString& strText, const bool bOnline, const DWORD& dwCharID )
{
	enum
	{
		ONLINE = 0,
	...
	m_pListText->SetTextData ( nIndex, dwCharID );
}
