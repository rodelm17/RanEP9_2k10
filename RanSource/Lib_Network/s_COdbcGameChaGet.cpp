...
//		strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
	...
	SQLCHAR    szChaGuName[CHAR_SZNAME+1] = {0}; SQLINTEGER cbChaGuName = SQL_NTS;// U?
	...
	SQLINTEGER  nChaContributionPoint, cbChaContributionPoint = SQL_NTS;
	...
	SQLINTEGER  nGachaPromisePoint, cbGachaPromisePoint = SQL_NTS;
...
//			strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
		...
		{		
			...
			::SQLGetData(pConn->hStmt, 4,  SQL_C_CHAR, szChaNameWChanum, CHR_ID_LENGTH+1, &cbChaNameWChanum); // Added column: ChaNameWChanum
			::SQLGetData(pConn->hStmt, 5,  SQL_C_CHAR, szChaNameWUserNum, CHR_ID_LENGTH+1, &cbChaNameWUserNum); // Added column: ChaNameWUserNum
			::SQLGetData(pConn->hStmt, 6,  SQL_C_LONG, &nChaTribe,      0, &cbChaTribe);
			::SQLGetData(pConn->hStmt, 7,  SQL_C_LONG, &nChaClass,      0, &cbChaClass);

			::SQLGetData(pConn->hStmt, 8,  SQL_C_LONG, &nChaBright,     0, &cbChaBright);
			::SQLGetData(pConn->hStmt, 9,  SQL_C_LONG, &nChaLevel,      0, &cbChaLevel);
			::SQLGetData(pConn->hStmt,10,  SQL_C_LONG, &nChaDex,        0, &cbChaDex);
			::SQLGetData(pConn->hStmt,11,  SQL_C_LONG, &nChaIntel,      0, &cbChaIntel);
			::SQLGetData(pConn->hStmt,12,  SQL_C_LONG, &nChaPower,      0, &cbChaPower);
			
			::SQLGetData(pConn->hStmt,13,  SQL_C_LONG, &nChaStrong,     0, &cbChaStrong);
			::SQLGetData(pConn->hStmt,14,  SQL_C_LONG, &nChaSpirit,     0, &cbChaSpirit);
			::SQLGetData(pConn->hStmt,15,  SQL_C_LONG, &nChaStrength,   0, &cbChaStrength);
			::SQLGetData(pConn->hStmt,16,  SQL_C_LONG, &nChaStRemain,   0, &cbChaStRemain);
			::SQLGetData(pConn->hStmt,17,  SQL_C_LONG, &nChaAttackP,    0, &cbChaAttackP);		
			
			::SQLGetData(pConn->hStmt,18,  SQL_C_LONG, &nChaDefenseP,   0, &cbChaDefenseP);
			::SQLGetData(pConn->hStmt,19,  SQL_C_LONG, &nChaFightA,     0, &cbChaFightA);
			::SQLGetData(pConn->hStmt,20,  SQL_C_LONG, &nChaShootA,     0, &cbChaShootA);
			::SQLGetData(pConn->hStmt,21,  SQL_C_LONG, &nChaSkillPoint, 0, &cbChaSkillPoint);
			::SQLGetData(pConn->hStmt,22,  SQL_C_LONG, &nChaHP,			0, &cbChaHP);
			
			::SQLGetData(pConn->hStmt,23,  SQL_C_LONG, &nChaMP,			0, &cbChaMP);
			::SQLGetData(pConn->hStmt,24,  SQL_C_LONG, &nChaSP,			0, &cbChaSP);
			::SQLGetData(pConn->hStmt,25,  SQL_C_LONG, &nChaPK,			0, &cbChaPK);
			::SQLGetData(pConn->hStmt,26,  SQL_C_LONG, &nChaStartMap,	0, &cbChaStartMap);
			::SQLGetData(pConn->hStmt,27,  SQL_C_LONG, &nChaStartGate,  0, &cbChaStartGate);
						
			::SQLGetData(pConn->hStmt,28,  SQL_C_DOUBLE, &fChaPosX,		0, &cbChaPosX);
			::SQLGetData(pConn->hStmt,29,  SQL_C_DOUBLE, &fChaPosY,		0, &cbChaPosY);
			::SQLGetData(pConn->hStmt,30,  SQL_C_DOUBLE, &fChaPosZ,		0, &cbChaPosZ);			
			::SQLGetData(pConn->hStmt,31,  SQL_C_SBIGINT, &llChaMoney,  0, &cbChaMoney);
			::SQLGetData(pConn->hStmt,32,  SQL_C_SBIGINT, &llChaExp,    0, &cbChaExp);
						
			::SQLGetData(pConn->hStmt,33,  SQL_C_LONG, &nChaSaveMap,    0, &cbChaSaveMap);
			::SQLGetData(pConn->hStmt,34,  SQL_C_DOUBLE, &fChaSavePosX,   0, &cbChaSavePosX);
			::SQLGetData(pConn->hStmt,35,  SQL_C_DOUBLE, &fChaSavePosY,   0, &cbChaSavePosY);
			::SQLGetData(pConn->hStmt,36,  SQL_C_DOUBLE, &fChaSavePosZ,   0, &cbChaSavePosZ);
			
			::SQLGetData(pConn->hStmt,37,  SQL_C_LONG, &nChaSchool,		0, &cbChaSchool);
			::SQLGetData(pConn->hStmt,38,  SQL_C_LONG, &nChaHair,		0, &cbChaHair);
			::SQLGetData(pConn->hStmt,39,  SQL_C_LONG, &nChaFace,		0, &cbChaFace);
			::SQLGetData(pConn->hStmt,40,  SQL_C_LONG, &nChaLiving,		0, &cbChaLiving);
			
			::SQLGetData(pConn->hStmt,41,  SQL_C_LONG, &nChaInvenLine, 0, &cbChaInvenLine);
			            
			::SQLGetData(pConn->hStmt,42,  SQL_C_LONG,   &nChaReturnMap,  0, &cbChaReturnMap);
			::SQLGetData(pConn->hStmt,43,  SQL_C_DOUBLE, &fChaReturnPosX, 0, &cbChaReturnPosX);
			::SQLGetData(pConn->hStmt,44,  SQL_C_DOUBLE, &fChaReturnPosY, 0, &cbChaReturnPosY);
			::SQLGetData(pConn->hStmt,45,  SQL_C_DOUBLE, &fChaReturnPosZ, 0, &cbChaReturnPosZ);

			::SQLGetData(pConn->hStmt,46,  SQL_C_LONG, &nGuNum, 0, &cbGuNum);
			::SQLGetData(pConn->hStmt,47,  SQL_C_CHAR, szChaGuName, CHAR_SZNAME, &cbChaGuName); 

			// 2005-12-29 Jgkim:Add nChaSex, nChaHairColor
			::SQLGetData(pConn->hStmt,48,  SQL_SMALLINT, &nChaSex,       0, &cbChaSex);
			::SQLGetData(pConn->hStmt,49,  SQL_C_LONG,   &nChaHairColor, 0, &cbChaHairColor);

			// Add to ChaReExp
			::SQLGetData(pConn->hStmt,50,  SQL_C_SBIGINT, &llChaReExp, 0, &cbChaReExp);

			// Add to SpMID, SpSID
			::SQLGetData(pConn->hStmt,51,  SQL_C_LONG, &nChaSpMID, 0, &cbChaSpMID);
			::SQLGetData(pConn->hStmt,52,  SQL_C_LONG, &nChaSpSID, 0, &cbChaSpSID);	

			::SQLGetData(pConn->hStmt,53,  SQL_C_DOUBLE, &fChaScaleRange, 0, &cbChaScaleRange);

			/*combatpoint logic, Juver, 2017/05/27 */
			::SQLGetData(pConn->hStmt,54,  SQL_C_LONG, &nChaCP,			0, &cbChaCP);

			/*contribution point, Juver, 2017/08/23 */
			::SQLGetData(pConn->hStmt,55,  SQL_C_LONG, &nChaContributionPoint, 0, &cbChaContributionPoint);	

			/*activity point, Juver, 2017/08/23 */
			::SQLGetData(pConn->hStmt,56,  SQL_C_LONG, &nChaActivityPoint, 0, &cbChaActivityPoint);	

			/*activity system, Juver, 2017/11/04 */
			::SQLGetData(pConn->hStmt,57,  SQL_C_CHAR, szChaBadge, CHAR_SZNAME, &cbChaBadge); 
			
			/* Gacha System, MontageDev 7/10/24 */
			::SQLGetData(pConn->hStmt,58,  SQL_C_LONG, &nGachaPromisePoint, 0, &cbGachaPromisePoint );
			
            pChaData2->m_dwCharID			= (DWORD) nChaNum; // ???????
			pChaData2->SetUserID( (DWORD) nUserNum ); // ???????
            pChaData2->m_dwGuild            = (DWORD) nGuNum; // U?????
           ...
           // Select character name based on nNameType parameter
            ...
            if (nNameType == 1 && cbChaNameWChanum != 0 && cbChaNameWChanum != -1) {
                ::StringCchCopy(pChaData2->m_szName, CHR_ID_LENGTH, (const char*) szChaNameWChanum);
            }
            else if (nNameType == 2 && cbChaNameWUserNum != 0 && cbChaNameWUserNum != -1) {
                ::StringCchCopy(pChaData2->m_szName, CHR_ID_LENGTH, (const char*) szChaNameWUserNum);
            }
            else if (cbChaName != 0 && cbChaName != -1) {
				::StringCchCopy(pChaData2->m_szName, CHR_ID_LENGTH, (const char*) szChaName);
            }

            if (cbChaGuName != 0 && cbChaGuName != -1) // U?????? ?????
			...
			/*contribution point, Juver, 2017/08/23 */
			pChaData2->m_dwContributionPoint = (DWORD)nChaContributionPoint;
			...
			/* Gacha System, MontageDev 7/10/24 */
			pChaData2->m_wGachaPromisePoint = (DWORD)nGachaPromisePoint;		
...
//	strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
	...
	sReturn = m_pGameDB->ReadImage("ChaInfo.ChaDailyLogin", nChaNum, ByteStream);
	if (sReturn == DB_ERROR)
		return DB_ERROR;
	else
		pChaData2->SETDAILYLOGIN_BYBUF(ByteStream);
		
	/*12-9-14, Battle Pass - CNDev*/
	sReturn = m_pGameDB->ReadImage("ChaInfo.ChaBattlePass", nChaNum, ByteStream);
	if (sReturn == DB_ERROR)
		return DB_ERROR;
	else
		pChaData2->SETBATTLEPASS_BYBUF(ByteStream);


	/* personal lock system, Juver, 2020/01/15 */
	PersonalLockRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->GetUserID(), pChaData2 );

	/* pk combo, Juver, 2021/02/11 */
	PKComboRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );

	LoadUserDBEtcCharData( pChaData2 );

	CombatRecordRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );
	GlobalRankingRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );
	DailyLoginRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );
	BattlePassRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );
	ChaGameRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );
	//PlatBuffRead( CCfg::GetInstance()->GetServerGroup(), pChaData2->m_dwCharID, pChaData2 );
							  ...
							  int nNameType)
{
	...
	TCHAR szTemp[512] = {0};
	_snprintf( szTemp, 512, "SELECT ChaName, "
							"(ChaName + '#' + CONVERT(varchar(10),ChaNum)) as ChaNameWChanum, "
							"(ChaName + '#' + CONVERT(varchar(10),UserNum)) as ChaNameWUserNum, "
							"ChaClass, ChaSchool, ChaDex, ChaIntel, ChaPower,"
	...
	while (true)
	{
		...
		{	
			...
			::SQLGetData(pConn->hStmt,  2, SQL_C_CHAR,    szChaNameWChanum, CHR_ID_LENGTH+1, &cbChaNameWChanum); // Added column: ChaNameWChanum
			::SQLGetData(pConn->hStmt,  3, SQL_C_CHAR,    szChaNameWUserNum, CHR_ID_LENGTH+1, &cbChaNameWUserNum); // Added column: ChaNameWUserNum
			...
			::SQLGetData(pConn->hStmt, 12, SQL_C_LONG,    &nChaLevel,    0, &cbChaLevel);
			::SQLGetData(pConn->hStmt, 13, SQL_C_LONG,    &nChaHair,	  0, &cbChaHair);

			::SQLGetData(pConn->hStmt, 14, SQL_C_LONG,    &nChaFace,     0, &cbChaFace);
			::SQLGetData(pConn->hStmt, 15, SQL_C_LONG,    &nChaBright,   0, &cbChaBright);

			// 2005-12-29 Jgkim:Add nChaSex, nChaHairColor
			::SQLGetData(pConn->hStmt, 16, SQL_SMALLINT,  &nChaSex,       0, &cbChaSex);
			::SQLGetData(pConn->hStmt, 17, SQL_C_LONG,    &nChaHairColor, 0, &cbChaHairColor);			

			::SQLGetData(pConn->hStmt, 18, SQL_C_SBIGINT, &llChaExp,    0, &cbChaExp);

			::SQLGetData(pConn->hStmt, 19, SQL_C_LONG,    &nChaSaveMap,    0, &cbChaSaveMap);
			::SQLGetData(pConn->hStmt, 20, SQL_C_LONG,    &nChaHP,			0, &cbChaHP);
			::SQLGetData(pConn->hStmt, 21, SQL_C_DOUBLE,  &fChaScaleRange, 0, &cbChaScaleRange);
			...
			// nNameType = 2: ChaNameWUserNum (ChaName + '#' + formatted UserNum)
			if (nNameType == 1 && cbChaNameWChanum != 0 && cbChaNameWChanum != -1) {
				::StringCchCopy(sci->m_szName, CHR_ID_LENGTH, (const char*) szChaNameWChanum);
			}
			else if (nNameType == 2 && cbChaNameWUserNum != 0 && cbChaNameWUserNum != -1) {
				::StringCchCopy(sci->m_szName, CHR_ID_LENGTH, (const char*) szChaNameWUserNum);
			}
			else if (cbChaName != 0 && cbChaName != -1) {
				::StringCchCopy(sci->m_szName, CHR_ID_LENGTH, (const char*) szChaName);
			}
