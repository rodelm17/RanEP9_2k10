...
HRESULT CUIMan::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	hr = RenderList ( &m_UpdateList, pd3dDevice );
	if ( FAILED ( hr ) )	goto _RETURN;
...
BOOL CUIMan::UpdateList ( CUIFocusContainer* pList, BOOL bFocusList, LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	...
	if( NSGUI::IsCharMoveAllBlock() && !bFocusList )	return TRUE;
	...
	for ( ; riter != riter_end; ++riter )
	{
		...
		if( !m_bExclusiveControl )
		{
			if( pControl->IsExclusiveControl() )
			{
				...
				m_DockingMan.SetDragControl( pControl );
			}
		}
			...
			if ( bFocusList ) // MEMO : 포커스 리스트라면...
			{
				if ( bFirstControl && ( CHECK_MOUSE_IN ( dwMsg ) || CHECK_KEYFOCUSED ( dwMsg ) ) )
				{
						...
						( dwMsg & UIMSG_RB_DOWN ) || ( CHECK_KEYFOCUSED( dwMsg ) ) )
					{
						...
						if( IsVisibleGroup( cID ) && IsGroupLock( cID ) )
						{
							if ( pControl != pList->GetTail() )
							{
...
HRESULT CUIMan::FrameMove ( LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	...
	if( !UpdateList( &m_UpdateList, TRUE, pd3dDevice, fTime, fElapsedTime ) )	return E_FAIL;

	if ( !m_bExclusiveControl )
	{
		m_DockingMan.ResetDragControl(); // 드래그 컨트롤 리셋
	}

	m_DockingMan.Update( fElapsedTime );
