...
extern BOOL	g_bFRAME_LIMIT;		// D3DPRESENT_INTERVAL_ONE
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	if (FAILED(m_pD3D->CreateDevice( AdapterToUse, DeviceType, m_hWndFocus,
		D3DCREATE_HARDWARE_VERTEXPROCESSING,
		&m_d3dpp, &m_pd3dDevice) ) )
	{
		AfxMessageBox( _T("CreateDevice NVIDIA_PERFHUD Error") );
		return E_FAIL;
	}
#else
...
HRESULT CD3DApplication::Render3DEnvironment()
{
	...
	//{
	//	// 다음 랜더링 될 시점을 구함 ( 60fps )
	//	fTimeDelta = fBeforeTime+0.01666f;
	...
	// Get the app's time, in seconds. Skip rendering if no time elapsed
	//FLOAT fAppTime        = static_cast< FLOAT >( fCurrentTime ); //DXUtil_Timer( TIMER_GETAPPTIME );
	FLOAT fAppTime        = DXUtil_Timer( TIMER_GETAPPTIME );
	FLOAT fElapsedAppTime = DXUtil_Timer( TIMER_GETELAPSEDTIME );
...
//-----------------------------------------------------------------------------
// Name : Present()
// DirectX 위에 설치된 GUI를 위해서 별도로 Present 관리
//-----------------------------------------------------------------------------
...
VOID CD3DApplication::Cleanup3DEnvironment()
{
	...
	if( m_pd3dDevice )
	{
		...
		// 원래는 Device 와 상관없는것들을 지워야 하는데, 이전까지 그런개념 없이 작업이 되어서 문제가 되어 이곳에서 삭제가 필요함.
		// 원래대로라면 m_pd3dDevice->Release(); 후에 되야하지만, 문제 발생으로 전에 작업됨.
		FinalCleanup();
