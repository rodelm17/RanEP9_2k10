...
CStringMemoryGSV4::CStringMemoryGSV4 ( UINT nBufSize ) 
	: m_dwOffSet(0)
	, m_nBufSize(0)
	, m_pBufferMem(NULL)
	, m_nBufSizeTemp(0)
...
CStringMemoryGSV4::~CStringMemoryGSV4 ()
{
	delete[] m_pBuffer;
	Close ();
}

BOOL CStringMemoryGSV4::Open (	LPCSTR _szZipFileName, 
							LPCSTR _szFileName, 
							BOOL bDecode, 
							BOOL bPack )
{
	GASSERT(m_pBufferMem==NULL&&"CStringMemoryGSV4::Open ()");
	...
	if( m_bDecode )
	{
		...
		if( CRijndaelGSV4::VERSION == nVersion )
		{
			m_nVersion = nVersion;
			m_oRijndael.Initialize( CRijndaelGSV4::sm_Version[nVersion-1].c_str(), CRijndaelGSV4::sm_KeyLength[nVersion-1], 
				CRijndaelGSV4::sm_chain0, CRijndaelGSV4::sm_KeyLength[nVersion-1] );
...
DWORD CStringMemoryGSV4::GetNextLine ( CString &strString )
{
...
DWORD CStringMemoryGSV4::GetNextLine ( PSTR &szString )
{
	if( m_pBufferMem == NULL ) return 0;
...
DWORD CStringMemoryGSV4::GetNextLine ( LPSTR szLine, UINT iLineSize )
{
	if( m_pBufferMem == NULL ) return 0;
	...
	*chTemp = 0;
	...
	while ( !bStop )
	{
		...
		for ( ; m_dwIndex<m_dwRead; m_dwIndex++ )
		{
			...
			{
				TCHAR szError[260] = {0};
				StringCchPrintf( szError, 260, "CStringMemoryGSV4::m_pBuffer Not enough. File:%s, Line:%u(Over Char Number:%d)",
