...
namespace DxGrapUtils
{
	...
	HRESULT CaptureScreen ( LPDIRECT3DDEVICEQ pDev )
	{
		...
		for ( int i=0; i<999; i++ )
		{
			if( RPARAM::emSERVICE_TYPE == EMSERVICE_CHINA )
			{
				StrPathName.Format ( "%s%s%s%03d.jpg", strCapturePath.c_str(), "직槿빻蹈", szTime, i );
			}
			else if ( RPARAM::emSERVICE_TYPE == EMSERVICE_FEYA )
			{
	...
	HRESULT CaptureScreen2 ( LPDIRECT3DDEVICEQ pDev, const char* szFileName )
	{
		HRESULT hr;
		...
		CString StrPathName;
		StrPathName.Format ( "%s%s.jpg", strCapturePath.c_str(), szFileName );
	...
	{ 
			...
			if ( FAILED(hr) )
			{
				CDebugSet::ToLogFile ( "ERROR : DxGrapUtils::ScreenGrab()-GetDisplayMode() %s", DXGetErrorString(hr) );
				return hr;
			}
			...
			if ( FAILED(hr) )
			{
				CDebugSet::ToLogFile ( "ERROR : DxGrapUtils::ScreenGrab()-CreateImageSurface() %s", DXGetErrorString(hr) );
				return hr;
			}
			...
			if ( FAILED(hr) )
			{ 
				CDebugSet::ToLogFile ( "ERROR : DxGrapUtils::ScreenGrab()-GetFrontBuffer() %s", DXGetErrorString(hr) );
				surf->Release();
				return hr; 
			} 
			...
			if ( FAILED(hr) )
			{ 
				CDebugSet::ToLogFile ( "ERROR : DxGrapUtils::ScreenGrab()-SaveFile() %s, %s", fileName, DXGetErrorString(hr) );
				surf->Release();
				return hr; 
			} 
