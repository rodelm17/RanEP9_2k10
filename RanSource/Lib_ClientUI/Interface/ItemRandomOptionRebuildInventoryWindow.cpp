...
#include "./ItemRandomOptionRebuildInventoryPage.h"
...
CItemRandomOptionRebuildInventoryWindow::CItemRandomOptionRebuildInventoryWindow()
	: m_pPage( NULL )
	, m_nONE_VIEW_SLOT( 4 )
...
void CItemRandomOptionRebuildInventoryWindow::CreateSubControl()
{
	m_pPage = new CItemRandomOptionRebuildInventoryPage;
	m_pPage->CreateSub( this, "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_PAGE", UI_FLAG_YSIZE, ITEM_RANDOM_OPTION_REBUILD_INVENTORY_PAGE );
	m_pPage->CreateSubControl();
	RegisterControl( m_pPage );

	CUIControl* pControl = CreateControl( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_MONEY_BACK" );
	pControl->SetAlignFlag( UI_FLAG_BOTTOM );

	CBasicButton* pButton = CreateFlipButton( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_MONEY_BUTTON", "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_MONEY_BUTTON_F", ITEM_RANDOM_OPTION_REBUILD_INVENTORY_MONEY_BUTTON, CBasicButton::CLICK_FLIP );
	pButton->CreateMouseOver( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_MONEY_BUTTON_F" );
	pButton->SetUseDynamic( TRUE );
	pButton->SetAlignFlag( UI_FLAG_BOTTOM );

	CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
	m_pMoneyTextBox = CreateStaticControl( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_MONEYTEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
	m_pMoneyTextBox->SetAlignFlag( UI_FLAG_BOTTOM );

	{
		CUIControl* pControl = CreateControl ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_GAME_POINTS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pGamePointsTextBox = CreateStaticControl ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_GAME_POINTS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pGamePointsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_GAME_POINTS_BUTTON", "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_GAME_POINTS_BUTTON_F", ITEM_RANDOM_OPTION_REBUILD_INVENTORY_GAME_POINTS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_GAME_POINTS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}

	{
		CUIControl* pControl = CreateControl ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WAR_CHIPS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pWarChipsTextBox = CreateStaticControl ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WAR_CHIPS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pWarChipsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WAR_CHIPS_BUTTON", "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WAR_CHIPS_BUTTON_F", ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WAR_CHIPS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "ITEM_RANDOM_OPTION_REBUILD_INVENTORY_WAR_CHIPS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}
}

void CItemRandomOptionRebuildInventoryWindow::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CItemRandomOptionRebuildInventoryWindow::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	switch( ControlID )
	{
...
HRESULT CItemRandomOptionRebuildInventoryWindow::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void CItemRandomOptionRebuildInventoryWindow::SetOneViewSlot( const INT& nONE_VIEW_SLOT )
{
	CUIControl TempControl;
	TempControl.Create( 1, "ITEM_RANDOM_OPTION_REBUILD_INVENSLOT_ITEM_SLOT0" );

	CUIControl TempControl2;
	TempControl2.Create( 2, "ITEM_RANDOM_OPTION_REBUILD_INVENSLOT_ITEM_SLOT1" );
