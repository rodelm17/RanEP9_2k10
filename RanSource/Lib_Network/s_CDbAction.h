...
protected:
	...
	CString m_strPCID_HWID;			//PCID
	CString m_strPCID_MAC;			//PCID
	bool m_bPCIDSingleLogin;	//PCID
	bool m_bPCIDCheckLogin;		//PCID
...
protected:
	...
	char  m_szFullName[USR_INFOFULLNAME_LENGTH];
	...
	//PCID
	CString m_strPCID_HWID;
	CString m_strPCID_MAC;
	BOOL m_bRegisterPCIDCheck;
	BOOL m_bRegisterEmailCheck;

	DWORD m_dwReferralUserNum;
	EMREFERRAL_REGISTER_TYPE m_emRegisterReferral;
};

/* login device check, Juver, 2020/11/06 */
class CAgentUserAccountPageLoginDeviceCheckSet : public CDbAction
{
public:
	CAgentUserAccountPageLoginDeviceCheckSet(
		DWORD dwUserNum,
		const TCHAR* szUser,
		const TCHAR* szPass,
	...
	virtual ~CAgentUserAccountPageLoginDeviceCheckSet() {} ;
	virtual int Execute(CServer* pServer);
...
protected:
	...
	bool m_bUserDeviceCheck;

};

/* login device check, Juver, 2020/11/07 */
class CAgentUserLoginDeviceCheckVerifyRequest : public CDbAction
{
public:
	CAgentUserLoginDeviceCheckVerifyRequest(
		const TCHAR* szUser,
		const TCHAR* szPass,
		int nSvrGrp,
		int nSvrNum,
	...
	virtual ~CAgentUserLoginDeviceCheckVerifyRequest() {} ;
	virtual int Execute(CServer* pServer);
...
/*dmk14 ingame web*/
class CAgentUserForgotPass : public CDbAction
{
public:
	CAgentUserForgotPass(
		const char* szUserID, // ID 
		const char* szPasswd, // PWD
		DWORD dwClient
		);
	virtual ~CAgentUserForgotPass() {} ;
	virtual int Execute(CServer* pServer);

protected:
	char  m_strUserID[USR_ID_LENGTH+1];
	char  m_strPasswd[USR_PASS_LENGTH];
	DWORD m_dwClientNum;
};

class CAgentUserForgotChangePass : public CDbAction
{
public:
	CAgentUserForgotChangePass(
		const char* szUserID, // ID 
		const char* szPasswd, // PWD
		const char* szPasswd2, // PWD2
		DWORD dwClient
		);
	virtual ~CAgentUserForgotChangePass() {} ;
	virtual int Execute(CServer* pServer);

protected:
	char  m_strUserID[USR_ID_LENGTH+1];
	char  m_strPasswd[USR_PASS_LENGTH];
	char  m_strPasswd2[USR_PASS_LENGTH];
	DWORD m_dwClientNum;
};

class CAgentUserSelcharChangePass : public CDbAction
{
public:
	CAgentUserSelcharChangePass(
		const char* szUserID, // ID 
		const char* szPasswd, // PWD
		const char* szPincode, // PWD
		const char* szNewPasswd, // newPW
		DWORD dwClient
		);
	virtual ~CAgentUserSelcharChangePass() {} ;
	virtual int Execute(CServer* pServer);

protected:
	char  m_strUserID[USR_ID_LENGTH+1];
	char  m_strPasswd[USR_PASS_LENGTH];
	char  m_strPincode[USR_PASS_LENGTH];
	char  m_strNewPasswd[USR_PASS_LENGTH];
	DWORD m_dwClientNum;
};

class CAgentUserSelcharChangePin : public CDbAction
{
public:
	CAgentUserSelcharChangePin(
		const char* szUserID, // ID 
		const char* szPasswd, // PWD
		const char* szPincode, // PWD
		const char* szNewPincode, // newPin
		DWORD dwClient
		);
	virtual ~CAgentUserSelcharChangePin() {} ;
	virtual int Execute(CServer* pServer);
...
protected:
	...
	char  m_strPasswd[USR_PASS_LENGTH];
	char  m_strPincode[USR_PASS_LENGTH];
	char  m_strNewPincode[USR_PASS_LENGTH];
	DWORD m_dwClientNum;
};

class CAgentReqGameTimeConvert : public CDbAction
{
public:
	CAgentReqGameTimeConvert(
		const char* szUserID, // ID 
		DWORD dwClient
		);
	virtual ~CAgentReqGameTimeConvert() {} ;
	virtual int Execute(CServer* pServer);
...
//6-4-17
class CAgentUserSelcharResetPin : public CDbAction
{
public:
	CAgentUserSelcharResetPin(
		const char* szUserID, // ID 
		const char* szPasswd, // PWD
		const char* szMail, // MAIL
		DWORD dwClient
		);
	virtual ~CAgentUserSelcharResetPin() {} ;
	virtual int Execute(CServer* pServer);

protected:
	char  m_strUserID[USR_ID_LENGTH+1];
	char  m_strPasswd[USR_PASS_LENGTH];
	char  m_strMail[USR_INFOMAIL_LENGTH];
	char  m_strNewpin[USR_PASS_LENGTH];
	DWORD m_dwClientNum;
};

class CAgentUserSelcharTopUp : public CDbAction
{
public:
	CAgentUserSelcharTopUp(
		const char* szUserID, // ID 
		const char* szCode, // PWD
		const char* szPin, // MAIL
		DWORD dwClient
		);
	virtual ~CAgentUserSelcharTopUp() {} ;
	virtual int Execute(CServer* pServer);

protected:
	char  m_strUserID[USR_ID_LENGTH+1];
	char  m_strPin[TOPUP_PIN_LENGTH];
	char  m_strCode[TOPUP_CODE_LENGTH];
	DWORD m_dwClientNum;
};

class CAgentUserSelcharChangeMail : public CDbAction
{
public:
	CAgentUserSelcharChangeMail(
		const char* szUserID, // ID 
		const char* szPasswd, // PWD
		const char* szPincode, // PWD
		const char* szMailOld, // MAIL
		const char* szMailNew, // MAIL
		DWORD dwClient
		);
	virtual ~CAgentUserSelcharChangeMail() {} ;
	virtual int Execute(CServer* pServer);
...
protected:
	...
	char  m_strPasswd[USR_PASS_LENGTH];
	char  m_strPincode[USR_PASS_LENGTH];
	char  m_strMailOld[USR_INFOMAIL_LENGTH];
	char  m_strMailNew[USR_INFOMAIL_LENGTH];
	DWORD m_dwClientNum;
};
