...
HRESULT GLPVPTyrannyAgent::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case NET_MSG_GCTRL_TYRANNY_F2A_RANKINFO_PC:
		{
			...
			//	netmsgclient.sRewardBuff[i] = m_sScheduleNext.m_sRewardBuff[i].sidSkill;
	...
	case NET_MSG_GCTRL_TYRANNY_F2A_TOWER_CAPTURE:
		{	
			...
			TowerCapture( pnetmsg->sidMob, pnetmsg->wSchool, pnetmsg->dwClubID, pnetmsg->dwClubMarkVer );
		}break;
	...
	case NET_MSG_GCTRL_TYRANNY_C2A_LATE_JOIN_REQ:
		{
			GLMSG::SNETPC_TYRANNY_C2A_LATE_JOIN_REQ* pnetmsg = ( GLMSG::SNETPC_TYRANNY_C2A_LATE_JOIN_REQ* ) nmg;
			PlayerLateJoinBattle( pnetmsg->dwCharID, pnetmsg->wLevel, pnetmsg->szHASH );
		}break;

	case NET_MSG_GCTRL_TYRANNY_F2A_GUILD_TOWER_CAPTURE:
		{
			GLMSG::SNETPC_TYRANNY_F2A_GUILD_TOWER_CAPTURE* pnetmsg = ( GLMSG::SNETPC_TYRANNY_F2A_GUILD_TOWER_CAPTURE* ) nmg;
			m_dwGuiID[pnetmsg->m_nIndex] = pnetmsg->m_dwGuiID;
			m_dwMarkVer[pnetmsg->m_nIndex] = pnetmsg->m_dwMarkVer;

			GLMSG::SNETPC_TYRANNY_A2F_GUILD_TOWER_CAPTURE netmsgfield;
			netmsgfield.m_dwGuiID = pnetmsg->m_dwGuiID;
			netmsgfield.m_dwMarkVer = pnetmsg->m_dwMarkVer;
			netmsgfield.m_nIndex = pnetmsg->m_nIndex;
