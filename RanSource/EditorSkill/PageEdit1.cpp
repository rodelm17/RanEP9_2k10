...
BEGIN_MESSAGE_MAP(CPageEdit1, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BUTTON_SUB_SKILL, &CPageEdit1::OnBnClickedButtonSubSkill)
	ON_EN_CHANGE(IDC_EDIT_SUB_SKILL_MID, &CPageEdit1::OnEnChangeEditSubSkillMid)
	ON_EN_CHANGE(IDC_EDIT_SUB_SKILL_SID, &CPageEdit1::OnEnChangeEditSubSkillSid)
END_MESSAGE_MAP()
...
void CPageEdit1::Loadeffskin( int nID )
{
	...
	CFileDialog dlg( TRUE, ".effskin_a", DxEffcharDataMan::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
void CPageEdit1::Loadtexture( int nID )
{
	...
	CFileDialog dlg( TRUE, ".dds", TextureManager::GetTexturePath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
void CPageEdit1::OnBnClickedButtonSubSkill()
{
	CDlgSkill dlg;
	if ( dlg.DoModal() == IDOK )
	{
		SNATIVEID sID = dlg.m_SELECTED;
		SetWin_Num_int( this, IDC_EDIT_SUB_SKILL_MID, sID.wMainID );
		SetWin_Num_int( this, IDC_EDIT_SUB_SKILL_SID, sID.wSubID );
		CheckSubSkill();
	}
}

/*multi skill Juver, 2018/09/04 */
void CPageEdit1::OnEnChangeEditSubSkillMid()
{
	CheckSubSkill();
}

/*multi skill Juver, 2018/09/04 */
void CPageEdit1::OnEnChangeEditSubSkillSid()
{
	CheckSubSkill();
}

/*multi skill Juver, 2018/09/04 */
void CPageEdit1::CheckSubSkill()
{
	WORD wMID = GetWin_Num_int( this, IDC_EDIT_SUB_SKILL_MID );
	WORD wSID = GetWin_Num_int( this, IDC_EDIT_SUB_SKILL_SID );
	PGLSKILL pSKILL = GLSkillMan::GetInstance().GetData( wMID, wSID );
	SetWin_Text( this, IDC_EDIT_SUB_SKILL_NAME, pSKILL? pSKILL->GetName() : "Unknown Skill" );
}