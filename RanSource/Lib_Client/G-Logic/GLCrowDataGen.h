...
struct SCROWGEN
{
	enum { VERSION = 0x0201, MAX_GEN_ITEM_ARRAY = 8, MAX_QGEN_ITEM_ARRAY = 4, };
	...
	std::string			m_strQtGenItem[MAX_QGEN_ITEM_ARRAY];	//	Question Item 설정 파일.
	std::string			m_strGenItem[MAX_GEN_ITEM_ARRAY];		//	발생 아이템 설정 파일.
	...
	void Assign ( SCROWGEN_101 &OldCrowGen )
	{
		...
		m_strGenItem[0] = OldCrowGen.m_szGenItem;
	}
	...
	SCROWGEN& operator = ( const SCROWGEN &value )
	{
		...
		/* multiple crow gen, Juver, 2018/02/21 */
		for ( int i=0; i<MAX_GEN_ITEM_ARRAY; ++i )
			m_strGenItem[i] = value.m_strGenItem[i];

		for ( int i=0; i<MAX_QGEN_ITEM_ARRAY; ++i )
			m_strQtGenItem[i] = value.m_strQtGenItem[i];
