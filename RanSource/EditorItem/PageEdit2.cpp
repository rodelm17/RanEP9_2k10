...
BEGIN_MESSAGE_MAP(CPageEdit2, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BUTTON_PAGE_NEXT2, OnBnClickedButtonPageNext2)
	...
	ON_BN_CLICKED(IDC_BUTTON_BOX_100_SORT, &CPageEdit2::OnBnClickedButtonBox100Sort)
...
void CPageEdit2::OnBnClickedButtonPageNext2()
{
	if ( !DataSave() )	return;
	...
	m_pSheetTab->ActivePageEdit3( m_CallPage, m_pItem );
	m_pItem = NULL;
}
...
void CPageEdit2::OnBnClickedButtonBox100Sort()
{
	if( !m_pDummyItem )	return;

	unsigned int size = m_pDummyItem->sRandomBox.vecBOX.size();
	for ( unsigned i=0; i<size; ++i )
	{
		m_pDummyItem->sRandomBox.vecBOX[i].fRATE = float( 100.f / size );
	}
