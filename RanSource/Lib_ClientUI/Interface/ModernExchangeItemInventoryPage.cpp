...
const INT CModernExchangeItemInventoryPage::nSTARTLINE = 0;	
const INT CModernExchangeItemInventoryPage::nOUTOFRANGE = -1;

CModernExchangeItemInventoryPage::CModernExchangeItemInventoryPage()
...
void CModernExchangeItemInventoryPage::CreateSubControl()
{
	CString strSlotKeyword[ nMAX_ONE_VIEW_SLOT ] =
	{
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT0",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT1",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT2",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT3",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT4",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT5",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT6",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT7",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT8",
		"MODERN_EXCHANGE_ITEM_INVENTORY_WINDOW_ITEM_SLOT9",
...
CItemSlotEx* CModernExchangeItemInventoryPage::CreateItemSlot( CString strKeyword, UIGUID ControlID )
{
...
CUIControl*	CModernExchangeItemInventoryPage::CreateControl( const TCHAR* szControl )
{
...
void CModernExchangeItemInventoryPage::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CModernExchangeItemInventoryPage::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
...
void CModernExchangeItemInventoryPage::ResetItemSlotRender ( int nStart, int nEnd )
{
	if ( nEnd < 0 )			return;
...
void CModernExchangeItemInventoryPage::SetItemSlotRender( int nStart, int nEnd )
{
	if ( nEnd < 0 ) return ;
	if ( nStart > nEnd ) return ;
...
void CModernExchangeItemInventoryPage::SetItemIndex( INT nPosX, INT nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

void CModernExchangeItemInventoryPage::GetItemIndex( INT* pnPosX, INT* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

void CModernExchangeItemInventoryPage::LoadItemPage( GLInventory &ItemData )
{
...
void CModernExchangeItemInventoryPage::LoadItem( SINVENITEM& ref_InvenItem )
{	
...
void CModernExchangeItemInventoryPage::UnLoadItemPage()
{
...
void CModernExchangeItemInventoryPage::UnLoadItem( INT nPosX, INT nPosY )
{
	m_pItemSlotArray[ nPosY ]->ResetItemImage( nPosX );
}

void CModernExchangeItemInventoryPage::ResetAllFlipItem()
{
	for( WORD y=0; y<EM_INVENSIZE_Y; ++y )
		for( WORD x=0; x<EM_INVENSIZE_X; ++x )
...
void CModernExchangeItemInventoryPage::UpdateFlipItem( GLInventory& ref_Inventory )
{
	ResetAllFlipItem();
...
void CModernExchangeItemInventoryPage::UpdateFlipItem( SINVEN_POS& ref_GarbageItem )
{
...
void CModernExchangeItemInventoryPage::UpdateBLOCK()
{
...
SINVENITEM& CModernExchangeItemInventoryPage::GetItem( INT nPosX, INT nPosY )
{
	return m_pItemSlotArray[ nPosY ]->GetItemImage( nPosX );
}

void CModernExchangeItemInventoryPage::SetOneViewSlot( const INT& nMAX_ONE_VIEW )
{
