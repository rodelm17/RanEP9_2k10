...
#include "../../Lib_Engine/Common/IniLoader.h" //pandora
...
//		cFILE.getflag( "fDISPRICE", 1, 1, fDISPRICE );
		...
		cFILE.getflag( "fUNIT_TIME_CROW",  1, 1, fUNIT_TIME_CROW );	
		cFILE.getflag( "fHP_INC_PER_CROW ", 1, 1, fHP_INC_PER_CROW );
		cFILE.getflag( "fMP_INC_PER_CROW ", 1, 1, fMP_INC_PER_CROW );
		cFILE.getflag( "fSP_INC_PER_CROW ", 1, 1, fSP_INC_PER_CROW );
		...
		cFILE.getflag( "fDAMAGE_DEC_MAX_G", 1, 1, fDAMAGE_DEC_MAX_G );
		...
		{
		cFILE.getflag( "fPK_POINT_DEC_PHY",	1+i, GLCL_NUM_CLASS, fPK_POINT_DEC_PHY[i] );
		cFILE.getflag( "fPK_POINT_DEC_MAGIC",	1+i, GLCL_NUM_CLASS, fPK_POINT_DEC_MAGIC[i] );
		}
		...
		CString strGachaItem;
		strGachaItem.Format ( "nidGachaItem", 0 );
		cFILE.getflag( strGachaItem.GetString(), 1, 2, nidGachaItem.wMainID );
		cFILE.getflag( strGachaItem.GetString(), 2, 2, nidGachaItem.wSubID );
		/* Gacha System, MontageDev 7/10/24 */
		cFILE.getflag( "wGachaMaxPoint",	1, 1, wGachaMaxPoint );
		...
		//////////////////////////////////////////////
		//MMR Rank
		cFILE.getflag( "bEnableRankName", 1, 1, bEnableRankName );
		cFILE.getflag( "bEnableRankMark", 1, 1, bEnableRankMark );
		cFILE.getflag( "dwMMRMax", 1, 1, dwMMRMax );
		//////////////////////////////////////////////
		...
		for ( int i=0; i < GLCI_NUM_8CLASS; i++ )
		{
			if ( cFILE.findflag(szSETFILE[i]) )
			{
				...
				bCONSTCLASS[i] = cCONSTCLASS[i].LOADFILE ( strArg.GetString() );
				if ( !bCONSTCLASS[i] )	return FALSE;
			}
		}
		...
		// 미니게임 - 홀짝
		for ( UINT i=0; i<MINIGAME_ODDEVEN::MAX_ROUND; ++i )
		{
		...
		cFILE.getflag("WOE_FILE", 1, 1, strWoeFile);
		...
		cFILE.getflag( "INT_DEF_RATE",	1, 1, fIntDefRate );

		/*character simple, Juver, 2017/10/01 */
		for ( int i=0; i<SLOT_WEAR_SIMPLE; ++i )
		{
		...
		for ( int i=0; i<SLOT_WEAR_SIMPLE; ++i )
		{
			cFILE.getflag( "strSimpleWearM_SG", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearM_SG[i] );
			cFILE.getflag( "strSimpleWearF_SG", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearF_SG[i] );
		}

		for ( int i=0; i<SLOT_WEAR_SIMPLE; ++i )
		{
			cFILE.getflag( "strSimpleWearM_MP", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearM_MP[i] );
			cFILE.getflag( "strSimpleWearF_MP", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearF_MP[i] );
		}

		for ( int i=0; i<SLOT_WEAR_SIMPLE; ++i )
		{
			cFILE.getflag( "strSimpleWearM_PHX", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearM_PHX[i] );
			cFILE.getflag( "strSimpleWearF_PHX", 1+i, SLOT_WEAR_SIMPLE, strSimpleWearF_PHX[i] );
		}
		...
		for ( int i=0; i<BATTLEPASS_LEVEL_MAX; ++i )
		{
			cFILE.getflag( "dwBattlePassEXP", 1+i, BATTLEPASS_LEVEL_MAX, dwBattlePassEXP[i] );
		}
		...
		cFILE.getflag( "fBECOME_AWAY_TIME", 1, 1, fBECOME_AWAY_TIME ); 
		cFILE.getflag( "nAWAY_GESTURE_NUMBER", 1, 1, nAWAY_GESTURE_NUMBER ); 

		/* pvp club death match, Juver, 2020/11/10 */
		cFILE.getflag( "CLUB_DEATH_MATCH_FILE",	1, 1, strClubDeathMatchFile );

		/* skill illusion, Juver, 2021/01/17 */
		cFILE.getflag( "fIllusionInterval", 1, 1, fIllusionInterval );

		cFILE.getflag( "PVP_PARTY_PBG_FILE",	1, 1, strPVPPBGFile);

		EXP_MAX_LOADFILE( "exptable_max.bin" );
		EXP_MAX_2nd_LOADFILE( "exptable_max_2nd.bin" );
		
		LOADFILE_PANDORA ( "pandorabox.ini" ); // Pandora
		...
		CPMAX_LOADFILE( "cp_max.bin" );//add cp
	...
	BOOL	EXP_MAX_LOADFILE ( char* szFileName )
	{
		...
		for ( int i=0; i<MAX_CHARACTER_LEVEL+1; ++i )	lnEXP_MAX_TABLE[i] = LLONG_MAX;
	...
	BOOL	EXP_MAX_2nd_LOADFILE ( char* szFileName )
	{
		...
		for ( int i=0; i<MAX_CHARACTER_LEVEL+1; ++i )	lnEXP_MAX_TABLE_2nd[i] = LLONG_MAX;
	...
	BOOL LOADFILE_PANDORA ( char* szFileName )
    {
        ...
        if( !cFILE.open ( strPath, true ) )
        {
            CDebugSet::ToLogFile ( "ERROR GLogicData::LOADFILE_PANDORA File Open %s", szFileName );
            return false;
        }

        cFILE.getflag( "PANDORA_SETTING", "GOLD_VALUE", 0, 1, sPandoraSetting.llGold );
        cFILE.getflag( "PANDORA_SETTING", "EP_VALUE", 0, 1, sPandoraSetting.wEP );
        cFILE.getflag( "PANDORA_SETTING", "PREM_CHANCE", 0, 1, sPandoraSetting.fPremChance );
        cFILE.getflag( "PANDORA_SETTING", "REQ_DELAY", 0, 1, sPandoraSetting.fReqDelay );

        DWORD dwNUM = cFILE.GetKeySize( "PANDORA", "BOX_ITEMS" );

        for( DWORD i=0; i<dwNUM; ++i )
        {
            GLPANDORA_BOX sPBox;
            cFILE.getflag( i, "PANDORA", "BOX_ITEMS", 0, 5, sPBox.sItemID.wMainID );
            cFILE.getflag( i, "PANDORA", "BOX_ITEMS", 1, 5, sPBox.sItemID.wSubID );
            cFILE.getflag( i, "PANDORA", "BOX_ITEMS", 2, 5, sPBox.fRate );
            cFILE.getflag( i, "PANDORA", "BOX_ITEMS", 3, 5, sPBox.llGold );
            cFILE.getflag( i, "PANDORA", "BOX_ITEMS", 4, 5, sPBox.wEP );
            vecPandoraBoxes.push_back(sPBox);
        }
        return TRUE;
    }
	...
	//add cp
	//for cpmax vector
	BOOL CPMAX_LOADFILE ( char* szFileName )
	{
		if( !szFileName || (strlen(szFileName) == 0) )	
			return FALSE;
		...
		vecCPMAX.clear();
		vecCPMAX.reserve( MAX_CP_LIST+1 );
		...
		if( !cFILE.open( strPath, true ) )
		{
			CDebugSet::ToLogFile ( "ERROR : GLCONST_CHAR::CPMAX_LOADFILE(), %s", szFileName );
			return FALSE;
		}

		TCHAR szTemp[10] = {0};

		for ( WORD i=0; i<MAX_CP_LIST+1; ++i )
		{
			WORD wValue = 0;
			_snprintf( szTemp, 10, "%u", i);
			cFILE.getflag( szTemp, 1, 1, wValue );

			if ( wValue == 0 )
				wValue = DEFAULT_CP_MAX;

			vecCPMAX.push_back(wValue);
		}
	...
	BOOL	LOADRANK ( char* szFileName )
	{
		...
		for ( DWORD i=0; (i<20); ++i )
		{
			...
			cFILE.getflag(strTemp.GetString(), 1, 3, dwRank_Point[i]);    
			cFILE.getflag(strTemp.GetString(), 2, 3, strRank_Name[i]);
			cFILE.getflag(strTemp.GetString(), 3, 3, strRank_Badge[i]);
		}
		return TRUE;
	}
	//////////////////////////////////////////////////////////////////////////////////////////////
...
{
	if ( RPARAM::bTextClassConst )
		return LOADFILE_STRING( szFileName, true );

	return LOADFILE_STREAM( szFileName );
}

BOOL GLCONST_CHARCLASS::LOADFILE_STRING ( const char* szFileName, bool bVersionCheck )
{
	if( !szFileName )				return FALSE;
	if( strlen(szFileName) == 0 )	return FALSE;
	...
	if ( bVersionCheck && dwVERSION != VERSION_TEXT )	return FALSE;
	...
	for ( DWORD i=0; i<MAX_HEAD; ++i )
		strHEAD_CPS[i] = "";

	for ( DWORD i=0; i<MAX_HAIR; ++i )
		strHAIR_CPS[i] = "";
...
BOOL GLCONST_CHARCLASS::LOADFILE_STREAM ( const char* szFileName )
{
	if( !szFileName )				return FALSE;
	if( strlen(szFileName) == 0 )	return FALSE;
	...
	StringCchCopy ( szErrorFileName, MAX_PATH, szFullPath );

	std::auto_ptr<basestream> pBStream( GLOGIC::openfile_basestream(GLOGIC::bGLOGIC_ZIPFILE, 
		GLOGIC::strGLOGIC_ZIPFILE.c_str(), szFullPath, szFileName,
		EMRCRYPT_BYTE_TYPE_NONE, GLOGIC::bGLOGIC_PACKFILE ) );

	if ( !pBStream.get() )	return FALSE;
	...
	if ( dwfile_version>=VERSION_ENCODE )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_CLASS_CONST_VER_1 );

	if ( dwfile_version>=ENCODE_VER_BYTE_CRYPT_2021_CLASS_CONST_V1 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_CLASS_CONST_V1 );

	if ( dwfile_version == VERSION_CLASSCONST )
	{
		SFile >> fWALKVELO;
		SFile >> fRUNVELO;	
		...
		SFile >> sBEGIN_STATS.wPow;
		SFile >> sBEGIN_STATS.wStr;
		SFile >> sBEGIN_STATS.wSpi;
		SFile >> sBEGIN_STATS.wDex;
		SFile >> sBEGIN_STATS.wInt;
		SFile >> sBEGIN_STATS.wSta;

		SFile >> sLVLUP_STATS.fPow;
		SFile >> sLVLUP_STATS.fStr;
		SFile >> sLVLUP_STATS.fSpi;
		SFile >> sLVLUP_STATS.fDex;
		SFile >> sLVLUP_STATS.fInt;
		SFile >> sLVLUP_STATS.fSta;

		SFile >> dwHEADNUM;
		SFile >> dwHEADNUM_SELECT;	

		for ( DWORD i=0; i<MAX_HEAD; ++i )
			SFile >> strHEAD_CPS[i];

		SFile >> dwHAIRNUM;	
		SFile >> dwHAIRNUM_SELECT;	

		for ( DWORD i=0; i<MAX_HAIR; ++i )
			SFile >> strHAIR_CPS[i];

		SFile >> strCLASS_EFFECT;
	}
	else if ( dwfile_version == 0x0100 )
	{
		SFile >> fWALKVELO;
		SFile >> fRUNVELO;	
...
BOOL GLCONST_CHARCLASS::SAVEFILE_STREAM ( const char* szFileName )
{
	...
	CSerialFile SFile;
	SFile.SetFileType ( "", VERSION_CLASSCONST );
	if ( !SFile.OpenFile ( FOT_WRITE, szFullPath ) )	return FALSE;
	SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_CLASS_CONST_V1 );
...
BOOL GLCONST_CHARCLASS::SAVEFILE_STRING ( const char* szFilePath )
{
	if ( !szFilePath )	return FALSE;

	CString strBuffer;
	strBuffer += _T("/////////////////////////////////////////////////////////////////////\n");
	strBuffer.AppendFormat( "VERSION\t%d\n", VERSION_TEXT );
	strBuffer += _T("/////////////////////////////////////////////////////////////////////\n" );
	strBuffer += _T("//\n" );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fWALKVELO\t%g\n", fWALKVELO );
	strBuffer.AppendFormat( "fRUNVELO\t%g\n", fRUNVELO );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fHP_STR\t%g\n", fHP_STR );
	strBuffer.AppendFormat( "fMP_SPI\t%g\n", fMP_SPI );
	strBuffer.AppendFormat( "fSP_STA\t%g\n", fSP_STA );
	strBuffer.AppendFormat( "fHIT_DEX\t%g\n", fHIT_DEX );
	strBuffer.AppendFormat( "fAVOID_DEX\t%g\n", fAVOID_DEX );
	strBuffer.AppendFormat( "fDEFENSE_DEX\t%g\n", fDEFENSE_DEX );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fPA_POW\t%g\n", fPA_POW );
	strBuffer.AppendFormat( "fPA_DEX\t%g\n", fPA_DEX );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fSA_POW\t%g\n", fSA_POW );
	strBuffer.AppendFormat( "fSA_DEX\t%g\n", fSA_DEX );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fMA_DEX\t%g\n", fMA_DEX );
	strBuffer.AppendFormat( "fMA_SPI\t%g\n", fMA_SPI );
	strBuffer.AppendFormat( "fMA_INT\t%g\n", fMA_INT );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fCONV_AP\t%g\n", fCONV_AP );
	strBuffer.AppendFormat( "fCONV_DP\t%g\n", fCONV_DP );
	strBuffer.AppendFormat( "fCONV_PA\t%g\n", fCONV_PA );
	strBuffer.AppendFormat( "fCONV_SA\t%g\n", fCONV_SA );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "wBEGIN_AP\t%d\n", wBEGIN_AP );
	strBuffer.AppendFormat( "wBEGIN_DP\t%d\n", wBEGIN_DP );
	strBuffer.AppendFormat( "wBEGIN_PA\t%d\n", wBEGIN_PA );
	strBuffer.AppendFormat( "wBEGIN_SA\t%d\n", wBEGIN_SA );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "fLVLUP_AP\t%g\n", fLVLUP_AP );
	strBuffer.AppendFormat( "fLVLUP_DP\t%g\n", fLVLUP_DP );
	strBuffer.AppendFormat( "fLVLUP_PA\t%g\n", fLVLUP_PA );
	strBuffer.AppendFormat( "fLVLUP_SA\t%g\n", fLVLUP_SA );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "sBEGIN_STATS\t[%d | %d]\t[%d | %d]\t[%d | %d]\n", sBEGIN_STATS.wPow, sBEGIN_STATS.wStr, sBEGIN_STATS.wSpi, sBEGIN_STATS.wDex, sBEGIN_STATS.wInt, sBEGIN_STATS.wSta );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "sLVLUP_STATS\t[%g | %g]\t[%g | %g]\t[%g | %g]\n", sLVLUP_STATS.fPow, sLVLUP_STATS.fStr, sLVLUP_STATS.fSpi, sLVLUP_STATS.fDex, sLVLUP_STATS.fInt, sLVLUP_STATS.fSta );
	strBuffer += _T("//\n" );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "dwHEADNUM\t%d\n", dwHEADNUM );
	strBuffer.AppendFormat( "dwHEADNUM_SELECT\t%d\n", dwHEADNUM_SELECT );
	strBuffer += _T("//\n" );
	...
	for ( DWORD i=0; i<dwHEADNUM; ++i )
	{
		...
		strHead += strHEAD_CPS[i];
	}

	if ( strHead.size() )
	{
		strBuffer.AppendFormat( "strHEAD_CPS\t%s\n", strHead.c_str() );
	}

	strBuffer += _T("//\n" );
	strBuffer += _T("//\n" );
	strBuffer.AppendFormat( "dwHAIRNUM\t%d\n", dwHAIRNUM );
	strBuffer.AppendFormat( "dwHAIRNUM_SELECT\t%d\n", dwHAIRNUM_SELECT );
	strBuffer += _T("//\n" );
	...
	for ( DWORD i=0; i<dwHAIRNUM; ++i )
	{
		...
		strHair += strHAIR_CPS[i];
	}

	if ( strHair.size() )
	{
		strBuffer.AppendFormat(  "strHAIR_CPS\t%s\n", strHair.c_str() );
	}

	strBuffer += _T("//\n" );
	strBuffer += _T("//\n" );

	if ( strCLASS_EFFECT.size() )
	{
		strBuffer.AppendFormat( "strCLASS_EFFECT\t%s\n", strCLASS_EFFECT.c_str() );
		strBuffer += _T("//\n" );
	}
