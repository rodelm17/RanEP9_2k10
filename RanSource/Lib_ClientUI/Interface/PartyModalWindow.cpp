...
void CPartyModalWindow::CreateSubControl ()
{
	...
	{
		pTextBox = CreateStaticControl ( "PARTY_MODAL_PRIVATE_STATIC", pFont8, dwColor, nAlign );
		pTextBox->AddText ( (char*)ID2GAMEWORD ( "PARTY_MODAL_TEXT_EX", 0 ) );

		m_pButtonPrivate = new CBasicButton;
		m_pButtonPrivate->CreateSub ( this, "PARTY_MODAL_PRIVATE_BUTTON", UI_FLAG_DEFAULT, PARTY_MODAL_PRIVATE_BUTTON );
		m_pButtonPrivate->CreateFlip ( "PARTY_MODAL_PRIVATE_BUTTON_F", CBasicButton::RADIO_FLIP );
		m_pButtonPrivate->SetControlNameEx ( "PARTY_MODAL_PRIVATE_BUTTON" );
		RegisterControl ( m_pButtonPrivate );
	}
}
...
void	CPartyModalWindow::LoadItem ()
{
	...
	if ( m_bReqModal )
	{		
		if(RPROFILE::dwPARTY_GET_ITEM == 0 || RPROFILE::dwPARTY_GET_ITEM >= EMPTYITEMOPT_SIZE )
		{
			RPROFILE::dwPARTY_GET_ITEM = 3;
		}

		m_pComboBoxItemOpen->SetText ( (char*)ID2GAMEWORD("PARTY_MODAL_ITEM_OPTION", RPROFILE::dwPARTY_GET_ITEM) );
	}
	else
	{		
...
void	CPartyModalWindow::LoadMoney ()
{
	...
	if ( m_bReqModal )
	{	
		if(RPROFILE::dwPARTY_GET_MONEY == 0 || RPROFILE::dwPARTY_GET_MONEY >= EMPTYMONEYOPT_SIZE )
		{
			RPROFILE::dwPARTY_GET_MONEY = 3;
		}

		m_pComboBoxMoneyOpen->SetText ( (char*)ID2GAMEWORD("PARTY_MODAL_MONEY_OPTION", RPROFILE::dwPARTY_GET_MONEY) );
	}
	else
	{		
		SPARTY_OPT* pPartyOption = CInnerInterface::GetInstance().GetPartyOption ();
...
void	CPartyModalWindow::LoadPrivate ()
{
	if ( m_bReqModal )
	{	
		m_pButtonPrivate->SetFlip ( RPROFILE::bPARTY_PRIVATE );
	}
	else
	{		
		SPARTY_OPT* pPartyOption = CInnerInterface::GetInstance().GetPartyOption ();
		m_pButtonPrivate->SetFlip ( pPartyOption->bPrivateParty );
	}
}