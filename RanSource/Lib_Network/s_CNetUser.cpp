...
//	ZeroMemory( m_szRecvGarbageMsg[2], 12 );
	...
	m_bUserAccountPageInit = false;
	m_bUserAccountPageWait = false;

	/* login device check, Juver, 2020/11/06 */
	m_bUserLoginDeviceCheck = false;
	m_bUserLoginDeviceCheckVerifyRequestInit = false;
	m_bUserLoginDeviceCheckVerifyRequestWait = false;
}
...
int CNetUser::addRcvMsg(void* pMsg, DWORD dwSize)
{
	...
	if (nTotalRcvSize < 0 )
	{
		...
		//CDebugSet::ToFileWithTime( "_invalid_buffer.txt", "ERROR:%d %s Buffer Negative", nTotalRcvSize, m_szIp);
		//CDebugSet::ToFile( "_invalid_buffer_ip.txt", "%s", m_szIp );
	...
	if (nTotalRcvSize > NET_DATA_CLIENT_MSG_BUFSIZE)
	{
		...
		//CDebugSet::ToFileWithTime( "_invalid_buffer.txt", "ERROR:%d %s Buffer Over", nTotalRcvSize, m_szIp);
		//CDebugSet::ToFile( "_invalid_buffer_ip.txt", "%s", m_szIp );
...
//		strcmp( szTempChar, m_szRecvGarbageMsg[2] ) != 0 &&
		strlen( szTempChar ) != 0 )
	{
		...
		CDebugSet::ToFileWithTime( "_invalid_msg_garbage.txt", "CNetUser::Client invalid Garbage Msg!! Channel %d, IP %s, UserNum %d, ID %s, GarbageMSG %s %s",
			m_nChannel, m_szIp, m_uib.nUserNum, m_uib.szUserID, szTempChar, szTempChar2 );

		CDebugSet::ToFile( "_invalid_msg_garbage_ip.txt", "%s", m_szIp );
	}
}
...
{	
	...
	else
	{
		...
		if( bClient )
		{
			...
			//	CDebugSet::ToFileWithTime( "_invalid_msg.txt", "CNetUser::Client invalid Msg!! Channel %d, IP %s, UserNum %d, ID %s",
			//		m_nChannel, m_szIp, m_uib.nUserNum, m_uib.szUserID );

			//	CDebugSet::ToFile( "_invalid_msg_ip.txt", "%s", m_szIp );
			}
			return msg;
		}else{
...
/*dmk14 ingame web*/
void CNetUser::SetPremiumPoints( DWORD dwPoints )
{
	m_dwPPoints = dwPoints;
}

DWORD CNetUser::GetPremiumPoints()
{
	return m_dwPPoints;
}

void CNetUser::SetCombatPoints( DWORD dwPoints )
{
	m_dwVPoints = dwPoints;
}

DWORD CNetUser::GetCombatPoints()
{
	return m_dwVPoints;
}
...
/* user flag verified, Juver, 2020/02/25 */
void CNetUser::SetUserFlagVerified( bool bUserFlagVerified )
{
	m_bUserFlagVerified = bUserFlagVerified;
}

/* user flag verified, Juver, 2020/02/25 */
bool CNetUser::GetUserFlagVerified()
{
	return m_bUserFlagVerified;
}

/* user flag restricted, Juver, 2020/04/20 */
void CNetUser::SetUserFlagRestricted( bool bUserFlagRestricted )
{
	m_bUserFlagRestricted = bUserFlagRestricted;
}

/* user flag restricted, Juver, 2020/04/20 */
bool CNetUser::GetUserFlagRestricted()
{
	return m_bUserFlagRestricted;
}

/* HWID watchlist, Juver, 2020/05/05 */
void CNetUser::SetUserFlagHWIDWatchList( bool bUserFlagHWIDWatchList )
{
	m_bUserFlagHWIDWatchList = bUserFlagHWIDWatchList;
}

/* HWID watchlist, Juver, 2020/05/05 */
bool CNetUser::GetUserFlagHWIDWatchList()
{
	return m_bUserFlagHWIDWatchList;
}


/* user account page, Juver, 2020/11/05 */
bool CNetUser::GetUserAccountPageInit()
{
	return m_bUserAccountPageInit;
}

/* user account page, Juver, 2020/11/05 */
void CNetUser::SetUserAccountPageInit( bool bSet )
{
	m_bUserAccountPageInit = bSet;
}

/* user account page, Juver, 2020/11/05 */
bool CNetUser::GetUserAccountPageWait()
{
	return m_bUserAccountPageWait;
}

/* user account page, Juver, 2020/11/05 */
void CNetUser::SetUserAccountPageWait( bool bSet )
{
	m_bUserAccountPageWait = bSet;
}

/* login device check, Juver, 2020/11/06 */
bool CNetUser::GetUserLoginDeviceCheck()
{
	return m_bUserLoginDeviceCheck;
}

/* login device check, Juver, 2020/11/06 */
void CNetUser::SetUserLoginDeviceCheck( bool bSet )
{
	m_bUserLoginDeviceCheck = bSet;
}

/* login device check, Juver, 2020/11/07 */
bool CNetUser::GetUserLoginDeviceCheckVerifyRequestInit()
{
	return m_bUserLoginDeviceCheckVerifyRequestInit;
}

/* login device check, Juver, 2020/11/07 */
void CNetUser::SetUserLoginDeviceCheckVerifyRequestInit( bool bSet )
{
	m_bUserLoginDeviceCheckVerifyRequestInit = bSet;
}

/* login device check, Juver, 2020/11/07 */
bool CNetUser::GetUserLoginDeviceCheckVerifyRequestWait()
{
	return m_bUserLoginDeviceCheckVerifyRequestWait;
}

/* login device check, Juver, 2020/11/07 */
void CNetUser::SetUserLoginDeviceCheckVerifyRequestWait( bool bSet )
{
	m_bUserLoginDeviceCheckVerifyRequestWait = bSet;
}