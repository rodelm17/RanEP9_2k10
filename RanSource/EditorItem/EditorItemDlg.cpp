...
bool CEditorItemDlg::DataLoad()
{
	...
	CFileDialog dlg( TRUE, ".isf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
bool CEditorItemDlg::DataSave()
{
	for ( int MID = 0; MID < GLItemMan::MAX_MID; MID++ )
	{
		for ( int SID = 0; SID < GLItemMan::MAX_SID; SID++ )
		{
			...
			{
				//pile num check
				if ( pItem->sDrugOp.wPileNum > 9999 )
				{
					CDebugSet::MsgBox( GetSafeHwnd(), "warning item:[%u/%u]%s pile num: %u more than 9999", MID, SID, pItem->GetName(), pItem->sDrugOp.wPileNum );
				}
			}
		}
	}
	...
	CString szFilter = "Item Setting File (*.isf)|*.isf|";
	CFileDialog dlg( FALSE, ".isf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
		...
		if ( m_SearchDlg.DoModal() == IDOK )
		{
			if ( m_pSheetTab )
			{
				if ( m_pSheetTab->GetActivePage () != &m_pSheetTab->m_PageTree )
				{
					...
					m_pSheetTab->m_PageEdit3.ItemClear();
					m_pSheetTab->ActivePageTree( TRUE );
				}
