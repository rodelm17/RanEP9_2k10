...
char		DxEffCharParticle::NAME[MAX_PATH]	= "DxEffCharParticle"; 
...
HRESULT	DxEffCharParticle::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		//	Note : 이팩트의 Property 를 저장.
		EFFCHAR_PROPERTY_PARTICLE_101	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_PARTICLE_101) );

		m_Property.m_dwFlag			= sProp.m_dwFlag;
		m_Property.m_dwParticlePec	= sProp.m_dwParticlePec;
		m_Property.m_fRandomRange	= sProp.m_fRandomRange;
		m_Property.m_dwColor		= sProp.m_dwColor;
		m_Property.m_vSize			= sProp.m_vSize;
		m_Property.m_fLifeMin		= sProp.m_fLifeMin;
		m_Property.m_fLifeMax		= sProp.m_fLifeMax;
		m_Property.m_vRotate		= sProp.m_vRotate;
		m_Property.m_vVelocity		= sProp.m_vVelocity;
		m_Property.m_dwSequenceCol	= sProp.m_dwSequenceCol;
		m_Property.m_dwSequenceRow	= sProp.m_dwSequenceRow;
		StringCchCopy( m_Property.m_szTexture, TEX_SIZE_64, sProp.m_szTexture );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharParticle::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharParticle::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharParticle::R2LoadFile", SFile.GetFileName(), dwVer );
