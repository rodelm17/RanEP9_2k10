...
BOOL CHECK_LB_DRAG_LIKE ( DWORD dwMsg )
{	
	return (dwMsg & UIMSG_LB_DRAG) ? TRUE:FALSE;
}
...
BOOL CHECK_LB_DUP_LIKE ( DWORD dwMsg )
{
	DWORD dwUpMsg = UIMSG_LB_DUP;
	return (dwMsg & dwUpMsg)?TRUE:FALSE;
}
...
BOOL CHECK_MOUSEIN_LBDUPLIKE ( DWORD dwMsg )
{
	return (CHECK_MOUSE_IN ( dwMsg ) && CHECK_LB_DUP_LIKE ( dwMsg ));
}
