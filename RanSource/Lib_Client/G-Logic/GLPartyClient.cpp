...
void GLPartyClient::SecedeEx ( DWORD dwGaeaID )
{
	GLPARTY_CLIENT* pParty = FindMember(dwGaeaID);
	if ( !pParty )	return;
...
void GLPartyClient::SecedeSelf ()
{
	DWORD dwMyGaeaID = GLGaeaClient::GetInstance().GetCharacter()->m_dwGaeaID;

	GLPARTY_CLIENT* pParty = FindMember(dwMyGaeaID);
	if ( !pParty )	return;
...
void GLPartyClient::AuthorityEx( DWORD dwGaeaID )
{
	GLPARTY_CLIENT* pParty = FindMember(dwGaeaID);
	if ( !pParty )	return;
...
BOOL GLPartyClient::MsgPartyAuthority( NET_MSG_GENERIC* nmg )
{
	...
	if ( pNewMaster )
	{
		...
		m_sMaster.m_wLevel = pNewMaster->m_wLevel;
	}
...
BOOL GLPartyClient::MsgPartyOptionChange( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_PARTY_OPTIONTOPARTY *pNetMsg = ( GLMSG::SNET_PARTY_OPTIONTOPARTY* ) nmg;
	...
	CString strItem = ID2GAMEWORD("PARTY_MODAL_ITEM_OPTION", m_sOption.emGET_ITEM );
	CString strMoney = ID2GAMEWORD("PARTY_MODAL_MONEY_OPTION", m_sOption.emGET_MONEY);
	CString strMessage  = CInnerInterface::GetInstance().MakeString( ID2GAMEINTEXT( "PARTY_OPTION_CHANGE" ), strItem.GetString(), strMoney.GetString() );
	CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::YELLOW, strMessage.GetString() );

	if ( m_sOption.bPrivateParty )
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::YELLOW, ID2GAMEINTEXT( "PARTY_PRIVACY_CHANGE_2" ) );
	else
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::YELLOW, ID2GAMEINTEXT( "PARTY_PRIVACY_CHANGE_1" ) );
...
BOOL GLPartyClient::MsgPartyMbrDetail ( NET_MSG_GENERIC* nmg )
{
	...
	pMember->m_wLevel = pNetMsg->wLevel;
...
BOOL GLPartyClient::MsgPartyLureFb ( NET_MSG_GENERIC* nmg )
{
	...
	case EMPARTY_LURE_REFUSE:
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("PARTY_LURE_REFUSE"), pNetMsg->szCharName );
		break;
	
	case EMPARTY_LURE_FULL:
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("PARTY_LURE_FULL"), pNetMsg->szCharName );
		break;

	case EMPARTY_LURE_OTHER:
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("PARTY_LURE_OTHER"), pNetMsg->szCharName );
		break;
	...
	case EMPARTY_LURE_BLOCK_MAP_MASTER:
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMPARTY_LURE_BLOCK_MAP_MASTER") );
		break;

		/*map party setting, Juver, 2018/06/29 */
	case EMPARTY_LURE_BLOCK_MAP_TARGET:
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMPARTY_LURE_BLOCK_MAP_TARGET") );
		break;

		/* pvp club death match, Juver, 2020/11/26 */
	case EMPARTY_LURE_OTHER_GUILD:
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMPARTY_LURE_OTHER_GUILD") );
		break;
	};
...
BOOL GLPartyClient::MsgPartyMbrItemPickUp ( NET_MSG_GENERIC* nmg )
{
	...
	{
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::LIGHTSKYBLUE, &pNetMsg->sitem_link, ID2GAMEINTEXT("PARTY_MBR_PICKUP_BRD"),
			pMember->m_szName, pNetMsg->sitem_link.GetFilter() );
	}
	else
	{
