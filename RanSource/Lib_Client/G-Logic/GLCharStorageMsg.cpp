...
HRESULT GLChar::MsgReqStorageToHold ( NET_MSG_GENERIC* nmg )
{
	...
	if ( isPersonalLock( EMPERSONAL_LOCK_LOCKER ) )		return E_FAIL;
	...
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqStorageToHold Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
...
HRESULT GLChar::MsgReqStorageExHold ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pInvenData->IsOverPile(pInvenItem->sItemCustom.wTurnNum ) )
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqStorageExHold Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
...
HRESULT GLChar::MsgReqStorageSaveMoney ( NET_MSG_GENERIC* nmg )
{
	...
	if ( RPARAM::block_locker_money_add )	return S_FALSE;

	/* personal lock system, Juver, 2019/12/09 */
	if ( isPersonalLock( EMPERSONAL_LOCK_LOCKER ) )			return S_FALSE;
...
HRESULT GLChar::MsgReqStorageDrawMoney ( NET_MSG_GENERIC* nmg )
{
	...
	/*locker money control, EJCode, 2018/10/23 */
	if ( RPARAM::block_locker_money_take )	return S_FALSE;
...
HRESULT GLChar::MsgReqStorageSplit ( NET_MSG_GENERIC* nmg )
{
	...
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqStorageSplit Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
