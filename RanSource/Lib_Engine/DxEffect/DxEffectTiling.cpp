...
{
	if ( ( dwVer==0x00000106 || dwVer==0x00000105 || dwVer==0x00000103 ) && dwSize==sizeof(TILING_PROPERTY_103) )
	{
	...
	else if ( dwVer==0x00000101 && dwSize==sizeof(TILING_PROPERTY) )
	{
		memcpy ( &m_Property, pProp, dwSize );
	}
	...
	{
		CDebugSet::ErrorVersion( "DxEffectTiling::SetProperty", dwVer );
	}
}
...
HRESULT DxEffectTiling::DeleteDeviceObjects ()
{
	...
	DxStaticMaterialMeshColor_THREAD::GetInstance().DeleteList();
...
HRESULT DxEffectTiling::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if( DxStaticMeshColor_THREAD::GetInstance().m_bOctreeMode )
	{
