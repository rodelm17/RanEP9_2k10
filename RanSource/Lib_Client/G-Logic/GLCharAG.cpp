...
GLCharAG::GLCharAG(void) 
	...
	, m_bStartTeleport(false)
	, m_bValidTeleport(false)
	, m_fTeleporTimer(0.0f)
	, m_dwFriendID(0)
	, m_wItemX(0)
	, m_wItemY(0)
	...
	, m_fHWIDWatchListDisconnectTimer(0.0f)
...
//	GLAgentServer::GetInstance().CONSOLEMSG_WRITE ( "[%u]%s m_bUserFlagVerified=%d", m_dwCharID, m_szName, m_bUserFlagVerified );
//
//	/* user flag restricted, Juver, 2020/04/21 */
//	GLAgentServer::GetInstance().CONSOLEMSG_WRITE ( "[%u]%s m_bUserFlagRestricted=%d", m_dwCharID, m_szName, m_bUserFlagRestricted );
//
//#endif
...
HRESULT GLCharAG::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if( m_bStartTeleport )
	{
		m_fTeleporTimer += fElapsedTime;
		if( m_fTeleporTimer >= 10.0f )	
		{
			m_bStartTeleport = FALSE;
			m_fTeleporTimer = 0.0f;

			GLAgentServer::GetInstance().Do2FriendReq( m_dwGaeaID , m_dwFriendID, m_wItemX, m_wItemY );
		}
	}
	...
	/* HWID watchlist, Juver, 2020/05/05 */
	if ( m_bUserFlagHWIDWatchlist )
	{
		m_fHWIDWatchListDisconnectTimer += fElapsedTime;
		if ( m_fHWIDWatchListDisconnectTimer >= HWID_WATCHLIST_KICK_TIME )
		{
			m_fHWIDWatchListDisconnectTimer = 0.0f;
			HWIDWatchListDisconnect();
		}
	}

	CaptchaUpdate(fElapsedTime);
...
HRESULT GLCharAG::MsgActState ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pNetMsg->bCMD )
	{
		if ( GLAgentServer::GetInstance().GetDBMan() )
		{
			char command[1024];

			if ( pNetMsg->dwActState & EM_REQ_VISIBLENONE )
				sprintf_s( command, 1024, "visible none in map [%u/%u]", m_sCurMapID.wMainID, m_sCurMapID.wSubID );
			else if ( pNetMsg->dwActState & EM_REQ_VISIBLEOFF )
				sprintf_s( command, 1024, "visible off in map [%u/%u]", m_sCurMapID.wMainID, m_sCurMapID.wSubID );
			else
				sprintf_s( command, 1024, "visible on in map [%u/%u]", m_sCurMapID.wMainID, m_sCurMapID.wSubID );

			gm_cmd_logs *pdb_action = new gm_cmd_logs( m_dwClientID, m_dwUserID, m_dwUserLvl, m_dwCharID, m_szName, command );
			GLAgentServer::GetInstance().GetDBMan()->AddJob ( pdb_action );
