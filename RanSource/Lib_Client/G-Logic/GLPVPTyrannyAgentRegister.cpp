...
BOOL GLPVPTyrannyAgent::InsertRegister( TYRANNY_REGISTER_DATA_MAP& mapData, DWORD dwCharID, const char* szName, WORD wClass, WORD wSchool, BOOL bLateJoin )
{
	if ( !szName )							return FALSE;
	if ( dwCharID == TYRANNY_PLAYER_NULL )	return FALSE;
	if ( wClass >= GLCI_NUM_8CLASS )		return FALSE;
...
void GLPVPTyrannyAgent::RequestPlayerRegister( DWORD dwCharID, WORD wLevel, BOOL bRegister, std::string strHash )
{	
	...
	// Guild requirement check
	if  ( RPARAM::bCTFGVG )
	{
		if ( !pchar->m_dwGuild ) {
			netmsgfb.emFB = TYRANNY_REGISTER_FB_NO_CLUB;
			GLAgentServer::GetInstance().SENDTOCLIENT ( pchar->m_dwClientID, &netmsgfb );
			return;
		}
	}
	...
	if ( bRegister )
	{
		...
		if ( pchar->m_wSchool == TYRANNY_SCHOOL_SG )
			PlayerRegister( pchar->m_dwCharID, m_mapPlayerDataSG, m_vecPlayerDataQueueSG, strHash );
		else if ( pchar->m_wSchool == TYRANNY_SCHOOL_MP )
			PlayerRegister( pchar->m_dwCharID, m_mapPlayerDataMP, m_vecPlayerDataQueueMP, strHash );
		else if ( pchar->m_wSchool == TYRANNY_SCHOOL_PHX )
			PlayerRegister( pchar->m_dwCharID, m_mapPlayerDataPHX, m_vecPlayerDataQueuePHX, strHash );
		else
		{
...
void GLPVPTyrannyAgent::PlayerRegister( DWORD dwCharID, TYRANNY_REGISTER_DATA_MAP& mapData, TYRANNY_REGISTER_DATA_QUEUE_VEC& vecData, std::string strHash )
{
	PGLCHARAG pchar = GLAgentServer::GetInstance().GetCharID( dwCharID );
	if ( !pchar )	return;
	...
	if ( m_bHashCheck )
	{
		...
		if ( strHash.empty() )
		{
			netmsgfb.emFB = TYRANNY_REGISTER_FB_DEVICE_INFO_ERROR;
			GLAgentServer::GetInstance().SENDTOCLIENT ( pchar->m_dwClientID, &netmsgfb );
			return;
		}
		...
		{
			netmsgfb.emFB = TYRANNY_REGISTER_FB_DEVICE_REGISTERED;
			GLAgentServer::GetInstance().SENDTOCLIENT ( pchar->m_dwClientID, &netmsgfb );
			return;
		}
	}
	...
	{
		//insert player data
		BOOL binsert = InsertRegister( mapData, pchar->m_dwCharID, pchar->m_szName, CharClassToIndex( pchar->m_emClass ), pchar->m_wSchool, FALSE );
		if ( binsert )
		{
...
/* PVPTyranny Hash Check, Juver, 2020/07/28 */
BOOL GLPVPTyrannyAgent::PCHashCheck( std::string strHash )
{
	if ( strHash.empty() )	return FALSE;

	TYRANNY_PLAYER_HASH_DATA_MAP_ITER it = m_mapPlayerHash.find( strHash );
...
/* PVPTyranny Hash Check, Juver, 2020/07/28 */
void GLPVPTyrannyAgent::PCHashInsert( std::string strHash, DWORD dwCharID )
{
	TYRANNY_PLAYER_HASH_DATA sData;
...
/* PVPTyranny Hash Check, Juver, 2020/07/28 */
void GLPVPTyrannyAgent::PCHashDelete( DWORD dwCharID )
{
	TYRANNY_PLAYER_HASH_DATA_MAP_ITER it_b = m_mapPlayerHash.begin();
	TYRANNY_PLAYER_HASH_DATA_MAP_ITER it_e = m_mapPlayerHash.end();
