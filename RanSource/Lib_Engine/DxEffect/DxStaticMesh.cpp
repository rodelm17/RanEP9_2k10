...
void DxStaticMesh::FrameMove( const float fTime, const float fElapsedTime )
{
...
void DxStaticMesh::Render_THREAD_( const LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME &sCV, LOADINGDATALIST &listLoadingData, CRITICAL_SECTION &CSLockLoading )
{
	PROFILE_BEGIN("DxOctree::Render_THREAD_");
	COLLISION::RenderAABBTreeTHREAD( pd3dDevice, sCV, m_pMeshTree, listLoadingData, CSLockLoading );
	PROFILE_END("DxOctree::Render_THREAD_");
}

void DxStaticMesh::Render_THREAD_Alpha( const LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME &sCV, LOADINGDATALIST &listLoadingData, CRITICAL_SECTION &CSLockLoading )
{
	PROFILE_BEGIN("DxOctree::Render_THREAD_Alpha");
	DxRenderStates::GetInstance().SetOnAlphaMap ( pd3dDevice );
	COLLISION::RenderAABBTreeTHREAD( pd3dDevice, sCV, m_pMeshAlphaTree, listLoadingData, CSLockLoading );
	DxRenderStates::GetInstance().ReSetOnAlphaMap ( pd3dDevice );
	PROFILE_END("DxOctree::Render_THREAD_Alpha");
}

void DxStaticMesh::Render_THREAD_SoftAlpha( const LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME &sCV, LOADINGDATALIST &listLoadingData, CRITICAL_SECTION &CSLockLoading )
{
	PROFILE_BEGIN("DxOctree::Render_THREAD_SoftAlpha");
	DxRenderStates::GetInstance().SetOnSoftAlphaMap ( pd3dDevice );
	COLLISION::RenderAABBTreeTHREAD( pd3dDevice, sCV, m_pMeshSoftAlphaTree, listLoadingData, CSLockLoading );

	float fBias;
	fBias = -0.0001f;
	pd3dDevice->SetRenderState ( D3DRS_DEPTHBIAS,	*((DWORD*)&fBias) );
	COLLISION::RenderAABBTreeTHREAD( pd3dDevice, sCV, m_pMeshSoftAlphaTree01, listLoadingData, CSLockLoading );

	fBias = -0.0002f;
	pd3dDevice->SetRenderState ( D3DRS_DEPTHBIAS,	*((DWORD*)&fBias) );
	COLLISION::RenderAABBTreeTHREAD( pd3dDevice, sCV, m_pMeshSoftAlphaTree02, listLoadingData, CSLockLoading );
...
DxSingleTexMesh* DxStaticMesh::FindMesh( std::string _strMeshName )
{
	MAPSINGLETEXMESH_ITER iter = m_mapMesh.find( _strMeshName.c_str() );
	if ( iter != m_mapMesh.end() )
	{
		return (*iter).second;
	}

	iter = m_mapMeshALPHA.find( _strMeshName.c_str() );
	if ( iter != m_mapMeshALPHA.end() )
	{
		return (*iter).second;
	}

	iter = m_mapMeshSOFTALPHA.find( _strMeshName.c_str() );
	if ( iter != m_mapMeshSOFTALPHA.end() )
	{
		return (*iter).second;
	}

	iter = m_mapMeshSOFTALPHA01.find( _strMeshName.c_str() );
	if ( iter != m_mapMeshSOFTALPHA01.end() )
	{
		return (*iter).second;
	}

	iter = m_mapMeshSOFTALPHA02.find( _strMeshName.c_str() );
	if ( iter != m_mapMeshSOFTALPHA02.end() )
	{
		return (*iter).second;
	}
