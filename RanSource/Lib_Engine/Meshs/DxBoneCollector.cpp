...
std::string DxSkeleton::GetSkeletonFilePath( const char* szFileName, BOOL &bEncrypted )
{
	CFileFindTree* pFileTree = DxBoneCollector::GetInstance().GetFileTree();
	if ( !pFileTree )	return "";
...
HRESULT DxSkeleton::LoadFile ( const char *szFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	strFileFind = GetSkeletonFilePath( szXFileName, bEncrypted );
	if ( strFileFind.size() <= 0 )
	{
		CDebugSet::ErrorFile( "DxSkeleton::LoadFile", szXFileName );
		goto e_Exit;
	}
	...
	if ( bEncrypted )
	{
		...
		if ( cXFile.LoadFileDec( strFileFind.c_str() ) )
		{
			...
			DXFILELOADMEMORY pMem;
			pMem.lpMemory = pData;
			pMem.dSize = nSize;

			hr = pxofapi->CreateEnumObject( &pMem, DXFILELOAD_FROMMEMORY, &pxofenum );
			if (FAILED(hr))	
			{
				CDebugSet::ErrorFile( "DxSkeleton::LoadFile", szXFileName );
				goto e_Exit;
	...
	{
		hr = pxofapi->CreateEnumObject((LPVOID)strFileFind.c_str(),	DXFILELOAD_FROMFILE, &pxofenum);
		if (FAILED(hr))
		{
			CDebugSet::ErrorFile( "DxSkeleton::LoadFile", szXFileName );
			goto e_Exit;
