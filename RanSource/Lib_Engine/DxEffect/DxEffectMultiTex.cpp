...
void DxEffectMultiTex::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffectMultiTex::SetProperty", dwVer );
	}
...
void DxEffectMultiTex::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectMultiTex::LoadBufferSet", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwSAVESIZE );
		return;
	}
...
void DxEffectMultiTex::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectMultiTex::LoadBuffer", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
