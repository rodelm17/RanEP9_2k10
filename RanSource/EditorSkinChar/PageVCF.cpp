...
BEGIN_MESSAGE_MAP(CPageVCF, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BUTTON_VCF_ANIM_ADDCFG_MULTI, OnBnClickedButtonVcfAnimAddcfgMulti)
...
void CPageVCF::OnBnClickedButtonVcfSave()
{
	...
	CString strsavefile = DxVehicleDataContainer::GetInstance().GetPath();
	strsavefile += m_pData->GetFileName();
	CFileDialog dlg( FALSE, NULL, strsavefile.GetString(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterContainerVCF, this );
	dlg.m_ofn.lpstrInitialDir = DxVehicleDataContainer::GetInstance().GetPath();
	if ( dlg.DoModal() != IDOK ) return;
...
void CPageVCF::OnBnClickedButtonVcfPieceCps()
{
	...
	CFileDialog dlg( true, NULL, DxSkinPieceContainer::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterPieceVPS, this );
...
void CPageVCF::OnBnClickedButtonVcfAnimAddcfgMulti()
{
	if ( !m_pSkin )	return;
	if ( !m_pData )	return;
