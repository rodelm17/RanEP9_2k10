		...
		case FILE_SBASIC:
			{
				...
				if( bPastLoad )
				{
					...
					{
						GASSERT(sizeof(SKILL::SSKILLBASIC_112)==dwSize);

						SKILL::SSKILLBASIC_112 sOldData;
						SFile.ReadBuffer ( &sOldData, sizeof(SKILL::SSKILLBASIC_112) );
						m_sBASIC.Assign ( sOldData );
					}
#ifdef _DEBUG
					else if (dwVer==0x0113)
					{
						GASSERT(sizeof(SKILL::SSKILLBASIC_113)==dwSize);

						SKILL::SSKILLBASIC_113 sOldData;
						SFile.ReadBuffer ( &sOldData, sizeof(SKILL::SSKILLBASIC_113) );
						m_sBASIC.Assign ( sOldData );
					}
					else if (dwVer==0x0114)
					{
						GASSERT(sizeof(SKILL::SSKILLBASIC_114)==dwSize);

						SKILL::SSKILLBASIC_114 sOldData;
						SFile.ReadBuffer ( &sOldData, sizeof(SKILL::SSKILLBASIC_114) );
						m_sBASIC.Assign ( sOldData );
					}
					else if (dwVer==0x0115)
					{
						GASSERT(sizeof(SKILL::SSKILLBASIC_115)==dwSize);

						SKILL::SSKILLBASIC_115 sOldData;
						SFile.ReadBuffer ( &sOldData, sizeof(SKILL::SSKILLBASIC_115) );
						m_sBASIC.Assign ( sOldData );
					}
#endif
					else if (dwVer==0x0200)
					{
						GASSERT(sizeof(SKILL::SSKILLBASIC_200)==dwSize);

						SKILL::SSKILLBASIC_200 sOldData;
						SFile.ReadBuffer ( &sOldData, sizeof(SKILL::SSKILLBASIC_200) );
						m_sBASIC.Assign ( sOldData );
					}
		...
		case FILE_SAPPLY:
			{
				...
				{
					m_sAPPLY.LOAD_0111( SFile );
				}
				else if ( dwVer==0x0112 )
				{
					m_sAPPLY.LOAD_0112( SFile );
				}
#endif
		...
		case FILE_SLEARN:
			{
				...
				{
					GASSERT(sizeof(SKILL::SLEARN_200)==dwSize);
					SKILL::SLEARN_200 sLEARN_OLD;
					SFile.ReadBuffer ( &sLEARN_OLD, sizeof(SKILL::SLEARN_200) );
					m_sLEARN.Assign ( sLEARN_OLD );
				}
				else if ( dwVer==0x0201 )
				{
					GASSERT(sizeof(SKILL::SLEARN_201)==dwSize);
					SKILL::SLEARN_201 sLEARN_OLD;
					SFile.ReadBuffer ( &sLEARN_OLD, sizeof(SKILL::SLEARN_201) );
					m_sLEARN.Assign ( sLEARN_OLD );
				}
				else if ( dwVer==0x0202 )
				{
					GASSERT(sizeof(SKILL::SLEARN_202)==dwSize);
					SKILL::SLEARN_202 sLEARN_OLD;
					SFile.ReadBuffer ( &sLEARN_OLD, sizeof(SKILL::SLEARN_202) );
					m_sLEARN.Assign ( sLEARN_OLD );
				}
		...
		case FILE_SPECIAL_SKILL:
			{
				...
				{
					m_sSPECIAL_SKILL.LOAD_0104 ( SFile );
				}
#endif
				else if ( dwVer==0x0103 )
				{
					m_sSPECIAL_SKILL.LOAD_0103 ( SFile );
				}
				else if ( dwVer==0x0101 )
				{
...
void GLSKILL::BuildHash()
{
	//create hash
	std::stringstream sStream;
	m_sBASIC.ReadStringStream(sStream);
	m_sAPPLY.ReadStringStream(sStream);
	m_sLEARN.ReadStringStream(sStream);
	m_sEXT_DATA.ReadStringStream(sStream);
	m_sSPECIAL_SKILL.ReadStringStream(sStream);
	...
	g_hashObj.SetHashString(sStream.str().c_str());

	memset(m_szHash, 0, sizeof(m_szHash));
	StringCchCopy(m_szHash, SKILL_HASH_SIZE, g_hashObj.DoHash().GetString());
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
HRESULT GLSkillMan::LoadFile ( const char* szFile, bool bPastLoad )
{
	...
	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_SKILL_VER_1 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT2 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_SKILL_VER_2 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT3 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_SKILL_VER_3 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT_2021_SKILL_V1 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_SKILL_V1 );
...
HRESULT GLSkillMan::SyncUpdateData ()
{
	...
	for ( int j = 0; j < MAX_CLASSSKILL; j++ )
	{
		...
		for ( int k=0; k<SKILL::MAX_SKILL_LEVEL; ++k )
		{
