...
BOOL DxSkinCharData::LoadFile ( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread, bool bREPRESH )
{
	...
	if ( dwVer >= VERSION_ENCRYPT_BYTE_CRYPT )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_SKIN_CONTAINER_VER_1 );

	if ( dwVer >= VERSION_ENCRYPT_BYTE_CRYPT2 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_SKIN_CONTAINER_VER_2 );

	if ( dwVer >= ENCODE_VER_BYTE_CRYPT_2021_SKIN_CHAR_V1 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_SKIN_CHAR_V1 );

	switch ( dwVer )
	{
	case VERSION_CHF:
		LOAD_0202( SFile, pd3dDevice );
		break;

	case 0x0201:
		LOAD_0201( SFile, pd3dDevice );
		break;
...
BOOL DxSkinCharData::LOAD_0201( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//skeleton name
	SFile >> m_strSkeleton;
	...
	SFile >> m_vMin;
	SFile >> m_bWorldObj;
	SFile >> m_bForceSafeSlot;

	return TRUE;
}

BOOL DxSkinCharData::LOAD_0202( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//skeleton name
	SFile >> m_strSkeleton;
