...
int COdbcManager::SetChatBlockTime(int nUserNum, const std::string& strUserID, __time64_t tBlockTime, const std::string& strReason, int nGMUserNum, const std::string& strGMUserID)
{
	if (nUserNum <= 0) return DB_ERROR;
	...
	//TCHAR issuerUser[12];
	TCHAR szLog[1024] = {0};
	_snprintf( szLog, 1023, "{call sp_LogChatBlock(%d,'%s','%s','%s',%d,'%s',?)}", nUserNum, strUserID.c_str(), strTime.GetString(), strReason.c_str(), nGMUserNum, strGMUserID.c_str() );
	m_pUserDB->ExecuteSpInt(szLog);
...
int	COdbcManager::AllServerUserLogout(int nSvrGrp)
{
	...
	//PCID
	TCHAR szTemp2[256] = {0};
	_snprintf( szTemp2, 256, "DELETE FROM UserKey WHERE PCIDSGNum = %d", nSvrGrp );
	nReturn = m_pUserDB->ExecuteSQL(szTemp2);
	if ( nReturn != DB_OK ){
		return nReturn;
	}
