...
#include "../Utils/GetWinVer.h"
#include "../Core/NSROption.h"
...
CTextUtil::CTextUtil()
	...
	, m_bStateBlocked( FALSE )

	, m_iTextureSizeSmall( T_SIZE_SMALL )
	, m_iTextureSizeBig( T_SIZE_BIG )
	, m_dwLastTick( 0 )
{
//#if defined( KRT_PARAM ) || defined ( _RELEASED ) || defined ( KR_PARAM )
...
VOID CTextUtil::InitDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice, SIZE* sizeTexture )
{
	...
	m_iTextureSizeSmall = new_min( T_SIZE_SMALL, sizeTexture->cx );
	m_iTextureSizeSmall = new_min( m_iTextureSizeSmall, sizeTexture->cy );

	m_iTextureSizeBig = new_min( T_SIZE_BIG, sizeTexture->cx );
	m_iTextureSizeBig = new_min( m_iTextureSizeBig, sizeTexture->cy );
...
VOID CTextUtil::SetStateBlock( BOOL bPrev )
{
	if( !m_pSavedSB || !m_pTextureSB )
		return;
	...
	if( bPrev )
	{
		...
		{
			CDebugSet::ToLogFileSTR(std::string("ERROR : CCTextUtil::SetStateBlock, bPrev && m_bStateBlocked"));
		}
		m_pTextureSB->Apply();

		m_bStateBlocked = TRUE;
	}
	else
	{
		if( m_bStateBlocked )
		...
		{
			CDebugSet::ToLogFileSTR(std::string("ERROR : CTextUtil::SetStateBlock, !bPrev && !m_bStateBlocked"));
		}
...
BOOL CTextUtil::DrawText( const TCHAR* strText, CD3DFontX* pFont, D3DCOLOR dwColor, FLOAT fX, FLOAT fY, FLOAT fZoom )
{
    if( !m_pd3dDevice )
		return FALSE;

	CTextGroup* pGroup = GetTextGroup( pFont );
	if( pGroup && pGroup->DrawText( strText, dwColor, fX, fY, fZoom ) )
		return TRUE;
