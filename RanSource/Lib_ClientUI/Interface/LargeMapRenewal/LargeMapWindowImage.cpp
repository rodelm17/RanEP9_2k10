...
void CLargeMapWindowImage::CreateSubControl ()
{
	...
	{	
		...
		pScrollBar->CreateBaseScrollBar ( "LARGEMAP_SCROLLBAR_H2", false );
...

	{
		...
		pScrollBar->CreateBaseScrollBar ( "LARGEMAP_SCROLLBAR_V2" );
		RegisterControl ( pScrollBar );
	...
	pMapImage->CreateSub ( this, "LARGEMAP_IMAGE2", UI_FLAG_XSIZE | UI_FLAG_YSIZE, LARGEMAP_IMAGE );
	RegisterControl ( pMapImage );
	m_pMap = pMapImage;

	CUIControl* pPlayerMark = new CUIControl;
	pPlayerMark->CreateSub ( this, "LARGEMAP_MARK2", UI_FLAG_DEFAULT, PLAYER_MARK );		
	RegisterControl ( pPlayerMark );
	m_pPlayerMark = pPlayerMark;

	CUIControl* pMousePosBack = new CUIControl;
	pMousePosBack->CreateSub ( this, "LARGEMAP_MOUSEPOS_BACK2", UI_FLAG_BOTTOM | UI_FLAG_RIGHT );
	RegisterControl ( pMousePosBack );		
	m_pMousePosBack = pMousePosBack;

	CBasicTextBox*	pTextBox = new CBasicTextBox;
	pTextBox->CreateSub ( this, "LARGEMAP_MOUSEPOS_TEXT2", UI_FLAG_BOTTOM | UI_FLAG_RIGHT );
	...
	pDirection->CreateSub ( this, "LARGEMAP_DIRECTION2", UI_FLAG_BOTTOM, LARGEMAP_DIRECTION );		
	...
	for ( int i = 0; i < MAXPARTY; ++i )
	{
		...
		pMark->CreateSub( this, strMark[i].GetString(), UI_FLAG_DEFAULT );
	...
	m_pTargetMark->CreateSub( this, "TARGET_MARK_2", UI_FLAG_DEFAULT );
...

		{
			...
			if ( !m_CrowList.empty())
			{
				...
				for( ; iter != iter_end; ++iter )
				{
					...
					CInnerInterface::GetInstance().GetLargeMapWindow()->m_bSUBSETTING[2] ? pSlot->SetDispName(false) : pSlot->SetDispName(false);
...
void CLargeMapWindowImage::TranslateMeg( UIGUID cID, DWORD dwMsg, CUIControl * pUIControl )
{
	...
	if ( CHECK_MOUSE_IN ( dwMsg ) )					
	{
		...
		bool bPreview = ( GLGaeaClient::GetInstance().GetActiveMapID() == m_sSelectedMapID ) ? false : true;
		if( CHECK_LB_UP_LIKE( dwMsg ) && !bPreview )				
			OnLButtonUp();
	}
	else if ( CHECK_RB_UP_LIKE ( dwMsg ) || CHECK_MB_UP_LIKE ( dwMsg ) )
	{												
...
void CLargeMapWindowImage::LoadMob(PCROWDATA pCrow, D3DXVECTOR3 vPos, GLCHARLOGIC_CLIENT *pCHAR )
{
	if ( !pCrow ) return;

	m_NEWID++;
	CLargeMapWindowSlotRender* pSlot = new CLargeMapWindowSlotRender;
	pSlot->CreateSub( this, "SKILLTIME_UNIT", UI_FLAG_DEFAULT, m_NEWID );
...
void CLargeMapWindowImage::ClearMobList()
{
	...
	if ( !m_CrowList.empty() )
	{
		...
		for( ; iter != iter_end; ++iter )
		{
			...
			CLargeMapWindowSlotRender* pSlot = iter->second;
			if( !pSlot )				continue;
			UIGUID cUID = pSlot->GetWndID();

			pSlot->SetVisibleSingle(FALSE);
			pSlot->UnLoadAll();

			DeleteControl( cUID, 0 );
		}
	}
...
void CLargeMapWindowImage::SetGateList( SNATIVEID sMapID )
{
	...
	if ( pMapNode )
	{
		...
		{
			DxLandGateMan* pGateMan = &cLevelFile.GetLandGateMan();
			if ( !pGateMan ) return;
