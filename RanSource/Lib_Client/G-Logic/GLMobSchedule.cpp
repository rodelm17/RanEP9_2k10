...
void GLMobSchedule::operator= ( const GLMobSchedule &MobSch )
{
	...
	m_dummy_var_1 = MobSch.m_dummy_var_1;
	m_dummy_var_2 = MobSch.m_dummy_var_2;
	m_dummy_var_3 = MobSch.m_dummy_var_3;
	m_dummy_var_4 = MobSch.m_dummy_var_4;
	m_dummy_var_5 = MobSch.m_dummy_var_5;
	m_dummy_var_6 = MobSch.m_dummy_var_6;
	m_dummy_var_7 = MobSch.m_dummy_var_7;
...
HRESULT GLMobSchedule::RenderGenPos ( LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwRenderColor /*= 0xffffffff*/ )
{
	...
	if ( pCrow->m_sBasic.m_emCrow == CROW_MATERIAL )	
		dwColor = 0xffff0000;
...
bool GLMobSchedule::IsRegen()
{
	...
	if( GetUseRegenTime() )
	{
		...
		for( int i = 0; i < 7; i++ )
		{
			//GetDayOfWeek() returns 1~7 starting sunday
...
bool GLMobSchedule::CheckCollision( NavigationMesh* pNaviMesh )
{
	...
	pNaviMesh->IsCollision( m_pAffineParts->vTrans + D3DXVECTOR3(0,+5,0), m_pAffineParts->vTrans - D3DXVECTOR3(0,+5,0), vStart, dwStartID, bCol );
	...
	for( ; pCur; pCur = pCur->pNext )
	{
		...
		pNaviMesh->IsCollision ( pCur->Data.vPos + D3DXVECTOR3(0,+5,0), pCur->Data.vPos - D3DXVECTOR3(0,+5,0), vEndPos, dwEndID, bCol );
		if( !bCol )
		{
			return false;
			break;
...
GLMobScheduleMan::GLMobScheduleMan() 
	: m_pLandMan(NULL)
	, m_bNoRespawn(FALSE) /*no respawn, Juver, 2018/06/29 */
{	
...
SGLNODE<GLMobSchedule*>* GLMobScheduleMan::FindMobSch ( DWORD dwGlobID )
{
	SGLNODE<GLMobSchedule*>* pCur = m_GLMobSchList.m_pHead;
	for ( ; pCur; pCur = pCur->pNext )
	{
		if ( pCur->Data->m_dwGlobID == dwGlobID )
		{
			return pCur;
		}
	}
...
GLMobSchedule* GLMobScheduleMan::FindMobScheduleCrowID ( const SNATIVEID& sCrowID )
{
	SGLNODE<GLMobSchedule*>* pCur = m_GLMobSchList.m_pHead;
...
HRESULT GLMobScheduleMan::FrameMove ( float fTime, float fElapsedTime )
{
	...
	for ( ; pCur; pCur = pCur->pNext )
	{
		...
		if ( dwGlobID  != UINT_MAX )
		{
			...
			/*no respawn, Juver, 2018/06/29 */
			if ( pMobSch->m_dwVolatFlags & GLMobSchedule::EM_EXMODE || m_bNoRespawn )
			{
...
void GLMobScheduleMan::SetLandMan ( GLLandMan* pLandMan )
{
	...
	if ( m_pLandMan )
	{
		SLEVEL_ETC_FUNC *petc_func = m_pLandMan->GetLevelEtcFunc();
		if( petc_func )	
			m_bNoRespawn = petc_func->m_bUseFunction[EMETCFUNC_NORESPAWN];	
	}
}
