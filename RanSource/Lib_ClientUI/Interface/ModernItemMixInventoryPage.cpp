...
const INT CModernItemMixInvenPage::nSTARTLINE = 0;	
const INT CModernItemMixInvenPage::nOUTOFRANGE = -1;

CModernItemMixInvenPage::CModernItemMixInvenPage()
...
void CModernItemMixInvenPage::CreateSubControl()
{
	CString strSlotKeyword[ nMAX_ONE_VIEW_SLOT ] =
	{
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT0",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT1",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT2",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT3",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT4",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT5",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT6",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT7",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT8",
		"MODERN_ITEMMIX_INVENTORY_WINDOW_ITEM_SLOT9",
...
CItemSlotEx* CModernItemMixInvenPage::CreateItemSlot( CString strKeyword, UIGUID ControlID )
{
...
CUIControl*	CModernItemMixInvenPage::CreateControl( const TCHAR* szControl )
{
...
void CModernItemMixInvenPage::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CModernItemMixInvenPage::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
...
void CModernItemMixInvenPage::ResetItemSlotRender( int nStart, int nEnd )
{
	if ( nEnd < 0 )			return;
...
void CModernItemMixInvenPage::SetItemSlotRender( int nStart, int nEnd )
{
	if ( nEnd < 0 ) return ;
	if ( nStart > nEnd ) return ;
...
void CModernItemMixInvenPage::SetItemIndex( INT nPosX, INT nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

void CModernItemMixInvenPage::GetItemIndex( INT* pnPosX, INT* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

void CModernItemMixInvenPage::LoadItemPage( GLInventory &ItemData )
{
...
void CModernItemMixInvenPage::LoadItem( SINVENITEM& ref_InvenItem )
{	
...
void CModernItemMixInvenPage::UnLoadItemPage()
{
...
void CModernItemMixInvenPage::UnLoadItem( INT nPosX, INT nPosY )
{
	m_pItemSlotArray[ nPosY ]->ResetItemImage( nPosX );
}

void CModernItemMixInvenPage::ResetAllFlipItem()
{
	for( WORD y=0; y<EM_INVENSIZE_Y; ++y )
		for( WORD x=0; x<EM_INVENSIZE_X; ++x )
...
void CModernItemMixInvenPage::UpdateFlipItem( GLInventory& ref_Inventory )
{
	ResetAllFlipItem();
...
void CModernItemMixInvenPage::UpdateFlipItem( SINVEN_POS& ref_GarbageItem )
{
...
void CModernItemMixInvenPage::UpdateBLOCK()
{
...
SINVENITEM& CModernItemMixInvenPage::GetItem( INT nPosX, INT nPosY )
{
	return m_pItemSlotArray[ nPosY ]->GetItemImage( nPosX );
}

void CModernItemMixInvenPage::SetOneViewSlot( const INT& nMAX_ONE_VIEW )
{
