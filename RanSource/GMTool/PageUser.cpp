...
BOOL CPageUser::OnInitDialog()
{
	...
	m_List.InsertColumn( 0, "UserNumber", LVCFMT_LEFT, ( 8* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 1, "UserName", LVCFMT_LEFT, ( 10* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 2, "UserEmail", LVCFMT_LEFT, ( 13* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 3, "FullName", LVCFMT_LEFT, ( 12* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 4, "UserType", LVCFMT_LEFT, ( 7* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 5, "LoginState", LVCFMT_LEFT, ( 7* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 6, "Available", LVCFMT_LEFT, ( 6* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 7, "UserBlock", LVCFMT_LEFT, ( 6* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 8, "UserBlockDate", LVCFMT_LEFT, ( 13* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 9, "ChatBlockDate", LVCFMT_LEFT, ( 13* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 10, "Points", LVCFMT_LEFT, ( 5* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
...
void CPageUser::OnBnClickedUserPageButtonSearch()
{
	...
	case USERPAGE_SEARCH_TYPE_USEREMAIL:
		{
			...
			if ( GMTOOL_GLOBAL::IsBadString2( strSearch.c_str() ) ){
				CDebugSet::MsgBoxAfx( "Search contains invalid characters!" );
				GotoDlgCtrl(GetDlgItem(IDC_USER_PAGE_EDIT_SEARCH)); 
				return;
			}
	...
	case USERPAGE_SEARCH_TYPE_USER_FULLNAME:
		{
			if( strSearch.size() <= 0 ){
				CDebugSet::MsgBoxAfx( "Invalid Search!" );
				GotoDlgCtrl(GetDlgItem(IDC_USER_PAGE_EDIT_SEARCH)); 
			...
			if ( GMTOOL_GLOBAL::IsBadString3( strSearch.c_str() ) ){
				CDebugSet::MsgBoxAfx( "Search contains invalid characters!" );
				GotoDlgCtrl(GetDlgItem(IDC_USER_PAGE_EDIT_SEARCH)); 
				return;
			}
			...
			CGMToolOdbcBase::GetInstance()->UserFind_UserFullName( strSearch.c_str(), mapUserSimple, GetSearchLimitNum( emLIMIT ) );
		}break;
	...
	case USERPAGE_SEARCH_TYPE_PCID_HWID:
		{
			if( strSearch.size() <= 0 ){
				CDebugSet::MsgBoxAfx( "Invalid Search!" );
				GotoDlgCtrl(GetDlgItem(IDC_USER_PAGE_EDIT_SEARCH)); 
			...
			CGMToolOdbcBase::GetInstance()->UserFind_UserPCID_HWID( strSearch.c_str(), mapUserSimple, GetSearchLimitNum( emLIMIT ) );
		}break;

	case USERPAGE_SEARCH_TYPE_PCID_MAC:
		{
			if( strSearch.size() <= 0 ){
				CDebugSet::MsgBoxAfx( "Invalid Search!" );
				GotoDlgCtrl(GetDlgItem(IDC_USER_PAGE_EDIT_SEARCH)); 
			...
			CGMToolOdbcBase::GetInstance()->UserFind_UserPCID_MAC( strSearch.c_str(), mapUserSimple, GetSearchLimitNum( emLIMIT ) );
		}break;

	case USERPAGE_SEARCH_TYPE_RESTRICTED:
		{
			CGMToolOdbcBase::GetInstance()->UserFind_UserRestricted( mapUserSimple, GetSearchLimitNum( emLIMIT ) );
		}break;
	...
	for( int i=0; mapiter != mapUserSimple.end(); ++mapiter, ++i ){
		...
		m_List.SetItemText( i, 3, sDATA.szUserFullName );
		m_List.SetItemText( i, 4, _HLIB::cstringformat( "%s", COMMENT::USER_TYPE[ NSUSER_TYPE::UserTypeToUserTypeIndex((NSUSER_TYPE::EMUSER_TYPE)sDATA.wUserType)].c_str() ).GetString() );
		m_List.SetItemText( i, 5, GMTOOL_GLOBAL::strUserStatus[sDATA.bUserLoginState].c_str() );
		m_List.SetItemText( i, 6, GMTOOL_GLOBAL::strBasic[sDATA.bUserAvailable].c_str() );
		m_List.SetItemText( i, 7, GMTOOL_GLOBAL::strBasic[sDATA.bUserBlock].c_str() );
		m_List.SetItemText( i, 8, _HLIB::cstring_timet12( sDATA.tUserBlockDate ).GetString() );
		m_List.SetItemText( i, 9, _HLIB::cstring_timet12( sDATA.tChatBlockDate ).GetString() );
		m_List.SetItemText( i, 10, _HLIB::cstring_dword( sDATA.dwUserPoint ).GetString() );
