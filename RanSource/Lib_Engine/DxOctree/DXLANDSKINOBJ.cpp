#include "stdafx.h"
#include "../Common/SerialFile.h"

#include "../Core/NSRParam.h"
#include "../DxCommon/DxMethods.h"
#include "../DxCommon/DxShadowMap.h"
#include "../DxCommon/DxSoftShadowMap.h"

#include "./DxLandSkinObj.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif


const DWORD	DXLANDSKINOBJ::VERSION = 0x0103;
DWORD		DXLANDSKINOBJ::SIZE = sizeof(char)*LANDSKINOBJ_MAXSZ +
								sizeof(char)*MAX_PATH +
								sizeof(D3DXMATRIX) +
								sizeof(D3DXVECTOR3) +
								sizeof(D3DXVECTOR3) +
								sizeof(D3DXVECTOR3) +
								sizeof(D3DXVECTOR3);

float		DXLANDSKINOBJ::m_fTime = 0.0f;
float		DXLANDSKINOBJ::m_fElapsedTime = 0.0f;

DXLANDSKINOBJ::DXLANDSKINOBJ(void) :
	m_vMax(3,18,3),
	m_vMin(-3,0,-3),
	m_vMaxOrg(3,18,3),
	m_vMinOrg(-3,0,-3),
	m_pSkinCharData(NULL),
	m_pSkinChar(NULL),
	m_pNext(NULL),
	m_bRender(TRUE)
{
	StringCchCopy( m_szName, LANDSKINOBJ_MAXSZ, "�̸�����" );
	memset( m_szFileName, 0, sizeof(char)*MAX_PATH );

	D3DXMatrixIdentity( &m_matWorld );
}

DXLANDSKINOBJ::~DXLANDSKINOBJ(void)
{
	SAFE_DELETE(m_pSkinChar);

	SAFE_DELETE(m_pNext);
}

HRESULT DXLANDSKINOBJ::PlaySkinChar ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( !m_pSkinCharData )
	{
		m_pSkinCharData = DxSkinCharDataContainer::GetInstance().LoadData( m_szFileName, pd3dDevice, FALSE );
		if ( !m_pSkinCharData )		return E_FAIL;
	}
	
	if ( !m_pSkinChar )
	{
		m_pSkinChar = new DxSkinChar;
		m_pSkinChar->SetCharData ( m_pSkinCharData, pd3dDevice );
	}

	return S_OK;
}

HRESULT DXLANDSKINOBJ::SetSkinChar ( char* szName, const D3DXMATRIX& matWorld, char* szFile, LPDIRECT3DDEVICE9 pd3dDevice )
{
	HRESULT hr;
	GASSERT(szFile);

	StringCchCopy( m_szName, LANDSKINOBJ_MAXSZ, szName );
	StringCchCopy( m_szFileName, MAX_PATH, szFile );

	m_vMax = m_vMaxOrg;
	m_vMin = m_vMinOrg;
	m_matWorld = matWorld;
	COLLISION::TransformAABB( m_vMax, m_vMin, m_matWorld );

	hr = PlaySkinChar ( pd3dDevice );
	if (FAILED(hr) )	return hr;

	SetAABBBOX( m_vMaxOrg, m_vMinOrg );

	m_vMax = m_vMaxOrg;
	m_vMin = m_vMinOrg;
	COLLISION::TransformAABB ( m_vMax, m_vMin, m_matWorld );

	return S_OK;
}

void DXLANDSKINOBJ::SetAABBBOX( D3DXVECTOR3 &vMax, D3DXVECTOR3 &vMin )
{
	if( m_pSkinChar )
	{
		vMax = m_pSkinChar->m_vMax;
		vMin = m_pSkinChar->m_vMin;
	}
}

HRESULT DXLANDSKINOBJ::Load ( CSerialFile	&SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
	DWORD dwVer, dwSize;
	
	SFile >> dwVer;
	SFile >> dwSize;

	if ( dwVer == 0x0104 )
	{
		SFile.ReadBuffer( m_szName, sizeof(char)*LANDSKINOBJ_MAXSZ );
		SFile.ReadBuffer( m_szFileName, sizeof(char)*MAX_PATH );
		SFile >> m_vMax;
		SFile >> m_vMaxOrg;
		SFile >> m_vMin;
		SFile >> m_vMinOrg;
		SFile.ReadBuffer( m_matWorld, sizeof(D3DXMATRIX) );

		//could be render distance
		float f1, f2;
		SFile >> f1;
		SFile >> f2;

		//same data as dxpiece
		DWORD _dwVER(0);
		DWORD _dwSIZE(0);
		SFile >> _dwVER;
		SFile >> _dwSIZE;
		DWORD dwCur = SFile.GetfTell ();
		SFile.SetOffSet ( dwCur+_dwSIZE );
	}
	else if ( dwVer == VERSION )
	{
		SFile.ReadBuffer( m_szName, sizeof(char)*LANDSKINOBJ_MAXSZ );
		SFile.ReadBuffer( m_szFileName, sizeof(char)*MAX_PATH );
		SFile >> m_vMax;
		SFile >> m_vMaxOrg;
		SFile >> m_vMin;
		SFile >> m_vMinOrg;
		SFile.ReadBuffer( m_matWorld, sizeof(D3DXMATRIX) );
	}
	else if ( dwVer == 0x0102 )
	{
		SFile.ReadBuffer( m_szName, sizeof(char)*LANDSKINOBJ_MAXSZ );
		SFile.ReadBuffer( m_szFileName, sizeof(char)*MAX_PATH );
		SFile.ReadBuffer( m_matWorld, sizeof(D3DXMATRIX) );

		SFile >> m_vMax;
		SFile >> m_vMin;
		SFile >> m_vMaxOrg;
		SFile >> m_vMinOrg;
	}
	else if ( dwVer == 0x0101 )
	{
		DXAFFINEPARTS m_sAffineParts;

		SFile.ReadBuffer ( m_szName, sizeof(char)*LANDSKINOBJ_MAXSZ );
		SFile.ReadBuffer ( m_szFileName, sizeof(char)*MAX_PATH );
		SFile.ReadBuffer ( &m_sAffineParts, sizeof(DXAFFINEPARTS) );

		D3DXMatrixCompX( m_matWorld, m_sAffineParts );

		SFile >> m_vMax;
		SFile >> m_vMin;

		SFile >> m_vMaxOrg;
		SFile >> m_vMinOrg;
	}
	else
	{
		CDebugSet::ErrorVersion( "DXLANDSKINOBJ::Load", SFile.GetFileName(), dwVer );
		SFile.SetOffSet ( SFile.GetfTell() + dwSize );
		hr = E_FAIL;
	}

	return hr;
}

HRESULT DXLANDSKINOBJ::Save ( CSerialFile	&SFile )
{
	SFile << (DWORD)VERSION;
	SFile << (DWORD)SIZE;		// ���Ѵٸ� �����ؾ���.

	SFile.WriteBuffer ( m_szName, sizeof(char)*LANDSKINOBJ_MAXSZ );
	SFile.WriteBuffer ( m_szFileName, sizeof(char)*MAX_PATH );
	SFile << m_vMax;
	SFile << m_vMaxOrg;
	SFile << m_vMin;
	SFile << m_vMinOrg;
	SFile.WriteBuffer ( m_matWorld, sizeof(D3DXMATRIX) );

	return S_OK;
}

HRESULT DXLANDSKINOBJ::Render ( LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME *pCV )
{
	if( !m_bRender ) return S_OK;

	// DEBUG LOGGING: Log every render call
	// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
	if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
		CDebugSet::ToLogFile( "DXLANDSKINOBJ::Render CALLED for: %s (FileName: %s)", m_szName, m_szFileName );
	}

	HRESULT hr;

	if ( !m_pSkinChar )
	{
		// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
		if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
			CDebugSet::ToLogFile( "  - Creating SkinChar..." );
		}
		hr = PlaySkinChar ( pd3dDevice );
		if (FAILED(hr) )
		{
			// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
			if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
				CDebugSet::ToLogFile( "  - ERROR: PlaySkinChar failed with HR: %d", hr );
			}
			return hr;
		}
	}

	if ( m_pSkinChar )
	{
		// Log character information
		// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
		if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
			CDebugSet::ToLogFile( "  - SkinChar created successfully" );
			CDebugSet::ToLogFile( "  - Character has %d equipment pieces", PIECE_SIZE );
		}
		
		// AUTO +9 UPGRADE FOR BACKGROUND CHARACTERS
		// Set +9 upgrade level for all equipment pieces that exist
		// Note: Grind Level 5 = +9 effects in-game (threshold system)
		// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
		if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
			CDebugSet::ToLogFile( "  - Applying +9 upgrades to equipment pieces (Grind Level 5)..." );
		}
		
		// Check each equipment piece
		for ( int i = 0; i < PIECE_SIZE; i++ )
		{
			if ( m_pSkinChar->GetPiece(i) && m_pSkinChar->GetPiece(i)->GetFileName() && strlen(m_pSkinChar->GetPiece(i)->GetFileName()) > 0 )
			{
				// Get current grind level
				int currentLevel = m_pSkinChar->GetPiece(i)->GetGrindLevel();
				// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
				if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
					CDebugSet::ToLogFile( "    Piece %d: %s (Grind Level: %d -> 5)", i, m_pSkinChar->GetPiece(i)->GetFileName(), currentLevel );
				}
				
				// Set to +9 (grind level 5 = +9 effects in-game)
				m_pSkinChar->GetPiece(i)->SetGrindLevel(5);
				
				// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
				if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
					CDebugSet::ToLogFile( "      - Piece %d upgraded to +9 (Grind Level 5)", i );
				}
			}
			else
			{
				// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
				if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
					CDebugSet::ToLogFile( "    Piece %d: NO FILE", i );
				}
			}
		}
		
		// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
		if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
			CDebugSet::ToLogFile( "  - +9 upgrades applied to all equipment pieces (Grind Level 5)" );
		}

		hr = m_pSkinChar->FrameMove ( m_fTime, m_fElapsedTime );
		if ( FAILED(hr) )
		{
			// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
			if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
				CDebugSet::ToLogFile( "  - ERROR: FrameMove failed with HR: %d", hr );
			}
			return hr;
		}

		hr = m_pSkinChar->Render ( pd3dDevice, m_matWorld );
		if ( FAILED(hr) )
		{
			// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
			if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
				CDebugSet::ToLogFile( "  - ERROR: Render failed with HR: %d", hr );
			}
			return hr;
		}

		DxShadowMap::GetInstance().RenderShadowCharMob ( m_pSkinChar, m_matWorld, pd3dDevice );		// 그림자 그림자

		//causes graphics glitch
		//DxSoftShadowMap::GetInstance().RenderShadowCharMob ( m_pSkinChar, m_matWorld, pd3dDevice );
	}
	else
	{
		// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
		if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
			CDebugSet::ToLogFile( "  - ERROR: m_pSkinChar is NULL after PlaySkinChar" );
		}
	}

	// Performance optimization: Only log if enabled - Added by Ace17 30/08/2025
	if (RPARAM::bEnableDebugLogging && RPARAM::bEnableRenderLogs) {
		CDebugSet::ToLogFile( "DXLANDSKINOBJ::Render COMPLETED for: %s", m_szName );
	}

	return S_OK;
}

HRESULT DXLANDSKINOBJ::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( m_pSkinChar )
	{
		return m_pSkinChar->RestoreDeviceObjects ( pd3dDevice );
	}

	return S_OK;
}

HRESULT DXLANDSKINOBJ::InvalidateDeviceObjects ()
{
	if ( m_pSkinChar )
	{
		return m_pSkinChar->InvalidateDeviceObjects ();
	}

	return S_OK;
}

