...
BEGIN_MESSAGE_MAP(CPageABF, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_BUTTON_ABF_ANIM_ADDCFG_MULTI, OnBnClickedButtonAbfAnimAddcfgMulti)
...
{
	CFileDialog dlg ( TRUE,NULL, DxAttBoneDataContainer::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterContainerABF, this );
...
void CPageABF::OnBnClickedButtonAbfSave()
{
	...
	CString strsavefile = DxAttBoneDataContainer::GetInstance().GetPath();
	strsavefile += m_pData->GetFileName();
	CFileDialog dlg( FALSE, NULL, strsavefile.GetString(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterContainerABF, this );
	dlg.m_ofn.lpstrInitialDir = DxAttBoneDataContainer::GetInstance().GetPath();
	if ( dlg.DoModal() != IDOK ) return;
...
void CPageABF::OnBnClickedButtonAbfPieceCps()
{
	...
	CFileDialog dlg( true, NULL, DxSkinPieceContainer::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterPieceAPS, this);
...
void CPageABF::OnBnClickedButtonAbfAnimAddcfgMulti()
{
	if ( !m_pSkin )	return;
	if ( !m_pData )	return;
