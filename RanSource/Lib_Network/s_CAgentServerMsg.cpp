...
{	
	...
	case NET_MSG_USER_ACCOUNT_PAGE_INIT:	UserAccountPageInit( pMsg );		break;	

	/* login device check, Juver, 2020/11/06 */
	case NET_MSG_USER_ACCOUNT_PAGE_LOGIN_DEVICE_CHECK_SET:
		UserAccountPageLoginDeviceCheckSet( pMsg );		
		break;	

	case NET_MSG_USER_LOGIN_DEVICE_CHECK_VERIFY_REQUEST_INIT:			
		UserLoginDeviceCheckVerifyRequestInit( pMsg );			
		break;

	case NET_MSG_USER_LOGIN_DEVICE_CHECK_VERIFY_REQUEST_ACTION:		
		UserLoginDeviceCheckVerifyRequestAction( pMsg );			
		break;
		
	/*dmk14 ingame web*/
	case NET_MSG_FORGOTPASS:				MsgForgotPass( pMsg );				break;
	case NET_MSG_CHANGEPASS:				MsgForgotChangePass( pMsg );		break;
	case NET_MSG_SELCHAR_CHANGEPASS:		MsgSelcharChangePass( pMsg );		break;
	case NET_MSG_SELCHAR_CHANGEPIN:			MsgSelcharChangePin( pMsg );		break;
	case NET_MSG_REQ_GAMETIME_CONVERT:		MsgReqGameTimeConvert( pMsg );		break;
	case NET_MSG_SELCHAR_RESETPIN:			MsgSelcharResetPin( pMsg );			break;
	case NET_MSG_SELCHAR_TOPUP:				MsgSelcharTopUp( pMsg );			break;
	case NET_MSG_SELCHAR_CHANGEMAIL:		MsgSelcharChangeMail( pMsg );		break;
	...
	case NET_MSG_GCTRL_2_FRIEND_REQ_CANCEL:
	...
	case NET_MSG_GCTRL_PING_PACKET_AGENT:
	...
	case NET_MSG_GCTRL_GM_COMMAND_EVENT_CONTRI_TYRANNY:
	case NET_MSG_GCTRL_GM_COMMAND_EVENT_CONTRI_SCHOOL_WAR:
	case NET_MSG_GCTRL_GM_COMMAND_EVENT_CONTRI_CAPTURE_THE_FLAG:

	case NET_MSG_GCTRL_GM_COMMAND_KICKOUT:
	
	/*ecall function Arc Development 08-16-2024*/
	case NET_MSG_CALL_GUILD_MEM:
	case NET_MSG_WOE_RANKING_AGT:
	case NET_MSG_CALL_MOVE2CHAR_AG:	/*ecall function Arc Development 08-16-2024*/
	...
	case NET_MSG_GCTRL_PARTY_FINDER_SEARCH:
	case NET_MSG_GCTRL_PARTY_FINDER_JOIN:
	case NET_MSG_GCTRL_PARTY_FINDER_REPLY:
	...
	case NET_MSG_GCTRL_GM_PCID:
	case NET_MSG_GCTRL_GM_PCID_REPLY:
	case NET_MSG_GCTRL_GM_PCID_REPLY2:
	...
	case NET_MSG_GCTRL_PVP_CLUB_DEATH_MATCH_F2A_FIELD_OUT_FB:
		{
			DWORD dwAgentClient = dwClient - m_pClientManager->GetMaxClient();
			MsgPVPClubDeathMatchReBirthOutFb ( nmg, dwAgentClient, dwGaeaID );
		}break;
	...
	case NET_MSG_GCTRL_PVP_PBG_F2A_FIELD_OUT_FB:
		{
			DWORD dwAgentClient = dwClient - m_pClientManager->GetMaxClient();
			MsgPVPPBGReBirthOutFb ( nmg, dwAgentClient, dwGaeaID );
		}
		break;
...
void CAgentServer::OtherMsgProcess(MSG_LIST* pMsg)
{
	...
	case NET_MSG_GCTRL_CODEX_NOTIFY_AGENT:

		/*character disconnect function, EJCode, 2018/11/25 */
	case NET_MSG_GCTRL_CHARACTER_DISCONNECT_FIELD_TO_AGENT:
	/////////////////////////////////////////////////////////////////////////////
	// LG-7 GlobalRanking
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_KILL_UPDATE:
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_RICH_UPDATE:
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_RESU_UPDATE:/*Top Resu, Mhundz */
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_MMR_UPDATE:/*Top MMR, Mhundz */
	/////////////////////////////////////////////////////////////////////////////
	...
	case NET_MSG_GCTRL_CLUB_SCORE_KILL_FLD_2_AGT:
	case NET_MSG_GCTRL_CLUB_SCORE_RESU_FLD_2_AGT:
...
void CAgentServer::MsgGameJoin(MSG_LIST* pMsg)
{
	...
	bool bUserFlagVerified = m_pClientManager->GetUserFlagVerified(dwClient);

	/* user flag restricted, Juver, 2020/04/20 */
	bool bUserFlagRestricted = m_pClientManager->GetUserFlagRestricted(dwClient);

	/* HWID watchlist, Juver, 2020/05/05 */
	bool bUserFlagHWIDWatchlist = m_pClientManager->GetUserFlagHWIDWatchList(dwClient);
	
	/*dmk14 ingame web*/
	DWORD dwPPoints = m_pClientManager->GetPremiumPoints(dwClient);
	DWORD dwVPoints = m_pClientManager->GetCombatPoints(dwClient);
...
int CAgentServer::MsgLobbyCharJoinField(MSG_LIST* pMsg)
{
	...
	::StringCchCopy( MsgConnect.szServerIP, MAX_IP_LENGTH+1, CCfg::GetInstance()->GetProxyIpField(pField->szServerIP, pField->szServerIPProxy) );
...
void CAgentServer::MsgCreateCharacter( MSG_LIST* pMsg )
{
	...
	/* class create control, Juver, 2018/02/28 */
	if ( !RPARAM::bCreateGunner && ( nIndex == GLCI_GUNNER_M || nIndex == GLCI_GUNNER_W ) ) 
	{
		nncfd.nResult = EM_CHA_NEW_FB_SUB_MAX;
		SendClient(dwClient, &nncfd);
		return;
	...
	/* class create control, Juver, 2018/02/28 */
	if ( !RPARAM::bCreateAssassin && ( nIndex == GLCI_ASSASSIN_M || nIndex == GLCI_ASSASSIN_W ) )
	{
		nncfd.nResult = EM_CHA_NEW_FB_SUB_MAX;
		SendClient(dwClient, &nncfd);
		return;
	...
	/* class create control, Juver, 2018/02/28 */
	if ( !RPARAM::bCreateMagician && ( nIndex == GLCI_TRICKER_M || nIndex == GLCI_TRICKER_M ) )
	{
		nncfd.nResult = EM_CHA_NEW_FB_SUB_MAX;
		SendClient(dwClient, &nncfd);
		return;
...
/* pvp club death match, Juver, 2020/11/26 */
void CAgentServer::MsgPVPClubDeathMatchReBirthOutFb	(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	if (nmg == NULL) return;
	...
	//CConsoleMessage::GetInstance()->Write(
	//	_T("MsgPVPClubDeathMatchReBirthOutFb ClientID[%u], GaeaID[%u]"), dwClientID, dwGaeaID );
...
void CAgentServer::MsgPVPPBGReBirthOutFb(NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID)
{
	if (nmg == NULL) return;
...
void CAgentServer::MsgGmPkMode ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if ( NSUSER_TYPE::USER_TYPE_GM1 <= nUSERLVL )
	{
		SendAllChannel ( nmg );
	}
}
...
void CAgentServer::MsgGmwherePcMap ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if ( NSUSER_TYPE::USER_TYPE_GM3 <= nUSERLVL )
	{
