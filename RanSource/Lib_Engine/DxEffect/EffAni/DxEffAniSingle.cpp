...
char		DxEffAniData_Single::NAME[64]		= "DxEffAniData_Single";
...
HRESULT	DxEffAniData_Single::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffAniData_Single::LoadFile", SFile.GetFileName(), dwVer );
...
/*ABL system, Juver, 2017/05/29 */
HRESULT	DxEffAniSingle::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxAttBone* pAttBone, const D3DXMATRIX& matCurPos )
{
	if ( !m_pSingleGroup)	return S_OK;
	...
	{
		LPD3DXMATRIX pBoneMatrix(NULL);
		if ( !SetPosition ( pAttBone, pBoneMatrix ) )	return S_OK;
...
/*ABL system, Juver, 2017/05/29 */
HRESULT	DxEffAniSingle::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxAttBoneRender* pAttBone, const D3DXMATRIX& matCurPos )
{
	if ( !m_pSingleGroup)	return S_OK;
...
/*vehicle system, Juver, 2017/08/03 */
HRESULT	DxEffAniSingle::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxVehicle* pVehicle, const D3DXMATRIX& matCurPos )
{
	if ( !m_pSingleGroup)	return S_OK;
	...
	{
		LPD3DXMATRIX pBoneMatrix(NULL);
		if ( !SetPosition ( pVehicle, pBoneMatrix ) )	return S_OK;
...
/*ABL system, Juver, 2017/05/29 */
BOOL	DxEffAniSingle::SetPosition ( DxAttBone* pAttBone, LPD3DXMATRIX& pMatrix )
{
	DxSkeleton::BONETRANSMAP* BoneMap = &pAttBone->GetSkeleton()->BoneMap;
...
/*ABL system, Juver, 2017/05/29 */
BOOL	DxEffAniSingle::SetPosition ( DxAttBoneRender* pAttBone, LPD3DXMATRIX& pMatrix )
{
	DxSkeleton::BONETRANSMAP* BoneMap = &pAttBone->GetSkeleton()->BoneMap;
...
BOOL	DxEffAniSingle::SetPosition ( DxVehicle* pVehicle, LPD3DXMATRIX& pMatrix )
{
	DxSkeleton::BONETRANSMAP* BoneMap = &pVehicle->GetSkeleton()->BoneMap;
