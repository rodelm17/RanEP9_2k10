...
HRESULT CDXUTDirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    if( rgdwAdjacency == NULL )
        return E_OUTOFMEMORY;
    V_PASS( s_pMesh->GenerateAdjacency(1e-6f,rgdwAdjacency) );
    V_PASS( s_pMesh->OptimizeInplace(D3DXMESHOPT_VERTEXCACHE, rgdwAdjacency, NULL, NULL, NULL) );
                                        ...
                                        const D3DXMATRIX* pmProj, const D3DXVECTOR3* pEyePt )
{
    ...
    V_PASS( s_pEffect->SetTechnique( "RenderWith1LightNoTexture" ) );
    V_PASS( s_pEffect->SetVector( "g_MaterialDiffuseColor", (D3DXVECTOR4*)&color ) );

    D3DXVECTOR3 vEyePt;
    D3DXVec3Normalize( &vEyePt, pEyePt );
    V_PASS( s_pEffect->SetValue( "g_LightDir", &vEyePt, sizeof(D3DXVECTOR3) ) );
    ...
    V_PASS( s_pEffect->SetMatrix( "g_mWorldViewProjection", &mWorldViewProj ) );
    V_PASS( s_pEffect->SetMatrix( "g_mWorld", &mWorld ) );

    for( int iSubset=0; iSubset<2; iSubset++ )
    {
        V_PASS( s_pEffect->Begin(&cPasses, 0) );
        for (iPass = 0; iPass < cPasses; iPass++)
        {
            V_PASS( s_pEffect->BeginPass(iPass) );
            V_PASS( s_pMesh->DrawSubset(iSubset) );
            V_PASS( s_pEffect->EndPass() );
        }
        V_PASS( s_pEffect->End() );
    }
