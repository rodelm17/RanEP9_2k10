...
void CEditorActivityDlg::OnBnClickedButtonFileLoad()
{
	...
	CFileDialog dlg( TRUE, ".asf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
...
{
	CString szFilter = "Activity Setting File (*.asf)|*.asf|";
	CFileDialog dlg( FALSE, ".asf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
...
void CEditorActivityDlg::OnBnClickedButtonCopy()
{
	int nSelect = m_List.GetNextItem(-1, LVNI_ALL | LVNI_SELECTED);
	if( nSelect == -1 ) return;
	DWORD dwID = m_List.GetItemData( nSelect );
