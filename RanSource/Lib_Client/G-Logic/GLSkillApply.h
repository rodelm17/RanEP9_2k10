...
namespace SKILL
{
	...
	struct SSPEC
	{
		...
		void Assign( SSPEC_109 &sOldData )
		{
			fVAR1 = sOldData.fVAR1;
			fVAR2 = sOldData.fVAR2;
			fVAR3 = sOldData.fVAR3;
			fVAR4 = sOldData.fVAR4;
			dwFLAG = sOldData.dwFLAG;
			dwNativeID = sOldData.dwNativeID;
		}
	};
	...
	struct SIMPACTS_108
	{
		...
		float				fADDON_VAR[MAX_SKILL_LEVEL];		//	변화 수치.
	...
	struct SSPECS_108
	{
		...
		SKILL::SSPEC_108	sSPEC[MAX_SKILL_LEVEL];			//	특수 기능 설정값.
	...
	{
		EMSPEC_ADDON		emSPEC;						//	특수.
		SKILL::SSPEC_109	sSPEC[MAX_SKILL_LEVEL];			//	특수 기능 설정값.

		SSPECS_109()
			: emSPEC(EMSPECA_NULL)
		{
			for ( int i=0; i<MAX_SKILL_LEVEL; ++i )	
				sSPEC[i] = SKILL::SSPEC_109();
		};
	};
	...
	struct SIMPACTS
	{
		EMIMPACT_ADDON		emADDON;					//	부가효과 종류.
		float				fADDON_VAR[MAX_SKILL_LEVEL];		//	변화 수치.
		float				fADDON_VAR2[MAX_SKILL_LEVEL];
	...
	{
		EMSPEC_ADDON		emSPEC;						//	특수.
		SKILL::SSPEC		sSPEC[MAX_SKILL_LEVEL];			//	특수 기능 설정값.
		...
		void Assign ( SSPECS_109 &sOldData )
		{
			emSPEC = sOldData.emSPEC;
	...
	struct SAPPLY_100
	{
		...
		SKILL::CDATA_LVL_100 sDATA_LVL[MAX_SKILL_LEVEL];		//	레벨별 설정 값.
		...
		SKILL::SSTATE_BLOW	sSTATE_BLOW[MAX_SKILL_LEVEL];		//	상태 이상 수치들.

		//	특수기능 설정.
		EMSPEC_ADDON		emSPEC;						//	특수.
		SKILL::SSPEC_100	sSPEC[MAX_SKILL_LEVEL];			//	특수 기능 설정값.
	};
	...
	struct SAPPLY_101
	{
		...
		EMELEMENT			emELEMENT;					//	( 수, 광, 화, 독, 없음 ) 속성.
		DWORD				dwCUREFLAG;					//	해독 종류.
		SKILL::CDATA_LVL_101	sDATA_LVL[MAX_SKILL_LEVEL];		//	레벨별 설정 값.
	...
	struct SAPPLY
	{
		...
		SKILL::EMTYPES		emBASIC_TYPE;				//	적용값 타입.
		EMELEMENT			emELEMENT;					//	( 수, 광, 화, 독, 없음 ) 속성.
		SKILL::CDATA_LVL	sDATA_LVL[MAX_SKILL_LEVEL];		//	레벨별 설정 값.
		DWORD				dwCUREFLAG;					//	해독 종류.
		...
		BOOL LOAD_0111 ( basestream &SFile );
		BOOL LOAD_0112 ( basestream &SFile );
