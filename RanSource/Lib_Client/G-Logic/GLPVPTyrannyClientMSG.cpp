...
HRESULT GLPVPTyrannyClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_TYRANNY_A2FC_STATE_REGISTER:
		{
			...
			CInnerInterface::GetInstance().PVPCompetitionButtonAlarm( EMCOMPETITION_NOTIFY_TYPE_TYRANNY_REGISTER );
	...
	case NET_MSG_GCTRL_TYRANNY_A2FC_NEXTSCHED:
		{
			...
			m_fRemainTimeToNextBattle = pnetmsg->fRemainTimeToNextBattle; // PVP Next Battle UI - Jxyyy
		}break;
	...
	case NET_MSG_GCTRL_TYRANNY_A2C_BATTLEINFO_PC:
		{
			...
			for( int i=0; i<TYRANNY_SCHOOL_SIZE; ++i )
				m_wRebirthTime[i] = pnetmsg->wRebirthTime[i];
			...
			m_sItemRewardWin = pnetmsg->sItemRewardWin;
			m_sItemRewardLoss = pnetmsg->sItemRewardLoss;
			m_wRewardLimitScoreWin = pnetmsg->wRewardLimitScoreWin;
			m_wRewardLimitScoreLost = pnetmsg->wRewardLimitScoreLost;
			
			m_fContributionPointMultiplier = pnetmsg->fContributionPointMultiplier;
		}break;
	...
	case NET_MSG_GCTRL_TYRANNY_A2C_REGISTER_FB:
		{
			...
			case TYRANNY_REGISTER_FB_DEVICE_INFO_ERROR:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_REGISTER_FB_DEVICE_INFO_ERROR") );
				break;

				/* PVPTyranny Hash Check, Juver, 2020/07/28 */
			case TYRANNY_REGISTER_FB_DEVICE_REGISTERED:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_REGISTER_FB_DEVICE_REGISTERED") );
				break;

			case TYRANNY_REGISTER_FB_NO_CLUB:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_REGISTER_FB_NO_CLUB") );
				break;
	...
	case NET_MSG_GCTRL_TYRANNY_F2C_TOWER_CAPTURE:
		{
			...
			if ( pnetmsg->wTowerIndex < TYRANNY_TOWER_SIZE )
			{
				...
				CInnerInterface::GetInstance().PVPTyrannyShowCaptureNotice( m_wTowerOwner[pnetmsg->wTowerIndex], pnetmsg->dwClubID, pnetmsg->dwClubMarkVer );

				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("TYRANNY_CAPTURE_INFO"), 
					ID2GAMEWORD( "TYRANNY_TOWER_TEXT", pnetmsg->wTowerIndex), pnetmsg->szClubName, GLCONST_CHAR::strSCHOOLNAME[pnetmsg->wTowerSchool].c_str() );
	...
	case NET_MSG_GCTRL_TYRANNY_A2C_TOWER_INFO:
		{
			...
			for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )
			{
				...
				m_dwGuiID[i] = pnetmsg->dwGuiID[i];
				m_dwMarkVer[i] = pnetmsg->dwMarkVer[i];
			}
		}break;
	case NET_MSG_GCTRL_TYRANNY_F2C_GUILD_TOWER_CAPTURE:
		{
			GLMSG::SNETPC_TYRANNY_F2C_GUILD_TOWER_CAPTURE* pnetmsg = ( GLMSG::SNETPC_TYRANNY_F2C_GUILD_TOWER_CAPTURE* ) nmg;
			m_dwGuiID[pnetmsg->m_nIndex] = pnetmsg->m_dwGuiID;
			m_dwMarkVer[pnetmsg->m_nIndex] = pnetmsg->m_dwMarkVer;
		}
		break;
	...
	/*case NET_MSG_GCTRL_TYRANNY_A2FC_REWARD_BUFF_INFO:
		{
			GLMSG::SNET_TYRANNY_A2FC_REWARD_BUFF_INFO* pnetmsg = ( GLMSG::SNET_TYRANNY_A2FC_REWARD_BUFF_INFO* ) nmg;

			for( int i=0; i<TYRANNY_REWARD_BUFF_NUM; ++i )
				m_sRewardBuff[i] = pnetmsg->sRewardBuff[i].sidSkill;
		}break;*/


	case NET_MSG_GCTRL_TYRANNY_A2C_LATE_JOIN_FB:
		{
			GLMSG::SNETPC_TYRANNY_A2C_LATE_JOIN_FB* pnetmsg = ( GLMSG::SNETPC_TYRANNY_A2C_LATE_JOIN_FB* ) nmg;
			switch( pnetmsg->emFB )
			{
			case TYRANNY_LATE_JOIN_FB_FAILED:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_FAILED") );
				break;

			case TYRANNY_LATE_JOIN_FB_NOTPOSSIBLE:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_NOTPOSSIBLE") );
				break;

			case TYRANNY_LATE_JOIN_FB_INBATTLE:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_INBATTLE") );
				break;

			case TYRANNY_LATE_JOIN_FB_ALREADY_JOINED:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_ALREADY_JOINED") );
				break;

			case TYRANNY_LATE_JOIN_FB_NO_CLUB:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_NO_CLUB") );
				break;

			case TYRANNY_LATE_JOIN_FB_FULL:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_FULL") );
				break;

			case TYRANNY_LATE_JOIN_FB_REGISTERED:
				{
					m_bRegistered = TRUE;
					m_bQueued = FALSE;
					m_wQueueNum = 0;
					CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_REGISTERED") );
				}
				break;

			case TYRANNY_LATE_JOIN_FB_INVALID_CHANNEL:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_INVALID_CHANNEL") );
				break;

			case TYRANNY_LATE_JOIN_FB_LEVEL_LOW:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_LEVEL_LOW") );
				break;

					/* PVPTyranny Hash Check, Juver, 2020/07/28 */
			case TYRANNY_LATE_JOIN_FB_DEVICE_REGISTERED:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_DEVICE_REGISTERED") );
				break;

			case TYRANNY_LATE_JOIN_FB_DEVICE_INFO_ERROR:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TYRANNY_LATE_JOIN_FB_DEVICE_INFO_ERROR") );
				break;
			};
		}break;
