...
bool CUIMan::m_bFirstControl = true;
...
BOOL CUIMan::RegisterDockingControl ( CUIControl* pNewControl, bool bParent )
{
	return m_DockingMan.RegisterControl( pNewControl, bParent );
}
...
void CUIMan::ShowGroupUpdate( UIGUID ControlID, bool bMakeMsg )
{
	CUIControl * pControl = m_UpdateList.IsInserted( ControlID );
	...
	else
	{
		pControl = m_ControlContainer.FindControl( ControlID );
		if( pControl )
			m_UpdateList.InsertTail( ControlID, pControl );
	}
}
...
void CUIMan::Refresh( UIGUID ControlID )
{
	CUIControl* pControl = m_ControlContainer.FindControl( ControlID );
	if( pControl )
		pControl->Refresh();
...
{
	const CUIControl* pControl = m_ControlContainer.FindControl( ControlID );
	if( pControl )	
...
void CUIMan::SetGroupLock(UIGUID ControlID, bool bLock)
{
	CUIControl* pControl = m_ControlContainer.FindControl(ControlID);

	if (pControl)
	{
		pControl->SetLock(bLock);
	}
}

bool CUIMan::IsGroupLock(UIGUID ControlID)
{
	CUIControl* pControl = m_ControlContainer.FindControl(ControlID);

	if (pControl)
	{
		return pControl->IsLock();
	}
...
CUIControl* CUIMan::GetExactFocusControl ()
{	
	return m_pFocusControl;
}

void CUIMan::ResetExactFocusControl()
{
	m_pFocusControl = NULL;
}

void CUIMan::SetExactFocusControl (CUIControl* FocusControl)
{
	m_pFocusControl = FocusControl;
}

void CUIMan::SetModalGroup( UIGUID ControlID, bool bModal )
{
	const CUIControlContainer::UICONTROL_LIST& test = m_ControlContainer.GetControl();

	for (CUIControlContainer::UICONTROL_LIST_CITER it = test.begin(); it != test.end(); ++it)
	{
		if ((*it)->GetWndID() != ControlID)
		{
			(*it)->SetLock(bModal);
		}
	}
