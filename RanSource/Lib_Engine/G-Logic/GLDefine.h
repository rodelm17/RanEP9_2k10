...
struct GLPADATA
{
	...
	void INCREASE ( const WORD wValue, const BOOL bRate=FALSE, const WORD additional = 0 )
	{
...
{
	EMNPC_TYPE_NORMAL			= 0,
	EMNPC_TYPE_POSTBOX			= 1,
	EMNPC_TYPE_EVENT_TOWER		= 2,
	EMNPC_TYPE_GAMBLING_GAME	= 3,
	EMNPC_TYPE_WOE				= 4,
	EMNPC_TYPE_SIZE				= 5,
};

/*pvp capture the flag, Juver, 2018/02/03 */
enum EMNPC_CTF_FLAG_TYPE
{
	EMNPC_CTF_FLAG_TYPE_NONE	= 0,
	EMNPC_CTF_FLAG_TYPE_A		= 1,
	EMNPC_CTF_FLAG_TYPE_B		= 2,
	EMNPC_CTF_FLAG_TYPE_SIZE	= 3,
};
...
enum EMSKILLCHECK
{
	...
	EMSKILL_NOTCP		= 11, /*combatpoint logic, Juver, 2017/05/28 */
	EMSKILL_NOTBULLET   = 12, /*gun-bullet logic, Juver, 2017/05/27 */
	EMSKILL_PROHIBIT	= 13, /*prohibit skill logic, Juver, 2017/06/06 */
...
typedef std::set<STARGETID>		TARID_SET;
typedef TARID_SET::iterator		TARID_SET_ITER;
...
typedef std::vector<STARID>		STARID_VEC;
...
enum DAMAGE_TYPE
{
	...
	DAMAGE_TYPE_ILLUSION			= 0x0100,	/* skill illusion, Juver, 2021/01/17 */
};

namespace COMMENT
{	
	extern std::string CROW[CROW_NUM];
	extern std::string NPC_TYPE[EMNPC_TYPE_SIZE];
	extern std::string CTF_TYPE[EMNPC_CTF_FLAG_TYPE_SIZE];	/*pvp capture the flag, Juver, 2018/02/03 */
	extern std::string CROW_SCHOOL[EMCROW_SCHOOL_SIZE+1];		/*crow school, Juver, 2018/08/02 */
