...
const int CBasicQuickSkillSlot2Ex::nNOT_INIT = -1;

CBasicQuickSkillSlot2Ex::CBasicQuickSkillSlot2Ex ()
	: m_nNumber ( nNOT_INIT )
	, m_pNumberText ( NULL )
	, m_nTabIndex(0)
{
...
void CBasicQuickSkillSlot2Ex::CreateSubControl ( INT nTabIndex )
{
	GASSERT( 0 <= nTabIndex && nTabIndex < MAX_TAB_INDEX );
	m_nTabIndex = nTabIndex;

	CBasicQuickSkillSlot2::CreateSubControl ();
...
void CBasicQuickSkillSlot2Ex::CreateNumberText ( CD3DFontPar* pFont8, int nNumber )
{
	m_nNumber = nNumber;
	...
	CString strNumber;	
	strNumber = CInnerInterface::GetInstance().GetdwKeyToString(RPROFILE::SkillSlot[nNumber]);

	CBasicTextBox* pNumberText = new CBasicTextBox;
	pNumberText->CreateSub ( this, "QUICK_SKILL_SLOT_NUMBER_H" );
...
void CBasicQuickSkillSlot2Ex::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	//	바로 상위 ( CBasicQuickSkillSlot2::Update() )를 부르지 않은 것은 의도된 것입니다.
...
void CBasicQuickSkillSlot2Ex::SetShotcutText( CString strTemp )
{
