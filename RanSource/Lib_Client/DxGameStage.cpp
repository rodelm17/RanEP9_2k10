...
void DxGameStage::SetCharacterDead( BOOL bDead  )
{
	m_sGameStageInfo.bCharacterDead = bDead;

	if ( m_sGameStageInfo.bCharacterDead )
	{
		m_bUseInterface = TRUE;
	}
}
...
HRESULT DxGameStage::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	RGAME_GLOBAL::SETCHARID(m_CharJoinData.m_CharData2.m_szName);
	RPROFILE::Load ();
	DXPARAMSET::LOADDATA();
...
HRESULT DxGameStage::FrameMove ( float m_fTime, float m_fElapsedTime )
{
	...
	hr = GLGaeaClient::GetInstance().FrameMove ( m_fTime, m_fElapsedTime, m_sGameStageInfo );
	if ( FAILED(hr) )	return hr;
	...
	/* pvp club death match, Juver, 2020/11/23 */
	PROFILE_BEGIN("PVPClubDeathMatchClient::FrameMove");
	PVPClubDeathMatchClient::GetInstance().FrameMove( m_fTime, m_fElapsedTime );
	PROFILE_END("PVPClubDeathMatchClient::FrameMove");

	PROFILE_BEGIN("PVPClubDeathMatchClient::FrameMove");
	PVPPBG::ManagerClient::GetInstance().FrameMove( m_fTime, m_fElapsedTime );
	PROFILE_END("PVPClubDeathMatchClient::FrameMove");
	...
	/*woe Arc Development 08-06-2024*/
	PROFILE_BEGIN("GLPVPWoeClient::FrameMove");
	GLPVPWoeClient::GetInstance().FrameMove(m_fTime, m_fElapsedTime);
	PROFILE_END("GLPVPWoeClient::FrameMove");
...
HRESULT DxGameStage::Render ()
{
	...
	hr = GLGaeaClient::GetInstance().Render ( CV, m_sGameStageInfo  );
	...
	GLGaeaClient::GetInstance().RenderPickAlpha( CV );
...
void DxGameStage::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_LOBBY_CHAR_JOIN:
		{
			...
			/*12-9-14, Battle Pass - CNDev*/
			m_CharJoinData.m_dwNumBattlePassProg = pCharJoin->dwNumBattlePassProg;
			m_CharJoinData.m_dwNumBattlePassDone = pCharJoin->dwNumBattlePassDone;
			m_CharJoinData.m_dwNumBattlePassRewardData = pCharJoin->dwNumBattlePassRewardData;
			...
			/*12-16-24, daily login - CNDev*/
			m_CharJoinData.m_dwNumDailyLoginData = pCharJoin->dwNumDailyLoginData;
			
			/* Codex, Archie 02/16/24 */
			m_CharJoinData.m_dwNumCodexProg = pCharJoin->dwNumCodexProg;
			m_CharJoinData.m_dwNumCodexDone = pCharJoin->dwNumCodexDone;
	...
	case NET_MSG_LOBBY_CHAR_BATTLEPASS_PROG:
		{
			GLMSG::SNETLOBBY_BATTLEPASS_PROG *pNetMsg = (GLMSG::SNETLOBBY_BATTLEPASS_PROG *) nmg;

			m_CharJoinData.m_dwReceiveBattlePassProg += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapBattlePassProg.insert ( std::make_pair ( pNetMsg->sBattlePass[i].dwBattlePassID, pNetMsg->sBattlePass[i] ) );
			}

			CDebugSet::ToListView ( "SNETLOBBY_BATTLEPASS_PROG" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
	...
	case NET_MSG_LOBBY_CHAR_BATTLEPASS_DONE:
		{
			GLMSG::SNETLOBBY_BATTLEPASS_DONE *pNetMsg = (GLMSG::SNETLOBBY_BATTLEPASS_DONE *) nmg;

			m_CharJoinData.m_dwReceiveBattlePassDone += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapBattlePassDone.insert ( std::make_pair ( pNetMsg->sBattlePass[i].dwBattlePassID, pNetMsg->sBattlePass[i] ) );
			}

			CDebugSet::ToListView ( "SNETLOBBY_BATTLEPASS_DONE" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
	...
	case NET_MSG_LOBBY_CHAR_BATTLEPASS_REWARD_DATA:
		{
			GLMSG::SNETLOBBY_BATTLEPASS_REWARD_DATA *pNetMsg = (GLMSG::SNETLOBBY_BATTLEPASS_REWARD_DATA *) nmg;

			m_CharJoinData.m_dwReceiveBattlePassRewardData += pNetMsg->dwNum;
			for ( WORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapBattlePassReward.insert ( std::make_pair ( pNetMsg->sBattlePass[i].dwLevel, pNetMsg->sBattlePass[i] ) );
			}

			CDebugSet::ToListView ( "SNETLOBBY_BATTLEPASS_REWARD_DATA" );
	...
	case NET_MSG_LOBBY_CHAR_INFO_PERSONAL_LOCK:
		{
			GLMSG::SNETLOBBY_CHAR_INFO_PERSONAL_LOCK *pNetMsg = (GLMSG::SNETLOBBY_CHAR_INFO_PERSONAL_LOCK *) nmg;

			for( int i=0; i<EMPERSONAL_LOCK_SIZE; ++i )
			{
				m_CharJoinData.m_CharData2.m_sPersonalLock[i] = pNetMsg->sPersonalLock[i];
			}
			
			m_CharJoinData.m_bReceivePersonalLock = true;

			CDebugSet::ToListView ( "NET_MSG_LOBBY_CHAR_INFO_PERSONAL_LOCK" );
	...
	case NET_MSG_LOBBY_CHAR_INFO_PK_COMBO_COUNT:
		{
			GLMSG::SNETLOBBY_CHAR_INFO_PK_COMBO_COUNT *pNetMsg = (GLMSG::SNETLOBBY_CHAR_INFO_PK_COMBO_COUNT *) nmg;

			m_CharJoinData.m_CharData2.m_sPKComboCount = pNetMsg->sPKComboCount;

			m_CharJoinData.m_bReceivePKComboCount = true;

			CDebugSet::ToListView ( "SNETLOBBY_CHAR_INFO_PK_COMBO_COUNT" );
	...
	case NET_MSG_LOBBY_CHAR_INFO_COMBAT_RECORD:
		{
			GLMSG::SNETLOBBY_CHAR_INFO_COMBAT_RECORD *pNetMsg = (GLMSG::SNETLOBBY_CHAR_INFO_COMBAT_RECORD *) nmg;

			m_CharJoinData.m_CharData2.m_sCombatRecord = pNetMsg->sCombatRecord;

			m_CharJoinData.m_bReceiveCombatRecord = true;

			CDebugSet::ToListView ( "SNETLOBBY_CHAR_INFO_COMBAT_RECORD" );
	...
	case NET_MSG_LOBBY_CHAR_DAILYLOGIN_DATA:
		{
			GLMSG::SNETLOBBY_DAILYLOGIN_DATA *pNetMsg = (GLMSG::SNETLOBBY_DAILYLOGIN_DATA *) nmg;

			m_CharJoinData.m_dwReceiveDailyLoginData += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapDailyLogin.insert ( std::make_pair ( pNetMsg->sDailyLogin[i].dwDay, pNetMsg->sDailyLogin[i] ) );
			}

			CDebugSet::ToListView ( "SNETLOBBY_DAILYLOGIN_DATA" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
	...
	case NET_MSG_LOBBY_CHAR_CODEX_PROG:
		{
			GLMSG::SNETLOBBY_CODEX_PROG *pNetMsg = (GLMSG::SNETLOBBY_CODEX_PROG *) nmg;

			m_CharJoinData.m_dwReceiveCodexProg += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapCodexProg.insert ( std::make_pair ( pNetMsg->sCodex[i].dwCodexID, pNetMsg->sCodex[i] ) );
			}


			CDebugSet::ToListView ( "NET_MSG_LOBBY_CHAR_CODEX_PROG" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
	...
	case NET_MSG_LOBBY_CHAR_CODEX_DONE:
		{
			GLMSG::SNETLOBBY_CODEX_DONE *pNetMsg = (GLMSG::SNETLOBBY_CODEX_DONE *) nmg;

			m_CharJoinData.m_dwReceiveCodexDone += pNetMsg->dwNum;
			for ( DWORD i=0; i<pNetMsg->dwNum; ++i )
			{
				m_CharJoinData.m_CharData2.m_mapCodexDone.insert ( std::make_pair ( pNetMsg->sCodex[i].dwCodexID, pNetMsg->sCodex[i] ) );
			}

			CDebugSet::ToListView ( "NET_MSG_LOBBY_CHAR_CODEX_DONE" );
			if ( m_CharJoinData.CheckReceive() )
			{
				NET_MSG_GENERIC nmg = { sizeof(NET_MSG_GENERIC), NET_MSG_LOBBY_GAME_COMPLETE };
				COuterInterface::GetInstance().MsgProcess ( &nmg );
