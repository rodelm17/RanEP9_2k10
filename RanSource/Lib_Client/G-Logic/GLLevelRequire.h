...
struct SLEVEL_REQUIRE
{
	...
	SNATIVEID	unknown_var1;
	SNATIVEID	unknown_var2;
	SNATIVEID	unknown_var3;
	SNATIVEID	unknown_var4;
	...
	void LOAD_0201 ( basestream &SFile, DWORD dwSIZE );
	void LOAD_0202 ( basestream &SFile, DWORD dwSIZE );

	static VOID SaveCsvHead( std::fstream &SFile );
	VOID SaveCsv( std::fstream &SFile );
	VOID LoadCsv( CStringArray &StrArray );

	EMREQFAIL ISCOMPLETE ( GLCHARLOGIC_SERVER * pCHARLOGIC ) const;
	EMREQFAIL ISCOMPLETE ( GLCHARLOGIC_CLIENT * pCHARLOGIC ) const;
};
