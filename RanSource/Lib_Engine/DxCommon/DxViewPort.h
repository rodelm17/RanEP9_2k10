...
#define VIEWPORT_MOVEVELO_DEFAULT	((FLOAT)  80.0f) 
...
public:
	...
	HRESULT SetProjection_FarPlane_Fog ();
	HRESULT SetViewPort ();
	void	SetFarPlane ( float fFarPlane )			{ m_fFarPlane = fFarPlane * 1.5f; }

	//adjust velo
public:
	float MoveVelocityGet()				{	return m_fVELOMOVE;	}
	void MoveVelocitySet( float fVELO )	{	m_fVELOMOVE = fVELO;	}
	void MoveVelocityReset()			{	m_fVELOMOVE = VIEWPORT_MOVEVELO_DEFAULT;	}
