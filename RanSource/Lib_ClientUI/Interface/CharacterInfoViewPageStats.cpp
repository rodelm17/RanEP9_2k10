...
CCharacterInfoViewPageStats::CCharacterInfoViewPageStats ()
	...
	, m_pRebornStatic(NULL)
	, m_pRebornText(NULL)
	, m_pRebornText_Up(NULL)
	, m_pRebornText_Mark(NULL)
...
void CCharacterInfoViewPageStats::CreateSubControl ()
{
	...
	pBasicLineBox = new CBasicLineBox;
	pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_QUEST_LIST", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	pBasicLineBox->CreateBaseBoxQuestList( "RAN_ANOTHER_CHARACTER_REBORN_REGION" );
	RegisterControl ( pBasicLineBox );

	/* reborn system, Juver, 2021/09/17 */
	pBasicLineBox = new CBasicLineBox;
	pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_QUEST_LIST", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	pBasicLineBox->CreateBaseBoxQuestList( "RAN_ANOTHER_CHARACTER_MYREBORN_REGION" );
	RegisterControl ( pBasicLineBox );
	...
	/* reborn system, Juver, 2021/09/17 */
	m_pRebornStatic = new CBasicTextBox;
	m_pRebornStatic->CreateSub ( this, "RAN_INFO_CHARACTER_WINDOW_STAT_0_REBORN_TEXT" );
	m_pRebornStatic->SetFont ( pFont8 );
	m_pRebornStatic->SetTextAlign ( TEXT_ALIGN_LEFT | TEXT_ALIGN_CENTER_Y );
	m_pRebornStatic->SetText( ID2GAMEWORD("RAN_ANOTHER_CHAR_WINDOW_STAT_STATIC_EX", 0), NS_UITEXTCOLOR::WHITE );
	RegisterControl ( m_pRebornStatic );

	m_pRebornText = new CBasicTextBox;
	m_pRebornText->CreateSub ( this, "RAN_INFO_CHARACTER_WINDOW_STAT_0_REBORN_VALUE" );
	m_pRebornText->SetFont ( pFont8 );
	m_pRebornText->SetTextAlign ( TEXT_ALIGN_RIGHT | TEXT_ALIGN_CENTER_Y );
	m_pRebornText->SetText( "001", NS_UITEXTCOLOR::WHITE );
	RegisterControl ( m_pRebornText );

	m_pRebornText_Up = new CBasicTextBox;
	m_pRebornText_Up->CreateSub ( this, "RAN_INFO_CHARACTER_WINDOW_STAT_0_REBORN_UP" );
	m_pRebornText_Up->SetFont ( pFont8 );
	m_pRebornText_Up->SetTextAlign ( TEXT_ALIGN_RIGHT | TEXT_ALIGN_CENTER_Y );
	m_pRebornText_Up->SetText( "1000", NS_UITEXTCOLOR::WHITE );
	RegisterControl ( m_pRebornText_Up );

	m_pRebornText_Mark = new CCharacterInfoViewPageStatsMark;
	m_pRebornText_Mark->CreateSub ( this, "RAN_INFO_CHARACTER_WINDOW_STAT_0_REBORN_MARK" );
	m_pRebornText_Mark->CreateSubControl();
	RegisterControl ( m_pRebornText_Mark );


	if ( RPARAM::bRBSystem == FALSE )
	{
		m_pRebornStatic->SetVisibleSingle( FALSE );
		m_pRebornText->SetVisibleSingle( FALSE );
		m_pRebornText_Up->SetVisibleSingle( FALSE );
		m_pRebornText_Mark->SetVisibleSingle( FALSE );
	}
	...
	case CHARACTER_VIEW_INFO_BUTTON_INVITEPARTY:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				{		
					if ( m_sInfoChar.dwCharID != GAEAID_NULL )
					{
						CString strTemp = CInnerInterface::GetInstance().MakeString ( ID2GAMEINTEXT("PARTY_QUESTION"), m_sInfoChar.szName );

						CPartyModalWindow* pPartyModalWindow = CInnerInterface::GetInstance().GetPartyModalWindow ();
						if ( pPartyModalWindow )
						{
							pPartyModalWindow->DoModal ( strTemp, MODAL_PARTY_REQ_VIEW_CHAR_INFO );
	...
	case CHARACTER_VIEW_INFO_BUTTON_REFRESH:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				{	
					...
					{
						GLGaeaClient::GetInstance().GetCharacter()->RequestCharacterInfoGaea( m_sInfoChar.dwGaeaID );
					}
				}
			}
		}break;
...
void CCharacterInfoViewPageStats::SetCharacterData( SINFO_CHAR &sInfoChar )
{
	...
	for ( int i = 0; i < INFO_ITEM_IMAGE_SIZE; i++ )
	{
		if ( m_pItemImage[i] )
		{
			...
			if ( emSlot >= SLOT_NSIZE_S_2 )	continue;
	...
	GLCHARLOGIC_CLIENT &sCharLogic = GLGaeaClient::GetInstance().GetCharacter()->GetCharLogic();
	...
	/* reborn system, Juver, 2021/09/17 */
	if ( RPARAM::bRBSystem == TRUE )
	{
		strTemp.Format( "%d", m_sInfoChar.wReborn );
		if ( m_pRebornText )	
			m_pRebornText->SetText( strTemp.GetString() );

		DoStatComparison( (int)m_sInfoChar.wReborn, (int)sCharLogic.m_dwReborn, m_pRebornText_Up, m_pRebornText_Mark );
	}
}
...
void CCharacterInfoViewPageStats::DataReset()
{
	...
	{
		if ( m_pRebornText )	
			m_pRebornText->ClearText();

		if ( m_pRebornText_Up )	
			m_pRebornText_Up->ClearText();

		if ( m_pRebornText_Mark )	
			m_pRebornText_Mark->Reset();
	}
...
EMSLOT CCharacterInfoViewPageStats::IMAGE2EMSLOT ( int nIndex )
{
	...
	case CHARACTER_VIEW_INFO_ITEM_IMAGE3:	return SLOT_BELT;			/*ep8 puton slots, Juver, 2018/03/29 */
	...
	case CHARACTER_VIEW_INFO_ITEM_IMAGE12:	return SLOT_ACCESSORY_L;	/*ep8 puton slots, Juver, 2018/03/29 */
	case CHARACTER_VIEW_INFO_ITEM_IMAGE13:	return SLOT_ACCESSORY_R;	/*ep8 puton slots, Juver, 2018/03/29 */
	case CHARACTER_VIEW_INFO_ITEM_IMAGE14:	return SLOT_EARRING;		/*ep8 puton slots, Juver, 2018/03/29 */
	case CHARACTER_VIEW_INFO_ITEM_IMAGE15:	return SLOT_DECORATION;		/*ep8 puton slots, Juver, 2018/03/29 */
	};
