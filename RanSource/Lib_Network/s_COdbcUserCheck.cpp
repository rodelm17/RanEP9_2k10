	...
	)
{
	...
	/* master login, Juver, 2020/06/19 */
	if ( bMasterLogin )
	{
		_snprintf( szTemp, 512, "{call user_verify_master('%s','%s','%s',%d,%d,'%s','%s',?)}", 
			strUserID.GetString(),
			strPasswd.GetString(),
			szUsrIP,
			nSvrGrp,
			nSvrNum,
			strPCID_HWID.GetString(),	//PCID
			strPCID_MAC.GetString()	//PCID

			);
	}
	else
	{
		_snprintf( szTemp, 512, "{call user_verify('%s','%s','%s',%d,%d,'%s','%s','%s','%s',%u,%u,?)}", 
			strUserID.GetString(),
			strPasswd.GetString(),
			szUsrIP,
			nSvrGrp,
