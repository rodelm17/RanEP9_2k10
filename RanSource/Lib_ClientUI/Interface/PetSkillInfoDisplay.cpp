...
void CPetSkillInfoDisplay::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( m_fUpdateTime >= PET_SKILL_INFO_DISPLAY_UPDATE_TIME )
	{
		...
		SNATIVEID sNativeID = GLGaeaClient::GetInstance().GetCharacter()->m_sPETSKILLFACT_A.sNATIVEID;
	...
	{
		CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine ( m_strSkillName.c_str(), NS_UITEXTCOLOR::GREENYELLOW );
	}
}