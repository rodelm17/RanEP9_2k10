...
public:
	...
	BOOL ArrangeInfo( const TCHAR *keyvalue, UITextureList::SControl& pControl );
	BOOL ArrangeInfo( const TCHAR *keyvalue );
	BOOL ArrangeFlipInfo(/*const*/ TCHAR *keyvalue);
	BOOL ArrangeExInfo(/*const*/ TCHAR *keyvalue);

	BOOL LoadText( const TCHAR * filename, const CString& country );
	...
	float GetTextureSizeX()						{ return m_fX_Size; }
	float GetTextureSizeY()						{ return m_fY_Size; }
	void Reset();
