...
void CPageABL::OnBnClickedButtonAblSave()
{
	...
	CString strsavefile = DxAttBoneLinkContainer::GetInstance().GetPath();
	strsavefile += m_pData->GetFileName();
	CFileDialog dlg( FALSE, NULL, strsavefile.GetString(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterBoneLinkABL, this );
	dlg.m_ofn.lpstrInitialDir = DxAttBoneLinkContainer::GetInstance().GetPath();
	if ( dlg.DoModal() != IDOK ) return;
...
void CPageABL::OnBnClickedAblButtonSkeleton()
{
	...
	CFileDialog dlgInput( true, NULL, DxBoneCollector::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterSkin, this );
