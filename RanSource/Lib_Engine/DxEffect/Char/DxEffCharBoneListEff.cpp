...
char		DxEffCharBoneListEff::NAME[MAX_PATH]	= "DxEffCharBoneListEff"; 
...
void DxEffCharBoneListEff::RenderEff( LPDIRECT3DDEVICEQ pd3dDevice, LPD3DXMATRIX pMatrix, const float fScale )
{
	...
	if ( g_bPREVIEW_CHAR_RENDER )
	{
		...
		matLocal = DxBillboardLookAt( &vBasePos, matViewLocal );
	}
...
void DxEffCharBoneListEff::ResetLocalPos()
{
	LISTCUSTOMBONE_ITER iter = m_listCustomBone.begin();
	for( ; iter!=m_listCustomBone.end(); ++iter )
	{
...
HRESULT	DxEffCharBoneListEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		//	Note : 이팩트의 Property 를 저장.
		EFFCHAR_PROPERTY_BONELISTEFF_100	sProperty;
		SFile.ReadBuffer ( &sProperty, sizeof(EFFCHAR_PROPERTY_BONELISTEFF_100) );

		m_Property.m_dwFlag			= sProperty.m_dwFlag;
		m_Property.m_dwParticlePec	= sProperty.m_dwParticlePec;
		m_Property.m_dwColor		= sProperty.m_dwColor;
		m_Property.m_dwSequenceCol	= sProperty.m_dwSequenceCol;
		m_Property.m_dwSequenceRow	= sProperty.m_dwSequenceRow;
		m_Property.m_fCreateLength	= sProperty.m_fCreateLength;
		m_Property.m_vLife			= sProperty.m_vLife;
		m_Property.m_vRotate		= sProperty.m_vRotate;
		m_Property.m_vVelocity		= sProperty.m_vVelocity;
		m_Property.m_vSize			= sProperty.m_vSize;
		StringCchCopy( m_Property.m_szTexture, BONELISTEFF_TEXNAME_SIZE, sProperty.m_szTexture );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharBoneListEff::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharBoneListEff::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharBoneListEff::R2LoadFile", SFile.GetFileName(), dwVer );
