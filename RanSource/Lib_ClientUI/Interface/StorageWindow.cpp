	...
	case STORAGE_BUTTON3:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( bUSABLE )
				{
					...
					{
						CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( ID2GAMEWORD("STORAGE_USABLE",0), NS_UITEXTCOLOR::GREENYELLOW );
						...
						CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strCombine, NS_UITEXTCOLOR::WHITESMOKE );
				...
				{
					CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine ( ID2GAMEWORD("STORAGE_USABLE",1), NS_UITEXTCOLOR::RED );
				}
			}
		}
		break;
	...
	case STORAGE_BUTTON4:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				{
					CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine ( ID2GAMEWORD("NOT_USE_PREMIUM"), NS_UITEXTCOLOR::RED );
				}
			}
		}
		break;
...
{	
...

	{
		for ( int i = 0; i < nUSE_STORAGE_CHANNEL; i++ )
		{
			...
			pClubStoragePage->CreateSub ( this, "CLUB_STORAGE_PAGE", UI_FLAG_DEFAULT, CLUB_STORAGE_PAGE0 + i );
	...
	CreateControl ( "CLUB_STORAGE_MONEY_BACK" );
	pButton = CreateFlipButton ( "CLUB_STORAGE_MONEY_BUTTON", "CLUB_STORAGE_MONEY_BUTTON_F", CLUB_STORAGE_MONEY_BUTTON, CBasicButton::CLICK_FLIP );
	pButton->CreateMouseOver ( "CLUB_STORAGE_MONEY_BUTTON_F" );
	pButton->SetUseDynamic ( TRUE );

	CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
	m_pMoneyTextBox = CreateStaticControl ( "CLUB_STORAGE_MONEY_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );

	CUIControl* pBlock = new CUIControl;
	pBlock->CreateSub ( this, "CLUB_STORAGE_BLOCK" );	
