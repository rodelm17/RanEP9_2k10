...
// custom flag
#define D3DFONTCUSTOM_MULTILINE  0x0008
...
public:
	CD3DFont( const TCHAR* strFontName, DWORD dwHeight, DWORD dwBufferMode, DWORD dwFlags=0L, language::LANGFLAG emLang=language::DEFAULT );
	~CD3DFont();
...
public:
	...
	virtual HRESULT GetTextExtent( const TCHAR* strText, const DWORD dwFlag, SIZE &Size );

	// Get the number of texts corresponding to the width size
	virtual int		GetTextCountByWidth ( const TCHAR* strText, const float fWidth, const float fRangePer=0.0f );
