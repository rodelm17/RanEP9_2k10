...
#define RANSEDIT_VERSION 0x0009
...
public:
	...
	enum { XOR_KEY_SIZE	= 256 };

	static TBYTE m_xor_key_006[XOR_KEY_SIZE];
	static TBYTE m_xor_key_007[XOR_KEY_SIZE];
	static TBYTE m_xor_key_008[XOR_KEY_SIZE];
	static TBYTE m_xor_key_009[XOR_KEY_SIZE];
...
public:
	...
	void Initialize( int nVersion, char const* keydata, int keydatalength, char const* chain=sm_chain0, int keylength=DEFAULT_KEY_LENGTH,
	...
	void EncryptFile(std::string const& rostrFileIn, std::string const& rostrFileOut);
	//Decryption for a File
	void DecryptFile(std::string const& rostrFileIn, std::string const& rostrFileOut);
...
public:
	...
	static std::string const sm_Version[VERSION];
