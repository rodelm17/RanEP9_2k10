...
enum EM2FRIEND_FB
{
	...
	EM2FRIEND_FB_PARTY_BLOCK		= 11,	

	/* map entry user verified, Juver, 2020/02/27 */
	EM2FRIEND_FB_USER_VERIFIED_MAP_ENTRY_BLOCK	= 12,		

	/*dmk14 fast teleport fix*/
	EM2FRIEND_FB_REQ				= 13,
	EM2FRIEND_FB_CANCEL				= 14,
};
...
enum EMVEHICLE_SET_FB
{
	...
	EMVEHICLE_SET_FB_FAIL_DEBUFF	= 6,	/*skill vehicle off, Juver, 2018/09/07 */
};
...
enum
{
	...
	EM_GLOBAL_RANKING_FB_TOP_RESU		= 14,/*Top Resu, Mhundz */
	EM_GLOBAL_RANKING_FB_TOP_MMR		= 15,/*Top MMR, Mhundz */
};
/////////////////////////////////////////////////////////////////////////////
...
namespace GLMSG
{
	...
	/*Top Resu, Mhundz */
	struct SNETPC_REQ_GLOBAL_RANKING_RESU_UPDATE
	{
		NET_MSG_GENERIC	nmg;
		STOP_RANK_RESU	sTopResu;
		BOOL			bWIN;

		SNETPC_REQ_GLOBAL_RANKING_RESU_UPDATE()
			: bWIN(FALSE)
		{
			nmg.dwSize	= sizeof(*this);
			nmg.nType	= NET_MSG_GCTRL_REQ_GLOBAL_RANKING_RESU_UPDATE;
		}
	};
	/*Top Resu, Mhundz */
	struct SNETPC_REQ_GLOBAL_RANKING_RESU_AGT
	{
		NET_MSG_GENERIC			nmg;
		WORD					emFB;
		WORD					wPart;
		WORD					wSize;
		STOP_RANK_RESU			sTopResu_[10];

		SNETPC_REQ_GLOBAL_RANKING_RESU_AGT()
			: emFB(EM_GLOBAL_RANKING_FB_NONE)
			, wPart(0)
			, wSize(0)
		{
			nmg.dwSize	= sizeof(*this);
			nmg.nType	= NET_MSG_GCTRL_REQ_GLOBAL_RANKING_RESU_AGT;
		}
	};
	/*Top MMR, Mhundz */
	struct SNETPC_REQ_GLOBAL_RANKING_MMR_UPDATE
	{
		NET_MSG_GENERIC	nmg;
		STOP_RANK_MMR	sTopMMR;
		BOOL			bWIN;

		SNETPC_REQ_GLOBAL_RANKING_MMR_UPDATE()
			: bWIN(FALSE)
		{
			nmg.dwSize	= sizeof(*this);
			nmg.nType	= NET_MSG_GCTRL_REQ_GLOBAL_RANKING_MMR_UPDATE;
		}
	};
	/*Top MMR, Mhundz */
	struct SNETPC_REQ_GLOBAL_RANKING_MMR_AGT
	{
		NET_MSG_GENERIC			nmg;
		WORD					emFB;
		WORD					wPart;
		WORD					wSize;
		STOP_RANK_MMR			sTopMMR_[10];

		SNETPC_REQ_GLOBAL_RANKING_MMR_AGT()
			: emFB(EM_GLOBAL_RANKING_FB_NONE)
			, wPart(0)
			, wSize(0)
		{
			nmg.dwSize	= sizeof(*this);
			nmg.nType	= NET_MSG_GCTRL_REQ_GLOBAL_RANKING_MMR_AGT;
		}
	};
	/////////////////////////////////////////////////////////////////////////////
	...
	struct SNETPC_ACTSTATE
	{
		...
		DWORD				dwActState;
		BOOL				bCMD;			/*gm command logs, Juver, 2018/08/17 */

		SNETPC_ACTSTATE () :
			dwActState(0),
			bCMD(FALSE)						/*gm command logs, Juver, 2018/08/17 */
		{
			nmg.dwSize = sizeof(*this);
	...
	struct SNETPC_UPDATE_PASSIVE_BRD : public SNETPC_BROAD
	{
		...
		SNETPC_UPDATE_PASSIVE_BRD ()
		{
			...
			BOOST_STATIC_ASSERT(sizeof(SNETPC_UPDATE_PASSIVE_BRD)<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_SKILL_PUSH_BRD : public SNETCROW_BROAD
	{
		D3DXVECTOR3			vMovePos;
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SKILL_PUSH_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* pull skill logic, Juver, 2020/12/14 */
	struct SNET_SKILL_PULL_BRD : public SNETCROW_BROAD
	{
		D3DXVECTOR3			vMovePos;
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SKILL_PULL_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* dash skill logic, Juver, 2020/12/14 */
	struct SNET_SKILL_DASH_BRD : public SNETCROW_BROAD
	{
		D3DXVECTOR3			vMovePos;
		float	fSpeed;
		DWORD	dwActionAnim;
		...
		SNET_SKILL_DASH_BRD ()
			: vMovePos(0,0,0)
			, fSpeed(0.0f)
			, dwActionAnim(AN_SUB_00_SIZE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SKILL_DASH_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* skill position shift, Juver, 2020/12/15 */
	struct SNET_SKILL_POSITION_SHIFT_BRD : public SNETCROW_BROAD
	{
		D3DXVECTOR3			vMovePos;
		float	fSpeed;

		SNET_SKILL_POSITION_SHIFT_BRD ()
			: vMovePos(0,0,0)
			, fSpeed(0.0f)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SKILL_POSITION_SHIFT_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNETPC_REQ_RANKNAME_BRD : public SNETPC_BROAD
	{
		BOOL		bRankName;
	...
	struct SNETPC_REQ_RANKMARK_BRD : public SNETPC_BROAD
	{
		BOOL		bRankMark;
	...
	/*dmk14 fast teleport fix*/
	struct SNETPC_2_FRIEND_REQ_CANCEL
	{
		NET_MSG_GENERIC			nmg;

		SNETPC_2_FRIEND_REQ_CANCEL () 
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_2_FRIEND_REQ_CANCEL;
		}
	};
	...
	struct SNETPC_GM_GENCHAR_PARTY
	{
		NET_MSG_GENERIC			nmg;
		DWORD	dwPartyID;

		SNETPC_GM_GENCHAR_PARTY ()
			: dwPartyID(0)
		{
			nmg.dwSize = sizeof(SNETPC_GM_GENCHAR_PARTY);
			nmg.nType = NET_MSG_GM_GENCHAR_PARTY;
		}
	};
...
//vietnamtest%%% && vietnamtest2
	...
	struct SNETPC_PING_PACKET_AGENT
	{
		NET_MSG_GENERIC		nmg;

		SNETPC_PING_PACKET_AGENT () 
		{
			nmg.dwSize = (DWORD) sizeof(SNETPC_PING_PACKET_AGENT);
			nmg.nType = NET_MSG_GCTRL_PING_PACKET_AGENT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*game stats, Juver, 2017/06/21 */
	struct SNETPC_PING_PACKET_AGENT_FB
	{
		NET_MSG_GENERIC		nmg;

		SNETPC_PING_PACKET_AGENT_FB()
		{
			nmg.dwSize = sizeof( SNETPC_PING_PACKET_AGENT_FB );
			nmg.nType = NET_MSG_GCTRL_PING_PACKET_AGENT_FB;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};

	/*game stats, Juver, 2017/06/21 */
	struct SNETPC_PING_PACKET_FIELD
	{
		NET_MSG_GENERIC		nmg;

		SNETPC_PING_PACKET_FIELD () 
		{
			nmg.dwSize = (DWORD) sizeof(SNETPC_PING_PACKET_FIELD);
			nmg.nType = NET_MSG_GCTRL_PING_PACKET_FIELD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*game stats, Juver, 2017/06/21 */
	struct SNETPC_PING_PACKET_FIELD_FB
	{
		NET_MSG_GENERIC		nmg;

		SNETPC_PING_PACKET_FIELD_FB()
		{
			nmg.dwSize = sizeof( SNETPC_PING_PACKET_FIELD_FB );
			nmg.nType = NET_MSG_GCTRL_PING_PACKET_FIELD_FB;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};
	...
	struct SNETPC_REQ_CHARINFO_GAEA
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwGaeaID;

		SNETPC_REQ_CHARINFO_GAEA ()
			: dwGaeaID( GAEAID_NULL)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_CHARINFO_GAEA;
		}
	};

	/* charinfoview, Juver, 2020/03/03 */
	struct SNETPC_REQ_CHARINFO_CHAR_ID
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwCharID;

		SNETPC_REQ_CHARINFO_CHAR_ID ()
			: dwCharID( GAEAID_NULL)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_CHARINFO_CHAR_ID;
		}
	};

	/* charinfoview, Juver, 2020/03/03 */
	struct SNETPC_REQ_CHARINFO_CHAR_NAME
	{
		NET_MSG_GENERIC		nmg;
		char				szCharName[CHAR_SZNAME];

		SNETPC_REQ_CHARINFO_CHAR_NAME () 
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_CHARINFO_CHAR_NAME;
	...
	struct SNETPC_REQ_CHARINFO_FB
	{
		...
		SNETPC_REQ_CHARINFO_FB ()
			: emFB(EMREQ_CHARINFO_FB_FAIL)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_CHARINFO_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNETPC_REQ_CHARINFO_FB)<=NET_DATA_BUFSIZE);
		}
	};

	/* charinfoview, Juver, 2020/03/03 */
	struct SNETPC_REQ_CHARINFO_FLD
	{
		NET_MSG_GENERIC		nmg;

		DWORD				dwCharIDViewer;
		DWORD				dwUserLevel;

		DWORD				dwCharIDRequested;

		SNETPC_REQ_CHARINFO_FLD ()
			: dwCharIDViewer(GAEAID_NULL)
			, dwUserLevel(0)
			, dwCharIDRequested(GAEAID_NULL)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_CHARINFO_FLD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* charinfoview, Juver, 2020/03/03 */
	struct SNETPC_REQ_CHARINFO_AGT
	{
		NET_MSG_GENERIC			nmg;
		DWORD					dwCharIDViewer;
		EMREQ_CHARINFO_AGT_FB	emFB;
		SINFO_CHAR				sCharInfo;

		SNETPC_REQ_CHARINFO_AGT ()
			: dwCharIDViewer(GAEAID_NULL)
			, emFB(EMREQ_CHARINFO_AGT_FB_FAIL)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_CHARINFO_AGT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNETPC_REQ_CHARINFO_AGT)<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNETPC_UPDATE_WAR_CHIPS
	{
		NET_MSG_GENERIC		nmg;
		DWORD dwWarChips;
		bool bNotice;

		SNETPC_UPDATE_WAR_CHIPS ()
			: dwWarChips(0)
			, bNotice(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_WAR_CHIPS;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_GAME_POINTS
	{
		NET_MSG_GENERIC		nmg;
		DWORD dwGamePoints;
		bool bNotice;

		SNETPC_UPDATE_GAME_POINTS ()
			: dwGamePoints(0)
			, bNotice(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_GAME_POINTS;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* play time system, Juver, 2021/01/26 */
	struct SNETPC_UPDATE_PLAY_TIME
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG	llPlayTime;
		BOOL		bNotice;

		SNETPC_UPDATE_PLAY_TIME ()
			: llPlayTime(0)
			, bNotice(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PLAY_TIME;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* play time system, Juver, 2021/01/26 */
	struct SNETPC_UPDATE_PLAY_POINT
	{
		NET_MSG_GENERIC		nmg;
		LONGLONG	llPlayPoint;
		BOOL		bNotice;

		SNETPC_UPDATE_PLAY_POINT ()
			: llPlayPoint(0)
			, bNotice(FALSE)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PLAY_POINT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* pk combo, Juver, 2021/02/09 */
	struct SNETPC_UPDATE_PK_COMBO
	{
		NET_MSG_GENERIC		nmg;
		EMPK_COMBO		emPKCombo;
		EMPKCOMBO_COUNT	emPKComboCount;
		DWORD			dwPKComboCount;

		SNETPC_UPDATE_PK_COMBO ()
			: emPKCombo(EMPK_COMBO_TYPE_SIZE)
			, emPKComboCount(EMPKCOMBO_COUNT_SIZE)
			, dwPKComboCount(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_PK_COMBO;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_COMBAT_RECORD
	{
		NET_MSG_GENERIC		nmg;
		nscombat_record::SDATA	sCombatRecord;

		SNETPC_UPDATE_COMBAT_RECORD ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_COMBAT_RECORD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	/* qitem inven, 2024/01/07 */
	struct SNETPC_REQ_USE_INVENQITEM
	{
		NET_MSG_GENERIC		nmg;
		WORD				wPosX;
		WORD				wPosY;

		SNETPC_REQ_USE_INVENQITEM () 
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_USE_INVENQITEM);
			nmg.nType = NET_MSG_GCTRL_REQ_USE_INVENQITEM;
		}
	};
	...
	struct SNET_CALL_GUILD_MEM : public NET_MSG_GENERIC
	{
		char	szName[CHAR_SZNAME];
	...
	struct SNETPC_REQ_RECMSG_NOTIF
	{
		NET_MSG_GENERIC		nmg;
		bool				bRecMsg;

		SNETPC_REQ_RECMSG_NOTIF () :
		bRecMsg(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_RECMSG_NOTIF);
			nmg.nType = NET_MSG_GCTRL_REQ_RECMSG_NOTIF;
		}
	};
	
	/*12-9-14, Battle Pass - CNDev*/
	struct SNETPC_UPDATE_BATTLEPASS_EXP
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwEXP;
		bool				bNotice;

		SNETPC_UPDATE_BATTLEPASS_EXP ()
			: dwEXP(0)
			, bNotice(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_BATTLEPASS_EXP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_UPDATE_BATTLEPASS_LEVEL
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwLevel;
		bool				bNotice;

		SNETPC_UPDATE_BATTLEPASS_LEVEL ()
			: dwLevel(0)
			, bNotice(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_BATTLEPASS_LEVEL;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	
	struct SNETPC_REQ_BATTLEPASS_REWARD_DATA
	{
		NET_MSG_GENERIC					nmg;
		SBATTLEPASS_REWARD_CHAR_DATA	sRewardData;

		SNETPC_REQ_BATTLEPASS_REWARD_DATA ()
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_REWARD_DATA;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
