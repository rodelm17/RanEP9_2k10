...
int CAgentThaiUserCheck::Execute (CServer* pServer)
{
	...
	case 35 : // EM_LOGIN_FB_LOGIN_DEVICE_CHECK_VERIFY
		NetMsgFB.nResult = EM_LOGIN_FB_LOGIN_DEVICE_CHECK_VERIFY;
		CConsoleMessage::GetInstance()->Write("CAgentThaiUserCheck result %d", nResult);
		break;
	}
...
CAgentUserCheck::CAgentUserCheck(
	...
	bool bMasterLogin			/* master login, Juver, 2020/06/18 */
	)
	: m_strUserID( szUserID )
	, m_strPasswd( szPasswd )	
	...
	, m_strPCID_HWID( szPCID_HWID )				//PCID
	, m_strPCID_MAC( szPCID_MAC )				//PCID
	, m_bPCIDSingleLogin( bPCIDSingleLogin )	//PCID
	, m_bPCIDCheckLogin( bPCIDCheckLogin )		//PCID

	, m_bMasterLogin(bMasterLogin)				/* master login, Juver, 2020/06/18 */
{	
	m_dwClient = dwClient;
	m_strUserIP = szUserIP;
}
...
int CAgentUserCheck::Execute(CServer* pServer)
{
	...
	int nResult = COdbcManager::GetInstance()->UserCheck(m_strUserID.GetString(),
														 ...
														 m_bPCIDSingleLogin,			//PCID
														 m_bPCIDCheckLogin,				//PCID
														 m_bMasterLogin					/* master login, Juver, 2020/06/19 */
														 );	
...
//	NetMsgFB.nThaiCC_Class = nResult1;
	...
	case 3 : // ID/PWD 가 일치하고, IP 정보는 없음 (일반적으로 로그인 성공)
		...
		nRetCode2 = COdbcManager::GetInstance()->getUserWatchlist( NetMsgFB.nUserNum, m_strPCID_HWID.GetString(), m_strPCID_MAC.GetString() );
		if ( nRetCode2 == 100 )
		{
			NetMsgFB.bUserFlagHWIDWatchList = true;
		}
	...
	case 30 : // EM_LOGIN_FB_SUB_REQUIRETIME
		...
		//PCID
	case 31 : // EM_LOGIN_FB_SUB_PCID_ACTIVE
		{
			NetMsgFB.nResult = EM_LOGIN_FB_SUB_PCID_ACTIVE;
			CConsoleMessage::GetInstance()->Write("CAgentUserCheck result %d PCID Active", nResult);
			RLOG::LogFile( "pcid.txt", "Login User: %s IP: %s HWID: %s MAC: %s is active", 
				m_strUserID.GetString(), m_strUserIP.GetString(), m_strPCID_HWID.GetString(), m_strPCID_MAC.GetString() );
		}break;

	case 32 : // EM_LOGIN_FB_SUB_PCID_BLOCKED
		{
			NetMsgFB.nResult = EM_LOGIN_FB_SUB_PCID_BLOCKED;
			CConsoleMessage::GetInstance()->Write("CAgentUserCheck result %d PCID Blocked", nResult);
			RLOG::LogFile( "pcid.txt", "Login User: %s IP: %s HWID: %s MAC: %s is blocked", 
				m_strUserID.GetString(), m_strUserIP.GetString(), m_strPCID_HWID.GetString(), m_strPCID_MAC.GetString() );
		}break;
...
int CAgentDaumUserCheck::Execute( CServer *pServer )
{
	...
	case 30 : // EM_LOGIN_FB_SUB_REQUIRETIME
		...
		/* login device check, Juver, 2020/11/07 */
	case 35 : // EM_LOGIN_FB_LOGIN_DEVICE_CHECK_VERIFY
		nlfd2.nResult = EM_LOGIN_FB_LOGIN_DEVICE_CHECK_VERIFY;
		CConsoleMessage::GetInstance()->Write("CAgentDaumUserCheck result %d", nResult);
		break;
	}
