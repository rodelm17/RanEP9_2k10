...
HRESULT GLCrowClient::CreateCrow ( GLLandManClient *pLandManClient, SDROP_CROW *pCrowDrop, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_dwTyrannyClubOwner = pCrowDrop->m_dwTyrannyClubOwner;
	m_dwTyrannyClubMarkVer = pCrowDrop->m_dwTyrannyClubMarkVer;
	...
	m_wWoeOwner = pCrowDrop->m_wWoeOwner;
	...
	if ( pCrowDrop->dwFLAGS&SDROP_CROW::CROW_GEM )
	{
		...
		STARGETID sTargetID(m_pCrowData->m_sBasic.m_emCrow,m_dwGlobID,m_vPos);
	...
	{	/*pvp tyranny, Juver, 2017/08/24 */
		...
		for ( int i=0; i< TYRANNY_SCHOOL_SIZE; ++ i )
			DxEffGroupPlayer::GetInstance().DeletePassiveEffect( GLCONST_CHAR::strSchoolEffect[i].c_str(), STARGETID( m_pCrowData->m_sBasic.m_emCrow, m_dwGlobID, m_vPos ) );

		if ( m_wTyrannyOwner < TYRANNY_SCHOOL_SIZE )
			DxEffGroupPlayer::GetInstance().NewEffGroup( GLCONST_CHAR::strSchoolEffect[m_wTyrannyOwner].c_str(), matEffect, &STARGETID( m_pCrowData->m_sBasic.m_emCrow, m_dwGlobID, m_vPos ) );
	}
...
float GLCrowClient::GetMoveVelo ()
{
	...
	return fVELO * GLCROWLOGIC_CLIENT::GETMOVEVELO();
}
	...
	case GLAT_IDLE:
		{
			...
			if ( m_pCrowData->IsBlockStun() )	bPANT = FALSE;
...
animation_continue:
	...
	{
	case GLAT_MOVE:
		fSkinAniElap *= GLCROWLOGIC_CLIENT::GETMOVEVELO();
		break;
...
HRESULT GLCrowClient::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( IsValidBody() )
	{
		GLCROWLOGIC_CLIENT::UPDATE_DATA ( fTime, fElapsedTime, TRUE );
	}
...
void GLCrowClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_SKILLFACT_BRD:
		{
			...
			GLOGICEX::VARIATION ( m_dwNowHP, m_pCrowData->m_sBasic.m_dwHP, pNetMsg->nVAR_HP );
			GLOGICEX::VARIATION ( m_wNowMP, m_pCrowData->m_sBasic.m_wMP, pNetMsg->nVAR_MP );
			GLOGICEX::VARIATION ( m_wNowSP, m_pCrowData->m_sBasic.m_wSP, pNetMsg->nVAR_SP );
	...
	case NET_MSG_GCTRL_TYRANNY_CROW_OWNER:
		{
			...
			m_dwTyrannyClubOwner = pNetMsg->dwTyrannyClubOwner;
			m_dwTyrannyClubMarkVer = pNetMsg->dwTyrannyClubMarkVer;
	...
	{
		GLMSG::SNET_MSG_WOE_CROW_OWNER* pNetMsg = (GLMSG::SNET_MSG_WOE_CROW_OWNER*)nmg;
		m_wWoeOwner = pNetMsg->wWoeOwner;
	}break;
...
/*skill transform, Juver, 2018/09/09 */
void GLCrowClient::skill_transform_change_skin()
{
	if ( !m_pd3dDevice )	return;
	if ( !m_pSkinChar )		return;
	if ( !m_pCrowData )		return;

	std::string skin_file = m_pCrowData->m_sAction.m_strSkinObj.c_str();
