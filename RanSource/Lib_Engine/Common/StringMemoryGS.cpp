...
CStringMemoryGS::CStringMemoryGS ( UINT nBufSize ) 
	: m_dwOffSet(0)
	, m_nBufSize(0)
	, m_pBufferMem(NULL)
	, m_nBufSizeTemp(0)
...
CStringMemoryGS::~CStringMemoryGS ()
{
	delete[] m_pBuffer;
	Close ();
}

BOOL CStringMemoryGS::Open (	LPCSTR _szZipFileName, 
							LPCSTR _szFileName, 
							BOOL bDecode, 
							BOOL bPack )
{
	GASSERT(m_pBufferMem==NULL&&"CStringMemoryGS::Open ()");
...
//		if( CRijndael::VERSION == nVersion )
		{
			...
			m_oRijndael.Initialize( 
				CRijndaelGS::sm_Version[nVersion-1].c_str(), 
				CRijndaelGS::sm_KeyLength[nVersion-1], 
				m_nVersionDate, 
				m_nVersion,
...
DWORD CStringMemoryGS::GetNextLine ( CString &strString )
{
...
DWORD CStringMemoryGS::GetNextLine ( PSTR &szString )
{
	if( m_pBufferMem == NULL ) return 0;
...
DWORD CStringMemoryGS::GetNextLine ( LPSTR szLine, UINT iLineSize )
{
	if( m_pBufferMem == NULL ) return 0;
	...
	*chTemp = 0;
	...
	while ( !bStop )
	{
		...
		for ( ; m_dwIndex<m_dwRead; m_dwIndex++ )
		{
			...
			{
				TCHAR szError[260] = {0};
				StringCchPrintf( szError, 260, "CStringMemoryGS::m_pBuffer Not enough. File:%s, Line:%u(Over Char Number:%d)",
