#pragma once

#include "../Lib_Client/G-Logic/GLItemMan.h"
#include "../Lib_Client/G-Logic/GLItem.h"
#include <vector>

// ItemBox dialog

class CItemBox : public CDialog
{
	DECLARE_DYNAMIC(CItemBox)

public:
	CItemBox(CWnd* pParent = NULL);   // standard constructor
	virtual ~CItemBox();

// Dialog Data
	enum { IDD = IDD_DIALOG_ITEMBOX };

public:
	CListBox m_listSELECT;
	CListBox m_listSELECTED;

	typedef std::vector<ITEM::SBOX_ITEM>	VEC;
	typedef VEC::iterator					VEC_ITER;

	VEC m_vecSELECTED;

	std::vector<SNATIVEID> m_vecData;
	std::vector<SNATIVEID> m_vecResult;

public:
	void	ReadData();
	void	ResetSelection();
	void	ShowSelections();
	void	ShowSelected();
	void	SearchData();

protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support

	DECLARE_MESSAGE_MAP()

public:
	virtual BOOL OnInitDialog();

	afx_msg void OnBnClickedButtonInsert();
	afx_msg void OnLbnSelchangeListSelect();
	afx_msg void OnBnClickedButtonRemove();
	afx_msg void OnBnClickedButtonSearch();
};
