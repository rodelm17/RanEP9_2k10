...
void DxInputDevice::InitKeyState ()
{
	...
	memset ( m_dwLastKeyboardTime, 0, sizeof ( DWORD ) * 256 );
...
void DxInputDevice::UpdateKeyState  ()
{
	...
	for ( DWORD i = 0; i < m_dwKeyElements; i++ )
	{
		...
		{
			/*quick slot delay, Juver, 2017/10/29 */
			if ( RPARAM::bQuickSlotDelay && m_dwLastKeyboardTime[nScanCode] != 0 &&
				m_KeyDidodata[i].dwTimeStamp - m_dwLastKeyboardTime[nScanCode] < RPARAM::dwQuickSlotDelay )
			{
				if ( nScanCode == RPROFILE::QuickSlot[0] || 
					nScanCode == RPROFILE::QuickSlot[1] || 
					nScanCode == RPROFILE::QuickSlot[2] ||
					nScanCode == RPROFILE::QuickSlot[3] || 
					nScanCode == RPROFILE::QuickSlot[4] || 
					nScanCode == RPROFILE::QuickSlot[5] )
				{
					continue;
				}
			}
			...
			/*quick slot delay, Juver, 2017/10/29 */
			m_dwLastKeyboardTime[nScanCode] = m_KeyDidodata[i].dwTimeStamp;
		}
