...
#define MAX_USER_ID USR_ID_LENGTH
...
#define USR_FULLNAME_LENGTH	50	//max size for userfullname

#define MAX_CHAR_ID CHR_ID_LENGTH
...
struct SGMTOOL_USER_SIMPLE
{
	...
	TCHAR		szUserFullName[USR_FULLNAME_LENGTH];
	WORD		wUserType;
		...
		, dwUserPoint(0)	
	{
		...
		SecureZeroMemory( szUserFullName, sizeof(char)*USR_FULLNAME_LENGTH );
	};
};
...
struct SGMTOOL_USER_NEW
{
		...
		, bUserFlagRestricted(false)
	{
		...
		memset( szUserFullName, 0, sizeof(char)*USR_FULLNAME_LENGTH );
	};
};
...
struct SGMTOOL_DATA_USER
{
	...
	//PCID
	TCHAR		szUserPCID_HWID[PCID_LENGTH];
	TCHAR		szUserPCID_MAC[PCID_LENGTH];
		...
		, wServerNumber(0)
	{
		...
		//PCID
		memset( szUserPCID_HWID, 0, sizeof(char)*PCID_LENGTH );
		memset( szUserPCID_MAC, 0, sizeof(char)*PCID_LENGTH );
	};
};
...
struct SGMTOOL_DATA_USER_IPLOG
{
	...
	TCHAR		szLogHWID[PCID_LENGTH];
	TCHAR		szLogMAC[PCID_LENGTH];
	__time64_t	tLogDate;
		...
		: tLogDate(0)
	{
		...
		memset( szLogHWID, 0, sizeof(char)*PCID_LENGTH );
		memset( szLogMAC, 0, sizeof(char)*PCID_LENGTH );
	};
};
