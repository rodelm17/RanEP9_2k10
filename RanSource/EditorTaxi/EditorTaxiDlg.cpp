...
#include "../Lib_Client/G-Logic/GLTaxiStation.h"
...
BEGIN_MESSAGE_MAP(CEditorTaxiDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_TAXI_COPY, OnBnClickedButtonTaxiCopy)
END_MESSAGE_MAP()
...
void CEditorTaxiDlg::OnBnClickedButtonFileLoad()
{
	...
	CFileDialog dlg( TRUE, ".tsf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
...
void CEditorTaxiDlg::OnBnClickedButtonFileSave()
{
	...
	CString szFilter = "TaxiStation Setting File (*.tsf)|*.tsf|";
	CFileDialog dlg( FALSE, ".tsf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
...
void CEditorTaxiDlg::OnBnClickedButtonTaxiCopy()
{
	int nSelect = m_ListTaxi.GetNextItem(-1, LVNI_ALL | LVNI_SELECTED);
	if( nSelect == -1 ) return;
	DWORD dwLinkID = m_ListTaxi.GetItemData( nSelect );
	...
	STAXI_STATION sStation = vecTaxiData[nSelect];
	if ( sStation.dwLINKID != dwLinkID )	return;
