	...
	MSG_LIST* pMsg )
{
	...
	CMsgCount::GetInstance().Add( nmg->nType, nmg->dwSize );
...
void CFieldServer::MsgJoinInfoFromClient(MSG_LIST* pMsg)
{
	...
	if ( !(pMsgIdent->emType==EMJOINTYPE_FIRST ||
		...
		pMsgIdent->emType==EMJOINTYPE_REBIRTH ||
		pMsgIdent->emType==EMJOINTYPE_PVP_CLUB_DEATH_MATCH_REBIRTH ) )											
		return;
	...
	{
		if( strcmp( pMsgIdent->client_hash, m_client_hash1 ) != 0 && strcmp( pMsgIdent->client_hash, m_client_hash2 ) != 0 )
		{
			//CDebugSet::ToFileWithTime( "_client_hash_check.txt", "char [%u] %s join failed mismatched hash:%s to %s or %s", pGLChar->m_dwCharID, pGLChar->m_szName, 
			//	pMsgIdent->client_hash, m_client_hash1, m_client_hash2 );
...
#ifdef USE_MSHIELD
	if ( pMsgIdent->nmShieldResult % MSHIELD_KEY != 0 )
	{
		CDebugSet::ToFileWithTime( "_mshield_login.txt", "char [%u] %s join failed result:%u invalid from key:%u multi:%u", pGLChar->m_dwCharID, pGLChar->m_szName, pMsgIdent->nmShieldResult, pMsgIdent->nmShieldKey, MSHIELD_KEY );
	...
	if( strcmp( pMsgIdent->szMShieldDllHash1, m_szMShieldDLLHash1 ) != 0 )
	{
		CDebugSet::ToFileWithTime( "_mshield_login.txt", "char [%u] %s join failed mismatched DLL1 hash:%s to %s", pGLChar->m_dwCharID, pGLChar->m_szName, pMsgIdent->szMShieldDllHash1, m_szMShieldDLLHash1 );
	...
	if( strcmp( pMsgIdent->szMShieldDllHash2, m_szMShieldDLLHash2 ) != 0 )
	{
		CDebugSet::ToFileWithTime( "_mshield_login.txt", "char [%u] %s join failed mismatched DLL2 hash:%s to %s", pGLChar->m_dwCharID, pGLChar->m_szName, pMsgIdent->szMShieldDllHash2, m_szMShieldDLLHash2 );
	...
	case EMJOINTYPE_REBIRTH:
		...
		/* pvp club death match, Juver, 2020/11/26 */
	case EMJOINTYPE_PVP_CLUB_DEATH_MATCH_REBIRTH:
		{
			//	부활 확인 메시지.
...
void CFieldServer::MsgGameJoinChar ( MSG_LIST* pMsg )
{
	...
	m_pClientManager->SetUserFlagVerified( dwClient, pNgj->bUserFlagVerified );

	/* user flag restricted, Juver, 2020/04/20 */
	m_pClientManager->SetUserFlagRestricted( dwClient, pNgj->bUserFlagRestricted );
	...
	pDBAction->Init( dwClient, 
					...
					dwChaNum, 
					sINFO, 
					pNgj->bUserFlagVerified,
					pNgj->bUserFlagRestricted,
					pNgj->dwPPoints,
					pNgj->dwVPoints );
...
//	char strTemp[100];
	...
	while (token != NULL)
	{
		...
		if( strcmp(token, "rehash") == 0 )
		{
			client_check_rehash();
			return 0;
		}

		if( strcmp(token, "msg_count_on") == 0 )
		{
			CMsgCount::GetInstance().Start();
			CConsoleMessage::GetInstance()->Write("msg_count_on");
			return 0;
		}

		if( strcmp(token, "msg_count_off") == 0 )
		{
			CMsgCount::GetInstance().Stop();
			CConsoleMessage::GetInstance()->Write("msg_count_off");
			return 0;
		}
