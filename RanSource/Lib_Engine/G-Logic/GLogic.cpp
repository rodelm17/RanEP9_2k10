...
namespace GLOGIC
{
	...
	void OpenPackFile ( const char * szAppPath )
	{
		if( bGLOGIC_PACKFILE )
		{
			...
			strPackDataPath += RPARAM::RPFDataPath;
	...
	basestream* openfile_basestream (	BOOL bZIP, 
										...
										const char* szFile, 
										EMRCRYPT_BYTE_TYPE emCryptType,
										BOOL bPack )
	{
		basestream *pBStream;
		if ( bZIP )
		{
			...
			if( !pMemory->OpenFile ( szZipFile, szFile, emCryptType, bPack ) )
			{
				SAFE_DELETE(pMemory);
				return NULL;
			}
		...
		else
		{
			...
			if ( !pFile->OpenFile ( FOT_READ, szFullPath, emCryptType ) )
			{
				SAFE_DELETE(pFile);
				return NULL;
			}
