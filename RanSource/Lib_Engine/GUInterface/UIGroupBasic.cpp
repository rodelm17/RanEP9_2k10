...
HRESULT CUIGroup::FrameMove( LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	if( !IsVisible() ) return S_OK;

	HRESULT hr = S_OK;
	hr = CUIControl::FrameMove ( pd3dDevice, fTime, fElapsedTime );
	if( FAILED( hr ) ) return hr;
	...
	for ( ; citer != citer_end; ++citer )
	{
		...
		hr = pControl->FrameMove ( pd3dDevice, fTime, fElapsedTime );
		if ( FAILED ( hr ) ) return hr;
	}
