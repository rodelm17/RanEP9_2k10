...
SINVENITEM* GLInventory::FindItem ( const SNATIVEID sNID, const WORD wGrade, const WORD wQuantity )
{
	CELL_MAP_ITER iter = m_ItemMap.begin();
	CELL_MAP_ITER iter_end = m_ItemMap.end();
	for ( ; iter!=iter_end; ++iter )
	{
		SINVENITEM* pInvenItem = (*iter).second;
		if (pInvenItem->sItemCustom.sNativeID == sNID && pInvenItem->sItemCustom.GETGRADE(EMGRINDING_NONE) == wGrade && pInvenItem->sItemCustom.wTurnNum == wQuantity)
		{
			return pInvenItem;
		}
	}
