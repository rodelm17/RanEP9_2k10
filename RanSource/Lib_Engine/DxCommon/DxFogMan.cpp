...
void	FOG_PROPERTY::LoadSet ( CSerialFile &SFile )
{
	...
	{
		CDebugSet::ErrorVersion( "FOG_PROPERTY::LoadSet", SFile.GetFileName(), dwVer );
...
DxFogMan::DxFogMan() :
	...
	m_bFogEnable(FALSE),
	m_bManualRange( FALSE ),
	m_fManualRange_Near( m_fRangeHigh_Near ),
	m_fManualRange_Far( m_fRangeHigh_Far ),
	m_fLastNear(720.0f),
	m_fLastFar(790.0f),
	m_fRangeMultiplier(1.0f)			/* camera zoom gm command, Juver, 2020/06/11 */
{
}
...
void DxFogMan::SetProperty ( FOG_PROPERTY& Property )
{
	...
	/* camera zoom gm command, Juver, 2020/06/11 */
	m_fRangeMultiplier = RENGINE_GLOBAL::fCameraZoomMultiplier;
}

float DxFogMan::GetFogRangeFar()
{
	if ( m_bManualRange )	return m_fManualRange_Far;
	// 강제 셋팅
...
void	DxFogMan::RenderFogSB ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bLobby /*= FALSE*/ )
{
	...
	if ( m_bFogStaticRange && m_emFogRange == EMFR_NSIZE )					// 강제 셋팅일 경우이다.
	{
	...
	if ( m_bManualRange )
	{
		fFogStart = m_fManualRange_Near;
		fFogEnd = m_fManualRange_Far;
	}
	...
	/* camera zoom gm command, Juver, 2020/06/11 */
	fFogStart = fFogStart * m_fRangeMultiplier;
	fFogEnd = fFogEnd * m_fRangeMultiplier;


	DxViewPort::GetInstance().SetFarPlane ( fFogEnd );				// View Cliping -> DxViewPort FrameMove Setting
	DxViewPort::GetInstance().SetProjection_FarPlane_Fog();	// 최대 시아를 제한한다.
	...
	m_fLastNear = fFogStart;
	m_fLastFar = fFogEnd;
}

void	DxFogMan::ManualRangeSet( BOOL bENABLE, float fNear, float fFar )
{
	m_bManualRange = bENABLE;

	if ( !m_bManualRange )
	{
		fNear = m_fRangeHigh_Near;
		fFar = m_fRangeHigh_Far;
	}

	m_fManualRange_Near = fNear;
	m_fManualRange_Far = fFar;
}

void	DxFogMan::ManualRangeGet( float& fNear, float& fFar )
{
	fNear = m_fManualRange_Near;
	fFar = m_fManualRange_Far;
}

BOOL	DxFogMan::ManualRangeIsDefault ()
{
	if ( !m_bManualRange )
		return TRUE;

	if ( m_bManualRange && ( m_fManualRange_Near <= m_fRangeHigh_Near && m_fManualRange_Far <=  m_fRangeHigh_Far ) )
		return TRUE;
...
/* camera zoom gm command, Juver, 2020/06/11 */
void DxFogMan::SetRangeMultiplier( float fVal )
{
	m_fRangeMultiplier = fVal;
}