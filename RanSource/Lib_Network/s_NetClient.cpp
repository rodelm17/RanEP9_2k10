...
int CNetClient::ConnectLoginServer( SLOGIN_ADDRESS* pLogin )
{	
	...
	{
		int nReturn = ConnectServer(pLogin[nLogin].strLoginIP.c_str(), pLogin[nLogin].nLoginPort, NET_STATE_LOGIN);
		if ( nReturn == NET_OK )
		{
			//login found stop connecting
			return nReturn;
			break;
		}
	}
