...
CClubWarRankingsDisplaySlot::CClubWarRankingsDisplaySlot ()
: m_pTextRank(NULL)
, m_pTextName(NULL)
...
void CClubWarRankingsDisplaySlot::CreateSubControl ()
{
	...
	m_pLineBox->CreateBaseBoxQuestList ( "PVP_TYRANNY_RANKING_DISPLAY_SLOT_BASE_LINEBOX" );
	...
	m_pTextRank->CreateSub ( this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_TEXT_BASE_RANK" );
	...
	m_pTextName->CreateSub ( this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_TEXT_BASE_NAME" );
	...
	m_pTextScoreKill->CreateSub ( this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_TEXT_BASE_SCORE_KILL" );
	...
	m_pTextScoreDeath->CreateSub ( this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_TEXT_BASE_SCORE_DEATH" );
	...
	m_pTextScoreResu->CreateSub ( this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_TEXT_BASE_SCORE_RESU" );
	...
	m_pIconDeath->CreateSub(this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_ICON_DEATH", UI_FLAG_DEFAULT);
	RegisterControl(m_pIconDeath);


	m_pIconResu = new CUIControl;
	m_pIconResu->CreateSub(this, "PVP_TYRANNY_RANKING_DISPLAY_SLOT_ICON_RESU", UI_FLAG_DEFAULT);
	RegisterControl(m_pIconResu);
}

void CClubWarRankingsDisplaySlot::ResetData()
{
	m_sClubWarRank	= SCLUBWAR_RANK();
...
HRESULT CClubWarRankingsDisplaySlot::Render(LPDIRECT3DDEVICEQ pd3dDevice)
{
	...
	BOOL bGuild = m_sClubWarRank.wGuNum ? TRUE : FALSE;
	if (bGuild && m_sClubWarRank.wGuNum > 0)
	{
		m_pIconGuild->SetVisibleSingle(TRUE);
		const int nSERVER					= GLGaeaClient::GetInstance().GetCharacter()->m_dwServerID;
		const DxClubMan::DXDATA& sMarkData	= DxClubMan::GetInstance().GetClubData(pd3dDevice, nSERVER, m_sClubWarRank.wGuNum, m_sClubWarRank.wGuMarkVer);
...
void CClubWarRankingsDisplaySlot::SetData( SCLUBWAR_RANK sClubWarRank, int nRank, std::string strName, DWORD dwCharID, int nClass,
											 WORD wGuNum, WORD wGuMarkVer, WORD wKill, WORD wDeath , WORD wResu, WORD wSchool )
{
	ResetData();

	m_sClubWarRank = sClubWarRank;
