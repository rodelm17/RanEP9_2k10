	...
	case SKILL_COMBO_ROLLOVER0:
		{
			...
			{				
				...
				if( nIndex > 0 )
				{
					...
					if ( GLGaeaClient::GetInstance().GetCharacter()->isValidPetBlockTime() )
					{
						CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMPET_REQ_SKILL_CHANGE_BLOCKED") );
						return;
					}
				}
...
void CPetWindow::SetFocusControl()					
{
...
void CPetWindow::ResetFocusControl()
{
