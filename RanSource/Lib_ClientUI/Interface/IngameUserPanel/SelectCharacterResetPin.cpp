...
#include "SelectCharacterResetPin.h"
...
int	CSelectCharacterResetPin::nLIMIT_PW = 11;
int CSelectCharacterResetPin::nLIMIT_EMAIL = 50;

CSelectCharacterResetPin::CSelectCharacterResetPin ()
	: nLIMIT_CAPTCHA(6)
	, strCaptcha( "" )
...
void CSelectCharacterResetPin::CreateSubControl ()
{
	...
	pTextBox->SetOneLineText ( "E-Mail" );	
...

	{
		...
		pEditBoxMan->CreateEditBox ( SELCHAR_EDIT_EMAIL, "CHARPAGE_CHANGE_EMAIL_PW2_EDIT", "REG_CARRAT", TRUE, UINT_MAX, pFont9, nLIMIT_EMAIL );
...
void CSelectCharacterResetPin::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{	
...
void CSelectCharacterResetPin::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case SELCHAR_BUTTON_OK:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				std::string strEmail = m_pEditBoxMan->GetEditString( SELCHAR_EDIT_EMAIL );

				if ( strPW.empty() || strEmail.empty() )
				{
				...
				if( !CheckString_Special2( strEmail.c_str() ) )
				{
					DoModalOuter ( "Invalid E-Mail format" );
					return;
				}

				CNetClient* pNetClient = DxGlobalStage::GetInstance().GetNetClient ();

				pNetClient->SndAccountResetPin( strPW.c_str(), strEmail.c_str() );		
...
void CSelectCharacterResetPin::ResetAll ( bool bEdit )
{
...
CBasicButton* CSelectCharacterResetPin::CreateFlipButton ( char* szButton, char* szButtonFlip, UIGUID ControlID )
{
...
BOOL CSelectCharacterResetPin::CheckString_Special2( CString strTemp )
{	
