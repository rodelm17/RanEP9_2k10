...
void CNPCItemExchangeWindow::LoadList( SNATIVEID sid_npc )
{
	...
	SNPC_ITEM_EXCHANGE* pItemExchange = GLCrowDataMan::GetInstance().NPCItemExchangeFind( pcrow->m_sAction.m_strItemExchangeFile );
	if ( !pItemExchange )	return;

	if ( pItemExchange->m_strTitle.size() )
		SetTitleName( pItemExchange->m_strTitle.c_str() );

	m_sidNPC = pcrow->m_sBasic.sNativeID;

	SNPC_ITEM_EXCHANGE_DATA_POINTER_MAP_ITER it_b = pItemExchange->m_mapData.begin();
	SNPC_ITEM_EXCHANGE_DATA_POINTER_MAP_ITER it_e = pItemExchange->m_mapData.end();
	for ( ; it_b != it_e; ++it_b )
	{
		const SNPC_ITEM_EXCHANGE_DATA& npc_exchange_data = *(*it_b).second;
