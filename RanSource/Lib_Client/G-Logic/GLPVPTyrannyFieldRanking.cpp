...
void GLPVPTyrannyField::InsertPlayerData( TYRANNY_REGISTER_DATA* pPlayerData )
{
	...
	sdata.bLateJoin		= pPlayerData->bLateJoin;
...
void GLPVPTyrannyField::ScoreKill( const SNATIVEID& sMapID, DWORD dwCharKilled, DWORD dwCharKiller )
{
	if ( m_emState != TYRANNY_STATE_BATTLE )	
		return;

	if ( sMapID != m_sScheduleNext.getBatteMap() )
		return;
...
void GLPVPTyrannyField::ScoreDamage( const SNATIVEID& sMapID, DWORD dwChar, DWORD dwDamage )
{
	if ( m_emState != TYRANNY_STATE_BATTLE )	
		return;
...
void GLPVPTyrannyField::ScoreHeal( const SNATIVEID& sMapID, DWORD dwChar, DWORD dwHeal, DWORD dwCharHealer )
{
	if ( m_emState != TYRANNY_STATE_BATTLE )	
		return;
...
void GLPVPTyrannyField::ScoreResu( const SNATIVEID& sMapID, DWORD dwChar )
{
	if ( m_emState != TYRANNY_STATE_BATTLE )	
		return;
...
void GLPVPTyrannyField::RankingSend()
{
	...
	for ( size_t i=0; i<vecdata.size(); ++i )
	{
		...
		{
			GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( m_sScheduleNext.getBatteMap().dwID, &netmsg_ranking );
	...
	GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( m_sScheduleNext.getBatteMap().dwID, &netmsg_ranking_end );
}