...
#include "../DxCommon/LandTypeDef.h"
...
HRESULT DxLandMan::Render ( LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME &cv )
{
	...
	//	Note : Optimum Octree
	//
	PROFILE_BEGIN("DxOptimumOctree::Render_STATIC_MESH");
	if ( m_pStaticMesh )
	{
	...
	PROFILE_END("DxOptimumOctree::Render_STATIC_MESH");


	PROFILE_BEGIN("DxOptimumOctree::Render_STATIC_MATERIAL_MESH");

	if ( m_pStaticMaterialMesh )
	{
		m_sMaterialWld.g_fVS_1_1_ColorMulti = m_sbasic.m_fVS_1_1_ColorMulti;
		m_sMaterialWld.Update( pd3dDevice, matIdentity );
		m_pStaticMaterialMesh->Render_THREAD_Normal( pd3dDevice, cv, m_sMaterialWld );
		m_pStaticMaterialMesh->Render_THREAD_MaterialEffect( pd3dDevice, cv, m_sMaterialWld );
		m_pStaticMaterialMesh->Render_THREAD_Blending ( pd3dDevice, cv, m_sMaterialWld );
		m_pStaticMaterialMesh->Render_THREAD_Alpha ( pd3dDevice, cv, m_sMaterialWld );
	}

	PROFILE_END("DxOptimumOctree::Render_STATIC_MATERIAL_MESH");
	...
	if ( m_pStaticSoundMan )
		m_pStaticSoundMan->Render ( pd3dDevice );

	// Note : 조각 파일 매니져
	m_sMaterialPie.g_fVS_1_1_ColorMulti = m_sbasic.m_fVS_1_1_ColorMulti;
	m_PieceManager.Render( pd3dDevice, cv, m_sMaterialPie );
...
HRESULT DxLandMan::Render_EFF ( LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME &cv )
{
	...
	if ( m_pStaticMaterialMesh )
	{
		m_sMaterialWld.g_fVS_1_1_ColorMulti = m_sbasic.m_fVS_1_1_ColorMulti;
		m_sMaterialWld.Update( pd3dDevice, matIdentity );
		m_pStaticMaterialMesh->Render_THREAD_SpecularBlending ( pd3dDevice, cv, m_sMaterialWld );
		m_pStaticMaterialMesh->Render_THREAD_SoftAlpha ( pd3dDevice, cv, m_sMaterialWld );
	}
	...
	if ( m_pshadowmesh )
		m_pshadowmesh->Render( pd3dDevice, matIdentity );
	...
	// Note : 조각 파일 매니져
	m_sMaterialPie.g_fVS_1_1_ColorMulti = m_sbasic.m_fVS_1_1_ColorMulti;
	m_PieceManager.RenderAlpha( pd3dDevice, cv, NULL, NULL, m_pStaticMesh, m_pStaticMaterialMesh, m_sMaterialPie );
...
void DxLandMan::RenderPickAlpha( LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME &cv )
{
	DxReplaceContainer::DrawRePlacePiecePickAlpha( pd3dDevice );		// Note : Prev PieceManager

	m_sMaterialPie.g_fVS_1_1_ColorMulti = m_sbasic.m_fVS_1_1_ColorMulti;
	m_PieceManager.RenderPickAlpha( pd3dDevice, cv, NULL, NULL, m_pStaticMesh, m_pStaticMaterialMesh, m_sMaterialPie );	// Note : Recent PieceManager
}
...
HRESULT	DxLandMan::Render_Reflect( LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME &cv )
{
	...
	// Note : 조각 파일 매니져
	m_sMaterialPie.g_fVS_1_1_ColorMulti = m_sbasic.m_fVS_1_1_ColorMulti;
	m_PieceManager.Render_Reflect( pd3dDevice, m_sMaterialPie );
						...
						LPDXFRAME &pDxFrame, const BOOL bFrontColl )
{
	...
	if ( m_pStaticMesh && m_pStaticMaterialMesh )
	{
		if( m_pStaticMesh->IsCollisionLine( vStart, vEnd, vCollision, vNewNormal, szName, bFrontColl ) )	bCollision = TRUE;
		else if( m_pStaticMaterialMesh->IsCollisionLine( vStart, vEnd, vCollision, vNewNormal, szName, bFrontColl ) )	bCollision = TRUE;
		else																								bCollision = FALSE;
	}
	else if ( m_pStaticMaterialMesh )
	{
		if( m_pStaticMaterialMesh->IsCollisionLine( vStart, vEnd, vCollision, vNewNormal, szName, bFrontColl ) )	bCollision = TRUE;
		else																								bCollision = FALSE;
	}
							...
							LPDXFRAME &pDxFrame, LPCSTR& szName, D3DXVECTOR3 *vNormal )
{
	...
	if ( m_pStaticMesh && m_pStaticMaterialMesh )
	{
		...
		else if( m_pStaticMaterialMesh->IsCollisionLine( vStart, vNewEnd, vCollision, vNewNormal, szName, TRUE ) )	bCollision = TRUE;
		else																						bCollision = FALSE;
	}
	else if ( m_pStaticMaterialMesh )
	{
		if( m_pStaticMaterialMesh->IsCollisionLine( vStart, vNewEnd, vCollision, vNewNormal, szName, TRUE ) )	bCollision = TRUE;
		else																						bCollision = FALSE;
	}
	else if ( m_pStaticMesh )
	{
...
void DxLandMan::IsCollisionNEW( const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, D3DXVECTOR3 &vCollision, BOOL &bCollision, const BOOL bBackColl, const BOOL bPiece )
{
	if ( m_pStaticMesh && m_pStaticMaterialMesh )
	{
		bCollision = FALSE;
		...
		else if( m_pStaticMaterialMesh->IsCollisionLine( vStart, vNewEnd, vCollision, vNewNormal, szName, !bBackColl ) )	bCollision = TRUE;
		else																								bCollision = FALSE;
	...
	else if ( m_pStaticMaterialMesh )
	{
		...
		const char* szName = NULL;
		D3DXVECTOR3 vNewNormal(0.f,1.f,0.f);
		if( m_pStaticMaterialMesh->IsCollisionLine( vStart, vNewEnd, vCollision, vNewNormal, szName, !bBackColl ) )	bCollision = TRUE;
		else																								bCollision = FALSE;
...
void DxLandMan::CleanUp ()
{
	...
	if( m_pStaticMaterialMesh )
	{
		m_pStaticMaterialMesh->EndThread();
		SAFE_DELETE( m_pStaticMaterialMesh );
	}

	SAFE_DELETE( m_pTexEffect );
	...
	if ( m_pshadowmesh )
	{
		m_pshadowmesh->CleanUp();
		SAFE_DELETE( m_pshadowmesh );
	}
}


void DxLandMan::CopyMeshAndCreateShadowMesh( const LPDIRECT3DDEVICEQ pd3dDevice )
{
	VECDXMESH vecmesh;

	if ( m_pStaticMaterialMesh )
		m_pStaticMaterialMesh->CopyMesh( pd3dDevice, vecmesh );

	SAFE_DELETE( m_pshadowmesh );
	m_pshadowmesh = new DxShadowMesh;
	m_pshadowmesh->CopyMesh( pd3dDevice, vecmesh );
}

void DxLandMan::CopyNaviAndCreateShadowMesh( const LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( !m_pNaviMesh )	return;

	VECDXMESH vecmesh;
	SCOPYMESHDATA data;

	m_pNaviMesh->CopyNaviToShadow( pd3dDevice, data );
	vecmesh.push_back( data );

	SAFE_DELETE( m_pshadowmesh );
	m_pshadowmesh = new DxShadowMesh;
	m_pshadowmesh->CopyMesh( pd3dDevice, vecmesh );

	SAFE_RELEASE( data.pIB );
	SAFE_RELEASE( data.pVB );
...
void DxLandMan::NaviFileCreateShadowMesh( const LPDIRECT3DDEVICEQ pd3dDevice, const TCHAR* file_name )
{
	if ( !m_pNaviMesh )	return;
	...
	StringCchCat( szPathName, MAX_PATH, file_name );

	NavigationMesh*	pNaviMesh = new NavigationMesh;
	BOOL file_load = pNaviMesh->Load_File( szPathName, pd3dDevice );
	if ( !file_load )
	{
	...
	pNaviMesh->CopyNaviToShadow( pd3dDevice, data );
	vecmesh.push_back( data );
