...
char		DxEffCharLine2BoneEff::NAME[MAX_PATH]	= "DxEffCharLine2BoneEff"; 
...
void EFFCHAR_PROPERTY_LINE2BONEEFF::Assign( EFFCHAR_PROPERTY_LINE2BONEEFF_0102 &sOldProperty )
{
	m_dwFlag			= sOldProperty.m_dwFlag;
	m_dwOtherColor		= sOldProperty.m_dwOtherColor;
	m_dwParticlePec		= sOldProperty.m_dwParticlePec;
...
void DxEffCharLine2BoneEff::ParticleListTEX::CleanUp()
{
	...
	while( pCur )
	{
		pDel = pCur;
		pCur = pCur->m_pNext;
		SAFE_DELETE( pDel );	// m_pParticlePoolÀº NULL·Î µÇ¾î ÀÖ¾î¼­ ½º½º·Î Delete¸¦ ÇÒ ¼ö ¹Û¿¡ ¾ø´Ù.
	}
}
...
void DxEffCharLine2BoneEff::ParticleListTEX::FrameMove( EFFCHAR_PROPERTY_LINE2BONEEFF& sProp, DXDATA& sData, float fElapsedTime )
{
	...
	while( pCur )
	{
		// Note : »ý¸íÀÌ ´Ù µÇ¾ú´ÂÁöµµ ¿©±â¼­ Ã¼Å©
		if( pCur->FrameMove( sProp, sData, fElapsedTime ) )	// »èÁ¦ ¾øÀ½.
		{
			pPrev = pCur;	// ÀÌÀü À§Ä¡ ¹é¾÷
			pCur = pCur->m_pNext;
		}
		else if( pPrev )	// Áß°£ ³à¼®À» »èÁ¦	// ¹é¾÷ ÇÊ¿ä ¾ø´Ù. pPrevÀÌ±â ¶§¹®¿¡.
		{
			pDel = pCur;
													...
													const EFFCHAR_PROPERTY_LINE2BONEEFF& sProp, DxSequenceUV& sSequenceUV )
{
	...
	{
		vFromPt	= D3DXVECTOR3( 0.0f, 0.3f, -35.0f );
		vLookatPt = D3DXVECTOR3( -0.02f, 0.0f, 5.0f );
	}
	...
	while( pCur )
	{
		...
		if( sProp.m_bWithCamMove )
		{
			...
			if( vDot3.y < 0.f )	vDot3.y = -vDot3.y;		// À§ÂÊÀ¸·Î¸¸ ¹Ù¶óº¸µµ·Ï...
			vDot3.y += 0.1f;							// ¹Ù¶óº¸´Â ½ÃÁ¡°ú ¹æÇâÀÌ °°Áö ¾Êµµ·Ï ÇÏ±â À§ÇÔ..

			D3DXVec2Normalize( &vDot3, &vDot3 );

			// Note : »õ·Î¿î ¹æÇâÀ» ±¸ÇÑ´Ù.
		...
		vBasePos += vViewDir * pCur->m_fBoneToCamDist;	// ¾à°£ ¾ÕµÚ·Î È¿°ú¸¦ ÁØ´Ù.
		...
		}else
		{
			...
			if ( g_bPREVIEW_CHAR_RENDER )
			{
				D3DXVECTOR3 vFromPtLocal	= D3DXVECTOR3( 0.0f, 0.3f, -35.0f );
				D3DXVECTOR3 vLookatPtLocal	= D3DXVECTOR3( -0.02f, 0.0f, 5.0f );
				D3DXVECTOR3 vUpVecLocal		= D3DXVECTOR3( 0.0f, 1.0f, 0.0f );
				D3DXMATRIX matViewLocal;
				D3DXMatrixIdentity ( &matViewLocal );
				D3DXMatrixLookAtLH ( &matViewLocal, &vFromPtLocal, &vLookatPtLocal, &vUpVecLocal );

				matBillboard1 = DxBillboardLookAt( &vParticlePos1, matViewLocal );
				matBillboard2 = DxBillboardLookAt( &vParticlePos2, matViewLocal );
			}
		...
		// Note : ½ÃÄö½º¸¦ »ç¿ëÇÒ °æ¿ì¿Í ¾Æ´Ò °æ¿ì	
		if( sProp.m_dwFlag & USE_SEQUENCE )
		{
			pTexUV = sSequenceUV.GetTexUV( sSequenceUV.GetTexCount(pCur->m_fRate) );
			if( !pTexUV )
			{
				pCur = pCur->m_pNext;	// ¹®Á¦°¡ ÀÖÀ¸¹Ç·Î »ý¼ºÇÏÁö ¾Ê´Â´Ù.
				continue;
			}
	...
	// Note : nCount °¡ ÆÄÆ¼Å¬ °¹¼ö¸¦ ³ªÅ¸³½´Ù.
    if( nCount > 0 )
	{
		OPTMCharParticle::DATA* pData = OPTMManager::GetInstance().m_sCharParticle.GetData();

		// Note : LockÀ» ÇÏ±â À§ÇÑ D3DLOCK_DISCARD or D3DLOCK_NOOVERWRITE
...
HRESULT DxEffCharLine2BoneEff::FrameMove( float fTime, float fElapsedTime )
{
	...
	if( m_pSkeleton->pBoneRoot )
	{
		...
		// º»Á¤º¸´Â Ä³¸¯ÅÍ °øÀ¯ÇÏ±â ¶§¹®¿¡ ·£´õ¸µ ½ÃÁ¡¿¡ ¹Ì¸® ÁÂÇ¥¸¦ ¹Þ¾ÆµÐ´Ù..
...
void DxEffCharLine2BoneEff::RenderEff( LPDIRECT3DDEVICEQ pd3dDevice, LPD3DXMATRIX pMatrix, const float fScale )
{
	...
	if( m_fElapsedTimeADD > 0.0333f )
	{
		...
		m_fParticleADD += m_dwParticlePec*m_fElapsedTimeADD;	// Note : »ý¼º °¹¼ö¸¦ Á¦¾îÇÑ´Ù.
		m_fElapsedTimeADD = 0.f;
	}
	...
	// Note : ¾Ë¸ÂÀº ºñÀ²À» Á¤ÇÔ.
	DWORD dwParticleCreate = (DWORD)m_fParticleADD;	// »ý¼º °¹¼ö ±¸ÇÔ.
	...
	// Note : ÆÄÆ¼Å¬ »ý¼º. < LineÀ» µ·´Ù.. »ý¼ºµÇ´Â À§Ä¡´Â Á¤·ÄµÇ¾î ÀÖ´Ù. >
	float fRate(0.f);
	D3DXMATRIX matInv;
	D3DXMATRIX matLocal;
	...
	for( DWORD i=0; i<m_vecBoneLINE.size(); )
	{
		...
		if( pBoneTran1 && pBoneTran2 )
		{
				...
				(*iter) <= m_vecBoneLINE[i]->m_fLengthEnd )
			{
				...
				++iter;
				continue;	// °°Àº À§Ä¡¸¦ ÇÑ¹ø´õ Ã¼Å©ÇØ¾ß ÇÑ´Ù.
			}
		}
...
void DxEffCharLine2BoneEff::DxCreateParticle( const D3DXVECTOR3 vPos, DxBoneTrans* rBoneTran )
{
	...
	D3DXVec3TransformCoord( &pParticle->m_vPos, &vPos, &rBoneTran->matCombined );	// À§Ä¡.
	...
	pParticle->m_fLife = m_vLife.x + m_fLifeDiff*RANDOM_POS;		// ·£´ýÇÏ°Ô »ðÀÔ
	pParticle->m_fSize = m_vSize.x + m_fSizeDiff*RANDOM_POS;		// ·£´ýÇÏ°Ô »ðÀÔ

	vDir = D3DXVECTOR3( RANDOM_NUM, RANDOM_NUM, RANDOM_NUM );
	pParticle->m_vPosOffSet =  vDir*m_fCreateLength;			// ÀÏÁ¤ÇÑ °Å¸®¸¦ ¶ç¾î ³õ´Â´Ù.

	pParticle->m_fSizeBase = pParticle->m_fSize;				// ±âº»°ª
	pParticle->m_fSizeDiff = (pParticle->m_fSize*m_vSize.z) - pParticle->m_fSize;	// °ªÀÇ º¯È­¸¦ À§ÇØ
...
//	D3DXVec3TransformCoord( &m_vPos, &m_vBasePos, &m_rBoneTran->matCombined );
	...
	//// Note : À§Ä¡°¡ °°Áö ¾Ê´Ù¸é ¹æÇâÀ» º¯È­ ½ÃÅ²´Ù.
	...
	//{
	//	// Note : ¹æÇâÀ» ±¸ÇÔ.
	//	m_vDir += (m_vPrevPos - m_vPos) * sProp.m_fShake;	// ¿òÁ÷ÀÓ¿¡ µû¸¥ ºÒ±æÀÇ º¯È­¸¦ ÁØ´Ù.
	//	D3DXVec3Normalize( &m_vDir, &m_vDir );
	//}

//	// Note : ¹æÇâÀ» ±¸ÇÔ.
//	m_vDir += sData.vDir * sProp.m_fShake;	// ¿òÁ÷ÀÓ¿¡ µû¸¥ ºÒ±æÀÇ º¯È­¸¦ ÁØ´Ù.
//	D3DXVec3Normalize( &m_vDir, &m_vDir );

	//// Note : Ä«¸Þ¶ó ¹æÇâ°ú ¿òÁ÷ÀÓ ¹æÇâÀÇ ³»ÀûÀ¸·Î ¾ËÆÄ°­µµ¸¦ Á¶ÀýÇÑ´Ù.
...
void DxEffCharLine2BoneEff::InsertLine( const TCHAR* pName1, const TCHAR* pName2 )
{
	...
	// Note : ÀÎµ¦½º´Â ´Ù ±¸ÇßÀ¸¹Ç·Î Line Á¤º¸¿¡ ³Ö´Â´Ù.
...
HRESULT	DxEffCharLine2BoneEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		EFFCHAR_PROPERTY_LINE2BONEEFF_0102 sOldProperty;
		SFile.ReadBuffer( &sOldProperty, sizeof(EFFCHAR_PROPERTY_LINE2BONEEFF_0102) );
	...
	{
		//	Note : ÀÌÆÑÆ®ÀÇ Property ¸¦ ·Îµå.
		EFFCHAR_PROPERTY_LINE2BONEEFF_0101 sProperty;
		SFile.ReadBuffer( &sProperty, sizeof(EFFCHAR_PROPERTY_LINE2BONEEFF_0101) );
		m_Property.Assign(sProperty);
	...
	else if ( dwVer == 0x0100 )
	{
		...
		//	Note : ÀÌÆÑÆ®ÀÇ Property ¸¦ ·Îµå
		EFFCHAR_PROPERTY_LINE2BONEEFF_0100 sProperty;
		SFile.ReadBuffer( &sProperty, sizeof(sProperty) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharLine2BoneEff::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharLine2BoneEff::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if ( dwVer == 0x0101 )
	{
		//	Note : ÀÌÆÑÆ®ÀÇ Property ¸¦ ·Îµå.
		EFFCHAR_PROPERTY_LINE2BONEEFF_0100 sProperty;
		SFile.ReadBuffer( &sProperty, sizeof(EFFCHAR_PROPERTY_LINE2BONEEFF_0100) );
		m_Property.Assign(sProperty);

		//ran2 added member of EFFCHAR_PROPERTY
		//this is temp solution to load the file 
		//coding the support will break existing version
		DWORD dwEffectUseFlag = 0;
		SFile >> dwEffectUseFlag;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharLine2BoneEff::R2LoadFile", SFile.GetFileName(), dwVer );
...
void DxEffCharLine2BoneEff::SetSkeleton( DxSkeleton* pSkeleton )
{
	...
	// ´Ù¸¥ Ä³¸¯ÅÍ¿¡ ÀÇÇØ º»°ªÀÌ ¹Ù²î±âÀü¿¡ ¹Ì¸® ÁÂÇ¥°ªÀ» ÀÔ·Â¹Þ¾Æ µÐ´Ù.
	if( m_pSkeleton->pBoneRoot )
	{
		m_vCharPos.x = m_pSkeleton->pBoneRoot->matCombined._41;
		m_vCharPos.y = m_pSkeleton->pBoneRoot->matCombined._42;
