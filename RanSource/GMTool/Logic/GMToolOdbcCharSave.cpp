...
#include "../../Lib_Client/DxLobyStage.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif

int CGMToolOdbcBase::CharDataSave( SGMToolCharData* pData )
{
	if ( !pData ){
		return DB_ERROR;
	}
	...
	_snprintf( szTemp, 1024, "UPDATE ChaInfo SET ChaBright=%d, ChaLevel=%u, ChaMoney=%I64d, "
		...
		"ChaName='%s', ChaSex=%d, ChaScaleRange=%f, ChaCP=%u, ChaContributionPoint=%u, "
		"ChaActivityPoint=%u, ChaBadge='%s', ChaPKScore=%u, ChaPKDeath=%u, ChaWarChips=%u, ChaGamePoints=%u, "
		...
		pCharData2->m_dwWarChips,
		pCharData2->m_dwGamePoints,

		/* play time system, Juver, 2021/01/26 */
		pCharData2->m_llPlayTime,
		pCharData2->m_llPlayPoint,
	...
	PersonalLockWrite( pCharData2 );

	/* pk combo, Juver, 2021/02/11 */
	PKComboWrite( pCharData2 );

	/* award title, Juver, 2021/02/14 */
	ChaAwardTitleSave( pCharData2->m_dwCharID, pData->mapAwardTitle );

	CombatRecordWrite( pCharData2 );
