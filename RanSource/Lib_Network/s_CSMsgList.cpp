...
void CSMsgList::AddMsg(DWORD dwClient, void* pBuffer, int nSize)
{
	...
	//packet crash fix
	if( nSize < 0 || nSize > NET_DATA_BUFSIZE )
	{
		CConsoleMessage::GetInstance()->Write(_T("CSMsgList::AddMsg nSize < 0 || nSize > NET_DATA_BUFSIZE dwClient %u, nSize %d"), dwClient, nSize );
		return;
	}
