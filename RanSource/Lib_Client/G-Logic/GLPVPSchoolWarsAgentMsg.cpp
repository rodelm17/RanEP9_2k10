...
HRESULT GLPVPSchoolWarsAgent::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case NET_MSG_GCTRL_SCHOOLWARS_C2A_REGISTER_REQ:
		{
			...
			RequestPlayerRegister( pnetmsg->dwCharID, pnetmsg->wLevel, pnetmsg->dwContri, pnetmsg->bRegister, pnetmsg->szHASH );
		}break;
	...
	case NET_MSG_GCTRL_SCHOOLWARS_F2A_MOB_KILL_UPDATE:
		{
			GLMSG::SNETPC_SCHOOLWARS_F2A_MOB_KILL_UPDATE* pnetmsg = ( GLMSG::SNETPC_SCHOOLWARS_F2A_MOB_KILL_UPDATE* ) nmg;

			SCHOOLWARS_CROW_SCORE* pcrow_score = GetCrowScore( pnetmsg->id_crow );
			PGLCHARAG pchar = GLAgentServer::GetInstance().GetCharID( pnetmsg->id_char );
			if ( pcrow_score && pchar )
			{
				WORD wPoint = pcrow_score->score_total;
				if ( m_bDoublePoint )
					wPoint = wPoint + wPoint;

				m_sScore[pchar->m_wSchool].wKills += wPoint;
				...
				GLMSG::SNETPC_SCHOOLWARS_A2F_MOB_KILL_UPDATE netmsgfield;
				netmsgfield.id_char = pnetmsg->id_char;
				netmsgfield.id_crow = pnetmsg->id_crow;
				netmsgfield.score_crow = wPoint;
				netmsgfield.double_point = m_bDoublePoint;
				netmsgfield.char_school = pchar->m_wSchool;
