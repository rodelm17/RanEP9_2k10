...
* vMember : 클럽멤버
*/
int COdbcManager::GetClubMember(DWORD dwClub, std::vector<GLCLUBMEMBER> &vMember, int nNameType)
{
	SQLRETURN sReturn = 0;
 ...
 //   strTemp << " ORDER BY ChaName";
	...
	TCHAR szTemp[512] = {0};
	_snprintf( szTemp, 512, "SELECT ci.ChaNum, ci.ChaName, (ci.ChaName + '#' + CONVERT(varchar(10),ci.ChaNum)) as ChaNameWChanum, (ci.ChaName + '#' + CONVERT(varchar(10),ci.UserNum)) as ChaNameWUserNum, ci.GuPosition FROM ChaInfo ci "
							"WHERE ci.GuNum=%u AND ci.ChaDeleted = 0 ORDER BY ci.ChaName", dwClub );
	...
	while (true)
	{
		...
		{	
			...
			::SQLGetData(pConn->hStmt, 3, SQL_C_CHAR,    szChaNameWChanum, CHR_ID_LENGTH+1, &cbChaNameWChanum);
			::SQLGetData(pConn->hStmt, 4, SQL_C_CHAR,    szChaNameWUserNum, CHR_ID_LENGTH+1, &cbChaNameWUserNum);
			::SQLGetData(pConn->hStmt, 5, SQL_C_LONG, &nGuPosition,             0, &cbGuPosition);

			// Select character name based on nNameType
			if (nNameType == 1 && cbChaNameWChanum != 0 && cbChaNameWChanum != -1)
			{
				::StringCchCopy(sMember.m_szName, CHR_ID_LENGTH, (const TCHAR*) szChaNameWChanum);
			}
			else if (nNameType == 2 && cbChaNameWUserNum != 0 && cbChaNameWUserNum != -1)
			{
				::StringCchCopy(sMember.m_szName, CHR_ID_LENGTH, (const TCHAR*) szChaNameWUserNum);
			}
...
* \return
*/
int COdbcManager::SetClubBattle( DWORD dwClubP, DWORD dwClubS, __time64_t tStartTime, __time64_t tEndTime, int nAlliance )
{
	if (dwClubP <= 0 || dwClubS <= 0)
	{
	...
	CTime cTimeStart(tStartTime);
	CTime cTimeEnd(tEndTime);
	CString strTimeStart = cTimeStart.Format( "%Y-%m-%d %H:%M:%S" );
	CString strEndTime = cTimeEnd.Format( "%Y-%m-%d %H:%M:%S" );

	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 128, "{call sp_RequestGuBattle(%u,%u,'%s','%s',%u,?)}", dwClubP, dwClubS, strTimeStart.GetString(), strEndTime.GetString(), nAlliance );
