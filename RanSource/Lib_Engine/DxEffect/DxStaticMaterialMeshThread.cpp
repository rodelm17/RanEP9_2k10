...
// -----------------------------------------------------------------------------------------------------------------------------------------
DxStaticMaterialMesh_THREAD::~DxStaticMaterialMesh_THREAD()
{
}

BOOL DxStaticMaterialMesh_THREAD::StartThread(
...
void DxStaticMaterialMesh_THREAD::EndThread()
{
...
unsigned int WINAPI DxStaticMaterialMesh_THREAD::LoadThread(
	LPVOID pData )
{
...
// -----------------------------------------------------------------------------------------------------------------------------------------
DxStaticMaterialMeshColor_THREAD& DxStaticMaterialMeshColor_THREAD::GetInstance()
{
	static DxStaticMaterialMeshColor_THREAD Instance;
	return Instance;
}

 DxStaticMaterialMeshColor_THREAD::DxStaticMaterialMeshColor_THREAD() :
	dwThreadID( UINT_MAX ),
	hThread( NULL ),
...
BOOL DxStaticMaterialMeshColor_THREAD::StartThread( LPDIRECT3DDEVICEQ pd3dDevice )
{
	// 쓰레드가 켜져 있다면 또 생성하지 않음
	if( m_sData.bEnable )	return TRUE;
...
void DxStaticMaterialMeshColor_THREAD::EndThread()
{
	// 쓰레드가 꺼져 있다면 또 지우지 않음.
	if( !m_sData.bEnable )	return;
...
unsigned int WINAPI DxStaticMaterialMeshColor_THREAD::LoadThread(
	LPVOID pData )
{
	DATA* _pData = (DATA*)pData;
...
void DxStaticMaterialMeshColor_THREAD::CheckListEmpty()
{
	EnterCriticalSection( &m_CSLockColor );
	{
...
void DxStaticMaterialMeshColor_THREAD::DeleteList()
{
