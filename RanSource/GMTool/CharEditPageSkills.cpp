...
void CCharEditPageSkills::OnBnClickedChareditSkillsButtonAdd()
{
	...
	if ( dlg.DoModal() == IDOK ){
		...
		for( ; iterData != mapSkills.end(); ++iterData ){
			...
			m_pData->m_ExpSkills.insert( std::make_pair(sInsertData.sID.dwID, SCHARSKILL(sInsertData.sID, sInsertData.wLEVEL,NATIVEID_NULL(),0)) );	
		}
...
void CCharEditPageSkills::OnBnClickedChareditSkillsButtonPreset()
{
	...
	if ( pPRESET ){
		...
		for( ; iter_data != pPRESET->mapData.end(); ++ iter_data ){
			...
			m_pData->m_ExpSkills.insert( std::make_pair(sDATA.sSKILLID.dwID, SCHARSKILL(sDATA.sSKILLID, sDATA.wLEVEL,NATIVEID_NULL(),0)) );	
		}
