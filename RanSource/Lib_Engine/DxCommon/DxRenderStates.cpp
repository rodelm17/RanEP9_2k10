...
DxRenderStates::DxRenderStates(void) :
	...
	m_pSavedLightMapSB(NULL),
	m_pDrawLightMapSB(NULL),

	m_pSavedBlendRenderSB(NULL),
	m_pDrawBlendRenderSB(NULL)
{
}
...
void DxRenderStates::SetLightMapSB ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	m_pSavedLightMapSB->Capture();
	m_pDrawLightMapSB->Apply();
}

void DxRenderStates::ReSetLightMapSB ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	m_pSavedLightMapSB->Apply();
}


void DxRenderStates::SetBlendRenderSB ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	m_pSavedBlendRenderSB->Capture();
	m_pDrawBlendRenderSB->Apply();
}

void DxRenderStates::ReSetBlendRenderSB ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	m_pSavedBlendRenderSB->Apply();
}
...
HRESULT DxRenderStates::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	for( UINT which=0; which<2; which++ )
	{
		...
		if( which==0 )	pd3dDevice->EndStateBlock( &m_pSavedLightMapSB );
		else			pd3dDevice->EndStateBlock( &m_pDrawLightMapSB );
	}
	...
	for( UINT which=0; which<2; which++ )
	{
		...
		if( which==0 )	pd3dDevice->EndStateBlock( &m_pSavedBlendRenderSB );
		else			pd3dDevice->EndStateBlock( &m_pDrawBlendRenderSB );
	}
...
HRESULT DxRenderStates::InvalidateDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	if ( pd3dDevice )
	{
		...
		SAFE_RELEASE_BLOCK ( pd3dDevice, m_pSavedLightMapSB );
		SAFE_RELEASE_BLOCK ( pd3dDevice, m_pDrawLightMapSB );

		SAFE_RELEASE_BLOCK ( pd3dDevice, m_pSavedBlendRenderSB );
		SAFE_RELEASE_BLOCK ( pd3dDevice, m_pDrawBlendRenderSB );
	}
