	...
	case MODERN_QUEST_WINDOW_DONE_INFO_REWARD_ITEM_SLOT:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( m_pItemSlotReward )
				{
					...
					{
						CInnerInterface::GetInstance().InfoDisplayShowItemSimple ( sInvenItem.sItemCustom );
...
void CModernQuestInfoDone::DataSet( DWORD dwQuestID )
{
	...
	if ( m_pTextRewardInfo )
	{
		...
		if ( !(pQuest->m_llGiftEXP || pQuest->m_dwGiftMONEY || pQuest->m_dwGiftContributionPoints || pQuest->m_dwGiftELEMENT || pQuest->m_dwGiftLIFEPOINT ||
