...
CNpcDialogueSet::CNpcDialogueSet ()
	...
	, m_bCurrencyShop(false)
	, m_bChaosMachine(false)	/* chaos machine, Juver, 2021/07/07 */
	, m_bContributionShop(false)
{
}
...
{
//	CFileDialog dlg( FALSE, ".csv", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, "csv file (*.csv)|*.csv|", pWnd );
//
...
BOOL CNpcDialogueSet::Save ( const char* szFileName )
{
	...
	SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_NPC_TALK_V1 );
	...
	SFile << m_bCurrencyShop;
	SFile << m_bChaosMachine;		/* chaos machine, Juver, 2021/07/07 */
	SFile << m_bContributionShop;

		/*self buff via NPC, DevArt22, 2023/03/18 */
	SFile << m_bSelfBuffBAOnly;
	SFile << m_bSelfBuffFull;
	SFile << m_bSelfBuffFullNoBA;
...
BOOL CNpcDialogueSet::LOAD_0017( basestream &SFile )
{
	std::string _strFileName;
	SFile >> _strFileName;
	m_strFileName = _strFileName.c_str();
...
BOOL CNpcDialogueSet::LOAD_0204( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::LOAD_0205( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::LOAD_0206( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::LOAD_0207( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::LOAD_0208( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::LOAD_0209( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::LOAD_0210( basestream &SFile )
{
	SFile >> m_bStorage;
	SFile >> m_bMarket;
	SFile >> m_bCure;
...
BOOL CNpcDialogueSet::Load ( const char* szFileName )
{
	...
	if ( wVer>=ENCODE_VER_BYTE_CRYPT )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_NPC_TALK_VER_1 );

	if ( wVer>=ENCODE_VER_BYTE_CRYPT2 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_NPC_TALK_VER_2 );

	if ( wVer>=ENCODE_VER_BYTE_CRYPT3 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_NPC_TALK_VER_3 );

	if ( wVer>=ENCODE_VER_BYTE_CRYPT_2021_NPC_TALK_V1 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_NPC_TALK_V1 );
	...
	case 0x0203:	LOAD_0203(SFile);	break;	/*item exchange, Juver, 2017/10/11 */
	case 0x0204:	LOAD_0204(SFile);	break;
	case 0x0205:	LOAD_0205(SFile);	break;
	case 0x0206:	LOAD_0206(SFile);	break;
	case 0x0207:	LOAD_0207(SFile);	break;
	case 0x0208:	LOAD_0208(SFile);	break;
	case 0x0209:	LOAD_0209(SFile);	break;	
	case VERSION:	LOAD_0210(SFile);	break;	 // Self Buff Via NPC
			...
			if ( pTalkControl )
			{
				...
				for ( ; iter!=iter_end; ++iter )
				{
					...
					if ( pTalk->m_nACTION==SNpcTalk::EM_BASIC )
					{
						...
						case SNpcTalk::EM_HIRE_ANIMAL:
							if ( !m_bHireAnimal )	m_bHireAnimal = true;
							break;
						...
						case SNpcTalk::EM_CURRENCY_SHOP:
							if ( !m_bCurrencyShop)		m_bCurrencyShop = true;
							break;

							/* chaos machine, Juver, 2021/07/07 */
						case SNpcTalk::EM_CHAOS_MACHINE:
							if (!m_bChaosMachine)		m_bChaosMachine = true;
							break;

						case SNpcTalk::EM_CONTRIBUTION_SHOP:
							if (!m_bContributionShop)		m_bContributionShop = true;
							break;
							/*self buff via NPC, DevArt22, 2023/03/18 */
						case SNpcTalk::EM_SELF_BUFF_BA_ONLY:
							if ( !m_bSelfBuffBAOnly )	m_bSelfBuffBAOnly = true;
							break;

							/*self buff via NPC, DevArt22, 2023/03/18 */
						case SNpcTalk::EM_SELF_BUFF_FULL:
							if ( !m_bSelfBuffFull )	m_bSelfBuffFull = true;
							break;

							/*self buff via NPC, DevArt22, 2023/03/18 */
						case SNpcTalk::EM_SELF_BUFF_FULL_NO_BA:
							if ( !m_bSelfBuffFullNoBA )	m_bSelfBuffFullNoBA = true;
							break;
						};
					}
				}
...
CNpcDialogueSet& CNpcDialogueSet::operator= ( CNpcDialogueSet &rvalue )
{
	...
	m_bHireAnimal		= rvalue.m_bHireAnimal;
	...
	m_bCurrencyShop		= rvalue.m_bCurrencyShop;
	m_bChaosMachine		= rvalue.m_bChaosMachine;			/* chaos machine, Juver, 2021/07/07 */
	m_bContributionShop	= rvalue.m_bContributionShop;
	
			/*self buff via NPC, DevArt22, 2023/03/18 */
	m_bSelfBuffBAOnly = rvalue.m_bSelfBuffBAOnly;
	m_bSelfBuffFull = rvalue.m_bSelfBuffFull;
	m_bSelfBuffFullNoBA = rvalue.m_bSelfBuffFullNoBA;
