...
HRESULT BLURSYS_PROPERTY::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "BLURSYS_PROPERTY::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT BLURSYS_PROPERTY::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT DxEffectBlurSys::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX &matComb )
{
	...
	if( m_fTime30 > 0.033f )
	{
		...
		if( !(m_dwRunFlag&EFF_PLY_DONE) && !(m_dwRunFlag&EFF_PLY_END) )
		{
			...
			// 낮은 fps 따라 보간 갯수를 줄일 수 있도록 한다.
			int nSubHermite(0);
			if ( m_fElapsedTime > 0.0716f )		// 14 fps 이하면 보간은 없다.
			{
				nSubHermite = -8;
			}
			...
			else if ( m_fElapsedTime > 0.02f )		// 50 fps 이하면 보간은 1번 감소한다.
			{
				nSubHermite = -1;
			}
...
int DxEffectBlurSys::SeparateNumOfLine( int nSubHermite )
{
	...
	{
		// 15 를 원하는 값으로 빼면 문제가 되서 8로 맞춰준다.
		if( nData==15 )	
			nData = 8;
