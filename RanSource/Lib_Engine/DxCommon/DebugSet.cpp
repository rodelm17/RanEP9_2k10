...
namespace CDebugSet
{
	...
	void TempOnOff( bool bOn, const char* szFileName )
	{
		...
		if( s_bOn )
		{
			...
			StringCchCopy(szFullPath, MAX_PATH, SUBPATH::APP_ROOT);
			StringCchCat( szFullPath, MAX_PATH, SUBPATH::SAVE_ROOT );
	...
	//blocks program window, good for global message-box
	//could also block all program threads
	...
	//blocks only active window, use only if program is single window
	//program threads continue
	void MsgBox( const char *szFormat, ... )
	{
		...
		MessageBox( NULL, szBuffer, "Info", MB_OK | MB_TOPMOST );
	}

	//passes hwnd of the window it should pop-up
	//program threads continue
	void MsgBox( HWND hWnd, const char *szFormat, ... )
	{
		...
		MessageBox( hWnd, szBuffer, "Info", MB_OK | MB_TOPMOST );
	}

	//with yes/no answer, returns the answer, pass hwnd of the window it should pop-up
	//program threads continue
	int MsgBoxYesNo( HWND hWnd, const char *szFormat, ... )
	{
		...
		return MessageBox( hWnd, szBuffer, "Info", MB_YESNO | MB_TOPMOST );
	}

	std::string strErrorVersion_LogFileName	= "DebugSetErrorVersion.txt";	//error version log filename
	std::string strErrorFile_LogFileName	= "DebugSetErrorFile.txt";		//error file log filename
	std::string strErrorCode_LogFileName	= "DebugSetErrorCode.txt";	//error code log filename

	BOOL	m_bErrorVersion_MessageBox	= TRUE;	//display message box for version error
	BOOL	m_bErrorVersion_WriteLog	= TRUE;	//write log file for version error

	BOOL	m_bErrorFile_MessageBox		= TRUE;	//display message box for file read Error
	BOOL	m_bErrorFile_WriteLog		= TRUE;	//write log file for file read error
	BOOL	m_bErrorFile_Remember		= TRUE;	//remember the error filename to avoid repeat
	BOOL	m_bErrorFile_ClipBoard		= TRUE;	//copy the error filename to clipboard for easy search

	BOOL	m_bErrorCode_MessageBox		= TRUE;	//display message box for code error
	BOOL	m_bErrorCode_WriteLog		= TRUE;	//write log file for code error

	std::vector<std::string>	m_vecErrorFile; //container for error filename 
	
	void SetErrorParam_Version( BOOL bMessageBox, BOOL bWriteLog, std::string strLogFileName )
	{
		m_bErrorVersion_MessageBox	= bMessageBox;
		m_bErrorVersion_WriteLog	= bWriteLog;

		if ( !strLogFileName.empty() )
			strErrorVersion_LogFileName = strLogFileName.c_str();
	}

	void SetErrorParam_File( BOOL bMessageBox, BOOL bWriteLog, BOOL bRemember, BOOL bClipBoard, std::string strLogFileName )
	{
		m_bErrorFile_MessageBox = bMessageBox;
		m_bErrorFile_WriteLog	= bWriteLog;
		m_bErrorFile_Remember	= bRemember;
		m_bErrorFile_ClipBoard	= bClipBoard;

		if ( !strLogFileName.empty() )
			strErrorFile_LogFileName = strLogFileName.c_str();
	}

	void SetErrorParam_Code( BOOL bMessageBox, BOOL bWriteLog, std::string strLogFileName )
	{
		m_bErrorCode_MessageBox = bMessageBox;
		m_bErrorCode_WriteLog	= bWriteLog;

		if ( !strLogFileName.empty() )
			strErrorCode_LogFileName = strLogFileName.c_str();
	}

	void ErrorVersion( const char* szFunction, const char* szFile, DWORD dwVer )
	{
		if ( !szFunction )	return;
		if ( !szFile )	return;

		char szTempChar[1024];
		sprintf( szTempChar, "%s~~Version Error! Function:%s FileName:%s Version:%d (%x)", GetCurTime().c_str(), szFunction, szFile, dwVer, dwVer );

		if ( m_bErrorVersion_MessageBox )
			MessageBox( NULL, szTempChar, "DebugSet", MB_OK );

		if ( m_bErrorVersion_WriteLog )
			ToFile( strErrorVersion_LogFileName.c_str(), "%s", szTempChar );
	}
	...
	void ErrorVersion( const char* szFunction, DWORD dwVer )
	{
		...
		char szTempChar[1024];
		sprintf( szTempChar, "%s~~Version Error! Function:%s Version:%d (%x)", GetCurTime().c_str(), szFunction, dwVer, dwVer );
	...
	void ErrorFile( const char* szFunction, const char* szFile )
	{
		...
		//some files are intentionally set with null
		if ( std::string("null")==szFile )	return;
		if ( std::string("NULL")==szFile )	return;
		if ( std::string("Null")==szFile )	return;

		//file probably just random unused strings
		std::string _strFileName = szFile;
		if(_strFileName.find_last_of(".") == std::string::npos)
			return;

		bool bFIND = FALSE;

		if ( m_bErrorFile_Remember )
		{
			size_t size = m_vecErrorFile.size();
			for ( size_t n=0; n<size; ++n )
			{
				std::string str = m_vecErrorFile[n];

				if ( strcmp( szFile, str.c_str() ) == 0 )
				{
					bFIND = true;
					break;
				}
		...
		{
			char szTempChar[1024];
			sprintf( szTempChar, "%s~~File Error! Function:%s FileName:%s", GetCurTime().c_str(), szFunction, szFile );

			if ( m_bErrorFile_MessageBox )
				MessageBox( NULL, szTempChar, "DebugSet", MB_OK );

			if ( m_bErrorFile_WriteLog )
				ToFile( strErrorFile_LogFileName.c_str(), "%s", szTempChar );

			if ( m_bErrorFile_Remember )
				m_vecErrorFile.push_back( szFile );

			if ( m_bErrorFile_ClipBoard )	
			{
				if(OpenClipboard( NULL ))
				{
					...
					char * buffer;
					EmptyClipboard();
					clipbuffer = GlobalAlloc(GMEM_DDESHARE, strlen( szFile )+1);
	...
	void ErrorCode( const char* szFunction, const char *szErrorFormat, ... )
	{
		...
		va_start(argList, szErrorFormat);
		StringCbVPrintf(szBuffer, MAX_STRLENGTH, szErrorFormat, argList );
		va_end(argList);

		char szTempChar[1024];
		sprintf( szTempChar, "%s~~Code Error! Function:%s Error:%s", GetCurTime().c_str(), szFunction, szBuffer );

		if ( m_bErrorCode_MessageBox )
			MessageBox( NULL, szTempChar, "DebugSet", MB_OK );

		if ( m_bErrorCode_WriteLog )
			ToFile( strErrorCode_LogFileName.c_str(), "%s", szTempChar );
	}

	// Performance Logging Toggle Functions - Added by Ace17 30/08/2025
	void ToggleDebugLogging() {
		RPARAM::bEnableDebugLogging = !RPARAM::bEnableDebugLogging;
	}

	void ToggleFileLogging() {
		RPARAM::bEnableFileLogging = !RPARAM::bEnableFileLogging;
	}

	void ToggleScreenLogging() {
		RPARAM::bEnableScreenLogging = !RPARAM::bEnableScreenLogging;
	}

	void ToggleStudentRecordLogs() {
		RPARAM::bEnableStudentRecordLogs = !RPARAM::bEnableStudentRecordLogs;
	}

	void ToggleRenderLogs() {
		RPARAM::bEnableRenderLogs = !RPARAM::bEnableRenderLogs;
	}

	void ToggleNetworkLogs() {
		RPARAM::bEnableNetworkLogs = !RPARAM::bEnableNetworkLogs;
	}

	void ToggleServerLogs() {
		RPARAM::bEnableServerLogs = !RPARAM::bEnableServerLogs;
	}

	void SetAllLoggingEnabled(bool bEnable) {
		RPARAM::bEnableDebugLogging = bEnable;
		RPARAM::bEnableFileLogging = bEnable;
		RPARAM::bEnableScreenLogging = bEnable;
		RPARAM::bEnableStudentRecordLogs = bEnable;
		RPARAM::bEnableRenderLogs = bEnable;
		RPARAM::bEnableNetworkLogs = bEnable;
		RPARAM::bEnableServerLogs = bEnable;
	}
};
