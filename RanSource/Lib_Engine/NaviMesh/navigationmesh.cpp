...
bool NavigationMesh::LineOfSightTest(DWORD StartID, const D3DXVECTOR3& StartPos, DWORD EndID, const D3DXVECTOR3& EndPos)
{
	...
	//limited to 100 because there is an infinite loop bug
	//ResolveMotionOnMesh was also limited to 100
	int i = 0;
	for( i=0; i<100; i++ )
	//while(1)
	{	
	...
	GASSERT( "Maximum number of tests exceeded LineOfSightTest" );
...
HRESULT	NavigationMesh::CopyNaviToShadow ( LPDIRECT3DDEVICEQ pd3dDevice, SCOPYMESHDATA& copy_mesh_data )
{	
	copy_mesh_data.dwVert = (DWORD)m_nNaviVertex;
	copy_mesh_data.dwFace = (DWORD)TotalCells();
	copy_mesh_data.dwFVF = VERTEXNORCOLORTEX_850::FVF;
	int fvfsize = D3DXGetFVFVertexSize( copy_mesh_data.dwFVF );

	SAFE_RELEASE( copy_mesh_data.pVB );
	pd3dDevice->CreateVertexBuffer( copy_mesh_data.dwVert*fvfsize, D3DUSAGE_WRITEONLY, copy_mesh_data.dwFVF, D3DPOOL_MANAGED, &copy_mesh_data.pVB, NULL );
	if( !copy_mesh_data.pVB )	return E_FAIL;
	
	SAFE_RELEASE( copy_mesh_data.pIB );
	pd3dDevice->CreateIndexBuffer( copy_mesh_data.dwFace*3*sizeof(WORD), 0L, D3DFMT_INDEX16, D3DPOOL_MANAGED, &copy_mesh_data.pIB, NULL );
	if( !copy_mesh_data.pIB )	return E_FAIL;
	
	
	VERTEXNORCOLORTEX_850	*pLockVertices = NULL;
	copy_mesh_data.pVB->Lock ( 0, 0, (VOID**)&pLockVertices, NULL );	
	for ( int i = 0; i < m_nNaviVertex; i++ )
	{
		pLockVertices[i].vPos = m_pNaviVertex[i];
	}
	copy_mesh_data.pVB->Unlock();

	WORD *pLockIndices = NULL;
	copy_mesh_data.pIB->Lock ( 0, 0, (VOID**)&pLockIndices, NULL );
	for ( int i = 0; i < TotalCells(); i++ )
	{
		pLockIndices[i*3 + 0] = (WORD)m_pDACell[i]->Vertex(0);
		pLockIndices[i*3 + 1] = (WORD)m_pDACell[i]->Vertex(1);
