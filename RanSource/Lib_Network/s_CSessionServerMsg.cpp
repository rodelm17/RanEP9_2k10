...
{	
	...
	case NET_MSG_IPFILTER_SERVER_TO_SESSION_BLOCK_ADD:
		IPFilterMsgBlockAdd( pMsg );
		break;

	case NET_MSG_IPFILTER_SERVER_TO_SESSION_BLOCK_DEL:
		IPFilterMsgBlockDel( pMsg );
		break;

	case NET_MSG_IPFILTER_SERVER_TO_SESSION_KNOWN_ADD:
		IPFilterMsgKnownAdd( pMsg );
		break;

	case NET_MSG_IPFILTER_SERVER_TO_SESSION_KNOWN_DEL:
		IPFilterMsgKnownDel( pMsg );
		break;

	case NET_MSG_IPFILTER_SERVER_TO_SESSION_LIST_SAVE:
		IPFilterSave();
		break;

	case NET_MSG_WRITE_IP_LOGIN_TO_SESSION:
		{
			NET_WRITE_IP_LOGIN_TO_SESSION* netMsg = reinterpret_cast < NET_WRITE_IP_LOGIN_TO_SESSION* > (pMsg->Buffer);
			//RLOG::LogFileNoDate( "login_ip.txt", "%s", netMsg->szIP );

			NET_WRITE_IP_SESSION_TO_SERVER netMsgServer;
			StringCchCopy( netMsgServer.szIP, MAX_IP_LENGTH, netMsg->szIP );
		...
		break;
		/*dmk14 whitelist*/
	case NET_MSG_IPFILTER_WHITELIST_ADD_SESSION:
		MsgServerBroadCastIPFilterWhiteListAdd( pMsg );
		break;

	case NET_MSG_IPFILTER_WHITELIST_DEL_SESSION:
		MsgServerBroadCastIPFilterWhiteListDel( pMsg );
		break;
	default:
		break;
	}
