...
const INT CModernItemTransferInvenPage::nSTARTLINE = 0;	
const INT CModernItemTransferInvenPage::nOUTOFRANGE = -1;

CModernItemTransferInvenPage::CModernItemTransferInvenPage()
...
void CModernItemTransferInvenPage::CreateSubControl()
{
	CString strSlotKeyword[ nMAX_ONE_VIEW_SLOT ] =
	{
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT0",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT1",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT2",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT3",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT4",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT5",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT6",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT7",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT8",
		"MODERN_ITEM_TRANSFER_INVENTORY_WINDOW_ITEM_SLOT9",
...
CItemSlotEx* CModernItemTransferInvenPage::CreateItemSlot( CString strKeyword, UIGUID ControlID )
{
...
CUIControl*	CModernItemTransferInvenPage::CreateControl( const TCHAR* szControl )
{
...
void CModernItemTransferInvenPage::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CModernItemTransferInvenPage::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
...
void CModernItemTransferInvenPage::ResetItemSlotRender ( int nStart, int nEnd )
{
	if ( nEnd < 0 )			return;
...
void CModernItemTransferInvenPage::SetItemSlotRender( int nStart, int nEnd )
{
	if ( nEnd < 0 ) return ;
	if ( nStart > nEnd ) return ;
...
void CModernItemTransferInvenPage::SetItemIndex( INT nPosX, INT nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

void CModernItemTransferInvenPage::GetItemIndex( INT* pnPosX, INT* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

void CModernItemTransferInvenPage::LoadItemPage( GLInventory &ItemData )
{
...
void CModernItemTransferInvenPage::LoadItem( SINVENITEM& ref_InvenItem )
{	
...
void CModernItemTransferInvenPage::UnLoadItemPage()
{
...
void CModernItemTransferInvenPage::UnLoadItem( INT nPosX, INT nPosY )
{
	m_pItemSlotArray[ nPosY ]->ResetItemImage( nPosX );
}

void CModernItemTransferInvenPage::ResetAllFlipItem()
{
	for( WORD y=0; y<EM_INVENSIZE_Y; ++y )
		for( WORD x=0; x<EM_INVENSIZE_X; ++x )
...
void CModernItemTransferInvenPage::UpdateFlipItem( GLInventory& ref_Inventory )
{
	ResetAllFlipItem();
...
void CModernItemTransferInvenPage::UpdateFlipItem( SINVEN_POS& ref_GarbageItem )
{
...
void CModernItemTransferInvenPage::UpdateBLOCK()
{
...
SINVENITEM& CModernItemTransferInvenPage::GetItem( INT nPosX, INT nPosY )
{
	return m_pItemSlotArray[ nPosY ]->GetItemImage( nPosX );
}

void CModernItemTransferInvenPage::SetOneViewSlot( const INT& nMAX_ONE_VIEW )
{
