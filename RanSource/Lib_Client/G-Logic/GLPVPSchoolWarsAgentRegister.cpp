...
void GLPVPSchoolWarsAgent::RequestPlayerRegister( DWORD dwCharID, WORD wLevel, DWORD dwContri, BOOL bRegister, std::string strHash )
{	
...
void GLPVPSchoolWarsAgent::PlayerRegister( DWORD dwCharID, SCHOOLWARS_REGISTER_DATA_MAP& mapData, SCHOOLWARS_REGISTER_DATA_QUEUE_VEC& vecData, std::string strHash )
{
	PGLCHARAG pchar = GLAgentServer::GetInstance().GetCharID( dwCharID );
	if ( !pchar )	return;
	...
	if ( m_bHashCheck )
	{
		...
		if ( strHash.empty() )
		{
			netmsgfb.emFB = SCHOOLWARS_REGISTER_FB_DEVICE_INFO_ERROR;
			GLAgentServer::GetInstance().SENDTOCLIENT ( pchar->m_dwClientID, &netmsgfb );
			return;
		}
		...
		{
			netmsgfb.emFB = SCHOOLWARS_REGISTER_FB_DEVICE_REGISTERED;
			GLAgentServer::GetInstance().SENDTOCLIENT ( pchar->m_dwClientID, &netmsgfb );
			return;
		}
	}
...
/* PVPSchoolWars hash check, Juver, 2021/07/17 */
BOOL GLPVPSchoolWarsAgent::PCHashCheck( std::string strHash )
{
	if ( strHash.empty() )	return FALSE;

	SCHOOLWARS_PLAYER_HASH_DATA_MAP_ITER it = m_mapPlayerHash.find( strHash );
...
/* PVPSchoolWars hash check, Juver, 2021/07/17 */
void GLPVPSchoolWarsAgent::PCHashInsert( std::string strHash, DWORD dwCharID )
{
	SCHOOLWARS_PLAYER_HASH_DATA sData;
...
/* PVPSchoolWars hash check, Juver, 2021/07/17 */
void GLPVPSchoolWarsAgent::PCHashDelete( DWORD dwCharID )
{
	SCHOOLWARS_PLAYER_HASH_DATA_MAP_ITER it_b = m_mapPlayerHash.begin();
	SCHOOLWARS_PLAYER_HASH_DATA_MAP_ITER it_e = m_mapPlayerHash.end();
