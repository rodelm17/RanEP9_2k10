...
void CAgentServer::ApexUserChatBlock( DWORD dwClient, WORD dwHour )
{
	...
	CString strChatBlockReason = "Apex ChatBlock";
	DWORD dwGMUserID = 0;
	CString strGMUserName = "ApexSystem";

	//	DB에 저장.
	CSetChatBlockTime *pDbAction = new CSetChatBlockTime(pCHAR->m_dwUserID, pCHAR->m_szUserName, pCHAR->m_tCHATBLOCK, strChatBlockReason.GetString(), dwGMUserID, strGMUserName.GetString() );
