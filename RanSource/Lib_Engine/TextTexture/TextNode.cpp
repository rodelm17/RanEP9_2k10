...
VOID CTextPart::DrawText( D3DCOLOR dwColor, FLOAT& fX, FLOAT fY )
{
	...
	if( m_bValid )
	{
		...
		if( pd3dDevice )
			pd3dDevice->DrawPrimitiveUP( D3DPT_TRIANGLEFAN, 2, m_stVertex, sizeof( UIVERTEX ) );
	}
...
VOID CTextPart::DrawText( D3DCOLOR dwColor, FLOAT& fX, FLOAT fY, FLOAT fZoom )
{
	if ( !m_pFont )
		return;
	...
	if( m_bValid )
	{
		fX -= floorf( (FLOAT)CTextUtil::m_iSpaceSize * fZoom );

		D3DXVECTOR2 vMin, vMax;
		vMin.x = fX;
		vMin.y = fY;
		vMax.x = vMin.x + floorf( (FLOAT)m_iWidth * fZoom );
		vMax.y = vMin.y + floorf( (FLOAT)m_pFont->GetHeightScreen() * fZoom );
		...
		fX += floorf( (FLOAT)m_iWidth * fZoom );
	...
	if( m_pLeft )
		m_pLeft->DrawText( dwColor, fX, fY, fZoom );

	if( m_pRight )
		m_pRight->DrawText( dwColor, fX, fY, fZoom );
}
