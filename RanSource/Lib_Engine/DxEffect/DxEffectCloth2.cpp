// DxEffectCloth2.cpp: implementation of the DxEffectBlur class.
//
//////////////////////////////////////////////////////////////////////
...
const DWORD DxEffectCloth2::TYPEID = DEF_EFFECT_CLOTH2;
const DWORD	DxEffectCloth2::VERSION = 0x00000200;
const char DxEffectCloth2::NAME[] = "DxEffectCloth2";
const DWORD DxEffectCloth2::FLAG = _EFF_REPLACE;

void DxEffectCloth2::GetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD &dwVer )
{
	pProp = (PBYTE) &m_Property;
	dwSize = sizeof(CLOTH2_PROPERTY);
	dwVer = VERSION;
}

void DxEffectCloth2::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	if ( ( dwVer==VERSION && dwSize==sizeof(CLOTH2_PROPERTY) ) )
	{
		memcpy ( &m_Property, pProp, dwSize );
	}
	else if ( ( dwVer==0x0100 && dwSize==sizeof(CLOTH2_PROPERTY_0100) ) )
	{
		CLOTH2_PROPERTY_0100	sProp;
		memcpy ( &sProp, pProp, dwSize );

		m_bPlay				= sProp.m_bPlay;
		m_bEachRender		= sProp.m_bEachRender;
		m_iNodeCount		= sProp.m_iNodeCount;
		m_fCoEfficient		= sProp.m_fCoEfficient;	
		m_vWindDirection	= sProp.m_vWindDirection;
		m_fWindForce		= sProp.m_fWindForce;
		m_fGravity			= sProp.m_fGravity;
		StringCchCopy( m_szTexture, MAX_PATH, sProp.m_szTexture );
	}
	else
	{
		CDebugSet::ErrorVersion( "DxEffectCloth2::SetProperty", dwVer );
	}
...
HRESULT DxEffectCloth2::CreateDevice ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	return S_OK;
}

HRESULT DxEffectCloth2::ReleaseDevice ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	return S_OK;
}

HRESULT DxEffectCloth2::AdaptToDxFrameChild ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
	DxMeshes *pmsMeshs;
...
HRESULT DxEffectCloth2::AdaptToDxFrame ( DxFrame *pFrame, LPDIRECT3DDEVICEQ pd3dDevice )
{
...
HRESULT DxEffectCloth2::AdaptToEffList ( const DxFrame *const pFrame, LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT ( pFrame );
...
HRESULT DxEffectCloth2::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
HRESULT DxEffectCloth2::DeleteDeviceObjects ()
{
...
HRESULT DxEffectCloth2::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT DxEffectCloth2::MeshUpdate ()
{
...
HRESULT DxEffectCloth2::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	if( !m_pVertArray )		return S_OK;
	if( !m_pIndexArray )	return S_OK;
 
	PROFILE_BEGIN("DxEffectCloth2");
...

	{
		...
		if ( FAILED(hr) )	
		{
			CDebugSet::ToLogFile( "DxEffectCloth2::Render() -- DxDynamicVB::m_sVB_PNT.pVB->Lock() -- Failed" );
			goto _RETURN;
		}
...
_RETURN:
	...
	PROFILE_END("DxEffectCloth2");

	return S_OK;
}

void DxEffectCloth2::DeleteClothData( int a )
{
	if ( a == 0 )
	{
		SAFE_DELETE_ARRAY( m_pVertArray );
...
void DxEffectCloth2::XFileLoad( LPDIRECT3DDEVICEQ pd3dDevice, LPD3DXMESH pLocalMesh )
{	
	HRESULT hr(S_OK);
	DeleteClothData( 0 );
	...
	if( FAILED(hr) )	
	{
		CDebugSet::ToLogFile( "DxEffectCloth2::XFileLoad() -- pVB->Lock() -- Failed" );
		return;
	}
	...
	if( FAILED(hr) )	
	{
		CDebugSet::ToLogFile( "DxEffectCloth2::XFileLoad() -- pIB->Lock() -- Failed" );
		return;
	}
...
void DxEffectCloth2::BaseBuffToDynamicBuff ( LPDIRECT3DDEVICEQ pd3dDevice )
{	
	DeleteClothData( 1 );
...
//노드 테이블 수를 리턴한다.
void DxEffectCloth2::CreateNodeTable( VERTEX *pVertices, DWORD dwNumIndices, WORD *pIndices )
{	
	...
	for( int i = 0 ; i < (int)dwNumIndices ; i++ )
...
void DxEffectCloth2::SetParticle(int index, D3DXVECTOR3 &vPosition )
{ 
	 //파티클 초기화 ^^
...
BOOL DxEffectCloth2::CreateVertex( LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwNumVertices, VERTEX* pVertices, DWORD dwNumIndices, WORD* pIndices )
{
	// 파티클을 생성하는 부분이다 ^^
	SAFE_DELETE_ARRAY( m_pParticle );	
	...
	{
		CDebugSet::ToLogFile( "DxEffectCloth2::CreateVertex() -- CreateIndexBuffer() -- Failed" );
		return FALSE;
	}
	...
	{
		CDebugSet::ToLogFile( "DxEffectCloth2::CreateVertex() -- pIB->Lock() -- Failed" );
		return FALSE;
	}
...
void  DxEffectCloth2::ComputeNormal(int nVertices,  VERTEX *pVertices, int nIndices, WORD *pIndices  )
{
	D3DXVECTOR3 v1, v2;
	D3DXVECTOR3 vNormal;
	int i;
...
inline float DxEffectCloth2::Random( int nMin, int nMax )
{
	srand( nMax );	
	return (float)(((rand()%nMax)+nMin)%nMax);
}

void DxEffectCloth2::SetParticleLock( int index , BOOL bLook )
{
...
void DxEffectCloth2::SetIndexLock()
{
	SAFE_DELETE_ARRAY ( m_pTempLockIndex );
...
void DxEffectCloth2::GetIndexLock()
{
	SAFE_DELETE_ARRAY ( m_pLockIndex );
...
char* DxEffectCloth2::GetStrList( DWORD i )
{
...
DWORD DxEffectCloth2::GetdwVertex( DWORD i )
{
	WORD k = m_pLockIndex[i];
...
void DxEffectCloth2::GetAABBSize ( D3DXVECTOR3 &vMax, D3DXVECTOR3 &vMin )
{
	D3DXVECTOR3 vBoxMax;
	D3DXVECTOR3 vBoxMin;
...
BOOL DxEffectCloth2::IsDetectDivision ( D3DXVECTOR3 &vDivMax, D3DXVECTOR3 &vDivMin )
{
	D3DXVECTOR3 vBoxMax;
	D3DXVECTOR3 vBoxMin;
...
void DxEffectCloth2::SaveBuffer ( CSerialFile &SFile )
{
	//	읽지 않고 스킵용으로 사용됨.
	SFile << sizeof(DWORD)+sizeof(VERTEX)*m_dwNumVertices
			...
			+sizeof(DWORD)+sizeof(NODE)*vecunknown_var.size()
			+sizeof(DWORD)+sizeof(float)+sizeof(float)+sizeof(float);
	...
	SFile << (DWORD)vecunknown_var.size();
	for ( DWORD i=0; i<vecunknown_var.size(); ++i )
	{
		NODE snode = vecunknown_var[i];
		SFile.WriteBuffer( &snode, sizeof(NODE) );
	}
...
void DxEffectCloth2::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
	DWORD dwBuffSize;
	...
	if ( dwVer == VERSION )
	{
		...
		if ( m_dwNumVertices && m_dwNumIndices )
		{
			...
			DWORD dwsize;
			SFile >> dwsize;
			vecunknown_var.clear();
			for ( DWORD i=0; i<dwsize; ++i )
			{
				NODE snode;
				SFile.ReadBuffer ( &snode, sizeof(NODE) );
				vecunknown_var.push_back( snode );
			}
	...
	else if ( dwVer == 0x0100 )
	{
		...
		if ( m_dwNumVertices && m_dwNumIndices )
		{
			...
			for( DWORD i=0; i<m_dwNumVertices; ++i )
			{
				...
				pVector = (D3DXVECTOR3*)( pTempVB + (nFVFSize*i) );
				pNormal = (D3DXVECTOR3*)( pTempVB + (nFVFSize*i) + 4*3 );
				pTex	= (D3DXVECTOR2*)( pTempVB + (nFVFSize*i) + 4*7 );

				m_pBaseVertArray[i].vPos = *pVector;
				m_pBaseVertArray[i].vNor = *pNormal;
				m_pBaseVertArray[i].vTex = *pTex;
			}
	...
	{
		CDebugSet::ErrorVersion( "DxEffectCloth2::LoadBufferSet", SFile.GetFileName(), dwVer );
...
void DxEffectCloth2::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
	DWORD dwBuffSize;
	SFile >> dwBuffSize; //	"읽지 않고 스킵" 할 때 사용되므로 실제 로드에서는 무의미.
	...
	{
		CDebugSet::ErrorVersion( "DxEffectCloth2::LoadBuffer", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwBuffSize );
		return;
	}
...
HRESULT DxEffectCloth2::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DxEffectCloth2* pSrcCloth = (DxEffectCloth2*) pSrcEffect;
	...
	vecunknown_var = pSrcCloth->vecunknown_var;
	unknown_var1 = pSrcCloth->unknown_var1;
	unknown_var2 = pSrcCloth->unknown_var2;
	unknown_var3 = pSrcCloth->unknown_var3;
	unknown_var4 = pSrcCloth->unknown_var4;
