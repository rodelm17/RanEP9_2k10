...
void CSelectChinaAreaPage::CreateSubControl ()
{
	...
	pTextButton->CreateBaseButton ( "SELECT_AREA_BUTTON_01", CBasicTextButton::SIZE22, CBasicButton::CLICK_FLIP, RPARAM::China_Region[0].strName.GetString() );
	...
	pTextButton->CreateBaseButton ( "SELECT_AREA_BUTTON_02", CBasicTextButton::SIZE22, CBasicButton::CLICK_FLIP, RPARAM::China_Region[1].strName.GetString() );
	pTextButton->SetFlip ( TRUE );
	RegisterControl ( pTextButton );

	pTextButton = new CBasicTextButton;
	pTextButton->CreateSub ( this, "BASIC_TEXT_BUTTON22", UI_FLAG_XSIZE, SELECT_AREA_BUTTON_02 );
	pTextButton->CreateBaseButton ( "SELECT_AREA_BUTTON_03", CBasicTextButton::SIZE22, CBasicButton::CLICK_FLIP, RPARAM::China_Region[2].strName.GetString() );
	pTextButton->SetFlip ( TRUE );
	RegisterControl ( pTextButton );
/*
	pTextButton = new CBasicTextButton;
...
{
	if( 0 > nIndex || nIndex >= RPARAM::MAX_CHINA_REGION )
	{
		GASSERT( 0 && "CSelectChinaAreaPage::SetParamChinaRegion()" );
		return ;
	}

	ROPTION::nChinaRegion = nIndex;

	RPARAM::sLoginAddress[0].strLoginIP = RPARAM::China_Region[nIndex].LoginAddress.GetString();

	for( int i=0 ; ; ++i )
	{
		if(RPARAM::China_Region[nIndex].HttpAddressTable[i].IsEmpty() )
			break;

		StringCchCopy(RPARAM::HttpAddressTable[i], RANPARAM_STRING_NUM_128, RPARAM::China_Region[nIndex].HttpAddressTable[i].GetString() );
	}

	RPARAM::strNewsURL = RPARAM::China_Region[nIndex].strNewsURL;
}
