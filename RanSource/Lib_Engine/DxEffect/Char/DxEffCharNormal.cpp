...
char		DxEffCharNormal::NAME[MAX_PATH]	= "DxEffCharNormal";
...
HRESULT	DxEffCharNormal::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		//official completely recoded this effect and
		//we have no way of supporting the new format
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharNormal::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharNormal::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharNormal::R2LoadFile", SFile.GetFileName(), dwVer );
