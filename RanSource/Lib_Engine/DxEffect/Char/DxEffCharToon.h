...
struct EFFCHAR_PROPERTY_TOON : public EFFCHAR_PROPERTY
{
	...
	char			m_szToonTex[TOON_TEXTURE_LENGTH];
	char			m_szObliqueTex[TOON_TEXTURE_LENGTH];	// 사선 
	...
	EFFCHAR_PROPERTY_TOON () :
		...
		m_fSizeADD(1.f)
	{
		SecureZeroMemory( m_szToonTex, sizeof(char)*TOON_TEXTURE_LENGTH );
		SecureZeroMemory( m_szObliqueTex, sizeof(char)*TOON_TEXTURE_LENGTH );

		StringCchCopy( m_szToonTex, TOON_TEXTURE_LENGTH, "Toon.dds" );
		StringCchCopy( m_szObliqueTex, TOON_TEXTURE_LENGTH, "Oblique.dds" );
	}
};
