...
{	
	...
	// 베트남 탐직방지 시스템의 추가로 캐릭터 저장하는 부분의 수정
		...
		_snprintf( szTemp, 2048, "UPDATE ChaInfo SET ChaBright=%d, ChaLevel=%u, ChaMoney=%I64d, "
					...
					"ChaSpMID = %u, ChaSpSID = %u, ChaScaleRange=%f, ChaCP=%u, ChaContributionPoint=%u, "
					"ChaActivityPoint=%u, ChaBadge='%s', "
					"ChaPromisePoint=%u "
					"WHERE ChaNum=%u", 
		...
		pCharData2->m_dwContributionPoint, 
		...
		pCharData2->m_wGachaPromisePoint,
...
//	lnTempMoney += lnTempValue;	
	...
	pCharData2->GETDAILYLOGIN_BYBUF(ByteStream);
	ByteStream.GetBuffer(pBuffer, dwSize);
	if (pBuffer != NULL)
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "UPDATE ChaInfo SET ChaInfo.ChaDailyLogin=? WHERE (ChaNum=%u)", dwChaNum );
		m_pGameDB->WriteImage( szTemp, dwChaNum, (BYTE *) pBuffer, dwSize );
		pBuffer = NULL;
	}
	...
	pCharData2->GETBATTLEPASS_BYBUF(ByteStream);
	ByteStream.GetBuffer(pBuffer, dwSize);
	if (pBuffer != NULL)
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "UPDATE ChaInfo SET ChaInfo.ChaBattlePass=? WHERE (ChaNum=%u)", dwChaNum );
		m_pGameDB->WriteImage( szTemp, dwChaNum, (BYTE *) pBuffer, dwSize );
		pBuffer = NULL;
	}	
	...
	PersonalLockWrite( CCfg::GetInstance()->GetServerGroup(), pCharData2->GetUserID(), pCharData2 );

	/* pk combo, Juver, 2021/02/11 */
	PKComboWrite( CCfg::GetInstance()->GetServerGroup(), pCharData2->m_dwCharID, pCharData2 );

	SaveUserDBEtcCharData( pCharData2 );

	CombatRecordWrite( CCfg::GetInstance()->GetServerGroup(), pCharData2->m_dwCharID, pCharData2 );
	GlobalRankingWrite( CCfg::GetInstance()->GetServerGroup(), pCharData2->m_dwCharID, pCharData2 );	
	DailyLoginWrite( CCfg::GetInstance()->GetServerGroup(), pCharData2->m_dwCharID, pCharData2 );
	BattlePassWrite( CCfg::GetInstance()->GetServerGroup(), pCharData2->m_dwCharID, pCharData2 );

	ChaGameSave( CCfg::GetInstance()->GetServerGroup(), pCharData2->m_dwCharID, pCharData2 );		
