...
WORD	DxSoundMan::m_SoundManVER = 101;
...
CSound*		DxSoundMan::LoadWav ( std::string strFileName, bool bStereo )
{
	...
	//	실제 WAV 파일 로드하기
	{
		...
		if ( FAILED ( hr ) )
		{
			CDebugSet::ErrorFile( "DxSoundMan::LoadWav", strFileName.c_str() );
...
{	
	...
	{
		CDebugSet::ErrorVersion( "DxSoundMan::LoadSet", SFile.GetFileName(), SoundManVER );
	}
