...
IMPLEMENT_DYNAMIC(CServerSettingTab, CPropertySheet)

CServerSettingTab::CServerSettingTab(CRect& rect, LOGFONT logfont, CWnd* pParentWnd)
	...
	m_PageBasic ( logfont ),
	m_PageDatabase ( logfont ),
	m_PageServer1 ( logfont ),
	m_PageServer2 ( logfont ),
	...
	m_pFont ( NULL )
{
	...
	m_PageBasic.m_psp.dwFlags |= PSP_USETITLE;
	m_PageBasic.m_psp.pszTitle = "Basic";

	m_PageDatabase.m_psp.dwFlags |= PSP_USETITLE;
	m_PageDatabase.m_psp.pszTitle = "Database";

	m_PageServer1.m_psp.dwFlags |= PSP_USETITLE;
	m_PageServer1.m_psp.pszTitle = "Server1";

	m_PageServer2.m_psp.dwFlags |= PSP_USETITLE;
	m_PageServer2.m_psp.pszTitle = "Server2";


	m_PageBasic.SetSheetTab ( this );
	m_PageDatabase.SetSheetTab ( this );
	m_PageServer1.SetSheetTab ( this );
	m_PageServer2.SetSheetTab ( this );

	AddPage ( &m_PageBasic );
	AddPage ( &m_PageDatabase );
	AddPage ( &m_PageServer1 );
	AddPage ( &m_PageServer2 );

	m_pFont = new CFont();
	m_pFont->CreateFontIndirect(&logfont);
}

CServerSettingTab::~CServerSettingTab()
{
	SAFE_DELETE( m_pFont );
}
...
BOOL CServerSettingTab::OnInitDialog() 
{
	HWND hTabWnd;
	CRect rectTabCtrl;
...
BEGIN_MESSAGE_MAP(CServerSettingTab, CPropertySheet)
	//{{AFX_MSG_MAP(CServerSettingTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

HBRUSH CServerSettingTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL CServerSettingTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void CServerSettingTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
...
void CServerSettingTab::InitPages()
{
...
void CServerSettingTab::ShowData()
{
	m_PageBasic.ShowData();
	m_PageDatabase.ShowData();
	m_PageServer1.ShowData();
	m_PageServer2.ShowData();
}
