...
BOOL DxFrame::LoadFile_BETA ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice, PDXEFFECTBASE &pEffGlobalHead, PDXFRAME &pAlphaMapHead )
{
	DWORD dw;
	SFile >> dw; //version
	...
	{
		pmsMeshs = new DxMeshes;
		pmsMeshs->LoadFile_BETA ( SFile, pd3dDevice );
	}
	...
	{	
		pframeSibling = new DxFrame;
		pframeSibling->LoadFile_BETA ( SFile, pd3dDevice, pEffGlobalHead, pAlphaMapHead );
	}
	...
	{
		pframeFirstChild = new DxFrame;
		pframeFirstChild->LoadFile_BETA ( SFile, pd3dDevice, pEffGlobalHead, pAlphaMapHead );
	}
