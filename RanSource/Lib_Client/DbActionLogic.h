...
#include "./G-Logic/GLPVPTyrannyData.h"
...
class CSetPoints : public CDbAction
{
public:
    CSetPoints( int nChaNum, DWORD dwUserID, DWORD dwClientID , WORD wType , int nAmount );
	virtual ~CSetPoints() {};
	virtual int Execute( CServer* pServer );
...
public:
	...
	* \return 
	*/
	CSetClubBattle(DWORD dwClubP, DWORD dwClubS, __time64_t tStartTime, __time64_t tEndTime, bool bAlliance = false );
...
public:
	...
	* \return 
	*/
	CSetChatBlockTime(int nUserNum, const char* szUserID, __time64_t tBlockTime, const char* szReason, int nGMUserNum, const char* szGMUserID);
...
class gm_cmd_logs : public CDbAction
{
public:
    gm_cmd_logs ( DWORD client_id, DWORD user_id, DWORD user_type, DWORD char_id, const char* char_name, const char* command_info );
    virtual ~gm_cmd_logs() {} ;
    virtual int Execute(CServer* pServer);
...
protected:
	...
	DWORD m_user_type;
	DWORD m_char_id;
	char  m_char_name[GM_CMD_LOG_NAME_TEXT_SIZE+1];	
	char  m_command_info[GM_CMD_LOG_COMMAND_TEXT_SIZE+1];	
};

class CLogPutOnDrop : public CDbAction
{
public:
	CLogPutOnDrop ( DWORD dwClientID, DWORD dwCharID, const char* szCharName, const char* szSerial, 
	...
	virtual ~CLogPutOnDrop() {} ;
	virtual int Execute(CServer* pServer);

protected:
	DWORD m_dwCharID;
	char m_szCharName[CHAR_SZNAME];
	char m_szSerial[LOG_PUTON_SERIAL_SIZE+1];
	...
	SNATIVEID m_sFactID_A;
	SNATIVEID m_sFactID_B;

	BOOL m_bPet;
	SNATIVEID m_sPetSkill_A;
	SNATIVEID m_sPetSkill_B;
};

class PCIDBlock : public CDbAction
{
public:
	PCIDBlock ( DWORD dwClientID, DWORD dwUserID, DWORD dwCharID, const char* szCharName, DWORD dwType, const char* szHWID, const char* szMAC );
	virtual ~PCIDBlock() {} ;
	virtual int Execute(CServer* pServer);
...
protected:
	DWORD m_dwUserID;
	DWORD m_dwCharID;
	char m_szName[CHAR_SZNAME+1];

	DWORD m_dwType;
	char  m_szHWID[PCID_LENGTH];	
	char  m_szMAC[PCID_LENGTH];	
};

/* user flag restricted, Juver, 2020/04/21 */
class UserRestrict : public CDbAction
{
public:
	UserRestrict ( DWORD dwClientID, DWORD dwUserID, bool bUserRestrict );
	virtual ~UserRestrict() {} ;
	virtual int Execute(CServer* pServer);

protected:
	DWORD m_dwUserID;
	bool m_bUserRestrict;
};

/* log gm party, Juver, 2021/06/13 */
class LogGMParty : public CDbAction
{
public:
	LogGMParty ( DWORD dwClientID, DWORD dwGMUserNum, DWORD dwGMChaNum, char* szGMChaName, DWORD dwGMUserType, DWORD dwTargetUserNum, DWORD dwTargetChaNum, char* szTargetChaName, DWORD dwTargetUserType );
	virtual ~LogGMParty() {} ;
	virtual int Execute(CServer* pServer);
...
protected:
	DWORD m_dwGMUserNum;
	DWORD m_dwGMChaNum;
	char m_szGMChaName[CHAR_SZNAME+1];
	DWORD m_dwGMUserType;
	
	DWORD m_dwTargetUserNum;
	DWORD m_dwTargetChaNum;
	char m_szTargetChaName[CHAR_SZNAME+1];
	DWORD m_dwTargetUserType;
};

#if defined( BUILD_CH ) || defined( BUILD_CHY )

/* wtf, Juver, 2021/06/24 */
class CWTFAction : public CDbAction
{
public:
	CWTFAction();
	virtual ~CWTFAction() {} ;
...
class LogMaxLevel : public CDbAction
{
public:
	LogMaxLevel ( DWORD dwClientID, DWORD dwUserID, DWORD dwCharID, const char* szName, WORD wLevel );
	virtual ~LogMaxLevel() {} ;
	virtual int Execute(CServer* pServer);
...
class CLogTyrannyResult : public CDbAction
{
public:
	CLogTyrannyResult ( WORD wTyrannyWinner, DWORD dwParticipantSG, DWORD dwParticipantMP, DWORD dwParticipantPHX );
	virtual ~CLogTyrannyResult() {} ;
	virtual int Execute(CServer* pServer);

protected:
	WORD m_wTyrannyWinner;
	DWORD m_dwParticipantSG;
	DWORD m_dwParticipantMP;
	DWORD m_dwParticipantPHX;
};

/* tyranny result log, Juver, 2021/09/09 */
class CLogTyrannyParticipant : public CDbAction
{
public:
	CLogTyrannyParticipant ( 
		int nSessionID,
		DWORD m_dwChaNum,
		const char* m_szName, 
		WORD m_wSchool, 
		...
		WORD m_wScoreKill,
		WORD m_wScoreDeath,
		DWORD m_dwScoreDamage,
		DWORD m_dwScoreHeal,
		WORD m_wScoreResu,
		DWORD m_dwScoreTotal,
		WORD m_wRewardPoint );

	virtual ~CLogTyrannyParticipant() {} ;
	virtual int Execute(CServer* pServer);
