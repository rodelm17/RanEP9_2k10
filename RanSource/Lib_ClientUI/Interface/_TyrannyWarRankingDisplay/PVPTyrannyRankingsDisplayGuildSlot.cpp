...
CPVPTyrannyRankingsDisplayGuildSlot::CPVPTyrannyRankingsDisplayGuildSlot ()
: m_pTextRank(NULL)
, m_pTextClub(NULL)
...
void CPVPTyrannyRankingsDisplayGuildSlot::CreateSubControl ()
{
...
void CPVPTyrannyRankingsDisplayGuildSlot::ResetData()
{
	m_sTyrannyClubRank	= STYRANNY_CLUB_RANK();
...
HRESULT CPVPTyrannyRankingsDisplayGuildSlot::Render(LPDIRECT3DDEVICEQ pd3dDevice)
{
	...
	BOOL bGuild = m_sTyrannyClubRank.wGuNum ? TRUE : FALSE;
	if (bGuild && m_sTyrannyClubRank.wGuNum > 0)
	{
		m_pIconGuild->SetVisibleSingle(TRUE);
		const int nSERVER					= GLGaeaClient::GetInstance().GetCharacter()->m_dwServerID;
		const DxClubMan::DXDATA& sMarkData	= DxClubMan::GetInstance().GetClubData(pd3dDevice, nSERVER, m_sTyrannyClubRank.dwClubID, m_sTyrannyClubRank.wGuMarkVer);
...
void CPVPTyrannyRankingsDisplayGuildSlot::SetData( STYRANNY_CLUB_RANK sTyrannyClubRank, int nRank, std::string strClub, DWORD dwClubID, WORD wGuMarkVer, WORD wKill, WORD wDeath , WORD wResu )
{
	ResetData();

	m_sTyrannyClubRank = sTyrannyClubRank;
