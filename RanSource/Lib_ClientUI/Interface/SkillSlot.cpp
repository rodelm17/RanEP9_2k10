...
//		GASSERT ( 0 && "무효화한 ID 입니다. 확인하십시오." );
	...
	if ( m_pSkillUpButton )
	{
		...
		{
			m_bLEVELUP_ENABLE = FALSE;
			m_bPURCHASE_ENABLE = GLGaeaClient::GetInstance().GetCharacter()->IsPurchaseSkill(m_sNativeID);
		}
		
		m_pSkillUpButton->SetFlip ( m_bLEVELUP_ENABLE || m_bPURCHASE_ENABLE );
		m_pSkillUpButton->SetVisibleMouseOver( m_bLEVELUP_ENABLE || m_bPURCHASE_ENABLE);
	}
