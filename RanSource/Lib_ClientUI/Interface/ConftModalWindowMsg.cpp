	...
	case CONFT_MODAL_USABLEHP_COMBO_ROLLOVER:
		{
			if ( m_bReqModal )	//	신청시에만 동작
			{
				...
				{				
					...
					if ( nIndex < 0 ) return ;
					RPROFILE::wCONFT_RECOVER = GLCONST_CHAR::wCONFT_RC_TYPE[nIndex];
					LoadUsableHP ();
				}
			}
		}
			...
			if ( m_bReqModal )	//	신청시에만 동작
			{
				...
				if ( CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
				{
					if ( m_bReqModal )
					{
						...
						if ( nIndex < 0 ) return ;
						RPROFILE::fCONFT_HP_RATE = GLCONST_CHAR::fCONFT_STATE_TYPE[nIndex];
					}
			...
			if ( m_bReqModal )	//	신청시에만 동작
			{
				...
				if ( CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
				{
					if ( m_bReqModal )
					{
						...
						if ( nIndex < 0 ) return ;
						RPROFILE::fCONFT_TAR_HP_RATE = GLCONST_CHAR::fCONFT_STATE_TYPE[nIndex];
					}
	...
	case MODAL_CONFT_OK:
		{
			...
			{
				//prevent duel accept when pet was used just a few seconds ago
