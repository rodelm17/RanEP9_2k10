...
namespace ITEM
{
	...
	BOOL SBOX::LOAD ( basestream &SFile )
	{
		...
		case 0x104:
			{
				...
				{
					SBOX_ITEM box;
					SFile.ReadBuffer ( &box, sizeof(SBOX_ITEM) );
		...
		case 0x103:
			{
				SFile.ReadBuffer ( sITEMS, sizeof(SBOX_ITEM)*SBOX_103::ITEM_SIZE );
				SFile >> bData;
			}break;
		...
		case 0x101:
			{
				SBOX_ITEM_101 _sOLD_ITEMS[SBOX_103::ITEM_SIZE];
				SFile.ReadBuffer ( _sOLD_ITEMS, sizeof(SBOX_ITEM_101)*SBOX_103::ITEM_SIZE );

				for ( int i=0; i<SBOX_103::ITEM_SIZE; ++i )
				{
		...
		case 0x100:
			{
				SNATIVEID _sOLD_ITEMS[SBOX_103::ITEM_SIZE];
				SFile.ReadBuffer ( _sOLD_ITEMS, sizeof(DWORD)*SBOX_103::ITEM_SIZE );
