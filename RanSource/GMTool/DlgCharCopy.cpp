...
BOOL CDlgCharCopy::OnInitDialog()
{
	...
	SetWin_Num_int( this, IDC_COPY_CHARUSER, m_pData->pCharData2->GetUserID() );
	SetWin_Num_int( this, IDC_COPY_CHARID, m_pData->pCharData2->m_dwCharID );
	SetWin_Text( this, IDC_COPY_CHARNAME, m_pData->pCharData2->m_szName );
	SetWin_Text( this, IDC_COPY_NEWNAME, m_pData->pCharData2->m_szName );
...
void CDlgCharCopy::OnBnClickedCopyButtonCopy()
{
	...
	StringCchCopy( m_pData->pCharData2->m_szName, CHAR_SZNAME, strName.c_str() );
	StringCchCopy( m_pData->pCharData2->m_szNick, CHAR_SZNAME, "" );
	m_pData->pCharData2->m_dwGuild = CLUB_NULL;

	SMAPNODE* pNode = GMTOOL_GLOBAL::gmtoolMapList.FindMapNode( m_pData->pCharData2->m_sStartMapID );
	if ( !pNode )
	{
		m_pData->pCharData2->m_sStartMapID = GLCONST_CHAR::nidSTARTMAP[m_pData->pCharData2->m_wSchool];
		m_pData->pCharData2->m_dwStartGate = GLCONST_CHAR::dwSTARTGATE[m_pData->pCharData2->m_wSchool];
		m_pData->pCharData2->m_vStartPos   = D3DXVECTOR3(0.0f,0.0f,0.0f);
	}
