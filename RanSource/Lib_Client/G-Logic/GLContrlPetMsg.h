...
enum EMPET_USECARD_FB
{
	...
	EMPET_USECARD_FB_ACTIONLIMIT   = 7,		/*skill pet off, Juver, 2018/09/07 */
	EMPET_USECARD_FB_TIME_DELAY    = 8,		/*pet fix add call delay, EJCode, 2018/11/28 */
};
...
namespace GLMSG
{
	...
	struct SNETPET_REQ_USEPETCARD
	{
		...
		WORD				wPosX;
		WORD				wPosY;
		BOOL				bCheckDelay;			/*pet fix add call delay, EJCode, 2018/11/28 */

		SNETPET_REQ_USEPETCARD () :
			wPosX(0),
			wPosY(0),
			bCheckDelay(FALSE)						/*pet fix add call delay, EJCode, 2018/11/28 */
		{
			nmg.dwSize = sizeof(*this);
