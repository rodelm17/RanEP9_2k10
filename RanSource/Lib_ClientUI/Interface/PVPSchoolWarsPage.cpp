...
void CPVPSchoolWarsPage::CreateSubControl ()
{
	...
	m_pTextMapName->AddText( ID2GAMEWORD( "PVP_COMPETITION_BUTTON", 1 ), NS_UITEXTCOLOR::ORANGE );
	...
	m_pRewardButton->CreateSub( this, "COMPETITION_SCHOOLWARS_PAGE_REWARD_INFO_BUTTON", UI_FLAG_DEFAULT, SCHOOLWARS_PAGE_INFO_REWARD_BUTTON );
	m_pRewardButton->CreateFlip( "COMPETITION_SCHOOLWARS_PAGE_REWARD_INFO_BUTTON_FLIP", CBasicButton::MOUSEIN_FLIP );
...
void CPVPSchoolWarsPage::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	switch( ControlID )
	{
	case SCHOOLWARS_PAGE_INFO_REWARD_BUTTON:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE(dwMsg) )
			{
				/*pvp reward info, Juver, 2018/07/25 */
				CInnerInterface::GetInstance().PVPSchoolWarsShowRewardInfo();
			}
		}break;
	};
}
...
void CPVPSchoolWarsPage::UpdateInfo()
{
	...
	if ( m_pTextInfoContri )
	{
		...
		DWORD dwContriReq = GLPVPSchoolWarsClient::GetInstance().m_dwContriReq;
		nIndex = m_pTextInfoContri->AddText( ID2GAMEWORD( "COMPETITION_SCHOOLWARS_BATTLE_INFO_TEXT", 7 ), NS_UITEXTCOLOR::WHITE );
		strCombine.Format( ID2GAMEWORD( "COMPETITION_SCHOOLWARS_BATTLE_INFO_CONTRI", 0 ), dwContriReq );
...
void CPVPSchoolWarsPage::SetCompetitionJoin( bool bEnable )
{
	if ( m_pInfoBattle )
