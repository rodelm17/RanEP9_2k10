...
#include "ClubWarRankingsDisplayResuSlot.h"
...
CClubWarRankingsDisplayResu::CClubWarRankingsDisplayResu()
: m_pRankText(NULL)
, m_pNameText(NULL)
, m_pScoreText(NULL)
, m_pSlotOwn(NULL)
...
void CClubWarRankingsDisplayResu::CreateSubControl ()
{
	...
	for( int i=0; i<CLUBWAR_RANKING_NUM; ++i )
	{
		m_pSlot[i] = new CClubWarRankingsDisplayResuSlot;
	...
	m_pSlotOwn = new CClubWarRankingsDisplayResuSlotSelf;
...
void CClubWarRankingsDisplayResu::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
}

void CClubWarRankingsDisplayResu::RefreshClubWarRankingResu()
{
	...
	if ( m_pSlotOwn )
	{
		const SCLUBWAR_RANK_RESU_EX& sMyRank = GLGaeaClient::GetInstance().m_sMyClubWarRankResu;
...
void CClubWarRankingsDisplayResu::ShowData()
{
	for( int i=0; i<CLUBWAR_RANKING_NUM; ++i )
	{
		if ( m_pSlot[i] )	
	...
	CLUBWAR_RANK_RESU_VEC& vecClubWarRank = GLGaeaClient::GetInstance().m_vecClubWarRankResu;
	...
	for ( int i = (int)m_nStart; i < nEnd; ++ i )
	{
		...
		if ( m_pSlot[nSlot] )
		{
			SCLUBWAR_RANK_RESU sClubWarRank = vecClubWarRank[i];
