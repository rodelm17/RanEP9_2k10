	...
	case MAP_WINDOW_MONSTER_ICON12:
		{
			if ( CHECK_MOUSE_IN(dwMsg))
			{
				if ( CHECK_MOUSE_IN(dwMsg) )
				{
					...
					{
						CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( m_pCrowData->GetName(), NS_UITEXTCOLOR::PALEGREEN );
						CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( m_strDesc.GetString(), NS_UITEXTCOLOR::WHITE );
	...
	case MAP_WINDOW_MONSTER_ICON_GATE:
		{
			if ( CHECK_MOUSE_IN(dwMsg))
			{
				if ( m_szMapName ) CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine( m_szMapName, NS_UITEXTCOLOR::WHITE );
			}
		}
		break;
	}
...
void CLargeMapWindowSlotRender::SetCrow( PCROWDATA pCrow,  GLCHARLOGIC_CLIENT *pCHAR )
{
	m_pCrowData = pCrow;
	...
	else
	{
		...
		else
		{
			...
			if ( pDialogue )
			{
				GLCHARLOGIC_CLIENT* pCHARLOGIC = pCHAR;
