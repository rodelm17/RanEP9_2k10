...
CCharEditTab::CCharEditTab( SGMToolCharData* pData, CRect& rect, LOGFONT logfont, CWnd* pParentWnd /*= NULL*/ )
	 : CPropertySheet("Sheet", pParentWnd),
	m_bNoTabs(FALSE),
	m_pWndParent(NULL),
	m_iDistanceFromTop(0),
	
	m_PageBasic ( logfont, pData->pCharData2 ),
	m_PagePutOn ( logfont, pData->pCharData2 ),
	m_PageInven ( logfont, pData->pCharData2 ),
	m_PageSkills ( logfont, pData->pCharData2 ),
	m_PageSlotAction ( logfont, pData->pCharData2 ),
	m_PageSlotSkill ( logfont, pData->pCharData2 ),
	m_PageQuest ( logfont, pData->pCharData2 ),
	m_PageCoolTime ( logfont, pData->pCharData2 ),
	m_PageLocker ( logfont, pData->pCharData2 ),
	m_PageItemFood ( logfont, pData->pCharData2 ),
	m_PageActivity ( logfont, pData->pCharData2 ),				/*activity system, Juver, 2017/10/23 */
	m_PagePersonalLock ( logfont, pData->pCharData2 ), 
	m_PagePKCombo ( logfont, pData->pCharData2 ),				/* pk combo, Juver, 2021/02/11 */
	m_PageAwardTitle( logfont, pData ),							/* award title, Juver, 2021/02/13 */
	m_PageCombatRecord( logfont, pData->pCharData2 ),	
	m_PageCodex ( logfont, pData->pCharData2 ),/*Item Codex, Jhoniex 2/14/2024 */
	...
	m_pData( NULL )
{
	...
	m_PagePersonalLock.SetSheetTab ( this );
	m_PagePKCombo.SetSheetTab ( this );				/* pk combo, Juver, 2021/02/11 */	
	m_PageAwardTitle.SetSheetTab ( this );			/* award title, Juver, 2021/02/13 */
	m_PageCombatRecord.SetSheetTab ( this );
	m_PageCodex.SetSheetTab ( this ); /*Item Codex, Jhoniex 2/14/2024 */
	...
	AddPage ( &m_PagePersonalLock );
	AddPage ( &m_PagePKCombo );						/* pk combo, Juver, 2021/02/11 */
	AddPage ( &m_PageAwardTitle );					/* award title, Juver, 2021/02/13 */
	AddPage ( &m_PageCombatRecord );
	AddPage ( &m_PageCodex );/*Item Codex, Jhoniex 2/14/2024 */
...
BOOL CCharEditTab::SaveInfo()
{
	...
	bSAVE = m_PagePersonalLock.DataSave();
	if ( !bSAVE )	
		return FALSE;

	/* pk combo, Juver, 2021/02/11 */
	bSAVE = m_PagePKCombo.DataSave();
	if ( !bSAVE )	 
		return FALSE;

	bSAVE = m_PageCombatRecord.DataSave();
	if ( !bSAVE )	
		return FALSE;
