...
BOOL GetWinVer(LPTSTR pszVersion, int *nVersion, LPTSTR pszMajorMinorBuild, LPTSTR pszServicePack)
{
	...
	*nVersion = WUNKNOWN;
	...
	lstrcpy(pszServicePack, osinfo.szCSDVersion);
	...
	else if (dwPlatformId == VER_PLATFORM_WIN32_NT)
	{
		...
		else if ((dwMajorVersion == 6) && ( dwMinorVersion == 0))
		{
			...
			else if ( osinfo.wProductType == VER_NT_DOMAIN_CONTROLLER || osinfo.wProductType == VER_NT_SERVER )
			{
				lstrcpy(pszVersion, WSVR2008STR );
				*nVersion = WSVR2008;
			}
		}
		else if ((dwMajorVersion == 6) && ( dwMinorVersion == 1))
		{
			...
			{
				lstrcpy(pszVersion, WSVR2008R2STR );
				*nVersion = WSVR2008R2;
			}
		}
		...
		else if ((dwMajorVersion == 6) && ( dwMinorVersion == 3))
		{
			...
			{
				lstrcpy(pszVersion, WSVR2012R2STR );
				*nVersion = WSVR2012R2;
			}	
		}
