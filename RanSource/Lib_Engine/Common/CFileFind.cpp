...
{
	std::string find = str.c_str();
	std::size_t nfound = find.rfind( '.' );
	if ( nfound != std::string::npos ){
		find.erase( nfound );
	}

	FILEMAP_ITER pos = m_mapFile.begin();
	FILEMAP_ITER end = m_mapFile.end();
	for ( ; pos != end; pos++ )
	{
		std::string check = pos->first;	
		std::size_t nfound2 = check.rfind( '.' );
		if ( nfound2 != std::string::npos ){
			check.erase( nfound2 );
		}
			
		if ( strcmp( find.c_str(), check.c_str() ) == 0  ){
			return &(pos->second);
		}
	}
