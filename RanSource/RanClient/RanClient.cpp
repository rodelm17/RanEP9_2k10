...
// RanClient.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CRanClientApp, CWinApp)
	ON_COMMAND(ID_HELP, OnHelp)
END_MESSAGE_MAP()


// CRanClientApp construction

CRanClientApp::CRanClientApp()
	: m_bIsActive(TRUE)
	, m_bRedrawScreen(FALSE)
	, m_bShutDown(FALSE)
{
	BUG_TRAP::BugTrapInstall( std::string(_T("RanClient")));
...
// The one and only CRanClientApp object

CRanClientApp theApp;


// CRanClientApp initialization

BOOL CRanClientApp::InitInstance()
{
	...
	CRanClientWnd* pWnd = new CRanClientWnd();
...
int CRanClientApp::ExitInstance() 
{
	m_pWnd = NULL;
...
void CRanClientApp::SetActive ( BOOL bActive )
{
	m_bIsActive = bActive;
