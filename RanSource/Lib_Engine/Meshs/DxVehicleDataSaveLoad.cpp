#include "stdafx.h"
#include "./DxVehicleData.h"

#include "../Common/SerialFile.h"
#include "../G-Logic/GLogic.h"

#include "./DxSkinPieceContainer.h"
#include "./DxSkinCharData.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif

BOOL DxVehicleData::SaveFile ( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, bool bTool )
{
	char szPathName[MAX_PATH] = "";
	StringCchCopy( szPathName, MAX_PATH, DxVehicleDataContainer::GetInstance().GetPath() );
	StringCchCat( szPathName, MAX_PATH, szFile );

	CSerialFile SFile;
	if ( !SFile.OpenFile ( FOT_WRITE, szPathName ) )		return FALSE;

	SFile << (DWORD)VERSION_VCF;

	SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_VEHICLE_DATA_V1 );

	//skeleton name
	SFile << m_strSkeleton;

	//piece
	SFile << (DWORD)PIECE_SIZE_TO_SAVE;
	for ( int i=0; i<PIECE_SIZE_TO_SAVE; ++i )
	{
		SFile << BOOL(m_pPieceArray[i]!=NULL);
		if ( !m_pPieceArray[i] )	continue;

		std::string strPieceFile = m_pPieceArray[i]->m_szFileName;
		SFile << strPieceFile;
	}

	//animation
	SFile << (DWORD)DxSkinAniControl::GETANIAMOUNT();
	for ( size_t n=0; n<m_vecAnim.size(); ++n )
	{
		std::string strNAME = m_vecAnim[n]->pAnimCont->m_szName;
		SFile << strNAME;
	}

	SFile << m_fScale;
	SFile << m_fHeight;
	SFile << m_fRadius;
	SFile << m_vMax;
	SFile << m_vMin;
	SFile << m_bWorldObj;

	SFile << m_strCharFront;
	SFile << m_affCharFront.vTrans;
	SFile << m_affCharFront.vRotate;
	SFile << m_affCharFront.vScale;
	
	SFile << m_strCharBack;
	SFile << m_affCharBack.vTrans;
	SFile << m_affCharBack.vRotate;
	SFile << m_affCharBack.vScale;
	
	SFile << (BOOL)FALSE;

	StringCchCopy( m_szFileName, FILE_LENGTH, szFile );

	return TRUE;
}

BOOL DxVehicleData::LOAD_Ver_0100( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwPieceSize(0);
	SFile >> dwPieceSize;
	for ( DWORD i=0; i<dwPieceSize; ++i )
	{
		BOOL bPieceUse = FALSE;
		std::string strPieceFile = "";

		SFile >> bPieceUse;
		if ( bPieceUse )
			SFile >> strPieceFile;

		if ( i >= PIECE_SIZE )	continue;

		if ( bPieceUse )
			m_strPIECE[i] = strPieceFile.c_str();
		else
			m_strPIECE[i].clear();
	}

	SFile >> m_strSkeleton;

	DWORD dwAniNum(0);
	SFile >> dwAniNum;
	m_vecANIFILE.clear();
	m_vecANIFILE.reserve ( dwAniNum );
	for ( DWORD i=0; i<dwAniNum; ++i )
	{
		std::string strANIFILE;
		SFile >> strANIFILE;

		CString strANISTRING = strANIFILE.c_str();
		int nIndex = strANISTRING.ReverseFind ( '.' );
		CString strANI = strANISTRING.Left(nIndex) + ".x";
		m_vecANIFILE.push_back ( strANI.GetString() );
	}

	SFile >> m_fScale;

	BOOL bExist = FALSE;
	bExist = FALSE;
	SFile >> bExist;
	if( bExist )	
	{
		SFile >> m_fHeight;
		SFile >> m_fRadius;
	}
	else			return TRUE;

	//unknown data
	DWORD	dwBoneScale(0);
	SFile >> dwBoneScale;
	for ( DWORD i=0; i < dwBoneScale; ++i )
	{
		SBONESCALE	sBoneScale;
		SFile >> sBoneScale.vScale;
		SFile >> sBoneScale.strBONE;
	}

	BOOL bBoundBox(FALSE);
	SFile >> bBoundBox;
	if ( bBoundBox )
	{
		SFile >> m_vMax;
		SFile >> m_vMin;
	}
	else
	{
		m_vMax = D3DXVECTOR3(7.f,20.f,7.f);
		m_vMin = D3DXVECTOR3(-7.f,0.f,-7.f);
	}

	SFile >> bExist;
	if( bExist )	
		SFile >> m_bWorldObj;
	else			
		return TRUE;

	SFile >> bExist;

	SFile >> m_affCharFront.vTrans;
	SFile >> m_affCharFront.vRotate;
	SFile >> m_strCharFront;

	SFile >> m_affCharBack.vTrans;
	SFile >> m_affCharBack.vRotate;
	SFile >> m_strCharBack;

	m_affCharFront.vScale = D3DXVECTOR3(1,1,1);
	m_affCharBack.vScale = D3DXVECTOR3(1,1,1);

	return TRUE;
}

BOOL DxVehicleData::LOAD_Ver_0101( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwPieceSize(0);
	SFile >> dwPieceSize;
	for ( DWORD i=0; i<dwPieceSize; ++i )
	{
		BOOL bPieceUse = FALSE;
		std::string strPieceFile = "";

		SFile >> bPieceUse;
		if ( bPieceUse )
			SFile >> strPieceFile;

		if ( i >= PIECE_SIZE )	continue;

		if ( bPieceUse )
			m_strPIECE[i] = strPieceFile.c_str();
		else
			m_strPIECE[i].clear();
	}

	SFile >> m_strSkeleton;

	DWORD dwAniNum(0);
	SFile >> dwAniNum;
	m_vecANIFILE.clear();
	m_vecANIFILE.reserve ( dwAniNum );
	for ( DWORD i=0; i<dwAniNum; ++i )
	{
		std::string strANIFILE;
		SFile >> strANIFILE;

		CString strANISTRING = strANIFILE.c_str();
		int nIndex = strANISTRING.ReverseFind ( '.' );
		CString strANI = strANISTRING.Left(nIndex) + ".x";
		m_vecANIFILE.push_back ( strANI.GetString() );
	}

	SFile >> m_fScale;

	BOOL bExist = FALSE;
	bExist = FALSE;
	SFile >> bExist;
	if( bExist )	
	{
		SFile >> m_fHeight;
		SFile >> m_fRadius;
	}
	else			return TRUE;

	//unknown data
	DWORD	dwBoneScale(0);
	SFile >> dwBoneScale;
	for ( DWORD i=0; i < dwBoneScale; ++i )
	{
		SBONESCALE	sBoneScale;
		SFile >> sBoneScale.vScale;
		SFile >> sBoneScale.strBONE;
	}

	BOOL bBoundBox(FALSE);
	SFile >> bBoundBox;
	if ( bBoundBox )
	{
		SFile >> m_vMax;
		SFile >> m_vMin;
	}
	else
	{
		m_vMax = D3DXVECTOR3(7.f,20.f,7.f);
		m_vMin = D3DXVECTOR3(-7.f,0.f,-7.f);
	}

	SFile >> bExist;
	if( bExist )	
		SFile >> m_bWorldObj;
	else			
		return TRUE;

	SFile >> bExist;

	SFile >> m_affCharFront.vTrans;
	SFile >> m_affCharFront.vRotate;
	SFile >> m_strCharFront;

	SFile >> m_affCharBack.vTrans;
	SFile >> m_affCharBack.vRotate;
	SFile >> m_strCharBack;

	m_affCharFront.vScale = D3DXVECTOR3(1,1,1);
	m_affCharBack.vScale = D3DXVECTOR3(1,1,1);

	return TRUE;
}


BOOL DxVehicleData::LOAD_Ver_0200( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//skeleton name
	SFile >> m_strSkeleton;

	//piece
	DWORD dwPieceSize(0);
	SFile >> dwPieceSize;
	for ( DWORD i=0; i<dwPieceSize; ++i )
	{
		BOOL bPieceUse = FALSE;
		std::string strPieceFile = "";

		SFile >> bPieceUse;
		if ( bPieceUse )
			SFile >> strPieceFile;

		if ( i >= PIECE_SIZE )	continue;

		if ( bPieceUse )
			m_strPIECE[i] = strPieceFile.c_str();
		else
			m_strPIECE[i].clear();
	}

	//animation
	DWORD dwAniNum(0);
	SFile >> dwAniNum;
	m_vecANIFILE.clear();
	m_vecANIFILE.reserve ( dwAniNum );

	for ( DWORD i=0; i<dwAniNum; ++i )
	{
		std::string strANIFILE;
		SFile >> strANIFILE;

		CString strANISTRING = strANIFILE.c_str();
		int nIndex = strANISTRING.ReverseFind ( '.' );
		CString strANI = strANISTRING.Left(nIndex) + ".x";
		m_vecANIFILE.push_back ( strANI.GetString() );
	}

	SFile >> m_fScale;
	SFile >> m_fHeight;
	SFile >> m_fRadius;
	SFile >> m_vMax;
	SFile >> m_vMin;
	SFile >> m_bWorldObj;

	SFile >> m_strCharFront;
	SFile >> m_affCharFront.vTrans;
	SFile >> m_affCharFront.vRotate;
	SFile >> m_affCharFront.vScale;

	SFile >> m_strCharBack;
	SFile >> m_affCharBack.vTrans;
	SFile >> m_affCharBack.vRotate;
	SFile >> m_affCharBack.vScale;

	BOOL bExist = FALSE;
	SFile >> bExist;

	return TRUE;
}


BOOL DxVehicleData::LOAD_Ver_0201( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	//skeleton name
	SFile >> m_strSkeleton;

	//piece
	DWORD dwPieceSize(0);
	SFile >> dwPieceSize;
	for ( DWORD i=0; i<dwPieceSize; ++i )
	{
		BOOL bPieceUse = FALSE;
		std::string strPieceFile = "";

		SFile >> bPieceUse;
		if ( bPieceUse )
			SFile >> strPieceFile;

		if ( i >= PIECE_SIZE )	continue;

		if ( bPieceUse )
			m_strPIECE[i] = strPieceFile.c_str();
		else
			m_strPIECE[i].clear();
	}

	//animation
	DWORD dwAniNum(0);
	SFile >> dwAniNum;
	m_vecANIFILE.clear();
	m_vecANIFILE.reserve ( dwAniNum );

	for ( DWORD i=0; i<dwAniNum; ++i )
	{
		std::string strANIFILE;
		SFile >> strANIFILE;

		CString strANISTRING = strANIFILE.c_str();
		int nIndex = strANISTRING.ReverseFind ( '.' );
		CString strANI = strANISTRING.Left(nIndex) + ".x";
		m_vecANIFILE.push_back ( strANI.GetString() );
	}

	SFile >> m_fScale;
	SFile >> m_fHeight;
	SFile >> m_fRadius;
	SFile >> m_vMax;
	SFile >> m_vMin;
	SFile >> m_bWorldObj;

	SFile >> m_strCharFront;
	SFile >> m_affCharFront.vTrans;
	SFile >> m_affCharFront.vRotate;
	SFile >> m_affCharFront.vScale;

	SFile >> m_strCharBack;
	SFile >> m_affCharBack.vTrans;
	SFile >> m_affCharBack.vRotate;
	SFile >> m_affCharBack.vScale;

	BOOL bExist = FALSE;
	SFile >> bExist;

	return TRUE;
}

BOOL DxVehicleData::LoadFile ( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread, bool bREPRESH )
{
	std::string strCheckFileExt = szFile;
	std::transform ( strCheckFileExt.begin(), strCheckFileExt.end(), strCheckFileExt.begin(), std::tolower );
	std::string::size_type idx = strCheckFileExt.find( ".vcf" );
	if ( idx == std::string::npos )	return FALSE;

	char szPathName[MAX_PATH] = "";
	StringCchCopy( szPathName, MAX_PATH, DxVehicleDataContainer::GetInstance().GetPath() );
	StringCchCat( szPathName, MAX_PATH, szFile );

	std::auto_ptr<basestream> pBStream( GLOGIC::openfile_basestream(GLOGIC::bENGLIB_ZIPFILE, 
										GLOGIC::strSKINOBJ_ZIPFILE.c_str(), 
										szPathName, 
										szFile ) );

	if ( !pBStream.get() )
	{
		if ( strlen( szFile ) > 0 )
			CDebugSet::ErrorFile( "DxVehicleData::LoadFile", szFile );

		return FALSE;
	}

	basestream &SFile = *pBStream;

	m_fHeight = 20.f;
	m_fRadius = 7.f;

	DWORD dwVer;
	SFile >> dwVer;

	if ( dwVer >= VERSION_ENCRYPT )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_SKIN_CONTAINER_VER_1 );

	if ( dwVer >= ENCODE_VER_BYTE_CRYPT_2021_VEHICLE_DATA_V1 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_VEHICLE_DATA_V1 );

	switch ( dwVer )
	{
	case VERSION_VCF:
		LOAD_Ver_0201( SFile, pd3dDevice );
		break;

	case 0x0200:
		LOAD_Ver_0200( SFile, pd3dDevice );
		break;

	case 0x0103:
	case 0x0102:
	case 0x0101:
		LOAD_Ver_0101( SFile, pd3dDevice );
		break;

	case 0x0100:
		LOAD_Ver_0100( SFile, pd3dDevice );
		break;

	default:
		CDebugSet::ErrorVersion( "DxVehicleData::LoadFile", SFile.GetFileName(), dwVer );
		return FALSE;
	};

	m_pSkeleton = DxBoneCollector::GetInstance().Load ( m_strSkeleton.c_str(), pd3dDevice );
	if ( !m_pSkeleton )		return FALSE;

	for ( DWORD i=0; i<PIECE_SIZE; ++i )
	{
		if ( !m_strPIECE[i].empty() )
		{
			SetPiece ( m_strPIECE[i].c_str(), pd3dDevice, bThread );
		}
	}

	for ( DWORD i=0; i<(DWORD)m_vecANIFILE.size(); ++i )
	{
		DxSkinAniControl::LoadAnimation ( m_vecANIFILE[i].c_str(), pd3dDevice, bREPRESH );
	}

	DxSkinAniControl::ClassifyAnimation ();
	DxSkinAniControl::SELECTANI ( AN_VEHICLE_STAY );

	if ( !DxSkinAniControl::GETCURANIMNODE() )
	{
		CDebugSet::ToLogFile ( "select ani fail %s", szFile );
	}

	StringCchCopy( m_szFileName, FILE_LENGTH, szFile );

	return TRUE;
}