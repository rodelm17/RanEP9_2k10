...
char		DxEffCharAmbient::NAME[MAX_PATH]	= "DxEffCharAmbient";
...
HRESULT	DxEffCharAmbient::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		EFFCHAR_PROPERTY_AMBIENT_103	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_AMBIENT_103) );

		m_dwFlage	= sProp.m_dwFlage;
		m_fFullTime = sProp.m_fFullTime;
		m_cColor	= sProp.m_cColor;
		m_fSrcColor	= sProp.m_fSrcColor;
	...
	{
		EFFCHAR_PROPERTY_AMBIENT_102	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_AMBIENT_102) );
	...
	{
		EFFCHAR_PROPERTY_AMBIENT_101	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_AMBIENT_101) );
	...
	{
		EFFCHAR_PROPERTY_AMBIENT_100	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_AMBIENT_100) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharAmbient::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharAmbient::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharAmbient::R2LoadFile", SFile.GetFileName(), dwVer );
		//	Note : 버전이 틀릴 경우에는 파일에 쓰여진 DATA영역을 건더 띄는 작업을 진행.
		//
		SFile.SetOffSet ( SFile.GetfTell()+dwSize );
