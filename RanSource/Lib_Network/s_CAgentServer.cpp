	...
	if (pFieldInfo != NULL)
	{
		for (int nCh=0; nCh<MAX_CHANNEL_NUMBER; ++nCh)
		{
			for (int nSvrNum=0; nSvrNum<FIELDSERVER_MAX; ++nSvrNum)
			{
				...
				m_IPFilter.AddIPKnown( pFieldInfo[nCh*MAX_CHANNEL_NUMBER+nSvrNum].szServerIP );
			}
		}
	}
		...
		else if ((dwClient >= NET_RESERVED_SLOT) && (dwClient < (DWORD) m_nMaxClient)) // client 연결 종료
		{
			...
			*/
			...
			if ( nUserNum > 0 )
			{
				...
				PGLCHARAG pChar = GLAgentServer::GetInstance().GetChar(dwGaeaID);
				if( pChar )	bOffVend = pChar->m_bOffVend;
...
//					CConsoleMessage::GetInstance()->Write( _T("ERROR:%d Invalid Gaea ID"), dwClient );
				...
				*/
				...
				if( pChar )
				{
					if( pChar->m_sPMarket.IsPremiumMarket() )
					{
						CConsoleMessage::GetInstance()->WriteConsole( _T("IsPremium Vend ClientID:%d"), dwClient );
					}
					else
					{
						GLAgentServer::GetInstance().ReserveDropOutPC(dwGaeaID); // lock
...
int CAgentServer::ExecuteCommand(char* strCmd)
{
	...
	while (token != NULL)
	{
		...
		if( strcmp(token, "koload") == 0 )
		{
			GLAgentServer::GetInstance().KickOutListLoad();
			return 0;
		}

		if( strcmp(token, "instance_clear") == 0 )
		{
			GLAgentServer::GetInstance().InstanceClear();
			return 0;
		}

		/* game notice, Juver, 2021/06/12 */
		if( strcmp(token, "gn_reload") == 0 )
		{
			GLAgentServer::GetInstance().GetGameNotice();
			return 0;
		}
...
void CAgentServer::IPFilterBlockAdd( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
...
void CAgentServer::IPFilterBlockDel( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
		CConsoleMessage::GetInstance()->Write("Invalid IP");
...
void CAgentServer::IPFilterKnownAdd( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
		CConsoleMessage::GetInstance()->Write("Invalid IP");
...
void CAgentServer::IPFilterKnownDel( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
		CConsoleMessage::GetInstance()->Write("Invalid IP");
...
void CAgentServer::IPFilterSessionBlockAdd( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
...
void CAgentServer::IPFilterSessionBlockDel( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
...
void CAgentServer::IPFilterSessionKnownAdd( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
...
void CAgentServer::IPFilterSessionKnownDel( std::string strIP )
{
	if ( strIP.size() <= 0 )
	{
...
void CAgentServer::IPFilterSave()
{
...
/*client hash check, EJCode, 2018/11/26 */
void CAgentServer::client_check_get_client_hash( std::string app_path )
{
	...
	//CDebugSet::ToFileWithTime( "_client_hash_check.txt", "agent read file: %s hash: %s", client_file_1.c_str(), m_client_hash1 );
	...
	//CDebugSet::ToFileWithTime( "_client_hash_check.txt", "agent read file: %s hash: %s", client_file_2.c_str(), m_client_hash2 );
}

void CAgentServer::client_check_get_mshield_hash( std::string app_path )
{
#ifdef USE_MSHIELD
	HASH::CHash g_hashObj_1;
	g_hashObj_1.SetHashAlgorithm( HASH::MD5 );
	...
	//CDebugSet::ToFileWithTime( "_mshield_hash_check.txt", "agent read MShield1 file: %s hash: %s", mshield_file_1.c_str(), m_szMShieldDLLHash1 );
	...
	//CDebugSet::ToFileWithTime( "_mshield_hash_check.txt", "agent read MShield2 file: %s hash: %s", mshield_file_2.c_str(), m_szMShieldDLLHash2 );
#endif // USE_MSHIELD
}

void CAgentServer::client_check_rehash()
{
	SERVER_UTIL::CSystemInfo SysInfo;
