...
std::string	SNpcTalk::szBASICDESC[EM_BASIC_SIZE] = 
{
	...
	"ItemExchange",
	"CurrencyShop",
	"ChaosMachine",	/* chaos machine, Juver, 2021/07/07 */
	"Buff - BA Only",
	"Buff - Full Buffs",
	"Buff - FB no BA"
};
...
BOOL SNpcTalk::LOAD_0010 ( basestream& SFile )
{
	SFile >> m_dwNID;
	SFile >> m_strTalk;
	SFile >> m_nACTION;
...
BOOL SNpcTalk::LOAD_0201 ( basestream& SFile )
{
	SFile >> m_dwNID;
	SFile >> m_strTalk;
	SFile >> m_nACTION;
...
#ifdef _DEBUG
	case 0x000A:	LOAD_0010 ( SFile );	break;
#endif
	
	case 0x0200:	LOAD_0200 ( SFile );	break;
	case SNpcTalk::VERSION:	LOAD_0201 ( SFile );	break;
...
SNpcTalk&	SNpcTalk::operator= ( const SNpcTalk& rNpcTalk )
{
	...
	unknown_var_1 = rNpcTalk.unknown_var_1;
...
BOOL SNpcTalk::DoTEST ( GLCHARLOGIC_SERVER *pCHARLOGIC, DWORD dwIndex )
{
			...
			else
			{
				...
				if ( pQUEST_END )
				{
					...
					if ( pQUEST_END->m_bCOMPLETE )
					{
						...
						// are quests already completed as a daily quest
						if ( pQUEST->IsDaily() )
						{
							CTime cTime = CTime::GetCurrentTime();
							CTime cTimeEnd(pQUEST_END->m_tEndTime);
...
BOOL SNpcTalk::DoTEST ( GLCHARLOGIC_CLIENT *pCHARLOGIC, DWORD dwIndex )
{
	if ( !pCHARLOGIC )		return FALSE;
