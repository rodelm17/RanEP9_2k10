...
char		DxEffCharUserColor::NAME[MAX_PATH]	= "DxEffCharUserColor";
...
void DxEffCharUserColor::Render( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bPieceRender )
{
	...
	for ( DWORD i=0; i<m_dwMaterials; i++ )
	{
		if ( m_pMaterials[i].bEffUse )
		{
			...
			{
				DWORD dwColor = RGBA_MAKE( BYTE(m_cColor.r*255.0f), BYTE(m_cColor.g*255.0f), BYTE(m_cColor.b*255.0f), BYTE(m_cColor.a*255.0f) );

				if ( m_pCharPart->GetUseHairColor() )
					dwColor = m_pCharPart->GetHairColor();

				if( m_dwFlage&USEUSERCOLOR1 && m_pCharPart->GetUseColor1() )
					dwColor = m_pCharPart->GetColor1();
				else if( m_dwFlage&USEUSERCOLOR2 && m_pCharPart->GetUseColor2() )
					dwColor = m_pCharPart->GetColor2();
...
HRESULT	DxEffCharUserColor::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		//	Note : 이팩트의 Property 를 저장.
		EFFCHAR_PROPERTY_USERCOLOR_101 sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_USERCOLOR_101) );

		m_dwFlage		= sProp.m_dwFlage;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharUserColor::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharUserColor::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	if( dwVer == 0x0102 )
	{
		...
		if ( dwEffectUseFlag > 0 )
		{
			m_dwFlage += USEUSERCOLOR2;
		}
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharUserColor::R2LoadFile", SFile.GetFileName(), dwVer );
