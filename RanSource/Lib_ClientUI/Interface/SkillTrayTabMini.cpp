...
CSkillTrayTabMini::CSkillTrayTabMini(void)
...
void CSkillTrayTabMini::CreateSubControl()
{
	...
	m_pButtonOpen->CreateSub( this, "QUICK_SKILL_TRAY_OPEN_BUTTON", UI_FLAG_DEFAULT, QUICK_SKILL_TRAY_OPEN_BUTTON );
	m_pButtonOpen->CreateFlip( "QUICK_SKILL_TRAY_OPEN_BUTTON_F", CBasicButton::MOUSEIN_FLIP );
	m_pButtonOpen->SetUseGlobalAction ( TRUE );	
	RegisterControl( m_pButtonOpen );

	m_pButtonLock = new CBasicButton;
	m_pButtonLock->CreateSub( this, "QUICK_SKILL_TRAY_LOCK_BUTTON_CLOSE_LOCK", UI_FLAG_DEFAULT, QUICK_SKILL_TRAY_LOCK_BUTTON );
	m_pButtonLock->CreateFlip( "QUICK_SKILL_TRAY_LOCK_BUTTON_CLOSE_LOCK_F", CBasicButton::MOUSEIN_FLIP );
	...
	m_pButtonUnlock->CreateSub( this, "QUICK_SKILL_TRAY_LOCK_BUTTON_CLOSE_UNLOCK", UI_FLAG_DEFAULT, QUICK_SKILL_TRAY_UNLOCK_BUTTON );
	m_pButtonUnlock->CreateFlip( "QUICK_SKILL_TRAY_LOCK_BUTTON_CLOSE_UNLOCK_F", CBasicButton::MOUSEIN_FLIP );
	m_pButtonUnlock->SetUseGlobalAction ( TRUE );	
	m_pButtonLock->SetVisibleSingle( TRUE );
	RegisterControl( m_pButtonUnlock );

	CUIControl* pControl = new CUIControl;
	pControl->CreateSub ( this, "QUICK_SKILL_TRAY_GAP_2", UI_FLAG_XSIZE | UI_FLAG_YSIZE  );
	pControl->SetVisibleSingle ( TRUE );
	RegisterControl ( pControl );
}

void CSkillTrayTabMini::Update( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
...
void CSkillTrayTabMini::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	switch ( ControlID )
	{
...
void CSkillTrayTabMini::SetVisibleSingle( BOOL bVisible )
{
...
void CSkillTrayTabMini::CheckMouseState ()
{
