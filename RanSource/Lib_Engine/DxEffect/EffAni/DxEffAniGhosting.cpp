...
char		DxEffAniData_Ghosting::NAME[64]	= "DxEffAniData_Ghosting";
...
HRESULT	DxEffAniData_Ghosting::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffAniData_Ghosting::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffAniGhosting::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinChar* pSkinChar, const D3DXMATRIX& matCurPos )
{
	...
	if ( !m_pSavedBaseSB )	return hr;
	if ( !m_pEffectBaseSB )	return hr;
...
/*ABL system, Juver, 2017/05/29 */
HRESULT	DxEffAniGhosting::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxAttBone* pAttBone, const D3DXMATRIX& matCurPos )
{
	HRESULT hr = S_OK;
	D3DXVECTOR3 vNormalUp;
	D3DXVECTOR3 vNormalDown;
	...
	if( !m_listGhosting.empty() )
	{
		DxSkinAniControl* pSkinAniControl = pAttBone;			// 이렇게 써도 되는지.. ㅡ,.ㅡ;
		...
		m_pSavedBaseSB->Capture();
		{
			...
			for ( ; iter!=m_listGhosting.end(); ++iter )
			{
				...
				sCharSetting.pmtrlPiece			= pAttBone->m_PartArray[PIECE_HEAD].m_pMaterialPiece;
				sCharSetting.pmtrlSpecular		= NULL;
				sCharSetting.bWorldIdentity		= FALSE;
				sCharSetting.emRDOP				= CTOP_BASE;
				...
				{
				case TRUE:
					if( !pAttBone->m_PartArray[PIECE_HEAD].m_pSkinMesh )	break;
					if( !pAttBone->m_PartArray[PIECE_HEAD].m_pmcMesh )		break;
				...
				for ( int i=1; i<PIECE_REV01; i++ )
				{
					if ( !pAttBone->m_PartArray[i].m_pSkinMesh )	continue;
					if ( !pAttBone->m_PartArray[i].m_pmcMesh )		continue;

					CHARSETTING sCharSetting;
					sCharSetting.pMeshContainerBase = pAttBone->m_PartArray[i].m_pmcMesh;
					sCharSetting.pmtrlPiece			= pAttBone->m_PartArray[i].m_pMaterialPiece;
					sCharSetting.pmtrlSpecular		= NULL;
					sCharSetting.bWorldIdentity		= FALSE;
					sCharSetting.emRDOP				= CTOP_BASE;

					pAttBone->m_PartArray[i].m_pSkinMesh->SetDrawState( TRUE, FALSE, FALSE, FALSE );
					pAttBone->m_PartArray[i].m_pSkinMesh->DrawMeshContainer( pd3dDevice, sCharSetting );
				}
			}
		}
		m_pSavedBaseSB->Apply();
...
/*ABL system, Juver, 2017/05/29 */
HRESULT	DxEffAniGhosting::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxAttBoneRender* pAttBone, const D3DXMATRIX& matCurPos )
{
	HRESULT hr = S_OK;
	D3DXVECTOR3 vNormalUp;
	D3DXVECTOR3 vNormalDown;
...
/*vehicle system, Juver, 2017/08/03 */
HRESULT	DxEffAniGhosting::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxVehicle* pVehicle, const D3DXMATRIX& matCurPos )
{
	HRESULT hr = S_OK;
	D3DXVECTOR3 vNormalUp;
	D3DXVECTOR3 vNormalDown;
	...
	if( !m_listGhosting.empty() )
	{
		DxSkinAniControl* pSkinAniControl = pVehicle;			// 이렇게 써도 되는지.. ㅡ,.ㅡ;
		...
		m_pSavedBaseSB->Capture();
		{
			...
			for ( ; iter!=m_listGhosting.end(); ++iter )
			{
				...
				sCharSetting.pmtrlPiece			= pVehicle->m_PartArray[PIECE_HEAD].m_pMaterialPiece;
				sCharSetting.pmtrlSpecular		= NULL;
				sCharSetting.bWorldIdentity		= FALSE;
				sCharSetting.emRDOP				= CTOP_BASE;
				...
				{
				case TRUE:
					if( !pVehicle->m_PartArray[PIECE_HEAD].m_pSkinMesh )	break;
					if( !pVehicle->m_PartArray[PIECE_HEAD].m_pmcMesh )		break;
				...
				for ( int i=1; i<PIECE_REV01; i++ )
				{
					if ( !pVehicle->m_PartArray[i].m_pSkinMesh )	continue;
					if ( !pVehicle->m_PartArray[i].m_pmcMesh )		continue;

					CHARSETTING sCharSetting;
					sCharSetting.pMeshContainerBase = pVehicle->m_PartArray[i].m_pmcMesh;
					sCharSetting.pmtrlPiece			= pVehicle->m_PartArray[i].m_pMaterialPiece;
					sCharSetting.pmtrlSpecular		= NULL;
					sCharSetting.bWorldIdentity		= FALSE;
					sCharSetting.emRDOP				= CTOP_BASE;

					pVehicle->m_PartArray[i].m_pSkinMesh->SetDrawState( TRUE, FALSE, FALSE, FALSE );
					pVehicle->m_PartArray[i].m_pSkinMesh->DrawMeshContainer( pd3dDevice, sCharSetting );
				}
			}
		}
		m_pSavedBaseSB->Apply();
