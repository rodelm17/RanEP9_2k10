...
BOOL DxEffSelfEffect::Import ( EMPIECECHAR _emType, GLEFFCHAR_VEC &vecEFFECT )
{
...
BOOL DxEffSelfEffect::LoadFile ( const char* szFile )
{
	...
	{
		if( strlen( szFile ) > 0 )
			CDebugSet::ErrorFile( "DxEffSelfEffect::LoadFile", szFile );
...
BOOL DxEffSelfEffectMan::PutEffect ( DxSkinChar *pSkinChar, const char* szEffFile )
{
...
BOOL DxEffSelfEffectMan::PutEffect ( DxAttBone *pAttBone, const char* szEffFile )
{
...
BOOL DxEffSelfEffectMan::PutEffect ( DxAttBoneRender *pAttBone, const char* szEffFile )
{
	if ( !pAttBone )	return FALSE;
	if ( !szEffFile || szEffFile[0]==NULL )	return E_FAIL;
...
BOOL DxEffSelfEffectMan::PutEffect ( DxVehicle *pVehicle, const char* szEffFile )
{
