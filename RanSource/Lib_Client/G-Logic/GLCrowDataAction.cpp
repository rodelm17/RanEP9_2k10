...
SCROWACTION& SCROWACTION::operator= ( const SCROWACTION &value )
{
	...
	m_strCurrencyShop		= value.m_strCurrencyShop;
	m_strContributionShop	= value.m_strContributionShop;
...
bool SCROWACTION::LOAD201 ( basestream &SFile )
{
	DWORD dwDATA(0);
	SFile >> m_dwActFlag;
	SFile >> dwDATA; m_emMoveType = ( EMMOVETYPE ) dwDATA;
...
bool SCROWACTION::LOAD202 ( basestream &SFile )
{
	DWORD dwDATA(0);
	SFile >> m_dwActFlag;
	SFile >> dwDATA; m_emMoveType = ( EMMOVETYPE ) dwDATA;
...
VOID SCROWACTION::SaveCsvHead ( std::fstream &SFile )
{
	...
	SFile << "strCurrencyShop" << ",";
	SFile << "strContributionShop" << ",";
}
...
VOID SCROWACTION::SaveCsv ( std::fstream &SFile )
{
	...
	STRUTIL::OutputStrCsv( SFile, m_strCurrencyShop );
	STRUTIL::OutputStrCsv( SFile, m_strContributionShop );
}
...
VOID SCROWACTION::LoadCsv ( CStringArray &StrArray, int& iCsvCur )
{
	...
	STRUTIL::InputStrCsv( StrArray[ iCsvCur++ ], m_strCurrencyShop ); 
	STRUTIL::InputStrCsv( StrArray[ iCsvCur++ ], m_strContributionShop ); 
}
