...
DxSurfaceTex::DxSurfaceTex() :
	...
	m_pShadowSuf(NULL),
	m_pShadowZBuf(NULL),
	m_pShadowTex2048(NULL),
	m_pShadowSuf2048(NULL),
	m_pShadowZBuf2048(NULL),
	m_pGlowTex(NULL),
	m_pGlowSuf(NULL),
	m_pWaveTex(NULL),
	m_pWaveSuf(NULL),
...
HRESULT DxSurfaceTex::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	//shadow 2048
	hr = pd3dDevice->CreateTexture( 2048, 2048, 1, D3DUSAGE_RENDERTARGET, m_cColorFormat, D3DPOOL_DEFAULT, &m_pShadowTex2048, NULL );
	if( FAILED(hr) )	goto _RETURN;
	hr = m_pShadowTex2048->GetSurfaceLevel( 0, &m_pShadowSuf2048 );
	if( FAILED(hr) )	goto _RETURN;
	hr = pd3dDevice->CreateDepthStencilSurface( 2048, 2048, m_cZFormat, D3DMULTISAMPLE_NONE, 0, TRUE, &m_pShadowZBuf2048, NULL );
	if( FAILED(hr) )	goto _RETURN;
	...
	//temp surface 2048
	hr = pd3dDevice->CreateTexture( 2048, 2048, 1, D3DUSAGE_RENDERTARGET, m_cColorFormat, D3DPOOL_DEFAULT, &m_pTempTex2048, NULL );
	if( FAILED(hr) )	goto _RETURN; 
	hr = m_pTempTex2048->GetSurfaceLevel( 0, &m_pTempSuf2048 );
	if( FAILED(hr) )	goto _RETURN;
...
HRESULT DxSurfaceTex::InvalidateDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	SAFE_RELEASE(m_pShadowTex2048);
	SAFE_RELEASE(m_pShadowSuf2048);
	SAFE_RELEASE(m_pShadowZBuf2048);
	...
	SAFE_RELEASE(m_pTempTex2048);
	SAFE_RELEASE(m_pTempSuf2048);
