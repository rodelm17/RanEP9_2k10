...
#include "../Lib_Engine/DxEffect/Char/DxEffSelfEffect.h" 
...
BEGIN_MESSAGE_MAP(CEditorSkinPieceView, CView)
	...
	ON_COMMAND(ID_FILE_OPEN_RAN2, &CEditorSkinPieceView::OnFileOpenRan2)
END_MESSAGE_MAP()
...
{
	CFileDialog dlgInput( true, NULL, DxSkinPieceContainer::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterPiece, this );
...
void CEditorSkinPieceView::OnFileSave()
{
	if ( m_pPiece )
	{
		CString strsavefile = DxSkinPieceContainer::GetInstance().GetPath();
		strsavefile += m_pPiece->m_szFileName;
		CFileDialog dlgInput( false, NULL, strsavefile.GetString(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterPiece, this );
		dlgInput.m_ofn.lpstrInitialDir = DxSkinPieceContainer::GetInstance().GetPath();
		if ( dlgInput.DoModal() != IDOK ) return;
...
void CEditorSkinPieceView::OnFileSaveeffskin()
{
	...
	CFileDialog dlgInput( false, NULL, DxEffSelfEffectMan::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterEffSkin, this );
...
void CEditorSkinPieceView::OnFileOpenRan2()
{
	CFileDialog dlgInput( true, NULL, DxSkinPieceContainer::GetInstance().GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, RFILE_DIALOG_FILTERS::strFilterPieceCPS, this );
	dlgInput.m_ofn.lpstrInitialDir = DxSkinPieceContainer::GetInstance().GetPath();
	if ( dlgInput.DoModal() != IDOK ) return;
	...
	m_pPiece = new DxSkinPiece;
	m_pPiece->InitDeviceObjects( m_pd3dDevice );
	HRESULT hr = m_pPiece->R2LoadPiece ( m_strFileName.c_str(), m_pd3dDevice, false/*, false*/ );
	if ( FAILED( hr ) )
	{
		SAFE_DELETE( m_pPiece );
		CDebugSet::MsgBox( "Error Loading Piece file :%s", m_strFileName.c_str() );
