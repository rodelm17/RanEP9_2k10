...
char		DxEffCharGhosting::NAME[MAX_PATH]	= "DxEffCharGhosting";
...
HRESULT	DxEffCharGhosting::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if ( dwVer == 0x0100 )
	{
		...
		m_Property.m_AniMType	= AN_SKILL_A;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharGhosting::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharGhosting::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharGhosting::R2LoadFile", SFile.GetFileName(), dwVer );
