...
std::string DxSkinMesh9::GetSkinFilePath( const char* szFileName, BOOL &bEncrypted )
{
	CFileFindTree* pFileTree = DxSkinMeshMan::GetInstance().GetFileTree();
	if ( !pFileTree )	return "";
...
HRESULT DxSkinMesh9::OnCreateSkin ( IDirect3DDevice9* pd3dDevice, const char* szSkinFile, const char *szSkeletonFile, BOOL bThread )
{
	...
	std::string strFileFind = GetSkinFilePath( m_szName, bEncrypted );
	if ( strFileFind.size() <= 0 )
	{
		CDebugSet::ErrorFile( "DxSkinMesh9::OnCreateSkin", m_szName );
		return E_FAIL;
	}
	...
	if ( bEncrypted )
	{
		...
		if ( cXFile.LoadFileDec( strFileFind.c_str() ) )
		{
			...
			V_RETURN( D3DXLoadMeshHierarchyFromXInMemory ( pData, nSize, D3DXMESH_MANAGED, pd3dDevice, &Alloc, NULL, &m_pFrameRoot, &m_pAnimController ) );
		}
	}
	else
	{
		SAFE_RELEASE( m_pAnimController );
		V_RETURN( D3DXLoadMeshHierarchyFromXA ( strFileFind.c_str(), D3DXMESH_MANAGED, pd3dDevice, &Alloc, NULL, &m_pFrameRoot, &m_pAnimController ) );
	}
...
HRESULT DxSkinMesh9::CalculateBoundingBox ( LPDIRECT3DDEVICEQ pd3dDevice, SMeshContainer* pmcMesh, D3DXVECTOR3 &vMax, D3DXVECTOR3 &vMin )
{
	...
	for ( DWORD iBone = 0; iBone < cBones; ++iBone )
	{
		D3DXMatrixMultiply
		(
			&DxSkinMesh9_NORMAL::g_pBoneMatrices[iBone],	// output
			&pmcMesh->pBoneOffsetMatrices[iBone], 
			pmcMesh->ppBoneMatrixPtrs[iBone]
		);
	}

	hr = pmcMesh->CalculateBoundingBox ( pd3dDevice, DxSkinMesh9_NORMAL::g_pBoneMatrices, vMax, vMin );
