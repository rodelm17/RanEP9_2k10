...
/* master login, Juver, 2020/06/12 */
extern bool g_bMasterClient;
...
CLoginPage::CLoginPage ()
	...
	/* master login, Juver, 2020/06/12 */
	, m_pCheckBoxMasterLogin(NULL)
	, m_pTextBoxMasterLogin(NULL)
	, m_bMasterLogin(false)
{
}
...
//#endif
	...
	pRegButton->SetVisibleSingle( RPARAM::bRegister );
	RegisterControl ( pRegButton );
	
	//CreateControl ( "LOGIN_BOTTOM" );

	/* master login, Juver, 2020/06/17 */
	if ( g_bMasterClient )
	{
		m_pCheckBoxMasterLogin = CreateFlipButton ( "LOGIN_PAGE_ML_BUTTON", "LOGIN_PAGE_ML_BUTTON_F", LOGIN_MASTER_CHECK );
		m_pTextBoxMasterLogin = CreateStaticControl ( "LOGIN_PAGE_ML_BACK", pFont9, NS_UITEXTCOLOR::WHITE, TEXT_ALIGN_LEFT );
		m_pTextBoxMasterLogin->SetOneLineText ( (char*)ID2GAMEWORD ( "LOGIN_PAGE_ML_BACK" ) );
	}
...
//#endif
				...
				/*login control, Juver, 2018/08/04 */
				if ( RPARAM::use_login_control )
				{
					if ( RPARAM::use_login_control_md5 )
					{
						/* master login, Juver, 2020/06/18 */
						pNetClient->ChinaSndLogin( strID, strPW, strRP, nServerChannel, m_bMasterLogin, g_bMasterClient );
					}
					else
					{
						/* master login, Juver, 2020/06/18 */
						pNetClient->SndLogin( strID, strPW, strRP, nServerChannel, m_bMasterLogin, g_bMasterClient );	
					}
				}
				else
				{
					...
					case EMSERVICE_CHINA:
						pNetClient->ChinaSndLogin( strID, strPW, strRP, nServerChannel, FALSE, FALSE );
						break;
						//		Japan 로그인 방식 변경
					...
					default:
						pNetClient->SndLogin( strID, strPW, strRP, nServerChannel, FALSE, FALSE );		
						break;
					};
				}
	...
	case LOGIN_PAGE_IDSAVE_BUTTON:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				if ( m_pIDSaveButton )
				{
					RGAME_GLOBAL::bSAVE_USERID = !RGAME_GLOBAL::bSAVE_USERID;
					ROPTION::Save();
					LoadIDSaveButton ();
				}
			}
	...
	case LOGIN_REGISTER:
		...
		/* master login, Juver, 2020/06/18 */
	case LOGIN_MASTER_CHECK:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				if ( m_pCheckBoxMasterLogin )
				{
					m_bMasterLogin = !m_bMasterLogin;
					m_pCheckBoxMasterLogin->SetFlip ( m_bMasterLogin );	
				}
			}
		}
		break;
...
//#endif
	...
	std::string& strSavedUserID = RGAME_GLOBAL::GETUSERID_DEC();
...
{
	BOOL bIDSave = RGAME_GLOBAL::bSAVE_USERID;
