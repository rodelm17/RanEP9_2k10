...
VOID CTextObject::DrawText( D3DCOLOR dwColor, FLOAT fX, FLOAT fY, FLOAT fZoom )
{
	m_pTexture->SetTexture();
	...
	fX += floorf( (FLOAT)CTextUtil::m_iSpaceSizeHalf * fZoom );
	fY -= floorf( (FLOAT)CTextUtil::m_iSpaceSizeHalf * fZoom );

	FLOAT fLeft = fX;

	m_pPart->DrawText( dwColor, fLeft, fY, fZoom );
