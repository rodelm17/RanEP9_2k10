...
CBasicInfoView::CBasicInfoView () :
	...
	m_pCP ( NULL ),
	m_pCPText ( NULL ),
	m_wCPBACK_NOW ( USHRT_MAX ),
	m_wCPBACK_MAX ( USHRT_MAX ),
...
void CBasicInfoView::SetCP ( WORD wNOW, WORD wMAX )
{
	...
	if ( m_wCPBACK_NOW != wNOW || m_wCPBACK_MAX != wMAX )
	{
		m_wCPBACK_NOW = wNOW;
		m_wCPBACK_MAX = wMAX;
	...
	case HP_TEXT:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine ( strText, NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
