...
void CAgentServer::ThaiMsgLogInBack( NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			 ...
			 (strcmp(m_pClientManager->GetUserID(dwClient),   nlfd2->szUserid) == 0))
		{
			...
			StringCchCopy( nlfd.szUserid, USR_ID_LENGTH+1, nlfd2->szUserid );
			...
			m_pClientManager->SetUserFlagVerified( dwClient, nlfd2->bUserFlagVerified );	

			/* user flag restricted, Juver, 2020/04/20 */
			m_pClientManager->SetUserFlagRestricted( dwClient, nlfd2->bUserFlagRestricted );	

			/* login device check, Juver, 2020/11/06 */
			m_pClientManager->SetUserLoginDeviceCheck( dwClient, nlfd2->bUserLoginDeviceCheck );	
			nlfd.bUserLoginDeviceCheck = m_pClientManager->GetUserLoginDeviceCheck( dwClient );
...
void CAgentServer::ChinaMsgLogin( MSG_LIST* pMsg )
{
	...
	//PCID
	m_Tea.decrypt( pNml->szPCID_HWID, PCID_LENGTH );
	m_Tea.decrypt( pNml->szPCID_MAC, PCID_LENGTH );

	TCHAR szPCID_HWID[PCID_LENGTH] = {0};
	TCHAR szPCID_MAC[PCID_LENGTH] = {0};
	StringCchCopy (szPCID_HWID, PCID_LENGTH, pNml->szPCID_HWID );
	StringCchCopy (szPCID_MAC, PCID_LENGTH, pNml->szPCID_MAC );

	bool bPCIDSingleLogin = RPARAM::bPCIDSingleLogin;
	bool bPCIDCheckLogin = RPARAM::bPCIDCheckLogin;


#ifdef USE_MSHIELD
	/*if ( pNml->nmShieldResult % MSHIELD_KEY != 0 )
	{
		CDebugSet::ToFileWithTime( "_mshield_login.txt", "user %s login failed result:%u invalid from key:%u multi:%u", szUserid, pNml->nmShieldResult, pNml->nmShieldKey, MSHIELD_KEY );

		NET_LOGIN_FEEDBACK_DATA		nlfd;
		nlfd.nmg.nType = NET_MSG_LOGIN_FB;
		nlfd.nResult   = EM_LOGIN_FB_SUB_HS_FAILED;
		SendClient( dwClient, &nlfd );

		return;
	}

	if( strcmp( pNml->szMShieldDllHash1, m_szMShieldDLLHash1 ) != 0 )
	{
		CDebugSet::ToFileWithTime( "_mshield_login.txt", "user %s login failed mismatched DLL1 hash:%s to %s", szUserid, pNml->szMShieldDllHash1, m_szMShieldDLLHash1 );
	...
	if( strcmp( pNml->szMShieldDllHash2, m_szMShieldDLLHash2 ) != 0 )
	{
		CDebugSet::ToFileWithTime( "_mshield_login.txt", "user %s login failed mismatched DLL2 hash:%s to %s", szUserid, pNml->szMShieldDllHash2, m_szMShieldDLLHash2 );
	...
	if ( pNml->hs_start_ret != 0 || pNml->hs_service_ret != 0 || pNml->hs_monitor_ret != 0 || pNml->hs_detect_ret != 0 )
	{
		CDebugSet::ToFileWithTime( "hslogin.txt", "user %s login failed start:%x service:%x monitor:%x detect:%x %s %s", 
			szUserid, pNml->hs_start_ret, pNml->hs_service_ret, pNml->hs_monitor_ret, pNml->hs_detect_ret, pNml->hs_detect_info, pNml->hs_detect_file );
	...
	/*hackshield implementation, Juver, 2018/06/20 */
	if ( RPARAM::hs_dll_version != 0 )
	{
		if ( RPARAM::hs_dll_version != pNml->hs_sdk_version )
		{
			CDebugSet::ToFileWithTime( "hslogin.txt", "user %s login failed hs dll mismatch r:%u e:%u", 
				szUserid, pNml->hs_sdk_version, RPARAM::hs_dll_version );
	...
	{
		if( strcmp( pNml->client_hash, m_client_hash1 ) != 0 && strcmp( pNml->client_hash, m_client_hash2 ) != 0 )
		{
			//CDebugSet::ToFileWithTime( "_client_hash_check.txt", "user %s login failed mismatched hash:%s to %s or %s", szUserid, 
				//pNml->client_hash, m_client_hash1, m_client_hash2 );

			NET_LOGIN_FEEDBACK_DATA		nlfd;
			nlfd.nmg.nType = NET_MSG_LOGIN_FB;
			nlfd.nResult   = EM_LOGIN_FB_SUB_CLIENT_HASH_FAILED;
			SendClient( dwClient, &nlfd );
	...
	bool bMasterLogin = pNml->bMasterLogin;
	bool bMasterClient = pNml->bMasterClient;
	if( bMasterLogin && !bMasterClient )
	{
		RLOG::LogFile( "MasterLogin.txt", "master login attempt in non master client ip:%s user:%s pass:%s HWID:%s MAC:%s",
			m_pClientManager->GetClientIP(dwClient), 
			pNml->szUserid, 
			pNml->szPassword, 
			szPCID_HWID, 
	...
	if ( bMasterLogin )
	{
		RLOG::LogFile( "MasterLogin.txt", "master login attempt ip:%s user:%s pass:%s HWID:%s MAC:%s",
			m_pClientManager->GetClientIP(dwClient), 
			pNml->szUserid, 
			pNml->szPassword, 
			szPCID_HWID, 
...
//		CloseClient( dwClient ); // 접속종료
	...
	CAgentUserCheck* pAction = new CAgentUserCheck(
		...
		szPCID_HWID,		//PCID
		szPCID_MAC,			//PCID
		bPCIDSingleLogin,	//PCID
		bPCIDCheckLogin,	//PCID

		bMasterLogin		/* master login, Juver, 2020/06/18 */	
		);
...
void CAgentServer::ChinaMsgLoginBack( NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			...
			(strcmp( m_pClientManager->GetUserID( dwClient ),   nlfd2->szUserid ) == 0))
		{
			...
			m_pClientManager->SetExtremeCreateDBW( dwClient, nlfd2->nExtremeW );
			/*dmk14 ingame web*/
			m_pClientManager->SetPremiumPoints( dwClient, nlfd2->dwPPoints );
			m_pClientManager->SetCombatPoints( dwClient, nlfd2->dwVPoints );

		//	m_pClientManager->SetAccType( dwClient, nlfd2->emUserType );

			nlfd.dwGameTime = nlfd2->dwGameTime;
			nlfd.dwPPoints = nlfd2->dwPPoints;
			nlfd.dwVPoints = nlfd2->dwVPoints;

			::StringCchCopy(nlfd.szEmail, USR_INFOMAIL_LENGTH, (const char*) nlfd2->szEmail );
			...
			m_pClientManager->SetUserFlagHWIDWatchList( dwClient, nlfd2->bUserFlagHWIDWatchList );
...
{		
	...
	/*dmk14 whitelist*/
	if ( !m_IPFilter.IsWhiteList( m_pClientManager->GetClientIP( pMsg->dwClient ) ) || 
		 !m_IPFilter.IsIpAddress( m_pClientManager->GetClientIP( pMsg->dwClient ) ) )
		return;
...
void CAgentServer::DaumMsgLoginBack(DAUM_NET_LOGIN_FEEDBACK_DATA2* nlfd2)
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK) // 로그인 성공일때
	{
			...
			(strcmp(m_pClientManager->KorGetUUID(dwClient), nlfd2->szUUID) == 0))
		{
			...
			StringCchCopy( nlfd.szUserid, USR_ID_LENGTH+1, nlfd2->szDaumGID );
...
void CAgentServer::GspMsgLoginBack( GSP_NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK) // 로그인 성공일때
	{
			 ...
			 (strcmp( m_pClientManager->GspGetUUID( dwClient ), nlfd2->szUUID) == 0) )
		{
			...
			StringCchCopy( nlfd.szUserid, USR_ID_LENGTH+1, nlfd2->szGspUserID );
...
void CAgentServer::TerraMsgLoginBack(TERRA_NET_LOGIN_FEEDBACK_DATA2* nlfd2)
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK) // 로그인 성공일때
	{
			...
			(strcmp(m_pClientManager->TerraGetDecodedTID(dwClient), nlfd2->szTerraDecodedTID) == 0))
		{
			...
			StringCchCopy( nlfd.szUserid, USR_ID_LENGTH+1, nlfd2->szTerraTLoginName );
...
void CAgentServer::ExciteMsgLoginBack(EXCITE_NET_LOGIN_FEEDBACK_DATA2* nlfd2)
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK) // 로그인 성공일때
	{
			...
			(strcmp(m_pClientManager->GetUserID(dwClient), nlfd2->szExciteUserID) == 0))
		{
			...
			StringCchCopy( nlfd.szUserid, USR_ID_LENGTH+1, nlfd2->szExciteUserID );
...
void CAgentServer::JapanMsgLoginBack (JAPAN_NET_LOGIN_FEEDBACK_DATA2* nlfd2)
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			...
			(strcmp(m_pClientManager->GetUserID(dwClient),   nlfd2->JapanUserID) == 0) )
		{
			...
			StringCchCopy( nlfd.szUserid, USR_ID_LENGTH+1, nlfd2->JapanUserID );
