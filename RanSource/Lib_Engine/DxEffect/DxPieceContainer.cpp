...
void DxPieceEdit::CleanUp()
{
	...
	TextureManager::ReleaseTexture( "_dummy_lightmap.dds", m_pTextureLightMapDay );
}
...
void DxPieceEdit::RenderFrame( const LPDIRECT3DDEVICEQ pd3dDevice, CLIPVOLUME &sCV )
{
	...
	{
		m_sMaterial.m_pLightMapDay = m_pTextureLightMapDay;
		m_pStaticAniFrame->Render_EDIT( pd3dDevice, m_matLocal, m_sMaterial );
	}
	...
	if( m_pPieceEffEDIT )
	{
		...
		m_pPieceEffEDIT->Render( pd3dDevice, m_matLocal, NULL, NULL, NULL, NULL );
...
void DxPieceEdit::FrameMove( const float fTime, const float fElapsedTime, const D3DXMATRIX& matWorld )
{
...
void DxPieceEdit::Render( const LPDIRECT3DDEVICEQ pd3dDevice, const CLIPVOLUME &sCV, const D3DXMATRIX& matWorld )
{
	...
	{
		m_sMaterial.m_pLightMapDay = m_pTextureLightMapDay;
		m_pStaticAniFrame->Render( pd3dDevice, matWorld, m_fAniTime, m_pAniTexEff->GetMapTexEff(), m_sMaterial );
		m_pStaticAniFrame->Render_Alpha( pd3dDevice, matWorld, m_pAniTexEff->GetMapTexEff(), m_sMaterial );
	}
...
void DxPieceEdit::SavePIE( const char* szName )
{
	...
	SFile << (DWORD)VERSION_PIE;

	SFile.BeginBlock();
	{
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_PIE_V1 );
...
BOOL DxPieceEdit::LOADPIE_0100 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vMin;
	SFile >> m_vMax;
	SFile.ReadBuffer( &m_matLocal, sizeof(D3DXMATRIX) );
...
BOOL DxPieceEdit::LOADPIE_0102 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vMin;
	SFile >> m_vMax;
	SFile >> m_vStaticMin;
...
BOOL DxPieceEdit::LOADPIE_0103 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0104 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0107 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0108 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0109 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0200 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0201 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0202 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> m_vStaticMin;
	SFile >> m_vStaticMax;
	SFile >> m_vMin;
...
BOOL DxPieceEdit::LOADPIE_0203 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> unknown_var1;
	SFile >> unknown_var2;
	SFile >> unknown_var3;
...
BOOL DxPieceEdit::LOADPIE_0204 ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	SFile >> unknown_var1;
	SFile >> unknown_var2;
	SFile >> unknown_var3;
...
HRESULT DxPieceEdit::LoadPIE( LPDIRECT3DDEVICEQ pd3dDevice, const char* szName )
{
	...
	if ( !bOpened )
	{
		CDebugSet::ErrorFile( "DxPieceEdit::LoadPIE", szName );
		return E_FAIL;
	}
	...
	if ( dwVer >= VERSION_ENCODE )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_MAP_PIE_VER_1 );

	if ( dwVer >= VERSION_ENCODE2 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_MAP_PIE_VER_2 );

	if ( dwVer >= VERSION_ENCODE3 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_MAP_PIE_VER_3 );

	if ( dwVer >= ENCODE_VER_BYTE_CRYPT_2021_PIE_V1 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_PIE_V1 );

	if( dwVer==VERSION_PIE )
	{
		LOADPIE_0204( SFile, pd3dDevice );
	}
	else if( dwVer==0x0203 )
	{
		LOADPIE_0203( SFile, pd3dDevice );
	}
	else if( dwVer==0x0202 )
	{
		LOADPIE_0202( SFile, pd3dDevice );
	}
	else if( dwVer==0x0201 )
	{
		LOADPIE_0201( SFile, pd3dDevice );
	}
	else if( dwVer==0x0200 )
	{
		LOADPIE_0200( SFile, pd3dDevice );
	}
#ifdef _DEBUG
	else if( dwVer==0x0109 && g_bWORLD_TOOL )
	{
		LOADPIE_0109( SFile, pd3dDevice );
	}
	else if( dwVer==0x0108 && g_bWORLD_TOOL )
	{
		LOADPIE_0108( SFile, pd3dDevice );
	}
	else if( dwVer==0x0107 && g_bWORLD_TOOL )
	{
		LOADPIE_0107( SFile, pd3dDevice );
	}
#endif
	else if( dwVer==0x0104 || dwVer==0x0105 || dwVer==0x0106 )
	{
		LOADPIE_0104( SFile, pd3dDevice );
	}
	else if( dwVer==0x0103 )
	{
		LOADPIE_0103( SFile, pd3dDevice );
	}
	else if( dwVer==0x0102 )
	{
		LOADPIE_0102( SFile, pd3dDevice );
	}
	else if( dwVer==0x0100 || dwVer==0x0101 )
	{
		LOADPIE_0100( SFile, pd3dDevice );
	}
	else
	{
		CDebugSet::ErrorVersion( "DxPieceEdit::LoadPIE", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
	...
	if ( bExist )
	{
		...
		else
		{
			...
			{
				//new DxPieceTexEff with different reading
				DWORD _dwVer;
				DWORD _dwSize;
				SFile >> _dwVer;
				SFile >> _dwSize;
	...
	TextureManager::LoadTexture( "_dummy_lightmap.dds", pd3dDevice, m_pTextureLightMapDay, 0L, 0L, FALSE );
