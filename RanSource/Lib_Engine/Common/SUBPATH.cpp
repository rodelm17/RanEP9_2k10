...
namespace SUBPATH
{
	...
	TCHAR PLAYINFO_ROOT[MAX_PATH]		= _T("\\Ran Online\\PlayInfo\\");
	TCHAR DEBUGINFO_ROOT[MAX_PATH]		= _T("\\Ran Online\\ErrorLog\\");
	TCHAR CAPTURE[MAX_PATH]				= _T("\\Ran Online\\ScreenShot\\");
	TCHAR CHATLOG[MAX_PATH]				= _T("\\Ran Online\\ChatLog\\");
	...
	TCHAR LEVEL_FILE_ROOT[MAX_PATH]		= _T("\\Data\\GLogic\\Level\\");

	TCHAR SOUND_FILE_ROOT[MAX_PATH]		= _T("\\Sounds\\Sfx\\");
	TCHAR BGM_FILE_ROOT[MAX_PATH]		= _T("\\Sounds\\BGM\\");

	TCHAR ENGINE_FILE[MAX_PATH]			= _T("\\Data\\Engine\\");

	/*server setting, Juver, 2018/06/17 */
	TCHAR SERVER_CONFIG_PATH[MAX_PATH]	= _T("\\cfg\\");
};