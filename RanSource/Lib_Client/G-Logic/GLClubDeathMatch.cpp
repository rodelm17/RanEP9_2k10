...
GLClubDeathMatch::GLClubDeathMatch () :
	...
	m_emBattleState(CDM_BATTLE_NOSTART),
	m_dwBattleOrder(0),
	m_dwLastBattleDay(0),
	m_fBattleTimer(0.0f),
	m_fCHECK_TIMER(0.0f),
	...
	if ( nHour == dwOTime )
	{
		...
		if ( nMinute == 50 )
		{
			if ( !m_bNotify10MinHour )
			{
				...
				m_emBattleState = CDM_BATTLE_READY;
...
{
	m_emBattleState = CDM_BATTLE_START;
...
{
	m_emBattleState = CDM_BATTLE_END;
...
void GLClubDeathMatch::CheckExtraClub ( float fElaps )
{
	...
	if ( m_fCHECK_TIMER > m_fCHECK_TIME )
	{
		...
		if ( pLandMan )
		{
			pLandMan->DoGateOutPCAll ( m_dwCLubMapGate, DxLandGate::DEFAULT_GATE_OUT_INDEX );
		}
		m_fCHECK_TIME += 10.0f;
	}
}
...
void GLClubDeathMatchAgentMan::ControlPVP ( bool bStart )
{
    for ( CDM_VEC::size_type i=0; i<m_vecClubDM.size(); ++i )
    {
        GLClubDeathMatch &sClubDM = m_vecClubDM[i];
...
bool GLClubDeathMatchFieldMan::ReadyBattle( DWORD dwID )
{
	...
	pCDM->SetBattle( GLClubDeathMatch::CDM_BATTLE_READY );
...
bool GLClubDeathMatchFieldMan::BeginBattle ( DWORD dwID )
{
	...
	pCDM->SetBattle( GLClubDeathMatch::CDM_BATTLE_START );
...
bool GLClubDeathMatchFieldMan::EndBattle ( DWORD dwID )
{
	...
	pCDM->SetBattle( GLClubDeathMatch::CDM_BATTLE_END_AWARD );
...
void GLClubDeathMatchFieldMan::UpdateCDMRanking( DWORD dwID )
{
	...
	for ( UINT i = 0; i < m_vecCdmRankNew.size(); ++i )
	{
