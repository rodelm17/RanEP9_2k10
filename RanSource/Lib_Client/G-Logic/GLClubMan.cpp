	...
	, m_dwAllianceBattleDraw(0)
{
	...
	{
		m_cStorage[i].SetState ( STORAGE_INVEN_X, CLUB_STORAGE_INVEN_Y );
	}
}
	...
	for ( ; pos!=end; )
	{
		...
		if ( GLCONST_CHAR::bCLUB_BATTLE )
		{
			...
			if ( cCLUB.GetBattleNum() > 0 )
			{
				...
				for ( ; pos!=end; )
				{
					//warning! sClubBattle does invalidate after deletion
					...
					if ( sClubBattle.m_tEndTime < curTime )
					{
						...
						if ( sClubBattleDel.m_sClubBattle.m_bAlliance ) 
						{
							...
							DelAllianceBattle( dwClubID, sClubBattleDel.m_sClubBattle.m_dwCLUBID );
                        }
						else
						{
							cCLUB.DELBATTLECLUB( (*del).first );
							GLCLUB* pCLUB = GetClub( sClubBattleDel.m_sClubBattle.m_dwCLUBID );
							if ( pCLUB ) pCLUB->DELBATTLECLUB( cCLUB.m_dwID );
						...
						cCLUB.m_mapBattleDel[sClubBattleDel.m_sClubBattle.m_dwCLUBID] = sClubBattleDel;

						GLMSG::SNET_CLUB_BATTLE_LAST_KILL_UPDATE_FLD NetMsg;
						NetMsg.dwClubID_A = dwClubID;
						NetMsg.dwClubID_B = sClubBattleDel.m_sClubBattle.m_dwCLUBID;
