...
void CNPCShopWindow::CreateSubControl ()
{
	...
	m_pButtonPurchase->CreateBaseButton ( "NPC_SHOP_BUTTON_PURCHASE", CBasicTextButton::SIZE40, CBasicButton::CLICK_FLIP, "aw" );
	...
	m_pBuyConfirm->SetFlip(RGAME_GLOBAL::bNPCShopConfirm );
	...
	case NPC_SHOP_CONFIRM_BUTTON:
		{
			...
			{
				RGAME_GLOBAL::bNPCShopConfirm = !RGAME_GLOBAL::bNPCShopConfirm;
				m_pBuyConfirm->SetFlip( RGAME_GLOBAL::bNPCShopConfirm );
			}
		}break;
...
void CNPCShopWindow::InitShop( SNATIVEID sidCrow )
{
	...
	//copy data
	m_wShopType = pNpcShop->m_wShopType;
	m_sCrow = pcrow->m_sBasic.sNativeID;

	SNPC_SHOP_TYPE_POINTER_MAP_ITER it_b = pNpcShop->m_mapType.begin();
	SNPC_SHOP_TYPE_POINTER_MAP_ITER it_e = pNpcShop->m_mapType.end();
	for ( ; it_b != it_e; ++it_b )
	{
		const SNPC_SHOP_TYPE& sData = *(*it_b).second;
...
void CNPCShopWindow::SelectType( DWORD dwType )
{
	...
	if ( pcrow )
	{
		...
		if ( pNpcShop )
		{
			SNPC_SHOP_ITEM_POINTER_MAP_ITER it_b = pNpcShop->m_mapItem.begin();
			SNPC_SHOP_ITEM_POINTER_MAP_ITER it_e = pNpcShop->m_mapItem.end();
			for ( ; it_b != it_e; ++it_b )
			{
				const SNPC_SHOP_ITEM& sData = *(*it_b).second;
