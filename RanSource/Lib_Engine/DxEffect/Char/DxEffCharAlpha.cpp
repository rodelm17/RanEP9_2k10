...
char		DxEffCharAlpha::NAME[MAX_PATH]	= "DxEffCharAlpha";
...
HRESULT	DxEffCharAlpha::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		EFFCHAR_PROPERTY_ALPHA_102	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_ALPHA_102) );

		m_dwFlags	= sProp.m_dwFlags;
		m_fHeight	= sProp.m_fHeight;
		m_fPlayTime = sProp.m_fPlayTime;
		m_fWaitTime	= sProp.m_fWaitTime;
		m_fAlpha	= sProp.m_fAlpha;
	...
	{
		EFFCHAR_PROPERTY_ALPHA_101	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_ALPHA_101) );
	...
	{
		EFFCHAR_PROPERTY_ALPHA_100	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_ALPHA_100) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharAlpha::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharAlpha::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharAlpha::R2LoadFile", SFile.GetFileName(), dwVer );
		//	Note : 버전이 틀릴 경우에는 파일에 쓰여진 DATA영역을 건더 띄는 작업을 진행.
		//
		SFile.SetOffSet ( SFile.GetfTell()+dwSize );
