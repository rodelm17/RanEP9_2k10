	...
	case QBOX_ON_OFF_BUTTON:
		{
			...
			{
				CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine( ID2GAMEINTEXT( "QBOX_BUTTON_MESSAGE" ), NS_UITEXTCOLOR::WHITE  );				
			}
	...
	case QBOX_GAUGE:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine( strPercent.GetString(), NS_UITEXTCOLOR::WHITE );				
			}
		}break;
	}
}
