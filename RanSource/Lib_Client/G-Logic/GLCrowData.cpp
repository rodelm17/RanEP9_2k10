...
DWORD SGENITEM::SELECTSPECID_SKILL ( float fGEN_RATE, bool& bSPECID, SNATIVEID& sidSkill )	//	발생 군.
{
	bool bSCALE = GLCONST_CHAR::fITEM_DROP_LOWER_SCALE!=1.0f || fGEN_RATE!=1.0f;

	float fNowRate = seqrandom::getpercent();
	DWORD dwSpecID = EMSPEC_NULL;
	float fRateL = m_sItemSpecSkill[0].fRateL;
	for ( int i=0; i<EMGSPEC; ++i )
	{
		float fRate = (m_sItemSpecSkill[i].fRateH-m_sItemSpecSkill[i].fRateL);
		if ( bSCALE && fRate < 2.0f )
		{
			fRate = fRate * (GLCONST_CHAR::fITEM_DROP_LOWER_SCALE*fGEN_RATE);
		}
		...
		if ( (fRateL<=fNowRate) && fNowRate<(fRateL+fRate) )
		{
			dwSpecID = m_sItemSpecSkill[i].dwSPECID;
			bSPECID = m_sItemSpecSkill[i].bSPECID;
			sidSkill = m_sItemSpecSkill[i].sidSkill;
			break;
		}
...
SNATIVEID SQTGENITEM::SELECTITEM () // Question 아이템 발생 by 경대
{
	...
	for ( int i=0; i<m_nItemNum; ++i )
	{
		...
		SITEM* pitem = GLItemMan::GetInstance().GetItem( m_sItem[i].m_sGenItemID );
		if ( !pitem )	continue;
		if ( !pitem->sGenerateOp.bGenEnable )	continue;
...
SCROWDATA& SCROWDATA::Assign ( SCROWDATA &Data )
{
	...
	m_mapMobDropSpecID = Data.m_mapMobDropSpecID;		// [MobDB][Construction] by edwin 20190804 ***** [Edition 1] 
	for ( int i=0; i<EMMAXATTACK; ++i )
		m_sCrowAttack[i] = Data.m_sCrowAttack[i];

	/* multiple crow gen, Juver, 2018/02/21 */
	m_vecGenItem = Data.m_vecGenItem;
	m_vecQtGenItem = Data.m_vecQtGenItem;

	m_sNpcTalkDlg = Data.m_sNpcTalkDlg;

	m_sCrowWarden = Data.m_sCrowWarden;
...
HRESULT SCROWDATA::LoadFile ( basestream &SFile, BOOL bServer, bool bPastLoad )
{
	...
	BOOL bcrow_attack_vector = FALSE;
	std::vector<SCROWATTACK>vec_crow_attack;

	/*block crow push/pull, Juver, 2019/02/14 */
	bool bcrow_check_push_pull = false;
		...
		case FILE_SBASIC:
			{
				...
				if( bPastLoad )
				{
					...
					else if ( dwDataVer==0x0205)
					{
						SCROWBASIC_205 sCrowBasicOld;
						GASSERT(sizeof(sCrowBasicOld)==dwDataSize);
						SFile.ReadBuffer ( &sCrowBasicOld, sizeof(sCrowBasicOld) );

						m_sBasic = sCrowBasicOld;
					}
					else if ( dwDataVer==0x0206)
					{
						SCROWBASIC_206 sCrowBasicOld;
						GASSERT(sizeof(sCrowBasicOld)==dwDataSize);
						SFile.ReadBuffer ( &sCrowBasicOld, sizeof(sCrowBasicOld) );

						m_sBasic = sCrowBasicOld;
					}
					else if ( dwDataVer==0x0207)
					{
						SCROWBASIC_207 sCrowBasicOld;
						GASSERT(sizeof(sCrowBasicOld)==dwDataSize);
						SFile.ReadBuffer ( &sCrowBasicOld, sizeof(sCrowBasicOld) );

						m_sBasic = sCrowBasicOld;
					}
					else if ( dwDataVer==0x0208)
					{
						SCROWBASIC_208 sCrowBasicOld;
						GASSERT(sizeof(sCrowBasicOld)==dwDataSize);
						SFile.ReadBuffer ( &sCrowBasicOld, sizeof(sCrowBasicOld) );

						m_sBasic = sCrowBasicOld;
					}
					else if ( dwDataVer==0x0209)
					{
						SCROWBASIC_209 sCrowBasicOld;
						GASSERT(sizeof(sCrowBasicOld)==dwDataSize);
						SFile.ReadBuffer ( &sCrowBasicOld, sizeof(sCrowBasicOld) );

						m_sBasic = sCrowBasicOld;
					}
					else if ( dwDataVer==0x0210)
					{
						SCROWBASIC_210 sCrowBasicOld;
						GASSERT(sizeof(sCrowBasicOld)==dwDataSize);
						SFile.ReadBuffer ( &sCrowBasicOld, sizeof(sCrowBasicOld) );
		...
		case FILE_SACTION:
			{
				...
				{
					m_sAction.LOAD201( SFile );
				}
				else if( dwDataVer == 0x0202 )
				{
					m_sAction.LOAD202( SFile );
				}
		...
		case FILE_SATTACK_EX:
			{
				...
				if ( dwDataVer == 0x0100 )
				{
					...
					SCROWATTACK sCrowAttack[3];
					sCrowAttack[0] = sOLDATTACK[0];
					sCrowAttack[1] = sOLDATTACK[1];
					sCrowAttack[2] = sOLDATTACK[2];

					vec_crow_attack.push_back( sCrowAttack[0] );
					vec_crow_attack.push_back( sCrowAttack[1] );
					vec_crow_attack.push_back( sCrowAttack[2] );

					bcrow_attack_vector = TRUE;
				}
				else if ( dwDataVer == 0x0101 )
				{
				...
				{
					SCROWATTACK sCrowAttack[3];
					sCrowAttack[0].LOAD_0105 ( SFile );
					sCrowAttack[1].LOAD_0105 ( SFile );
					sCrowAttack[2].LOAD_0105 ( SFile );
				...
				else if ( dwDataVer==0x0107 )
				{
					...
					for( DWORD i=0; i<dwNum; ++ i )
					{
						...
						vec_crow_attack.push_back( sCrowAttack );
					}
				...
				else if ( dwDataVer==0x0108 )
				{
					...
					DWORD dwIndex(0);
					SFile >> dwNum;
					SFile >> dwIndex;	//dont know what for
				...
				{
					for( int i=0; i<EMMAXATTACK; ++i )
						m_sCrowAttack[i].LOAD_0200 ( SFile );
				}
		...
		case FILE_SWARDEN:
			{
				...
				if ( dwDataVer==0x0100 )
				{
					m_sCrowWarden.LOAD_0100(SFile);
				}
				else if ( dwDataVer==SCROWWARDEN::VERSION )
				{
					m_sCrowWarden.LOAD(SFile);
				}
				else
				{
					CDebugSet::ErrorVersion( "SCROWDATA::SCROWWARDEN", dwDataVer );
					SFile.SetOffSet ( SFile.GetfTell() + dwDataSize );
				}
			}
			break;
	...
	if ( bcrowfix )
	{
		if ( m_sBasic.m_emCrow == CROW_ZONE_NAME || m_sBasic.m_emCrow == CROW_GATE_NAME )
		{
			SCROWBASIC sbasic_copy = m_sBasic;
			m_sBasic = SCROWBASIC();
			m_sBasic.sNativeID = sbasic_copy.sNativeID;
			m_sBasic.m_emCrow = sbasic_copy.m_emCrow;
			StringCchCopy( m_sBasic.m_szName, CHAR_SZNAME, sbasic_copy.m_szName );
			
			m_sAction = SCROWACTION();
			m_sAction.m_strSkinObj = "trans.chf";
			...
			m_sAction.m_vecPatternList.push_back( sPattern );

			m_sGenerate = SCROWGEN();
		}
	}

	if ( bcrow_attack_vector )
	{
		for( int i=0; i<EMMAXATTACK; ++i )
			m_sCrowAttack[i] = SCROWATTACK();

		for ( int i=0; i<(int)vec_crow_attack.size(); ++i )
		{
			SCROWATTACK &attack = vec_crow_attack[i];

			if ( i < EMMAXATTACK )
				m_sCrowAttack[i] = attack;
		}
	}

	/*block crow push/pull, Juver, 2019/02/14 */
	if ( bcrow_check_push_pull )
	{
		if ( m_sAction.m_dwActFlag&EMCROWACT_POSHOLD )
		{
			m_sBasic.m_bBlockPushPull = true;
		}
	}
	...
	if ( bServer )
	{
		/* multiple crow gen, Juver, 2018/02/21 */
		m_vecGenItem.clear();
		m_vecQtGenItem.clear();

		/* multiple crow gen, Juver, 2018/02/21 */
		for ( int i=0; i<SCROWGEN::MAX_GEN_ITEM_ARRAY; ++i )
		{
			if ( !m_sGenerate.m_strGenItem[i].empty() )
			{
				SGENITEM sgen_item;
				BOOL bload = sgen_item.LOADFILE ( m_sGenerate.m_strGenItem[i].c_str() );
				if ( bload )
					m_vecGenItem.push_back( sgen_item );
			}
		}
		...
		for ( int i=0; i<SCROWGEN::MAX_QGEN_ITEM_ARRAY; ++i )
		{
			...
			//
			if ( !m_sGenerate.m_strQtGenItem[i].empty() )
			{
				SQTGENITEM sqgen_item;
				BOOL bload = sqgen_item.LOADFILE ( m_sGenerate.m_strQtGenItem[i].c_str() );
				if ( bload )
					m_vecQtGenItem.push_back( sqgen_item );
			}
		}
...
std::string SCROWDATA::GetCrowSale( DWORD dwChannel ) 
{
	if ( dwChannel >= SCROWACTION::SALENUM )	return "";
	return m_sAction.m_strSaleFile[dwChannel];
}
...
HRESULT SCROWDATA::SaveFile ( CSerialFile &SFile )
{
	...
	SFile << static_cast<DWORD> ( FILE_SBASIC );
	{
		...
		CString cstrName;
		cstrName.Format( _T("CN_%03d_%03d"), m_sBasic.sNativeID.wMainID, m_sBasic.sNativeID.wSubID ); // by 경대
		StringCchCopy ( m_sBasic.m_szName, CHAR_SZNAME, cstrName.GetString() );
	...
	SFile << static_cast<DWORD> ( FILE_SWARDEN );
	{
		SFile << static_cast<DWORD> ( SCROWWARDEN::VERSION );
		SFile << static_cast<DWORD> ( sizeof(SCROWWARDEN) );
		m_sCrowWarden.SAVE(SFile);
	}
...
{
	const char *szName = GLStringTable::GetInstance().GetString ( m_sBasic.m_szName, GLStringTable::CROW );
	if ( !szName )	return m_sBasic.m_szName;
...
VOID SCROWDATA::LoadCsv ( CStringArray &StrArray )
{
	...
	m_sCrowWarden.LoadCsv( StrArray, iCsvCur );
}
...
GLCrowDataMan::~GLCrowDataMan ()
{
	...
	CurrencyShopCleanUp();
	NPCShopCleanUp();
	NPCItemExchangeCleanUp();
	NPCContributionShopCleanUp();
}
...
BOOL GLCrowDataMan::InsertCrowData(WORD wMID, WORD wSID, PCROWDATA pCrowData, bool binner)
{
    ...
    /* Boss Spawn Viewer, Review000 */
    ///////////////////////////////////////////////
    //--find and insert m_mapMobDropSpecID
    for (int i = 0; i < SCROWGEN::MAX_GEN_ITEM_ARRAY; ++i)
    {
        if (!pCrowData->m_sGenerate.m_strGenItem[i].empty())
        {
            std::string strSearch = pCrowData->m_sGenerate.m_strGenItem[i];
            ...
            DWORD dwID;
            std::string token;
            SGENITEM sGenItem[10];

            if ((pos = strSearch.find(strDelimiter)) == std::string::npos)
            {
                sGenItem[0].LOADFILE(pCrowData->m_sGenerate.m_strGenItem[i].c_str());

                for (int _b = 0; _b < SGENITEM::EMGSPEC; _b++)
                {
                    dwID = sGenItem[0].m_sItemSpec[_b].dwSPECID;
                    if (dwID < 1) continue;

                    if (pCrowData->m_mapMobDropSpecID.count(dwID) == 0)
                    {
                        pCrowData->m_mapMobDropSpecID.insert(std::make_pair(dwID, dwID));
                    }
                }
            }
            else
            {
                while ((pos = strSearch.find(strDelimiter)) != std::string::npos)
                {
                    ...
                    sGenItem[wIndex].LOADFILE(token.c_str());
                    strSearch.erase(0, pos + strDelimiter.length());

                    for (int _b = 0; _b < SGENITEM::EMGSPEC; _b++)
                    {
                        dwID = sGenItem[wIndex].m_sItemSpec[_b].dwSPECID;
                        if (dwID < 1) continue;
...
//	///////////////////////////////////////////////
//	//--find and insert m_mapMobDropSpecID
//	if ( !pCrowData->m_sGenerate.m_strGenItem.empty() )
//	{
//		std::string strSearch = pCrowData->m_sGenerate.m_strGenItem.c_str();
//		std::string strDelimiter = ";";
//
//		size_t pos = 0;
//		WORD wIndex = 0;
...
//		if( (pos = strSearch.find(strDelimiter)) == std::string::npos ) 
//		{
//			sGenItem[0].LOADFILE ( pCrowData->m_sGenerate.m_strGenItem.c_str() );
//			
//			for( int _b = 0; _b < SGENITEM::EMGSPEC; _b++ )
//			{
//				dwID = sGenItem[0].m_sItemSpec[_b].dwSPECID;
//				if ( dwID < 1 ) continue;
//				
//				if( pCrowData->m_mapMobDropSpecID.count( dwID ) == 0 )	//--check duplicate
//				{
//					pCrowData->m_mapMobDropSpecID.insert( std::make_pair( dwID, dwID ));	//--serverside do not insert this map
//				}
//			}
//		}
//		else
...
HRESULT GLCrowDataMan::LoadFile ( const char* szFile, BOOL bServer, bool bPastLoad )
{
	...
	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_CROW_VER_1 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT2 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_CROW_VER_2 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT3 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_CROW_VER_3 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT_2021_CROW_V1 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_CROW_V1 );
	...
	for ( DWORD i = 0; i < dwNumCrow; i++ )
	{
		...
		AdditionalLoad( &sCrowData );
		InsertCrowData ( sCrowData.m_sBasic.sNativeID, &sCrowData, true );
	}
...
HRESULT	GLCrowDataMan::SyncStringTable()
{
	...
	for ( int j = 0; j < MAX_CROW_SID; j++ )
	{
		...
		if( m_ppPCROWDATA[i][j]->m_sBasic.m_szName[0] != _T('\0') )
		{
			strBuffer1.Format( _T("CN_%03d_%03d\t%s\r\n"), i, j, m_ppPCROWDATA[i][j]->m_sBasic.m_szName );
