...
BEGIN_MESSAGE_MAP(CMiniAWnd, CWnd)
	ON_WM_ACTIVATEAPP()
	ON_WM_SETCURSOR()
	ON_WM_KEYDOWN()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CMiniAWnd message handlers

BOOL CMiniAWnd::Create() 
{
	CMiniAApp *pRanClientApp = (CMiniAApp *) AfxGetApp();
...
void CMiniAWnd::OnActivateApp(BOOL bActive, DWORD hTask)
{
	CWnd::OnActivateApp(bActive, hTask);
}

BOOL CMiniAWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if ( ROPTION::bGameCursor )
	{
...
void CMiniAWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	if ( ROPTION::bGameCursor )
	{
		HCURSOR hCursl = CCursor::GetInstance().GetCurCursor();
...
void CMiniAWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	switch(nChar)
	{
	case VK_ESCAPE:
...
LRESULT CMiniAWnd::OnNetNotify(WPARAM wParam, LPARAM lParam)
{
