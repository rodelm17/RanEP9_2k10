...
int CBasicTextButton::AddText( CString strTemp, const D3DCOLOR& TextColor /* = NS_UITEXTUTIL::ENABLE */ )
{
	if ( m_pButtonText == NULL )	
		return -1; 

	return m_pButtonText->AddText ( strTemp, TextColor );
}

void CBasicTextButton::AddString( int nIndex, CString strTemp, const D3DCOLOR& TextColor /* = NS_UITEXTUTIL::ENABLE */ )
{
	if ( m_pButtonText == NULL )	
		return;

	m_pButtonText->AddString( nIndex, strTemp, TextColor );
}

void CBasicTextButton::ClearText()
{
	if ( m_pButtonText == NULL )	
		return;

	m_pButtonText->ClearText();
}