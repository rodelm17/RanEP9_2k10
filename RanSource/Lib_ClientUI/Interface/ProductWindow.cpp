				...
				if ( UIMSG_PRODUCT_WINDOW_LIST_SELECT & dwMsg )
				{
					if( m_pList )
					{
						if ( m_pSelectProduct )	
						{
							...
							const ITEM_MIX* pitem_mix = GLItemMixMan::GetInstance().GetProduct( m_pList->GetSelectData() );
							if ( pitem_mix )
							{
								SITEM* pitem_result = GLItemMan::GetInstance().GetItem( pitem_mix->sResultItem.sNID );
								if ( pitem_result )
								{
									m_pSelectProduct->AddText( pitem_result->GetName(), COMMENT::ITEMCOLOR[pitem_result->sBasicOp.emLevel] );
								}
							}

							if ( m_pSelectProduct->empty() )
