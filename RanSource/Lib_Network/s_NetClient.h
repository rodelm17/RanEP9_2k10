...
#include "./s_CBit.h"
#include "./minTea.h"
#include "../Lib_Engine/G-Logic/DxMsgProcessor.h"
...
public:
	...
	int		ConnectLoginServer( SLOGIN_ADDRESS* pLogin ); /*login port, Juver, 2017/11/16 */
	...
	*/
	...
	int UserAccountPageInit( bool bInit, int nChannel );

	/* login device check, Juver, 2020/11/06 */
	int UserAccountPageLoginDeviceCheckSet( const TCHAR* szPassword, bool bSet );
	int UserLoginDeviceCheckVerifyInit( bool bInit, int nChannel );
	int UserLoginDeviceCheckVerifyRequest( const TCHAR* szUserID, const TCHAR* szPass, int nChannel=0 );

	/*dmk14 ingame web*/
	int SndForgotPass( 
				const char* szUserID,
		        const char* szUserPassword );
	int SndForgotChangePass(
				const char* szUserPassword,
				const char* szUserPassword2 );
	int SndAccountChangePass(
				const char* szUserPassword,
				const char* szUserPincode,
				const char* szUserNewPass );
	int SndAccountChangePin(
				const char* szUserPassword,
				const char* szUserPincode,
				const char* szUserNewPin  );
	int SndAccountResetPin(
				const char* szUserPassword,
				const char* szUserMail );
	int SndAccountChangeMail(
				const char* szUserPassword,
				const char* szUserPin,
				const char* szUserMailOld,
				const char* szUserMailNew );
	int SndAccountTopUp( const char* szCode,const char* szPin );
	int ReqGameTimeConvert();
