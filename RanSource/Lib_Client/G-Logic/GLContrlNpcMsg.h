...
namespace GLMSG
{
	...
	struct SNETPC_REQ_SELFBUFF_BA_ONLY
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETPC_REQ_SELFBUFF_BA_ONLY () 
			: dwNpcID(0)
			, dwGlobalID(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_SELFBUFF_BA_ONLY);
			nmg.nType = NET_MSG_GCTRL_SELFBUFF_BA_ONLY;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*self buff via NPC, DevArt22, 2023/03/18 */
	struct SNETPC_REQ_SELFBUFF_BA_ONLY_FB
	{
		NET_MSG_GENERIC		nmg;

		EM_SELFBUFF_FB		emFB;

		SNETPC_REQ_SELFBUFF_BA_ONLY_FB () 
			: emFB(EM_SELFBUFF_FAIL)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_SELFBUFF_BA_ONLY_FB);
			nmg.nType = NET_MSG_GCTRL_SELFBUFF_BA_ONLY_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*self buff via NPC, DevArt22, 2023/03/18 */
	struct SNETPC_REQ_SELFBUFF_FULL
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETPC_REQ_SELFBUFF_FULL () 
			: dwNpcID(0)
			, dwGlobalID(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_SELFBUFF_FULL);
			nmg.nType = NET_MSG_GCTRL_SELFBUFF_FULL;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*self buff via NPC, DevArt22, 2023/03/18 */
	struct SNETPC_REQ_SELFBUFF_FULL_FB
	{
		NET_MSG_GENERIC		nmg;

		EM_SELFBUFF_FB		emFB;

		SNETPC_REQ_SELFBUFF_FULL_FB () 
			: emFB(EM_SELFBUFF_FAIL)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_SELFBUFF_FULL_FB);
			nmg.nType = NET_MSG_GCTRL_SELFBUFF_FULL_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*self buff via NPC, DevArt22, 2023/03/18 */
	struct SNETPC_REQ_SELFBUFF_FULL_NO_BA
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETPC_REQ_SELFBUFF_FULL_NO_BA () 
			: dwNpcID(0)
			, dwGlobalID(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_SELFBUFF_FULL_NO_BA);
			nmg.nType = NET_MSG_GCTRL_SELFBUFF_FULL_NO_BA;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*self buff via NPC, DevArt22, 2023/03/18 */
	struct SNETPC_REQ_SELFBUFF_FULL_NO_BA_FB
	{
		NET_MSG_GENERIC		nmg;

		EM_SELFBUFF_FB		emFB;

		SNETPC_REQ_SELFBUFF_FULL_NO_BA_FB () 
			: emFB(EM_SELFBUFF_FAIL)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_SELFBUFF_FULL_NO_BA_FB);
			nmg.nType = NET_MSG_GCTRL_SELFBUFF_FULL_NO_BA_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
