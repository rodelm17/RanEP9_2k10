...
CEP7ChaosMachineInvenWindow::CEP7ChaosMachineInvenWindow()
	: m_pPage( NULL )
...
CMultiModeButton* CEP7ChaosMachineInvenWindow::CreateRadioButton( char* szControl, char* szText, const UIGUID& cID /*= NO_ID*/ )
{
	...
	pControl->CreateTextBox( "EP7_CHAOS_MACHINE_INVENTORY_WINDOW_RADIO_BUTTON_TEXT", 9, TEXT_ALIGN_CENTER_X | TEXT_ALIGN_CENTER_Y, _DEFAULT_FONT_SHADOW_FLAG );
...
void CEP7ChaosMachineInvenWindow::CreateSubControl()
{
	m_pPage = new CEP7ChaosMachineInvenPage;
	m_pPage->CreateSub( this, "EP7_CHAOS_MACHINE_INVENTORY_PAGE", UI_FLAG_YSIZE, CHAOS_MACHINE_INVEN_PAGE );
	m_pPage->CreateSubControl();
	m_pPage->SetOneViewSlot ( EP7_CHAOS_MACHINE_INVEN_DEFAULT_VIEW );
	RegisterControl( m_pPage );

	CUIControl* pControl = CreateControl( "EP7_CHAOS_MACHINE_INVENTORY_MONEY_BACK" );
	pControl->SetAlignFlag( UI_FLAG_BOTTOM );

	CBasicButton* pButton = CreateFlipButton( "EP7_CHAOS_MACHINE_INVENTORY_MONEY_BUTTON", "EP7_CHAOS_MACHINE_INVENTORY_MONEY_BUTTON_F", CHAOS_MACHINE_INVEN_MONEY_BUTTON, CBasicButton::CLICK_FLIP );
	pButton->CreateMouseOver( "EP7_CHAOS_MACHINE_INVENTORY_MONEY_BUTTON_F" );
	pButton->SetUseDynamic( TRUE );
	pButton->SetAlignFlag( UI_FLAG_BOTTOM );

	CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
	m_pMoneyTextBox = CreateStaticControl( "EP7_CHAOS_MACHINE_INVENTORY_MONEYTEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
	m_pMoneyTextBox->SetAlignFlag( UI_FLAG_BOTTOM );

#if defined( BUILD_EP7 )
	{
		CUIControl* pControl = CreateControl ( "EP7_CHAOS_MACHINE_INVENTORY_GAME_POINTS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pGamePointsTextBox = CreateStaticControl ( "EP7_CHAOS_MACHINE_INVENTORY_GAME_POINTS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pGamePointsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "EP7_CHAOS_MACHINE_INVENTORY_GAME_POINTS_BUTTON", "EP7_CHAOS_MACHINE_INVENTORY_GAME_POINTS_BUTTON_F", CHAOS_MACHINE_INVEN_GAME_POINTS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "EP7_CHAOS_MACHINE_INVENTORY_GAME_POINTS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}

	{
		CUIControl* pControl = CreateControl ( "EP7_CHAOS_MACHINE_INVENTORY_WAR_CHIPS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pWarChipsTextBox = CreateStaticControl ( "EP7_CHAOS_MACHINE_INVENTORY_WAR_CHIPS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pWarChipsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "EP7_CHAOS_MACHINE_INVENTORY_WAR_CHIPS_BUTTON", "EP7_CHAOS_MACHINE_INVENTORY_WAR_CHIPS_BUTTON_F", CHAOS_MACHINE_INVEN_WAR_CHIPS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "EP7_CHAOS_MACHINE_INVENTORY_WAR_CHIPS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}
#endif
	...
	{
		"EP7_CHAOS_MACHINE_INVENTORY_WINDOW_RADIO_BUTTON_1",
		"EP7_CHAOS_MACHINE_INVENTORY_WINDOW_RADIO_BUTTON_2",
	};
...
void CEP7ChaosMachineInvenWindow::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CEP7ChaosMachineInvenWindow::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	switch( ControlID )
	{
	case CHAOS_MACHINE_INVEN_PAGE:
		{
...
HRESULT CEP7ChaosMachineInvenWindow::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void CEP7ChaosMachineInvenWindow::SetVisibleSingle ( BOOL bVisible )
{
...
void CEP7ChaosMachineInvenWindow::SetOneViewSlot( const INT& nONE_VIEW_SLOT )
{
}

void CEP7ChaosMachineInvenWindow::OpenInvenPage ( int nPage )
{
	if ( nPage < 0 )	return;
