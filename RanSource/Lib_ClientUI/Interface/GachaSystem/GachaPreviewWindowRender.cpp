...
CGachaPreviewWindowRender::CGachaPreviewWindowRender ()
	: m_pSkinChar( NULL )
	, m_bReady( false )
	, m_fRenderDelay( 0.0f )
...
void CGachaPreviewWindowRender::CreateSubControl ()
{
}

void CGachaPreviewWindowRender::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
}

HRESULT CGachaPreviewWindowRender::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT CGachaPreviewWindowRender::DeleteDeviceObjects ()
{
	HRESULT hr = S_OK;
...
HRESULT CGachaPreviewWindowRender::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr = S_OK;
...
HRESULT CGachaPreviewWindowRender::FrameMove( LPDIRECT3DDEVICEQ pd3dDevice, float fTime, float fElapsedTime )
{
	HRESULT hr = S_OK;
...
void CGachaPreviewWindowRender::CreateModel( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void CGachaPreviewWindowRender::RenderModel( LPDIRECT3DDEVICEQ pd3dDevice )
{
	try
	{
		if ( m_pSkinChar )
...
void CGachaPreviewWindowRender::UpdateSuit( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void CGachaPreviewWindowRender::ClearRender()
{
	if ( m_pSkinChar )
	{
		m_bReady = false;
...
void CGachaPreviewWindowRender::ChangeStance()
{
	if ( !m_pSkinChar )	return;
	m_pSkinChar->SELECTANI( AN_PLACID, AN_SUB_NONE );
