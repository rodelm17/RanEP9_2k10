...
struct EFFCHAR_PROPERTY_BLUR_100
{
	BOOL			m_bUse;
	BOOL			m_bBezier;
	BOOL			m_bAlphaVel;
...
struct EFFCHAR_PROPERTY_BLUR_101
{
...
struct EFFCHAR_PROPERTY_BLUR_102
{
	BOOL			m_bBezier;
	int				m_nBlend;
	DWORD			m_dwTraceIndexUP;			//	추적 포인터 인덱스.
...
struct EFFCHAR_PROPERTY_BLUR_103
{
	BOOL			m_bBezier;
	int				m_nBlend;
...
struct EFFCHAR_PROPERTY_BLUR_104
{
	DWORD			m_dwFlag;
...
struct EFFCHAR_PROPERTY_BLUR_105
{
	...
	EFFCHAR_PROPERTY_BLUR_105()
	{
		SecureZeroMemory( m_szTraceUP, sizeof(char)*STRACE_NSIZE );
		SecureZeroMemory( m_szTraceDOWN, sizeof(char)*STRACE_NSIZE );
		SecureZeroMemory( m_szTexture, sizeof(char)*TEXTURE_LENGTH );
	};
};
