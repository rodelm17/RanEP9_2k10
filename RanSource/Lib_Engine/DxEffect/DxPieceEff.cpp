...
void DxLandEff::Render( LPDIRECT3DDEVICEQ pd3dDevice, const D3DXMATRIX& matWorld, DxStaticMesh* pStaticMesh, DxFrameMesh* pFrameMesh, 
					   DxMultiStaticMesh* pMultiStaticMesh, DxMultiStaticMaterialMesh* pMultiStaticMaterialMesh )
{
	if( !m_pSingleGroup )	return;
	...
	if( m_bGroudPos )
	{
		if( !m_bCheckColl )	// Collision은 1번만 한다.
		{
			...
			if( pMultiStaticMesh )
			{
				...
				// Note : 충돌 되었을 때만.
				//			bFrontColl = TRUE
				if( pMultiStaticMesh->IsCollisionLine( vStart, vEnd, m_vCollPos, vNor, TRUE ) )	m_bCheckColl = TRUE;
			}

			if( pMultiStaticMaterialMesh )
			{
				D3DXVECTOR3 vStart, vEnd, vNor;
				vStart.x = m_pSingleGroup->m_matWorld._41;
				vStart.y = m_pSingleGroup->m_matWorld._42;
				...
				// Note : 충돌 되었을 때만.
				//			bFrontColl = TRUE
				if( pMultiStaticMaterialMesh->IsCollisionLine( vStart, vEnd, m_vCollPos, vNor, TRUE ) )	m_bCheckColl = TRUE;
			}
...
const DWORD DxPieceEff::VERSION2	= 0x0103;
						...
						DxMultiStaticMesh* pMultiStaticMesh, DxMultiStaticMaterialMesh* pMultiStaticMaterialMesh )
{
	...
	for( ; iter!=m_vecLandEff.end(); ++iter )
	{
		(*iter)->Render( pd3dDevice, matWorld, pStaticMesh, pFrameMesh, pMultiStaticMesh, pMultiStaticMaterialMesh );
	}
...
void DxPieceEff::Load( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, DxAnimationMan* pAnimation )
{
	...
	{
		CDebugSet::ErrorVersion( "DxPieceEff::Load", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet( dwCur+dwBufferSize );
	}
}
