...
BOOL CPageItemShop::OnInitDialog()
{
	...
	m_List.InsertColumn( 2, "Item Name", LVCFMT_LEFT, ( 36* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 3, "Stock", LVCFMT_LEFT, ( 7* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 4, "Price", LVCFMT_LEFT, ( 7* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 5, "Type", LVCFMT_LEFT, ( 7* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 6, "Currency", LVCFMT_LEFT, ( 30* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
	m_List.InsertColumn( 7, "IsSale", LVCFMT_LEFT, ( 30* ( rectCtrl.right - rectCtrl.left ) ) / 100 );
...
void CPageItemShop::ShowResults()
{
	...
	for( int i=0; i<(int)m_vecResult.size(); ++i )
	{
		...
		m_List.SetItemText( i, 3, _HLIB::cstring_dword( sDATA.wItemStock ).GetString() );
		m_List.SetItemText( i, 4, _HLIB::cstring_dword( sDATA.dwPrice ).GetString() );
		m_List.SetItemText( i, 5, _HLIB::cstring_dword( sDATA.wType ).GetString() );
		m_List.SetItemText( i, 6, _HLIB::cstring_dword( sDATA.wCurrency ).GetString() );
		m_List.SetItemText( i, 7, _HLIB::cstring_dword( sDATA.wIsSale ).GetString() );
	...
	else
	{
			...
			++ iteritem )
		{
			...
			case EMSHOPITEM2_SEARCH_STOCK:
				{
					strFIND.Format( "%d", sDATA.wItemStock );
				}break;
			case EMSHOPITEM2_SEARCH_PRICE:
				{
					strFIND.Format( "%d", sDATA.dwPrice );
				}break;
			};
