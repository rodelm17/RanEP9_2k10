...
CSerialMemory::CSerialMemory ( const char *_szZipFileName, const char *_szFileName, EMRCRYPT_BYTE_TYPE emCryptType ) 
	: m_pBuffer(NULL)
	, m_nBufSize(0)
	, m_dwOffSet(0)
{
	...
	OpenFile ( _szZipFileName, _szFileName, emCryptType );
}
...
void CSerialMemory::SetEncodeType ( EMRCRYPT_BYTE_TYPE emCryptType )
{
	m_emCrypt_Type = emCryptType; 
}
...
BOOL CSerialMemory::OpenFile (	const char *_szZipFileName, const char *_szFileName, EMRCRYPT_BYTE_TYPE emCryptType, BOOL bPack )
{
	//GASSERT(_szFileName);
	//GASSERT(_szZipFileName);
...
inline size_t CSerialMemory::read ( void* pbuffer, DWORD dwSize )
{
	...
	if ( m_emCrypt_Type != EMRCRYPT_BYTE_TYPE_NONE )
	{
		m_cCrypt.SetType(m_emCrypt_Type);
		m_cCrypt.Decrypt((BYTE*)pbuffer, dwSize);
	}
...
BOOL CSerialMemory::operator >> ( D3DXVECTOR2 &Value )
{
	GASSERT(m_OpenType==FOT_READ);
	int Num = 0;
...
BOOL CSerialMemory::operator >> ( D3DXVECTOR3 &Value )
{
	GASSERT(m_OpenType==FOT_READ);
	int Num = 0;
...
BOOL CSerialMemory::operator >> ( D3DXVECTOR4 &Value )
{
	GASSERT(m_OpenType==FOT_READ);
	int Num = 0;
...
BOOL CSerialMemory::operator >> ( D3DXMATRIX &Value )
{
	GASSERT(m_OpenType==FOT_READ);
	int Num = 0;
