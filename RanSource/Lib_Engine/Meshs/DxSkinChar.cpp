...
void DxSkinChar::SetCharData ( DxSkinCharData* pCharData, LPDIRECT3DDEVICEQ pd3dDevice, bool bCharacterData /*= FALSE*/ )
{
	...
	m_bForceSafeSlot = pCharData->m_bForceSafeSlot;
...
HRESULT DxSkinChar::FrameMove ( const float fTime, const float fElapsedTime, const BOOL bContinue, const BOOL bFreeze, const BOOL bAttackMode )
{
	...
	BOOL attack_mode = bAttackMode;
	if ( m_bForceSafeSlot )	attack_mode = FALSE;
	...
	{
		if ( m_pAttBone[i] )
			m_pAttBone[i]->FrameMove( fTime, fElapsedTime, bContinue, bFreeze, attack_mode );
	}

	/*vehicle system, Juver, 2017/08/04 */
	if ( m_pVehicle )
		m_pVehicle->FrameMove( fTime, fElapsedTime, bContinue, bFreeze, attack_mode );

	m_PartArray[PIECE_RHAND].SetAttackMode ( attack_mode );
	m_PartArray[PIECE_LHAND].SetAttackMode ( attack_mode );
