...
int CNetClient::MsgConnectClientToField(NET_MSG_GENERIC* nmg)
{
	...
	if ( RPARAM::client_hash_check_use )
	{
		...
		::StringCchCopy(MsgIdentity.client_hash, CLIENT_HASH_SIZE, g_hashObj.DoHash().GetString() );
	}

#ifdef USE_MSHIELD
	MsgIdentity.nmShieldKey = MShieldGlobal::nKey;
	MsgIdentity.nmShieldResult = MShieldGlobal::nResult;
	...
	if ( !strAppPath.IsEmpty() )
	{
		...
		::StringCchCopy(MsgIdentity.szMShieldDllHash1, CLIENT_HASH_SIZE, g_hashObj1.DoHash().GetString() );
		::StringCchCopy(MsgIdentity.szMShieldDllHash2, CLIENT_HASH_SIZE, g_hashObj2.DoHash().GetString() );
	}
#endif
...
void CNetClient::MsgGameSvrInfoEnd(NET_MSG_GENERIC* nmg)
{
	...
	/*dmk14 whitelist*/
	int nSize;
	NET_REQ_GAME_SVR_WHILELIST msg;
	::StringCchCopy(msg.szEnCrypt, ENCRYPT_KEY+1, m_szEncryptKey);
	m_Tea.encrypt (msg.szEnCrypt, ENCRYPT_KEY+1);
	Send( reinterpret_cast<char *> (&msg), msg.nmg.dwSize );
}
