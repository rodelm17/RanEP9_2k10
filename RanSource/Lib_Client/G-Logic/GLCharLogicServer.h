...
#ifndef GLLOGICCHARSERVER_H_INCLUDED__
#define GLLOGICCHARSERVER_H_INCLUDED__
...
//
struct GLCHARLOGIC_SERVER :public SCHARDATA2
{	
	...
	SPASSIVE_SKILL_DATA_SERVER	m_sSUM_PASSIVE;					//	페시브 가산값.

	int					m_nSUM_PIERCE;					//	관통 정도 합산.
	float				m_fSUM_TARGET_RANGE;			//	공격 가능거리 합산.

	/*skill range spec logic, Juver, 2017/06/06 */
	float				m_fSUM_ATTACK_RANGE;
	float				m_fSUM_APPLY_RANGE;
...
//	탈것 추가
	...
	SEFFECT_RELEASE_DATA_MAP	m_mapEffectRelease;
	...
	SKILL_DELAY_DATA_MAP		m_mapSkillDelay;
	...
	SKILL_STIGMA_DATA_MAP		m_mapSkillStigma;
	...
	SKILL_ILLUSION				m_sSkillIllusion;
	...
	SPKCOMBO_STATE				m_sPKComboState;

	/*anti shit*/
	float				m_fAttVeloSkill;
	float				m_fAttVeloItem;
	//anti shit skillcut-as
	float				m_bIsSkillProcess;
...
public:
	...
	float GETSUM_TARRANGE () const;
	int GETSUM_PIERCE () const;

	WORD GETBODYRADIUS () const		{ return CHARACTER_BODY_RADIUS; }
...
public:
	...
	BOOL SIMPLE_CHECK_ITEM ( SNATIVEID sNativeID, bool bLevelCheck );
	BOOL SIMPLE_CHECK_ITEM ( SNATIVEID sNativeID, SNATIVEID sDisguiseID, bool bLevelCheck );
...
public:
	...
	{ 
		/*continuous damage skill logic, Juver, 2017/06/10 */	
		CONTINUOUSDAMAGE_DELETE(m_sSKILLFACT[i].sNATIVEID);	

		/* skill effect release, Juver, 2020/04/30 */
		EffectReleaseDelete(m_sSKILLFACT[i].sNATIVEID); 

		/* skill delay addition, Juver, 2020/12/09 */
		SkillDelayDataDelete(m_sSKILLFACT[i].sNATIVEID);

		/* skill illusion, Juver, 2021/01/17 */
		if ( m_sSKILLFACT[i].sNATIVEID ==  m_sSkillIllusion.sSkillID )
...
public:
	/* skill effect release, Juver, 2020/04/30 */
	BOOL EffectReleaseExist( SNATIVEID _sID );
	void EffectReleaseInsert( const SEFFECT_RELEASE_DATA& _sData );
	void EffectReleaseDelete( SNATIVEID _sID );

	/* skill delay addition, Juver, 2020/12/09 */
	BOOL SkillDelayDataExist( SNATIVEID _sID );
	void SkillDelayDataInsert( const SSKILL_DELAY_DATA& _sDATA );
	void SkillDelayDataDelete( SNATIVEID _sID );
...
#endif // GLLOGICCHARSERVER_H_INCLUDED__
