...
BOOL GLLandMan::LoadFile ( const char *szFile, const char *szWLDFile /*= NULL*/ )
{
	...
	if ( pMAP_NODE )
	{
		...
		m_bCPReset				= pMAP_NODE->bCPReset; 
		m_bCPResetDeath			= pMAP_NODE->bCPResetDeath;
		...
		m_bOpenPrivateMarket	= pMAP_NODE->bOpenPrivateMarket; 
		...
		m_bBlockParty			= pMAP_NODE->bBlockParty;

		/*force revive, Juver, 2018/07/09 */
		m_bForceRevive			= pMAP_NODE->bForceRevive;

		/* open pk function, Juver, 2019/12/23 */
		m_bOpenPK				= pMAP_NODE->bOpenPK;

		/* death hide, Juver, 2020/02/02 */
		m_bDeathHide			= pMAP_NODE->bDeathHide;

		/* map entry user verified, Juver, 2020/02/27 */
		m_bUserVerifiedMapEntry	= pMAP_NODE->bUserVerifiedMapEntry;

		/* costume combine map restriction, Juver, 2020/09/11 */
		m_bCostumeCombineRestrictMoveSpeed		= pMAP_NODE->bCostumeCombineRestrictMoveSpeed;
		m_bCostumeCombineRestrictAttackSpeed	= pMAP_NODE->bCostumeCombineRestrictAttackSpeed;

		/* trade restricted, Juver, 2021/06/07 */
		m_bTradeRestricted		= pMAP_NODE->bTradeRestricted;

		m_bAllowHideDead		= pMAP_NODE->bAllowHideDead;

		m_bShowPkMark			= pMAP_NODE->bShowPkMark;

		m_bUseAutoPotion		= pMAP_NODE->bUseAutoPotion;
		m_bAutoPotionManual		= pMAP_NODE->bAutoPotionManual;

		m_bUseAutoPilot			= pMAP_NODE->bUseAutoPilot;

		m_bPKMapCount			= pMAP_NODE->bPKMapCount;

		m_bUseCaptcha			= pMAP_NODE->bUseCaptcha;

		m_bBlockGeneralChat		= pMAP_NODE->bBlockGeneralChat;

		m_bResetSkillDelayOnDie	= pMAP_NODE->bResetSkillDelayOnDie;
	}
