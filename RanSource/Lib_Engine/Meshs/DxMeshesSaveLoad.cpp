...
BOOL	DxMeshes::LoadFile_BETA ( CSerialFile &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dw;
	SFile >> dw; //version
	...
	else
	{
		...
		else
		{
			for ( DWORD i = 0; i < cMaterials; i++ )
			{
				D3DEXMATERIAL material;
				DWORD something;
				SFile.ReadBuffer ( &material, sizeof ( D3DEXMATERIAL ));
				SFile >> something;

				exMaterials[i] = material;
			}
		}
	...
	m_pDxOctreeMesh = new DxOctreeMesh;
	m_pDxOctreeMesh->LoadFile_BETA ( SFile, pd3dDevice );
	...
	{
		pMeshNext = new DxMeshes;
		pMeshNext->LoadFile_BETA ( SFile, pd3dDevice );
	}
