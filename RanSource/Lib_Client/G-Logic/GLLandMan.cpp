	...
	m_bPVPWoeMap(false)	/*woe Arc Development 08-06-2024*/
{
	...
	m_MobSchManClient.SetLandMan( this );
...
HRESULT GLLandMan::SetMap ()
{
	...
	/*dmk14 large map window ep9*/
	m_MobSchManClient.FrameMove( 0.0f, 0.0f );
...
BOOL GLLandMan::DropGenItem ( D3DXVECTOR3 &vPos, const SNATIVEID &sNID, EMITEMGEN emGENTYPE, EMGROUP emGroup, DWORD dwHoldGID )
{
	...
	/* mob item drop direct +, Juver, 2020/10/17 */
	if ( RPARAM::bMobItemDropDirectPlus && emGENTYPE == EMGEN_MOB )
	{
		sITEM_NEW.cDAMAGE = (BYTE)pITEM->sBasicOp.wGradeAttack;
		sITEM_NEW.cDEFENSE = (BYTE)pITEM->sBasicOp.wGradeDefense;
	}
...
DWORD GLLandMan::DropCrow ( GLMobSchedule *pMobSch )
{
	...
	if ( pNewCrow->m_pCrowData->m_sBasic.m_dwNotice& EMCROW_NOTICE_RESPAWN ||
		pNewCrow->m_pCrowData->m_sBasic.m_dwNotice&EMCROW_NOTICE_RESPAWN2 )
	{
		const char *szMAP_NAME = GLGaeaServer::GetInstance().GetMapName ( m_sMapID );
		if ( szMAP_NAME )
		{
			CString strKey;
			CString strTEXT;
			strKey.Format ( "CROW_RESPAWN_%03d_%03d", pNewCrow->m_pCrowData->m_sBasic.sNativeID.wMainID, pNewCrow->m_pCrowData->m_sBasic.sNativeID.wSubID );
			strTEXT.Format ( ID2SERVERTEXT(strKey.GetString()), pNewCrow->m_pCrowData->GetName(), szMAP_NAME );

			GLMSG::SNET_SERVER_GENERALCHAT NetMsg;
			NetMsg.SETTEXT ( strTEXT.GetString() );
			//NetMsg.bUseAdminDisplay = BOOL( pNewCrow->m_pCrowData->m_sBasic.m_dwNotice&EMCROW_NOTICE_RESPAWN2 );
...
//	if ( !bCol )	return UINT_MAX;
	...
	if ( pNewCrow->m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_KNOCK )
	{
		...
		if ( szMAP_NAME )
		{
			...
			// Log the event
			CString strLog;
			strLog.Format("Boss Respawned: [%s], Map: [%s], Boss ID: [%03d_%03d]", 
						pNewCrow->m_pCrowData->GetName(), szMAP_NAME, 
						pNewCrow->m_pCrowData->sNativeID.wMainID, 
						pNewCrow->m_pCrowData->sNativeID.wSubID);
	        
			CDebugSet::ToFileWithTime("BossMobSpawnLogs.txt", strLog);
...
PGLCHAR GLLandMan::FindEnemyChar ( const PGLCROW pFinder )
{
	...
	int nRange = (int) pFinder->m_pCrowData->m_sBasic.m_wViewRange;
	...
	for ( ; pQuadNode; pQuadNode = pQuadNode->pFindNext )
	{
		...
		for ( ; pCur; pCur = pCur->pNext )
		{
			...
			if ( !pChar->IsVisibleDetect( pFinder->m_bSkillShowInvisible, 
										pFinder->m_wSkillShowInvisibleLevel, 
										pFinder->m_wSkillShowInvisibleRange, 
										pFinder->GetPosition() ) )	continue;
	...
	if ( m_GlobPCList.m_dwAmount > 0 || m_bEmulator )
	{
		...
		while ( pCurCrow )
		{
			...
			if ( pCrow->m_bNpcRecall )
			{
				...
				__time64_t tGenTime = pCrow->m_tGenTime +  pCrow->m_pCrowData->m_sBasic.m_dwGenTime;
...
BOOL GLLandMan::IsDeleteInstantMap ( float fElapsedTime )
{
	...
	{
		if ( m_GlobPETList.m_dwAmount == 0 && m_GlobSummonList.m_dwAmount == 0 )
		{
...
BOOL GLLandMan::MoveOutInstantMap( PGLCHAR pChar )
{
	...
	if( !pOutGate )
	{
		...
		if ( !pMapNode )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pOutGate !pMapNode %d %d", 
				pChar->m_dwCharID, 
				pChar->m_szName, 
				sMoveMapId.wMainID, 
				sMoveMapId.wSubID );
		...
		GLLandMan* pInLandMan = NULL;
		DxLandGateMan* pInGateMan = NULL;
		pInLandMan = glGaeaServer.GetByMapID ( pOutGate->GetToMapID( DxLandGate::DEFAULT_GATE_OUT_INDEX ) );
		if ( !pInLandMan )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInLandMan %d %d",
				pChar->m_dwCharID, 
				pChar->m_szName, 
				pOutGate->GetToMapID( DxLandGate::DEFAULT_GATE_OUT_INDEX ).wMainID, 
				pOutGate->GetToMapID( DxLandGate::DEFAULT_GATE_OUT_INDEX ).wSubID );
		...
		if ( !pInGateMan )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInGateMan", 
				pChar->m_dwCharID, 
				pChar->m_szName );
		...
		if ( !pInGate )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInGate %d %d %d", 
				pChar->m_dwCharID, 
				pChar->m_szName, 
				pOutGate->GetToMapID( DxLandGate::DEFAULT_GATE_OUT_INDEX ).wMainID, 
				pOutGate->GetToMapID( DxLandGate::DEFAULT_GATE_OUT_INDEX ).wSubID, 
				pOutGate->GetToGateID( DxLandGate::DEFAULT_GATE_OUT_INDEX ) );
		...
		if ( !pMapNode )
		{
			CDebugSet::ToFileWithTime( "instancemap_moveout.txt", "[%d] %s !pInGate %d %d", 
				pChar->m_dwCharID, 
				pChar->m_szName, 
				sMoveMapId.wMainID, 
				sMoveMapId.wSubID );
		...
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
			...
			else
			{
				if( pChar->GetPartyID() != GetInstantMapHostID() )
				{
					...
					{
						//break loop
						//m_GlobPCList might have deleted or moved order
						break;
					}
				}
			}
			...
			if ( pCHAR )
			{
				...
				if ( sACT.nVAR_HP<0 )
				{
					...
					case SKILL::EMAPPLY_PHY_LONG:
						nVAR_HP = int(sACT.nVAR_HP*GLCONST_CHAR::fPK_POINT_DEC_PHY[ CharClassToClassIndex(pCHAR->m_emClass) ] );
						break;
					case SKILL::EMAPPLY_MAGIC:
						nVAR_HP = int(sACT.nVAR_HP*GLCONST_CHAR::fPK_POINT_DEC_MAGIC[ CharClassToClassIndex(pCHAR->m_emClass) ] );
						break;
					};

					/* skill illusion, Juver, 2021/01/17 */
					if (nVAR_HP == 0)	nVAR_HP = -1;
				}
			}
		}
		else if ( sACT.sID.emCrow==CROW_SUMMON && pREACTOR->GetCrow()==CROW_PC )
		{
			...
			if ( pSummon )
			{
				...
				if ( sACT.nVAR_HP<0 )
				{
					...
					case SKILL::EMAPPLY_PHY_SHORT:
					case SKILL::EMAPPLY_PHY_LONG:
						nVAR_HP = int(sACT.nVAR_HP*GLCONST_CHAR::fPK_POINT_DEC_PHY[ CharClassToClassIndex( pSummon->m_pOwner->m_emClass ) ] );
						break;
					case SKILL::EMAPPLY_MAGIC:
						nVAR_HP = int(sACT.nVAR_HP*GLCONST_CHAR::fPK_POINT_DEC_MAGIC[ CharClassToClassIndex( pSummon->m_pOwner->m_emClass ) ] );
						break;
					};
...
void GLLandMan::DoSkillActEx ( const SSKILLACTEX &sACTEX )
{
	...
	//self and target movement, prioritize effect
	if ( sACTEX.bPositionShiftSwap )
	{
		PositionShiftSwap( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPositionShiftSpeed );
	}
	else
	{
		//self movement, only one can be active at the same time
		if ( sACTEX.bPosisionShiftDash )
		{
			PositionShiftDash( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPositionShiftSpeed );
		}
		else if ( sACTEX.bUseTeleport )
		{
			SkillTeleport( sACTEX.sID, sACTEX.sID_TAR, sACTEX.vTeleportPos );
		}
		else if ( sACTEX.bDash )
		{
			DashAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fDashSpeed, sACTEX.dwActionAnim );
		}
		
		//target movement, only one can be active at the same time

		if ( sACTEX.bPositionShiftPull )
		{
			PositionShiftPull( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPositionShiftSpeed );
		}
		...
		else if ( sACTEX.bCrushBlow )
		{
			// CrushBlowOne
			PushPullAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL );
		}
		else if ( sACTEX.bPushPull || sACTEX.bCrushingBlow )
		{
			//generic push pull and crushing blow
	...
	if ( bCoA )
	{
		...
		if ( bCoB )
		{
			if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
				pREACTOR->ReceiveSkillPush ( vCoB, GLCONST_CHAR::fPUSHPULL_VELO );
		}
	}
}
...
void GLLandMan::PullAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED )
{
	...
	if ( bCoA )
	{
		...
		{
			if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
				pREACTOR->ReceiveSkillPull ( vCoB, _fSPEED );
		}
	}
}

/* dash skill logic, Juver, 2020/12/14 */
void GLLandMan::DashAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED, const DWORD _dwActionAnim )
{
	if ( _fSPEED == 0.0f )	return;
	if ( !GetNavi() )	return;
	...
	if ( bCoA )
	{
		...
		{
			if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
				pREACTOR->ReceiveSkillDash ( vCoB, _fSPEED, _dwActionAnim );
		}
	}
}

/*skill teleport, Juver, 2018/09/06 */
void GLLandMan::SkillTeleport( const STARGETID _dwActorID, const STARGETID _dwTargetID, const D3DXVECTOR3 &_vTargetPos )
{
	GLACTOR* pActor = GLGaeaServer::GetInstance().GetTarget ( this, _dwTargetID );
	if ( !pActor )		return;
	if ( !GetNavi() )	return;
	
	if ( _vTargetPos == D3DXVECTOR3( 0.0f,0.0f,0.0f ) )	return;

	//jdevtodo
	//add distance check


	D3DXVECTOR3 vPosA = pActor->GetPosition();
	D3DXVECTOR3 vPosB = _vTargetPos;
	...
	if ( bCoA && bCoB )
	{
		if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
		{
			pActor->ReceiveSkillTeleport ( _vTargetPos );
		}
	}
}

/* skill position shift, Juver, 2020/12/15 */
void GLLandMan::PositionShiftSwap( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED )
{
	if ( _fSPEED == 0.0f )		return;
	if ( !GetNavi() )			return;

	GLACTOR* pActorSelf = GLGaeaServer::GetInstance().GetTarget ( this, _dwActorID );
	if ( !pActorSelf )			return;

	GLACTOR* pActorTarget = GLGaeaServer::GetInstance().GetTarget ( this, _dwTargetID );
	if ( !pActorTarget )		return;

	D3DXVECTOR3 vPosSelf = pActorSelf->GetPosition();
	D3DXVECTOR3 vPosTarget = pActorTarget->GetPosition();

	{
		D3DXVECTOR3 vDirection = ( vPosTarget - vPosSelf );
		float fDistance = D3DXVec3Length ( &vDirection );

		vDirection.y = 0.0f;
		D3DXVec3Normalize (  &vDirection, &vDirection );

		D3DXVECTOR3 vPosFrom = vPosSelf;
		D3DXVECTOR3 vPosTo = vPosSelf + vDirection * fDistance;
		...
		GetNavi()->IsCollision ( D3DXVECTOR3(vPosFrom.x,vPosFrom.y+5.0f,vPosFrom.z), D3DXVECTOR3(vPosFrom.x,vPosFrom.y-5.0f,vPosFrom.z), vCoA, dwCoA, bCoA );
		if ( bCoA )
		{
			GetNavi()->IsCollision ( D3DXVECTOR3(vPosTo.x,vPosTo.y+5.0f,vPosTo.z), D3DXVECTOR3(vPosTo.x,vPosTo.y-5.0f,vPosTo.z), vCoB, dwCoB, bCoB );
			if ( !bCoB )
			{
				float fDxLength = 0.5f;
				if ( _fSPEED > 5.0f ) fDxLength = _fSPEED * 0.1f;

				float fDistance2 = fDistance - fDxLength;
				while ( 0.0f < fDistance2 )
				{
					vPosTo = vPosSelf + vDirection * fDistance2;
					GetNavi()->IsCollision ( D3DXVECTOR3(vPosTo.x,vPosTo.y+5.0f,vPosTo.z), D3DXVECTOR3(vPosTo.x,vPosTo.y-5.0f,vPosTo.z), vCoB, dwCoB, bCoB );
					if ( bCoB )		break;
					fDistance2 -= fDxLength;
				};
			}
			...
			{
				if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
					pActorSelf->ReceivePositionShift ( vCoB, _fSPEED );
			}
		}
	}

	{
		D3DXVECTOR3 vDirection = ( vPosSelf - vPosTarget );
		float fDistance = D3DXVec3Length ( &vDirection );

		vDirection.y = 0.0f;
		D3DXVec3Normalize (  &vDirection, &vDirection );

		D3DXVECTOR3 vPosFrom = vPosTarget;
		D3DXVECTOR3 vPosTo = vPosTarget + vDirection * fDistance;
		...
		if ( bCoA )
		{
			...
			if ( !bCoB )
			{
				...
				while ( 0.0f < fDistance2 )
				{
					vPosTo = vPosTarget + vDirection * fDistance2;
					GetNavi()->IsCollision ( D3DXVECTOR3(vPosTo.x,vPosTo.y+5.0f,vPosTo.z), D3DXVECTOR3(vPosTo.x,vPosTo.y-5.0f,vPosTo.z), vCoB, dwCoB, bCoB );
					if ( bCoB )		break;
					fDistance2 -= fDxLength;
				};
			...
			{
				if ( GetNavi()->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
					pActorTarget->ReceivePositionShift ( vCoB, _fSPEED );
			}
		}
	}
...
/* skill position shift, Juver, 2020/12/15 */
void GLLandMan::PositionShiftDash( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED )
{
	if ( _fSPEED == 0.0f )		return;
	if ( !GetNavi() )			return;
	...
	D3DXVECTOR3 vPosition = pActorSelf->GetPosition();
	D3DXVECTOR3 vDirection = ( pActorTarget->GetPosition() - vPosition );
	float fDistance = D3DXVec3Length ( &vDirection );

	vDirection.y = 0.0f;
	D3DXVec3Normalize (  &vDirection, &vDirection );

	D3DXVECTOR3 vPosFrom = vPosition;
	D3DXVECTOR3 vPosTo = vPosition + vDirection * fDistance;
	...
	if ( bCoA )
	{
		...
		if ( !bCoB )
		{
			...
			while ( 0.0f < fDistance2 )
			{
				vPosTo = vPosFrom + vDirection * fDistance2;
				GetNavi()->IsCollision ( D3DXVECTOR3(vPosTo.x,vPosTo.y+5.0f,vPosTo.z), D3DXVECTOR3(vPosTo.x,vPosTo.y-5.0f,vPosTo.z), vCoB, dwCoB, bCoB );
				if ( bCoB )		break;
				fDistance2 -= fDxLength;
			};
...
/* skill position shift, Juver, 2020/12/15 */
void GLLandMan::PositionShiftPull( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED )
{
	if ( _fSPEED == 0.0f )		return;
	if ( !GetNavi() )			return;
	...
	D3DXVECTOR3 vPosTarget = pActorTarget->GetPosition();
	D3DXVECTOR3 vDirection = ( pActorSelf->GetPosition() - vPosTarget );
	float fDistance = D3DXVec3Length ( &vDirection );
...
void GLLandMan::RegSkillActEx ( const SSKILLACTEX &sACTEX )
{
	...
	PGLSKILL pskill_data = GLSkillMan::GetInstance().GetData( sACTEX.idSKILL );
	if ( pskill_data )
	{
		if ( pskill_data->m_sAPPLY.IsSpec( EMSPECA_TRANSFORM ) ) 
		{
			GLACTOR* pactor_target = GLGaeaServer::GetInstance().GetTarget ( this, sACTEX.sID_TAR );
			if ( pactor_target && pactor_target->IsAction( GLAT_SKILL) )
			{
				SSKILLACTEX sACTEX_NEW = sACTEX;
				sACTEX_NEW.fDELAY = 0.2f;
				m_listSkillActEx.push_back(sACTEX_NEW);
...
// *****************************************************
BOOL GLLandMan::DoGateOutPCAll ( DWORD dwExceptClubID, DWORD dwGateID, DWORD dwToIndex )
{
	...
	sNetMsg.dwToIndex = dwToIndex;	/* multi gate out, Juver, 2020/11/16 */

	if ( RPARAM::bClubWarGMMapStay )
	{
...
BOOL GLLandMan::DoGateOutPCAll ( DWORD dwGateID, DWORD dwToIndex )
{
	GLMSG::SNETREQ_GATEOUT sNetMsg;
	sNetMsg.dwGateID = dwGateID;
	sNetMsg.dwToIndex = dwToIndex;	/* multi gate out, Juver, 2020/11/16 */
	...
	if ( RPARAM::bClubWarGMMapStay )
	{
		...
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
			if ( pCharNode && pCharNode->Data->m_dwUserLvl >= NSUSER_TYPE::USER_TYPE_GM4 )
			{
				continue;
			}
