...
CPVPPBGRankings::CPVPPBGRankings ()
	: m_pButtonClose(NULL)
	, m_pInfoExit(NULL)
	, m_nPage(-1)
	, m_fUpdateTime(0.0f)
...
void CPVPPBGRankings::CreateSubControl ()
{
	CD3DFontPar * pFont = DxFontMan::GetInstance().LoadDxFont( _DEFAULT_FONT, 8, _DEFAULT_FONT_SHADOW_FLAG );

	std::string strbutton[RANK_SIZE] = 
	{
		"PVP_PBG_RESULT_SORTBUTTON_TOTAL",
	};

	for( int i=0; i<RANK_SIZE; ++i )
	{
		m_pButtonRank[i] = CreateTabButton( strbutton[i].c_str(), ID2GAMEWORD("PVP_PBG_RESULT_BUTTON", i ), RANK_BUTTON_ALL +i );

		m_pPageRanking[i] = new CPVPPBGRankingPage;
		m_pPageRanking[i]->CreateSub ( this, "PVP_PBG_RESULT_RANKING", UI_FLAG_XSIZE | UI_FLAG_YSIZE, RANK_PAGE_ALL + i );
		m_pPageRanking[i]->CreateSubControl();
		m_pPageRanking[i]->SetVisibleSingle( FALSE );
		RegisterControl ( m_pPageRanking[i] );
	}

	m_pButtonClose = new CBasicTextButton;
	m_pButtonClose->CreateSub ( this, "BASIC_TEXT_BUTTON22", UI_FLAG_XSIZE, RANK_BUTTON_CLOSE );
	m_pButtonClose->CreateBaseButton ( "PVP_PBG_RESULT_BUTTON_CLOSE", CBasicTextButton::SIZE22, CBasicButton::CLICK_FLIP, ID2GAMEWORD("PVP_PBG_RESULT_BUTTON", 4 ) );
	m_pButtonClose->SetFlip ( TRUE );
	RegisterControl ( m_pButtonClose );

	m_pInfoExit = new CBasicTextBox;
	m_pInfoExit->CreateSub ( this, "PVP_PBG_RESULT_TEXT_TIME", UI_FLAG_DEFAULT );
	m_pInfoExit->SetFont ( pFont );
	m_pInfoExit->SetTextAlign ( TEXT_ALIGN_CENTER_X | TEXT_ALIGN_CENTER_Y );	
	RegisterControl ( m_pInfoExit );
}

CMultiModeButton* CPVPPBGRankings::CreateTabButton( const char* szButtonBox, const char* szButtonText, UIGUID ControlID )
{
...
void CPVPPBGRankings::SetVisibleSingle ( BOOL bVisible )
{
	CUIWindowEx::SetVisibleSingle ( bVisible );

	if( bVisible )
	{
		SetVisiblePage( RANK_ALL );
	}
	else
	{
	}
...
void CPVPPBGRankings::TranslateUIMessage ( UIGUID cID, DWORD dwMsg )
{
	...
	case RANK_BUTTON_ALL:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				int nPage = cID - RANK_BUTTON_ALL;
				SetVisiblePage( nPage );
			}
		}break;

	case ET_CONTROL_BUTTON:
	case RANK_BUTTON_CLOSE:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				CInnerInterface::GetInstance().HideGroup( GetWndID() );
				PVPPBG::ManagerClient::GetInstance().RequestMapExit();
			}
		}break;
...
void CPVPPBGRankings::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{		
	...
	if ( m_fUpdateTime >= 0.5f )
	{
		...
		if ( m_pInfoExit )
		{
			...
			float fremain_time = PVPPBG::ManagerClient::GetInstance().m_fRemain;
			if ( fremain_time > 0.0f )
			{
				CString strCombine;
				strCombine.Format ( "< %s %02d:%02d >", ID2GAMEWORD( "PVP_PBG_RESULT_TEXT", 0 ), (DWORD)fremain_time/60, (DWORD)fremain_time%60 );
...
void CPVPPBGRankings::SetVisiblePage( int nPage )
{
...
void CPVPPBGRankings::UpdateRankings()
{
	for( int i=0; i<RANK_SIZE; ++i )
	{
		if ( m_pPageRanking[i] )
