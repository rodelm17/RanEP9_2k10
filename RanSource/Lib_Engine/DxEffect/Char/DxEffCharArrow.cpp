...
char		DxEffCharArrow::NAME[MAX_PATH]	= "DxEffCharArrow";
...
HRESULT	DxEffCharArrow::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		EFFCHAR_PROPERTY_ARROW_100	sProperty;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProperty, sizeof(EFFCHAR_PROPERTY_ARROW_100) );

		m_Property.m_dwFlag		= sProperty.m_dwFlag;
		m_Property.m_bUse		= sProperty.m_bUse;
		m_Property.m_nBlend		= sProperty.m_nBlend;
		m_Property.m_nSec		= sProperty.m_nSec;
		m_Property.m_fSetRate	= sProperty.m_fSetRate;
		m_Property.m_fFullTime	= sProperty.m_fFullTime;
		m_Property.m_fAlphaTime = sProperty.m_fAlphaTime;
		m_Property.m_fDepth		= sProperty.m_fDepth;
		m_Property.m_fSize		= sProperty.m_fSize;
		m_Property.m_vEffStart	= D3DXVECTOR3 ( 0.f, 0.f, 0.f );
		m_Property.m_vEffEnd	= D3DXVECTOR3 ( 0.f, -5.f, 0.f );
		m_Property.m_vColor		= sProperty.m_vColor;

		StringCchCopy( m_Property.m_szMeshFile, MAX_PATH, sProperty.m_szMeshFile );

		SecureZeroMemory( m_szFileName_R, sizeof(char)*MAX_PATH );
		SecureZeroMemory( m_szFileName_C, sizeof(char)*MAX_PATH );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharArrow::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharArrow::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharArrow::R2LoadFile", SFile.GetFileName(), dwVer );
