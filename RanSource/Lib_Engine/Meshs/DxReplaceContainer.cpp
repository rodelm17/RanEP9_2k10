...
HRESULT DXREPLACEPIECE::Load ( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else
	{
		...
		CDebugSet::ErrorFile( "DXREPLACEPIECE::Load", szFile );
	}
...
void DxPieceQuickSort2::RenderPickAlpha( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		DXPIECEOBJ* pPiece = m_pArrayPiece[i].m_pPiece;
		if ( !pPiece )	continue;
		if ( !pPiece->pReplacePiece )	continue;
...
{
	REPLACEMAP_ITER iter = m_mapReplace.find ( std::string(szFile) );
