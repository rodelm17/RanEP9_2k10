...
void CItemPreviewWindow::CreateSubControl ()
{
	...
	m_pStanceButton->SetFlip(RGAME_GLOBAL::bItemPreviewStance );
...
void CItemPreviewWindow::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case ITEM_PREVIEW_ITEMSLOT_BELT: // SLOT BELT - JX
		{
	...
	case ITEM_PREVIEW_STANCE_BUTTON:
		{
			...
			{
				RGAME_GLOBAL::bItemPreviewStance = !RGAME_GLOBAL::bItemPreviewStance;
				m_pStanceButton->SetFlip(RGAME_GLOBAL::bItemPreviewStance );
...
void CItemPreviewWindow::StartPreview( SITEMCUSTOM sItem )
{
	...
	m_pRender->m_sPreviewData.sPutOn[EMITEM_PREVIEW_SLOT_BELT] = pCharacter->m_PutOnItems[SLOT_BELT]; // SLOT BELT - JX
	...
	if ( pItemCheck->sBasicOp.emItemType == ITEM_SUIT )
	{
		...
		case SUIT_BELT:			nSlotNum = EMITEM_PREVIEW_SLOT_BELT;	break; // SLOT BELT - JX
		case SUIT_ACCESSORY:	nSlotNum = EMITEM_PREVIEW_SLOT_ACCESSORY;	break; // SLOT AURA - JX
		};
	...
	else if ( pItemCheck->sBasicOp.emItemType == ITEM_BOX )
	{
		for ( int i=0; i<ITEM::SBOX::ITEM_SIZE; ++i )
		{
			...
			if( pItemBox )
			{
				...
				if ( pItemBox->sBasicOp.bItemColor )
				{
					sItemCustomBox.wColor1 = pItemBox->sBasicOp.wItemColor1;
					sItemCustomBox.wColor2 = pItemBox->sBasicOp.wItemColor2;
				}
