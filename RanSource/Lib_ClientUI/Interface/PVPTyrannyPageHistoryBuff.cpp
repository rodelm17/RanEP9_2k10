	...
	case TYRANNY_PAGE_HISTORY_BUFF_SLOT3:
		{
			if( CHECK_MOUSE_IN( dwMsg ) )
			{
				...
				{
					SNATIVEID sidSkill = GLPVPTyrannyClient::GetInstance().m_sScheduleNext.m_sRewardBuff[nSelect].sidSkill;
					PGLSKILL pSkill = GLSkillMan::GetInstance().GetData( sidSkill );
					if ( pSkill )
					{
						CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine( pSkill->GetName(), NS_UITEXTCOLOR::WHITE );
					}
				}
			}
		}break;
...
void CPVPTyrannyPageHistoryBuff::ShowInfo()
{
	for ( int i=0; i<TYRANNY_REWARD_BUFF_NUM; ++i )
	{
		SNATIVEID sidSkill = GLPVPTyrannyClient::GetInstance().m_sScheduleNext.m_sRewardBuff[i].sidSkill;
		PGLSKILL pSkill = GLSkillMan::GetInstance().GetData( sidSkill );
