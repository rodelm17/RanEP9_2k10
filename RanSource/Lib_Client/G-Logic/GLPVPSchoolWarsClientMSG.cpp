...
HRESULT GLPVPSchoolWarsClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_SCHOOLWARS_A2FC_STATE_REGISTER:
		{
			...
			CInnerInterface::GetInstance().PVPCompetitionButtonAlarm( EMCOMPETITION_NOTIFY_TYPE_SCHOOL_WAR_REGISTER );
	...
	case NET_MSG_GCTRL_SCHOOLWARS_A2C_REGISTER_FB:
		{
			...
			case SCHOOLWARS_REGISTER_FB_DEVICE_INFO_ERROR:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("SCHOOLWARS_REGISTER_FB_DEVICE_INFO_ERROR") );
				break;

				/* PVPSchoolWars hash check, Juver, 2021/07/17 */
			case SCHOOLWARS_REGISTER_FB_DEVICE_REGISTERED:
				CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("SCHOOLWARS_REGISTER_FB_DEVICE_REGISTERED") );
				break;
	...
	case NET_MSG_GCTRL_SCHOOLWARS_F2C_MOB_KILL_NOTICE:
		{
			GLMSG::SNETPC_SCHOOLWARS_F2C_MOB_KILL_UPDATE* pnetmsg = ( GLMSG::SNETPC_SCHOOLWARS_F2C_MOB_KILL_UPDATE* ) nmg;
			PCROWDATA pcrow = GLCrowDataMan::GetInstance().GetCrowData( pnetmsg->id_crow );
			if ( pcrow && pnetmsg->char_school < SCHOOLWARS_SCHOOL_SIZE )
			{
				if ( pnetmsg->double_point )
					CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("SCHOOLWARS_MOB_KILL2"), GLCONST_CHAR::strSCHOOLNAME[pnetmsg->char_school].c_str(), pnetmsg->score_crow, pcrow->GetName() );
				else
					CInnerInterface::GetInstance().PrintMsgText( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("SCHOOLWARS_MOB_KILL"), GLCONST_CHAR::strSCHOOLNAME[pnetmsg->char_school].c_str(), pnetmsg->score_crow, pcrow->GetName() );
			}
