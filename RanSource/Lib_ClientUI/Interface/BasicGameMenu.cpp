			...
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					if ( !CInnerInterface::GetInstance().IsVisibleGroup ( INVENTORY_WINDOW )
						&&  !CInnerInterface::GetInstance().IsInventoryBlocked () )
					{
	...
	case MENU_LARGEMAP_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				{	/*EP9 Large Map Renewal*/
					...
					{
						if ( !CInnerInterface::GetInstance().IsVisibleGroup ( LARGEMAP_WINDOW2 ) )
						{
							CInnerInterface::GetInstance().ShowGroupFocus ( LARGEMAP_WINDOW2 );
						}
						else
						{
...
void CBasicGameMenu::InitShotCutString()
{
	...
	for ( i=0; i< BASIC_MENU_NUM; ++i){
		...
		nIndex = RPROFILE::BasicMenuToRanparam[i];
		strTemp = CInnerInterface::GetInstance().GetdwKeyToString(RPROFILE::MenuShortcut[nIndex]);
