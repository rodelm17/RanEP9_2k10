...
DWORD	DxSkinMesh9_NORMAL::g_NumBoneMatricesMax = 0;
D3DXMATRIXA16*	DxSkinMesh9_NORMAL::g_pBoneMatrices	= NULL;
...
void DxSkinMesh9_NORMAL::DrawMeshContainer( IDirect3DDevice9 *pd3dDevice, const CHARSETTING& sSetting )
{
    ...
    UINT iMatrixIndex;
    D3DXMATRIXA16 matBoneMatrix_UP;
			...
			if( m_dwMaxVertexBlendMatrices >= NumBlend + 1 )
			{
				if( bHeight )
				{
					...
					for (DWORD i = 0; i < dwNumInfl; ++i)
					{
						...
						if (iMatrixIndex != UINT_MAX)
						{
							...
							D3DXMatrixMultiply( &g_pBoneMatrices[i], &pMeshContainer->pBoneOffsetMatrices[iMatrixIndex], &matBoneMatrix_UP );
							pd3dDevice->SetTransform( D3DTS_WORLDMATRIX( i ), &g_pBoneMatrices[i] );
						}
					}
				}
				else
				{
					...
					for (DWORD i = 0; i < dwNumInfl; ++i)
					{
						...
						if (iMatrixIndex != UINT_MAX)
						{
							D3DXMatrixMultiply( &g_pBoneMatrices[i], &pMeshContainer->pBoneOffsetMatrices[iMatrixIndex], pMeshContainer->ppBoneMatrixPtrs[iMatrixIndex] );
							pd3dDevice->SetTransform( D3DTS_WORLDMATRIX( i ), &g_pBoneMatrices[i] );
						}
					}
				}
				...
				if( m_dwMaxVertexBlendMatrices < NumBlend + 1)
				{
					...
					for (DWORD i = 0; i < dwNumInfl; ++i)
					{
						...
						if (iMatrixIndex != UINT_MAX)
						{
							...
							D3DXMatrixMultiply( &g_pBoneMatrices[i], &pMeshContainer->pBoneOffsetMatrices[iMatrixIndex], &matBoneMatrix_UP );
							V_PASS( pd3dDevice->SetTransform( D3DTS_WORLDMATRIX( i ), &g_pBoneMatrices[i] ) );
						}
					}
    ...
    else  // standard mesh, just draw it after setting material properties
    {
		if( sSetting.bWorldIdentity )
		{
			...
			V_PASS( pd3dDevice->SetTransform(D3DTS_WORLD, &matIdentity) );
		}
		else
		{
	...
	pd3dDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, RENGINE_GLOBAL::dwSamplerMinFilter );
	pd3dDevice->SetSamplerState( 0, D3DSAMP_MAGFILTER, RENGINE_GLOBAL::dwSamplerMagFilter );
	pd3dDevice->SetSamplerState( 0, D3DSAMP_MAXANISOTROPY, RENGINE_GLOBAL::dwSamplerMaxAnisotropy );
