...
HRESULT GLSummonField::Create ( GLLandMan* pLandMan, PGLCHAR pOwner, PGLSUMMON_LOGIC_SERVER pSummonData, D3DXVECTOR3 vtargetpos, SUMMON_DATA_SKILL *psummon_skill )
{
	...
	//	Note : 몹 로직 데이터 초기화.
	GLSUMMONLOGIC_SERVER::INIT_DATA ();
...
void GLSummonField::CleanUp ()
{
	...
	GLSUMMONLOGIC_SERVER::RESET_DATA();
...
BOOL GLSummonField::IsVisibleDetect ( const bool bSkillShowInvisible, const WORD wSkillShowInvisibleLevel, const WORD wSkillShowInvisibleRange, const D3DXVECTOR3& vPos )
{
...
HRESULT GLSummonField::FrameMove( float fTime, float fElapsedTime )
{
	...
	if ( IsValidBody () )
	{
		...
		if ( m_dwNowHP!=0 )
		{
			...
			GLSUMMONLOGIC_SERVER::UPDATE_DATA ( fTime, fElapsedTime );
		}else{
		...
		if ( wThisKey >= sStrike.wDivKey )
		{
			if ( sStrike.bHit )
			{
				...
				else
				{
					...
					dwDamageFlag = GLSUMMONLOGIC_SERVER::CALCDAMAGE ( nDamage, m_dwGUID, m_TargetID, m_pLandMan, NULL, 0, NULL, sAniAttack.m_wDivCount );
...
inline DWORD GLSummonField::ToDamage ( const STARGETID &sTargetID, const int nDAMAGE, const BOOL bShock )
{
	...
	return pACTOR_TAR->ReceiveDamage ( GLSUMMONLOGIC_SERVER::GETCROW(), m_dwGUID, nDAMAGE, bShock );
}
...
void GLSummonField::DamageProc ( const int nDAMAGE, DWORD dwDamageFlag )
{
	...
	if ( pACTOR && m_pAttackProp->emBLOW_TYPE!=EMBLOW_NONE )
	{
		...
		{
			bBLOW = GLOGICEX::CHECKSTATEBLOW ( m_pAttackProp->fBLOW_RATE*fPOWER, GLSUMMONLOGIC_SERVER::GETLEVEL(), pACTOR->GetLevel(), nBLOWRESIST );
		}
...
void GLSummonField::PreAttackProc ()
{
	...
	{
		bool bhit = GLSUMMONLOGIC_SERVER::CHECKHIT(m_TargetID,m_pLandMan) ? true : false;
...
void GLSummonField::PreStrikeProc ( BOOL bSkill, BOOL bLowSP )
{
	...
	for ( int i = 0; i < sAniAttack.m_wDivCount; i++ )
	{
		...
		{
			if ( !GLSUMMONLOGIC_SERVER::CHECKHIT(sTargetID,m_pLandMan) )		bhit = false;
		}
...
DWORD GLSummonField::ReceiveDamage ( const EMCROW emACrow, const DWORD dwAID, const DWORD dwDamage, const BOOL bShock )
{
	...
	//	Note : 공격 받을때 반응 감안?
	//		( emACrow, dwAID )
	DWORD dwDxHP = GLSUMMONLOGIC_SERVER::RECEIVE_DAMAGE ( (WORD) dwDamage );
...
/* push skill logic, Juver, 2020/12/14 */
void GLSummonField::ReceiveSkillPush ( const D3DXVECTOR3 &vMovePos, const float& fSpeed )
{
	if ( !IsValidBody () )		return;
...
/* pull skill logic, Juver, 2020/12/14 */
void GLSummonField::ReceiveSkillPull ( const D3DXVECTOR3 &vMovePos, const float& fSpeed )
{
	if ( !IsValidBody () )		return;
...
void GLSummonField::ReceiveSkillDash ( const D3DXVECTOR3 &vMovePos, const float& fSpeed, const DWORD& dwActionAnim )
{
	if ( !IsValidBody () )		return;
...
void GLSummonField::ReceiveSkillTeleport( const D3DXVECTOR3& vtarget_pos )
{
	if ( !IsValidBody () )		return;
...
/* skill position shift, Juver, 2020/12/15 */
void GLSummonField::ReceivePositionShift ( const D3DXVECTOR3 &vMovePos, const float& fSpeed )
{
	if ( !IsValidBody () )		return;
...
bool GLSummonField::SetAttackTarget( STARGETID sAttackTargetID )
{
	...
	if ( pskill_base && m_pOwner )
	{
		...
		if ( !m_pOwner->IsReActionable( pTARGET, benemy, true ) )
			return FALSE;
	...
	if ( pskill_use && m_pOwner )
	{
		...
		for( int i=0; i<EMTARGET_NET; ++i )
		{
			...
			if ( !m_pOwner->IsReActionable( pskill_target, benemy, true ) )	continue;
...
HRESULT GLSummonField::UpdateViewAround ()
{
	...
	common_vector_free( m_vecCROW_VIEW );
	common_vector_free( m_vecSUMMON_VIEW );
...
void GLSummonField::LinkSkillStigmaTarget ( const EMCROW _emACrow, const DWORD _dwAID, const SNATIVEID& _sSkillID, const float& fDistance, const float& fDamage, const DWORD& dwAttackFlag, const SNATIVEID& sMapID )
{
...
/* skill stigma, Juver, 2020/12/13 */
BOOL GLSummonField::CheckSkillStigmaExist( const EMCROW _emACrow, const DWORD _dwAID, const SNATIVEID& _sSkillID ) const
{
	BOOL bFOUND = FALSE;
...
void GLSummonField::ReceiveSkillBuffDurationChange( const SNATIVEID& _sSkillID, const DWORD& _dwFlags, const float& fSeconds )
{
	GLMSG::SNETPC_F2C_SKILL_BUFF_DURATION_CHANGE_TIME_MULTIPLE_BRD NetMsgBrd;
	NetMsgBrd.dwID = m_dwGUID;
	NetMsgBrd.emCrow = CROW_SUMMON;
...
void GLSummonField::ReactDamage( GLACTOR* pAttacker, DWORD dwDamageFlags )
{
	/* skill illusion, Juver, 2021/01/17 */
	if( dwDamageFlags & DAMAGE_TYPE_ILLUSION )
	{
		...
		if ( m_pLandMan )
		{
			STARGETID dwActorID = STARGETID(CROW_SUMMON,m_dwGUID);
...
/* skill stigma, Juver, 2020/12/12 */
void GLSummonField::SkillStigmaCheck()
{
	...
	for ( ; it1_b != it1_e; )
	{
		...
		for ( ; it2_b != it2_e; )
		{
			...
			if ( pACTOR )
			{
				...
				BOOL bExist = pACTOR->CheckSkillStigmaExist( CROW_SUMMON, m_dwGUID, sData.sSkillID );
				if ( !bExist )
					bDeleteTarget = TRUE;
			}
			else
