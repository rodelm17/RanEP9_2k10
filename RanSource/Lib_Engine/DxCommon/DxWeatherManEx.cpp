...
const	DWORD	DxEffectRain::VERSION	=	0x104;
...
void	DxEffectRain::LoadSet ( CSerialFile &SFile )
{
	...
	{
		RAIN_PROPERTY_103	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(RAIN_PROPERTY_103) );
		m_dwFaces = sProp.m_dwFaces;
		m_vColor = sProp.m_vColor;
		m_iHeight = sProp.m_iHeight;
		m_fLength = sProp.m_fLength;
		m_fSpeed = sProp.m_fSpeed;
		m_iRadius = sProp.m_iRadius;
	...
	{
		CDebugSet::ErrorVersion( "DxEffectRain::LoadSet", SFile.GetFileName(), dwVer );
...
const	DWORD	DxEffectCloud::VERSION	=	0x101;
...
void	DxEffectCloud::LoadSet ( CSerialFile &SFile )
{
	...
	{
		CLOUD_PROPERTY_100	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(CLOUD_PROPERTY_100) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffectCloud::LoadSet", SFile.GetFileName(), dwVer );
		dwBuffSize -= sizeof(DWORD);
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwBuffSize );
	}
