...
const DWORD CClubWindow::dwCooldownDuration = 600000; // 10 minutes Ecall Cooldown
...
CClubWindow::CClubWindow ()
	...
	, m_pCallButton(NULL)	/*ecall function Arc Development 08-16-2024*/
	, dwLastCallTime(0)
{
}
...
void CClubWindow::CreateSubControl ()
{
	...
	m_pCallButton = CreateTextButton14("CLUB_CALL_BUTTON", CLUB_CALL_BUTTON, const_cast<char*>(ID2GAMEWORD("CLUB_BUTTON", 18)));
...
void CClubWindow::TranslateUIMessage ( UIGUID cID, DWORD dwMsg )
{
	...
	case CLUB_PARTY_BUTTON:
		{
			if( CHECK_MOUSEIN_LBUPLIKE( dwMsg ) )
			{
				...
				{
					pPartyModalWindow->DoModal ( strTemp, MODAL_PARTY_REQ_GUILD_WINDOW );
	...
	/*ecall function Arc Development 08-16-2024*/
	case CLUB_CALL_BUTTON:
		{
			if (CHECK_MOUSEIN_LBUPLIKE(dwMsg))
			{
				DWORD dwCurrentTime = GetTickCount();

				if (dwCurrentTime - dwLastCallTime < dwCooldownDuration)
				{
					DWORD dwRemainingTime = (dwCooldownDuration - (dwCurrentTime - dwLastCallTime)) / 1000; // Convert ms to seconds

					CString strCooldownMessage;
					strCooldownMessage.Format("Cooldown active. Please wait %u seconds.", dwRemainingTime);

					CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, strCooldownMessage.GetString());
					break;
				}
				...
				// Check if the map is a valid WoE map
				if (!pLand->m_bPVPWoeMap && !pLand->m_bPVPTyrannyMap)
				{
					CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, "Action denied. This is not a War map.");
					break;
				}

				DWORD dwCallerMapID = pLand->GetMapID().dwID;
				DWORD dwCallerID = GLGaeaClient::GetInstance().GetCharacter()->m_dwCharID;

				bool bMemberInSameMap = false;

				for (CLUBMEMBERS_ITER iter = mapMembers.begin(); iter != mapMembers.end(); ++iter)
				{
					GLCLUBMEMBER& sClubMember = iter->second;

					if (sClubMember.dwID == dwCallerID)
						continue; // Skip the caller themselves

					if (sClubMember.nidMAP.dwID == dwCallerMapID)
					{
						bMemberInSameMap = true;

						GLMSG::SNET_CALL_GUILD_MEM netmsg;
						StringCchCopy(netmsg.szName, CHAR_SZNAME, sClubMember.m_szName);
						NETSEND(&netmsg);
					}
				}
				...
				if (bMemberInSameMap)
				{
					dwLastCallTime = dwCurrentTime; // Set cooldown only if members were called
					CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::GREEN_WOW, "Members called successfully.");
				}
				else
				{
					CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::DISABLE, "No club members are in the same map. Call failed.");
				}
			}
		}
		break;
...
void CClubWindow::ResetState ()
{
	...
	/*ecall function Arc Development 08-16-2024*/
	m_pCallButton->SetVisibleSingle(FALSE);
...
void CClubWindow::RefreshState ()
{
	...
	if( m_bClubMaster ) 
	{
		m_pBreakupButton->SetVisibleSingle( TRUE );
		m_pCallButton->SetVisibleSingle(TRUE);	/*ecall function Arc Development 08-16-2024*/
	}
