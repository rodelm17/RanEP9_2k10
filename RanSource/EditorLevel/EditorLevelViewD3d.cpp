...
void CEditorLevelView::OnInitialUpdate()
{
	...
	strcpy ( szEditorPath, RPATH::getAppPath() );
...
HRESULT CEditorLevelView::Render()
{
	...
	if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
	{
		...
		if ( m_pDxLandMan)	
			m_pDxLandMan->RenderPickAlpha( m_pd3dDevice, CV );
				...
				if ( pLevelEtcFunc )
				{
					if ( pLevelEtcFunc->m_bUseFunction[EMETCFUNC_LANDEFFECT] )
					{
						for( int i = 0; i < (int)pLevelEtcFunc->m_vecLandEffect.size(); i++ )
						{
							...
							EDITMESHS::RENDERAABB ( m_pd3dDevice, landEffect.vMaxPos, landEffect.vMinPos, 0xffffff00 );
						}
					}
				}
			}
