...
enum
{
	...
	EM_INVEN_SLOT_SAVE_SIZE = EM_INVENSIZE_Y - ( EM_INVEN_DEF_SIZE_Y + EM_INVEN_PREMIUM_SIZE ),

};

/*inventory sort, Juver, 2018/07/07 */
class INVEN_SORT_OPERATOR
{
public:
	bool operator() ( const SINVENITEM &l, const SINVENITEM &r )
	{
		if ( l.sItemCustom.sNativeID.wMainID < r.sItemCustom.sNativeID.wMainID ) return true;
		else if ( l.sItemCustom.sNativeID.wMainID == r.sItemCustom.sNativeID.wMainID ) 
		{	
			if ( l.sItemCustom.sNativeID.wSubID < r.sItemCustom.sNativeID.wSubID ) return true;
		}
...
public:
	...
	SINVENITEM* FindItem ( const SNATIVEID sNID, const WORD wGrade, const WORD wQuantity );
