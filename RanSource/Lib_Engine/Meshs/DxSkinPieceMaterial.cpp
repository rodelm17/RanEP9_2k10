...
void SMATERIAL_PIECE::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwVERSION, dwBLOCKSIZE;
	SFile >> dwVERSION;
	SFile >> dwBLOCKSIZE;
	...
	{
		CDebugSet::ErrorVersion ( "SMATERIAL_PIECE::R2LoadFile", SFile.GetFileName(), dwVERSION );
		SFile.SetOffSet ( SFile.GetfTell() + dwBLOCKSIZE );
	}
