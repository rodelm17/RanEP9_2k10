...
bool CROWREN::DIFFERENT ( const CROWREN &sINFO )
{
	...
	//////////////////////////////////////////////////////////////////
	//MMR Rank
	if ( m_dwMMR != sINFO.m_dwMMR )						return true;
	if ( m_bRankName != sINFO.m_bRankName )				return true;
	if ( m_bRankMark != sINFO.m_bRankMark )				return true;
	//////////////////////////////////////////////////////////////////
...
void CROWREN::INIT ( GLCharacter *pMYCHAR )
{
	...
	//////////////////////////////////////////////////////////////////
	//MMR Rank
	m_dwMMR =  pMYCHAR->m_dwMMR ;
	m_bRankName =  pMYCHAR->m_bRankName ;
	m_bRankMark =  pMYCHAR->m_bRankMark ;
	//////////////////////////////////////////////////////////////////
	StringCchCopy ( m_szNAME, CHAR_SZNAME, pMYCHAR->m_szName );

	/*hide gm info, Juver, 2017/11/26 */
	m_bHideGMInfo = BOOL( pMYCHAR->m_dwUserLvl >= NSUSER_TYPE::USER_TYPE_GM3 );
...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR )
{
	...
	/*hide gm info, Juver, 2017/11/26 */
	m_bHideGMInfo = BOOL( pMYCHAR->m_dwUserLvl >= NSUSER_TYPE::USER_TYPE_GM4 );

	/*pvp capture the flag, Juver, 2018/01/31 */
	m_wCaptureTheFlagTeam = pMYCHAR->m_wCaptureTheFlagTeam;

	if ( pMYCHAR->m_dwUserLvl >= NSUSER_TYPE::USER_TYPE_GM3  )
		m_dwCOLOR = NS_UITEXTCOLOR::GREEN_WOW;
	else if ( pMYCHAR->m_dwUserLvl == NSUSER_TYPE::USER_TYPE_GM4 )
		m_dwCOLOR = NS_UITEXTCOLOR::AQUA;
}
...
void CROWREN::INIT ( GLCharClient *pCHAR )
{
	...
	//////////////////////////////////////////////////////////////////
	//MMR Rank
	m_dwMMR =  pCHAR->GetCharData().m_dwMMR ;
	m_bRankName =  pCHAR->GetCharData().m_bRankName ;
	m_bRankMark =  pCHAR->GetCharData().m_bRankMark ;
	//////////////////////////////////////////////////////////////////

	StringCchCopy ( m_szNAME, CHAR_SZNAME, pCHAR->GetCharData().szName );

	/*hide gm info, Juver, 2017/11/26 */
	m_bHideGMInfo = BOOL( pCHAR->GetCharData().dwUserLevel >= NSUSER_TYPE::USER_TYPE_GM3 );
...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR, GLCharClient *pCHAR )
{
	...
	//	Note : 적대 행위자.
	if ( pMYCHAR->IsPK_TAR(pCHAR,false,false) )
	{
	...
	/*hide gm info, Juver, 2017/11/26 */
	m_bHideGMInfo = BOOL( pCHAR->GetCharData().dwUserLevel >= NSUSER_TYPE::USER_TYPE_GM4 );

	/*pvp capture the flag, Juver, 2018/01/31 */
	m_wCaptureTheFlagTeam = pCHAR->GetCharData().m_wCaptureTheFlagTeam;

	if ( pCHAR->GetCharData().dwUserLevel >= NSUSER_TYPE::USER_TYPE_GM3  )
		m_dwCOLOR = NS_UITEXTCOLOR::GREEN_WOW;
	else if ( pCHAR->GetCharData().dwUserLevel == NSUSER_TYPE::USER_TYPE_GM4 )
		m_dwCOLOR = NS_UITEXTCOLOR::AQUA;
}
...
void CROWREN::INIT ( GLCrowClient *pCROW )
{
	...
	m_wSCHOOL = pCROW->m_pCrowData->m_sBasic.m_emCrowSchool;
}
...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR, GLCrowClient *pCROW )
{
	...
	if ( emCROW == CROW_NPC || emCROW == CROW_ZONE_NAME || emCROW == CROW_GATE_NAME )	
	...
	else
	{
		...
		if ( pCROW->m_pCrowData->m_sBasic.m_emCrowSchool != EMCROW_SCHOOL_SIZE && 
			pMYCHAR->GetCharData().m_wSchool == pCROW->m_pCrowData->m_sBasic.m_emCrowSchool )
		{
			m_dwCOLOR = NS_UITEXTCOLOR::BRIGHTGREEN;
		}
	}
...
{
	WORD wMY_LEVEL = pMYCHAR->GETLEVEL();
	WORD wMOB_LEVEL = pMaterial->m_pCrowData->m_sBasic.m_wLevel;
	float fEXP_RATE = GLCONST_CHAR::GETEXP_RATE ( wMY_LEVEL, wMOB_LEVEL );
...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR, CItemClientDrop *pITEM )
{
	...
	SITEM *pITEM_DATA = GLItemMan::GetInstance().GetItem ( pITEM->sItemClient.sNativeID );
	if ( pITEM_DATA )
		m_dwCOLOR = COMMENT::ITEMCOLOR[pITEM_DATA->sBasicOp.emLevel];
}
