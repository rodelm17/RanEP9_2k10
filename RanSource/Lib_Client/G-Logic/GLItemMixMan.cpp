...
HRESULT GLItemMixMan::LoadFile ( const char* szFile, BOOL bServer )
{
	...
	if ( dwFILEVER>=VERSION_ENCODE )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_ITEM_MIX_VER_1 );

	if ( dwFILEVER>=ENCODE_VER_BYTE_CRYPT_2021_ITEM_MIX_V1 )
		SFile.SetEncodeType ( EMRCRYPT_BYTE_TYPE_2021_ITEM_MIX_V1 );
...
HRESULT GLItemMixMan::SaveCsvFile ( CWnd* pWnd )
{
	...
	ITEM_MIX::SaveCsvHead( streamFILE );

	for ( DWORD d=0; d<ITEMMIX_MAXKEY; ++d )
	{
		ITEMMIX_KEYMAP_ITER it = m_mapKeyItemMix.find(d);
		if ( it != m_mapKeyItemMix.end() )
		{
			(*it).second.SaveCsv( streamFILE ); 
		}
	}
...
HRESULT GLItemMixMan::LoadCsvFile ( CWnd* pWnd )
{
	CFileDialog dlg( TRUE, ".csv", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, "csv file (*.csv)|*.csv|", pWnd );
