	...
	if ( pMaster )
	{
		...
		if ( m_pConditionTextBox )
		{
			...
			if ( m_sPartyOption != sPartyOption )
			{
				...
				if ( sPartyOption.bPrivateParty )
					m_pConditionTextBox->AddText ( ID2GAMEINTEXT( "PARTY_PRIVACY_TEXT_2" ), NS_UITEXTCOLOR::PRIVATE );	
				else
					m_pConditionTextBox->AddText ( ID2GAMEINTEXT( "PARTY_PRIVACY_TEXT_1" ), NS_UITEXTCOLOR::PRIVATE );	
