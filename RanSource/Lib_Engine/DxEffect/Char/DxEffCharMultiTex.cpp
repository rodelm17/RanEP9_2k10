...
char		DxEffCharMultiTex::NAME[MAX_PATH]	= "DxEffCharMultiTex";
...
HRESULT	DxEffCharMultiTex::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if ( dwVer == 0x0105 )
	{
		//	Note : 이팩트의 Property 를 읽기
		//
		EFFCHAR_PROPERTY_MULTITEX_103_104_105	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_MULTITEX_103_104_105) );

		m_dwFlag		= sProp.m_dwFlag;
		m_dwMaterials	= sProp.m_dwMaterials;
		m_nBlend		= sProp.m_nBlend;
		m_nTexNum		= sProp.m_nTexNum;
		m_cDiffuse1		= sProp.m_cDiffuse1;
		m_cDiffuse2		= sProp.m_cDiffuse2;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharMultiTex::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharMultiTex::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharMultiTex::R2LoadFile", SFile.GetFileName(), dwVer );
