...
HRESULT GLCharAG::MsgClubMemberReqAgt ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pCHAR->m_tSECEDE!=0 )
	{
		...
		CTimeSpan sSpan(GLCONST_CHAR::dwCLUB_JOINBLOCK_DAY,0,0,0);
...
HRESULT GLCharAG::MsgClubAuthorityReq ( NET_MSG_GENERIC* nmg )
{
	...
	//block transfer of guild master when cdm is running
	if ( GLClubDeathMatchAgentMan::GetInstance().IsBattle() )
	{
		NetMsgFB.emFB = EMCLUB_AUTHORITY_REQ_FB_CDM_TIME;
		GLAgentServer::GetInstance().SENDTOCLIENT ( m_dwClientID, &NetMsgFB );
		return S_FALSE;
	}
...
HRESULT GLCharAG::MsgClubAuthorityReqAns ( NET_MSG_GENERIC* nmg )
{
	...
	if ( PVPClubDeathMatchAgent::GetInstance().IsBattle() )		return S_FALSE;
...
HRESULT GLCharAG::MsgClubAllianceReq ( NET_MSG_GENERIC* nmg )
{
	...
	{
		NetMsgFB.emFB = EMCLUB_ALLIANCE_REQ_FB_DISABLED;
		GLAgentServer::GetInstance().SENDTOCLIENT ( m_dwClientID, &NetMsgFB );
		return S_FALSE;
	}
...
HRESULT GLCharAG::MsgClubAllianceReqAns ( NET_MSG_GENERIC* nmg )
{
	...
	/*club alliance setting, Juver, 2018/06/13 */
	if ( !RPARAM::bUseClubAlliance )	return FALSE;
...
HRESULT GLCharAG::MsgClubBattleReqAns ( NET_MSG_GENERIC* nmg )
{
	...
	pCLUB_CHIEF->m_mapBattleReady.insert ( std::make_pair( sClubBattle.m_dwCLUBID, sClubBattle ) );	
	...
	//	db에 저장 ( A기준, B기준 저장 )
	CSetClubBattle *pDbActionA = new CSetClubBattle ( pCLUB_CHIEF->m_dwID, pCLUB_INDIAN->m_dwID, cStartTime.GetTime(), cEndTime.GetTime() );
	CSetClubBattle *pDbActionB = new CSetClubBattle ( pCLUB_INDIAN->m_dwID, pCLUB_CHIEF->m_dwID, cStartTime.GetTime(), cEndTime.GetTime() );
	GLDBMan* pDbMan = GLAgentServer::GetInstance().GetDBMan();
	if ( pDbMan )
	{
