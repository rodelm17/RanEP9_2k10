	...
	case GLAT_DIE:
		{
			...
			STARGETID sTargetID(m_pSummonCrowData->m_sBasic.m_emCrow,m_dwGUID,m_vPos);
...
HRESULT GLSummonClient::Create ( const PGLSUMMONLOGIC_CLIENT pSummonData, D3DXVECTOR3 vPos, D3DXVECTOR3 vDir, NavigationMesh* pNavi, LPDIRECT3DDEVICEQ pd3dDevice )
{
...
/*skill transform, Juver, 2018/09/09 */
void GLSummonClient::skill_transform_change_skin()
{
	if ( !m_pd3dDevice )	return;
	if ( !m_pSkinChar )		return;
	if ( !m_pSummonCrowData )		return;

	std::string skin_file = m_pSummonCrowData->m_sAction.m_strSkinObj.c_str();
