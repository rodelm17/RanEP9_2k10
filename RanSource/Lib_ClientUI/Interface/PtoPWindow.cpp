			...
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					else
					{
						...
						if ( pLand && pLand->IsTradeRestricted() )
						{
							CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("TRADE_RESTRICTED_MAP") );
						}
						else
						{
	...
	case PTOPMENU_REFER_CHAR:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					GLGaeaClient::GetInstance().GetCharacter()->RequestCharacterInfoGaea( m_dwGaeaID );
				}
			}
		}
		break;
...
//		동맹 메뉴
		{
			...
			if( sClub.IsMaster(dwCharID) && pCharClient->IsClubMaster() && RPARAM::bUseClubAlliance )
			{
