...
bool CGLCommentFile::LOADFILE( const char * szFileName )
{
	...
	for (int i=0; i<SKILL::EMSPEC_POSITION_SHIFT_TYPE_SIZE; ++i )
		cFILE.getflag( "SPEC_POSITION_SHIFT_TYPE", i, 0, 1, COMMENT::SPEC_POSITION_SHIFT_TYPE[i]);
	...
	for (int i=0; i<EMCROW_SCHOOL_SIZE+1; ++i )
		cFILE.getflag( "CROW_SCHOOL", i, 0, 1, COMMENT::CROW_SCHOOL[i]);
	...
	for (int i=0; i<VEHICLE_SUBTYPE_SIZE; ++i )
		cFILE.getflag( "VEHICLE_SUBTYPE_BIKE_V2", i, 0, 1, COMMENT::VEHICLE_SUBTYPE_BIKE_V2[i] );
	...
	for (int i=0; i<EMCODEX_NOTIFY_SIZE; ++i )
		cFILE.getflag( "CODEX_NOTIFY", i, 0, 1, COMMENT::CODEX_NOTIFY[i]);
	for (int i=0; i<EMCODEX_TYPE_SIZE; ++i )
		cFILE.getflag( "CODEX_MID", i, 0, 1, COMMENT::CODEX_MID[i]);
	/*product item, Juver, 2017/10/15 */
	...
	for (int i=0; i<EMBATTLEPASS_TYPE_SIZE; ++i )
		cFILE.getflag( "BATTLEPASS_MID", i, 0, 1, COMMENT::BATTLEPASS_MID[i]);

	for (int i=0; i<NSUSER_TYPE::USER_TYPE_INDEX_SIZE; ++i )
		cFILE.getflag( "USER_TYPE", i, 0, 1, COMMENT::USER_TYPE[i]);

	/* post system, Juver, 2021/02/05 */
	for (int i=0; i<EMPOSTBOX_DESIGN_TYPE_SIZE; ++i )
		cFILE.getflag( "POSTBOX_DESIGN_TYPE", i, 0, 1, COMMENT::POSTBOX_DESIGN_TYPE[i]);


	/* set item option, Juver, 2021/09/04 */
	for (int i=0; i<EMSET_OPTION_SLOT_SIZE; ++i )
	{
		cFILE.getflag( "SET_OPTION_SLOT", i, 0, 1, COMMENT::SET_OPTION_SLOT[i]);
	}

	for (int i=0; i<EMSET_OPTION_VAR_SIZE; ++i )
	{
		cFILE.getflag( "SET_OPTION_VAR", i, 0, 3, COMMENT::SET_OPTION_VAR[i]);
		cFILE.getflag( "SET_OPTION_VAR", i, 1, 3, COMMENT::bSET_OPTION_VAR_PERCENT[i]);
		cFILE.getflag( "SET_OPTION_VAR", i, 2, 3, COMMENT::fSET_OPTION_VAR_MULTIPLIER[i]);
	}

	for (int i=0; i<EMVIP_LEVEL_SIZE; ++i )
	{
		cFILE.getflag( "CHAR_VIP", i, 0, 1, COMMENT::CHAR_VIP[i]);
	}
