...
#pragma warning(disable:4503) //decorated name truncated warning
...
public:
	...
	//Added getflag functions which take int as key parameter
	//key is converted to char and used as parameter to original function
	//limitation is the key on the textfile must be 3 digit

	template<typename TYPE>
	bool getflag ( const STRING section, const int nKey, const int nIDX, const int nSIZE, TYPE &VALUE )
	{
		char szKey[4] = "";
		StringCchPrintf( szKey, 4, "%03d", nKey );
		return getflag ( section, szKey, nIDX, nSIZE, VALUE );
	}

	template<>
	bool getflag<bool> ( const STRING section, const int nKey, const int nIDX, const int nSIZE, bool &bVALUE )
	{
		char szKey[4] = "";
		StringCchPrintf( szKey, 4, "%03d", nKey );
		return getflag ( section, szKey, nIDX, nSIZE, bVALUE );
	}

	template<>
	bool getflag<float> ( const STRING section, const int nKey, const int nIDX, const int nSIZE, float &fVALUE )
	{
		char szKey[4] = "";
		StringCchPrintf( szKey, 4, "%03d", nKey );
		return getflag ( section, szKey, nIDX, nSIZE, fVALUE );
	}

	template<>
	bool getflag<STRING> ( const STRING section, const int nKey, const int nIDX, const int nSIZE, STRING &strVALUE )
	{
		char szKey[4] = "";
		StringCchPrintf( szKey, 4, "%03d", nKey );
		return getflag ( section, szKey, nIDX, nSIZE, strVALUE );
	}

	template<>
	bool getflag<CString> ( const STRING section, const int nKey, const int nIDX, const int nSIZE, CString &strVALUE )
	{
		char szKey[4] = "";
		StringCchPrintf( szKey, 4, "%03d", nKey );
		return getflag ( section, szKey, nIDX, nSIZE, strVALUE );
	...
	bool getflag_sz ( const STRING section, const int nKey, const int nIDX, const int nSIZE, TCHAR* szVALUE, int nStrLen )
	{
		GASSERT(szVALUE&&"gltexfile::getflag_sz");
		if ( NULL==szVALUE )	return false;
		...
		char szKey[4] = "";
		StringCchPrintf( szKey, 4, "%03d", nKey );
		return getflag_sz ( section, szKey, nIDX, nSIZE, szVALUE, nStrLen );
	}
};
