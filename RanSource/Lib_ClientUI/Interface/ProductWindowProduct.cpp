...
#include "../../Lib_Client/G-Logic/GLItemMixMan.h"
...
CProductWindowProduct::CProductWindowProduct ()
	...
	, m_pTextProductPlayTimeReq(NULL)
	, m_pTextProductPlayPointUse(NULL)

	, m_pTextProductContriUse(NULL)
{
...
void CProductWindowProduct::CreateSubControl ()
{
	...
	m_pTextProductPlayTimeReq = new CBasicTextBox;
	m_pTextProductPlayTimeReq->CreateSub ( this, "RAN_PRODUCT_WINDOW_GOODS_PRODUCT_PLAY_TIME_REQ" );
	m_pTextProductPlayTimeReq->SetFont ( pFont9 );
	m_pTextProductPlayTimeReq->SetTextAlign ( TEXT_ALIGN_LEFT | TEXT_ALIGN_CENTER_Y );
	m_pTextProductPlayTimeReq->SetText( "--" );
	RegisterControl ( m_pTextProductPlayTimeReq );

	/* play time system, Juver, 2021/01/27 */
	m_pTextProductPlayPointUse = new CBasicTextBox;
	m_pTextProductPlayPointUse->CreateSub ( this, "RAN_PRODUCT_WINDOW_GOODS_PRODUCT_PLAY_POINT_USE" );
	m_pTextProductPlayPointUse->SetFont ( pFont9 );
	m_pTextProductPlayPointUse->SetTextAlign ( TEXT_ALIGN_LEFT | TEXT_ALIGN_CENTER_Y );
	m_pTextProductPlayPointUse->SetText( "--" );
	RegisterControl ( m_pTextProductPlayPointUse );

	m_pTextProductContriUse = new CBasicTextBox;
	m_pTextProductContriUse->CreateSub ( this, "RAN_PRODUCT_WINDOW_GOODS_PRODUCT_CONTRI_USE" );
	m_pTextProductContriUse->SetFont ( pFont9 );
	m_pTextProductContriUse->SetTextAlign ( TEXT_ALIGN_LEFT | TEXT_ALIGN_CENTER_Y );
	m_pTextProductContriUse->SetText( "--" );
	RegisterControl ( m_pTextProductContriUse );
...
void CProductWindowProduct::ResetData()
{
	...
	/* play time system, Juver, 2021/01/27 */
	if ( m_pTextProductPlayTimeReq )	m_pTextProductPlayTimeReq->ClearText();
	if ( m_pTextProductPlayPointUse )	m_pTextProductPlayPointUse->ClearText();

	if  ( m_pTextProductContriUse )
		m_pTextProductContriUse->ClearText();
...
void CProductWindowProduct::SetData( DWORD dwID )
{
	...
	/* play time system, Juver, 2021/01/27 */
	if ( m_pTextProductPlayTimeReq )
	{
		m_pTextProductPlayTimeReq->ClearText();

		if ( m_sItemMix.llPlayTimeReq )
		{
			int nIndex = m_pTextProductPlayTimeReq->AddText( ID2GAMEWORD( "RAN_PRODUCT_WINDOW_TEXT_EX", 0 ), NS_UITEXTCOLOR::WHITE );
			strTemp.Format( " %I64d", m_sItemMix.llPlayTimeReq );
			m_pTextProductPlayTimeReq->AddString( nIndex, strTemp.GetString(), NS_UITEXTCOLOR::ORANGE );
		}
	}

	/* play time system, Juver, 2021/01/27 */
	if ( m_pTextProductPlayPointUse )
	{
		m_pTextProductPlayPointUse->ClearText();

		if ( m_sItemMix.llPlayPointUse )
		{
			int nIndex = m_pTextProductPlayPointUse->AddText( ID2GAMEWORD( "RAN_PRODUCT_WINDOW_TEXT_EX", 1 ), NS_UITEXTCOLOR::WHITE );
			strTemp.Format( " %I64d", m_sItemMix.llPlayPointUse );
			m_pTextProductPlayPointUse->AddString( nIndex, strTemp.GetString(), NS_UITEXTCOLOR::ORANGE );
		}
	}
	...
	if ( m_pTextProductContriUse )
	{
		...
		if ( m_sItemMix.dwContriUse )
		{
			int nIndex = m_pTextProductContriUse->AddText( ID2GAMEWORD( "RAN_PRODUCT_WINDOW_TEXT_EX", 2 ), NS_UITEXTCOLOR::WHITE );
			strTemp.Format( " %d", m_sItemMix.dwContriUse );
			m_pTextProductContriUse->AddString( nIndex, strTemp.GetString(), NS_UITEXTCOLOR::ORANGE );
		}
	}
	...
	{
		CString price = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( (LONGLONG)m_sItemMix.dwPrice, 3, "," );
		...
		strTemp.Format( " : %s", price.GetString() );
...
void CProductWindowProduct::UpdateInfo()
{
	...
	/* play time system, Juver, 2021/01/27 */
	BOOL bPlayTimeReqFail = FALSE;
	if ( pCharacter->m_llPlayTime < m_sItemMix.llPlayTimeReq )
		bPlayTimeReqFail = TRUE;

	/* play time system, Juver, 2021/01/27 */
	BOOL bPlayPointUseFail = FALSE;
	if ( pCharacter->m_llPlayPoint < m_sItemMix.llPlayPointUse )
		bPlayPointUseFail = TRUE;

	BOOL bContriUseFail = FALSE;
	if ( pCharacter->m_dwContributionPoint < m_sItemMix.dwContriUse )
		bContriUseFail = TRUE;
	...
	if ( m_pTextInfo )
	{
		...
		if ( m_sItemMix.dwKey != UINT_MAX )
		{
			...
			/* play time system, Juver, 2021/01/27 */
			else if ( bPlayTimeReqFail )
				m_pTextInfo->AddText( ID2GAMEWORD( "RAN_PRODUCT_GOODS_MESSAGE_EX", 0 ), NS_UITEXTCOLOR::ORANGE );

			/* play time system, Juver, 2021/01/27 */
			else if ( bPlayPointUseFail )
				m_pTextInfo->AddText( ID2GAMEWORD( "RAN_PRODUCT_GOODS_MESSAGE_EX", 1 ), NS_UITEXTCOLOR::ORANGE );
			else if ( bContriUseFail )
				m_pTextInfo->AddText( ID2GAMEWORD( "RAN_PRODUCT_GOODS_MESSAGE_EX", 2 ), NS_UITEXTCOLOR::ORANGE );
			else
