...
#include "../G-Logic/GLCharLogicServer.h"
#include "../G-Logic/GLCharLogicClient.h"
...
BOOL SNpcTalkCondition::SAVE ( CSerialFile& SFile )
{
	...
	SFile << static_cast<DWORD>(m_emVIPLevel);
...
BOOL SNpcTalkCondition::LOAD_0011 ( basestream& SFile )
{
	DWORD dwData;
	int nCount(0);
	...
	SFile >> dwData;	
	m_emVIPLevel = static_cast<EMVIP_LEVEL>(dwData);

	return TRUE;
}

BOOL SNpcTalkCondition::LOAD_0012 ( basestream& SFile )
{
	DWORD dwData;
	int nCount(0);
...
BOOL SNpcTalkCondition::LOAD ( basestream& SFile )
{
	...
	case SNpcTalkCondition::VERSION:	
		LOAD_0012(SFile);	
		break;
...
SNpcTalkCondition&	SNpcTalkCondition::operator= ( const SNpcTalkCondition& rNpcTalkCondition )
{
	...
	m_emVIPLevel = rNpcTalkCondition.m_emVIPLevel;

	m_dwRebornReq = rNpcTalkCondition.m_dwRebornReq;
...
BOOL SNpcTalkCondition::DoTEST ( GLCHARLOGIC_SERVER *pCHAR )
{
	BOOL bValid(FALSE);
	SNATIVEID sNID;
	...
	if ( RPARAM::bVIPSystem && m_emVIPLevel != EMVIP_LEVEL_NONE )
	{
		bValid = ( static_cast<int>(pCHAR->m_emVIPLevel) >= static_cast<int>(m_emVIPLevel) );
		if ( !bValid )	
			return FALSE;
	}

	if ( RPARAM::bRBSystem && m_dwRebornReq != 0 )
	{
		bValid = ( pCHAR->m_dwReborn >= m_dwRebornReq );
		if ( !bValid )	
			return FALSE;
	}
...
BOOL SNpcTalkCondition::DoTEST ( GLCHARLOGIC_CLIENT *pCHAR )
{
	BOOL bValid(FALSE);
	SNATIVEID sNID;
	...
	if ( m_dwQUEST_NID!=UINT_MAX )
	{
		...
		if ( !pQPROC )								
			return FALSE;
		if ( pQPROC->m_dwSTEP!=m_dwQUEST_STEP )		
			return FALSE;
	}
	...
	//	소지 스킬 검사.
	{
		...
		{
			sNID.dwID = (*iter).dwNID;
			if ( !pCHAR->ISLEARNED_SKILL ( sNID, (*iter).wLevel ) )	
				return FALSE;
		}
	}
	...
	/*npc talk buff condition, Juver, 2017/10/03 */
	{
		...
		{
			sNID.dwID = (*iter).dwID;
			if ( !pCHAR->ISHAVE_BUFF ( sNID, (*iter).wLevel ) )	
				return FALSE;
		}
	}

	//	필요 소지 금액.
	if ( pCHAR->m_lnMoney < m_dwMONEY )		
		return FALSE;
...
BOOL SNpcTalkCondition::EmptyCondition()
{
	...
	if( m_emVIPLevel != EMVIP_LEVEL_NONE )
		return FALSE;
...
void SNpcTalkCondition::GetConditionText( std::string& strName )
{
	...
	if ( m_emVIPLevel!=EMVIP_LEVEL_NONE )
	{
		strCondition << "VIP : " << m_emVIPLevel << COMMENT::CHAR_VIP[m_emVIPLevel].c_str() << "Character Value" << "\r\n";
	}

	if ( m_dwRebornReq!=0 )
	{
		strCondition << "RB : " << m_dwRebornReq << "\r\n";
	}
