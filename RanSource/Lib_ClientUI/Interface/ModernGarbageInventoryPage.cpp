...
const INT CModernGarbageInventoryPage::nSTARTLINE = 0;	
const INT CModernGarbageInventoryPage::nOUTOFRANGE = -1;

CModernGarbageInventoryPage::CModernGarbageInventoryPage()
...
void CModernGarbageInventoryPage::CreateSubControl()
{
	CString strSlotKeyword[ nMAX_ONE_VIEW_SLOT ] =
	{
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT0",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT1",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT2",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT3",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT4",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT5",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT6",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT7",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT8",
		"MODERN_GARBAGE_INVENTORY_WINDOW_ITEM_SLOT9",
...
CItemSlotEx* CModernGarbageInventoryPage::CreateItemSlot( CString strKeyword, UIGUID ControlID )
{
...
CUIControl*	CModernGarbageInventoryPage::CreateControl( const TCHAR* szControl )
{
...
void CModernGarbageInventoryPage::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
void CModernGarbageInventoryPage::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	if( ITEM_SLOT0 <= ControlID && ControlID < ITEM_SLOT_END )
	{
		if( CHECK_MOUSE_IN( dwMsg ) )
		{
			...
			CItemSlotEx* pSlot = m_pItemSlotArray[ nPosY ];
...
void CModernGarbageInventoryPage::ResetItemSlotRender ( int nStart, int nEnd )
{
	if ( nEnd < 0 )			return;
...
void CModernGarbageInventoryPage::SetItemSlotRender ( int nStart, int nEnd )
{
	if ( nEnd < 0 ) return ;
	if ( nStart > nEnd ) return ;
...
void CModernGarbageInventoryPage::SetItemIndex( INT nPosX, INT nPosY )
{
	m_nPosX = nPosX;
	m_nPosY = nPosY;
}

void CModernGarbageInventoryPage::GetItemIndex( INT* pnPosX, INT* pnPosY )
{
	*pnPosX = m_nPosX;
	*pnPosY = m_nPosY;
}

void CModernGarbageInventoryPage::LoadItemPage( GLInventory &ItemData )
{
...
void CModernGarbageInventoryPage::LoadItem( SINVENITEM& ref_InvenItem )
{	
...
void CModernGarbageInventoryPage::UnLoadItemPage()
{
...
void CModernGarbageInventoryPage::UnLoadItem( INT nPosX, INT nPosY )
{
	m_pItemSlotArray[ nPosY ]->ResetItemImage( nPosX );
}

void CModernGarbageInventoryPage::ResetAllFlipItem()
{
	for( WORD y=0; y<EM_INVENSIZE_Y; ++y )
		for( WORD x=0; x<EM_INVENSIZE_X; ++x )
...
void CModernGarbageInventoryPage::UpdateFlipItem( GLInventory& ref_Inventory )
{
	ResetAllFlipItem();
...
void CModernGarbageInventoryPage::UpdateFlipItem( SINVEN_POS& ref_GarbageItem )
{
...
void CModernGarbageInventoryPage::UpdateBLOCK()
{
...
SINVENITEM& CModernGarbageInventoryPage::GetItem( INT nPosX, INT nPosY )
{
	return m_pItemSlotArray[ nPosY ]->GetItemImage( nPosX );
}

void CModernGarbageInventoryPage::SetOneViewSlot( const INT& nMAX_ONE_VIEW )
{
