...
void CDlgSearch::SearchInit()
{
	...
	if ( emSearch == EMSEARCH_MID || emSearch == EMSEARCH_SID || emSearch == EMSEARCH_NAME || emSearch == EMSEARCH_CHF || emSearch == EMSEARCH_SALE || emSearch == EMSEARCH_TALK || 
		emSearch == EMSEARCH_GENITEM || emSearch == EMSEARCH_GENITEMQ || emSearch == EMSEARCH_GENID )
	{
			...
			for ( int MID = 0; MID < GLCrowDataMan::MAX_CROW_MID; MID++ )
			{
				for ( int SID = 0; SID < GLCrowDataMan::MAX_CROW_MID; SID++ )
				{
					...
					if ( pCROW )
					{
						...
						{
							std::string strFindString = pCROW->m_sGenerate.m_strGenItem[i];
			...
			for ( int MID = 0; MID < GLCrowDataMan::MAX_CROW_MID; MID++ )
			{
				for ( int SID = 0; SID < GLCrowDataMan::MAX_CROW_MID; SID++ )
				{
					...
					if ( pCROW )
					{
						...
						for ( int i=0; i<SCROWGEN::MAX_GEN_ITEM_ARRAY; ++i )
						{
							std::string strFindString = pCROW->m_sGenerate.m_strQtGenItem[i];
	...
	case EMSEARCH_GENID:
		{
			...
			for ( int MID = 0; MID < GLCrowDataMan::MAX_CROW_MID; MID++ )
			{
				for ( int SID = 0; SID < GLCrowDataMan::MAX_CROW_MID; SID++ )
				{
					...
					if ( pCROW )
					{
						for ( unsigned int n=0; n<pCROW->m_vecGenItem.size(); ++n )
						{
							const SGENITEM &gen_item = pCROW->m_vecGenItem[n];

							for ( int gen=0; gen<SGENITEM::EMGSPEC; ++gen )
							{
								if ( gen_item.m_sItemSpec[gen].dwSPECID == (DWORD)nSEARCH )
								{
									SSEARCH sResult;
									sResult.sID = pCROW->m_sBasic.sNativeID;
									sResult.strNAME = pCROW->GetName();
								...
								if ( gen_item.m_sItemSpecSkill[gen].dwSPECID == (DWORD)nSEARCH )
								{
									SSEARCH sResult;
									sResult.sID = pCROW->m_sBasic.sNativeID;
									sResult.strNAME = pCROW->GetName();
									sResult.strFIND = pCROW->m_sGenerate.m_strGenItem[n].c_str();
									vec.push_back( sResult );
								}
							}
						}
