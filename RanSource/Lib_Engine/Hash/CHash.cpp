...
CString CHash::DoHash()
{
	...
	if (hashFormatting == LOWERCASE_DASHES || hashFormatting == UPPERCASE_DASHES)
	{
		const int nLength = retHash.GetLength ();

		for ( int i = nLength - 2; 0 < i; i -= 2 )
		{
			retHash.Insert ( i, "-" );
		}
	}
	...
	if (hashFormatting == UPPERCASE_NOSPACES || hashFormatting == UPPERCASE_SPACES || hashFormatting == UPPERCASE_DASHES)
	{
		retHash.MakeUpper();
	}
...
{
	if (Style == 1 || Style == 2 || Style == 3 || Style == 4 || Style == 5 || Style == 6)
	{
