...
HRESULT DxSkinPiece::LoadPiece( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	...
	if ( dwVer >= VERSION_ENCRYPT )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_SKIN_PIECE_VER_1 );

	if ( dwVer >= ENCODE_VER_BYTE_CRYPT_2021_SKIN_PIECE_V1 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_SKIN_PIECE_V1 );

	switch ( dwVer )
	{
	case VERSION_CPS:
		hr = LoadPiece_0203( SFile, pd3dDevice, bThread );
		break;

	case 0x0202:
		hr = LoadPiece_0202( SFile, pd3dDevice, bThread );
		break;
...
HRESULT DxSkinPiece::LoadPiece_0201 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	...
	SFile >> dwSize;
	//m_emType = (EMPIECECHAR) dwSize;
	EMPIECECHAR_0201 emPIECEOLD = (EMPIECECHAR_0201) dwSize;
	m_emType = PIECE_0201_TO_PIECE( emPIECEOLD );
...
HRESULT DxSkinPiece::LoadPiece_0202 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr(S_OK);
...
HRESULT DxSkinPiece::LoadPiece_0203 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr(S_OK);
...
HRESULT DxSkinPiece::R2LoadPiece( const char* szFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr;
	...
	{
		if ( strlen( szFile ) > 0 )
			CDebugSet::ErrorFile( "DxSkinPiece::R2LoadPiece", szFile );
	...
	{
	case 0x0108:
		hr = R2LoadPiece_0108( SFile, pd3dDevice, bThread );
		break;

	case 0x0107:
		hr = R2LoadPiece_0107( SFile, pd3dDevice, bThread );
		break;

	case 0x0106:
		hr = R2LoadPiece_0106( SFile, pd3dDevice, bThread );
		break;

	case 0x0105:
		hr = R2LoadPiece_0105( SFile, pd3dDevice, bThread );
		break;

	case 0x0104:
		hr = R2LoadPiece_0104( SFile, pd3dDevice, bThread );
		break;

	case 0x0103:
		hr = R2LoadPiece_0103( SFile, pd3dDevice, bThread );
		break;

	case 0x0102:
		hr = R2LoadPiece_0102( SFile, pd3dDevice, bThread );
		break;

	case 0x0101:
		hr = R2LoadPiece_0101( SFile, pd3dDevice, bThread );
		break;

	default:
		{
			CDebugSet::ErrorVersion ( "DxSkinPiece::R2LoadPiece", SFile.GetFileName(), dwVer );
			return E_FAIL;
		}break;
	};
	...
	if ( FAILED(hr) )	
	{
		CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece", "Error Load SkinPiece :%s", m_szFileName );
		return hr;
	}
...
HRESULT DxSkinPiece::R2LoadPiece_0101 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0101", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}

		pEffChar->SetLinkObj ( NULL, this );
		hr = pEffChar->R2LoadFile ( SFile, pd3dDevice );
		if (FAILED(hr))
		{
			SAFE_DELETE(pEffChar);
			continue;
...
HRESULT DxSkinPiece::R2LoadPiece_0102 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0102", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}
...
HRESULT DxSkinPiece::R2LoadPiece_0103 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0103", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}
...
HRESULT DxSkinPiece::R2LoadPiece_0104 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	if ( m_dwMaterialNum!=0 )
	{
		...
		for ( DWORD i=0; i<m_dwMaterialNum; ++i )
		{
			m_pMaterialPiece[i].R2LoadFile ( SFile, pd3dDevice );
		}
	}
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0104", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}
...
HRESULT DxSkinPiece::R2LoadPiece_0105 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0105", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}
...
HRESULT DxSkinPiece::R2LoadPiece_0106 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0106", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}
...
HRESULT DxSkinPiece::R2LoadPiece_0107 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
	...
	for ( DWORD i=0; i<dwSize; i++ )
	{
		...
		if ( !pEffChar )	
		{
			CDebugSet::ErrorCode( "DxSkinPiece::R2LoadPiece_0107", "Unable to DxEffCharMan::GetInstance().CreateEffInstance  TypeID:%d", TypeID );
			return E_FAIL;
		}
...
HRESULT DxSkinPiece::R2LoadPiece_0108 ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, const BOOL bThread )
{
	HRESULT hr = S_OK;
