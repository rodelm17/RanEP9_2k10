...
#include "SelectCharacterChangePass.h"
...
int	CSelectCharacterChangePass::nLIMIT_ID = 11;
int	CSelectCharacterChangePass::nLIMIT_PW = 11;

CSelectCharacterChangePass::CSelectCharacterChangePass ()
	: nLIMIT_CAPTCHA(6)
	, strCaptcha( "" )
	, pTextBoxCaptcha( NULL )
	, pTextBoxLvlPass( NULL )
{
}
...
void CSelectCharacterChangePass::CreateSubControl ()
{
	...
	pTextBox->SetOneLineText ( "New-Password" );
	...
	pTextBoxLvlPass = CreateStaticControl ( "REG_PAGE_PW_LVL", pFont9, NS_UITEXTCOLOR::WHITE, TEXT_ALIGN_CENTER_X );
	{
...

	{
		...
		pEditBoxMan->CreateEditBox ( SELCHAR_EDIT_NEWPW, "CHARPAGE_CHANGE_PASS_NEWPW_EDIT", "REG_CARRAT", TRUE, UINT_MAX, pFont9, nLIMIT_PW );
		pEditBoxMan->CreateEditBox ( SELCHAR_EDIT_NEWPW2, "CHARPAGE_CHANGE_PASS_NEWPW2_EDIT", "REG_CARRAT", TRUE, UINT_MAX, pFont9, nLIMIT_PW );
		...
		pEditBoxMan->SetHide ( SELCHAR_EDIT_NEWPW, TRUE );
		pEditBoxMan->SetHide ( SELCHAR_EDIT_NEWPW2, TRUE );
...
void CSelectCharacterChangePass::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{	
...
void CSelectCharacterChangePass::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case SELCHAR_BUTTON_OK:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				std::string strNEWPW = m_pEditBoxMan->GetEditString ( SELCHAR_EDIT_NEWPW );
				std::string strNEWPW2 = m_pEditBoxMan->GetEditString ( SELCHAR_EDIT_NEWPW2 );
				...
				if ( !CheckString ( strPW.c_str() ) || !CheckString ( strPIN.c_str() ) || !CheckString ( strNEWPW.c_str() ) || !CheckString ( strNEWPW2.c_str() ) )
				{
				...
				//std::transform ( strPW.begin(), strPW.end(), strPW.begin(), tolower );
				//std::transform ( strPIN.begin(), strPIN.end(), strPIN.begin(), tolower );
				//std::transform ( strNEWPW.begin(), strNEWPW.end(), strNEWPW.begin(), tolower );

				pNetClient->SndAccountChangePass( strPW.c_str(), strPIN.c_str(), strNEWPW.c_str() );		
...
void CSelectCharacterChangePass::ResetAll ( bool bEdit )
{
	m_pEditBoxMan->EndEdit ();

	m_pEditBoxMan->ClearEdit ( SELCHAR_EDIT_PW );
	m_pEditBoxMan->ClearEdit ( SELCHAR_EDIT_PIN );
	m_pEditBoxMan->ClearEdit ( SELCHAR_EDIT_NEWPW );
	m_pEditBoxMan->ClearEdit ( SELCHAR_EDIT_NEWPW2 );
...
CBasicButton* CSelectCharacterChangePass::CreateFlipButton ( char* szButton, char* szButtonFlip, UIGUID ControlID )
{
...
BOOL CSelectCharacterChangePass::CheckString( CString strTemp )
{	
