...
struct SSKILLACTEX
{
	...
	float		fPositionShiftSpeed;
	bool		bPositionShiftSwap;
	bool		bPosisionShiftDash;
	bool		bPositionShiftPull;
	...
	SSKILLACTEX () :
		...
		/*skill teleport, Juver, 2018/09/06 */
		bUseTeleport(false),
		vTeleportPos(0.0f,0.0f,0.0f),

		/* skill position shift, Juver, 2020/12/15 */
		fPositionShiftSpeed(0.0f),
		bPositionShiftSwap(false),
		bPosisionShiftDash(false),
		bPositionShiftPull(false)
	...
	bool VALID() const
	{
		...
		if ( bCrushBlow == true )				return true;
		if ( bPushPull == true )				return true;
		if ( bCrushingBlow == true )			return true;
		if ( bPush == true )					return true;	/* push skill logic, Juver, 2020/12/14 */
		if ( bPull == true )					return true;	/* pull skill logic, Juver, 2020/12/14 */
		...
		if ( bDash == true)						return true;	/* dash skill logic, Juver, 2020/12/15 */
		if ( bUseTeleport == true )				return true;	/*skill teleport, Juver, 2018/09/06 */

		if ( bPositionShiftSwap == true )		return true;	/* skill position shift, Juver, 2020/12/15 */
		if ( bPosisionShiftDash == true )		return true;	/* skill position shift, Juver, 2020/12/15 */
		if ( bPositionShiftPull == true )		return true;	/* skill position shift, Juver, 2020/12/15 */
...
public:
	...
	GLCROWLIST* GetGlobBossCrowList ()				{ return &m_GlobBOSSCROWList; }

	bool IsInstantMapStuck()						{ return m_bInstantMapStuck; }
public:
...
public:
	...
	/* dash skill logic, Juver, 2020/12/14 */
	void DashAct( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED, const DWORD _dwActionAnim );

	/*skill teleport, Juver, 2018/09/06 */
	void SkillTeleport( const STARGETID _dwActorID, const STARGETID _dwTargetID, const D3DXVECTOR3 &_vTargetPos );

	/* skill position shift, Juver, 2020/12/15 */
	void PositionShiftSwap( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED );
	void PositionShiftDash( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED );
	void PositionShiftPull( const STARGETID _dwActorID, const STARGETID _dwTargetID, const float _fSPEED );
...
public:
	...
	BOOL DoGateOutPCAll ( DWORD dwExceptClubID, DWORD dwGateID, DWORD dwToIndex );
	BOOL DoGateOutPCAll ( DWORD dwGateID, DWORD dwToIndex );
