...
void GLCROWLOGIC_CLIENT::RESET_DATA ()
{
...
const char* GLCROWLOGIC_CLIENT::GETNAME ()
{
	GASSERT(m_pCrowData);
	return m_pCrowData->GetName();
}

int GLCROWLOGIC_CLIENT::GETFORCE_LOW () const
{
...
int GLCROWLOGIC_CLIENT::GETFORCE_HIGH () const
{
...
/*skill range spec logic, Juver, 2017/06/06 */
WORD GLCROWLOGIC_CLIENT::GETSKILLRANGE_APPLY ( const GLSKILL &sSKILL, const WORD dwLEVEL ) const
{
...
BOOL GLCROWLOGIC_CLIENT::INIT_DATA ()
{
...
DWORD GLCROWLOGIC_CLIENT::RECEIVE_DAMAGE ( const WORD wDamage )
{
...
void GLCROWLOGIC_CLIENT::UPDATE_DATA ( float fTime, float fElapsedTime, BOOL bClient )
{
...
float GLCROWLOGIC_CLIENT::GETATTVELO ()
{
	return m_fATTVELO<0.0f?0.0f:m_fATTVELO;
}

float GLCROWLOGIC_CLIENT::GETMOVEVELO ()
{
	float fMOVE = m_fSTATE_MOVE + m_fSKILL_MOVE;
	return fMOVE<0.0f?0.0f:fMOVE;
}

void GLCROWLOGIC_CLIENT::SETACTIVESKILL ( SNATIVEID skill_id, WORD wLevel )
{
	m_idACTIVESKILL = skill_id;
	m_wACTIVESKILL_LVL = wLevel;
}

EMSKILLCHECK GLCROWLOGIC_CLIENT::CHECHSKILL ( DWORD dwAType,  bool bNotLearn )
{
...
void GLCROWLOGIC_CLIENT::ACCOUNTSKILL ( WORD wStrikeNum )
{
	//	스킬 정보 가져옴.
...
/*continuous damage skill logic, Juver, 2017/06/10 */
BOOL GLCROWLOGIC_CLIENT::CONTINUOUSDAMAGE_EXIST( SNATIVEID _sID )
{
...
/*continuous damage skill logic, Juver, 2017/06/10 */
void GLCROWLOGIC_CLIENT::CONTINUOUSDAMAGE_INSERT( SCONTINUOUS_DAMAGE_DATA _sDATA )
{
...
/*continuous damage skill logic, Juver, 2017/06/10 */
void GLCROWLOGIC_CLIENT::CONTINUOUSDAMAGE_DELETE( SNATIVEID _sID )
{
