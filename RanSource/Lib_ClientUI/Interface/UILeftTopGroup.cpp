...
#include "./BasicRebornDisplay.h"		/* reborn system, Juver, 2021/09/17 */
...
// #endif
	...
	if ( RPARAM::bRBSystem == FALSE )
		pRebornDisplay->SetVisibleSingle( FALSE );
}
...
void	CUILeftTopGroup::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( pCharacter )
	{
		...
		if ( UIKeyCheck::GetInstance()->Check (RPROFILE::QuickSlot[0], DXKEY_DOWN ) ) nIndex = 0;
		if ( UIKeyCheck::GetInstance()->Check (RPROFILE::QuickSlot[1], DXKEY_DOWN ) ) nIndex = 1;
		if ( UIKeyCheck::GetInstance()->Check (RPROFILE::QuickSlot[2], DXKEY_DOWN ) ) nIndex = 2;
		if ( UIKeyCheck::GetInstance()->Check (RPROFILE::QuickSlot[3], DXKEY_DOWN ) ) nIndex = 3;
		if ( UIKeyCheck::GetInstance()->Check (RPROFILE::QuickSlot[4], DXKEY_DOWN ) ) nIndex = 4;
		if ( UIKeyCheck::GetInstance()->Check (RPROFILE::QuickSlot[5], DXKEY_DOWN ) ) nIndex = 5;
	...
	case BASIC_QUICK_POTION_TRAY:
		{
			...
			if ( UIMSG_TRAY_SETTING_LBUP & dwMsg )
			{
				CInnerInterface::GetInstance().OpenAutoPotionWindow();
			}
		}
		break;
	}
