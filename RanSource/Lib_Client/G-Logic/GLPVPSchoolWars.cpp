...
bool GLPVPSchoolWars::Load( std::string strFile )
{
	...
	m_mapCrowScore.clear();

	DWORD crow_num = cFILE.GetKeySize( "EVENT", "CROW_SCORE" );
	for( DWORD i=0; i<crow_num; ++i )
	{
		SCHOOLWARS_CROW_SCORE score;
		cFILE.getflag( i, "EVENT", "CROW_SCORE", 0, 4, score.crow.wMainID );
		cFILE.getflag( i, "EVENT", "CROW_SCORE", 1, 4, score.crow.wSubID );
		cFILE.getflag( i, "EVENT", "CROW_SCORE", 2, 4, score.score_total );
		cFILE.getflag( i, "EVENT", "CROW_SCORE", 3, 4, score.score_each );
		m_mapCrowScore.insert( std::make_pair(score.crow.dwID, score) );
	}
	...
	for( DWORD i=0; i<dwnum; ++i )
	{
		...
		cFILE.getflag( i, "EVENT", "BATTLE_TIME", 0, 4, sched.wStartDay );
		cFILE.getflag( i, "EVENT", "BATTLE_TIME", 1, 4, sched.wStartHour );
		cFILE.getflag( i, "EVENT", "BATTLE_TIME", 2, 4, sched.wStartMinute );
		cFILE.getflag( i, "EVENT", "BATTLE_TIME", 3, 4, sched.bCombatRecord );
...
SCHOOLWARS_CROW_SCORE* GLPVPSchoolWars::GetCrowScore( SNATIVEID idcrow )
{
	if ( idcrow == NATIVEID_NULL() )	return NULL;

	SCHOOLWARS_CROW_SCORE_MAP_ITER it = m_mapCrowScore.find( idcrow.dwID );
	if( it != m_mapCrowScore.end() ){
		return &(*it).second;
	}
...
BOOL GLPVPSchoolWars::SetContributionPointMultiplier( float fMulti )
{
	m_fContributionPointMultiplier = fMulti;
	return TRUE;
}

float GLPVPSchoolWars::GetContributionPointMultiplier()
{
