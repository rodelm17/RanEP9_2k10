...
//--------------------------------------------------------------------
CEP7ItemMixInvenWindow::CEP7ItemMixInvenWindow()	// »ﬁ¡ˆ≈Î
	: m_pPage( NULL )
...
CEP7ItemMixInvenWindow::~CEP7ItemMixInvenWindow()
{
}

CMultiModeButton* CEP7ItemMixInvenWindow::CreateRadioButton( char* szControl, char* szText, const UIGUID& cID /*= NO_ID*/ )
{
	...
	pControl->CreateTextBox( "EP7_ITEMMIX_INVENTORY_WINDOW_RADIO_BUTTON_TEXT", 9, TEXT_ALIGN_CENTER_X | TEXT_ALIGN_CENTER_Y, _DEFAULT_FONT_SHADOW_FLAG );
...
VOID CEP7ItemMixInvenWindow::CreateSubControl()
{
	m_pPage = new CEP7ItemMixInvenPage;
	m_pPage->CreateSub( this, "EP7_ITEMMIX_INVENTORY_PAGE", UI_FLAG_YSIZE, ITEMMIXINVEN_PAGE );
	m_pPage->CreateSubControl();
	m_pPage->SetOneViewSlot ( EP7_ITEMMIX_INVEN_DEFAULT_VIEW );
	RegisterControl( m_pPage );

	CUIControl* pControl = CreateControl( "EP7_ITEMMIX_INVENTORY_MONEY_BACK" );
	pControl->SetAlignFlag( UI_FLAG_BOTTOM );

	CBasicButton* pButton = CreateFlipButton( "EP7_ITEMMIX_INVENTORY_MONEY_BUTTON", "EP7_ITEMMIX_INVENTORY_MONEY_BUTTON_F", ITEMMIXINVEN_MONEY_BUTTON, CBasicButton::CLICK_FLIP );
	pButton->CreateMouseOver( "EP7_ITEMMIX_INVENTORY_MONEY_BUTTON_F" );
	pButton->SetUseDynamic( TRUE );
	pButton->SetAlignFlag( UI_FLAG_BOTTOM );

	CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
	m_pMoneyTextBox = CreateStaticControl( "EP7_ITEMMIX_INVENTORY_MONEYTEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
	m_pMoneyTextBox->SetAlignFlag( UI_FLAG_BOTTOM );

#if defined( BUILD_EP7 )
	{
		CUIControl* pControl = CreateControl ( "EP7_ITEMMIX_INVENTORY_GAME_POINTS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pGamePointsTextBox = CreateStaticControl ( "EP7_ITEMMIX_INVENTORY_GAME_POINTS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pGamePointsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "EP7_ITEMMIX_INVENTORY_GAME_POINTS_BUTTON", "EP7_ITEMMIX_INVENTORY_GAME_POINTS_BUTTON_F", ITEMMIXINVEN_GAME_POINTS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "EP7_ITEMMIX_INVENTORY_GAME_POINTS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}

	{
		CUIControl* pControl = CreateControl ( "EP7_ITEMMIX_INVENTORY_WAR_CHIPS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pWarChipsTextBox = CreateStaticControl ( "EP7_ITEMMIX_INVENTORY_WAR_CHIPS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pWarChipsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "EP7_ITEMMIX_INVENTORY_WAR_CHIPS_BUTTON", "EP7_ITEMMIX_INVENTORY_WAR_CHIPS_BUTTON_F", ITEMMIXINVEN_WAR_CHIPS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "EP7_ITEMMIX_INVENTORY_WAR_CHIPS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}
#endif
	...
	{
		"EP7_ITEMMIX_INVENTORY_WINDOW_RADIO_BUTTON_1",
		"EP7_ITEMMIX_INVENTORY_WINDOW_RADIO_BUTTON_2",
	};
...
VOID CEP7ItemMixInvenWindow::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
...
VOID CEP7ItemMixInvenWindow::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	switch( ControlID )
	{
...
HRESULT CEP7ItemMixInvenWindow::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
void CEP7ItemMixInvenWindow::SetVisibleSingle ( BOOL bVisible )
{
...
VOID CEP7ItemMixInvenWindow::SetOneViewSlot( const INT& nONE_VIEW_SLOT )
{
}

void CEP7ItemMixInvenWindow::OpenInvenPage ( int nPage )
{
	if ( nPage < 0 )	return;
