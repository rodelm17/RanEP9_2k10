...
#include "./G-Logic/GLLandMan.h"
#include "./G-Logic/GLMobSchedule.h"
#include "./G-Logic/GLogicData.h"

#include "./G-Logic/GLQuestMan.h"
#include "./G-Logic/GLPartyClient.h"
#include "./G-Logic/GLFriendClient.h"

#include "./G-Logic/GLBusStation.h"
#include "./G-Logic/GLTaxiStation.h"
#include "./G-Logic/GLItemMixMan.h"
...
#include "./G-Logic/GLStringTable.h"
#include "./G-Logic/GLItemMan.h"

#include "../Lib_Engine/DxCommon/D3DFont.h"
#include "./G-Logic/Emoticon.h"
#include "../Lib_Engine/Utils/RanFilter.h"
#include "../Lib_Engine/Common/Unzipper.h"
...
#include "../Lib_Engine/GUInterface/UIConfigMgr.h"
#include "../Lib_ClientUI/Interface/LoadingThread.h"
#include "../Lib_Engine/DxCommon/DxFontMan.h"

/*pvp tyranny, Juver, 2017/08/24 */
#include "./G-Logic/GLPVPTyrannyClient.h"

/*activity system, Juver, 2017/10/21 */
#include "./G-Logic/GLActivity.h"
/* Codex, Archie 02/16/24 */
#include "./G-Logic/GLCodex.h"

/*school wars, Juver, 2018/01/19 */
#include "./G-Logic/GLPVPSchoolWarsClient.h"

/*12-9-14, Battle Pass - CNDev*/
#include "./G-Logic/GLBattlePass.h"

/*pvp capture the flag, Juver, 2018/01/24 */
#include "./G-Logic/GLPVPCaptureTheFlagClient.h"

/*skill transform, Juver, 2018/09/09 */
#include "./G-Logic/GLTransformSkinSet.h"

#include "./G-Logic/PVPClubDeathMatchClient.h"

/* award title, Juver, 2021/02/13 */
#include "./G-Logic/GLAwardTitleClient.h"
/*12-16-24, daily login - CNDev*/
#include "./G-Logic/GLDailyLogin.h"

#include "./MShieldGlobal.h"

#include "./G-Logic/GLPVPWoeClient.h"
...
#include "./G-Logic/PVPPartyBattleGroundsClient.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif

/* master login, Juver, 2020/06/11 */
bool g_bMasterClient = false;
...
void DxGlobalStage::OnInitLogin ()
{
	...
	GLGaeaClient::GetInstance().PartyFinderReset();
...
HRESULT DxGlobalStage::OneTimeSceneInit ( const char* szAppPath, HWND hWnd, BOOL bEmulate, WORD wWidth, WORD wHeight, const char* szMapList )
{
	...
	if ( !bEmulate )
	{
		...
		/* Codex, Archie 02/16/24 */
		GLCodex::GetInstance().SetPath( szFullPath );
		
		/*12-9-14, Battle Pass - CNDev*/
		GLBattlePass::GetInstance().SetPath( szFullPath );
	...
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIOuterCfg01.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIOuterCfg02.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIOuterCfg03.GetString(), RPARAM::strCountry );

	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIInnerCfg01.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIInnerCfg02.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIInnerCfg03.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIInnerCfg04.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIInnerCfg05.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIInnerCfg06.GetString(), RPARAM::strCountry );

	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIExtCfg01.GetString(), RPARAM::strCountry );
	CInterfaceCfg::GetInstance().LoadText(RPARAM::strUIExtCfg02.GetString(), RPARAM::strCountry );

	CUIConfigMgr::GetInstance().SetPath(szFullPath);
	CUIConfigMgr::GetInstance().LoadTextureList( RPARAM::strUITextureList.GetString() );
	CUIConfigMgr::GetInstance().LoadControlList( RPARAM::strUIXmlList.GetString(), RPARAM::strCountry );
...
HRESULT DxGlobalStage::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	//     DxSkinCharDataContainer::GetInstance().LoadData( skin_file.c_str(), m_pd3dDevice, TRUE );  // ORIGINAL (SLOW)
	// }
	...
	{
		std::string skin_file = (*iter).second;
		DxSkinCharDataContainer::GetInstance().LoadData( skin_file.c_str(), m_pd3dDevice, FALSE );  // OPTIMIZED (FAST)
	}
	...
	if ( !m_bEmulate )
	{
		...
		if ( m_NetClient.ConnectLoginServer(RPARAM::sLoginAddress) <= NET_ERROR )
...
HRESULT DxGlobalStage::FrameMove ( float m_fTime, float m_fElapsedTime )
{
	...
	if ( hs_close_game )
	{
		...
		if ( hs_close_timer >= RPARAM::hs_close_timer )
		{
			hs_close_timer = 0.0f;
			hs_close_game = FALSE;

			hs_send_close_request();
		}
	}
#endif
...
void DxGlobalStage::MsgProcessFrame ( NET_MSG_GENERIC *nmg )
{
	...
	case NET_MSG_LOBBY_CHAR_ACTIVITY_DONE: 
		...
		/*12-9-14, Battle Pass - CNDev*/
	case NET_MSG_LOBBY_CHAR_BATTLEPASS_PROG: 
	case NET_MSG_LOBBY_CHAR_BATTLEPASS_DONE: 
	case NET_MSG_LOBBY_CHAR_BATTLEPASS_REWARD_DATA:

		/* personal lock system, Juver, 2019/12/06 */
	case NET_MSG_LOBBY_CHAR_INFO_PERSONAL_LOCK:

		/* pk combo, Juver, 2021/02/09 */
	case NET_MSG_LOBBY_CHAR_INFO_PK_COMBO_COUNT:

	case NET_MSG_LOBBY_CHAR_INFO_COMBAT_RECORD:
	
		/*12-16-24, daily login - CNDev*/
	case NET_MSG_LOBBY_CHAR_DAILYLOGIN_DATA:
	...
	/* Codex, Archie 02/16/24 */
	case NET_MSG_LOBBY_CHAR_CODEX_PROG: 
	case NET_MSG_LOBBY_CHAR_CODEX_DONE: 
	...
	case NET_MSG_GCTRL_REGISTER_CODEX_FB: /* Codex, Archie 02/16/24 */
	...
	case NET_MSG_GCTRL_TYRANNY_RANK_POINT_UPDATE:
	...
	case NET_MSG_GCTRL_CLUBWAR_RANK_POINT_UPDATE:
	...
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_RESU_AGT:/*Top Resu, Mhundz */
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_MMR_AGT:/*Top MMR, Mhundz */
	/////////////////////////////////////////////////////////////////////////////
	...
	/* Codex, Archie 02/16/24 */
	case NET_MSG_GCTRL_CODEX_COMPLETE_BRD:
	...
	case NET_MSG_GCTRL_TYRANNY_A2C_LATE_JOIN_FB:
	case NET_MSG_GCTRL_TYRANNY_F2C_GUILD_TOWER_CAPTURE:
		{
	...
	case NET_MSG_GCTRL_WOE_F2C_TOWER_WINNER:
	...
	case NET_MSG_WOE_FIRST_CAPTURE:
	case NET_MSG_WOE_A2C_DECLARE_WINNER:
	case NET_MSG_WOE_A2C_TOWER_WINNER:
	{
		GLPVPWoeClient::GetInstance().MsgProcess(nmg);
	}break;
	...
	case NET_MSG_GCTRL_SCHOOLWARS_F2C_MOB_KILL_NOTICE:
		{
	...
	case NET_MSG_GCTRL_CHARACTER_DISCONNECT_AGENT_TO_CHAR:
		{
			GLMSG::SNET_CHARACTER_DISCONNECT_AGENT_TO_CHAR* pnetmsg = ( GLMSG::SNET_CHARACTER_DISCONNECT_AGENT_TO_CHAR* ) nmg;

			GetNetClient()->CloseConnect();

			if ( pnetmsg->bCloseClient )
			{
				CloseGame();
			}
	...
	case NET_MSG_GCTRL_PVP_CLUB_DEATH_MATCH_F2C_DOUBLE_POINT_UPDATE:
		{
			PVPClubDeathMatchClient::GetInstance().MsgProcess( nmg );
		}break;

		/* award title, Juver, 2021/02/12 */
	case NET_MSG_GCTRL_A2C_AWARD_TITLE_UPDATE_DATA:
	case NET_MSG_GCTRL_A2C_AWARD_TITLE_UPDATE_DONE:
		{
			GLAwardTitleClient::GetInstance().MsgProcess ( nmg );
		}
		break;
	...
	case NET_MSG_GCTRL_PVP_PBG_F2C_RANKING_END:
		{
			PVPPBG::ManagerClient::GetInstance().MsgProcess( nmg );
		}
		break;
...
{
	if (ROPTION::bScrWndFullSize )	ToRestoreSize ();
	else								ToFullSize ();
...
{
	if ( !ROPTION::bScrWindowed )	return S_FALSE;

	ROPTION::bScrWndFullSize = TRUE;
...
{
	if ( !ROPTION::bScrWndHalfSize )	return S_FALSE;
	if ( !ROPTION::bScrWindowed )		return S_FALSE;

	ROPTION::bScrWndFullSize = FALSE;

	LONG nWindowStyle = WS_OVERLAPPED | WS_VISIBLE | WS_CAPTION |
						WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_CLIPCHILDREN;
	SetWindowLong ( m_hWnd, GWL_STYLE, nWindowStyle );

	int wndSizeX = (int)ROPTION::dwScrWidth;
	int wndSizeY = (int)ROPTION::dwScrHeight;
...
/*hackshield implementation, Juver, 2018/06/19 */
void DxGlobalStage::hs_send_close_request()
{
	if ( m_emThisStage != EM_STAGE_GAME )	return;

	GLMSG::SNETPC_HS_CLIENT_CLOSE msg;
	NETSENDTOFIELD ( &msg );
}

/*hackshield implementation, Juver, 2018/06/19 */
void DxGlobalStage::hs_send_run_status( DWORD hs_sdk_version )
{
	if ( m_emThisStage != EM_STAGE_GAME )	return;
}
#endif
...
void DxGlobalStage::MShieldLoginFB( std::string strUser )
{
	StringCchCopy( MShieldGlobal::szLastUser, MSHIELD_LAST_USER_SIZE, strUser.c_str() );
	::PostMessage ( m_hWnd, MSHIELD_LOGIN_FEEDBACK, 0, 0 );	
}

void DxGlobalStage::MShieldResetInfo()
{
	SecureZeroMemory (MShieldGlobal::szLastUser, MSHIELD_LAST_USER_SIZE );
	::PostMessage ( m_hWnd, MSHIELD_RESET_INFO_FEEDBACK, 0, 0 );	
}

void DxGlobalStage::MShieldRequestHandShake( UINT nKey )
{
	MShieldGlobal::nHandShakeRequestKey = nKey;
	//::PostMessage ( m_hWnd, MSHIELD_HANDSHAKE_REQUEST, 0, 0 );	
}

void DxGlobalStage::MShieldReplyHandShake()
{
	//GLMSG::SNETPC_MSHIELD_HANDSHAKE_REPLY NetMsg;
	//NetMsg.nReply = MShieldGlobal::nHandShakeResultKey;
	//NETSEND ( &NetMsg );
}
