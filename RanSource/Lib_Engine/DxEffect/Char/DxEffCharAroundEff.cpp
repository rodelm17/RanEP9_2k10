...
DWORD		DxEffCharAroundEff::TYPEID			= EMEFFCHAR_AROUNDEFFECT;
DWORD		DxEffCharAroundEff::VERSION			= 0x0001;
char		DxEffCharAroundEff::NAME[MAX_PATH]	= "DxEffCharAroundEff"; 

DxEffCharAroundEff::DxEffCharAroundEff(void)
{
	m_pTexture = NULL;	
}
...
DxEffChar* DxEffCharAroundEff::CloneInstance ( LPDIRECT3DDEVICEQ pd3dDevice, DxCharPart* pCharPart, DxSkinPiece* pSkinPiece )
{
	DxEffCharAroundEff *pEffChar = new DxEffCharAroundEff;
...
void DxEffCharAroundEff::InsertAroundEffect( SAROUNDEFF_DATA addData, bool bTool /* = FALSE */ )
{
	if( bTool )
	{
...
void DxEffCharAroundEff::DeleteAroundEffect( int nIndex )
{
	if( m_vecAroundEffList.size() < (size_t)nIndex ) return;
	m_vecAroundEffList.erase( m_vecAroundEffList.begin() + nIndex );
}

HRESULT DxEffCharAroundEff::CreateDevice ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	return S_OK;
}
HRESULT DxEffCharAroundEff::ReleaseDevice ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	return S_OK;
}
HRESULT DxEffCharAroundEff::OneTimeSceneInit ()
{
	return S_OK;
}
HRESULT DxEffCharAroundEff::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
HRESULT DxEffCharAroundEff::RestoreDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{

	return S_OK;
}
HRESULT DxEffCharAroundEff::InvalidateDeviceObjects ()
{
	return S_OK;
}
HRESULT DxEffCharAroundEff::DeleteDeviceObjects ()
{
	TextureManager::ReleaseTexture( m_szTexture, m_pTexture );
	return S_OK;
}
HRESULT DxEffCharAroundEff::FinalCleanup ()
{
	return S_OK;
}
...
HRESULT DxEffCharAroundEff::FrameMove ( float fTime, float fElapsedTime )
{
...
void DxEffCharAroundEff::RenderEff( LPDIRECT3DDEVICEQ pd3dDevice, LPD3DXMATRIX pMatrix, const float fScale )
{
	...
	{	
		...
		if( pBoneTran )
		{
			...
			if ( g_bPREVIEW_CHAR_RENDER )
			{
				...
				matLocal = DxBillboardLookAt( &vPos, matViewLocal );
			}
...
void DxEffCharAroundEff::RenderCountText( DWORD nCount, const D3DXVECTOR3& vPos )
{
	D3DXVECTOR3 vOut;
...
void DxEffCharAroundEff::RenderEDIT( LPDIRECT3DDEVICEQ pd3dDevice, LPD3DXMATRIX pMatrix, const float fScale )
{
...
HRESULT	DxEffCharAroundEff::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharAroundEff::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharAroundEff::SaveFile ( basestream &SFile )
{
	SFile << TYPEID;
	SFile << VERSION;
...
HRESULT	DxEffCharAroundEff::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharAroundEff::R2LoadFile", SFile.GetFileName(), dwVer );
