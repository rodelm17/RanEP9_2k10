...
CServer::CServer(
	...
	, m_dwFrameCount(0)
	, m_nServerType(0)
	, bRPCIDHashValid(FALSE)		/*ccfg security, Juver, 2018/07/08 */
{
...
{	
	...
	std::string strCfgFileName = p.filename().string().c_str();

	int	nRetCode = CCfg::GetInstance()->Load( strCfgFileName.c_str() );
	if (nRetCode != 0)
	{
...
/*ccfg security, Juver, 2018/07/08 */
int CServer::CheckRPCIDHash()
{
	std::string strCurrentHashCPU = md5( RPCID::getStringCPU() );
	std::string strCurrentHashLAN = md5( RPCID::getStringLAN() );
	std::string strCurrentHashDISK = md5( RPCID::getStringDISK() );
	std::string strCurrentHashPCNAME = md5( RPCID::getStringPCNAME() );
	std::string strCurrentHashCombined = md5( RPCID::getStringCombined() );

	std::string strCFGHashCPU = CCfg::GetInstance()->getHashCPU();
	std::string strCFGHashLAN = CCfg::GetInstance()->getHashLAN();
	std::string strCFGHashDISK = CCfg::GetInstance()->getHashDISK();
	std::string strCFGHashPCNAME = CCfg::GetInstance()->getHashPCNAME();
	std::string strCFGHashCOMBINED = CCfg::GetInstance()->getHashCOMBINED();

	if ( ( strCurrentHashCPU != strCFGHashCPU ) || 
		( strCurrentHashLAN != strCFGHashLAN ) ||  
		( strCurrentHashDISK != strCFGHashDISK ) || 
		( strCurrentHashPCNAME != strCFGHashPCNAME ) || 
		( strCurrentHashCombined != strCFGHashCOMBINED ) )
	{
		bRPCIDHashValid = FALSE;
		return NET_ERROR;
	}

	bRPCIDHashValid = TRUE;
