...
#include "../../Lib_ClientUI/Interface/SkillTrayTabDefine.h"
...
void GLCharacter::ReqStatsUpModern ( SCHARSTATS sStats, BOOL bCommand /*= FALSE*/)
{
	...
	DWORD dwTOTALSPI = m_sStats.wSpi + sStats.wSpi;
	DWORD dwTOTALSTR = m_sStats.wStr + sStats.wStr;
	DWORD dwTOTALSTA = m_sStats.wSta + sStats.wSta;

	BOOL bOVERPOW = dwTOTALPOW > GLCONST_CHAR::dwMaxPow;
	BOOL bOVERDEX = dwTOTALDEX > GLCONST_CHAR::dwMaxDex;
	BOOL bOVERSPI = dwTOTALSPI > GLCONST_CHAR::dwMaxSpi;
	BOOL bOVERSTR = dwTOTALSTR > GLCONST_CHAR::dwMaxStr;
	BOOL bOVERSTA = dwTOTALSTA > GLCONST_CHAR::dwMaxSta;
	BOOL bOVERINT = dwTOTALINT > GLCONST_CHAR::dwMaxInt;

	if ( bOVERPOW )	CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("OVERMAX_POW") );
	if ( bOVERDEX )	CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("OVERMAX_DEX") );
	if ( bOVERSPI )	CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("OVERMAX_SPI") );
	if ( bOVERSTR )	CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("OVERMAX_STR") );
	if ( bOVERSTA )	CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("OVERMAX_STA") );
	if ( bOVERINT )	CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("OVERMAX_INT") );

	if ( bOVERPOW || bOVERDEX || bOVERSPI || bOVERSTR || bOVERSTA || bOVERINT )
	{
	return;
	}*/
...
/*EP9 Large Map Renewal*/
void GLCharacter::ReqRetrieveMobInMap( SNATIVEID sMapID )
{
	if ( RPARAM::bRNLargeMapWindow == 1 )
	{
		CInnerInterface::GetInstance().GetLargeMapWindow()->SetLargeMap( sMapID, this );
	}
}
...
HRESULT GLCharacter::ReqClubInfoUpdate( bool bUpdate )
{
	if ( !bUpdate ) 
	{
		...
		{
			if ( CInnerInterface::GetInstance().IsVisibleGroup( CLUB_WINDOW ) || 
				 CInnerInterface::GetInstance().IsVisibleGroup( LARGEMAP_WINDOW2 ) ) 
				return S_OK;
		}
	}
...
void GLCharacter::ReqCodexRegister ( SNATIVEID sItemID, DWORD m_dwActivityID, WORD wGrade, WORD wQuantity )
{
	...
	//	Note : 서버에 상자 오픈을 요청.
	//
	GLMSG::SNET_REGISTER_CODEX NetMsg;
	NetMsg.sItemID = sItemID;
