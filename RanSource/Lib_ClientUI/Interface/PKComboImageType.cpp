...
CPKComboImageType::CPKComboImageType () :
    m_bPLAY ( false )
{
}
...
void CPKComboImageType::CreateSubControl ( CString strKeyword )
{
...
void	CPKComboImageType::START()
{
	RESET ();
	m_bPLAY = true;
}

void	CPKComboImageType::RESET()
{
	m_fPlayedTime = 0.0f;
...
void CPKComboImageType::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( ISPLAYING() )
	{
		...
		for ( int nIndex = 0; nIndex < XCONTROL_SIZE; ++nIndex )
		{
			...
			if ( m_fPlayedTime < m_fBEGIN_TIME[nIndex] ) bIsVISIBLE = true;
		}	
