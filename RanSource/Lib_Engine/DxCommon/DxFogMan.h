...
protected:
	BOOL	m_bManualRange;
	float	m_fManualRange_Near;
	float	m_fManualRange_Far;

	/* camera zoom gm command, Juver, 2020/06/11 */
	float	m_fRangeMultiplier;

public:
	void		RenderFogSB ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bLobby = FALSE );
...
public:
	void		ManualRangeSet( BOOL bENABLE, float fNear, float fFar );
	void		ManualRangeGet( float& fNear, float& fFar );
	BOOL		ManualRangeIsDefault();

	/* camera zoom gm command, Juver, 2020/06/11 */
	void		SetRangeMultiplier( float fVal );
