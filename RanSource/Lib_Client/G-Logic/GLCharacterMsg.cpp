...
#include "../Lib_ClientUI/BossDetailsWindow.h"
/////////////////////////////////////////////////////////////////////////////
// LG-7 GlobalRanking
...
HRESULT GLCharacter::UpdateClientState ( GLMSG::SNETPC_UPDATE_STATE *pNetMsg )
{
	...
	m_sMP = pNetMsg->sMP;
	m_sSP = pNetMsg->sSP;
	m_bSafeTime = pNetMsg->bSafeTime; /* SafeTime Logic - Montage, 3-28-25 */
...
void GLCharacter::ReceiveDamage ( WORD wDamage, DWORD dwDamageFlag, STARGETID sACTOR )
{
	...
	//	Note : 타격값 반영.
	GLCHARLOGIC_CLIENT::RECEIVE_DAMAGE ( wDamage );
...
void GLCharacter::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_RESU_AGT:
		{
			GLMSG::SNETPC_REQ_GLOBAL_RANKING_RESU_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_RESU_AGT*)nmg;

			if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
				for (int i = 0; i < 10; i++)
					if (pNetMsg->sTopResu_[i].dwChaNum > 0)
						CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingResu(pNetMsg->wPart, pNetMsg->sTopResu_[i], i);
		}
		break;
	/*Top MMR, Mhundz */
	case NET_MSG_GCTRL_REQ_GLOBAL_RANKING_MMR_AGT:
		{
			GLMSG::SNETPC_REQ_GLOBAL_RANKING_MMR_AGT* pNetMsg = (GLMSG::SNETPC_REQ_GLOBAL_RANKING_MMR_AGT*)nmg;

			if (pNetMsg->emFB > EM_GLOBAL_RANKING_FB_DELAY)
				for (int i = 0; i < 10; i++)
					if (pNetMsg->sTopMMR_[i].dwChaNum > 0)
						CInnerInterface::GetInstance().GetGlobalRankingWindow()->SetRankingMMR(pNetMsg->wPart, pNetMsg->sTopMMR_[i], i);
		}
		break;
	...
	case NET_MSG_GCTRL_SET_POINTS_FB:
		{
			...
			case EMREQ_SET_POINTS_FB_FAIL:
				{
					CInnerInterface::GetInstance().PrintConsoleTextDlg( "Point Update Failed" );
				}break;
			case EMREQ_SET_POINTS_FB_OK:
				{
					...
					case 1:
						{
							if ( pNetMsg->nPoint > 0 )
								CInnerInterface::GetInstance().PrintConsoleTextDlg( "E-Points Increased %d", pNetMsg->nPoint );
							else
								CInnerInterface::GetInstance().PrintConsoleTextDlg( "E-Points Decreased %d", pNetMsg->nPoint );

							m_dwPPoints += pNetMsg->nPoint;
						}
						break;
					case 2:
						{
							if ( pNetMsg->nPoint > 0 )
								CInnerInterface::GetInstance().PrintConsoleTextDlg( "G-Points Increased %d", pNetMsg->nPoint );
							else
								CInnerInterface::GetInstance().PrintConsoleTextDlg( "G-Points Decreased %d", pNetMsg->nPoint );

							m_dwVPoints += pNetMsg->nPoint;
						}
						break;
					default:	break;
					};
	...
	case NET_MSG_GCTRL_PICKUP_ITEM:
		{
			...
			SITEM * pITEM = GLItemMan::GetInstance().GetItem ( pNetMsg->sitem_link.sNativeID );
			if ( pITEM )
			{
				/*link drop item, EJCode, 2018/10/20 */
				if( RPARAM::link_drop_item )
				{
					...
					if ( pitem_disguise && pITEM->sBasicOp.emItemType == ITEM_WRAPPER_BOX )
						CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, &pNetMsg->sitem_link, ID2GAMEINTEXT("PICKUP_ITEM_WRAP"), pITEM->GetName(), pNetMsg->sitem_link.GetFilter() );
					else
						CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, &pNetMsg->sitem_link, ID2GAMEINTEXT("PICKUP_ITEM"), pNetMsg->sitem_link.GetFilter() );
				}
				else
				{
					/*item wrapper, Juver, 2018/01/12 */
...
*/
	...
	case NET_MSG_GCTRL_PUTON_RELEASE:
		{
			...
			{
				//	재장전할 아이탬인지 알아봄.
				if ( pNetMsg->emSlot == emLHand && GLCHARLOGIC_CLIENT::VALID_SLOT_ITEM(pNetMsg->emSlot) )
				{
					SITEMCUSTOM sItemCustom = GLCHARLOGIC_CLIENT::GET_SLOT_ITEM(pNetMsg->emSlot);
			...
			//	SLOT 변경.
			GLCHARLOGIC_CLIENT::RELEASE_SLOT_ITEM ( pNetMsg->emSlot );
	...
	case NET_MSG_GCTRL_PUTON_UPDATE:
		{
			...
			{
				GLCHARLOGIC_CLIENT::RELEASE_SLOT_ITEM ( pNetMsg->emSlotRelease );
			}
			...
			//	SLOT 변경.
			GLCHARLOGIC_CLIENT::SLOT_ITEM ( pNetMsg->sItemCustom, pNetMsg->emSlot );

			if ( pNetMsg->emSlot != SLOT_HOLD )
			{
				WORD wACP = GLCHARLOGIC_CLIENT::CALC_ACCEPTP ( pNetMsg->sItemCustom.sNativeID );
	...
	case NET_MSG_GCTRL_PUTON_CHANGE:
		{
			...
			GLCHARLOGIC_CLIENT::INIT_DATA ( FALSE, FALSE, pNetMsg->fCONFT_HP_RATE );
	...
	case NET_MSG_GCTRL_REQ_LEARNSKILL_FB:
		{
			...
			if ( pNetMsg->emCHECK == EMSKILL_OK )
			{
				...
				m_ExpSkills.insert ( std::make_pair ( pNetMsg->skill_id.dwID, SCHARSKILL(pNetMsg->skill_id,0,pNetMsg->item_id,pNetMsg->tBORNTIME) ) );
	...
	case NET_MSG_GCTRL_SKILL_EXPIRE_FB:
		{
			GLMSG::SNETPC_SKILL_EXPIRE_FB *pNetMsg = (GLMSG::SNETPC_SKILL_EXPIRE_FB *)nmg;

			PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( pNetMsg->sSkillID );
			if ( !pSkill )											break;

			SKILL_MAP_ITER iter_del = m_ExpSkills.find ( pNetMsg->sSkillID.dwID );
			if ( iter_del!=m_ExpSkills.end() )	
			{
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("SKILL_DEL_TIMELMT"), pSkill->GetName() );
	...
	case NET_MSG_GCTRL_REQ_SKILL_TARGET_FB:
		{
			GLMSG::SNETPC_REQ_SKILL_TARGET_FB *pNetMsg = (GLMSG::SNETPC_REQ_SKILL_TARGET_FB *)nmg;
			SkillTargetFB( pNetMsg );
		}break;
...
*/
	...
	case NET_MSG_GCTRL_INVEN_ITEM_UPDATE:
		{
			...
			if ( pNetMsg->bChangeID == false )
			{
				if ( pInvenItem->sItemCustom.sNativeID != pNetMsg->sItemCustom.sNativeID )	
					break;
			}
	...
	case NET_MSG_GCTRL_REGISTER_CODEX_FB:
		{
			...
			case EMREQ_REGISTER_CODEX_FB_INVALIDITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_REGISTER_CODEX_FB_INVALIDITEM") );
			case EMREQ_REGISTER_CODEX_FB_DONE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::BRIGHTGREEN, ID2GAMEINTEXT("EMREQ_REGISTER_CODEX_FB_DONE") );
	...
	case NET_MSG_GCTRL_INVEN_RANDOMBOXOPEN_FB:
		{
			...
			case EMINVEN_RANDOMBOXOPEN_FB_OK:
				{
					CInnerInterface::GetInstance().GachaWindowSelect (pNetMsg->nSelected);
	...
	case NET_MSG_GCTRL_UPDATE_CHA_PROMISE_POINTS://gacha system, jhoniex 7/7/2024
		{
			GLMSG::SNETPC_UPDATE_CHA_PROMISE_POINTS* pNetMsg = ( GLMSG::SNETPC_UPDATE_CHA_PROMISE_POINTS* ) nmg;
			m_wGachaPromisePoint = pNetMsg->dwVal;
		}break;
	...
	case NET_MSG_GCTRL_INVEN_RESET_SKST_FB:
		{
			...
			case EMREQ_RESET_SKST_FB_OK_NSPR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_RESET_SKST_FB_OK_NSPR"),
					pNetMsg->wITEM_NUM );
	...
	case NET_MSG_GCTRL_INVEN_GARBAGEOPEN_FB:
		{
			...
			case EMREQ_GARBAGEOPEN_FB_OK:
				...
				CInnerInterface::GetInstance().OpenItemGarbageWindow( TRUE );
				break;
	...
	case NET_MSG_CHAT_LOUDSPEAKER_FB:
		{
			...
			case EMCHAT_LOUDSPEAKER_REQUIRE_VERIFIED:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMCHAT_LOUDSPEAKER_REQUIRE_VERIFIED") );
				break;
			};
		}
		break;
...
// 경험치회복_정의_Npc
			...
			case EMPMARKET_BUY_FB_OK:
				if ( pNetMsg->dwGaeaID == m_dwGaeaID )
				{
					...
					if ( pSALEITEM )
					{
						...
						if ( pITEM )
						{							
							CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMPMARKET_BUY_FB_OK_SELLER"), pITEM->GetName(), pNetMsg->dwNum, pNetMsg->m_szName );
						}
					}
				}
				else
	...
	case NET_MSG_GCTRL_2_FRIEND_FB:
		{
			...
			case EM2FRIEND_FB_REQ:
				{
					CInnerInterface::GetInstance().WAITSERVER_DIALOGUE_OPEN ( ID2GAMEINTEXT("WAITSERVER_MESSAGE"), WAITSERVER_TOFRIEND, 10.0f );
				}
				break;

			case EM2FRIEND_FB_CANCEL:
				{
					CInnerInterface::GetInstance().WAITSERVER_DIALOGUE_CLOSE ();
				}
				break;
			...
			case EM2FRIEND_FB_PARTY_BLOCK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EM2FRIEND_FB_PARTY_BLOCK") );
				break;

				/* map entry user verified, Juver, 2020/02/27 */
			case EM2FRIEND_FB_USER_VERIFIED_MAP_ENTRY_BLOCK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EM2FRIEND_FB_USER_VERIFIED_MAP_ENTRY_BLOCK") );
				break;
			};
		}
		break;
	...
	case NET_MSG_GCTRL_SELFBUFF_BA_ONLY_FB:
		{
			GLMSG::SNETPC_REQ_SELFBUFF_BA_ONLY_FB *pNetMsg = (GLMSG::SNETPC_REQ_SELFBUFF_BA_ONLY_FB *) nmg;

			switch ( pNetMsg->emFB )
			{
			case EM_SELFBUFF_FAIL:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SELFBUFF_FAIL") );
				break;

			case EM_SELFBUFF_SUCCESS:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EM_SELFBUFF_SUCCESS") );
				break;

			case EM_SELFBUFF_NO_MONEY:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SELFBUFF_NO_MONEY") );
				break;
			};
		}
		break;
	...
	case NET_MSG_GCTRL_SELFBUFF_FULL_FB:
		{
			GLMSG::SNETPC_REQ_SELFBUFF_FULL_FB *pNetMsg = (GLMSG::SNETPC_REQ_SELFBUFF_FULL_FB *) nmg;
	...
	case NET_MSG_GCTRL_SELFBUFF_FULL_NO_BA_FB:
		{
			GLMSG::SNETPC_REQ_SELFBUFF_FULL_NO_BA_FB *pNetMsg = (GLMSG::SNETPC_REQ_SELFBUFF_FULL_NO_BA_FB *) nmg;
	...
	case NET_MSG_REBUILD_RESULT:	// ITEMREBUILD_MARK
		{
			...
			case EMREBUILD_RESULT_INVALID_ITEM:
				InitRebuildData();
				CInnerInterface::GetInstance().HideGroup( ITEM_REBUILD_WINDOW );
				CInnerInterface::GetInstance().PrintMsgTextDlg( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT( "EMREBUILD_RESULT_INVALID_ITEM" ) );
				break;

				/* Item Reform use limit, Juver, 2021/07/23 */
			case EMREBUILD_RESULT_MAXED:
				InitRebuildData();
				CInnerInterface::GetInstance().HideGroup( ITEM_REBUILD_WINDOW );
				CInnerInterface::GetInstance().PrintMsgTextDlg( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT( "EMREBUILD_RESULT_MAXED" ) );
				break;
			}
		}
		break;
	...
	case NET_MSG_GCTRL_GARBAGE_RESULT_FB:
		{
			...
			case EMGARBAGE_RESULT_FB_OK:
				...
				CInnerInterface::GetInstance().PrintMsgTextDlg( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT( "EMGARBAGE_RESULT_FB_OK" ) );
				break;
	...
	case NET_MSG_GCTRL_INVEN_RESET_STATS_FB:
		{
			GLMSG::SNET_INVEN_RESET_STATS_FB *pNetMsg = (GLMSG::SNET_INVEN_RESET_STATS_FB *) nmg;

			switch ( pNetMsg->emFB )
			{
			case EMREQ_RESET_STATS_FB_FAIL:
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ("Stats reset failed."));
				}break;

			case EMREQ_RESET_STATS_FB_OK:
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ("Stats reseted!"));
					STATS_RESET();
				}break;

			case EMREQ_RESET_STATS_FB_NOITEM:
				{
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ("You don't have the required card for reset stats."));
				}break;

			case EMREQ_RESET_STATS_FB_NOTITEM:
				{
...
//				CInnerInterface::GetInstance().WAITSERVER_DIALOGUE_OPEN ( ID2GAMEINTEXT("CHINA_GAIN_CLOSEGAME"), WAITSERVER_CLOSEGAME, 30 );
	...
	case NET_MSG_GCTRL_ACTIVE_VEHICLE_FB:
		{
			...
			case EMVEHICLE_SET_FB_FAIL_DEBUFF:
	...
	case NET_MSG_GCTRL_PING_PACKET_AGENT_FB:
		{
			GLMSG::SNETPC_PING_PACKET_AGENT_FB *pNetMsg = (GLMSG::SNETPC_PING_PACKET_AGENT_FB *)nmg;
						
			int nLastClockDiff = (int)clock() - m_clock_Packet_agent;

			
			{
				CInnerInterface::GetInstance().GetGameStatsDisplay()->UpdatePingAgent( float(nLastClockDiff) );
	...
	case NET_MSG_GCTRL_PING_PACKET_FIELD_FB:
		{
			GLMSG::SNETPC_PING_PACKET_FIELD_FB *pNetMsg = (GLMSG::SNETPC_PING_PACKET_FIELD_FB *)nmg;

			int nLastClockDiff = (int)clock() - m_clock_Packet_field;

			
			{
				CInnerInterface::GetInstance().GetGameStatsDisplay()->UpdatePingField( float(nLastClockDiff) );
	...
	case NET_MSG_GCTRL_NPCSHOP_PURCHASE_MONEY_FB:
		{
			...
			case EMNPCSHOP_PURCHASE_FB_INVALID_SHOP:
	...
	case NET_MSG_GCTRL_UPDATE_CONTRIBUTION_POINT:
		{
			...
			DWORD dwPoint = m_dwContributionPoint;
			m_dwContributionPoint = pNetMsg->dwPoint;

			if ( pNetMsg->bNotice && m_dwContributionPoint > dwPoint )
			{
				DWORD dwAdded = m_dwContributionPoint - dwPoint;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("CONTRIBUTION_POINT_UPDATE"), dwAdded );
			}
		}break;
	...
	case NET_MSG_GCTRL_NPC_ITEM_EXCHANGE_TRADE_FB:
		{
			...
			case EMNPC_ITEM_EXCHANGE_FB_NOMONEY:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_ITEM_EXCHANGE_FB_NOMONEY") );
				break;
			/* NpcExchange Contri Requirement | Jxyy | 11-21-24*/
			case EMNPC_ITEM_EXCHANGE_FB_NOCONTRI:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_ITEM_EXCHANGE_FB_NOCONTRI") );
				break;
			...
			case EMNPC_ITEM_EXCHANGE_FB_INVALID_DATA:
			...
			case EMNPC_ITEM_EXCHANGE_FB_NO_PLAY_TIME:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_ITEM_EXCHANGE_FB_NO_PLAY_TIME") );
				break;
			};
		}break;
	...
	case NET_MSG_VEHICLE_REQ_CHANGE_COLOR_FB:
		{
			...
			case EMREQ_CHANGE_BIKE_COLOR_FB_VEHICLE_NOT_COLOR:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_BIKE_COLOR_NOT_VEHICLE_COLOR") );
				break;
	...
	case NET_MSG_GCTRL_UPDATE_PK_KILL:
		{
			GLMSG::SNETPC_UPDATE_PK_KILL* pNetMsg = ( GLMSG::SNETPC_UPDATE_PK_KILL* ) nmg;
	...
	case NET_MSG_GCTRL_PK_STREAK:
		{
			GLMSG::SNETPC_PK_STREAK* pNetMsg = ( GLMSG::SNETPC_PK_STREAK* ) nmg;

			CString strPKSTREAK[PK_STREAK_SIZE + 1] = 
			{
				"none",
				"FIRST BLOOD",
				"DOUBLE KILL",
			...
			CString strTEXT;
			DWORD kills = pNetMsg->dwPkCounter;
			if (kills >= PK_STREAK_SIZE) kills = PK_STREAK_SIZE;

			PLANDMANCLIENT plandmanclient = GLGaeaClient::GetInstance().GetActiveMap();
   // 
			if (plandmanclient)
			{
				bool bPVPTyranny = plandmanclient->m_bPVPTyrannyMap; // Check for Tyranny
				bool bClubWars = plandmanclient->m_bClubBattle; // Assuming this is how you check for Club Wars
			//	bool bPVPWoe = plandmanclient->m_bPVPWoeMap;
			//	bool bPVPSchoolWars = plandmanclient->m_bPVPSchoolWarsMap;

				if (bPVPTyranny)
				{
					int line0 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeMultiColor(NS_UITEXTCOLOR::PALEGREEN, "%s ", pNetMsg->szName);
					int line1 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line0, NS_UITEXTCOLOR::WHITE, "is now ");
					int line2 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line1, NS_UITEXTCOLOR::RED, "%s ", strPKSTREAK[kills].GetString());
					int line3 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line2, NS_UITEXTCOLOR::WHITE, "at ");
					int line4 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line3, NS_UITEXTCOLOR::PALEGREEN, "Tyranny Wars ");
					//strTEXT.Format("%s is now on %s in Tyranny Wars!", pNetMsg->szName, strPKSTREAK[kills].GetString());
				}
				else if (bClubWars)
				{
					int line0 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeMultiColor(NS_UITEXTCOLOR::PALEGREEN, "%s ", pNetMsg->szName);
					...
					int line2 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line1, NS_UITEXTCOLOR::RED, "%s ", strPKSTREAK[kills].GetString());
					int line3 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line2, NS_UITEXTCOLOR::WHITE, "at ");
					int line4 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line3, NS_UITEXTCOLOR::PALEGREEN, "Club Wars ");
				}
			//	else if (bPVPWoe)
			//	{
			//		strTEXT.Format("%s is now on %s in War of Emperium!", pNetMsg->szName, strPKSTREAK[kills].GetString());
			//	}
			//	else if (bPVPSchoolWars)
			//	{
			//		strTEXT.Format("%s is now on %s in School Wars!", pNetMsg->szName, strPKSTREAK[kills].GetString());
			//	}
			//	else
			//	{
			//		strTEXT.Format("%s is now on %s. Somebody Kill Him!", pNetMsg->szName, strPKSTREAK[kills].GetString());
			//	}
			}
			else
			{
				int line0 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeMultiColor(NS_UITEXTCOLOR::BRIGHTGREEN, "%s ", pNetMsg->szName);
				int line1 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line0, NS_UITEXTCOLOR::WHITE, "is now ");
				int line2 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line1, NS_UITEXTCOLOR::RED, "%s", strPKSTREAK[kills].GetString());
				//int line3 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddString(line2, NS_UITEXTCOLOR::BRIGHTGREEN, " at Tyranny Wars ");

				//strTEXT.Format("%s is now %s.", pNetMsg->szName, strPKSTREAK[kills].GetString());
			}

			

			//strTEXT.Format("%s!! %s has reached %d killing streaks", strPKSTREAK[kills].GetString(), pNetMsg->szName ,pNetMsg->dwPkCounter);
			//strTEXT.Format("%s is now on %s.", pNetMsg->szName, strPKSTREAK[kills].GetString());


			//CInnerInterface::GetInstance().ShowPKStreakKillNotice(NS_UITEXTCOLOR::GOLD, strTEXT);
		}    
		break;

	case NET_MSG_GCTRL_RESU_STREAK:
	{
		GLMSG::SNETPC_RESU_STREAK* pNetMsg = (GLMSG::SNETPC_RESU_STREAK*)nmg;

		CString strRESUSTREAK[PK_STREAK_SIZE + 1] =
		{
			"none",
			"FIRST BLOOD",
			"DOUBLE KILL",
		...
		CString strTEXT;
		DWORD kills = pNetMsg->dwResuCounter;
		if (kills >= PK_STREAK_SIZE) kills = PK_STREAK_SIZE;

		//strTEXT.Format("%s is now on %s.", pNetMsg->szName, strRESUSTREAK[kills].GetString());
		//strTEXT.Format("%s! %s's resu streak: %d",  strRESUSTREAK[kills].GetString(), pNetMsg->szName, pNetMsg->dwResuCounter);

		int line0 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeMultiColorPKDead(NS_UITEXTCOLOR::RED, "%s! ", strRESUSTREAK[kills].GetString());
		int line1 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line0, NS_UITEXTCOLOR::AQUAMARINE, "%s", pNetMsg->szName);
		int line2 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line1, NS_UITEXTCOLOR::WHITE, "'s resu streak : ");
		int line3 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line2, NS_UITEXTCOLOR::WHITE, "%d ", pNetMsg->dwResuCounter);
	...
	case NET_MSG_GCTRL_PK_STREAK_KILL:
		{
			GLMSG::SNETPC_PK_STREAK_KILL* pNetMsg = ( GLMSG::SNETPC_PK_STREAK_KILL* ) nmg;
			...
			//strTEXT.Format ( "%s shutdown '%s' for being %s", pNetMsg->szNameKiller, pNetMsg->szName, strPKSTREAK[ kills ].GetString() );

			int line0 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeMultiColorPKDead(NS_UITEXTCOLOR::PALEGREEN, "%s ", pNetMsg->szNameKiller);
			int line1 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line0, NS_UITEXTCOLOR::WHITE, "shutdown ");
			int line2 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line1, NS_UITEXTCOLOR::RED, "'%s' ", pNetMsg->szName);
			int line3 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line2, NS_UITEXTCOLOR::WHITE, "for being ");
			int line4 = CInnerInterface::GetInstance().PVPCompetitionShowNoticeAddStringPKDead(line3, NS_UITEXTCOLOR::PALEGREEN, "%s ",strPKSTREAK[ kills ].GetString() );

			//CInnerInterface::GetInstance().ShowPKStreakShutdownNotice ( NS_UITEXTCOLOR::RED, strTEXT );

		}	
		break;
		/* PK Streak Sigaw - Jxyy End*/
	/*Top Resu, Mhundz */
	case NET_MSG_GCTRL_UPDATE_RESU_SCORE2:
		{
			GLMSG::SNETPC_UPDATE_RESU_SCORE2* pNetMsg = ( GLMSG::SNETPC_UPDATE_RESU_SCORE2* ) nmg;
			m_dwResu = pNetMsg->wVal;
		}break;
	/*Top MMR, Mhundz */
	case NET_MSG_GCTRL_UPDATE_MMR_SCORE:
		{
			GLMSG::SNETPC_UPDATE_MMR_SCORE* pNetMsg = ( GLMSG::SNETPC_UPDATE_MMR_SCORE* ) nmg;
			m_dwMMR = pNetMsg->wVal;
		}break;
	...
	case NET_MSG_GCTRL_INVEN_RANDOM_OPTION_CHANGE_FB:
		{
			...
			case EM_RANDOM_OPTION_CHANGE_DONE_CHANGE:
				...
				break;
			case EM_RANDOM_OPTION_CHANGE_TARGET_INVALID:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_RANDOM_OPTION_CHANGE_TARGET_INVALID") );
				break;
			};
		}break;
	...
	case NET_MSG_VEHICLE_REQ_CHANGE_CAR_COLOR_FB:
		{
			...
			case EMREQ_CHANGE_CAR_COLOR_FB_VEHICLE_NOT_COLOR:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_CHANGE_CAR_COLOR_FB_VEHICLE_NOT_COLOR") );
				break;
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_ITEM_FB:
		{	
			GLMSG::SNETPC_GM_COMMAND_GET_ITEM_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_ITEM_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_ITEM_FB_OK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_ITEM_FB_OK") );
				break;

			case EMREQ_GM_ITEM_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_ITEM_FB_FAIL") );
				break;

			case EMREQ_GM_ITEM_FB_INVALID_PASS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_ITEM_FB_INVALID_PASS") );
				break;

			case EMREQ_GM_ITEM_FB_INVALID_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_ITEM_FB_INVALID_ITEM") );
				break;

			case EMREQ_GM_ITEM_FB_INVEN_SPACE:
	...
	case NET_MSG_GCTRL_GM_COMMAND_INVEN_OPEN_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_INVEN_OPEN_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_INVEN_OPEN_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_INVEN_OPEN_FB_OK:
				{
					m_wINVENLINE = pNetMsg->wInvenLine;
					m_cInventory.SetAddLine ( GetOnINVENLINE(), true );
					CInnerInterface::GetInstance().SetInventorySlotViewSize ( EM_INVEN_DEF_SIZE_Y + GetOnINVENLINE() );
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_INVEN_OPEN_FB_OK") );
				}break;

			case EMREQ_GM_INVEN_OPEN_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_INVEN_OPEN_FB_FAIL") );
				break;

			case EMREQ_GM_INVEN_OPEN_FB_INVALID_PASS:
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_SKILL_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_SKILL_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_SKILL_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_SKILL_FB_OK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_SKILL_FB_OK") );
				break;

			case EMREQ_GM_SKILL_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SKILL_FB_FAIL") );
				break;

			case EMREQ_GM_SKILL_FB_INVALID_PASS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SKILL_FB_INVALID_PASS") );
				break;

			case EMREQ_GM_SKILL_FB_INVALID_SKILL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SKILL_FB_INVALID_SKILL") );
				break;

			case EMREQ_GM_SKILL_FB_CANNOT_LEARN:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SKILL_FB_CANNOT_LEARN") );
				break;
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_SKILL2_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_SKILL2_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_SKILL2_FB* ) nmg;

			PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( pNetMsg->sidSkill );
			if ( pSkill )
			{		
				m_ExpSkills.insert ( std::make_pair ( pNetMsg->sidSkill.dwID, SCHARSKILL(pNetMsg->sidSkill,pNetMsg->wLevel,NATIVEID_NULL(),0) ) );
				...
				STARGETID sTargetID(CROW_PC,m_dwGaeaID,m_vPos);
				DxEffGroupPlayer::GetInstance().NewEffGroup( GLCONST_CHAR::strSKILL_LEARN_EFFECT.c_str(), m_matTrans, &sTargetID );
			}
		}break;
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_LEVEL_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_LEVEL_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_LEVEL_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_LEVEL_FB_OK:
				{
					m_sExperience.lnNow = 0;
					m_sExperience.lnMax = GET_LEVELUP_EXP();

					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_LEVEL_FB_OK") );
				}break;

			case EMREQ_GM_LEVEL_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_LEVEL_FB_FAIL") );
				break;

			case EMREQ_GM_LEVEL_FB_INVALID_PASS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_LEVEL_FB_INVALID_PASS") );
				break;

			case EMREQ_GM_LEVEL_FB_MAX_LEVEL:
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_STATS_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_STATS_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_STATS_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_STATS_FB_OK:
				{
					m_sStats = pNetMsg->sStats;
					...
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_STATS_FB_OK") );
				}break;

			case EMREQ_GM_STATS_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_STATS_FB_FAIL") );
				break;

			case EMREQ_GM_STATS_FB_INVALID_PASS:
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_STATSP_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_STATSP_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_STATSP_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_STATSP_FB_OK:
				{
					m_wStatsPoint = pNetMsg->wStatsP;
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_STATSP_FB_OK") );
				}break;

			case EMREQ_GM_STATSP_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_STATSP_FB_FAIL") );
				break;

			case EMREQ_GM_STATSP_FB_INVALID_PASS:
	...
	case NET_MSG_GCTRL_GM_COMMAND_GET_SKILLP_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_SKILLP_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_SKILLP_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_SKILLP_FB_OK:
				{
					m_dwSkillPoint = pNetMsg->dwSkillP;
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_SKILLP_FB_OK") );
				}break;

			case EMREQ_GM_SKILLP_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SKILLP_FB_FAIL") );
				break;

			case EMREQ_GM_SKILLP_FB_INVALID_PASS:
	...
	case NET_MSG_GCTRL_REQ_STATSUP_MODERN_FB:
		{
			GLMSG::SNETPC_REQ_STATSUP_MODERN_FB *pNetMsg = (GLMSG::SNETPC_REQ_STATSUP_MODERN_FB *)nmg;

			SCHARSTATS sStatsPrev = m_sStats;

			STATSUP_MODERN(pNetMsg->sStats);

			if ( pNetMsg->bCommand == TRUE)
			{
				int nPow = static_cast<int>( std::abs( m_sStats.wPow - sStatsPrev.wPow ) );
				if ( nPow != 0 )
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("COMMAND_ADDED_POW"), nPow );

				int nDex = static_cast<int>( std::abs( m_sStats.wDex - sStatsPrev.wDex ) );
				if ( nDex != 0 )
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("COMMAND_ADDED_DEX"), nDex );

				int nSpi = static_cast<int>( std::abs( m_sStats.wSpi - sStatsPrev.wSpi ) );
				if ( nSpi != 0 )
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("COMMAND_ADDED_INT"), nSpi );

				int nSta = static_cast<int>( std::abs( m_sStats.wSta - sStatsPrev.wSta ) );
				if ( nSta != 0 )
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("COMMAND_ADDED_STM"), nSta );

				int nStr = static_cast<int>( std::abs( m_sStats.wStr - sStatsPrev.wStr ) );
				if ( nStr != 0 )
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("COMMAND_ADDED_VIT"), nStr );
			}
		}break;
	...
	case NET_MSG_GCTRL_INVEN_ITEM_RANDOM_OPTION_REBUILD_FB:
		{	
			GLMSG::SNETPC_INVEN_ITEM_RANDOM_OPTION_REBUILD_FB* pNetMsg = ( GLMSG::SNETPC_INVEN_ITEM_RANDOM_OPTION_REBUILD_FB* ) nmg;

			switch( pNetMsg->emFB )
			{
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_ITEM_RANDOM_OPTION_REBUILD_FAILED") );
				break;
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_NOT_CARD:
				...
				break;
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_NOT_TARGET:
				...
				break;
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_TARGET_NOT_SUIT:
				...
				break;
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_TARGET_NOT_RANDOM:
				...
				break;
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_TARGET_DESTROYED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_ITEM_RANDOM_OPTION_REBUILD_TARGET_DESTROYED") );
				break;
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_SUCCESS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_ITEM_RANDOM_OPTION_REBUILD_SUCCESS") );
				break;

			case EMFB_ITEM_RANDOM_OPTION_REBUILD_SELECT_ERROR:
			...
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_DELAY_TIME:
			...
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_TASK_ACTIVE:
				...
				/* Item Reform use limit, Juver, 2021/07/23 */
			case EMFB_ITEM_RANDOM_OPTION_REBUILD_TARGET_MAXED:
	...
	case NET_MSG_GCTRL_INVEN_ITEM_SORT_FB:
		{
			GLMSG::SNETPC_INVEN_SORT_FB* pNetMsg = ( GLMSG::SNETPC_INVEN_SORT_FB* ) nmg;

			switch( pNetMsg->emFB )
			{
			case EMFB_ITEM_INVEN_SORT_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_ITEM_INVEN_SORT_FAILED") );
				break;
			case EMFB_ITEM_INVEN_SORT_DELAY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_ITEM_INVEN_SORT_DELAY") );
				break;
			case EMFB_ITEM_INVEN_SORT_DONE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_ITEM_INVEN_SORT_DONE") );
				break;
			case EMFB_ITEM_INVEN_SORT_DISABLED:
	...
	case NET_MSG_GCTRL_INVEN_ITEM_SORT_UPDATE:
		{
			GLMSG::SNETPC_INVEN_SORT_UPDATE* pNetMsg = ( GLMSG::SNETPC_INVEN_SORT_UPDATE* ) nmg;
			...
			{
				m_cInventory.InsertItem ( pNetMsg->sInvenItem[i].sItemCustom, pNetMsg->sInvenItem[i].wPosX, pNetMsg->sInvenItem[i].wPosY );
			}
	...
	case NET_MSG_GCTRL_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC_FB:
		{
			...
			case EMREQ_ITEM_BOX_SPECIFIC_BAD_SELECTED_EXIST:
				...
				break;
			case EMREQ_ITEM_BOX_SPECIFIC_NO_INVEN_SPACE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_ITEM_BOX_SPECIFIC_NO_INVEN_SPACE") );
				break;
			case EMREQ_ITEM_BOX_SPECIFIC_DONE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_ITEM_BOX_SPECIFIC_DONE") );
				break;
			}
		}break;
	...
	//	{
	//		GLMSG::SNETPC_HIDE_COSTUME_FB* pNetMsg = ( GLMSG::SNETPC_HIDE_COSTUME_FB* ) nmg;

	//		switch( pNetMsg->fb )
	//		{
	//		case EMFB_HIDE_COSTUME_DELAY_TIME:
	//			{
	//				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_HIDE_COSTUME_DELAY_TIME") );
	//			}break;

	//		case EMFB_HIDE_COSTUME_OK:
	//			{
	//				m_hide_costume = pNetMsg->hide_costume;
	//				UpdateSuit();

	//				if ( m_hide_costume )
	//					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_HIDE_COSTUME_HIDE") );
	//				else
	//					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_HIDE_COSTUME_SHOW") );
	...
	case NET_MSG_GCTRL_REQ_SKILL_TAB_CHANGE_FB:
		{
			GLMSG::SNETPC_SKILL_TAB_CHANGE_FB* pNetMsg = ( GLMSG::SNETPC_SKILL_TAB_CHANGE_FB* ) nmg;
	...
	case NET_MSG_GCTRL_CLIENT_TICK_COUNT_CHECK_REQUEST:
		{
			GLMSG::SNETPC_CLIENT_TICK_COUNT_CHECK_REQUEST* pNetMsg = ( GLMSG::SNETPC_CLIENT_TICK_COUNT_CHECK_REQUEST* ) nmg;
			client_tick_count_check( pNetMsg->key1, pNetMsg->key2 );
		}break;

		/*gm command send item, Juver, 2019/03/23 */
	case NET_MSG_GCTRL_GM_COMMAND_A2C_SEND_ITEM_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_A2C_SEND_ITEM_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_A2C_SEND_ITEM_FB* ) nmg;

			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_SEND_ITEM_FB_OK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_FB_OK") );
				break;

			case EMREQ_GM_SEND_ITEM_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_FB_FAIL") );
				break;

			case EMREQ_GM_SEND_ITEM_FB_INVALID_PASS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_FB_INVALID_PASS") );
				break;

			case EMREQ_GM_SEND_ITEM_FB_INVALID_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_FB_INVALID_ITEM") );
				break;

			case EMREQ_GM_SEND_ITEM_FB_NO_INVEN_SPACE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_FB_NO_INVEN_SPACE") );
				break;

			case EMREQ_GM_SEND_ITEM_FB_INVALID_CHARACTER:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_FB_INVALID_CHARACTER") );
				break;
			};
		}break;
	...
	case NET_MSG_GCTRL_GM_COMMAND_A2C_SEND_ITEM_NOTICE:
		{
			GLMSG::SNETPC_GM_COMMAND_A2C_SEND_ITEM_NOTICE* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_A2C_SEND_ITEM_NOTICE* ) nmg;

			std::string strItemName  = "Unknown Item";
			SITEM* pItem = GLItemMan::GetInstance().GetItem( pNetMsg->sidItem );
			if ( pItem )
				strItemName = pItem->GetName();

			CString strText;
			strText.Format( ID2GAMEINTEXT("EMREQ_GM_SEND_ITEM_TO_TARGET" ), strItemName.c_str(), pNetMsg->szName );

			CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, strText.GetString() );
		}break;

	case NET_MSG_GCTRL_CAPTURE_PUTON_DROP:
		{
			GLMSG::SNETPC_CAPTURE_PUTON_DROP* pNetMsg = ( GLMSG::SNETPC_CAPTURE_PUTON_DROP* ) nmg;
			DxGrapUtils::CaptureScreen2 ( m_pd3dDevice, pNetMsg->szSerial );
		}break;

	case NET_MSG_GCTRL_GM_COMMAND_GET_CROW_TIME_FB:
		{
			GLMSG::SNETPC_GM_COMMAND_GET_CROW_TIME_FB* pNetMsg = ( GLMSG::SNETPC_GM_COMMAND_GET_CROW_TIME_FB* ) nmg;

			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_GET_CROW_TIME_FB_OK:
				{
					SCROWDATA* pCrowData = GLCrowDataMan::GetInstance().GetCrowData ( pNetMsg->sMobID );
					SMAPNODE* pMapNode = GLGaeaClient::GetInstance().FindMapNode( pNetMsg->sMapID );
					if ( pCrowData && pMapNode )
					{
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_GET_CROW_TIME_FB_OK_1"), 
							pCrowData->m_sBasic.sNativeID.wMainID, pCrowData->m_sBasic.sNativeID.wSubID, pCrowData->GetName(),
							pMapNode->sNativeID.wMainID, pMapNode->sNativeID.wSubID, pMapNode->strMapName.c_str() );

						if ( pNetMsg->bAlive )
						{
							CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_GET_CROW_TIME_FB_OK_2") );
						}
						else
						{
							if ( pNetMsg->bUseRegenTime )
							{
								std::string strDay = "";
								if ( pNetMsg->bDayOfWeek[0] )	strDay += "Sunday,";
								if ( pNetMsg->bDayOfWeek[1] )	strDay += "Monday,";
								if ( pNetMsg->bDayOfWeek[2] )	strDay += "Tuesday,";
								if ( pNetMsg->bDayOfWeek[3] )	strDay += "Wednesday,";
								if ( pNetMsg->bDayOfWeek[4] )	strDay += "Thursday,";
								if ( pNetMsg->bDayOfWeek[5] )	strDay += "Friday,";
								if ( pNetMsg->bDayOfWeek[6] )	strDay += "Saturday,";

								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_GET_CROW_TIME_FB_OK_3"), pNetMsg->nRegenHour, pNetMsg->nRegenMinute, strDay.c_str() );
							}
							else
							{
								CTimeSpan timeSpan(0,0,0,(int)pNetMsg->fTimer);
	...
	case NET_MSG_GCTRL_UPDATE_WAR_CHIPS:
		{
			GLMSG::SNETPC_UPDATE_WAR_CHIPS *pNetMsg = (GLMSG::SNETPC_UPDATE_WAR_CHIPS *)nmg;

			DWORD dwWarChips = m_dwWarChips;
			m_dwWarChips = pNetMsg->dwWarChips;

			if ( pNetMsg->bNotice && dwWarChips != m_dwWarChips )
			{
				if ( m_dwWarChips > dwWarChips )
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("WAR_CHIPS_UPDATE_PLUS"), m_dwWarChips - dwWarChips );
				else
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("WAR_CHIPS_UPDATE_MINUS"), dwWarChips - m_dwWarChips );	
			}
		}break;

	case NET_MSG_GCTRL_UPDATE_GAME_POINTS:
		{
			GLMSG::SNETPC_UPDATE_GAME_POINTS *pNetMsg = (GLMSG::SNETPC_UPDATE_GAME_POINTS *)nmg;

			DWORD dwGamePoints = m_dwGamePoints;
			m_dwGamePoints = pNetMsg->dwGamePoints;

			if ( pNetMsg->bNotice && dwGamePoints != m_dwGamePoints )
			{
				if ( m_dwGamePoints > dwGamePoints )
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("GAME_POINTS_UPDATE_PLUS"), m_dwGamePoints - dwGamePoints );
				else
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("GAME_POINTS_UPDATE_MINUS"), dwGamePoints - m_dwGamePoints );	
			}
		}break;

	case NET_MSG_GCTRL_CURRENCY_SHOP_PURCHASE_FB:
		{
			GLMSG::SNETPC_REQ_CURRENCY_SHOP_PURCHASE_FB *pNetMsg = (GLMSG::SNETPC_REQ_CURRENCY_SHOP_PURCHASE_FB *)nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_CURRENCY_SHOP_PURCHASE_ERROR:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_CURRENCY_SHOP_PURCHASE_ERROR") );
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_INVALID_CROW:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_INVALID_SHOP:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_INVALID_SHOP_ITEM:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_INVALID_ITEM_DATA:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_NOT_ENOUGH_MONEY:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_NOT_ENOUGH_GAME_POINTS:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_NOT_ENOUGH_WAR_CHIPS:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_NOT_ENOUGH_INVEN_SPACE:
				...
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_ERROR_COST:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_CURRENCY_SHOP_PURCHASE_ERROR_COST") );
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_ERROR_INSERT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_CURRENCY_SHOP_PURCHASE_ERROR_INSERT") );
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_DELAY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_CURRENCY_SHOP_PURCHASE_DELAY") );
				break;
			case EMREQ_CURRENCY_SHOP_PURCHASE_DONE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_CURRENCY_SHOP_PURCHASE_DONE") );
				break;
			};
			...
			switch( pNetMsg->emFB )
			{
				...
				case EMREQ_CURRENCY_SHOP_PURCHASE_ERROR_COST:
				case EMREQ_CURRENCY_SHOP_PURCHASE_DONE:
					CInnerInterface::GetInstance().CurrencyShopUpdatePrice();
					break;
			};
		}break;
	...
	case NET_MSG_GCTRL_PERSONAL_LOCK_REQUEST_FB:
		{
			GLMSG::SNETPC_PERSONAL_LOCK_REQUEST_FB *pNetMsg = (GLMSG::SNETPC_PERSONAL_LOCK_REQUEST_FB *)nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_PERSONAL_LOCK_REQUEST_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PERSONAL_LOCK_REQUEST_FAIL") );
				break;

			case EMREQ_PERSONAL_LOCK_REQUEST_FUNCTION_DISABLED:
			...
			case EMREQ_PERSONAL_LOCK_REQUEST_INVALID_LOCK_TYPE:
			...
			case EMREQ_PERSONAL_LOCK_REQUEST_REQUIRE_TIME:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PERSONAL_LOCK_REQUEST_REQUIRE_TIME") );
				break;

			case EMREQ_PERSONAL_LOCK_REQUEST_PIN_TO_CREATE:
				CInnerInterface::GetInstance().PersonalLockPinCreate( pNetMsg->emLock );
				break;

			case EMREQ_PERSONAL_LOCK_REQUEST_PIN_TO_INPUT:
				CInnerInterface::GetInstance().PersonalLockPinInput( pNetMsg->emLock );
				break;
			};
		}break;
	...
	case NET_MSG_GCTRL_PERSONAL_LOCK_CREATE_PIN_FB:
		{
			GLMSG::SNETPC_PERSONAL_LOCK_CREATE_PIN_FB *pNetMsg = (GLMSG::SNETPC_PERSONAL_LOCK_CREATE_PIN_FB *)nmg;

			switch( pNetMsg->emFB )
			{
			case EMFB_PERSONAL_LOCK_CREATE_PIN_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_CREATE_PIN_FAILED") );
				break;

			case EMFB_PERSONAL_LOCK_CREATE_PIN_FUNCTION_DISABLED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_CREATE_PIN_FAILED") );
				break;

			case EMFB_PERSONAL_LOCK_CREATE_PIN_INVALID_LOCK_TYPE:
			...
			case EMFB_PERSONAL_LOCK_CREATE_PIN_ALREADY_ENABLED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_CREATE_PIN_ALREADY_ENABLED") );
				break;

			case EMFB_PERSONAL_LOCK_CREATE_PIN_KEY_EMPTY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_CREATE_PIN_KEY_EMPTY") );
				break;

			case EMFB_PERSONAL_LOCK_CREATE_PIN_KEY_SIZE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_CREATE_PIN_KEY_SIZE") );
				break;

			case EMFB_PERSONAL_LOCK_CREATE_PIN_KEY_MISMATCH:
			...
			case EMFB_PERSONAL_LOCK_CREATE_PIN_KEY_NUMBER_ONLY:
			...
			case EMFB_PERSONAL_LOCK_CREATE_PIN_TIME_OUT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_CREATE_PIN_TIME_OUT") );
				break;

			case EMFB_PERSONAL_LOCK_CREATE_PIN_PUTON_DONE:
				{
					if ( pNetMsg->emLock < EMPERSONAL_LOCK_SIZE )
					{
						CString strMsg;
						strMsg.Format( "%s %s", ID2GAMEWORD( "PERSONAL_LOCK_INFO", pNetMsg->emLock ), ID2GAMEINTEXT( "EMFB_PERSONAL_LOCK_CREATE_PIN_DONE" ) );
						
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, strMsg.GetString() );
					}
				}break;
	...
	case NET_MSG_GCTRL_PERSONAL_LOCK_INPUT_PIN_FB:
		{
			GLMSG::SNETPC_PERSONAL_LOCK_INPUT_PIN_FB *pNetMsg = (GLMSG::SNETPC_PERSONAL_LOCK_INPUT_PIN_FB *)nmg;

			switch( pNetMsg->emFB )
			{
			case EMFB_PERSONAL_LOCK_INPUT_PIN_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_INPUT_PIN_FAILED") );
				break;

			case EMFB_PERSONAL_LOCK_INPUT_PIN_FUNCTION_DISABLED:
			...
			case EMFB_PERSONAL_LOCK_INPUT_PIN_INVALID_LOCK_TYPE:
			...
			case EMFB_PERSONAL_LOCK_INPUT_PIN_TIME_OUT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_INPUT_PIN_TIME_OUT") );
				break;

			case EMFB_PERSONAL_LOCK_INPUT_PIN_KEY_WRONG:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_INPUT_PIN_KEY_WRONG") );
				break;

			case EMFB_PERSONAL_LOCK_INPUT_PIN_KEY_WRONG_MAXED:
				{
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_INPUT_PIN_KEY_WRONG_MAXED") );
				}break;

			case EMFB_PERSONAL_LOCK_INPUT_PIN_DONE:
				{
					if ( pNetMsg->emLock < EMPERSONAL_LOCK_SIZE )
					{
						m_sPersonalLock[pNetMsg->emLock].bLock = pNetMsg->bLock;

						if ( m_sPersonalLock[pNetMsg->emLock].bLock )
						{
							CString strMsg;
							strMsg.Format( "%s %s", ID2GAMEWORD( "PERSONAL_LOCK_INFO", pNetMsg->emLock ), ID2GAMEINTEXT( "EMFB_PERSONAL_LOCK_INPUT_PIN_DONE_1" ) );
							CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, strMsg.GetString() );
						}
						else
						{
							CString strMsg;
							strMsg.Format( "%s %s", ID2GAMEWORD( "PERSONAL_LOCK_INFO", pNetMsg->emLock ), ID2GAMEINTEXT( "EMFB_PERSONAL_LOCK_INPUT_PIN_DONE_2" ) );
							CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, strMsg.GetString() );
						}
					}
				}break;
	...
	case NET_MSG_GCTRL_PERSONAL_LOCK_RESET_PIN_FB:
		{
			GLMSG::SNETPC_PERSONAL_LOCK_PIN_RESET_FB *pNetMsg = (GLMSG::SNETPC_PERSONAL_LOCK_PIN_RESET_FB *)nmg;

			switch ( pNetMsg->emFB )
			{
			case EMFB_PERSONAL_LOCK_PIN_RESET_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RESET_FAILED") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_RESET_FUNCTION_DISABLED:
			...
			case EMFB_PERSONAL_LOCK_PIN_RESET_INVALID_LOCK_TYPE:
			...
			case EMFB_PERSONAL_LOCK_PIN_RESET_TIME_OUT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RESET_TIME_OUT") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_RESET_NOT_CARD:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RESET_NOT_CARD") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_RESET_DONE:
				{
					for ( int i=0; i<EMPERSONAL_LOCK_SIZE; ++i )
					{
						m_sPersonalLock[i].bLock = pNetMsg->bLock[i];
					}

					if ( pNetMsg->emLock >= EMPERSONAL_LOCK_SIZE )
					{
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RESET_DONE_ALL") );
					}
					else
					{
						CString strMsg;
						strMsg.Format( "%s %s", ID2GAMEWORD( "PERSONAL_LOCK_INFO", pNetMsg->emLock ), ID2GAMEINTEXT( "EMFB_PERSONAL_LOCK_PIN_RESET_DONE" ) );
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, strMsg.GetString() );
					}
				}break;
			};
	...
	case NET_MSG_GCTRL_PERSONAL_LOCK_CHANGE_PIN_FB:
		{
			GLMSG::SNETPC_PERSONAL_LOCK_PIN_CHANGE_FB *pNetMsg = (GLMSG::SNETPC_PERSONAL_LOCK_PIN_CHANGE_FB *)nmg;

			switch( pNetMsg->emFB )
			{
			case EMFB_PERSONAL_LOCK_PIN_CHANGE_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_CHANGE_FAILED") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_CHANGE_FUNCTION_DISABLED:
			...
			case EMFB_PERSONAL_LOCK_PIN_CHANGE_INVALID_LOCK_TYPE:
			...
			case EMFB_PERSONAL_LOCK_PIN_CHANGE_PIN_MISMATCH:
			...
			case EMFB_PERSONAL_LOCK_PIN_CHANGE_TIME_OUT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_CHANGE_TIME_OUT") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_CHANGE_PIN_INCORRECT:
				{
					if ( pNetMsg->emLock < EMPERSONAL_LOCK_SIZE )
					{
						CString strTemp;
						strTemp.Format( ID2GAMEINTEXT( "EMFB_PERSONAL_LOCK_PIN_CHANGE_PIN_INCORRECT" ), ID2GAMEWORD( "PERSONAL_LOCK_INFO", pNetMsg->emLock ) );
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, strTemp.GetString() );
					}
				}break;

			case EMFB_PERSONAL_LOCK_PIN_CHANGE_PIN_DONE:
				{
					if ( pNetMsg->emLock < EMPERSONAL_LOCK_SIZE )
					{
						CString strTemp;
						strTemp.Format( ID2GAMEINTEXT( "EMFB_PERSONAL_LOCK_PIN_CHANGE_PIN_DONE" ), ID2GAMEWORD( "PERSONAL_LOCK_INFO", pNetMsg->emLock ) );
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, strTemp.GetString() );
					}
				}break;
	...
	case NET_MSG_GCTRL_PERSONAL_LOCK_RECOVER_PIN_FB:
		{
			GLMSG::SNETPC_PERSONAL_LOCK_PIN_RECOVER_FB *pNetMsg = (GLMSG::SNETPC_PERSONAL_LOCK_PIN_RECOVER_FB *)nmg;

			switch( pNetMsg->emFB )
			{
			case EMFB_PERSONAL_LOCK_PIN_RECOVER_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RECOVER_FAILED") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_RECOVER_FUNCTION_DISABLED:
			...
			case EMFB_PERSONAL_LOCK_PIN_RECOVER_INVALID_LOCK_TYPE:
			...
			case EMFB_PERSONAL_LOCK_PIN_RECOVER_TIME_OUT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RECOVER_TIME_OUT") );
				break;

			case EMFB_PERSONAL_LOCK_PIN_RECOVER_NOT_CARD:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RECOVER_NOT_CARD") );
				break;
				
			case EMFB_PERSONAL_LOCK_PIN_RECOVER_DONE:
				{
					CInnerInterface::GetInstance().PersonalLockPinRecoverResultOpen();

					if ( pNetMsg->emLock >= EMPERSONAL_LOCK_SIZE )
					{
						for( int i=0; i<EMPERSONAL_LOCK_SIZE; ++i )
						{
							CInnerInterface::GetInstance().PersonalLockPinRecoverResultUpdate( &pNetMsg->sPin[i], (EMPERSONAL_LOCK)i );
						}
					}
					else
					{
						CInnerInterface::GetInstance().PersonalLockPinRecoverResultUpdate( &pNetMsg->sPin[pNetMsg->emLock], pNetMsg->emLock );
					}

					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_PERSONAL_LOCK_PIN_RECOVER_DONE") );
				}break;
			}
		}break;
	...
	case NET_MSG_GCTRL_GM_USER_RESTRICT_UPDATE:
		{
			GLMSG::SNET_GM_USER_RESTRICT_UPDATE *pNetMsg = (GLMSG::SNET_GM_USER_RESTRICT_UPDATE *)nmg;
			m_bUserFlagRestricted = pNetMsg->bUserRestricted;

//#ifdef _DEBUG
//			/* user flag restricted, Juver, 2020/04/21 */
//			CInnerInterface::GetInstance().PrintConsoleTextDlg ( "m_bUserFlagRestricted=%d", m_bUserFlagRestricted );
//#endif
	...
	case NET_MSG_GCTRL_GM_INVEN_CLEAR_FB:
		{
			GLMSG::SNETPC_GM_INVEN_CLEAR_FB *pNetMsg = (GLMSG::SNETPC_GM_INVEN_CLEAR_FB *)nmg;
			switch( pNetMsg->emFB )
			{
			case EMREQ_GM_INVEN_CLEAR_FB_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_INVEN_CLEAR_FB_FAILED") );
				break;

			case EMREQ_GM_INVEN_CLEAR_FB_NOT_GM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_INVEN_CLEAR_FB_NOT_GM") );
				break;

			case EMREQ_GM_INVEN_CLEAR_FB_INVALID_PASS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_INVEN_CLEAR_FB_INVALID_PASS") );
				break;

			case EMREQ_GM_INVEN_CLEAR_FB_SUCCESS:
				{
					m_cInventory.DeleteItemAll();
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_INVEN_CLEAR_FB_SUCCESS") );
				}break;
			};
		}break;
	...
	case NET_MSG_GCTRL_F2C_SKILL_DELAY_ADJUSTMENT:
		{
			GLMSG::SNETPC_F2C_SKILL_DELAY_ADJUSTMENT *pNetMsg = (GLMSG::SNETPC_F2C_SKILL_DELAY_ADJUSTMENT *)nmg;
			
			DELAY_MAP_ITER it = m_SKILLDELAY.find( pNetMsg->sSkillID.dwID );
			if ( it != m_SKILLDELAY.end() )	
			{
				float &fDelay = (*it).second;
				fDelay = fDelay * pNetMsg->fAdjust;
			}
			else
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("GAME_ERROR_SKILL_DELAY_NOT_FOUND"), pNetMsg->sSkillID.wMainID, pNetMsg->sSkillID.wSubID );
			}
		}
		break;
	...
	case NET_MSG_GCTRL_F2C_SKILL_DELAY_ADJUSTMENT_MULTI:
		{
			GLMSG::SNETPC_F2C_SKILL_DELAY_ADJUSTMENT_MULTI* pNetMsg = ( GLMSG::SNETPC_F2C_SKILL_DELAY_ADJUSTMENT_MULTI* ) nmg;
			for( int i=0; i<pNetMsg->wDataNum; ++i )
			{
				DELAY_MAP_ITER it = m_SKILLDELAY.find( pNetMsg->sData[i].sSkillID.dwID );
				if ( it != m_SKILLDELAY.end() )	
				{
					float &fDelay = (*it).second;
					fDelay = fDelay * pNetMsg->sData[i].fAdjust;
				}
			}
		}break;
	...
	case NET_MSG_GCTRL_UPDATE_PLAY_TIME:
		{
			GLMSG::SNETPC_UPDATE_PLAY_TIME *pNetMsg = (GLMSG::SNETPC_UPDATE_PLAY_TIME *)nmg;

			LONGLONG llPrevValue = m_llPlayTime;
			m_llPlayTime = pNetMsg->llPlayTime;

			if ( pNetMsg->bNotice && llPrevValue != m_llPlayTime )
			{
				if ( m_llPlayTime > llPrevValue )
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("CHARACTER_PLAY_TIME_UPDATE_PLUS"), m_llPlayTime - llPrevValue );
				else
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("CHARACTER_PLAY_TIME_UPDATE_MINUS"), llPrevValue - m_llPlayTime );	
			}
		}break;
	...
	case NET_MSG_GCTRL_UPDATE_PLAY_POINT:
		{
			GLMSG::SNETPC_UPDATE_PLAY_POINT *pNetMsg = (GLMSG::SNETPC_UPDATE_PLAY_POINT *)nmg;

			LONGLONG llPrevValue = m_llPlayPoint;
			m_llPlayPoint = pNetMsg->llPlayPoint;

			if ( pNetMsg->bNotice && llPrevValue != m_llPlayPoint )
			{
				if ( m_llPlayPoint > llPrevValue )
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("CHARACTER_PLAY_POINT_UPDATE_PLUS"), m_llPlayPoint - llPrevValue );
				else
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("CHARACTER_PLAY_POINT_UPDATE_MINUS"), llPrevValue - m_llPlayPoint );	
			}
		}break;
	...
	case NET_MSG_GCTRL_UPDATE_PK_COMBO:
		{
			GLMSG::SNETPC_UPDATE_PK_COMBO *pNetMsg = (GLMSG::SNETPC_UPDATE_PK_COMBO *)nmg;
			if ( pNetMsg->emPKCombo < EMPK_COMBO_TYPE_SIZE )
			{
				CInnerInterface::GetInstance().PKComboSet ( (int)pNetMsg->emPKCombo );
				DxSoundLib::GetInstance()->PlaySound ( GLPKCombo::GetInstance().strPKComboSoundText[pNetMsg->emPKCombo].c_str() ); 
			}

			if ( pNetMsg->emPKComboCount < EMPKCOMBO_COUNT_SIZE && pNetMsg->dwPKComboCount != 0 )
			{
				m_sPKComboCount.dwCount[pNetMsg->emPKComboCount] = pNetMsg->dwPKComboCount;
			}
		}break;
	...
	case NET_MSG_GCTRL_GM_GAME_NOTICE_RELOAD_FB:
		{
			GLMSG::SNETPC_GM_GAME_NOTICE_RELOAD_FB *pNetMsg = (GLMSG::SNETPC_GM_GAME_NOTICE_RELOAD_FB *)nmg;

			switch ( pNetMsg->emFB )
			{
			case EMREQ_GM_GAME_NOTICE_RELOAD_FB_OK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_GAME_NOTICE_RELOAD_FB_OK") );
				break;

			case EMREQ_GM_GAME_NOTICE_RELOAD_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_GAME_NOTICE_RELOAD_FB_FAIL") );
				break;

			case EMREQ_GM_GAME_NOTICE_RELOAD_FB_INVALID_PASS:
	...
	case NET_MSG_GCTRL_INVEN_CHAOS_MACHINE_FB:
		{
			GLMSG::SNETPC_INVEN_CHAOS_MACHINE_FB *pNetMsg = (GLMSG::SNETPC_INVEN_CHAOS_MACHINE_FB *)nmg;

			switch ( pNetMsg->emFB )
			{
			case EMFB_CHAOS_MACHINE_ERROR:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_ERROR") );
				break;

			case EMFB_CHAOS_MACHINE_INVALID_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_INVALID_ITEM") );
				break;

			case EMFB_CHAOS_MACHINE_INVALID_NPC:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_INVALID_NPC") );
				break;

			case EMFB_CHAOS_MACHINE_NON_DROP:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_NON_DROP") );
				break;

			case EMFB_CHAOS_MACHINE_COST:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_COST") );
				break;

			case EMFB_CHAOS_MACHINE_NO_INVEN_SPACE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_NO_INVEN_SPACE") );
				break;

			case EMFB_CHAOS_MACHINE_NO_MATERIAL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_NO_MATERIAL") );
				break;

			case EMFB_CHAOS_MACHINE_FAILED_DELETE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_FAILED_DELETE") );
				break;

			case EMFB_CHAOS_MACHINE_FAILED:
				{
					DxSoundLib::GetInstance()->PlaySound ( "GRINDING_BROKEN" );
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_FAILED") );

					ResetChaosItemMoveMainItem();
				}break;

			case EMFB_CHAOS_MACHINE_SUCCESS:
				{
					DxSoundLib::GetInstance()->PlaySound ( "GRINDING_SUCCEED" );
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMFB_CHAOS_MACHINE_SUCCESS") );
	...
	case NET_MSG_GCTRL_MAX_LEVEL_NOTICE_CLIENT:
		{
			GLMSG::SNETPC_MAX_LEVEL_NOTICE_CLIENT *pNetMsg = (GLMSG::SNETPC_MAX_LEVEL_NOTICE_CLIENT *)nmg;
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::BRIGHTGREEN, ID2GAMEINTEXT("MAX_LEVEL_NOTICE"), pNetMsg->szName, pNetMsg->wLevel );
		}break;

		/* set item option, Juver, 2021/09/04 */
	case NET_MSG_GCTRL_INVEN_SET_OPTION_INSERT_FB:
		{
			GLMSG::SNET_INVEN_SET_OPTION_INSERT_FB* pNetMsg = ( GLMSG::SNET_INVEN_SET_OPTION_INSERT_FB* ) nmg;

			switch ( pNetMsg->emFB )
			{
			case EM_SET_OPTION_INSERT_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_FAILED") );
				break;
			case EM_SET_OPTION_INSERT_INVALID_TARGET_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_INVALID_TARGET_ITEM") );
				break;
			case EM_SET_OPTION_INSERT_INVALID_CARD_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_INVALID_CARD_ITEM") );
				break;
			case EM_SET_OPTION_INSERT_TARGET_NOT_SUIT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_TARGET_NOT_SUIT") );
				break;
			case EM_SET_OPTION_INSERT_TARGET_NOT_SET_OPTION:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_TARGET_NOT_SET_OPTION") );
				break;
			case EM_SET_OPTION_INSERT_TARGET_ALREADY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_TARGET_ALREADY") );
				break;
			case EM_SET_OPTION_INSERT_DONE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EM_SET_OPTION_INSERT_DONE") );
				break;
			};
		}break;
	...
	case NET_MSG_GCTRL_INVEN_SET_OPTION_REROLL_FB:
		{
			GLMSG::SNET_INVEN_SET_OPTION_REROLL_FB* pNetMsg = ( GLMSG::SNET_INVEN_SET_OPTION_REROLL_FB* ) nmg;

			switch ( pNetMsg->emFB )
			{
			case EM_SET_OPTION_REROLL_FAILED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_REROLL_FAILED") );
				break;
			case EM_SET_OPTION_REROLL_INVALID_TARGET_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_REROLL_INVALID_TARGET_ITEM") );
				break;
			case EM_SET_OPTION_REROLL_INVALID_CARD_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_REROLL_INVALID_CARD_ITEM") );
				break;
			case EM_SET_OPTION_REROLL_TARGET_NOT_SUIT:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_REROLL_TARGET_NOT_SUIT") );
				break;
			case EM_SET_OPTION_REROLL_TARGET_NOT_SET_OPTION:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EM_SET_OPTION_REROLL_TARGET_NOT_SET_OPTION") );
				break;
			case EM_SET_OPTION_REROLL_DONE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EM_SET_OPTION_REROLL_DONE") );
				break;
			};
		}break;
	...
	case NET_MSG_GCTRL_NPC_CONTRIBUTION_SHOP_PURCHASE_FB:
		{
			GLMSG::SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE_FB* pNetMsg = ( GLMSG::SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE_FB* ) nmg;

			switch ( pNetMsg->emFB )
			{
			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_ITEM") );
				break;

			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_CROW:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_CROW") );
				break;

			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_SHOP:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_SHOP") );
				break;

			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_SHOPITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_SHOPITEM") );
				break;

			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_NOT_CONTRIBUTION:
			...
			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_NOT_INVEN_SPACE:
			...
			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_PURCHASED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_PURCHASED") );
				break;

			case EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_NOT_GOLD:
	...
	case NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_FB:
		{
			GLMSG::SNETPC_REQ_MAXRV_PREVIEW_FB* pNetMsg = ( GLMSG::SNETPC_REQ_MAXRV_PREVIEW_FB* ) nmg;
			switch( pNetMsg->emFB )
			{
			case EMMAXRV_PREVIEW_FB_ERROR:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMMAXRV_PREVIEW_FB_ERROR") );
				break;

			case EMMAXRV_PREVIEW_FB_INVALID_ITEM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMMAXRV_PREVIEW_FB_INVALID_ITEM") );
				break;

			case EMMAXRV_PREVIEW_FB_INVALID_ITEM_RV:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMMAXRV_PREVIEW_FB_INVALID_ITEM_RV") );
				break;

			case EMMAXRV_PREVIEW_FB_START:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMMAXRV_PREVIEW_FB_START") );
				break;
			};
		}
		break;
	...
	case NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_DATA_INIT:
		{
			GLMSG::SNETPC_REQ_MAXRV_PREVIEW_DATA_INIT* pNetMsg = ( GLMSG::SNETPC_REQ_MAXRV_PREVIEW_DATA_INIT* ) nmg;
			m_sMaxRvPreview.m_dwOptionMobGenWait = pNetMsg->dwMaxRandomMobGen;
			m_sMaxRvPreview.m_dwOptionRebuildWait = pNetMsg->dwMaxRandomRebuild;
			m_sMaxRvPreview.m_sItemID = pNetMsg->sID;

			if ( m_sMaxRvPreview.CheckComplete() )
			{
				CInnerInterface::GetInstance().MaxRvPreviewDisplayShow();
			}
		}
		break;
	...
	case NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_DATA_MOB_GEN:
		{
			GLMSG::SNETPC_REQ_MAXRV_PREVIEW_DATA_MOB_GEN* pNetMsg = ( GLMSG::SNETPC_REQ_MAXRV_PREVIEW_DATA_MOB_GEN* ) nmg;
			m_sMaxRvPreview.m_vecOptionMobGen.push_back( pNetMsg->sOption );
	...
	case NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_DATA_REBUILD:
		{
			GLMSG::SNETPC_REQ_MAXRV_PREVIEW_DATA_REBUILD* pNetMsg = ( GLMSG::SNETPC_REQ_MAXRV_PREVIEW_DATA_REBUILD* ) nmg;
			m_sMaxRvPreview.m_vecOptionRebuild.push_back( pNetMsg->sOption );
	...
	case NET_MSG_GCTRL_GM_GIS_RELOAD_FB:
		{
			GLMSG::SNETPC_GM_GIS_RELOAD_FB *pNetMsg = (GLMSG::SNETPC_GM_GIS_RELOAD_FB *)nmg;

			switch ( pNetMsg->emFB )
			{
			case EMREQ_GM_GIS_RELOAD_FB_OK:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_GIS_RELOAD_FB_OK") );
				break;

			case EMREQ_GM_GIS_RELOAD_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_GIS_RELOAD_FB_FAIL") );
				break;

			case EMREQ_GM_GIS_RELOAD_FB_INVALID_PASS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_GIS_RELOAD_FB_INVALID_PASS") );
				break;

			case EMREQ_GM_GIS_RELOAD_FB_RELOADING:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_GM_GIS_RELOAD_FB_RELOADING") );
				break;
	...
	case NET_MSG_GCTRL_GM_GIS_RELOAD_DONE:
		{
			GLMSG::SNETPC_GM_GIS_RELOAD_DONE *pNetMsg = (GLMSG::SNETPC_GM_GIS_RELOAD_DONE *)nmg;
			CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_GM_GIS_RELOAD_DONE") );
		}break;

	case NET_MSG_GCTRL_REQ_PURCHASE_SKILL_FB:
		{
			GLMSG::SNETPC_REQ_PURCHASE_SKILL_FB *pNetMsg = (GLMSG::SNETPC_REQ_PURCHASE_SKILL_FB *)nmg;
			switch ( pNetMsg->emFB )
			{
			case EMREQ_PURCHASE_SKILL_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PURCHASE_SKILL_FB_FAIL") );
				break;

			case EMREQ_PURCHASE_SKILL_FB_ALREADY_LEARNED:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PURCHASE_SKILL_FB_ALREADY_LEARNED") );
				break;

			case EMREQ_PURCHASE_SKILL_FB_REQ_STATS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PURCHASE_SKILL_FB_REQ_STATS") );
				break;

			case EMREQ_PURCHASE_SKILL_FB_REQ_COST:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PURCHASE_SKILL_FB_REQ_COST") );
				break;

			case EMREQ_PURCHASE_SKILL_FB_CANNOT_PURCHASE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_PURCHASE_SKILL_FB_CANNOT_PURCHASE") );
				break;
			};
		}break;
	...
	case NET_MSG_ENCHANT_NOTIFY_CLIENT:
		{
			GLMSG::SNETPC_ENCHANT_NOTIFY_CLIENT *pNetMsg = (GLMSG::SNETPC_ENCHANT_NOTIFY_CLIENT *)nmg;
			CInnerInterface::GetInstance().DisplayEnchantMessage( pNetMsg->szName, pNetMsg->sItemCustom );
		}
		break;

	case NET_MSG_COMBO_WHORE_NOTIFY_CLIENT:
		{
			GLMSG::SNETPC_COMBO_WHORE_NOTIFY_CLIENT *pNetMsg = (GLMSG::SNETPC_COMBO_WHORE_NOTIFY_CLIENT *)nmg;
			CInnerInterface::GetInstance().DisplayComboWhoreMessage( pNetMsg->szName );
		}break;

	case NET_MSG_GCTRL_REQ_AUTO_POTION_FB:
		{
			GLMSG::SNETPC_REQ_AUTO_POTION_FB *pNetMsg = (GLMSG::SNETPC_REQ_AUTO_POTION_FB *)nmg;
			switch ( pNetMsg->emFB )
			{
			case EMREQ_AUTO_POTION_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_AUTO_POTION_FB_FAIL") );
				break;

			case EMREQ_AUTO_POTION_FB_INVALID_VALUE:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_AUTO_POTION_FB_INVALID_VALUE") );
				break;

			case EMREQ_AUTO_POTION_FB_DONE:
				{
					if ( m_pAutoPotion )
					{
						...
						CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMREQ_AUTO_POTION_FB_DONE") );
					}
					
				}
				break;

			case EMREQ_AUTO_POTION_FB_NOT_MAP:
	...
	case NET_MSG_GCTRL_REQ_AUTO_PILOT_RESET_FB:
		{
			if ( m_pAutoPilot )
				m_pAutoPilot->MsgProcess(nmg);
		}
		break;

	case NET_MSG_GCTRL_CARD_REBORN_A_FB:
		{
			GLMSG::SNETPC_CARD_REBORN_A_FB *pNetMsg = (GLMSG::SNETPC_CARD_REBORN_A_FB *) nmg;
			switch ( pNetMsg->emFB )
			{
			case EMINVEN_CARD_REBORN_FB_NOLEVEL:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_NOT_LEVEL") );
				break;
			case EMINVEN_CARD_REBORN_FB_NOMONEY:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_NOT_MONEY") );
				break;
			case EMINVEN_CARD_REBORN_FB_NOITEMTYPE:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_ITEMTYPE_ERROR") );
				break;
			case EMINVEN_CARD_REBORN_FB_NOITEMDATA:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_ITEM_ERROR") );
				break;
			case EMINVEN_CARD_REBORN_FB_NOITEMINVEN:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_POS_ERROR") );
				break;
			case EMINVEN_CARD_REBORN_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_FAIL") );
				break;
			case EMINVEN_CARD_REBORN_FB_OK:
				DxGlobalStage::GetInstance().GameToLobbyStage ();
				break;
			case EMINVEN_CARD_REBORN_FB_CLOSE:
				{
					CInnerInterface::GetInstance().HideGroup ( INVENTORY_WINDOW ); //hide inven window to prevent player doing shit
					CInnerInterface::GetInstance().HideGroup ( CHARACTER_REBORN_A_WINDOW ); //hide reborn window
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN_CARD_OK") );
				}break;
			};
		}
		break;
	...
	case NET_MSG_GCTRL_CARD_REBORN_B_FB:
		{
			GLMSG::SNETPC_CARD_REBORN_B_FB *pNetMsg = (GLMSG::SNETPC_CARD_REBORN_B_FB *) nmg;
			if ( pNetMsg->bOK )
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("REBORN2_OK") );
				m_dwReborn = pNetMsg->wReborn;
			}
			else
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("REBORN2_FAILED") );
			}
		}
		break;
	...
	case NET_MSG_GCTRL_REQ_EXCHANGE_ITEM_FB:
		{
			GLMSG::SNET_REQ_EXCHANGE_ITEM_FB *pNetMsg = (GLMSG::SNET_REQ_EXCHANGE_ITEM_FB *) nmg;
			
			switch ( pNetMsg->emFB )
			{
			case EMEXCHANGE_ITEM_ERROR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_ERROR") );
				break;

			case EMEXCHANGE_ITEM_CARD_ERROR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_CARD_ERROR") );
				break;

			case EMEXCHANGE_ITEM_ITEM_ERROR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_ITEM_ERROR") );
				break;

			case EMEXCHANGE_ITEM_ITEMTYPE_ERROR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_ITEMTYPE_ERROR") );
				break;

			case EMEXCHANGE_ITEM_INVALID_ITEM:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_INVALID_ITEM") );
				break;

			case EMEXCHANGE_ITEM_ERROR_NOCIP:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_ERROR_NOCIP") );
				break;

			case EMEXCHANGE_ITEM_DONE:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMEXCHANGE_ITEM_DONE") );
				break;
			};

			//close the change item card window
	...
	case NET_MSG_GCTRL_UPDATE_CHANGE_ITEM_POINTS:
		{
			GLMSG::SNETPC_UPDATE_CHANGE_ITEM_POINTS *pNetMsg = (GLMSG::SNETPC_UPDATE_CHANGE_ITEM_POINTS *) nmg;
			m_wExchangeItemPoints = pNetMsg->wChangeItemPoints;
		}
		break;

	case NET_MSG_GCTRL_REQ_EXCHANGE_ITEM_CARD_FB:
		{
			GLMSG::SNETPC_REQ_EXCHANGE_ITEM_CARD_FB *pNetMsg = (GLMSG::SNETPC_REQ_EXCHANGE_ITEM_CARD_FB *) nmg;

			switch ( pNetMsg->emFb )
			{
			case EMEXCHANGE_ITEM_ERROR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_POINT_ERROR") );
				break;

			case EMEXCHANGE_ITEM_POINT_CARD_ERROR:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMEXCHANGE_ITEM_POINT_CARD_ERROR") );
				break;

			case EMEXCHANGE_ITEM_POINT_CARD_DONE:
				{
					WORD wPrev = m_wExchangeItemPoints;
					m_wExchangeItemPoints = pNetMsg->wChangeItemPoint;
					CInnerInterface::GetInstance().PrintMsgText ( 
						NS_UITEXTCOLOR::PALEGREEN, 
						ID2GAMEINTEXT("EMEXCHANGE_ITEM_POINT_CARD_DONE"),
						std::abs( m_wExchangeItemPoints - wPrev ) );
				}
				break;
			};
		}
	...
	case NET_MSG_GCTRL_UPDATE_COMBAT_RECORD:
		{
			GLMSG::SNETPC_UPDATE_COMBAT_RECORD *pNetMsg = (GLMSG::SNETPC_UPDATE_COMBAT_RECORD *) nmg;
			m_sCombatRecord = pNetMsg->sCombatRecord;

//			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::GOLDENROD, "GLMSG::SNETPC_UPDATE_COMBAT_RECORD" );
		}
		break;

	case NET_MSG_GCTRL_CAPTCHA_UPDATE_SHOW_AC:
		{
			GLMSG::SNET_CAPTCHA_UPDATE_SHOW_AC *pNetMsg = (GLMSG::SNET_CAPTCHA_UPDATE_SHOW_AC *) nmg;

			m_bCaptchaWait = true;
			m_fCaptchaWaitTimer = pNetMsg->fWaitTime;
			
			CInnerInterface::GetInstance().ShowCaptcha( pNetMsg->dwCaptcha );
		}
		break;

	case NET_MSG_GCTRL_CAPTCHA_UPDATE_INPUT_AC:
		{
			GLMSG::SNET_CAPTCHA_UPDATE_INPUT_AC *pNetMsg = (GLMSG::SNET_CAPTCHA_UPDATE_INPUT_AC *) nmg;

			switch ( pNetMsg->emFb )
			{
			case EMCAPTCHA_CA_ERROR:
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMCAPTCHA_CA_ERROR") );
					CInnerInterface::GetInstance().HideCaptcha();
					CaptchaReset();
				}
				break;

			case EMCAPTCHA_CA_INCORRECT:
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMCAPTCHA_CA_INCORRECT") );
					CInnerInterface::GetInstance().CaptchaStartEdit();
				}
				break;

			case EMCAPTCHA_CA_SUCCESS:
				{
					CaptchaReset();
					
					CInnerInterface::GetInstance().HideCaptcha();
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMCAPTCHA_CA_SUCCESS") );
				}
				break;
			}
		}
	...
	case NET_MSG_GCTRL_UPDATE_BATTLEPASS_EXP:
		{
			GLMSG::SNETPC_UPDATE_BATTLEPASS_EXP *pNetMsg = (GLMSG::SNETPC_UPDATE_BATTLEPASS_EXP *)nmg;

			DWORD dwEXP = m_dwBattlePassEXP;
			m_dwBattlePassEXP = pNetMsg->dwEXP;

			if ( pNetMsg->bNotice && m_dwBattlePassEXP > dwEXP )
			{
				DWORD dwAdded = m_dwBattlePassEXP - dwEXP;
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("BATTLEPASS_EXP_UPDATE"), dwAdded );
			}
		}break;

	case NET_MSG_GCTRL_UPDATE_BATTLEPASS_LEVEL:
		{
			GLMSG::SNETPC_UPDATE_BATTLEPASS_LEVEL *pNetMsg = (GLMSG::SNETPC_UPDATE_BATTLEPASS_LEVEL *)nmg;

			m_dwBattlePassLevel = pNetMsg->dwLevel;

			if ( pNetMsg->bNotice )
			{
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("BATTLEPASS_LEVEL_UPDATE"), m_dwBattlePassLevel );
			}
	...
	case NET_MSG_GCTRL_BATTLEPASS_UPDATE:
		{
			GLMSG::SNETPC_BATTLEPASS_UPDATE *pNetMsg = (GLMSG::SNETPC_BATTLEPASS_UPDATE *)nmg;

			SBATTLEPASS_CHAR_DATA sbattlepass_char_data = pNetMsg->sData;

			SBATTLEPASS_CHAR_DATA* pbattlepass_char_data = GetBattlePassProg( sbattlepass_char_data.dwBattlePassID );
			if ( pbattlepass_char_data )
			{
				*pbattlepass_char_data = sbattlepass_char_data;
			}
		}break;

	case NET_MSG_GCTRL_BATTLEPASS_COMPLETE:
		{
			GLMSG::SNETPC_BATTLEPASS_COMPLETE *pNetMsg = (GLMSG::SNETPC_BATTLEPASS_COMPLETE *)nmg;

			SBATTLEPASS_CHAR_DATA sbattlepass_char_data = pNetMsg->sData;

			SBATTLEPASS_CHAR_DATA_MAP_ITER iterdel = m_mapBattlePassProg.find( sbattlepass_char_data.dwBattlePassID );
			if ( iterdel != m_mapBattlePassProg.end() )
			{
				m_mapBattlePassProg.erase( iterdel );
			}
	...
	case NET_MSG_GCTRL_BATTLEPASS_NOTIFY_CLIENT:
		{
			GLMSG::SNETPC_BATTLEPASS_NOTIFY_CLIENT *pNetMsg = (GLMSG::SNETPC_BATTLEPASS_NOTIFY_CLIENT *)nmg;
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::LIGHTSALMON, ID2GAMEINTEXT("BATTLEPASS_MISSION_COMPLETION"), pNetMsg->szBattlePassTitle );
		}break;
	
	case NET_MSG_GCTRL_REQ_BATTLEPASS_REWARD_DATA:
		{
			GLMSG::SNETPC_REQ_BATTLEPASS_REWARD_DATA *pNetMsg = (GLMSG::SNETPC_REQ_BATTLEPASS_REWARD_DATA *)nmg;

			DWORD dwLevel = pNetMsg->sRewardData.dwLevel;

			SBATTLEPASS_REWARD_CHAR_DATA_MAP_ITER iter = m_mapBattlePassReward.find(dwLevel);
			if (iter != m_mapBattlePassReward.end()) 
			{
				SBATTLEPASS_REWARD_CHAR_DATA& claimData = iter->second;
				claimData.bNormalClaimed = pNetMsg->sRewardData.bNormalClaimed;
				claimData.bPremiumClaimed = pNetMsg->sRewardData.bPremiumClaimed;
			} 
			else 
			{
				SBATTLEPASS_REWARD_CHAR_DATA newClaimData;
				newClaimData.dwLevel = dwLevel;
				newClaimData.bNormalClaimed = pNetMsg->sRewardData.bNormalClaimed;
				newClaimData.bPremiumClaimed = pNetMsg->sRewardData.bPremiumClaimed;
				m_mapBattlePassReward.insert(std::make_pair(dwLevel, newClaimData));
			}
		}
		break;
	...
	case NET_MSG_GCTRL_REQ_BATTLEPASS_PREMIUM_CARD_FB:
		{
			GLMSG::SNETPC_REQ_BATTLEPASS_PREMIUM_CARD_FB* pNetMsg = ( GLMSG::SNETPC_REQ_BATTLEPASS_PREMIUM_CARD_FB* ) nmg;

			switch( pNetMsg->emFB )
			{
			case EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_FAIL:
			...
			case EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_INVALID:
			...
			case EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_NOTCARD:
			...
			case EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_ALREADY:
			...
			case EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_SUCCESS:
				{
					CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_SUCCESS") );
					m_bBattlePassPremium = pNetMsg->bPremium;
				}
				break;
			};
		}break;
	...
	case NET_MSG_GCTRL_REQ_BATTLEPASS_LEVELUP_CARD_FB:
		{
			GLMSG::SNETPC_REQ_BATTLEPASS_LEVELUP_CARD_FB* pNetMsg = ( GLMSG::SNETPC_REQ_BATTLEPASS_LEVELUP_CARD_FB* ) nmg;

			switch( pNetMsg->emFB )
			{
			case EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_FAIL:
			...
			case EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_INVALID:
			...
			case EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_NOTCARD:
			...
			case EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_NOTPREMIUM:
			...
			case EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_MAX:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_MAX") );
				break;

			case EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_SUCCESS:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_SUCCESS") );
				break;
			};
		}break;
	...
	case NET_MSG_GCTRL_REQ_BATTLEPASS_REWARD_CLAIM_FB:
		{
			GLMSG::SNETPC_REQ_BATTLEPASS_REWARD_CLAIM_FB *pNetMsg = (GLMSG::SNETPC_REQ_BATTLEPASS_REWARD_CLAIM_FB *)nmg;
			switch( pNetMsg->emFB )
			{
			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_FAIL") );
				break;

			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTLEVEL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTLEVEL") );
				break;

			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTREWARD:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTREWARD") );
				break;

			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_ALREADY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_ALREADY") );
				break;

			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTPREMIUM:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTPREMIUM") );
				break;

			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTINVEN:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_NOTINVEN") );
				break;

			case EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_SUCCESS:
				{
					SITEM* pItem = GLItemMan::GetInstance().GetItem(pNetMsg->sItemID);
					if (pItem)
					{
						CString strTemp;
						strTemp.Format(ID2GAMEINTEXT("EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_SUCCESS"), pItem->GetName());
						CInnerInterface::GetInstance().PrintMsgTextDlg(NS_UITEXTCOLOR::PALEGREEN, strTemp.GetString());
					}
				}
				break;
	...
	case NET_MSG_GCTRL_DAILYLOGIN_UPDATE:
		{
			...
			{
				GLMSG::SNETPC_DAILYLOGIN_UPDATE *pNetMsg = (GLMSG::SNETPC_DAILYLOGIN_UPDATE *)nmg;

				m_dwDayStreakCounter = pNetMsg->dwStreak;

				if ( RPARAM::bFeatureDailyLogin )
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::LIGHTSALMON, ID2GAMEINTEXT("DAILYLOGIN_COMPLETION") );
				}
	...
	case NET_MSG_GCTRL_DAILYLOGIN_COMPLETE:
		{
			GLMSG::SNETPC_DAILYLOGIN_COMPLETE* pNetMsg = (GLMSG::SNETPC_DAILYLOGIN_COMPLETE*)nmg;

			DWORD dwDay = pNetMsg->sData.dwDay;

			SDAILYLOGIN_CHAR_DATA_MAP_ITER iter = m_mapDailyLogin.find(dwDay);
			if (iter != m_mapDailyLogin.end()) 
			{
				SDAILYLOGIN_CHAR_DATA& claimData = iter->second;
				claimData.bRewardClaimed = pNetMsg->sData.bRewardClaimed;
			} 
			else 
			{
				SDAILYLOGIN_CHAR_DATA newClaimData;
				newClaimData.dwDay = dwDay;
				newClaimData.bRewardClaimed = pNetMsg->sData.bRewardClaimed;
				m_mapDailyLogin.insert(std::make_pair(dwDay, newClaimData));
			}
		}
		break;
	...
	case NET_MSG_GCTRL_DAILYLOGIN_NOTIFY_CLIENT:
		{
			GLMSG::SNETPC_DAILYLOGIN_NOTIFY_CLIENT *pNetMsg = (GLMSG::SNETPC_DAILYLOGIN_NOTIFY_CLIENT *)nmg;

			m_fDailyLoginRemain = pNetMsg->fRemain;
			m_bDailyLoginRemain = true;
		}break;

	case NET_MSG_GCTRL_REQ_DAILYLOGIN_REWARD_CLAIM_FB:
		{
			GLMSG::SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM_FB *pNetMsg = (GLMSG::SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM_FB *)nmg;
			switch( pNetMsg->emFB )
			{
			case EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_FAIL:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_FAIL") );
				break;

			case EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_NOTDAY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_NOTDAY") );
				break;

			case EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_NOTREWARD:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_NOTREWARD") );
				break;

			case EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_ALREADY:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_ALREADY") );
				break;

			case EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_NOTINVEN:
				CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_NOTINVEN") );
				break;

			case EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_SUCCESS:
				{
					SITEM* pItem = GLItemMan::GetInstance().GetItem(pNetMsg->sItemID);
					if (pItem)
					{
						CString strTemp;
						strTemp.Format(ID2GAMEINTEXT("EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_SUCCESS"), pItem->GetName());
						CInnerInterface::GetInstance().PrintMsgTextDlg(NS_UITEXTCOLOR::PALEGREEN, strTemp.GetString());
					}
				}
				break;
	...
	case NET_MSG_GCTRL_GET_ITEMSHOP_FROMDB_FB:
		{
			...
			switch ( pNetMsg->emFB )
			{
			case EMREQ_ITEMSHOP_FROMDB_FB_END:
				{
					CInnerInterface::GetInstance().ItemShopVisible ();
				}break;

			case EMREQ_ITEMSHOP_FROMDB_FB_OK:
				{
	...
	case NET_MSG_GCTRL_BUY_ITEMSHOP_ITEM:
		{
			...
			if ( pNetMsg->bBuy )
			{
				CInnerInterface::GetInstance().PrintConsoleTextDlg ( ID2GAMEWORD("BUY_ITEMSHOP_ITEM",0));
				//Add Refresh After Buy
				CInnerInterface::GetInstance().HideGroup(ITEMSHOP_WINDOW);
				CInnerInterface::GetInstance().SetItemShopDelay(10.0f);
				CInnerInterface::GetInstance().ItemShopAuth();
			}
			else
			{
				CInnerInterface::GetInstance().PrintConsoleTextDlg ( ID2GAMEWORD("BUY_ITEMSHOP_ITEM",1));
			}
