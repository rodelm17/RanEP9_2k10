...
struct EFFCHAR_PROPERTY_SHOCK_100
{
	BOOL			m_bUse;
	int				m_nBlend;
	char			m_szTraceUP[STRACE_NSIZE];		//	ÃßÀû Æ÷ÀÎÅÍ ÀÎµ¦½º.
	char			m_szTraceDOWN[STRACE_NSIZE];	//	ÃßÀû Æ÷ÀÎÅÍ ÀÎµ¦½º.

	DWORD			m_dwFlag;

	DWORD			m_dwLightning;	// Àü±â °¹¼ö

	DWORD			m_dwDivision;	// ºÐÇÒ °¹¼ö.

	float			m_fWidth_In;
	float			m_fWidth_Out;

	DWORD			m_dwMaxLenth;	// Out ÈÖµµ·Ï
	DWORD			m_dwVelocity;	// In ÈÖµµ·Ï
	...
	{
		SecureZeroMemory( m_szTraceUP, sizeof(char)*STRACE_NSIZE );
		SecureZeroMemory( m_szTraceDOWN, sizeof(char)*STRACE_NSIZE );

		SecureZeroMemory( m_szTexture_In, sizeof(char)*STRING_NUMBER_256 );
		SecureZeroMemory( m_szTexture_Out, sizeof(char)*STRING_NUMBER_256 );
	};
};

struct EFFCHAR_PROPERTY_SHOCK_101
{
	DWORD			m_dwFlag;
	float			m_fFullTime;
	D3DXCOLOR		m_cColor;
