...
bool GLPVPSchoolWarsClient::FrameMove ( float fTime, float fElapsedTime  )
{
	...
	if ( IsBattle() )
	{
		if ( m_bDoublePoint )
		{
			...
			if ( m_fDoublePoint <= 0.0f )
			{
				...
				if ( GLGaeaClient::GetInstance().GetActiveMap()->m_bPVPSchoolWarsMap )
				{
					...
					CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_DOUBLE_POINT_OFF") );
				}
			}
		}
		else
		{
			m_fDoublePoint += fElapsedTime;
			if ( m_fDoublePoint >= SCHOOLWARS_DOUBLE_POINT_TIME )
			{
				m_bDoublePoint = true;
				...
				if ( GLGaeaClient::GetInstance().GetActiveMap()->m_bPVPSchoolWarsMap )
				{
					...
					CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_DOUBLE_POINT_ON") );
				}
			}
		}
	}
...
void GLPVPSchoolWarsClient::DoStateRegister()
{
	...
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_REG") );
}

void GLPVPSchoolWarsClient::DoStateBattle()
{
	CInnerInterface::GetInstance().PrintConsoleText( ID2GAMEINTEXT("PVP_SCHOOLWARS_BATTLE") );
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_BATTLE") );
...
void GLPVPSchoolWarsClient::DoStateReward()
{
	...
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_REWARD") );
}

void GLPVPSchoolWarsClient::DoStateEnd()
{
	CInnerInterface::GetInstance().PrintConsoleText( ID2GAMEINTEXT("PVP_SCHOOLWARS_ENDED") );
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_ENDED") );
...
void GLPVPSchoolWarsClient::NotifyTimeToStart( WORD wTime )
{
	if ( wTime > 0 ){
		...
		CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_SCHOOLWARS_TOBATTLE"), wTime );
	}
}
...
//	}
	...
	if (/* m_emState == SCHOOLWARS_STATE_REGISTER */ !m_bRegistered )
	{
...
BOOL GLPVPSchoolWarsClient::SetContributionPointMultiplier( float fMulti )
{
	m_fContributionPointMultiplier = fMulti;
	return TRUE;
}

float GLPVPSchoolWarsClient::GetContributionPointMultiplier()
{
