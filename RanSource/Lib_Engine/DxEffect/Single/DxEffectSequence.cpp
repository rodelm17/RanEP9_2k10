...
HRESULT SEQUENCE_PROPERTY::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "SEQUENCE_PROPERTY::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT SEQUENCE_PROPERTY::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT DxEffectSequence::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX &matComb )
{
	...
	if ( m_dwFlag&USEBILLBOARD )
	{
		...
		if ( g_bPREVIEW_CHAR_RENDER )
		{
			...
			matLocal = DxBillboardLookAt( &vPlayPos, matViewLocal );
		}
	...
	else if ( m_dwFlag&USEBILLBOARDUP )
	{
		...
		if ( g_bPREVIEW_CHAR_RENDER )
		{
			...
			matTrans = DxBillboardLookAtHeight( &m_vPlayPos, matViewLocal );
		}
