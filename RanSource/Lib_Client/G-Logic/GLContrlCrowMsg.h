...
namespace GLMSG
{
	...
	/*woe Arc Development 08-06-2024*/
	struct SNET_MSG_WOE_CROW_OWNER : public SNETCROW
	{
		WORD	wWoeOwner;

		SNET_MSG_WOE_CROW_OWNER()
			: wWoeOwner(0)
		{
			nmg.dwSize = sizeof(SNET_MSG_WOE_CROW_OWNER);
			nmg.nType = NET_MSG_GCTRL_WOE_CROW_OWNER;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	/*pvp tyranny, Juver, 2017/08/25 */
	...
	{
		WORD	wTyrannyOwner;
		DWORD	dwTyrannyClubOwner;
		DWORD	dwTyrannyClubMarkVer;

		SNETCROW_TYRANNY_OWNER () 
			: wTyrannyOwner( TYRANNY_SCHOOL_SIZE )
			, dwTyrannyClubOwner(CLUB_NULL)
			, dwTyrannyClubMarkVer(0)
		{
