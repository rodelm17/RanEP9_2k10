...
void CItemTransferWindow::Update( INT x, INT y, BYTE LB, BYTE MB, BYTE RB, INT nScroll, FLOAT fElapsedTime, BOOL bFirstControl )
{
	...
	if ( m_pTextCost )
	{
		...
		if ( RPARAM::dwItemTransferCost )
		{
			CString strTemp = NS_UITEXTCONTROL::MAKE_MONEY_FORMAT( RPARAM::dwItemTransferCost, 3, "," );
			...
			if( CHECK_MOUSE_IN( dwMsg ) )
			{
				...
				if( sItemCustom_OLD.sNativeID != NATIVEID_NULL() && sItemCustom_NEW.sNativeID != NATIVEID_NULL() )
				{
					...
					if ( DxInputDevice::GetInstance().GetKeyState ( DIK_LCONTROL  ) & DXKEY_DOWNED )
					{
						if ( dwMsg & UIMSG_LB_UP )
						{
							CInnerInterface::GetInstance().ReqMaxRVShow( sItemCustom_OLD );
							return;
						}
					}
				}
...
void CItemTransferWindow::TransferItemStats()
{
	...
	if ( RPARAM::dwItemTransferCost != 0 && GLGaeaClient::GetInstance().GetCharacter()->m_lnMoney < RPARAM::dwItemTransferCost ) 
	{
