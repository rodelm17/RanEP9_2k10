...
#include "./Core/NSREngineGlobal.h"
...
#include "./DxEffect/DxStaticMaterialMeshThread.h"
...
HRESULT DxResponseMan::OneTimeSceneInit ( const char* szAppPath, CWnd *pWndApp, const CString& strFontSys, language::LANGFLAG dwLang, const CString& strFontName  )
{
	GASSERT(pWndApp);
	GASSERT(szAppPath);
	...
	RENGINE_GLOBAL::OneTimeSceneInit();
	...
	//	Note : 디버그샛의 초기화.
	//
	CDebugSet::OneTimeSceneInit ( szAppPath, true );
	...
	StringCchCopy( szFullPath, MAX_PATH, szAppPath );
	StringCchCat( szFullPath, MAX_PATH, SUBPATH::ENGINE_FILE );
	GLVMSetting::GetInstance().SetPath( szFullPath );

	StringCchCopy( szFullPath, MAX_PATH, szAppPath );
	StringCchCat( szFullPath, MAX_PATH, SUBPATH::ENGINE_FILE );
	DxMaterialManager::GetInstance().SetPath( szFullPath );

	StringCchCopy( szFullPath, MAX_PATH, szAppPath );
	StringCchCat( szFullPath, MAX_PATH, SUBPATH::ENGINE_FILE );
	DxShaderManager::GetInstance().SetPath( szFullPath );
	DxShaderData::GetInstance().SetPath( szFullPath );
	...
	DxStaticMaterialMeshColor_THREAD::GetInstance().StartThread( m_pd3dDevice );
...
HRESULT DxResponseMan::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bCAPTUREMOUSE )
{
	...
	RENGINE_GLOBAL::InitDeviceObjects( m_pd3dDevice );

	GLVMSetting::GetInstance().LoadFile();

	DxShaderData::GetInstance().LoadFile( "GameShader.shsf" );
	DxShaderManager::GetInstance().CreateDevice( m_pd3dDevice );
	DxMaterialManager::GetInstance().LoadFile( pd3dDevice, "GameMaterial.mmsf" );
	...
	DxSoftShadowMap::GetInstance().InitDeviceObjects ( m_pD3D, m_pd3dDevice, m_d3dsdBackBuffer );	// 쉐도우 맵 초기화
...
HRESULT DxResponseMan::SetTextureFilters ()
{
	...
	m_pd3dDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, RENGINE_GLOBAL::dwSamplerMinFilter );
	m_pd3dDevice->SetSamplerState( 0, D3DSAMP_MAGFILTER, RENGINE_GLOBAL::dwSamplerMagFilter );
	m_pd3dDevice->SetSamplerState( 1, D3DSAMP_MINFILTER, RENGINE_GLOBAL::dwSamplerMinFilter );
	m_pd3dDevice->SetSamplerState( 1, D3DSAMP_MAGFILTER, RENGINE_GLOBAL::dwSamplerMagFilter );
	m_pd3dDevice->SetSamplerState( 2, D3DSAMP_MINFILTER, RENGINE_GLOBAL::dwSamplerMinFilter );
	m_pd3dDevice->SetSamplerState( 2, D3DSAMP_MAGFILTER, RENGINE_GLOBAL::dwSamplerMagFilter );
	m_pd3dDevice->SetSamplerState( 3, D3DSAMP_MINFILTER, RENGINE_GLOBAL::dwSamplerMinFilter );
	m_pd3dDevice->SetSamplerState( 3, D3DSAMP_MAGFILTER, RENGINE_GLOBAL::dwSamplerMagFilter );
	m_pd3dDevice->SetSamplerState( 0, D3DSAMP_MAXANISOTROPY, RENGINE_GLOBAL::dwSamplerMaxAnisotropy );
	m_pd3dDevice->SetSamplerState( 1, D3DSAMP_MAXANISOTROPY, RENGINE_GLOBAL::dwSamplerMaxAnisotropy );
	m_pd3dDevice->SetSamplerState( 2, D3DSAMP_MAXANISOTROPY, RENGINE_GLOBAL::dwSamplerMaxAnisotropy );
	m_pd3dDevice->SetSamplerState( 3, D3DSAMP_MAXANISOTROPY, RENGINE_GLOBAL::dwSamplerMaxAnisotropy );
...
HRESULT DxResponseMan::RestoreDeviceObjects ()
{
	...
	RENGINE_GLOBAL::RestoreDeviceObjects();
	...
	DxShaderManager::GetInstance().ResetDevice( m_pd3dDevice );
	...
	DxSoftShadowMap::GetInstance().RestoreDeviceObjects ( m_pd3dDevice );		// 쉐도우 맵
...
HRESULT DxResponseMan::FrameMove ( float fTime, float fElapsedTime, BOOL bDefWin, BOOL bGame )
{
	...
	RENGINE_GLOBAL::FrameMove( fTime, fElapsedTime );
	...
	DxShaderManager::GetInstance().FrameMove( m_pd3dDevice, fTime, fElapsedTime );

	// Note : List가 비워있는지를 조사.
	DxStaticMeshColor_THREAD::GetInstance().CheckListEmpty();
	DxStaticMaterialMeshColor_THREAD::GetInstance().CheckListEmpty();

	// Note 
	DxShadowMap::GetInstance().FrameMove ( fTime, fElapsedTime );
	DxSoftShadowMap::GetInstance().FrameMove ( fTime, fElapsedTime );
...
HRESULT DxResponseMan::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	RENGINE_GLOBAL::Render( pd3dDevice );
...
HRESULT DxResponseMan::InvalidateDeviceObjects ()
{
	...
	RENGINE_GLOBAL::InvalidateDeviceObjects();
	...
	DxSoftShadowMap::GetInstance().InvalidateDeviceObjects ( m_pd3dDevice );	// 쉐도우 맵
	...
	DxShaderManager::GetInstance().LostDevice( m_pd3dDevice );
...
HRESULT DxResponseMan::DeleteDeviceObjects ()
{
	...
	DxSoftShadowMap::GetInstance().DeleteDeviceObjects ();		// 쉐도우 맵
	...
	DxShaderManager::GetInstance().DestroyDevice();
	...
	RENGINE_GLOBAL::DeleteDeviceObjects();
...
HRESULT DxResponseMan::FinalCleanup ()
{
	...
	DxStaticMaterialMeshColor_THREAD::GetInstance().EndThread();
	...
	DxMaterialManager::GetInstance().ClearTexture();
...
HRESULT DxResponseMan::DoInterimClean ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	RENGINE_GLOBAL::DoInterimClean( pd3dDevice );
