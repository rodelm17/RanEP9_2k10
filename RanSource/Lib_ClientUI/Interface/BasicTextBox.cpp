...
CBasicTextBox::CBasicTextBox () :
	...
	m_bChat ( false )/* Chat Color, Mhundz 02/22/25 */
...
{	
	...
	for ( int i = nPartIndex; i < nPartSize; ++i )
	{
		...
		for ( int j = nLineIndex; j < nLineCount; ++j )
		{
			...
			if ( m_nAlign & TEXT_ALIGN_BOTH_X && 1 < vectorTextWord.size() && !m_bChat )/* Chat Color, Mhundz 02/22/25 */
			{
				long lGAP = 0;
				long OneGap = 0;
...
/* Chat Color, Mhundz 02/22/25 */
int	CBasicTextBox::AddTextChat ( STEXTCHAT strText, SITEMLINK* pItemLink, const D3DCOLOR& TextColor )
{
	...
	if( strText.strName.GetLength() ) {
		strText.strName = "(" + strText.strName + ")";
	}

	CString strTemp;

	strTemp = strText.strHeader + strText.strName + strText.strMain;

	NS_UITEXTUTIL::ClearCRLFText ();
	NS_UITEXTUTIL::MULTILINETEXT& MLText = NS_UITEXTUTIL::GetMLTextWithoutCRLF ( strTemp, GetLocalPos().sizeX, m_pFont );
	...
	if ( pItemLink )
	{
		...
		if ( pITEMFIND )
		{
			...
			int nSTART = 0;
			{
				...
				std::string strSEARCH = strTemp.GetString();
	...
	{		
		...
		else
		{
			...
			{
				STEXTWORD sTextWord;
				if ( m_bChat  && i == 0 && strText.strName.GetLength() )
				{
					// Chat Type (
					sTextWord.strWord = MLText[i].strLine.Mid ( 0, strText.strHeader.GetLength() + 1 );
					sTextWord.dwColor = TextColor;
					...
					if ( strText.bStaff )
					sTextWord.dwColor = D3DCOLOR_ARGB(0xFF,0x00,0xFF,0x00);
					else
					sTextWord.dwColor = D3DCOLOR_ARGB(0xFF,0x1E,0x90,0xFF);
					m_pFont->GetTextExtent ( sTextWord.strWord.GetString(), strSize );
					sTextWord.strSize = strSize;
					...
					sTextWord.strWord = MLText[i].strLine.Mid( (strText.strHeader.GetLength() + strText.strName.GetLength()) - 1 );
					sTextWord.dwColor = TextColor;
					m_pFont->GetTextExtent ( sTextWord.strWord.GetString(), strSize );
					sTextWord.strSize = strSize;
				...
				else if ( i == 0 && !strText.strName.GetLength() )
				{
					strText.strMain = MLText[i].strLine.Mid ( strText.strHeader.GetLength() );


					sTextWord.strWord = strText.strHeader;
					sTextWord.dwColor = TextColor;
					m_pFont->GetTextExtent ( sTextWord.strWord.GetString(), strSize );
					sTextWord.strSize = strSize;
					...
					sTextWord.strWord = strText.strMain;
					sTextWord.dwColor = TextColor;
					m_pFont->GetTextExtent ( sTextWord.strWord.GetString(), strSize );
					sTextWord.strSize = strSize;
				...
				{
					/*if( strText.strHeader.GetLength() && !strText.strName.GetLength() ) 
					{
