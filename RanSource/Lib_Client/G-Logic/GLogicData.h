...
enum GLCONST
{
	...
	MAX_CP_LIST			= 50000,	//maximum array size
	DEFAULT_CP_MAX		= 1,		//default maximum cp incase the array has no value
};
...
struct GLCONST_CHARCLASS
{
	...
	{
		VERSION_TEXT			= 1,
		VERSION_CLASSCONST		= 0x0101,

		ENCODE_VER_BYTE_CRYPT_2021_CLASS_CONST_V1 = 0x0101,
	...
	BOOL LOADFILE_STRING ( const char* szFileName, bool bVersionCheck = false );
	BOOL LOADFILE_STREAM ( const char* szFileName );
	BOOL SAVEFILE_STREAM ( const char* szFileName );
	BOOL SAVEFILE_STRING ( const char* szFilePath );
...
//	extern float		fDISPRICE;				//	아이템 재판매 가격.
	...
	extern float		fUNIT_TIME_CROW;		//	회복 단위 시간.		
	extern float		fHP_INC_PER_CROW;		//	단위시간당 HP 회복율(%)
	extern float		fMP_INC_PER_CROW;		//	단위시간당 MP 회복율(%)
	extern float		fSP_INC_PER_CROW;		//	단위시간당 SP 회복율(%)
	...
	extern float		fDAMAGE_DEC_MAX_G;
	...
	extern float		fPK_POINT_DEC_PHY[GLCL_NUM_CLASS];		//  pk 데미지 감소율 ( 물리 ).
	extern float		fPK_POINT_DEC_MAGIC[GLCL_NUM_CLASS];	//  pk 데미지 감소율 ( 마법 ).
	...
	extern LONGLONG		lnEXP_MAX_TABLE[MAX_CHARACTER_LEVEL+1];
	extern LONGLONG		lnEXP_MAX_TABLE_2nd[MAX_CHARACTER_LEVEL+1];
	extern DWORD		dwEXP_PC_TABLE[MAX_CHARACTER_LEVEL+1];
	...
	extern SNATIVEID	nidGachaItem;
	extern WORD			wGachaMaxPoint;
	...
	//////////////////////////////////////////////
	//MMR Rank
	extern DWORD		dwRank_Point[10];
	extern std::string	strRank_Name[10];
	extern std::string	strRank_Badge[10];
	extern BOOL			bEnableRankName;
	extern BOOL			bEnableRankMark;
	extern WORD			dwMMRMax;
	//////////////////////////////////////////////
	...
	extern BOOL					bCONSTCLASS[GLCI_NUM_8CLASS];
	...
	extern	std::string strSimpleWearM_SG[SLOT_WEAR_SIMPLE];
	extern	std::string strSimpleWearF_SG[SLOT_WEAR_SIMPLE];
	extern	std::string strSimpleWearM_MP[SLOT_WEAR_SIMPLE];
	extern	std::string strSimpleWearF_MP[SLOT_WEAR_SIMPLE];
	extern	std::string strSimpleWearM_PHX[SLOT_WEAR_SIMPLE];
	extern	std::string strSimpleWearF_PHX[SLOT_WEAR_SIMPLE];
	...
	extern DWORD		dwBattlePassEXP[BATTLEPASS_LEVEL_MAX];
	...
	extern float	fBECOME_AWAY_TIME;
	extern int		nAWAY_GESTURE_NUMBER;	

	/* pvp club death match, Juver, 2020/11/10 */
	extern std::string strClubDeathMatchFile;

	extern std::string strPVPPBGFile;
	...
	BOOL 	LOADFILE_PANDORA ( char* szFileName ); //Pandora
	//////////////////////////////////////////////////
	//MMR Rank
	...
	extern std::vector<WORD>	vecCPMAX; //add cp

	BOOL CPMAX_LOADFILE ( char* szFileName );//add cp
