...
void GLPetClient::ProceedSkill_A ()
{
	...
	PETSKILL_MAP_CITER learniter = m_ExpSkills.find ( m_sActiveSkillID_A.dwID );
						...
						case ITEM_DRUG_HP_MP_SP_CURE:
							{
								if ( pITEM->sDrugOp.bRatio )
								{
									...
									{
										m_pOwner->ReqInvenDrug ( pInvenItem->wPosX, pInvenItem->wPosY, FALSE );
										return;
									}
								}
								else
...
void GLPetClient::ProceedSkill_B ()
{
	...
	PETSKILL_MAP_CITER learniter = m_ExpSkills.find ( m_sActiveSkillID_B.dwID );
	if ( learniter==m_ExpSkills.end() )		return;
