...
#include "../Lib_Client/G-Logic/GLProductDefine.h"
...
BEGIN_MESSAGE_MAP(CEditorItemMixDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_ITEMMIX_COPY, OnBnClickedButtonItemmixCopy)
	ON_BN_CLICKED(IDC_BUTTON_CSV_LOAD, &CEditorItemMixDlg::OnBnClickedButtonCsvLoad)
	ON_BN_CLICKED(IDC_BUTTON_CSV_SAVE, &CEditorItemMixDlg::OnBnClickedButtonCsvSave)
END_MESSAGE_MAP()
...
void CEditorItemMixDlg::OnBnClickedButtonFileLoad()
{
	...
	CFileDialog dlg( TRUE, ".ims", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
{
	CString szFilter = "Item Mix Setting File (*.ims)|*.ims|";
	CFileDialog dlg( FALSE, ".ims", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this);
...
void CEditorItemMixDlg::OnBnClickedButtonItemmixSave()
{
	...
	sITEMMIX.llPlayTimeReq = GetWin_Num_LONGLONG( this, IDC_EDIT_DATA_PLAY_TIME_REQ );
	sITEMMIX.llPlayPointUse = GetWin_Num_LONGLONG( this, IDC_EDIT_DATA_PLAY_POINT_USE );

	sITEMMIX.dwContriUse = GetWin_Num_int( this, IDC_EDIT_DATA_CONTRI_USE );
	...
	sITEMMIX.bUseStat = GetWin_Check( this, IDC_CHECK_USE_STAT );
...
void CEditorItemMixDlg::ItemMixShow( ITEM_MIX sITEMMIX, BOOL bENABLE, BOOL bADD )
{
	...
	/* play time system, Juver, 2021/01/26 */
	SetWin_Enable( this, IDC_EDIT_DATA_PLAY_TIME_REQ, bENABLE );
	SetWin_Enable( this, IDC_EDIT_DATA_PLAY_POINT_USE, bENABLE );

	SetWin_Enable( this, IDC_EDIT_DATA_CONTRI_USE, bENABLE );
	...
	/* play time system, Juver, 2021/01/26 */
	SetWin_Num_LONGLONG( this, IDC_EDIT_DATA_PLAY_TIME_REQ, sITEMMIX.llPlayTimeReq );
	SetWin_Num_LONGLONG( this, IDC_EDIT_DATA_PLAY_POINT_USE, sITEMMIX.llPlayPointUse );

	SetWin_Num_int( this, IDC_EDIT_DATA_CONTRI_USE, sITEMMIX.dwContriUse );
	...
	SetWin_Enable( this, IDC_CHECK_USE_STAT, bENABLE );
	...
	SetWin_Check( this, IDC_CHECK_USE_STAT, sITEMMIX.bUseStat );
...
void CEditorItemMixDlg::OnBnClickedButtonItemmixCopy()
{
	int nSelect = m_ListItemMix.GetNextItem(-1, LVNI_ALL | LVNI_SELECTED);
	if( nSelect == -1 ) return;
	DWORD dwKeyFind = m_ListItemMix.GetItemData( nSelect );
	...
	ITEMMIX_KEYMAP_ITER iterpos = mapItemMix.find(dwKeyFind);
	if ( iterpos != mapItemMix.end() )
	{
		ITEM_MIX sITEMMIX = (*iterpos).second;
		sITEMMIX.dwKey = dwNEWKEY;
...
void CEditorItemMixDlg::OnBnClickedButtonCsvLoad()
{
	if( GLItemMixMan::GetInstance().LoadCsvFile( this ) == S_OK )
		MessageBox( _T("CSV Load Success"), _T("Success"), MB_OK );

	UpdateItemMixList();
}

void CEditorItemMixDlg::OnBnClickedButtonCsvSave()
{
	if( GLItemMixMan::GetInstance().SaveCsvFile( this ) == S_OK )
