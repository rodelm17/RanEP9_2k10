...
//
//	class DxStaticMaterialMesh_THREAD : 기존의 Thread Loading 부분 또한 이곳으로 옮겨졌음.
...
class DxStaticMaterialMesh_THREAD
{
protected:
...
class DxStaticMaterialMeshColor_THREAD
{
protected:
	DWORD	dwThreadID;			//	스레드 ID
	HANDLE	hThread;			//	스레드 핸들
...
public:
	DxStaticMaterialMeshColor_THREAD();
	~DxStaticMaterialMeshColor_THREAD();

public:
	static DxStaticMaterialMeshColor_THREAD& GetInstance();
};
