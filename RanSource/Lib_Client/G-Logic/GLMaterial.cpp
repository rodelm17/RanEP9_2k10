...
HRESULT GLMaterial::UpdateGathering( float fElapsedTime )
{
	...
	if ( m_fGatherTime < 0.0f )
	{
		...
		if ( RANDOM_GEN( m_pCrowData->m_sBasic.m_fGatherRate ) ) 
		{
			bool bDrop = false;
		...
		if ( sGenItem.ISGEN(fGenRate) )
		{
			...
			for ( WORD i=0; i<wGenNum; ++i )
			{
				...
				if ( pChar->IsInsertToInven ( &cDropItem ) )
				{
					...
					GLITEMLMT::GetInstance().ReqItemRoute ( sITEM_NEW, ID_MOB, m_pCrowData->m_sBasic.sNativeID.dwID, ID_CHAR, pChar->m_dwCharID, 
				...
				else
				{
					...
					{
						bDrop = true;
						m_pLandMan->DropItem ( m_vPos, &(cDropItem.sItemCustom), emItemHoldGrp, dwItemHoldID );
					}
				}
			...
			if ( pChar )
			{
				for ( WORD i=0; i<wGenNum; ++i )
				{
					...
					if ( !pChar->ISHAVE_BUFF( sidskill, 1 ) )	continue;
