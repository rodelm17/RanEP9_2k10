...
enum EMREQ_REGISTER_CODEX_FB
{
	...
	EMREQ_REGISTER_CODEX_FB_DONE= 7,	//	잘못된 아이템이 들어 있음.
};
...
enum EMREBUILD_RESULT	// ITEMREBUILD_MARK
{
	...
	EMREBUILD_RESULT_INVALID_ITEM	= 6,
	EMREBUILD_RESULT_MAXED			= 7,	/* Item Reform use limit, Juver, 2021/07/23 */
};
...
enum EMNPCSHOP_PURCHASE_FB
{
	...
	EMNPCSHOP_PURCHASE_FB_INVALID_SHOP		= 7,
};

/* Reset Stats Item */
enum EMREQ_RESET_STATS_FB
{
	EMREQ_RESET_STATS_FB_FAIL		= 0,
	EMREQ_RESET_STATS_FB_OK			= 1,
	EMREQ_RESET_STATS_FB_NOITEM		= 2,
	EMREQ_RESET_STATS_FB_NOTITEM	= 3,
};

enum EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB
{
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_PURCHASED			= 0,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_ITEM		= 1,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_CROW		= 2,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_SHOPITEM	= 3,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_INVALID_SHOP		= 4,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_NOT_CONTRIBUTION	= 5,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_NOT_INVEN_SPACE		= 6,
	EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_NOT_GOLD			= 7,
};

enum EMMAXRV_PREVIEW_FB
{
	EMMAXRV_PREVIEW_FB_ERROR			= 0,
	EMMAXRV_PREVIEW_FB_INVALID_ITEM		= 1,
	EMMAXRV_PREVIEW_FB_INVALID_ITEM_RV	= 2,
	EMMAXRV_PREVIEW_FB_COOLTIME			= 3,
	EMMAXRV_PREVIEW_FB_START			= 4,
};
...
namespace GLMSG
{
	...
	struct SNETPC_PICKUP_ITEM
	{
			...
			: sid_disguise(false) /*item wrapper, Juver, 2018/01/12 */
		{
			...
			BOOST_STATIC_ASSERT(sizeof(SNETPC_PICKUP_ITEM)<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_OPEN_GACHA
	{
		...
		SNET_OPEN_GACHA () 
		{
			nmg.dwSize = sizeof(SNET_OPEN_GACHA);
			nmg.nType = NET_MSG_GCTRL_OPEN_GACHA;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	/* Gacha System, MontageDev 7/10/24 */
	struct SNETPC_UPDATE_CHA_PROMISE_POINTS
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwVal;
		char				szName[CHAR_SZNAME];

		SNETPC_UPDATE_CHA_PROMISE_POINTS ()
			: dwVal(0)
		{
			memset (szName, 0, sizeof(char) * CHAR_SZNAME);

			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_UPDATE_CHA_PROMISE_POINTS;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_INVEN_RANDOMBOXOPEN_FB
	{
		...
		WORD			nSelected; /* Gacha System, MontageDev 7/10/24 */

		SNET_INVEN_RANDOMBOXOPEN_FB () 
			: emFB(EMINVEN_RANDOMBOXOPEN_FB_FAIL)
			, nSelected(0) /* Gacha System, MontageDev 7/10/24 */
		{
	...
	{
		NET_MSG_GENERIC		nmg;
		WORD				wPosX[ITEM_GARBAGE_MAX_SLOT];	// 삭제할 아이템 좌표
		WORD				wPosY[ITEM_GARBAGE_MAX_SLOT];	// 삭제할 아이템 좌표
		BOOL				bUseCard;
			...
			: bUseCard(TRUE)
		{
			...
			{
				wPosX[i] = EM_INVENSIZE_X;
				wPosY[i] = EM_INVENSIZE_Y;
			}
	...
	struct SNET_INVEN_RESET_STATS
	{
		...
		SNET_INVEN_RESET_STATS () 
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNET_INVEN_RESET_STATS);
			nmg.nType = NET_MSG_GCTRL_INVEN_RESET_STATS;
		}
	};

	struct SNET_INVEN_RESET_STATS_FB
	{
		NET_MSG_GENERIC			nmg;

		WORD					wSTATS_POINT;
		EMREQ_RESET_STATS_FB	emFB;

		SNET_INVEN_RESET_STATS_FB () 
			: wSTATS_POINT(0)
			, emFB(EMREQ_RESET_STATS_FB_FAIL)
		{
			nmg.dwSize = sizeof(SNET_INVEN_RESET_STATS_FB);
			nmg.nType = NET_MSG_GCTRL_INVEN_RESET_STATS_FB;
		}
	};
	...
	struct SNET_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC
	{
		NET_MSG_GENERIC			nmg;
		...
		WORD		pos_y;
		SNATIVEID	item_id;
		SNATIVEID	item_id_selected;
		int			item_selected_index;

		WORD		wLevel;

		SNET_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC () 
			: pos_x(0)
			, pos_y(0)
			, item_id(false)
			, item_id_selected(false)
			, item_selected_index(false)
			, wLevel(0)
		{
			nmg.dwSize = sizeof(SNET_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC);
			nmg.nType = NET_MSG_GCTRL_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/*specific item box, Juver, 2018/09/03 */
	struct SNET_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC_FB
	{
		NET_MSG_GENERIC		nmg;

		EMREQ_ITEM_BOX_SPECIFIC	emFB;

		SNET_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC_FB () 
			: emFB(EMREQ_ITEM_BOX_SPECIFIC_FAILED)
		{
			nmg.dwSize = sizeof(SNET_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC_FB);
			nmg.nType = NET_MSG_GCTRL_INVEN_REQ_OPEN_ITEM_BOX_SPECIFIC_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_CURRENCY_SHOP_PURCHASE
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETPC_REQ_CURRENCY_SHOP_PURCHASE () 
			: sCrowID(false)
			, sItemID(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_CURRENCY_SHOP_PURCHASE);
			nmg.nType = NET_MSG_GCTRL_CURRENCY_SHOP_PURCHASE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};


	struct SNETPC_REQ_CURRENCY_SHOP_PURCHASE_FB
	{
		NET_MSG_GENERIC				nmg;
		EMREQ_CURRENCY_SHOP_PURCHASE_FB	emFB;
		
		SNETPC_REQ_CURRENCY_SHOP_PURCHASE_FB ()
			: emFB(EMREQ_CURRENCY_SHOP_PURCHASE_ERROR)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_CURRENCY_SHOP_PURCHASE_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_INVEN_SET_OPTION_INSERT
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_SET_OPTION_INSERT () 
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNET_INVEN_SET_OPTION_INSERT);
			nmg.nType = NET_MSG_GCTRL_INVEN_SET_OPTION_INSERT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* set item option, Juver, 2021/09/04 */
	struct SNET_INVEN_SET_OPTION_INSERT_FB
	{
		NET_MSG_GENERIC		nmg;

		EM_SET_OPTION_INSERT_FB	emFB;

		SNET_INVEN_SET_OPTION_INSERT_FB () 
			: emFB(EM_SET_OPTION_INSERT_FAILED)
		{
			nmg.dwSize = sizeof(SNET_INVEN_SET_OPTION_INSERT_FB);
			nmg.nType = NET_MSG_GCTRL_INVEN_SET_OPTION_INSERT_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* set item option, Juver, 2021/09/04 */
	struct SNET_INVEN_SET_OPTION_REROLL
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_SET_OPTION_REROLL () 
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNET_INVEN_SET_OPTION_REROLL);
			nmg.nType = NET_MSG_GCTRL_INVEN_SET_OPTION_REROLL;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	/* set item option, Juver, 2021/09/04 */
	struct SNET_INVEN_SET_OPTION_REROLL_FB
	{
		NET_MSG_GENERIC		nmg;

		EM_SET_OPTION_REROLL_FB	emFB;

		SNET_INVEN_SET_OPTION_REROLL_FB () 
			: emFB(EM_SET_OPTION_REROLL_FAILED)
		{
			nmg.dwSize = sizeof(SNET_INVEN_SET_OPTION_REROLL_FB);
			nmg.nType = NET_MSG_GCTRL_INVEN_SET_OPTION_REROLL_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE () 
			: sidCrow(NATIVEID_NULL())
			, sidItem(NATIVEID_NULL())
		{
			nmg.dwSize = sizeof(SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE);
			nmg.nType = NET_MSG_GCTRL_NPC_CONTRIBUTION_SHOP_PURCHASE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE_FB
	{
		NET_MSG_GENERIC			nmg;
		EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB	emFB;

		SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE_FB()
			: emFB( EMNPC_CONTRIBUTION_SHOP_PURCHASE_FB_PURCHASED )
		{
			nmg.dwSize = sizeof(SNETPC_REQ_NPC_CONTRIBUTION_SHOP_PURCHASE_FB);
			nmg.nType = NET_MSG_GCTRL_NPC_CONTRIBUTION_SHOP_PURCHASE_FB;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};

	struct SNETPC_REQ_MAXRV_PREVIEW
	{
		NET_MSG_GENERIC		nmg;

		SNATIVEID sID;

		SNETPC_REQ_MAXRV_PREVIEW () 
			: sID(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_MAXRV_PREVIEW);
			nmg.nType = NET_MSG_GCTRL_REQ_MAXRV_PREVIEW;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_MAXRV_PREVIEW_FB
	{
		NET_MSG_GENERIC		nmg;

		EMMAXRV_PREVIEW_FB	emFB;

		SNETPC_REQ_MAXRV_PREVIEW_FB () 
			: emFB(EMMAXRV_PREVIEW_FB_ERROR)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_MAXRV_PREVIEW_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_MAXRV_PREVIEW_DATA_INIT
	{
		NET_MSG_GENERIC		nmg;

		SNATIVEID sID;

		DWORD dwMaxRandomMobGen;
		DWORD dwMaxRandomRebuild;

		SNETPC_REQ_MAXRV_PREVIEW_DATA_INIT () 
			: sID(false)
			, dwMaxRandomMobGen(0)
			, dwMaxRandomRebuild(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_MAXRV_PREVIEW_DATA_INIT);
			nmg.nType = NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_DATA_INIT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_MAXRV_PREVIEW_DATA_MOB_GEN
	{
		NET_MSG_GENERIC		nmg;
		SMAXRV_OPTION sOption;

		SNETPC_REQ_MAXRV_PREVIEW_DATA_MOB_GEN () 
		{
			nmg.dwSize = sizeof(SNETPC_REQ_MAXRV_PREVIEW_DATA_MOB_GEN);
			nmg.nType = NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_DATA_MOB_GEN;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_MAXRV_PREVIEW_DATA_REBUILD
	{
		NET_MSG_GENERIC		nmg;
		SMAXRV_OPTION sOption;

		SNETPC_REQ_MAXRV_PREVIEW_DATA_REBUILD () 
		{
			nmg.dwSize = sizeof(SNETPC_REQ_MAXRV_PREVIEW_DATA_REBUILD);
			nmg.nType = NET_MSG_GCTRL_REQ_MAXRV_PREVIEW_DATA_REBUILD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};


	struct SNETPC_REQ_PURCHASE_SKILL
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID	sSkillID;

		SNETPC_REQ_PURCHASE_SKILL () 
			: sSkillID(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_PURCHASE_SKILL);
			nmg.nType = NET_MSG_GCTRL_REQ_PURCHASE_SKILL;
		}
	};

	struct SNETPC_REQ_PURCHASE_SKILL_FB
	{
		NET_MSG_GENERIC		nmg;
		SNATIVEID	sSkillID;
		EMREQ_PURCHASE_SKILL_FB emFB;

		SNETPC_REQ_PURCHASE_SKILL_FB () 
			: sSkillID(false)
			, emFB(EMREQ_PURCHASE_SKILL_FB_FAIL)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_PURCHASE_SKILL);
			nmg.nType = NET_MSG_GCTRL_REQ_PURCHASE_SKILL_FB;
		}
	};

	struct SNETPC_REQ_AUTO_POTION
	{
		NET_MSG_GENERIC		nmg;
		float fHP;
		float fMP;
		...
		SNETPC_REQ_AUTO_POTION () 
			: fHP(100.0f)
			, fMP(100.0f)
			, fSP(100.0f)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_AUTO_POTION);
			nmg.nType = NET_MSG_GCTRL_REQ_AUTO_POTION;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_AUTO_POTION_FB
	{
		NET_MSG_GENERIC		nmg;

		EMREQ_AUTO_POTION_FB emFB;

		float fHP;
		float fMP;
		float fSP;

		SNETPC_REQ_AUTO_POTION_FB () 
			: emFB(EMREQ_AUTO_POTION_FB_FAIL)
			, fHP(100.0f)
			, fMP(100.0f)
			, fSP(100.0f)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_AUTO_POTION_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_AUTO_POTION_FB;
		}
	};


	struct SNETPC_REQ_AUTO_PILOT_SET
	{
		NET_MSG_GENERIC		nmg;
		BOOL bActive;
		float fRange;

		SNETPC_REQ_AUTO_PILOT_SET () 
			: bActive(FALSE)
			, fRange(0.0f)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_AUTO_PILOT_SET);
			nmg.nType = NET_MSG_GCTRL_REQ_AUTO_PILOT_SET;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_AUTO_PILOT_SET_FB
	{
		NET_MSG_GENERIC		nmg;

		EMREQ_AUTO_PILOT_FB emFB;

		BOOL bActive;
		float fRange;

		SNETPC_REQ_AUTO_PILOT_SET_FB () 
			: emFB(EMREQ_AUTO_PILOT_FB_FAIL)
			, bActive(FALSE)
			, fRange(0.0f)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_AUTO_PILOT_SET_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_AUTO_PILOT_SET_FB;
		}
	};

	struct SNETPC_REQ_AUTO_PILOT_RESET
	{
		NET_MSG_GENERIC		nmg;
		BOOL bActive;

		SNETPC_REQ_AUTO_PILOT_RESET () 
			: bActive(FALSE)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_AUTO_PILOT_RESET);
			nmg.nType = NET_MSG_GCTRL_REQ_AUTO_PILOT_RESET;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_AUTO_PILOT_RESET_FB
	{
		NET_MSG_GENERIC		nmg;
		BOOL bActive;

		SNETPC_REQ_AUTO_PILOT_RESET_FB () 
			: bActive(FALSE)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_AUTO_PILOT_RESET_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_AUTO_PILOT_RESET_FB;
		}
	};
	
	/*12-9-14, Battle Pass - CNDev*/
	struct SNETPC_REQ_BATTLEPASS_PREMIUM_CARD
	{
		NET_MSG_GENERIC		nmg;
		WORD				wPosX;
		WORD				wPosY;

		SNETPC_REQ_BATTLEPASS_PREMIUM_CARD ()
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_BATTLEPASS_PREMIUM_CARD);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_PREMIUM_CARD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_BATTLEPASS_PREMIUM_CARD_FB
	{
		NET_MSG_GENERIC							nmg;

		EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_FB	emFB;
		bool									bPremium;

		SNETPC_REQ_BATTLEPASS_PREMIUM_CARD_FB () 
			: emFB(EMINVEN_REQ_BATTLEPASS_PREMIUM_CARD_FAIL)
			, bPremium(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_BATTLEPASS_PREMIUM_CARD_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_PREMIUM_CARD_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};


	struct SNETPC_REQ_BATTLEPASS_LEVELUP_CARD
	{
		NET_MSG_GENERIC		nmg;
		WORD				wPosX;
		WORD				wPosY;

		SNETPC_REQ_BATTLEPASS_LEVELUP_CARD ()
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_BATTLEPASS_LEVELUP_CARD);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_LEVELUP_CARD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_BATTLEPASS_LEVELUP_CARD_FB
	{
		NET_MSG_GENERIC							nmg;

		EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_FB	emFB;

		SNETPC_REQ_BATTLEPASS_LEVELUP_CARD_FB () 
			: emFB(EMINVEN_REQ_BATTLEPASS_LEVELUP_CARD_FAIL)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_BATTLEPASS_LEVELUP_CARD_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_LEVELUP_CARD_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_BATTLEPASS_REWARD_CLAIM
	{
		NET_MSG_GENERIC		nmg;
		...
		SNETPC_REQ_BATTLEPASS_REWARD_CLAIM () 
			: dwLevel(0)
			, bPremium(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_BATTLEPASS_REWARD_CLAIM);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_REWARD_CLAIM;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_BATTLEPASS_REWARD_CLAIM_FB
	{
		NET_MSG_GENERIC							nmg;
		SNATIVEID								sItemID;
		EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_FB	emFB;

		SNETPC_REQ_BATTLEPASS_REWARD_CLAIM_FB()
			: emFB( EMINVEN_REQ_BATTLEPASS_REWARD_CLAIM_FAIL )
			, sItemID(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_BATTLEPASS_REWARD_CLAIM_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_BATTLEPASS_REWARD_CLAIM_FB;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};

	//Item Card Extend Costume Expiration
	struct SNET_INVEN_DISGUISE_EXTEND
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_DISGUISE_EXTEND () 
			: wPosX(0)
			, wPosY(0)
		{
			nmg.dwSize = sizeof(SNET_INVEN_DISGUISE_EXTEND);
			nmg.nType = NET_MSG_GCTRL_INVEN_DISGUISE_EXTEND;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	/*12-16-24, daily login - CNDev*/
	struct SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM
	{
		NET_MSG_GENERIC		nmg;

		DWORD				dwDay;

		SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM () 
			: dwDay(0)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM);
			nmg.nType = NET_MSG_GCTRL_REQ_DAILYLOGIN_REWARD_CLAIM;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM_FB
	{
		NET_MSG_GENERIC							nmg;
		SNATIVEID								sItemID;
		EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_FB	emFB;

		SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM_FB()
			: emFB( EMINVEN_REQ_DAILYLOGIN_REWARD_CLAIM_FAIL )
			, sItemID(false)
		{
			nmg.dwSize = sizeof(SNETPC_REQ_DAILYLOGIN_REWARD_CLAIM_FB);
			nmg.nType = NET_MSG_GCTRL_REQ_DAILYLOGIN_REWARD_CLAIM_FB;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};
	...
	struct SNET_GET_ITEMSHOP_FROMDB_FB
	{
		...
		SNET_GET_ITEMSHOP_FROMDB_FB ()
			: emFB(EMREQ_ITEMSHOP_FROMDB_FB_END)
			, nidITEM(false)
			, wPrice(0)
			, wStock(0)
			, wCurrency(0)
