...
CNpcDialogue::CNpcDialogue () :
	m_dwNID(UINT_MAX),
	m_done_info(FALSE)		/*target information display, Juver, 2018/08/22 */
{
}
...
//
DWORD CNpcDialogue::FindConditionSatisfied ( GLCHARLOGIC_SERVER *pCHAR )
{
...
//	Note : 조건에 맞는 CASE를 찾는다. 없을 경우 Default CASE를 선택.
//
DWORD CNpcDialogue::FindConditionSatisfied ( GLCHARLOGIC_CLIENT *pCHAR )
{
	DLGCASE_ITER iter = m_vecCase.begin();
	DLGCASE_ITER iter_end = m_vecCase.end();
	for ( ; iter!=iter_end; ++iter )
