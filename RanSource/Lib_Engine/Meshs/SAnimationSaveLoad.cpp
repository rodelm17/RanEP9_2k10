...
HRESULT SAnimContainer::LoadAnimFromFile ( const TSTRING &strFilePath )
{
	...
	if ( dwVersion >= VERSION_ENCODE )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_ANI_BIN_VER_1 );

	if ( dwVersion >= ENCODE_VER_BYTE_CRYPT_2021_ANI_BIN_V1 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_ANI_BIN_V1 );
...
HRESULT  SAnimContainer::Load_0201( CSerialFile &SFile )
{
	std::for_each(m_listAniUPBODY.begin(),m_listAniUPBODY.end(),std_afunc::DeleteObject());
	std::for_each(m_listAnimation.begin(),m_listAnimation.end(),std_afunc::DeleteObject());
	m_listAniUPBODY.clear();
...
HRESULT SAnimContainer::LoadAnimFromFile ( const TSTRING &strFilePath, const DxSkeleton* pSkeleton, SANILIST &listANI, SANILIST &listANIUPBODY )
{
	...
	case 0x0200:		Load_0200( SFile, pSkeleton, listANI, listANIUPBODY );	break;
	case VERSION:		Load_0201( SFile, pSkeleton, listANI, listANIUPBODY );	break;
	default:			
		{
...
HRESULT  SAnimContainer::Load_0201( CSerialFile &SFile, const DxSkeleton* pSkeleton, SANILIST &listANI, SANILIST &listANIUPBODY )
{
	DWORD dwNumAni(0);
