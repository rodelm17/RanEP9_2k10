...
void CLargeMapWindow::CreateSubControl ()
{
	...
	/* map window mark, Juver, 2020/03/31 */
	m_pMarkDummy = new CUIControl;
	m_pMarkDummy->CreateSub ( this, "MAP_WINDOW_SETTING_LABEL" );
	m_pMarkDummy->SetVisibleSingle ( FALSE );
	RegisterControl ( m_pMarkDummy );
}
...
void CLargeMapWindow::SetMapAxisInfo ( GLMapAxisInfo& sInfo, const CString& strMapName )
{
	...
	SetTitleName( strMapName.GetString() );
...
void CLargeMapWindow::OnLButtonUp()
{
	...
	DWORD dwKeyLControl = dxInputDev.GetKeyState ( DIK_LCONTROL );
	BOOL bCTRL = BOOL(dwKeyLControl & DXKEY_DOWNED);
	if ( bCTRL )
	{
		PLANDMANCLIENT pLandClient = GLGaeaClient::GetInstance().GetActiveMap();
		GLMapAxisInfo &sMapAxisInfo = pLandClient->GetMapAxisInfo();
		int nPosX, nPosY;
		sMapAxisInfo.Convert2MapPos ( vWorldPos.x, vWorldPos.z, nPosX, nPosY );

		if( !pCharacter->LargeMapTeleportTo( nPosX, nPosY ) )
		{
...
void CLargeMapWindow::UPDATE_CHAR_POS ( CUIControl * pControl, const D3DXVECTOR3& vPlayerPos, bool bTextBox /*= false*/ )
{
	GASSERT( pControl );
	...
	if ( bTextBox )
	{
		...
		const UIRECT& rcControlGlobalPos = pControl->GetGlobalPos ();

		if ( rcControlGlobalPos.left < rcMapGlobalPos.left )
		{
			pControl->SetVisibleSingle ( FALSE );
		}

		if ( rcControlGlobalPos.right > rcMapGlobalPos.right )
		{
			pControl->SetVisibleSingle ( FALSE );
		}

		if ( rcControlGlobalPos.top < rcMapGlobalPos.top )
		{
			pControl->SetVisibleSingle ( FALSE );
		}

		if ( rcControlGlobalPos.bottom > rcMapGlobalPos.bottom )
		{
			pControl->SetVisibleSingle ( FALSE );
		}
	}
