...
void GLPVPCaptureTheFlagClient::DoStateRegister()
{
	...
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_REG") );
}
...
void GLPVPCaptureTheFlagClient::DoStateBattle()
{
	...
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_BATTLE") );
...
void GLPVPCaptureTheFlagClient::DoStateReward()
{
	...
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_REWARD") );
}
...
void GLPVPCaptureTheFlagClient::DoStateEnd()
{
	...
	CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_ENDED") );
...
void GLPVPCaptureTheFlagClient::NotifyTimeToStart( WORD wTime )
{
	if ( wTime > 0 ){
		if ( wTime <= 1 )
		{
			...
			CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_TOBATTLE2"), wTime );
		}
		else
		{
			CInnerInterface::GetInstance().PrintConsoleText( ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_TOBATTLE"), wTime );
			CInnerInterface::GetInstance().PVPCompetitionShowNotice( NS_UITEXTCOLOR::WHITE, ID2GAMEINTEXT("PVP_CAPTURE_THE_FLAG_TOBATTLE"), wTime );
		}
...
void GLPVPCaptureTheFlagClient::DoRegister( WORD wTeam )
{
	...
	if ( m_emState == CAPTURE_THE_FLAG_STATE_REGISTER )
	{
		...
		netmsgregister.dwContributionReq = GLGaeaClient::GetInstance().GetCharacter()->m_dwContributionPoint;
...
BOOL GLPVPCaptureTheFlagClient::SetContributionPointMultiplier( float fMulti )
{
	m_fContributionPointMultiplier = fMulti;
	return TRUE;
}

float GLPVPCaptureTheFlagClient::GetContributionPointMultiplier()
{
