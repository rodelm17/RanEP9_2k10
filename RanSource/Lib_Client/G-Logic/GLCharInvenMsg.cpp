...
bool GLChar::DoDrugInvenItem ( WORD wPosX, WORD wPosY, bool b2AGENT )
{
	...
	if ( pItem->sDrugOp.bInstance || pItem->sBasicOp.emItemType == ITEM_QITEM )
	{
	...
	DoBattlePassUseItem( pItem->sBasicOp.sNativeID, 1 );
	return true;
}
...
bool GLChar::DoDrugSlotItem ( EMSLOT emSLOT,  WORD wNum )
{
	...
	// Corrected sprintf format (added missing opening quote and fixed placement)
	sprintf(szTempChar, "[%s][%s], Equipped Item %02d_%02d, %s x(%d/%d), GenNum(%d)", 
			m_szUID, m_szName, 
			sSLOTITEM.sNativeID.wMainID, sSLOTITEM.sNativeID.wSubID, 
			pItem->GetName(), sSLOTITEM.wTurnNum, 
			pItem->sDrugOp.wPileNum, sSLOTITEM.lnGenNum);

	str = szTempChar;

	// Item Logs - JX
	CDebugSet::ToFileWithTime("InventoryLogs.txt", str.c_str());
	return true;
}
		...
		for ( ; iter!=ItemMap.end(); ++iter )
		{
			...
			else
			{
				...
				if ( pParty )
				{
					...
					pItemDrop->sItemCustom.Convert( NetMsg.sitem_link );
					NetMsg.sitem_link.BuildFilter( pItem->GetName(), pItemDrop->sItemCustom.GETGRADE(EMGRINDING_NONE), FALSE, "" );
					NetMsg.sitem_link.bUSE = TRUE;
	...
	if ( pItem->ISPILE() )
	{
		...
		for ( ; iter!=ItemMap.end(); ++iter )
		{
			...
			else
			{
				...
				/*link drop item, EJCode, 2018/10/20 */
				pItemDrop->sItemCustom.Convert( NetMsgPickUp.sitem_link );
				NetMsgPickUp.sitem_link.BuildFilter( pItem->GetName(), pItemDrop->sItemCustom.GETGRADE(EMGRINDING_NONE), FALSE, "" );
				NetMsgPickUp.sitem_link.bUSE = TRUE;
				...
				/*12-9-14, Battle Pass - CNDev*/
				DoBattlePassTakeItem( pItemDrop->sItemCustom.sNativeID, pItemDrop->sItemCustom.wTurnNum );
				...
				{	// Item Logs - JX
					...
					if ( pItem )
					{
						char szTempChar[256] = {0,};
						std::string str;
						sprintf( szTempChar, ",[%s][%s], Pickup Drop Item %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
							pItemDrop->sItemCustom.sNativeID.wMainID,
							pItemDrop->sItemCustom.sNativeID.wSubID,
							pItem->GetName(),
							pItemDrop->sItemCustom.wTurnNum,
							pItem->sDrugOp.wPileNum,
							pItemDrop->sItemCustom.lnGenNum);
						str = szTempChar;

						CDebugSet::ToFileWithTime ( "InventoryLogs.txt",str.c_str() );
//						CString strName;
//						strName.Format ( "%s.txt",m_szName );
//						CDebugSet::ToFileWithTime ( strName.GetString(),str.c_str() );
//						strName.Format ( "%d.txt",m_dwUserID );
//						CDebugSet::ToFileWithTime ( strName.GetString(),str.c_str() );
					}
				}
				//	Note : 아이탬 Land에서 삭제.
		...
		{ 	// Item Logs - JX
			...
			if ( pItem )
			{
				char szTempChar[256] = {0,};
				std::string str;
				sprintf( szTempChar, ",[%s][%s], Pickup Drop Pile Item %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
					pItemDrop->sItemCustom.sNativeID.wMainID,
					pItemDrop->sItemCustom.sNativeID.wSubID,
					pItem->GetName(),
					pItemDrop->sItemCustom.wTurnNum,
...
HRESULT GLChar::InsertToInvenEx ( PITEMDROP pItemDrop )
{
	...
	{	// Item Logs - JX
		...
		if ( pItem )
		{
			char szTempChar[256] = {0,};
			std::string str;
			sprintf( szTempChar, ",[%s][%s], Pickup EX Drop Item %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
				pItemDrop->sItemCustom.sNativeID.wMainID,
				pItemDrop->sItemCustom.sNativeID.wSubID,
				pItem->GetName(),
				pItemDrop->sItemCustom.wTurnNum,
...
HRESULT GLChar::InsertToInven ( PMONEYDROP pMoneyDrop )
{
	...
	if( pMoneyDrop->bDropMonster )
	{
		...
		//get gold drop monster drop
		m_sCombatRecord.increasePVEMoneyGain( pMoneyDrop->lnAmount );
		
		/*12-9-14, Battle Pass - CNDev*/
		DoBattlePassGainGold( pMoneyDrop->lnAmount );
	}
...
HRESULT GLChar::MsgReqInvenToHold ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pITEM->IsOverPile(pInvenItem->sItemCustom.wTurnNum ) )
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqInvenToHold Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
	...
	{	// Item Logs - JX
		...
		if ( pItem )
		{
			...
			// Log based on action: Holding or Equipping
			bool isEquipped = (pITEM->sBasicOp.emItemType == ITEM_SUIT);
			const char* action = isEquipped ? "Equipped Item" : "Hold Item From Inventory";

			sprintf( szTempChar, ",[%s][%s], %s %02d_%02d, %s x(%d/%d), GenNum(%d)",
						m_szUID, m_szName,
						action,
						pInvenItem->sItemCustom.sNativeID.wMainID,
						pInvenItem->sItemCustom.sNativeID.wSubID,
						pITEM->GetName(),
						pInvenItem->sItemCustom.wTurnNum,
						pITEM->sDrugOp.wPileNum,
						pInvenItem->sItemCustom.lnGenNum);
			str = szTempChar;
...
HRESULT GLChar::MsgReqInvenExHold ( NET_MSG_GENERIC* nmg )
{
	...
	/* item pile checker, Juver, 2020/10/02 */
	if ( pInvenData->IsOverPile(pInvenItem->sItemCustom.wTurnNum ) )
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqInvenExHold Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
			pInvenData->sBasicOp.sNativeID.wMainID,
			pInvenData->sBasicOp.sNativeID.wSubID,
			pInvenData->GetName(),
			pInvenItem->sItemCustom.wTurnNum );
	...
	if ( bMERGE )
	{
		...
		if ( wTurnNum <= pInvenData->sDrugOp.wPileNum )
		{
			...
			{	// Item Logs - JX
				char szTempChar[256] = {0,};
				std::string str;
				sprintf( szTempChar, "[%s][%s], Merged Item %02d_%02d, %s x(%d/%d), GenNum(%d),", m_szUID, m_szName, 
										sItemHold.sNativeID.wMainID,
										sItemHold.sNativeID.wSubID,
										pHoldData->GetName(),
										sItemHold.wTurnNum,
										pHoldData->sDrugOp.wPileNum,
										sItemHold.lnGenNum);
				str = szTempChar;
		
				
				sprintf( szTempChar, " to Item %02d_%02d, %s, x(%d/%d), GenNum(%d)",
										sInven_BackUp.sItemCustom.sNativeID.wMainID,
										sInven_BackUp.sItemCustom.sNativeID.wSubID,
										pInvenData->GetName(),
										sInven_BackUp.sItemCustom.wTurnNum,
										pInvenData->sDrugOp.wPileNum,
										sInven_BackUp.sItemCustom.lnGenNum );
				str += szTempChar;
		...
		else
		{
			...
			{	// Item Logs - JX
				char szTempChar[256] = {0,};
				std::string str;
				sprintf( szTempChar, ",[%s][%s], Merged & Split Item %02d_%02d, %s x(%d/%d), GenNum(%d),", m_szUID, m_szName, 
										sItemHold.sNativeID.wMainID,
										sItemHold.sNativeID.wSubID,
										pHoldData->GetName(),
										sItemHold.wTurnNum,
...
HRESULT GLChar::MsgReqInvenSplit ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pItem->IsOverPile(pInvenItem->sItemCustom.wTurnNum ) )
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqInvenSplit Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
	...
	// Item Logs - JX
	SITEM* pItemNew = GLItemMan::GetInstance().GetItem ( pNewItem->sItemCustom.sNativeID );
	if ( !pItemNew )													return E_FAIL;

	{
		char szTempChar[256] = {0,};
		std::string str;
		sprintf( szTempChar, ",[%s][%s], Split Item %02d_%02d, %s x(%d/%d), GenNum(%d),", m_szUID, m_szName, 
								pInvenItem->sItemCustom.sNativeID.wMainID,
								pInvenItem->sItemCustom.sNativeID.wSubID,
								pItemNew->GetName(),
								pInvenItem->sItemCustom.wTurnNum,
								pItemNew->sDrugOp.wPileNum,
								pInvenItem->sItemCustom.lnGenNum);
		str = szTempChar;

		sprintf( szTempChar, " Insert New Item %02d_%02d, %s, x(%d/%d), GenNum(%d)",
								pNewItem->sItemCustom.sNativeID.wMainID,
								pNewItem->sItemCustom.sNativeID.wSubID,
								pItemNew->GetName(),
								pNewItem->sItemCustom.wTurnNum,
								pItemNew->sDrugOp.wPileNum,
								pNewItem->sItemCustom.lnGenNum );
		str += szTempChar;
...
HRESULT GLChar::MsgReqHoldToInven ( NET_MSG_GENERIC* nmg )
{
	...
	{ 	// Item Logs - JX
		...
		if ( pITEM )
		{
			char szTempChar[256] = {0,};
			std::string str;
			sprintf( szTempChar, ",[%s][%s], Insert from item hold Item %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
									sINVENITEM.sItemCustom.sNativeID.wMainID,
									sINVENITEM.sItemCustom.sNativeID.wSubID,
									pITEM->GetName(),
									sINVENITEM.sItemCustom.wTurnNum,
									pITEM->sDrugOp.wPileNum,
									sINVENITEM.sItemCustom.lnGenNum);
			str = szTempChar;
...
HRESULT GLChar::MsgReqHoldToField ( NET_MSG_GENERIC* nmg )
{
	...
	if( pitem_costume && !pitem_costume->sBasicOp.IsTHROW() )	return FALSE;
	...
	{	// Item Logs - JX
		char szTempChar[256] = {0,};
		std::string str;
		sprintf( szTempChar, ",[%s][%s], Drop Item %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
								sITEMCUSTOM.sNativeID.wMainID,
								sITEMCUSTOM.sNativeID.wSubID,
								pITEM->GetName(),
								sITEMCUSTOM.wTurnNum,
								pITEM->sDrugOp.wPileNum,
								sITEMCUSTOM.lnGenNum);
		str = szTempChar;
...
HRESULT GLChar::MsgReqSlotToHold ( NET_MSG_GENERIC* nmg )
{
	...
	if ( isPersonalLock( EMPERSONAL_LOCK_EQUIP ) && !( pNetMsg->emSlot == SLOT_LHAND || pNetMsg->emSlot == SLOT_LHAND_S ) )
		return S_FALSE;
	...
	if( pNetMsg->emSlot == SLOT_RHAND || 
		pNetMsg->emSlot == SLOT_RHAND_S )
	{
		summon_remove_all();
	}
...
bool GLChar::InsertPileItem ( const SITEMCUSTOM& sItemCustom, WORD wREQINSRTNUM, EMITEM_ROUTE emRoute /*= EMITEM_ROUTE_SIZE*/ )
{
	SITEM* pItem = GLItemMan::GetInstance().GetItem ( sItemCustom.sNativeID );
	if ( !pItem )				return false;
	...
	for ( ; iter!=ItemMap.end(); ++iter )
	{
		...
		if ( wREQINSRTNUM > wSURPLUSNUM )
		{
			...
			//log inserted
			if ( emRoute != EMITEM_ROUTE_SIZE )
			{
				GLITEMLMT::GetInstance().ReqItemRoute ( sITEMCUSTOM, ID_CHAR, 0, ID_CHAR, m_dwCharID, emRoute, wSURPLUSNUM );
			}
		}
		else
		{
			...
			if ( emRoute != EMITEM_ROUTE_SIZE )
			{
				GLITEMLMT::GetInstance().ReqItemRoute ( sITEMCUSTOM, ID_CHAR, 0, ID_CHAR, m_dwCharID, emRoute, wREQINSRTNUM );
			}
			...
			{	// Item Logs - JX
				...
				if ( pITEM )
				{
					char szTempChar[256] = {0,};
					std::string str;
					sprintf( szTempChar, ",[%s][%s], InsertPileItem x(%d) %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
											wREQINSRTNUM,
											sITEMCUSTOM.sNativeID.wMainID,
											sITEMCUSTOM.sNativeID.wSubID,
											pITEM->GetName(),
	...
	for ( WORD i=0; i<wITEMNUM; ++i )
	{
		...
		if ( emRoute != EMITEM_ROUTE_SIZE )
		{
			GLITEMLMT::GetInstance().ReqItemRoute ( sITEMCUSTOM, ID_CHAR, 0, ID_CHAR, m_dwCharID, emRoute, sITEMCUSTOM.wTurnNum );
		}
		...
		{	// Item Logs - JX
			...
			if ( pITEM )
			{
				char szTempChar[256] = {0,};
				std::string str;
				sprintf( szTempChar, ",[%s][%s], InsertPileItem New %02d_%02d, %s x(%d/%d), GenNum(%d)", m_szUID, m_szName, 
					sITEMCUSTOM.sNativeID.wMainID,
					sITEMCUSTOM.sNativeID.wSubID,
					pITEM->GetName(),
					sITEMCUSTOM.wTurnNum,
...
bool GLChar::DeletePileItem ( SNATIVEID sNID, WORD wREQDELNUM, EMITEM_ROUTE emRoute /*= EMITEM_ROUTE_SIZE*/ )
{
	...
	for ( ; iter!=ItemMap.end(); ++iter )
	{
		...
		if ( sITEMCUSTOM.wTurnNum>wREQDELNUM )
		{
			...
			if ( emRoute != EMITEM_ROUTE_SIZE )
			{
				GLITEMLMT::GetInstance().ReqItemRoute ( sITEMCUSTOM, ID_CHAR, m_dwCharID, ID_CHAR, 0, emRoute, wREQDELNUM );
			}
		...
		else
		{
			...
			if ( emRoute != EMITEM_ROUTE_SIZE )
			{
				GLITEMLMT::GetInstance().ReqItemRoute ( sITEMCUSTOM, ID_CHAR, m_dwCharID, ID_CHAR, 0, emRoute, sITEMCUSTOM.wTurnNum );
			}
...
HRESULT GLChar::MsgReqBuyFromNpc ( NET_MSG_GENERIC* nmg )
{
	...
	std::string strCrowSale = pCrowData->GetCrowSale( pNetMsg->dwChannel );
	...
	else
	{
		...
		sITEM_NEW.cChnID = (BYTE)GLGaeaServer::GetInstance().GetServerChannel();
		sITEM_NEW.cFieldID = (BYTE) GLGaeaServer::GetInstance().GetFieldSvrID();
		sITEM_NEW.lnGenNum = GLITEMLMT::GetInstance().RegItemGen ( sNID, (EMITEMGEN)sITEM_NEW.cGenType );
...
HRESULT GLChar::MsgReqSaleToNpc ( NET_MSG_GENERIC* nmg )
{
	...
	SITEM* pitem_costume = GLItemMan::GetInstance().GetItem( sHOLD_ITEM.nidDISGUISE );
...
*/
	...
	// Debug log for item sale
	CString strTemp;
	strTemp.Format(",[%s][%s], Sale To NPC, Item ID: [%02d_%02d], Item Name: [%s], Sale Price: [%I64d], Remaining Money: [%I64d]",
				m_szUID, m_szName, pItem->sBasicOp.sNativeID.wMainID, pItem->sBasicOp.sNativeID.wSubID, 
				pItem->GetName(), llDisPRICE2, m_lnMoney);
	CDebugSet::ToFileWithTime("NpcSellItemLogs.txt", strTemp);
	//	아이탬 삭제.
	RELEASE_HOLD_ITEM();
...
HRESULT GLChar::MsgReqInvenDrug ( NET_MSG_GENERIC* nmg )
{
	...
	/*dmk14 burn pots abusive player use hp Freeze*/
	DoDrugInvenItem ( pNetMsg->wPosX, pNetMsg->wPosY );
...
HRESULT GLChar::MsgReqInvenBoxOpen ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_wLevel != pNetMsg->wLevel )
	{
		//CDebugSet::ToFileWithTime( "openbox.txt", "MsgReqInvenBoxOpen mismatch level server:%u, client:%u", m_wLevel, pNetMsg->wLevel );
		return E_FAIL;
	}
	...
	if ( !SIMPLE_CHECK_ITEM( pItem->sBasicOp.sNativeID, true ) )
	{		
		NetMsgFB.emFB = EMREQ_BOXOPEN_FB_NOITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	...
	// Item Logs - JX
	CString strTemp;
	strTemp.Format( ",[%s][%s],Box Open Item, [%02d_%02d]",m_szUID, m_szName, pItem->sBasicOp.sNativeID.wMainID,pItem->sBasicOp.sNativeID.wSubID);
	CDebugSet::ToFileWithTime ( "InventoryLogs.txt",strTemp );
	...
	// Item Logs - JX
	strTemp.Format( ",Insert Item From Box:");
	CDebugSet::ToFileWithTime ( "InventoryLogs.txt",strTemp );
	...
	for ( int i=0; i<ITEM::SBOX::ITEM_SIZE; ++i )
	{
		...
		// Item Logs - JX
		strTemp.Format( ",[%02d_%02d][x%d/%d]", pITEM->sBasicOp.sNativeID.wMainID,pITEM->sBasicOp.sNativeID.wSubID,sITEM_NEW.wTurnNum,pITEM->sDrugOp.wPileNum);
		CDebugSet::ToFileWithTime ( "InventoryLogs.txt",strTemp );
		//	[자신에게] 메시지 발생.
		GLMSG::SNETPC_INVEN_INSERT NetMsg_Inven;
		...
		DoActivityTakeItem( sITEM_NEW.sNativeID, sITEM_NEW.wTurnNum );
		/*12-9-14, Battle Pass - CNDev*/
		DoBattlePassTakeItem( sITEM_NEW.sNativeID, sITEM_NEW.wTurnNum );
	}
...
HRESULT GLChar::MsgReqOpenGacha ( NET_MSG_GENERIC* nmg )
{
	if ( !IsValidBody() )	return E_FAIL;

	GLMSG::SNET_OPEN_GACHA *pNetMsg = (GLMSG::SNET_OPEN_GACHA *) nmg;

	GLMSG::SNET_INVEN_RANDOMBOXOPEN_FB NetMsgFB;
	
	SNATIVEID sItemID = GLCONST_CHAR::nidGachaItem;
	WORD wPosX = 0;
	WORD wPosY = 0;
	...
	bool bPromisedItem = false;
	if ( m_wGachaPromisePoint == GLCONST_CHAR::wGachaMaxPoint )
			bPromisedItem = true;
	...
	for ( DWORD i=0; i<pItem->sRandomBox.vecBOX.size(); ++i )
	{
		...
		//promised item shits.
		if ( bPromisedItem && i == 0 )
		{
			sGENITEM = sITEM;
			nSelected = i;
			break;
		}
		if ( fLOW <= fNowRate && fNowRate < (fLOW+sITEM.fRATE) && i != 0 )//0 chance to get first item.
		{
			sGENITEM = sITEM;
			nSelected = i;
			break;
	...
	//promise item announce
	if ( bPromisedItem )
	{
		GLMSG::SNET_SERVER_GENERALCHAT NetMsg;
		CString strTEXT;
		strTEXT.Format ( "Character '%s' got the promised '%s' on Gacha Box.", m_szName, pITEM_DATA->GetName() );
	...
	//rare item announcemnet
	if ( pITEM_DATA->sBasicOp.emLevel >= LEVEL_RARE && !bPromisedItem )//except promiseditem to avoid double announce
	{
		GLMSG::SNET_SERVER_GENERALCHAT NetMsg;
		CString strTEXT;
		strTEXT.Format ( "Character '%s' got rare item '%s' from Gacha Box.", m_szName, pITEM_DATA->GetName() );
		NetMsg.SETTEXT ( strTEXT.GetString() );
		m_pGLGaeaServer->SENDTOAGENT (&NetMsg );
	}
	
	m_wGachaPromisePoint++;
	
	if ( m_wGachaPromisePoint > GLCONST_CHAR::wGachaMaxPoint )
		m_wGachaPromisePoint = 0;
	
	GLMSG::SNETPC_UPDATE_CHA_PROMISE_POINTS net_msg_client1;
	net_msg_client1.dwVal = m_wGachaPromisePoint;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID, &net_msg_client1 );

	//	Note : 상자를 성공적으로 열었음.
	//
	NetMsgFB.emFB = EMINVEN_RANDOMBOXOPEN_FB_OK;
	NetMsgFB.nSelected = nSelected;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);

	return S_OK;
}
/* Gacha System, MontageDev 7/10/24 end*/
HRESULT GLChar::MsgReqInvenRandomBoxOpen ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_wLevel != pNetMsg->wLevel )
	{
		//CDebugSet::ToFileWithTime( "openbox.txt", "MsgReqInvenRandomBoxOpen mismatch level server:%u, client:%u", m_wLevel, pNetMsg->wLevel );
		return E_FAIL;
	}
	...
	// Item Logs - JX
	CString strTemp;
	strTemp.Format( ",[%s][%s],Random Box Open Item, [%02d_%02d]",m_szUID, m_szName, pItem->sBasicOp.sNativeID.wMainID,pItem->sBasicOp.sNativeID.wSubID);
	CDebugSet::ToFileWithTime ( "InventoryLogs.txt",strTemp );
	strTemp.Format( ",Insert Item From Random Box, [%02d_%02d][x%d]", pITEM_DATA->sBasicOp.sNativeID.wMainID,pITEM_DATA->sBasicOp.sNativeID.wSubID,sITEM_NEW.wTurnNum,pITEM_DATA->sDrugOp.wPileNum);
	CDebugSet::ToFileWithTime ( "InventoryLogs.txt",strTemp );
...
HRESULT GLChar::MsgReqInvenDisJunction ( NET_MSG_GENERIC* nmg )
{
	...
	sITEM_NEW.wTurnNum = 1;
	sITEM_NEW.cGenType = EMGEN_ACTION_USER;
	sITEM_NEW.cChnID = (BYTE)GLGaeaServer::GetInstance().GetServerChannel();
	sITEM_NEW.cFieldID = (BYTE)GLGaeaServer::GetInstance().GetFieldSvrID();
	sITEM_NEW.lnGenNum = GLITEMLMT::GetInstance().RegItemGen ( sITEM_NEW.sNativeID, (EMITEMGEN)sITEM_NEW.cGenType );
...
HRESULT GLChar::MsgReqInvenGrinding ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pHold->sBasicOp.wItemRefinesID != ITEM_REFINE_ID_DEFAULT || pItem->sBasicOp.wItemRefinesID != ITEM_REFINE_ID_DEFAULT )
	{
	...
	BOOL bAttEnchant = FALSE;
	BOOL bDefEnchant = FALSE;

	if ( pHold->sGrindingOp.emTYPE == EMGRINDING_DAMAGE )
		bAttEnchant = TRUE;

	if ( pHold->sGrindingOp.emTYPE == EMGRINDING_DEFENSE )
		bDefEnchant = TRUE;
	...
	if ( pITEM && pITEM->sBasicOp.emItemType == ITEM_ANTI_DISAPPEAR )
	{
		...
		{
			emANTIDISAPPEAR = EMANTIDISAPPEAR_ON;
			if( pITEM->sGrindingOp.bNoFail )	emANTIDISAPPEAR = EMANTIDISAPPEAR_ON_NORESET;
		}
	}
	...
	if( emANTIDISAPPEAR == EMANTIDISAPPEAR_USE )
	{
		...
		DoDrugSlotItem(SLOT_LOGIC_NECKLACE);
	}
	...
	/* Upgrade Announcement - Jxyyy */

	if (pHold->sGrindingOp.emTYPE == EMGRINDING_DAMAGE || pHold->sGrindingOp.emTYPE == EMGRINDING_DEFENSE)
	{
		// Check for Anti Items
		SITEM* pAntiDisappearItem = GET_SLOT_ITEMDATA(SLOT_NECK); // Assuming SLOT_NECK holds the anti disappear item
		SITEM* pAntiResetItem = GET_SLOT_ITEMDATA(SLOT_NECK); // Assuming same slot, adjust if different

		bool bUsingAntiDisappear = (pAntiDisappearItem && pAntiDisappearItem->sBasicOp.emItemType == ITEM_ANTI_DISAPPEAR);

		// Sabog Item (Vanished Announcement)
		if (bTERMINATE && cGrade >= 4)
		{
			CString strTextVanished;
			GLMSG::SNET_SERVER_GENERALCHAT NetMsgVanished;
			strTextVanished.Format("%s' item %s has vanished due to an upgrade attempt to +%d using %s.", 
									m_szName, pItem->GetName(), cGrade + 1, pHold->GetName());
			NetMsgVanished.SETTEXT(strTextVanished.GetString());
			NetMsgVanished.bNotify = true;
			NetMsgVanished.iColor = 8; // Color for vanished messages
			GLGaeaServer::GetInstance().SENDTOALLCLIENT(&NetMsgVanished);
			CDebugSet::ToFileWithTime("ItemUpgradeVanishedLogs.txt", strTextVanished);
			strTextVanished = "";

			// Skip failure message since the item is vanished
			//return;
		}

		// Upgrade Success Announcement
		if (bSucceeded && cGrade >= 6)
		{
			BYTE cNewGrade = cGrade + 1;
			CString strTextGen;
			GLMSG::SNET_SERVER_GENERALCHAT NetMsgGen;

			// Create the announcement text based on the presence of the anti items
			if (bUsingAntiDisappear) {
				strTextGen.Format("%s succeeded upgrading %s to +%d using %s with Protected Necklace.", 
								   m_szName, pItem->GetName(), cNewGrade, pHold->GetName());
			} 
			else {
				strTextGen.Format("%s succeeded upgrading %s to +%d using %s without Protection Items.", 
								   m_szName, pItem->GetName(), cNewGrade, pHold->GetName());
			}

			CDebugSet::ToFileWithTime("ItemUpgradeSuccessLogs.txt", strTextGen);

			NetMsgGen.SETTEXT(strTextGen.GetString());
			NetMsgGen.bNotify = true;
			NetMsgGen.iColor = 11; // Color for success messages
			GLGaeaServer::GetInstance().SENDTOALLCLIENT(&NetMsgGen);
		}

		// Fail Announcement (Only if the item didn't vanish)
		else if (!bSucceeded && !bTERMINATE && cGrade >= 6)
		{
			BYTE cNewGrade = cGrade + 1;
			CString strTextFail;
			GLMSG::SNET_SERVER_GENERALCHAT NetMsgFail;

			if (bUsingAntiDisappear) {
				strTextFail.Format("%s failed upgrading %s to +%d using %s with Protected Necklace.", 
									m_szName, pItem->GetName(), cNewGrade, pHold->GetName());
			} 
			else {
				strTextFail.Format("%s failed upgrading %s to +%d using %s without Protection Items.", 
									m_szName, pItem->GetName(), cNewGrade, pHold->GetName());
			}

			CDebugSet::ToFileWithTime("ItemUpgradeFailedLogs.txt", strTextFail);

			NetMsgFail.SETTEXT(strTextFail.GetString());
			NetMsgFail.bNotify = true;
			NetMsgFail.iColor = 10; // Color for failure messages
			GLGaeaServer::GetInstance().SENDTOALLCLIENT(&NetMsgFail);
		}
	}
	/* Upgrade Announcement - Jxyyy End */
	...
	if ( !bTERMINATE )
	{
		...
		if ( RPARAM::bEnchantNotice && bSucceeded && bRESET == FALSE )
		{
			if ( (bAttEnchant && pInvenItem->sItemCustom.GETGRADE(EMGRINDING_DAMAGE) >= RPARAM::wEnchantNoticeMinLevel ) ||
				( bDefEnchant && pInvenItem->sItemCustom.GETGRADE(EMGRINDING_DEFENSE) >= RPARAM::wEnchantNoticeMinLevel ) )
			{
				GLMSG::SNETPC_ENCHANT_NOTIFY_AGT NetMsgEnchantAgt;
				StringCbCopy( NetMsgEnchantAgt.szName, CHAR_SZNAME, m_szName);
				NetMsgEnchantAgt.sItemCustom = pInvenItem->sItemCustom;
				GLGaeaServer::GetInstance().SENDTOAGENT ( m_dwClientID, &NetMsgEnchantAgt );
			}
		}
	...
	else
	{
		...
		// Item Logs - JX
		CString strTemp;
		strTemp.Format( ",[%s][%s], Upgrading Item %s [%02d_%02d] Terminated/Vanished, using %s [%02d_%02d]",m_szUID, m_szName, pItem->GetName(), pItem->sBasicOp.sNativeID.wMainID,pItem->sBasicOp.sNativeID.wSubID,
			pHold->GetName(), pHold->sBasicOp.sNativeID.wMainID,pHold->sBasicOp.sNativeID.wSubID );
...
HRESULT GLChar::MsgReqInvenResetSkSt ( NET_MSG_GENERIC* nmg )
{
	...
	if ( !bINSERT && RPARAM::bSkillResetReturnSphere )
	{
	...
	if( RPARAM::bSkillResetReturnSphere )
	{
	for ( int i=EMGRADE_MIN; i<EMGRADE; ++i )
	for ( int j=0; j<wGRADE[i]; ++j )
		{
	...
	{
		MsgFB.emFB = EMREQ_RESET_SKST_FB_OK_NSPR;
		MsgFB.wITEM_NUM = wNUM;
		MsgFB.wSTATS_POINT = pITEM->sDrugOp.wCureVolume;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
...
HRESULT GLChar::MsgReqInvenPremiumSet ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_wLevel != pNetMsg->wLevel )
	{
		//CDebugSet::ToFileWithTime( "openbox.txt", "MsgReqInvenPremiumSet mismatch level server:%u, client:%u", m_wLevel, pNetMsg->wLevel );
		return E_FAIL;
	}
...
HRESULT GLChar::MsgReqInvenGenderChange( NET_MSG_GENERIC* nmg )
{
	...
	EMCHARCLASS emClass = charclass_change_gender( m_emClass );
	...
	m_wSex = CharClassToGender( m_emClass );
...
HRESULT GLChar::MsgLoudSpeaker ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_dwUserLvl < NSUSER_TYPE::USER_TYPE_GM3 )
	{
		...
		if ( RPARAM::bUserVerifiedLoudSpeaker )
		{
			if ( !m_bUserFlagVerified )
			{
				NetMsgFB.emFB = EMCHAT_LOUDSPEAKER_REQUIRE_VERIFIED;
				GLGaeaServer::GetInstance().SENDTOCLIENT ( m_dwClientID, &NetMsgFB );
				return S_OK;
			}
		}
...
HRESULT GLChar::MsgReqRebuildResult( NET_MSG_GENERIC* nmg )	// ITEMREBUILD_MARK
{
	...
	case EMREBUILD_RESULT_SUCCESS:	// 클라이언트에서 개조 시도 요청
		{
			...
			if ( !pItem->sBasicOp.IsUseRebuild() )
			{
				pNetMsg->emResult = EMREBUILD_RESULT_INVALID_ITEM;
				GLGaeaServer::GetInstance().SENDTOCLIENT( m_dwClientID, pNetMsg );
				return S_OK;
			}

			/* Item Reform use limit, Juver, 2021/07/23 */
			if ( pItem->sBasicOp.wMaxItemReform != ITEM_REFORM_INFINITE )
			{
				if ( sRebuildItem.wReformUseCount >= pItem->sBasicOp.wMaxItemReform )
				{
					pNetMsg->emResult = EMREBUILD_RESULT_MAXED;
					GLGaeaServer::GetInstance().SENDTOCLIENT( m_dwClientID, pNetMsg );
					return S_OK;
				}
			}
			...
			SRANDOM_OPTION_GEN* pRANDOM_SET = GLItemMan::GetInstance().GetItemRandomOpt( sRebuildItem.sNativeID );
			...
			else
			{
				...
				if( pInvenItem )
				{
					...
					/* Item Reform use limit, Juver, 2021/07/23 */
					pInvenItem->sItemCustom.wReformUseCount ++;
	...
	case EMREBUILD_RESULT_MAXED:		/* Item Reform use limit, Juver, 2021/07/23 */
		{
			InitRebuildData();
			CloseRebuild();
...
HRESULT GLChar::MsgReqGarbageResult( NET_MSG_GENERIC* nmg )	// ITEMREBUILD_MARK
{
	...
	{
		WORD wDeleteItemPosX = pNetMsg->wPosX[i];
		WORD wDeleteItemPosY = pNetMsg->wPosY[i];

		SINVENITEM* pINVENITEM_DEL = m_cInventory.GetItem ( wDeleteItemPosX,wDeleteItemPosY );
		...
		// 인벤토리에서 아이템 제거
		m_cInventory.DeleteItem( wDeleteItemPosX, wDeleteItemPosY );

		// 삭제한 아이템 제거
		GLMSG::SNETPC_INVEN_DELETE NetMsg_Inven_Delete;
		NetMsg_Inven_Delete.wPosX = wDeleteItemPosX;
		NetMsg_Inven_Delete.wPosY = wDeleteItemPosY;
		GLGaeaServer::GetInstance().SENDTOCLIENT( m_dwClientID, &NetMsg_Inven_Delete );	
...
HRESULT GLChar::MsgReqItemMix( NET_MSG_GENERIC* nmg )
{
	...
	if ( pItemMix->dwPrice > 0 )
	{
		...
		if ( pItemMix->dwPrice >=  EMMONEY_LOG )
		{
			GLITEMLMT::GetInstance().ReqMoneyExc ( ID_CHAR, m_dwCharID, ID_CHAR, 0, -LONGLONG(pItemMix->dwPrice), EMITEM_ROUTE_DELETE );
			GLITEMLMT::GetInstance().ReqMoneyExc ( ID_CHAR, m_dwCharID, ID_CHAR, 0, m_lnMoney, EMITEM_ROUTE_CHAR );
		}
	}
	...
	else
	{
		...
		sITEM_NEW.cDAMAGE = (BYTE)pItemResult->sBasicOp.wGradeAttack;
		sITEM_NEW.cDEFENSE = (BYTE)pItemResult->sBasicOp.wGradeDefense;
	}
...
/* Reset Stats Item */
HRESULT GLChar::MsgReqResetStats ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_INVEN_RESET_STATS *pNetMsg = (GLMSG::SNET_INVEN_RESET_STATS *)nmg;

	GLMSG::SNET_INVEN_RESET_STATS_FB NetMsgFB;

	SINVENITEM* pINVENITEM = m_cInventory.GetItem ( pNetMsg->wPosX, pNetMsg->wPosY );
	if ( !pINVENITEM )
	{
		NetMsgFB.emFB = EMREQ_RESET_STATS_FB_NOITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	...
	{
		NetMsgFB.emFB = EMREQ_RESET_STATS_FB_NOTITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	...
	if ( !SIMPLE_CHECK_ITEM( pITEM->sBasicOp.sNativeID, true ) )
	{		
		NetMsgFB.emFB = EMREQ_RESET_STATS_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);		
		return E_FAIL;
	}
	...
	NetMsgFB.emFB = EMREQ_RESET_STATS_FB_OK;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
...
HRESULT GLChar::MsgReqRegisterCodex ( NET_MSG_GENERIC* nmg )
{
	if ( !IsValidBody() )	return E_FAIL;

	GLMSG::SNET_REGISTER_CODEX *pNetMsg = (GLMSG::SNET_REGISTER_CODEX *) nmg;

	GLMSG::SNET_REGISTER_CODEX_FB NetMsgFB;

	WORD wPosX;
	WORD wPosY;

	SINVENITEM* pInvenItem = m_cInventory.FindItem ( pNetMsg->sItemID, pNetMsg->wGrade, pNetMsg->wQuantity);
	if ( !pInvenItem )
	{
		NetMsgFB.emFB = EMREQ_REGISTER_CODEX_FB_NOITEM;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	...
	if ( !pItem )
	{
		NetMsgFB.emFB = EMREQ_REGISTER_CODEX_FB_NOBOX;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	
	UINT cGRADE = pInvenItem->sItemCustom.GETGRADE ( EMGRINDING_NONE );
	
	bool bReturn = false;
	
	SCODEX_CHAR_DATA_MAP_ITER iter_codex_done = m_mapCodexDone.find( pNetMsg->sActivityID );
	if ( iter_codex_done != m_mapCodexDone.end() )
	{
		NetMsgFB.emFB = EMREQ_REGISTER_CODEX_FB_DONE;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	}
	
	SCODEX_CHAR_DATA_MAP_ITER iter_codex_prog = m_mapCodexProg.find( pNetMsg->sActivityID );
	if ( iter_codex_prog != m_mapCodexProg.end() )
	{
		SCODEX_CHAR_DATA& scodex_char_data = (*iter_codex_prog).second;
		
		if ( scodex_char_data.sidProgressItem1 == pNetMsg->sItemID )
		{
		...
		if ( scodex_char_data.sidProgressItem2 == pNetMsg->sItemID )
		{
		...
		if ( scodex_char_data.sidProgressItem3 == pNetMsg->sItemID )
		{
		...
		if ( scodex_char_data.sidProgressItem4 == pNetMsg->sItemID )
		{
		...
		if ( scodex_char_data.sidProgressItem5 == pNetMsg->sItemID )
		{
	...
	DoCodexRegisterItem( pNetMsg->sItemID, pNetMsg->sActivityID, pNetMsg->wQuantity, cGRADE );
	/*{
		NetMsgFB.emFB = EMREQ_REGISTER_CODEX_FB_NOBOX;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);
		return E_FAIL;
	...
	//	Note : 상자를 성공적으로 열었음.
	//
	NetMsgFB.emFB = EMREQ_REGISTER_CODEX_FB_OK;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&NetMsgFB);

	return S_OK;
}
// Contribution Item - JX
HRESULT GLChar::MsgReqCardContribution ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNETPC_CARD_CONTRIBUTION *pNetMsg = (GLMSG::SNETPC_CARD_CONTRIBUTION *)nmg;
	GLMSG::SNETPC_CARD_CONTRIBUTION_FB MsgFB;

	//cant find the item!
	SINVENITEM* pINVENITEM = m_cInventory.GetItem ( pNetMsg->wPosX, pNetMsg->wPosY );
	if ( !pINVENITEM )
	{
		MsgFB.emFB = EMINVEN_CARD_CONTRIBUTION_FB_NOITEMINVEN;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( CheckCoolTime( pINVENITEM->sItemCustom.sNativeID ) )
	{
		MsgFB.emFB = EMINVEN_CARD_CONTRIBUTION_FB_FAIL;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( !pITEM )
	{
		MsgFB.emFB = EMINVEN_CARD_CONTRIBUTION_FB_NOITEMDATA;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	{
		MsgFB.emFB = EMINVEN_CARD_CONTRIBUTION_FB_NOITEMTYPE;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
		return E_FAIL;
	}
	...
	if ( pITEM->sBasicOp.emItemType == ITEM_CONTRIBUTION_CARD )
	{
		DWORD dwPointReward = pITEM->sDrugOp.wCureVolume;

		m_dwContributionPoint += dwPointReward;
	...
	MsgFB.emFB = EMINVEN_CARD_CONTRIBUTION_FB_CLOSE;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);

	DoDrugInvenItem ( pNetMsg->wPosX, pNetMsg->wPosY );

	MsgFB.emFB = EMINVEN_CARD_CONTRIBUTION_FB_OK;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID,&MsgFB);
