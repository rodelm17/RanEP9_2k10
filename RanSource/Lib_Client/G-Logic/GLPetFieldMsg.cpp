...
HRESULT GLPetField::MsgChangeActiveSkill_A ( NET_MSG_GENERIC* nmg )
{
	...
	//prevent activation of pet skill when map does not allow it
	if ( m_pOwner && m_pOwner->m_pLandMan )
	{
		if ( !m_pOwner->m_pLandMan->IsPetActivity() )
		{
			m_pOwner->m_sPETSKILLFACT_A.RESET();

			CDebugSet::ToFileWithTime( "_petcheck.txt", "[%u]%s MsgChangeActiveSkill_A pet skill:[%u~%u] activation blocked, current map:[%u~%u] does not allow pet", 
				m_pOwner->m_dwCharID, m_pOwner->m_szName, pNetMsg->sSkillID.wMainID, pNetMsg->sSkillID.wSubID, m_pOwner->m_pLandMan->GetMapID().wMainID, m_pOwner->m_pLandMan->GetMapID().wSubID );
...
HRESULT GLPetField::MsgChangeActiveSkill_B ( NET_MSG_GENERIC* nmg )
{
	...
	if ( m_pOwner && m_pOwner->m_pLandMan )
	{
		if ( !m_pOwner->m_pLandMan->IsPetActivity() )
		{
			...
			CDebugSet::ToFileWithTime( "_petcheck.txt", "[%u]%s MsgChangeActiveSkill_B pet skill:[%u~%u] activation blocked, current map:[%u~%u] does not allow pet", 
				m_pOwner->m_dwCharID, m_pOwner->m_szName, pNetMsg->sSkillID.wMainID, pNetMsg->sSkillID.wSubID, m_pOwner->m_pLandMan->GetMapID().wMainID, m_pOwner->m_pLandMan->GetMapID().wSubID );
