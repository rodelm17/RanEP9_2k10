...
#include "../Lib_Network/NETCOMMENTS.h"
...
HRESULT DxLobyStage::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_LandCharSlt.LoadFile ( RPARAM::strSelectCharMapFile.c_str(), m_pd3dDevice, FALSE );

	m_LandLogin.InitDeviceObjects ( m_pd3dDevice );
	m_LandLogin.LoadFile ( RPARAM::strLoginMapFile.c_str(), m_pd3dDevice, FALSE );

	m_LandCreate.InitDeviceObjects ( m_pd3dDevice );
	m_LandCreate.LoadFile ( RPARAM::strCreateCharMapFile.c_str(), m_pd3dDevice, FALSE );
...
//		DxBgmSound::GetInstance().Stop ();
		...
		case ANNOUNCEMENT_PAGE: /*XINCODE Lobby Announcement*/
			{
				if( ControlIDBack==SELECT_SERVER_PAGE || 
					...
					ControlIDBack==LOGIN_DEVICE_CHECK_VERIFY_PAGE ||
					ControlIDBack==ANNOUNCEMENT_PAGE ||  /*XINCODE Lobby Announcement*/
					/*dmk14 ingame web*/
					ControlIDBack==FORGOTPASS_PAGE||
					ControlIDBack==FORGOTPASSCHANGE_PAGE ) 
					
					break;
				
				m_LandLogin.ActiveMap ();
				DxCamAniMan* pCamAniMan = m_LandLogin.GetCamAniMan();
				pCamAniMan->ActiveCameraPos ( (char*)RPARAM::strLoginMapCameraPos.c_str());
			}break;

		case SELECT_CHARACTER_PAGE:
		/*dmk14 ingame web*/
		case SELECT_CHARACTER_PAGE_CHANGEPASS:
		case SELECT_CHARACTER_PAGE_CHANGEPIN:
		case SELECT_CHARACTER_PAGE_CHANGEMAIL:
		case SELECT_CHARACTER_PAGE_RESETPIN:
		case SELECT_CHARACTER_PAGE_TOPUP:
		case SELECT_CHARACTER_PAGE_GAMETIMECONVERTUI:
			{
				if( ControlIDBack==SELECT_CHARACTER_PAGE ||
					/*dmk14 ingame web*/
					ControlIDBack==SELECT_CHARACTER_PAGE_CHANGEPASS || 
					ControlIDBack==SELECT_CHARACTER_PAGE_CHANGEPIN || 
					ControlIDBack==SELECT_CHARACTER_PAGE_CHANGEMAIL ||
					ControlIDBack==SELECT_CHARACTER_PAGE_RESETPIN ||
					ControlIDBack==SELECT_CHARACTER_PAGE_TOPUP ||
					ControlIDBack==SELECT_CHARACTER_PAGE_GAMETIMECONVERTUI
					) 
					break;
				...
				pCamAniMan->ActiveCameraPos ( (char*)RPARAM::strSelectCharMapCameraPos.c_str() );
		...
		case USER_ACCOUNT_PAGE:		
			{
				if( ControlIDBack==USER_ACCOUNT_PAGE ) 
					break;
		...
		case CHARACTER_CREATE_ROTATE:
			{
				...
				pCamAniMan->ActiveCameraPos ( (char*)RPARAM::strCreateCharMapCameraPos.c_str() );
	...
	case ANNOUNCEMENT_PAGE: /*XINCODE Lobby Announcement*/
	/*dmk14 ingame web*/
	case FORGOTPASS_PAGE:
	case FORGOTPASSCHANGE_PAGE:
		{
			DxCamAniMan* pCamAniMan = m_LandLogin.GetCamAniMan();
			pCamAniMan->ActiveCameraAni ( (char*)RPARAM::strLoginMapCameraAni.c_str() );
	...
	case SELECT_CHARACTER_PAGE_GAMETIMECONVERTUI:
		{
			DxCamAniMan* pCamAniMan = m_LandLogin.GetCamAniMan();
			pCamAniMan->ActiveCameraAni ( (char*)RPARAM::strSelectCharMapCameraAni.c_str() );
	...
	case CHARACTER_CREATE_ROTATE:
		{
			DxCamAniMan* pCamAniMan = m_LandLogin.GetCamAniMan();
			pCamAniMan->ActiveCameraAni ( (char*)RPARAM::strCreateCharMapCameraAni.c_str() );
...
HRESULT DxLobyStage::Render ()
{
	...
	case SECPASS_CHECKPAGE:
	case REGISTER_PAGE:							/*register page, Juver, 2017/11/17 */
	case LOGIN_DEVICE_CHECK_VERIFY_PAGE:		/* login device check, Juver, 2020/11/07 *//*dmk14 ingame web*/
	case ANNOUNCEMENT_PAGE: /*XINCODE Lobby Announcement*/
	case FORGOTPASS_PAGE:
	case FORGOTPASSCHANGE_PAGE:
		{
			...
			m_LandLogin.RenderPickAlpha( m_pd3dDevice, CV );
	...
	case SELECT_CHARACTER_PAGE_GAMETIMECONVERTUI:
		{
			...
			m_LandCharSlt.RenderPickAlpha( m_pd3dDevice, CV );
	...
	case CHARACTER_CREATE_ROTATE:
		{
			...
			m_LandCreate.RenderPickAlpha( m_pd3dDevice, CV );
			DxWeatherMan::GetInstance()->Render ( m_pd3dDevice );
		}break;
	};
