...
CCTyrannyWarRankingDisplayPlayerResuScoreSlot::CCTyrannyWarRankingDisplayPlayerResuScoreSlot ()
...
void CCTyrannyWarRankingDisplayPlayerResuScoreSlot::CreateSubControl ()
{
	...
	m_pLineBox->CreateBaseBoxQuestList ( "TYRANNY_RANKING_LINEBOX_RESU_WHITE_SELF_SLOT" );
	...
	m_pLineBoxWhite->CreateBaseBoxWhiteBlankBody ( "TYRANNY_RANKING_LINEBOX_RESU_WHITE_SLOTRANK" );
	...
	m_pLineBoxRankWhite->CreateBaseBoxWhiteBlankBody ( "TYRANNY_RANKING_LINEBOX_RESU_WHITE_RANKNUMBER" );
	...
	m_pLineBoxCharacterDetailsWhite->CreateBaseBoxWhiteBlankBody ( "TYRANNY_RANKING_LINEBOX_RESU_WHITE_CHARACTER_DETAILS" );
	...
	m_pLineBoxCharacterNameWhite->CreateBaseBoxWhiteBlankBody ( "TYRANNY_RANKING_LINEBOX_RESU_WHITE_CHARACTER_NAME" );
	...
	m_pResuRankTextSlot->CreateSub ( this, "TYRANNY_RANKING_TEXT_BASE_RANK_GUILD_SLOT" );
	...
	m_pResuNameTextSlot->CreateSub ( this, "TYRANNY_RANKING_TEXT_BASE_CHARACTER_NAME_RESURRECTION_SLOT" );
	...
	m_pResuScoreTextSlot->CreateSub ( this, "TYRANNY_RANKING_TEXT_BASE_RESURRECTION_SCORE_RESULT" );
	...
	{
		"TYRANNY_RANKING_RESURRECTION_CHARACTER_SCHOOL_WINDOW_SM",
		"TYRANNY_RANKING_RESURRECTION_CHARACTER_SCHOOL_WINDOW_HA",
		"TYRANNY_RANKING_RESURRECTION_CHARACTER_SCHOOL_WINDOW_BH",
	};
	...
	m_pResuIcon->CreateSub ( this, "TYRANNY_RANKING_RESURRECTION_SLOT_IMAGES", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	m_pResuIcon->SetVisibleSingle ( FALSE );
	RegisterControl ( m_pResuIcon );

	m_pClassImage = new CUIControl;
	m_pClassImage->CreateSub(this, "TYRANNY_RANKING_RESURRECTION_SLOT_CLASS_IMAGE", UI_FLAG_DEFAULT, CLASS_DUMMY);
	RegisterControl(m_pClassImage);


	m_pGuildImage = new CUIControl;
	m_pGuildImage->CreateSub(this, "TYRANNY_RANKING_RESURRECTION_CLUB_MARK", UI_FLAG_DEFAULT, GUILD_DUMMY);
...
CUIControl* CCTyrannyWarRankingDisplayPlayerResuScoreSlot::CreateControl ( const char* szControl )
{
...
void CCTyrannyWarRankingDisplayPlayerResuScoreSlot::TranslateUIMessage( UIGUID cID, DWORD dwMsg )
{
	CUIGroup::TranslateUIMessage( cID, dwMsg );
}

void CCTyrannyWarRankingDisplayPlayerResuScoreSlot::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	if ( !IsVisible () ) return ;
...
HRESULT CCTyrannyWarRankingDisplayPlayerResuScoreSlot::InitDeviceObjects(LPDIRECT3DDEVICEQ pd3dDevice)
{
	GASSERT(pd3dDevice);
...
HRESULT CCTyrannyWarRankingDisplayPlayerResuScoreSlot::Render(LPDIRECT3DDEVICEQ pd3dDevice)
{
	GASSERT( pd3dDevice );
...
void CCTyrannyWarRankingDisplayPlayerResuScoreSlot::ResetData()
{
...
void CCTyrannyWarRankingDisplayPlayerResuScoreSlot::SetData( int nRank, std::string strName, WORD wResuScore, WORD wSchool, int nChaClass, WORD wGuNum, WORD wGuMarkVer )
{
	ResetData();
