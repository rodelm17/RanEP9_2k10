...
HRESULT GLChar::MsgReqClubStorageToHold ( NET_MSG_GENERIC* nmg )
{
	...
	SITEM* pInvenData = GLItemMan::GetInstance().GetItem ( pInvenItem->sItemCustom.sNativeID );
	if ( !pInvenData )	return S_FALSE;

	if ( pInvenData->IsOverPile(pInvenItem->sItemCustom.wTurnNum ) )
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqClubStorageToHold Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
...
HRESULT GLChar::MsgReqClubStorageExHold ( NET_MSG_GENERIC* nmg )
{
	...
	{
		if ( !pITEM->sBasicOp.IsClubLocker() )	return FALSE;

		SITEM* pitem_costume = GLItemMan::GetInstance().GetItem( GET_HOLD_ITEM().nidDISGUISE );
		if( pitem_costume && !pitem_costume->sBasicOp.IsClubLocker() )	return FALSE;
	...
	if ( pInvenData->IsOverPile(pInvenItem->sItemCustom.wTurnNum ) )
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqClubStorageExHold Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
...
HRESULT GLChar::MsgReqClubHoldToStorage ( NET_MSG_GENERIC* nmg )
{
	...
	//	거래옵션
	/*item wrapper, Juver, 2018/01/12 */
	if ( pHoldData->sBasicOp.emItemType != ITEM_WRAPPER_BOX )
	{
		if ( !pHoldData->sBasicOp.IsClubLocker() )	return FALSE;
...
HRESULT GLChar::MsgReqClubStorageSplit ( NET_MSG_GENERIC* nmg )
{
	...
	{
		RLOG::LogFile( "ItemPileCheck.txt", "[%u]%s MsgReqClubStorageSplit Item Pos[%u/%u] Item [%u/%u]%s pile:%u", 
			m_dwCharID, 
			m_szName, 
			pNetMsg->wPosX, 
			pNetMsg->wPosY,
...
HRESULT GLChar::MsgReqClubStorageSaveMoney ( NET_MSG_GENERIC* nmg )
{
	...
	if ( RPARAM::block_guild_locker_money_add )		return S_FALSE;
...
HRESULT GLChar::MsgReqClubStorageDrawMoney ( NET_MSG_GENERIC* nmg )
{
	...
	/*locker money control, EJCode, 2018/10/23 */
	if ( RPARAM::block_guild_locker_money_take )	return S_FALSE;
