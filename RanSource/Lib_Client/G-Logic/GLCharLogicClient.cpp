...
void GLCHARLOGIC_CLIENT::RESET_DATA ()
{
	int i=0;
	...
	m_sSUM_PASSIVE = SPASSIVE_SKILL_DATA_CLIENT();
	...
	SumTargetRangeSet(0.0f);
	SumTargetRangeSet2(0.0f);
	
	/*skill range spec logic, Juver, 2017/06/06 */
	//m_fSUM_ATTACK_RANGE = 0.0f;
	SumAttackRangeSet(0.0f);
	SumAttackRangeSet2(0.0f);

	//m_fSUM_APPLY_RANGE = 0.0f;
	SumApplyRangeSet(0.0f);
	SumApplyRangeSet2(0.0f);
...
void GLCHARLOGIC_CLIENT::INIT_NEW_CHAR ( const EMCHARINDEX _emCIndex, const DWORD dwUserID, const DWORD dwServerID, const char* szCharName,
...
const SCHARSTATS& GLCHARLOGIC_CLIENT::GETSTATS_ADD () const
{
...
const SCHARSTATS& GLCHARLOGIC_CLIENT::GETSTATS_ITEM () const
{
	return m_sSUMITEM.sStats;
}

void GLCHARLOGIC_CLIENT::OptionMoveUp()
{
	m_fOPTION_MOVE += 0.1f;
...
void GLCHARLOGIC_CLIENT::OptionMoveDown()
{
	m_fOPTION_MOVE -= 0.1f;
...
void GLCHARLOGIC_CLIENT::SUM_ADDITION ( float fCONFT_POINT_RATE )
{
	int i=0;
...
void GLCHARLOGIC_CLIENT::SUM_ITEM ()
{
	m_sSUMITEM.RESET();
...
void GLCHARLOGIC_CLIENT::SUM_PASSIVE ()
{
	m_sSUM_PASSIVE = SPASSIVE_SKILL_DATA_CLIENT();
...
void GLCHARLOGIC_CLIENT::INIT_RECOVER ( int nRECOVER )
{
...
void GLCHARLOGIC_CLIENT::RELEASE_SLOT_ITEM ( EMSLOT _slot )
{
...
void GLCHARLOGIC_CLIENT::SLOT_ITEM ( const SITEMCUSTOM &sItemCustom, EMSLOT _slot )
{
...
void GLCHARLOGIC_CLIENT::HOLD_ITEM ( const SITEMCUSTOM &sItemCustom )
{
...
void GLCHARLOGIC_CLIENT::RELEASE_HOLD_ITEM ()
{
...
SITEM* GLCHARLOGIC_CLIENT::GET_ELMT_ITEM ()
{
	EMSLOT emRHand = GetCurRHand();
	EMSLOT emLHand = GetCurLHand();
...
BOOL GLCHARLOGIC_CLIENT::INIT_DATA ( BOOL bNEW, BOOL bReGen, float fCONFT_POINT_RATE, bool bInitNowExp )
{
	int i = 0;
...
WORD GLCHARLOGIC_CLIENT::RECEIVE_DAMAGE ( const WORD wDamage )
{
...
BOOL GLCHARLOGIC_CLIENT::RECEIVE_QITEMFACT ( const SNATIVEID &nidITEM, BOOL bMobGen )
{
...
void GLCHARLOGIC_CLIENT::RECEIVE_EVENTFACT_BEGIN ( const EMGM_EVENT_TYPE emType, const WORD wValue )
{
	m_sEVENTFACT.SetEVENT( emType, wValue );
}

void GLCHARLOGIC_CLIENT::RECEIVE_EVENTFACT_END ( const EMGM_EVENT_TYPE emType )
{
...
//
bool GLCHARLOGIC_CLIENT::UPDATE_QITEMFACT ( float fElapsedTime )
{
...
void GLCHARLOGIC_CLIENT::UPDATE_MAX_POINT ( float fCONFT_POINT_RATE )
{
	EMCHARINDEX emCI = GETCHARINDEX ();										//	케릭터의 종류 인덱스.
	const GLCONST_CHARCLASS &cCHARCONST = GLCONST_CHAR::cCONSTCLASS[emCI];	//	케릭터의 상수들.
...
void GLCHARLOGIC_CLIENT::UPDATE_DATA ( float fTime, float fElapsedTime, BOOL bClient, float fCONFT_POINT_RATE )
{
	//	사용된 스킬 딜래이 갱신.
	...
	SumPierceSet(m_sSUM_PASSIVE.m_nPIERCE);
	SumPierceSet2(m_sSUM_PASSIVE.m_nPIERCE);

	//m_fSUM_TARGET_RANGE		= m_sSUM_PASSIVE.m_fTARGET_RANGE;
	SumTargetRangeSet(m_sSUM_PASSIVE.m_fTARGET_RANGE);
	SumTargetRangeSet2(m_sSUM_PASSIVE.m_fTARGET_RANGE);

	//m_fSUM_ATTACK_RANGE		= m_sSUM_PASSIVE.m_fATTACK_RANGE;
	SumAttackRangeSet(m_sSUM_PASSIVE.m_fATTACK_RANGE);
	SumAttackRangeSet2(m_sSUM_PASSIVE.m_fATTACK_RANGE);

	//m_fSUM_APPLY_RANGE		= m_sSUM_PASSIVE.m_fAPPLY_RANGE;
	SumApplyRangeSet(m_sSUM_PASSIVE.m_fAPPLY_RANGE);
	SumApplyRangeSet2(m_sSUM_PASSIVE.m_fAPPLY_RANGE);
	...
	for ( int i=0; i<SKILLFACT_SIZE; ++i )
	{
		...
		for( int nSpec=0; nSpec<SKILL::MAX_SPEC; ++nSpec )
		{
			...
			case EMSPECA_PIERCE:	
				{
					//m_nSUM_PIERCE += int ( sSKEFF.sSpecs[nSpec].fSPECVAR1 );

					int _nSUM_PIERCE = SumPierceGet();
					_nSUM_PIERCE += int ( sSKEFF.sSpecs[nSpec].fSPECVAR1 );
					SumPierceSet( _nSUM_PIERCE );
					SumPierceSet2( _nSUM_PIERCE );
				}
				break;

			case EMSPECA_TARRANGE:	
				{
					//m_fSUM_TARGET_RANGE += sSKEFF.sSpecs[nSpec].fSPECVAR1;	

					float _fSUM_TARGET_RANGE = SumTargetRangeGet();
					_fSUM_TARGET_RANGE += sSKEFF.sSpecs[nSpec].fSPECVAR1;
					SumTargetRangeSet(_fSUM_TARGET_RANGE);
					SumTargetRangeSet2(_fSUM_TARGET_RANGE);
				}
			...
			case EMSPECA_CHANGE_ATTACK_RANGE:
				{
					...
					float _fSUM_ATTACK_RANGE = SumAttackRangeGet();
					_fSUM_ATTACK_RANGE += sSKEFF.sSpecs[nSpec].fSPECVAR1;
					SumAttackRangeSet(_fSUM_ATTACK_RANGE);
					SumAttackRangeSet2(_fSUM_ATTACK_RANGE);
				}	
				break;
			...
			case EMSPECA_CHANGE_APPLY_RANGE:
				{
					...
					float _fSUM_APPLY_RANGE = SumApplyRangeGet();
					_fSUM_APPLY_RANGE += sSKEFF.sSpecs[nSpec].fSPECVAR1;
					SumApplyRangeSet(_fSUM_APPLY_RANGE);
					SumApplyRangeSet2(_fSUM_APPLY_RANGE);
				}
...
//			for ( i=0; i<SKILLFACT_SIZE; ++i )		DISABLESKEFF ( i );
	...
	for ( int i=0; i<FITEMFACT_SIZE; ++i )
	{
		...
		for( int nSpec=0; nSpec<SKILL::MAX_SPEC; ++nSpec )
		{
			...
			case EMSPECA_PIERCE:
				{
					//m_nSUM_PIERCE += int ( sFITEMEFF.sSpecs[nSpec].fSPECVAR1 );

					int _nSUM_PIERCE = SumPierceGet();
					_nSUM_PIERCE += int ( sFITEMEFF.sSpecs[nSpec].fSPECVAR1 );
					SumPierceSet( _nSUM_PIERCE );
					SumPierceSet2( _nSUM_PIERCE );
				}
				break;
			...
			case EMSPECA_TARRANGE:
				{
					//m_fSUM_TARGET_RANGE += sFITEMEFF.sSpecs[nSpec].fSPECVAR1;	

					float _fSUM_TARGET_RANGE = SumTargetRangeGet();
					_fSUM_TARGET_RANGE += sFITEMEFF.sSpecs[nSpec].fSPECVAR1;
					SumTargetRangeSet(_fSUM_TARGET_RANGE);
					SumTargetRangeSet2(_fSUM_TARGET_RANGE);
				}	
				break;
			...
			case EMSPECA_CHANGE_ATTACK_RANGE:
				{
					...
					float _fSUM_ATTACK_RANGE = SumAttackRangeGet();
					_fSUM_ATTACK_RANGE += sFITEMEFF.sSpecs[nSpec].fSPECVAR1;
					SumAttackRangeSet(_fSUM_ATTACK_RANGE);
					SumAttackRangeSet2(_fSUM_ATTACK_RANGE);
				}
				break;
			...
			case EMSPECA_CHANGE_APPLY_RANGE:
				{
					...
					float _fSUM_APPLY_RANGE = SumApplyRangeGet();
					_fSUM_APPLY_RANGE += sFITEMEFF.sSpecs[nSpec].fSPECVAR1;
					SumApplyRangeSet(_fSUM_APPLY_RANGE);
					SumApplyRangeSet2(_fSUM_APPLY_RANGE);
				}
				break;
	...
	for ( int i=0; i<SYSTEM_BUFF_SIZE; ++i )
	{
		...
		for( int nSpec=0; nSpec<SKILL::MAX_SPEC; ++nSpec )
		{
			...
			case EMSPECA_PIERCE:	
				{
					//m_nSUM_PIERCE += int ( ssystem_buff.sSpecs[nSpec].fSPECVAR1 );	

					int _nSUM_PIERCE = SumPierceGet();
					_nSUM_PIERCE += int ( ssystem_buff.sSpecs[nSpec].fSPECVAR1 );
					SumPierceSet( _nSUM_PIERCE );
					SumPierceSet2( _nSUM_PIERCE );
				}
				break;
			...
			case EMSPECA_TARRANGE:		
				{
					//m_fSUM_TARGET_RANGE += ssystem_buff.sSpecs[nSpec].fSPECVAR1;		

					float _fSUM_TARGET_RANGE = SumTargetRangeGet();
					_fSUM_TARGET_RANGE += ssystem_buff.sSpecs[nSpec].fSPECVAR1;
					SumTargetRangeSet(_fSUM_TARGET_RANGE);
					SumTargetRangeSet2(_fSUM_TARGET_RANGE);
				}
			...
			case EMSPECA_CHANGE_ATTACK_RANGE:
				{
					...
					float _fSUM_ATTACK_RANGE = SumAttackRangeGet();
					_fSUM_ATTACK_RANGE += ssystem_buff.sSpecs[nSpec].fSPECVAR1;
					SumAttackRangeSet(_fSUM_ATTACK_RANGE);
					SumAttackRangeSet2(_fSUM_ATTACK_RANGE);
				}	
				break;
			...
			case EMSPECA_CHANGE_APPLY_RANGE:
				{
					...
					float _fSUM_APPLY_RANGE = SumApplyRangeGet();
					_fSUM_APPLY_RANGE += ssystem_buff.sSpecs[nSpec].fSPECVAR1;
					SumApplyRangeSet(_fSUM_APPLY_RANGE);
					SumApplyRangeSet2(_fSUM_APPLY_RANGE);
				}
				break;
...
float GLCHARLOGIC_CLIENT::GETATTVELO ()
{
...
float GLCHARLOGIC_CLIENT::GETMOVEVELO ()
{
	/*additional RV, Juver, 2018/06/15 */
...
float GLCHARLOGIC_CLIENT::GET_MOVE_VELO() const
{
	/*additional RV, Juver, 2018/06/15 */
...
float GLCHARLOGIC_CLIENT::GET_EXP_TOTAL() const
{
	float fEXP = m_fEXP_RATE+m_sSUMITEM.fExpRate;
	return fEXP<0.0f?0.0f:fEXP;
}

float GLCHARLOGIC_CLIENT::GET_HP_INCR() const
{
	float fVal = 0.0f;
...
float GLCHARLOGIC_CLIENT::GET_MP_INCR() const
{
	float fVal = 0.0f;
...
float GLCHARLOGIC_CLIENT::GET_SP_INCR() const
{
	float fVal = 0.0f;
...
//	단순히 장착위치 인지만 검사.
BOOL GLCHARLOGIC_CLIENT::CHECKSLOT_ITEM ( SNATIVEID sNativeID, EMSLOT emSlot )
{
...
BOOL GLCHARLOGIC_CLIENT::ISEMPTY_SLOT ( SNATIVEID sNativeID, EMSLOT emSlot )
{
	SITEM* pItem = GLItemMan::GetInstance().GetItem ( sNativeID );
	if ( pItem==NULL )	return FALSE;
...
BOOL GLCHARLOGIC_CLIENT::ACCEPT_ITEM ( SNATIVEID sNativeID )
{
	if ( sNativeID==NATIVEID_NULL() )							return FALSE;
...
BOOL GLCHARLOGIC_CLIENT::SIMPLE_CHECK_ITEM ( SNATIVEID sNativeID )
{
	if ( sNativeID==NATIVEID_NULL() )							return FALSE;
...
BOOL GLCHARLOGIC_CLIENT::SIMPLE_CHECK_ITEM ( SNATIVEID sNativeID, SNATIVEID sDisguiseID )
{
	if ( sNativeID==NATIVEID_NULL() )							return FALSE;
...
BOOL GLCHARLOGIC_CLIENT::ACCEPT_ITEM ( SNATIVEID sNativeID, SNATIVEID sDisguiseID )
{
	if ( sNativeID==NATIVEID_NULL() )							return FALSE;
	if ( sDisguiseID==NATIVEID_NULL() )							return FALSE;
...
WORD GLCHARLOGIC_CLIENT::CALC_ACCEPTP ( SNATIVEID sNativeID )
{
	WORD wATP = 0;
...
EMBEGINATTACK_FB GLCHARLOGIC_CLIENT::BEGIN_ATTACK ( WORD wStrikeNum )
{
	EMSLOT emRHand = GetCurRHand();
	EMSLOT emLHand = GetCurLHand();
...
BOOL GLCHARLOGIC_CLIENT::VALID_LEVELUP () const
{
...
LONGLONG GLCHARLOGIC_CLIENT::GET_LEVELUP_EXP () const
{
	if( m_CHARINDEX == GLCI_EXTREME_M || m_CHARINDEX == GLCI_EXTREME_W )
	{
...
void GLCHARLOGIC_CLIENT::LEVLEUP ( bool bInitNowExp )
{
...
void GLCHARLOGIC_CLIENT::STATSUP ( EMSTATS emStats )
{
	switch ( emStats )
	{
...
void GLCHARLOGIC_CLIENT::STATSUP_MODERN ( SCHARSTATS	sStats )
{
...
// Desc: 스킬정보 리셋
// *****************************************************
bool GLCHARLOGIC_CLIENT::RESET_SKILL ()
{
...
// Desc: 스텟정보 리셋
// *****************************************************
bool GLCHARLOGIC_CLIENT::RESET_STATS ( const WORD wDIS )
{
	EMCHARINDEX emINDEX = CharClassToIndex(m_emClass);
	const SCHARDATA2& sCHARDATA = GLCONST_CHAR::GET_CHAR_DATA2(m_wSchool,emINDEX);
...
bool GLCHARLOGIC_CLIENT::RESET_STATS_SKILL ( const WORD wDIS_STAT )
{
...
void GLCHARLOGIC_CLIENT::UPDATESKILLDELAY ( float fElapsedTime )
{
...
void GLCHARLOGIC_CLIENT::LEARN_SKILL ( SNATIVEID skill_id, SNATIVEID item_id, __time64_t _tBornTime )
{
	//	Note : 스킬 정보 가져옴.
...
bool GLCHARLOGIC_CLIENT::LEARN_SKILL_QUEST ( SNATIVEID skill_id )
{
	//	Note : 스킬 정보 가져옴.
	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( skill_id.wMainID, skill_id.wSubID );
...
VOID GLCHARLOGIC_CLIENT::LVLUP_SKILL ( SNATIVEID skill_id, WORD wToLevel )
{
	//	Note : 스킬 정보 가져옴.
	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( skill_id.wMainID, skill_id.wSubID );
	if ( !pSkill )											return;
...
BOOL GLCHARLOGIC_CLIENT::ISLEARNED_SKILL ( SNATIVEID skill_id, WORD wLEVEL )
{
	//	Note : 스킬 정보 가져옴.
	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( skill_id.wMainID, skill_id.wSubID );
	if ( !pSkill )											return FALSE;
...
BOOL GLCHARLOGIC_CLIENT::ISMASTER_SKILL ( SNATIVEID skill_id )
{
	//	Note : 스킬 정보 가져옴.
	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( skill_id.wMainID, skill_id.wSubID );
	if ( !pSkill )												return FALSE;
...
SCHARSKILL* GLCHARLOGIC_CLIENT::GETLEARNED_SKILL ( SNATIVEID skill_id )
{
	//	Note : 익힌 스킬인지 검사.
	//
	SKILL_MAP_ITER learniter = m_ExpSkills.find ( skill_id.dwID );
...
EMSKILL_LEARNCHECK GLCHARLOGIC_CLIENT::CHECKLEARNABLE_SKILL ( SNATIVEID skill_id )
{
	WORD wSKILL_LEVEL = 0;
...
void GLCHARLOGIC_CLIENT::SETACTIVESKILL ( SNATIVEID skill_id )
{
	m_idACTIVESKILL = skill_id;
}

/*extreme auto tab, Juver, 2018/09/10 */
EMSKILLCHECK GLCHARLOGIC_CLIENT::CHECHSKILL ( SNATIVEID skill_id, WORD wStrikeNum, bool bNotLearn, EMSLOT emRHand, EMSLOT emLHand )
{
	/*prohibit skill logic, Juver, 2017/06/06 */
...
void GLCHARLOGIC_CLIENT::ACCOUNTSKILL ( SNATIVEID skill_id, WORD wStrikeNum, bool bServer )
{
...
BOOL GLCHARLOGIC_CLIENT::DOGRINDING(	SITEMCUSTOM &sCusItem, 
								const SITEM *pHold, 
								bool &_bRESET, 
								bool &_bTERMINATE, 
...
//
BOOL GLCHARLOGIC_CLIENT::ISHAVEITEM ( SNATIVEID &sNID, DWORD *pNum )
{
	DWORD dwNum = 0;
	for ( int i=0; i<SLOT_TSIZE; ++i )
	{
...
BOOL GLCHARLOGIC_CLIENT::ISREVIVE ()
{
	/*ep8 puton slots, Juver, 2018/03/30 */
...
float GLCHARLOGIC_CLIENT::GETSUM_TARRANGE ()
{
	//return m_fSUM_TARGET_RANGE;

	return SumTargetRangeGet();
}

int GLCHARLOGIC_CLIENT::GETSUM_PIERCE()
{
...
/*skill range spec logic, Juver, 2017/06/06 */
WORD GLCHARLOGIC_CLIENT::GETSKILLRANGE_APPLY ( const GLSKILL &sSKILL, const WORD dwLEVEL )
{
...
DWORD GLCHARLOGIC_CLIENT::GET_PK_LEVEL ()
{
...
float GLCHARLOGIC_CLIENT::GET_PK_DECEXP_RATE ()
{
...
std::string GLCHARLOGIC_CLIENT::GET_PK_NAME ()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
...
DWORD GLCHARLOGIC_CLIENT::GET_PK_COLOR ()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
...
float GLCHARLOGIC_CLIENT::GET_PK_SHOP2BUY ()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
	if ( dwLEVEL==UINT_MAX )	return 100.0f;
...
float GLCHARLOGIC_CLIENT::GET_PK_SHOP2SALE ()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
	if ( dwLEVEL==UINT_MAX )	return 100.0f;
...
DWORD GLCHARLOGIC_CLIENT::GET_PK_ITEMDROP_NUM ()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
	if ( dwLEVEL==UINT_MAX )
	{
...
float GLCHARLOGIC_CLIENT::GET_PK_ITEMDROP_RATE ()
{
	DWORD dwLEVEL = GET_PK_LEVEL();
	float fRATE = 0;
...
bool GLCHARLOGIC_CLIENT::ISPLAYKILLING ()
{
	return !m_mapPlayHostile.empty();
}

//	Note : 자신이 범죄자인가?
bool GLCHARLOGIC_CLIENT::ISOFFENDER ()
{
	return ( m_nBright < GLCONST_CHAR::sPK_STATE[0].nPKPOINT );
}

bool GLCHARLOGIC_CLIENT::IS_HOSTILE_ACTOR ( DWORD dwCHARID )
{
...
bool GLCHARLOGIC_CLIENT::ADD_PLAYHOSTILE ( DWORD dwCHARID, BOOL bBAD )
{
...
bool GLCHARLOGIC_CLIENT::DEL_PLAYHOSTILE ( DWORD dwCHARID )
{
	MAPPLAYHOSTILE_ITER pos = m_mapPlayHostile.find ( dwCHARID );
	if ( pos!=m_mapPlayHostile.end() )
	{
...
void GLCHARLOGIC_CLIENT::DEL_PLAYHOSTILE_ALL ()
{
...
bool GLCHARLOGIC_CLIENT::IS_PLAYHOSTILE ( DWORD dwCHARID )
{
	MAPPLAYHOSTILE_ITER pos = m_mapPlayHostile.find ( dwCHARID );
	return ( pos != m_mapPlayHostile.end() );
}

EMSLOT GLCHARLOGIC_CLIENT::GetCurRHand()
{
	if( IsUseArmSub() ) return SLOT_RHAND_S;
	else				return SLOT_RHAND;
}

EMSLOT GLCHARLOGIC_CLIENT::GetCurLHand()
{
	if( IsUseArmSub() ) return SLOT_LHAND_S;
	else				return SLOT_LHAND;
}

BOOL GLCHARLOGIC_CLIENT::VALID_SLOT_ITEM ( EMSLOT _slot )							
{ 
...
BOOL GLCHARLOGIC_CLIENT::ISLONGRANGE_ARMS ()							
{
	EMSLOT emRHand = GetCurRHand();
...
BOOL GLCHARLOGIC_CLIENT::ISLONGRANGE_ARMS ( GLITEM_ATT emAttack )
{
	switch( emAttack )
	{
...
BOOL GLCHARLOGIC_CLIENT::IsCurUseArm( EMSLOT emSlot )
{
	if( IsUseArmSub() )
	{
...
void GLCHARLOGIC_CLIENT::DISABLEALLLANDEFF()
{
	for( int i = 0; i < EMLANDEFFECT_MULTI; i++ )
	{
...
void GLCHARLOGIC_CLIENT::ADDLANDEFF( SLANDEFFECT landEffect, int iNum )
{
...
/*continuous damage skill logic, Juver, 2017/06/10 */
BOOL GLCHARLOGIC_CLIENT::CONTINUOUSDAMAGE_EXIST( SNATIVEID _sID )
{
...
/*continuous damage skill logic, Juver, 2017/06/10 */
void GLCHARLOGIC_CLIENT::CONTINUOUSDAMAGE_INSERT( SCONTINUOUS_DAMAGE_DATA _sDATA )
{
...
/*continuous damage skill logic, Juver, 2017/06/10 */
void GLCHARLOGIC_CLIENT::CONTINUOUSDAMAGE_DELETE( SNATIVEID _sID )
{
...
BOOL GLCHARLOGIC_CLIENT::ISHAVE_BUFF( SNATIVEID skill_id, WORD wLEVEL/*=1*/ )
{
	//level range 1~9
...
SACTIVITY_CHAR_DATA* GLCHARLOGIC_CLIENT::GetActivityProg( DWORD dwActivityID )
{
...
/*activity system, Juver, 2017/10/30 */
SACTIVITY_CHAR_DATA* GLCHARLOGIC_CLIENT::GetActivityDone( DWORD dwActivityID )
{
	if ( dwActivityID == UINT_MAX )	return NULL;
...
/*activity system, Juver, 2017/10/30 */
DWORD GLCHARLOGIC_CLIENT::GetActivityProgNum()
{
	return DWORD( m_mapActivityProg.size() );
}

/*activity system, Juver, 2017/10/30 */
DWORD GLCHARLOGIC_CLIENT::GetActivityDoneNum()
{
	return DWORD( m_mapActivityDone.size() );
}

/* Reset Stats Item */
bool GLCHARLOGIC_CLIENT::STATS_RESET ()
{
...
WORD GLCHARLOGIC_CLIENT::get_additional_gain_hp()  const
{
	return (WORD)m_sSUMITEM.nadd_gain_hp;
}

/*official random value, Juver, 2018/06/12 */
WORD GLCHARLOGIC_CLIENT::get_additional_gain_mp()  const
{
	return (WORD)m_sSUMITEM.nadd_gain_mp;
}

/*official random value, Juver, 2018/06/12 */
WORD GLCHARLOGIC_CLIENT::get_additional_gain_sp()  const
{
	return (WORD)m_sSUMITEM.nadd_gain_sp;
}

/*official random value, Juver, 2018/06/12 */
WORD GLCHARLOGIC_CLIENT::get_additional_gain_cp()  const
{
	return (WORD)m_sSUMITEM.nadd_gain_cp;
}

/*12-9-14, Battle Pass - CNDev*/
SBATTLEPASS_CHAR_DATA* GLCHARLOGIC_CLIENT::GetBattlePassProg( DWORD dwBattlePassID )
{
...
SBATTLEPASS_CHAR_DATA* GLCHARLOGIC_CLIENT::GetBattlePassDone( DWORD dwBattlePassID )
{
	if ( dwBattlePassID == UINT_MAX )	return NULL;
...
DWORD GLCHARLOGIC_CLIENT::GetBattlePassProgNum()
{
	return DWORD( m_mapBattlePassProg.size() );
}

DWORD GLCHARLOGIC_CLIENT::GetBattlePassDoneNum()
{
	return DWORD( m_mapBattlePassDone.size() );
}
/*additional RV, Juver, 2018/06/14 */
float GLCHARLOGIC_CLIENT::get_critical_hit_chance()  const
{
...
/*additional RV, Juver, 2018/06/14 */
float GLCHARLOGIC_CLIENT::get_critical_hit_damage()  const
{
...
/*additional RV, Juver, 2018/06/14 */
float GLCHARLOGIC_CLIENT::get_crushing_blow_chance()  const
{
...
/*additional RV, Juver, 2018/06/14 */
float GLCHARLOGIC_CLIENT::get_crushing_blow_damage()  const
{
	int val = int( GLCONST_CHAR::dwCRUSHING_BLOW_DAMAGE + (int)m_sSUMITEM.fadd_crushing_blow_damage );
	return (float)val;
}

EMCHECK_WEAR GLCHARLOGIC_CLIENT::CHECK_WEAR_ITEM_SLOT( SNATIVEID sItemID, EMSLOT emPutOnSlot )
{
...
bool GLCHARLOGIC_CLIENT::SetOptionIsSuit( EMSUIT emSuit )
{
	return false;
}
/* Codex, Archie 02/16/24 */
SCODEX_CHAR_DATA* GLCHARLOGIC_CLIENT::GetCodexProg( DWORD dwCodexID )
{
...
/* Codex, Archie 02/16/24 */
SCODEX_CHAR_DATA* GLCHARLOGIC_CLIENT::GetCodexDone( DWORD dwCodexID )
{
	if ( dwCodexID == UINT_MAX )	return NULL;
...
/* Codex, Archie 02/16/24 */
DWORD GLCHARLOGIC_CLIENT::GetCodexProgNum()
{
	return DWORD( m_mapCodexProg.size() );
}

/* Codex, Archie 02/16/24 */
DWORD GLCHARLOGIC_CLIENT::GetCodexDoneNum()
{
	return DWORD( m_mapCodexDone.size() );
}
/* Codex HP, Archie 02/16/24 */
void GLCHARLOGIC_CLIENT::CODEX_STATS( int nIndex, DWORD dwStatPoint )
{
...
//MMR Rank
void GLCHARLOGIC_CLIENT::ShowRankName( BOOL bShowName ) 
{
	m_bRankName = bShowName;
}
void GLCHARLOGIC_CLIENT::ShowRankMark( BOOL bShowMark )
{
