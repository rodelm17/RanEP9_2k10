...
public:
	...
	enum EM_BASIC
	{
		...
		EM_CURRENCY_SHOP,
		EM_CHAOS_MACHINE,	/* chaos machine, Juver, 2021/07/07 */

		EM_CONTRIBUTION_SHOP,

						/*self buff via NPC, DevArt22, 2023/03/18 */
		EM_SELF_BUFF_BA_ONLY,
		EM_SELF_BUFF_FULL,
		EM_SELF_BUFF_FULL_NO_BA,
		EM_BASIC_SIZE
	};
...
public:	
	...
	BOOL DoTEST ( GLCHARLOGIC_SERVER *pCHARLOGIC, DWORD dwIndex = UINT_MAX );
	BOOL DoTEST ( GLCHARLOGIC_CLIENT *pCHARLOGIC, DWORD dwIndex = UINT_MAX );
