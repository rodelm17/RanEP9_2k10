...
HRESULT PARTICLESYS_PROPERTY::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if( dwVer == 0x106 )
	{
		...
		//	Note : 이팩트의 Property 를 저장.
		PROPERTY_106	sProp;
		SFile.ReadBuffer( &sProp, sizeof(PROPERTY_106) );
	...
	{
		CDebugSet::ErrorVersion( "PARTICLESYS_PROPERTY::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT PARTICLESYS_PROPERTY::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
...
HRESULT PARTICLESYS_PROPERTY::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( m_dwFlag & USEMESH )
	{
		...
		if ( !m_pMesh )
		{
			CDebugSet::ErrorCode( "PARTICLESYS_PROPERTY::InitDeviceObjects", "load failed :%s", m_szMeshFile );
			//return E_FAIL;
		}
