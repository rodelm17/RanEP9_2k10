...
//--------------------------------------------------------------------
CGarbageInventoryWindow::CGarbageInventoryWindow()	// ÈÞÁöÅë
	: m_pPage( NULL )
	, m_nONE_VIEW_SLOT( 4 )
...
VOID CGarbageInventoryWindow::CreateSubControl()
{
	...
	{
		CUIControl* pControl = CreateControl ( "GARBAGE_INVENTORY_GAME_POINTS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pGamePointsTextBox = CreateStaticControl ( "GARBAGE_INVENTORY_GAME_POINTS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pGamePointsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "GARBAGE_INVENTORY_GAME_POINTS_BUTTON", "GARBAGE_INVENTORY_GAME_POINTS_BUTTON_F", GARBAGEINVENTORY_GAME_POINTS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "GARBAGE_INVENTORY_GAME_POINTS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}

	{
		CUIControl* pControl = CreateControl ( "GARBAGE_INVENTORY_WAR_CHIPS_BACK" );
		pControl->SetAlignFlag ( UI_FLAG_BOTTOM );

		CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW | D3DFONT_ASCII );
		m_pWarChipsTextBox = CreateStaticControl ( "GARBAGE_INVENTORY_WAR_CHIPS_TEXT", pFont8, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		m_pWarChipsTextBox->SetAlignFlag ( UI_FLAG_BOTTOM );

		CBasicButton* pButton = NULL;
		pButton = CreateFlipButton ( "GARBAGE_INVENTORY_WAR_CHIPS_BUTTON", "GARBAGE_INVENTORY_WAR_CHIPS_BUTTON_F", GARBAGEINVENTORY_WAR_CHIPS_BUTTON, CBasicButton::CLICK_FLIP );
		pButton->CreateMouseOver ( "GARBAGE_INVENTORY_WAR_CHIPS_BUTTON_F" );
		pButton->SetAlignFlag ( UI_FLAG_BOTTOM );
		pButton->SetUseDynamic ( TRUE );
	}
}
...
VOID CGarbageInventoryWindow::TranslateUIMessage( UIGUID ControlID, DWORD dwMsg )
{
	...
	case GARBAGEINVENTORY_PAGE:
		if( CHECK_MOUSE_IN( dwMsg ) )
		{
			...
			CDebugSet::ToView( 1, 1, "CGarbageInventoryWindow::TranslateUIMessage : %d %d", nPosX, nPosY );
