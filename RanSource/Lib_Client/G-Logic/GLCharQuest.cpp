...
void GLChar::QuestStartFromMapReach ( SNATIVEID nidMAP )
{
	GLQuestMan::QUESTGEN_RANGE* pRANGE = GLQuestMan::GetInstance().FindStartFromMapReach ( nidMAP );
	if ( !pRANGE )		return;

	if ( nidMAP == NATIVEID_NULL() )	return;
	...
	for ( ; pos!=pRANGE->second; ++pos )
	{
		...
		//jdevtodo
		//when position and range condition is not met, insert the quest to another container and evaluate every frame
		/*if ( m_pLandMan && pQUEST->m_sSTARTOPT.wPosX != USHRT_MAX && pQUEST->m_sSTARTOPT.wPosY != USHRT_MAX )
		{
		int nPosX(0);
		int nPosY(0);
		...
		if ( pQUEST->m_sSTARTOPT.wPosX > pQUEST->m_sSTARTOPT.wSIZE )	wMINX_REACH = pQUEST->m_sSTARTOPT.wPosX - pQUEST->m_sSTARTOPT.wSIZE;
		else															wMINX_REACH = 0;

		if ( pQUEST->m_sSTARTOPT.wPosY > pQUEST->m_sSTARTOPT.wSIZE )	wMINY_REACH = pQUEST->m_sSTARTOPT.wPosY - pQUEST->m_sSTARTOPT.wSIZE;
		else															wMINY_REACH = 0;

		wMAXX_REACH = pQUEST->m_sSTARTOPT.wPosX + pQUEST->m_sSTARTOPT.wSIZE;
		wMAXY_REACH = pQUEST->m_sSTARTOPT.wPosY + pQUEST->m_sSTARTOPT.wSIZE;

		if ( !( wMINX_REACH < nPosX && nPosX < wMAXX_REACH && wMINY_REACH < nPosY && nPosY < wMAXY_REACH ) )	continue;
		}*/
...
bool GLChar::DoQuestProgress ( GLQUESTPROG* pQUEST_PROG )
{
	...
	if ( bPROG )
	{
		...
		if( stepMoveMap.nidMAP.wMainID != USHRT_MAX &&
			stepMoveMap.nidMAP.wSubID  != USHRT_MAX )
		{
			QuestMoveProgressEvent( dwQID, stepMoveMap.nidMAP, stepMoveMap.wPosX, stepMoveMap.wPosY, stepMoveMap.dwGateID );
		}
...
void GLChar::DoQuestGenerateReward ( GLQUEST* pQUEST )
{
	...
	if ( pQUEST->m_dwGiftContributionPoints>0 )
	{
		DWORD dwAmount = pQUEST->m_dwGiftContributionPoints;

		m_dwContributionPoint += dwAmount;

		GLMSG::SNETPC_UPDATE_CONTRIBUTION_POINT NetMsgContributionPoint;
		NetMsgContributionPoint.dwPoint = m_dwContributionPoint;
		glGaeaServer.SENDTOCLIENT ( m_dwClientID, (NET_MSG_GENERIC*) &NetMsgContributionPoint );
	}
...
bool GLChar::QuestMoveProgressEvent ( DWORD questid, SNATIVEID mapID, DWORD wPosX, DWORD wPosY, DWORD dwGateID, bool bStartMove /*= FALSE*/ )
{
	BOOL add = GLGaeaServer::GetInstance().quest_move_insert( m_dwCharID, mapID, wPosX, wPosY, dwGateID, bStartMove, questid );
...
bool GLChar::CheckQuestProgressEvent ( BYTE eventType, GLQUEST* pQUEST )
{
	...
	if( pQUEST->m_ProgressEvent.bUseProgressEvent )
	{
		...
		{
			if( !QuestMoveProgressEvent( pQUEST->m_sNID.dwID, progressMap.nidMAP, progressMap.wPosX, progressMap.wPosY, progressMap.dwGateID ) ) return FALSE;	
		}
		//  Note : 만약 시작시 캐릭터의 시작 지점으로 이동을 해야하는 경우
		if( dwProgress == EMQP_START_MOVE )
		{
			if( !QuestMoveProgressEvent( pQUEST->m_sNID.dwID, m_sStartMapID, USHRT_MAX, USHRT_MAX, 0, TRUE ) ) return FALSE;
		}
