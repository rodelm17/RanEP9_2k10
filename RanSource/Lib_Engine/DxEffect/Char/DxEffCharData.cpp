								  ...
								  D3DXVECTOR3 *pDir, float fTarDir )
{
	...
	for ( int i=0; i<PIECE_SIZE; i++ )
	{
		if ( pSkinChar->m_pAttBone[i] )
			PutEffect( pSkinChar->m_pAttBone[i], szEffFile, pDir, fTarDir );
	}

	/*vehicle system, Juver, 2017/08/06 */
	if ( pSkinChar->m_pVehicle )
		PutEffect( pSkinChar->m_pVehicle, szEffFile, pDir, fTarDir );
...
BOOL DxEffcharDataMan::PutPassiveEffect ( DxSkinChar *pSkinChar, const char* szEffFile, D3DXVECTOR3 *pDir, float fTarDir )
{
	...
	{
		if ( pSkinChar->m_pAttBone[i] )
			PutPassiveEffect( pSkinChar->m_pAttBone[i], szEffFile, pDir, fTarDir );
	}

	/*vehicle system, Juver, 2017/08/06 */
	if ( pSkinChar->m_pVehicle )
		PutPassiveEffect( pSkinChar->m_pVehicle, szEffFile, pDir, fTarDir );
...
void DxEffcharDataMan::OutEffect ( DxSkinChar *pSkinChar, std::string strEffFile )
{
	...
	{
		if ( pSkinChar->m_pAttBone[i] )
			OutEffect( pSkinChar->m_pAttBone[i], strEffFile );
	}

	/*vehicle system, Juver, 2017/08/06 */
	if ( pSkinChar->m_pVehicle )
		OutEffect( pSkinChar->m_pVehicle, strEffFile );
}
