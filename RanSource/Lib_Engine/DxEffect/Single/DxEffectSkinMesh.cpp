...
#include "../../Meshs/DxSkinCharData.h"
...
DxEffSingle* SKINMESH_PROPERTY::NEWOBJ ()
{
	...
	if( m_pd3dDevice )
	{
		pEffect->m_SkinChar.DeleteDeviceObjects();
		pEffect->m_SkinChar.ClearAll();

		DxSkinCharData* pSkinCharData = DxSkinCharDataContainer::GetInstance().LoadData( m_szMeshFile, m_pd3dDevice, TRUE );
		if( pSkinCharData )
		{
			pEffect->m_SkinChar.SetCharData( pSkinCharData, m_pd3dDevice );
			pEffect->m_SkinChar.DEFAULTANI();
		}
	}
...
HRESULT SKINMESH_PROPERTY::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "SKINMESH_PROPERTY::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT SKINMESH_PROPERTY::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	//	Note : 형제, 자식 읽기.
	//
	EFF_PROPERTY::R2LoadFile ( SFile, pd3dDevice );
...
DxEffectSkinMesh::~DxEffectSkinMesh(void)
{
	DxSkinCharDataContainer::GetInstance().ReleaseData( m_SkinChar.m_szFileName );
...
{
	D3DXVECTOR3 vMaxMesh = m_SkinChar.m_vMax;
	D3DXVECTOR3 vMinMesh = m_SkinChar.m_vMin;
...
HRESULT DxEffectSkinMesh::RestoreDeviceObjects( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	m_SkinChar.RestoreDeviceObjects( pd3dDevice );
...
HRESULT DxEffectSkinMesh::FrameMove ( float fTime, float fElapsedTime )
{
	...
	m_SkinChar.FrameMove ( fTime, fElapsedTime );
...
HRESULT DxEffectSkinMesh::Render ( LPDIRECT3DDEVICEQ pd3dDevice, D3DXMATRIX &matComb )
{
	...
	m_SkinChar.Render ( pd3dDevice, matWorld );
