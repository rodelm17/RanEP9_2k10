...
void DxTexEffFlowUV::Render( LPDIRECT3DDEVICEQ pd3dDevice, const DWORD dwVert, const DWORD dwFaces, VERTEXNORCOLORTEX_850* pVertSrc )
{
	...
	for( DWORD i=0; i<dwVert; ++i )
	{
		...
		sBackUp.vTex2 = pVertSrc[i].vTex1 + m_vLastSpeed;
		sBackUp.fTex3 = 1.f;
...
void DxTexEffFlowUV::LoadPSF( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile )
{
	...
	{
		CDebugSet::ErrorVersion( "DxTexEffFlowUV::LoadPSF", SFile.GetFileName(), dwVer );
