...
class	CBasicTextBox;
class	CMapWindowMark;

/* map window mark, Juver, 2020/03/31 */
struct SMAP_MARK_DATA
{
	UIGUID cID;
	D3DXVECTOR3 vPos;
	bool bTextBox;
...
private:
	void	UPDATE_CHAR_POS ( CUIControl * pControl, const D3DXVECTOR3& vPlayerPos, bool bTextBox = false );
...
private:
	/* map window mark, Juver, 2020/03/31 */
	typedef	std::vector<SMAP_MARK_DATA>		VEC_MAP_MARK;
	CUIControl*		m_pMarkDummy;
	VEC_MAP_MARK	m_vecMapMark;
	UIGUID			m_NEWID;
...
public:
	...
	void MAP_MARK_RESET();
	void MAP_MARK_INIT();
	void MAP_MARK_UPDATE();

	bool MAP_MARK_CREATE_MARK( std::string strMark, std::string strLabel, const D3DXVECTOR3& vPos );
	bool MAP_MARK_CREATE_LABEL ( std::string strLabel, const D3DXVECTOR3& vPos );
};