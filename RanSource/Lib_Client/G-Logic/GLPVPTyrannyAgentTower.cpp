...
void GLPVPTyrannyAgent::RequestTowerInfo( DWORD dwCharID )
{
	...
	for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )
	{
		...
		netmsgclient.dwGuiID[i] = m_dwGuiID[i];
		netmsgclient.dwMarkVer[i] = m_dwMarkVer[i];
	}

	GLAgentServer::GetInstance().SENDTOCLIENT( pchar->m_dwClientID, &netmsgclient );
}

void GLPVPTyrannyAgent::TowerCapture( SNATIVEID sidMob, WORD wSchool, DWORD dwClubID, DWORD dwClubMarkVer )
{
	...
	for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )
	{
		...
		if ( sidMob == m_sTowerData[i].sidCrow /*&& m_sTowerData[i].wOwner != wSchool*/ )
		{
			...
			netmsgfield.dwClubID = dwClubID;
			netmsgfield.dwClubMarkVer = dwClubMarkVer;
			GLAgentServer::GetInstance().SENDTOCHANNEL( &netmsgfield, 0 );

			if ( (m_bNonTowerEnd == FALSE) && (m_sTowerScore[ m_sTowerData[i].wOwner ].wCapture  >= TYRANNY_TOWER_SIZE) )
			{
