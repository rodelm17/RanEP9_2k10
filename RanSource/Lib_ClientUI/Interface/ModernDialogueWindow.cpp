...
const	int		CModernDialogueWindow::nSTARTLINE = 0;
const	int		CModernDialogueWindow::nSTARTINDEX = 1;
const	float	CModernDialogueWindow::fANSWER_PART_INTERVAL = 8.0f;

CModernDialogueWindow::CModernDialogueWindow ()
	: m_dwRandomTime ( 0 )
	, m_fRandomTime(0.0f)
{
}
...
void CModernDialogueWindow::CreateSubControl ()
{
	CD3DFontPar* pFont9 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 9, _DEFAULT_FONT_SHADOW_FLAG );

	CUIControl* pControl = new CUIControl;
	pControl->CreateSub ( this, "MODERN_DIALOGUE_WINDOW_BACKGROUND" );
	RegisterControl ( pControl );

	CBasicLineBox* pBasicLineBox = new CBasicLineBox;
	pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_WHITE", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	pBasicLineBox->CreateBaseBoxWhiteBlankBody ( "MODERN_DIALOGUE_WINDOW_BACKGROUND_LINE" );
	RegisterControl ( pBasicLineBox );

	pBasicLineBox = new CBasicLineBox;
	pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_DIALOGUE", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	pBasicLineBox->CreateBaseBoxDialogue ( "MODERN_DIALOGUE_WINDOW_TALK_BACK" );
	RegisterControl ( pBasicLineBox );

	pBasicLineBox = new CBasicLineBox;
	pBasicLineBox->CreateSub ( this, "BASIC_LINE_BOX_DIALOGUE", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
	pBasicLineBox->CreateBaseBoxDialogue ( "MODERN_DIALOGUE_WINDOW_ANSWER_BACK" );
	RegisterControl ( pBasicLineBox );

	{
		m_pQuestionBox = new CBasicTextBox;
		m_pQuestionBox->CreateSub ( this, "MODERN_DIALOGUE_WINDOW_TALK_TEXT" );
		...
		m_pScrollBar->CreateBaseScrollBar ( "MODERN_DIALOGUE_WINDOW_TALK_TEXT_SCROLL_BAR" );
...

	{
		...
		m_pAnswerBox->CreateSub ( this, "MODERN_DIALOGUE_WINDOW_TALK_ANSWER_TEXT", UI_FLAG_DEFAULT, DIALOGUE_ANSWER_TEXTBOX );
		...
		m_pAnswerScrollBar->CreateBaseScrollBar ( "MODERN_DIALOGUE_WINDOW_TALK_BUTTON_SCROLL_BAR" );
		...
		// RandomTime
	{
		...
		m_pRandomTimeButton->CreateBaseButton ( "MODERN_DIALOGUE_WINDOW_RANDOM_TIME_BUTTON", nBUTTONSIZE, CBasicButton::RADIO_FLIP, ID2GAMEWORD( "RANDOM_TIME_BUTTON",0));
		...
		m_pRandomTimeBack->CreateBaseBoxEditBox ( "MODERN_DIALOGUE_WINDOW_RANDOM_TIME_BACK" );
		RegisterControl ( m_pRandomTimeBack );    


		m_pRandomTimeText = new CBasicTextBox;
		m_pRandomTimeText->CreateSub ( this, "MODERN_DIALOGUE_WINDOW_RANDOM_TIME_TEXT", UI_FLAG_DEFAULT, RANDOM_TIME_TEXT );
...
void CModernDialogueWindow::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
...
void CModernDialogueWindow::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case DIALOGUE_ANSWER_TEXTBOX:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				m_pAnswerBox->SetOverColor ( nIndex, NS_UITEXTCOLOR::YELLOW );
				...
				else
				{
					...
					{						
						...
						if ( !(pQuest->m_llGiftEXP || pQuest->m_dwGiftMONEY || pQuest->m_dwGiftContributionPoints ||
						...
						{
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( ID2GAMEWORD("QUEST_GIFT",9), NS_UITEXTCOLOR::GREENYELLOW );
							...
							if ( pQuest->m_llGiftEXP )
							{
								...
								CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strCombine, NS_UITEXTCOLOR::WHITE );
							}
							...
							if ( pQuest->m_dwGiftContributionPoints )
							{
								DWORD dwContributionPoint = pQuest->m_dwGiftContributionPoints;
								strCombine.Format ( "%s : %d", ID2GAMEWORD("QUEST_GIFT_EX",0), dwContributionPoint );
								CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strCombine, NS_UITEXTCOLOR::WHITE );
							}
							...
							if ( pQuest->m_vecGiftITEM.size () )
							{
								...
								CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strItem, NS_UITEXTCOLOR::WHITE );
							}
							...
							if ( pQuest->m_vecGiftSKILL.size () )
							{
								...
								CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strSkill, NS_UITEXTCOLOR::WHITE );
							}
					...
					else if ( pNpcTalk->m_nACTION == SNpcTalk::EM_BASIC && pNpcTalk->m_dwACTION_NO == SNpcTalk::EM_SELF_BUFF_BA_ONLY )
					{
						if ( CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineBegin () )
						{
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( "Get the following buffs:", NS_UITEXTCOLOR::GREENYELLOW );

							SNATIVEID sSelfBuffBAOnly = GLCONST_CHAR::m_sSelfBuffBAOnly.sidSkill;
							PGLSKILL pSKILL = GLSkillMan::GetInstance().GetData ( sSelfBuffBAOnly );
							if ( !pSKILL )	return;
							...
							strLevel.Format("Lv. %d", GLCONST_CHAR::m_sSelfBuffBAOnly.wLevel + 1);
							strSkillName += strLevel;
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strSkillName, NS_UITEXTCOLOR::WHITE );
							
							CString strFee;
							strFee.Format("Gold Fee. %d", GLCONST_CHAR::wSelfBuffPayBA );
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strFee, NS_UITEXTCOLOR::GOLD );

							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineEnd ();
						}
					}
					/*self buff via NPC, DevArt22, 2023/03/18 */
					else if ( pNpcTalk->m_nACTION == SNpcTalk::EM_BASIC && pNpcTalk->m_dwACTION_NO == SNpcTalk::EM_SELF_BUFF_FULL )
					{
						if ( CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineBegin () )
						{
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( "Good for Accuracy/Hitter builds", NS_UITEXTCOLOR::GREENYELLOW );
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( "Get the following buffs:", NS_UITEXTCOLOR::GREENYELLOW );

							for( int i=0; i<MAX_NPC_SELFBUFF; ++i )
							{
								SNATIVEID sSelfBuffFull = GLCONST_CHAR::m_sSelfBuffFull[i].sidSkill;
								PGLSKILL pSKILL = GLSkillMan::GetInstance().GetData ( sSelfBuffFull );
								if ( !pSKILL )	continue;
								...
								strLevel.Format("Lv. %d", GLCONST_CHAR::m_sSelfBuffFull[i].wLevel + 1);
								strSkillName += strLevel;
								CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strSkillName, NS_UITEXTCOLOR::WHITE );
							}
							
							CString strFee;
							strFee.Format("Gold Fee. %d", GLCONST_CHAR::wSelfBuffPayFull );
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strFee, NS_UITEXTCOLOR::GOLD );
					...
					/*self buff via NPC, DevArt22, 2023/03/18 */
					else if ( pNpcTalk->m_nACTION == SNpcTalk::EM_BASIC &&  pNpcTalk->m_dwACTION_NO == SNpcTalk::EM_SELF_BUFF_FULL_NO_BA )
					{
						if ( CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineBegin () )
						{
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( "Good for Evasion/Setter builds", NS_UITEXTCOLOR::GREENYELLOW );
							CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( "Get the following buffs:", NS_UITEXTCOLOR::GREENYELLOW );

							for( int i=0; i<MAX_NPC_SELFBUFF-1; ++i )
							{
								SNATIVEID sSelfBuffFullNoBa = GLCONST_CHAR::m_sSelfBuffFullNoBA[i].sidSkill;
								PGLSKILL pSKILL = GLSkillMan::GetInstance().GetData ( sSelfBuffFullNoBa );
								if ( !pSKILL )	continue;
								...
								strLevel.Format("Lv. %d", GLCONST_CHAR::m_sSelfBuffFullNoBA[i].wLevel + 1);
								strSkillName += strLevel;
								CInnerInterface::GetInstance().InfoDisplayShowCommonMultiLineInsert ( strSkillName, NS_UITEXTCOLOR::WHITE );
							}
...
void CModernDialogueWindow::TranslateTalkMessage ( SNpcTalk* pNpcTalk )
{
...
void CModernDialogueWindow::TranslateBasicMessage ( SNpcTalk* pNpcTalk )
{

	// Need Add New Item Con.
	DWORD dwMsg = pNpcTalk->m_dwACTION_NO;
	DWORD dwGlobalID = pNpcTalk->m_dwGLOB_ID;
	SNPC_ITEM *pParamA = pNpcTalk->m_sNeedItem;
	...
	case SNpcTalk::EM_MARKET:
		{
			...
			CInnerInterface::GetInstance().SetMarketWindowOpen ( pCrow->m_pCrowData->m_sBasic.sNativeID );
			CInnerInterface::GetInstance().ShowGroupFocus ( INVENTORY_WINDOW );
		}
		break;
	...
	case SNpcTalk::EM_NPC_SHOP:
		{
			...
			CInnerInterface::GetInstance().SetNPCShopWindowOpen ( pCrow->m_pCrowData->m_sBasic.sNativeID );
		}
		break;
	...
	case SNpcTalk::EM_ITEM_EXCHANGE:
		{
			...
			CInnerInterface::GetInstance().SetNPCItemExchangeWindowOpen ( pCrow->m_pCrowData->m_sBasic.sNativeID );
		}
		break;

	case SNpcTalk::EM_CURRENCY_SHOP:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			CInnerInterface::GetInstance().HideGroup( CURRENCY_SHOP_WINDOW );
			CInnerInterface::GetInstance().SetDefaultPosInterface( CURRENCY_SHOP_WINDOW );
			CInnerInterface::GetInstance().CurrencyShopOpen ( pCrow->m_pCrowData->m_sBasic.sNativeID );
		}
		break;

		/* chaos machine, Juver, 2021/07/07 */
	case SNpcTalk::EM_CHAOS_MACHINE:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			CInnerInterface::GetInstance().OpenChaosMachineWindow( m_sCrowID.dwID );
		}
		break;

	case SNpcTalk::EM_CONTRIBUTION_SHOP:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			CInnerInterface::GetInstance().HideGroup( CONTRIBUTION_SHOP_WINDOW );
			CInnerInterface::GetInstance().SetDefaultPosInterface( CONTRIBUTION_SHOP_WINDOW );
			CInnerInterface::GetInstance().OpenContributionShopWindow( pCrow->m_pCrowData->m_sBasic.sNativeID );
		}
		break;
		
		/*self buff via NPC, DevArt22, 2023/03/18 */
	case SNpcTalk::EM_SELF_BUFF_BA_ONLY:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			GLGaeaClient::GetInstance().GetCharacter()->ReqSelfBuffBAOnly ( m_sCrowID.dwID, dwGlobalID );
		}
		break;

		/*self buff via NPC, DevArt22, 2023/03/18 */
	case SNpcTalk::EM_SELF_BUFF_FULL:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			GLGaeaClient::GetInstance().GetCharacter()->ReqSelfBuffFull ( m_sCrowID.dwID, dwGlobalID );
		}
		break;

		/*self buff via NPC, DevArt22, 2023/03/18 */
	case SNpcTalk::EM_SELF_BUFF_FULL_NO_BA:
		{
			CInnerInterface::GetInstance().HideGroup ( GetWndID () );
			GLGaeaClient::GetInstance().GetCharacter()->ReqSelfBuffFullNoBA ( m_sCrowID.dwID, dwGlobalID );
		}
		break;
	};
}

void CModernDialogueWindow::TranslateQuestStartMessage ( SNpcTalk* pNpcTalk )
{
	GASSERT(pNpcTalk&&"CModernDialogueWindow::TranslateQuestStartMessage");
...
void CModernDialogueWindow::TranslateQuestStepMessage ( SNpcTalk* pNpcTalk )
{
	GASSERT(pNpcTalk&&"CModernDialogueWindow::TranslateQuestStepMessage");
...
void CModernDialogueWindow::LoadBasicTalk ()
{
	if ( !m_pDialogueCase ) return ;
...
void CModernDialogueWindow::LoadButtonTalk ( bool bRandom )
{
	...
	if ( pNpcTalkControl )
	{
		...
		std::vector<SNpcTalk*>::iterator	pos = vecTalk.begin();
		std::vector<SNpcTalk*>::iterator	pos_end = vecTalk.end();
			...
			case SNpcTalk::EM_BASIC:
				{
					...
					case SNpcTalk::EM_SELF_BUFF_BA_ONLY:	nImageType = ICON_HEAL; break;	/*self buff via NPC, DevArt22, 2023/03/18 */
					case SNpcTalk::EM_SELF_BUFF_FULL:		nImageType = ICON_HEAL; break;	/*self buff via NPC, DevArt22, 2023/03/18 */
					case SNpcTalk::EM_SELF_BUFF_FULL_NO_BA:	nImageType = ICON_HEAL; break;	/*self buff via NPC, DevArt22, 2023/03/18 */
...
void CModernDialogueWindow::LoadNode( DWORD dwNID, bool bRandom, DWORD dwRandomTime )
{
...
BOOL CModernDialogueWindow::SetDialogueData ( STARGETID sCrowID, GLCHARLOGIC_CLIENT *pCHAR )
{
	GASSERT ( pCHAR );
	...
	PCROWDATA pCrowData = GLCrowDataMan::GetInstance().GetCrowData( pCrow->m_pCrowData->m_sBasic.sNativeID );
