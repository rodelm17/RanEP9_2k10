...
BOOL SANIMCONINFO::LoadFile ( const char *szFileName, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVersion >= VERSION_ENCODE )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_ANI_CFG_VER_1 );

	if ( dwVersion >= ENCODE_VER_BYTE_CRYPT_2021_ANI_CFG_V1 )
		SFile.SetEncodeType( EMRCRYPT_BYTE_TYPE_2021_ANI_CFG_V1 );

	switch( dwVersion )
	{
	case SANIMCONINFO::VERSION:	
		LoadFile_0201( SFile, pd3dDevice );	
		break;
...
void SANIMCONINFO::LoadFile_0111_0112_0114_0115( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	std::transform ( strCfgFile.begin(), strCfgFile.end(), strCfgFile.begin(), tolower );
	std::transform ( strXFile.begin(), strXFile.end(), strXFile.begin(), tolower );
	...
	StringCchCopy( m_szName, ACF_SZNAME, strXFile.c_str() );
	StringCchCopy( m_szSkeletion, ACF_SZNAME, strSkeleton.c_str() );
...
void SANIMCONINFO::LoadFile_0201( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwSize(0);
	DWORD dwType(0);
