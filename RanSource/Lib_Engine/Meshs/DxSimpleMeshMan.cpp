...
#include "../DxCommon/MemoryXFile.h"
...
std::string DxSimMesh::GetMeshFilePath( const char* szFileName, BOOL &bEncrypted )
{
	CFileFindTree* pFileTree = DxSimpleMeshMan::GetInstance().GetFileTree();
	if ( !pFileTree )	return "";
	...
	//find encrypted file first
	std::string strFindEnc = strFileName.c_str();
	std::size_t nfound = strFindEnc.rfind( '.' );
	if ( nfound != std::string::npos )
	{
		strFindEnc.erase( nfound );
		strFindEnc.append( ".mxf" );
		std::string* pFilePath = pFileTree->FindPathName ( std::string(strFindEnc.c_str()) );
		if ( pFilePath )
		{
			bEncrypted = TRUE;
			return pFilePath->c_str();
		}
	}

	//encrypted file not found, try to find the original file requested
	std::string* pFilePath = pFileTree->FindPathName ( std::string(strFileName.c_str()) );
	if ( pFilePath )
	{
		//file found, check if encrypted
		std::string strFileFind = pFilePath->c_str();
		if ( strFileFind.rfind(".mxf") != std::string::npos )	bEncrypted = TRUE;

		return pFilePath->c_str();
	}

	////now try looking for file with different extension
	//pFilePath = pFileTree->FindPathNameNoExtension ( std::string(strFileName.c_str()) );
	//if ( pFilePath )
	//{
	//	std::string strFileFind = pFilePath->c_str();
	//	if ( strFileFind.rfind(".mxf") != std::string::npos )	bEncrypted = TRUE;
...
HRESULT DxSimMesh::Create ( LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwFVF, const char* szFilename, const char* szFilename1, const char* szFilename2 )
{
	...
	if ( m_bMorph )
	{
		...
		for ( DWORD i=0; i<3; ++i )
		{
			...
			std::string strFileName = szFILENAME[i];
			std::string strFileFind = GetMeshFilePath( strFileName.c_str(), bEncrypted );

			if ( strFileFind.size() <= 0 )
			{
				CDebugSet::ErrorFile( "DxSimMesh::Create", strFileName.c_str() );
			...
			{
				CMemoryXFile cXFile;
				if ( cXFile.LoadFileDec( strFileFind.c_str() ) )
				{
					BYTE* pData = cXFile.GetData();
					int nSize = cXFile.GetSize();

					SAFE_RELEASE( pAdjacencyBuffer );
					SAFE_RELEASE( pMtrlBuffer );

					if ( FAILED( hr = D3DXLoadMeshFromXInMemory ( pData, nSize, D3DXMESH_MANAGED, pd3dDevice, &pAdjacencyBuffer, &pMtrlBuffer, NULL, &m_dwNumMaterials, &pLOCALMESH[i] ) ) )
					{
						SAFE_RELEASE( pLOCALMESH[i] );
						CDebugSet::ToLogFile ( "ERROR : D3DXLoadMeshFromXInMemory() %s, hr: %s", strFileName.c_str(), DXGetErrorString(hr) );
						return hr;
					}
				}
			}
			else
			{
				...
				if ( FAILED( hr = D3DXLoadMeshFromX ( strFileFind.c_str(), D3DXMESH_MANAGED, pd3dDevice, &pAdjacencyBuffer, &pMtrlBuffer, NULL, &m_dwNumMaterials, &pLOCALMESH[i] ) ) )
				{
					SAFE_RELEASE( pLOCALMESH[i] );
					CDebugSet::ToLogFile ( "ERROR : D3DXLoadMeshFromX() %s, hr: %s", szFILENAME[i], DXGetErrorString(hr) );
					return hr;
				}
			}
								...
								(DWORD*)pAdjacencyBuffer->GetBufferPointer(), NULL, NULL, NULL ) ) )
			{
				...
				CDebugSet::ToLogFile ( "ERROR : OptimizeInplace() %s, hr: %s", szFilename, DXGetErrorString(hr) );
				return hr;
			}
	...
	else
	{
		...
		if ( bEncrypted )
		{
			...
			if ( cXFile.LoadFileDec( strFileFind.c_str() ) )
			{
				...
				if ( FAILED( hr = D3DXLoadMeshFromXInMemory ( pData, nSize, D3DXMESH_MANAGED, pd3dDevice, &pAdjacencyBuffer, &pMtrlBuffer, NULL, &m_dwNumMaterials, &m_pLocalMesh ) ) )
				{
					CDebugSet::ToLogFile ( "ERROR : D3DXLoadMeshFromXInMemory() %s, hr: %s", strFileName.c_str(), DXGetErrorString(hr) );
					return hr;
				}
		...
		{
			if ( FAILED( hr = D3DXLoadMeshFromX ( strFileFind.c_str(), D3DXMESH_MANAGED, pd3dDevice, &pAdjacencyBuffer, &pMtrlBuffer, NULL, &m_dwNumMaterials, &m_pLocalMesh ) ) )
			{
				CDebugSet::ToLogFile ( "ERROR : D3DXLoadMeshFromX() %s, hr: %s", szFilename, DXGetErrorString(hr) );
				return hr;
			}
		}
...
void DxSimpleMeshMan::SetPath( char* szPath )
{
	StringCchCopy( m_szPath, MAX_PATH, szPath );

	m_FileTree.CreateTree ( std::string(m_szPath) );
}
