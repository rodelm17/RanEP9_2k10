...
BEGIN_MESSAGE_MAP(CEditorMobNpcDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_ANI_ATTACK_FIX, &CEditorMobNpcDlg::OnBnClickedButtonAniAttackFix)
END_MESSAGE_MAP()
...
bool CEditorMobNpcDlg::DataLoad()
{
	...
	CFileDialog dlg( TRUE,".mnsf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
...
{
	CString szFilter = "MobNpc Setting File (*.mnsf)|*.mnsf|";
	CFileDialog dlg( FALSE, ".mnsf", GLOGIC::GetPath(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
	...
	for ( int MID = 0; MID < GLCrowDataMan::MAX_CROW_MID; MID++ )
	{
		...
		{        		
			...
			if ( pCROW )
			{
				...
				{
					if ( pCROW->m_sBasic.m_dwHP >= 2 )
						pCROW->m_sBasic.m_dwHP = pCROW->m_sBasic.m_dwHP / 2;
				}
			}
		}
	}
...
void CEditorMobNpcDlg::OnBnClickedButtonAniAttackFix()
{
	for ( int MID = 0; MID < GLCrowDataMan::MAX_CROW_MID; MID++ )
	{
		for ( int SID = 0; SID < GLCrowDataMan::MAX_CROW_SID; SID++ )
		{        		
			...
			if ( pCROW )
			{
				if ( pCROW->m_sBasic.m_emCrow == CROW_MOB )
				{
					DxSkinDataDummy skin_data;
					BOOL load = skin_data.LoadFile( pCROW->GetSkinObjFile() );
					if ( !load )	continue;
					
					for ( int i=0; i<SCROWDATA::EMMAXATTACK; ++i )
					{
						if ( pCROW->m_sCrowAttack[i].bUsed )
						{
							SANIMCONINFO* pinfo = skin_data.FindAniInfo( pCROW->m_sCrowAttack[i].strAniFile.c_str() );
							if ( pinfo )
							{
								pCROW->m_sCrowAttack[i].sAniAttack.Assign( pinfo );
							}
							else
							{
								CDebugSet::ToFileWithTime( "_editor_mob_npc.txt", "[%u/%u]%s invalid ani file :%s attack disabled", MID, SID, pCROW->GetName(),
								pCROW->m_sCrowAttack[i].strAniFile.c_str() );

								pCROW->m_sCrowAttack[i].bUsed = FALSE;
							}
						}
					}
				}
