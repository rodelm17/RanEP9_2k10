...
BOOL DxAABBNode::LoadFile_BETA ( CSerialFile &SFile )
{
	DWORD _dwVER;
	DWORD _dwSIZE;
	...
	if ( _dwVER == 0x0100 )
	{
		...
		{
			pLeftChild = new DxAABBNode();
			pLeftChild->LoadFile_BETA ( SFile );
		}	
		...
		{
			pRightChild = new DxAABBNode();
			pRightChild->LoadFile_BETA ( SFile );
		}
	}
	else
	{
		CDebugSet::ErrorVersion( "DxAABBNode::LoadFile_BETA", SFile.GetFileName(), _dwVER );
