...
char		DxEffCharMark::NAME[MAX_PATH]	= "DxEffCharMark";
...
HRESULT	DxEffCharMark::SettingState ( LPDIRECT3DDEVICEQ pd3dDevice, SKINEFFDATA& sSKINEFFDATA )
{
	...
	if ( g_bPREVIEW_CHAR_RENDER )
	{
		...
		__vTempFromPt2 = vFromPtLocal;

		D3DXVECTOR3	vDirect2 = vLookatPtLocal - vFromPtLocal;
		D3DXVec3Normalize ( &vDirect2, &vDirect2 );

		D3DXVECTOR3 vTempFromPt2 = vFromPtLocal + (vDirect2*2.f);

		D3DXMATRIX matViewLocal;
		D3DXMatrixIdentity ( &matViewLocal );
		D3DXMatrixLookAtLH ( &matViewLocal, &vTempFromPt2, &vLookatPtLocal, &vUpVecLocal );
		pd3dDevice->SetTransform ( D3DTS_VIEW, &matViewLocal );
	}
...
HRESULT	DxEffCharMark::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharMark::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharMark::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharMark::R2LoadFile", SFile.GetFileName(), dwVer );
