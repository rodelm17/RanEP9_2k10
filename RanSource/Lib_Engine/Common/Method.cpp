...
char const* IMethod::sm_szErrorMsg1000 = "Illegal Key Length!";
char const* IMethod::sm_szErrorMsg1001 = "Illegal Key Data!";
...
{
	if(false==m_bInit)
		throw std::runtime_error(std::string(sm_szErrorMsg1));
...
void IMethod::SetMode(int iMode)
{
	...
	if(iMode<ECB || iMode>CFB)
		throw std::runtime_error(std::string(sm_szErrorMsg2));
	m_iMode = iMode;
}
...
void IMethod::SetPadding(int iPadding)
{
	...
	if(iPadding<ZEROES || iPadding>PKCS7)
		throw std::runtime_error(std::string(sm_szErrorMsg3));
	m_iPadding = iPadding;
}
...
void IMethod::HelpThrow(std::string const& rostrFileIn)
{
	//ostrstream ostr;
	...
	throw std::runtime_error(ostrMsg);
}
