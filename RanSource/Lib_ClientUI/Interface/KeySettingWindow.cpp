...
{	
	...
	for( i = 0; i<QUICK_SLOT_NUM; ++i)
	{
		...
		RPROFILE::QuickSlot[i] = GetStringTodwKey( m_QuickSlot[i] );
	...
	for ( i = 0; i<QUICK_SKILL_NUM; ++i)
	{
		...
		RPROFILE::SkillSlot[i] = GetStringTodwKey( m_SkillSlot[i] );
	...
	for ( i = 0; i<MENU_SHOTCUT_NUM; ++i)
	{
		...
		RPROFILE::MenuShortcut[RPROFILE::KeySettingToRanparam[i]] = GetStringTodwKey( m_MenuShotcut[i] );
...
void CKeySettingWindow::GetShotCutKey(){
	...
	// 퀵슬롯 단축키값 얻어오기
	for( i = 0; i< QUICK_SLOT_NUM; ++i){
		m_QuickSlot[i] = GetdwKeyToString(RPROFILE::QuickSlot[i]);
		m_pEditBox[i]->SetEditString ( m_QuickSlot[i] );	
	}

	// 스킬슬롯 단축키값 얻어오기
	for( i = 0; i< QUICK_SKILL_NUM; ++i){
		m_SkillSlot[i] = GetdwKeyToString(RPROFILE::SkillSlot[i]);
	...
	for(i = 0; i< MENU_SHOTCUT_NUM; ++i)
	{
		nIndex = RPROFILE::KeySettingToRanparam[i];
		m_MenuShotcut[i] = GetdwKeyToString(RPROFILE::MenuShortcut[nIndex]);
...
void CKeySettingWindow::SetDefaultKey()
{
	...
	int nIndex = 0;
	for( i = 0; i< QUICK_SLOT_NUM; ++i){
		m_QuickSlot[i] = GetdwKeyToString(RPROFILE::DefaultShortcutKey[i] );
		m_pEditBox[i]->SetEditString ( m_QuickSlot[i] );	
	}

	// 스킬슬롯 단축키값 얻어오기
	for( i = 0; i< QUICK_SKILL_NUM; ++i){
		m_SkillSlot[i] = GetdwKeyToString(RPROFILE::DefaultShortcutKey[i+QUICK_SLOT_NUM]);
		m_pEditBox[i+QUICK_SLOT_NUM]->SetEditString ( m_SkillSlot[i] );	
	}
	...
	for(i = 0; i< MENU_SHOTCUT_NUM; ++i){
		nIndex = RPROFILE::KeySettingToRanparam[i];
		m_MenuShotcut[i] = GetdwKeyToString( RPROFILE::DefaultShortcutKey[nIndex+QUICK_SLOT_NUM+QUICK_SKILL_NUM]);
		m_pEditBox[i+QUICK_SLOT_NUM+QUICK_SKILL_NUM]->SetEditString ( m_MenuShotcut[i] );	
	}
