...
HRESULT GLPVPSchoolWarsField::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case NET_MSG_GCTRL_SCHOOLWARS_A2F_MOB_KILL_NOTICE:
		{
			GLMSG::SNETPC_SCHOOLWARS_A2F_MOB_KILL_UPDATE* pnetmsg = ( GLMSG::SNETPC_SCHOOLWARS_A2F_MOB_KILL_UPDATE* ) nmg;

			GLMSG::SNETPC_SCHOOLWARS_F2C_MOB_KILL_UPDATE netmsgclient;
			netmsgclient.id_crow = pnetmsg->id_crow;
			netmsgclient.id_char = pnetmsg->id_char;
			netmsgclient.score_crow = pnetmsg->score_crow;
			netmsgclient.double_point = pnetmsg->double_point;
			netmsgclient.char_school = pnetmsg->char_school;
			GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( m_sMap.dwID, &netmsgclient );
		}break;
