...
DWORD		DxEffCharCloneBlur::VERSION			= 0x0101;
char		DxEffCharCloneBlur::NAME[MAX_PATH]	= "DxEffCharCloneBlur";
...
HRESULT	DxEffCharCloneBlur::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		EFFCHAR_PROPERTY_CLONEBLUR_0100	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_CLONEBLUR_0100) );

		m_Property.m_bUse = sProp.m_bUse;
		m_Property.m_bBezier = sProp.m_bBezier;
		m_Property.m_fWidth = sProp.m_fWidth;
		m_Property.m_nBlend = sProp.m_nBlend;
		m_Property.m_dwFaces = sProp.m_dwFaces;
		m_Property.m_vColorUP = sProp.m_vColorUP;
		m_Property.m_vColorDOWN = sProp.m_vColorDOWN;

		StringCchCopy( m_Property.m_szTexture, MAX_PATH, sProp.m_szTexture ); 
		StringCchCopy( m_Property.m_szTraceUP, STRACE_NSIZE, sProp.m_szTraceUP);
		StringCchCopy( m_Property.m_szTraceDOWN, STRACE_NSIZE, sProp.m_szTraceDOWN);
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharCloneBlur::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharCloneBlur::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharCloneBlur::R2LoadFile", SFile.GetFileName(), dwVer );
