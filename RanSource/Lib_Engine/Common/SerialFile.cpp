...
CSerialFile::CSerialFile ( FOPENTYPE _Type, const char *_szFileName, EMRCRYPT_BYTE_TYPE emCryptType ) :
	m_FileStream(NULL)
{
	GASSERT(_szFileName);

	OpenFile ( _Type, _szFileName, emCryptType );
}
...
void CSerialFile::SetEncodeType ( EMRCRYPT_BYTE_TYPE emCryptType )
{
...
BOOL CSerialFile::OpenFile ( const FOPENTYPE _Type, const char *_szFileName, EMRCRYPT_BYTE_TYPE emCryptType )
{
	GASSERT(_szFileName);
...
inline size_t CSerialFile::write ( const void* pbuffer, DWORD dwSize )
{
	...
	if ( m_emCrypt_Type != EMRCRYPT_BYTE_TYPE_NONE )
	{
		...
		m_cCrypt.SetType(m_emCrypt_Type);
		m_cCrypt.Encrypt((BYTE*)ptemp, dwSize);
...
BOOL CSerialFile::operator << ( D3DXVECTOR2 Value )
{
	GASSERT(m_OpenType==FOT_WRITE);
	int Num = 0;
...
BOOL CSerialFile::operator << ( D3DXVECTOR3 Value )
{
	GASSERT(m_OpenType==FOT_WRITE);
	int Num = 0;
...
BOOL CSerialFile::operator << ( D3DXVECTOR4 Value )
{
	GASSERT(m_OpenType==FOT_WRITE);
	int Num = 0;
...
BOOL CSerialFile::operator << ( D3DXMATRIX Value )
{
	GASSERT(m_OpenType==FOT_WRITE);
	int Num = 0;
