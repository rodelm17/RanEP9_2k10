...
enum EMCLUB_AUTHORITY_REQ_FB
{
	...
	EMCLUB_AUTHORITY_REQ_FB_CDM_TIME	= 10,	//	cdm is running
};
...
enum EMCLUB_ALLIANCE_REQ_FB
{
	...
	EMCLUB_ALLIANCE_REQ_FB_DISABLED			= 20,	/*club alliance setting, Juver, 2018/06/13 */
};
...
namespace GLMSG
{
	...
	struct SNET_CLUB_MEMBER_STATE
	{
			...
			: dwNumber(0)
		{
			...
			BOOST_STATIC_ASSERT(sizeof(SNET_CLUB_MEMBER_STATE)<=NET_DATA_BUFSIZE);
		}
...
//		enum { RANKING_MAX = 9 };
	...
	struct SNET_CLUB_SCORE_KILL_FLD_2_AGT
	{
		NET_MSG_GENERIC			nmg;
		
		DWORD	dwID;
		DWORD	dwCharIDKilled;
		DWORD	dwCharIDKiller;

		SNET_CLUB_SCORE_KILL_FLD_2_AGT () 
			: dwID(0)
			, dwCharIDKilled(0)
			, dwCharIDKiller(0)
		{
			nmg.dwSize = sizeof(SNET_CLUB_SCORE_KILL_FLD_2_AGT);
			nmg.nType = NET_MSG_GCTRL_CLUB_SCORE_KILL_FLD_2_AGT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNET_CLUB_SCORE_KILL_FLD_2_AGT)<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_CLUB_SCORE_RESU_FLD_2_AGT
	{
		NET_MSG_GENERIC			nmg;
		...
		SNET_CLUB_SCORE_RESU_FLD_2_AGT () 
			: dwID(0)
			, dwCharID(0)
		{
			nmg.dwSize = sizeof(SNET_CLUB_SCORE_RESU_FLD_2_AGT);
			nmg.nType = NET_MSG_GCTRL_CLUB_SCORE_RESU_FLD_2_AGT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNET_CLUB_SCORE_RESU_FLD_2_AGT)<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_CLUB_SCORE_A2C_RANKING_START
	{
		NET_MSG_GENERIC		nmg;
		DWORD		dwGuidanceID;
		char		szGuidanceName[PVP_CLUB_WAR_NAME_SIZE];
		...
		SNETPC_CLUB_SCORE_A2C_RANKING_START ()
			: dwGuidanceID(0)
			, dwOwnerGuildID(0)
			, dwGuildMarkVer(0)
			, fBonusMain(1.0f)
			, fBonusAlly(1.0f)
		{
			memset (szGuidanceName, 0, sizeof(char) * PVP_CLUB_WAR_NAME_SIZE);		
			memset (szOwnerGuildName, 0, sizeof(char) * PVP_CLUB_WAR_PLAYER_DATA_GUILD_NAME_SIZE);	

			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_CLUB_SCORE_RANKING_START_A2C;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNETPC_CLUB_SCORE_A2C_RANKING_START)<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_CLUB_SCORE_A2C_RANKING_UPDATE
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwGuidanceID;
		WORD				wRankNum;
		SPVP_CLUB_WAR_PLAYER_DATA	sPlayerData[PVP_CLUB_WAR_PACKET_RANK_NUM];

		SNETPC_CLUB_SCORE_A2C_RANKING_UPDATE () 
			: dwGuidanceID(0)
			, wRankNum(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_CLUB_SCORE_RANKING_UPDATE_A2C;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNETPC_CLUB_SCORE_A2C_RANKING_UPDATE)<=NET_DATA_BUFSIZE);
		}

		BOOL ADDRANK ( const SPVP_CLUB_WAR_PLAYER_DATA& sRANK )
		{
			if ( wRankNum == PVP_CLUB_WAR_PACKET_RANK_NUM )	return FALSE;

			sPlayerData[wRankNum] = sRANK;
			wRankNum++;

			nmg.dwSize = sizeof(*this) - sizeof(SPVP_CLUB_WAR_PLAYER_DATA) * ( PVP_CLUB_WAR_PACKET_RANK_NUM - wRankNum );
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
			return TRUE;
		}
		...
		{
			wRankNum = 0;
			for( int i=0; i<PVP_CLUB_WAR_PACKET_RANK_NUM; ++i )
			{
				sPlayerData[i] = SPVP_CLUB_WAR_PLAYER_DATA();
			}
		}
	};

	struct SNETPC_CLUB_SCORE_F2C_RANKING_END
	{
		NET_MSG_GENERIC		nmg;
		DWORD		dwGuidanceID;
		...
		SNETPC_CLUB_SCORE_F2C_RANKING_END ()
			: dwGuidanceID(0)
			, wPlayerNum(0)
			
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_CLUB_SCORE_RANKING_END_A2C;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNETPC_CLUB_SCORE_F2C_RANKING_END)<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_CLUB_SCORE_A2F_SEND_SCORE
	{
		NET_MSG_GENERIC			nmg;
		DWORD	dwCharID;
		WORD	wRewardPoint;

		SNETPC_CLUB_SCORE_A2F_SEND_SCORE () 
			: dwCharID(0)
			, wRewardPoint(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_CLUB_SCORE_SEND_SCORE_A2F;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			BOOST_STATIC_ASSERT(sizeof(SNETPC_CLUB_SCORE_A2F_SEND_SCORE)<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_REQ_BATTLE_RANKING
	{
		NET_MSG_GENERIC	nmg;
		int				nPage;

		SNET_REQ_BATTLE_RANKING()
			: nPage(0)
		{
			nmg.dwSize	= sizeof(SNET_REQ_BATTLE_RANKING);
			nmg.nType	= NET_MSG_GCTRL_REQ_BATTLE_RANKING;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};

	struct SNET_REQ_MY_BATTLE_RANKING
	{
		NET_MSG_GENERIC	nmg;
		int				nPage;

		SNET_REQ_MY_BATTLE_RANKING()
			: nPage(0)
		{
			nmg.dwSize	= sizeof(SNET_REQ_MY_BATTLE_RANKING);
			nmg.nType	= NET_MSG_GCTRL_REQ_MY_BATTLE_RANKING;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};

	struct SNET_REQ_BATTLE_RANKING_FB
	{
		NET_MSG_GENERIC	nmg;

		DWORD				dwID;
		SBATTLE_RANKING		sRanking_[10];

		SNET_REQ_BATTLE_RANKING_FB()
			: dwID(0)
		{
			nmg.dwSize	= sizeof(SNET_REQ_BATTLE_RANKING_FB);
			nmg.nType	= NET_MSG_GCTRL_REQ_BATTLE_RANKING_FB;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	
	struct SNET_REQ_MY_BATTLE_RANKING_FB
	{
		NET_MSG_GENERIC	nmg;
		SBATTLE_RANKING		sRanking;

		SNET_REQ_MY_BATTLE_RANKING_FB()
		{
			nmg.dwSize	= sizeof(SNET_REQ_MY_BATTLE_RANKING_FB);
			nmg.nType	= NET_MSG_GCTRL_REQ_MY_BATTLE_RANKING_FB;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};

	struct SNET_REQ_CLUB_BATTLE_RANKING_FB
	{
		NET_MSG_GENERIC	nmg;
		...
		SNET_REQ_CLUB_BATTLE_RANKING_FB()
			: dwID(0)
		{
			nmg.dwSize	= sizeof(SNET_REQ_CLUB_BATTLE_RANKING_FB);
			nmg.nType	= NET_MSG_GCTRL_REQ_CLUB_BATTLE_RANKING_FB;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};

	struct SNET_REQ_MY_CLUB_BATTLE_RANKING_FB
	{
		NET_MSG_GENERIC		nmg;
		SBATTLE_RANKING		sRanking;

		SNET_REQ_MY_CLUB_BATTLE_RANKING_FB()
		{
			nmg.dwSize	= sizeof(SNET_REQ_MY_CLUB_BATTLE_RANKING_FB);
			nmg.nType	= NET_MSG_GCTRL_REQ_MY_CLUB_BATTLE_RANKING_FB;
			GASSERT(nmg.dwSize <= NET_DATA_BUFSIZE);
		}
	};
	...
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_CLUB_RANK_POINT_UPDATE
	{
		NET_MSG_GENERIC			nmg;
		...
		SNET_CLUBWAR_CLUB_RANK_POINT_UPDATE () 
			: bKillPoint(false)		
			, bResuPoint(false)
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_CLUB_RANK_POINT_UPDATE);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_CLUB_RANK_POINT_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}        
	};
	
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_CLUB_MYRANK_UPDATE
	{
		NET_MSG_GENERIC			nmg;

		SCLUBWAR_CLUB_RANK_EX	sMyClubWarClubRank;		

		SNET_CLUBWAR_CLUB_MYRANK_UPDATE () 
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_CLUB_MYRANK_UPDATE);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_CLUB_MYRANK_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}        
	};
	
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_CLUB_RANKING_UPDATE
	{
		NET_MSG_GENERIC		nmg;

		WORD				wRankNum;		
		SCLUBWAR_CLUB_RANK	sClubWarClubRank[CLUBWAR_CLUB_RANKING_NUM];

		SNET_CLUBWAR_CLUB_RANKING_UPDATE () 
			: wRankNum(0)
		{
			nmg.dwSize = sizeof(NET_MSG_GENERIC) + sizeof(WORD);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_CLUB_RANKING_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}

		bool ADDCLUB ( const SCLUBWAR_CLUB_RANK& sCLUBWARRank )
		{
			if ( CLUBWAR_CLUB_RANKING_NUM==wRankNum )		return false;

			sClubWarClubRank[wRankNum] = sCLUBWARRank;

			++wRankNum;

			nmg.dwSize = sizeof(NET_MSG_GENERIC) + sizeof(WORD) + sizeof(SCLUBWAR_CLUB_RANK)*wRankNum;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
			return true;
		}
	...
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_CLUB_RANKING_REQ
	{
		NET_MSG_GENERIC		nmg;    

		DWORD				dwMapID;		

		SNET_CLUBWAR_CLUB_RANKING_REQ () 
			: dwMapID(UINT_MAX)		
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_CLUB_RANKING_REQ);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_CLUB_RANKING_REQ;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}       
	...
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_RANK_POINT_UPDATE
	{
		NET_MSG_GENERIC			nmg;
		...
		SNET_CLUBWAR_RANK_POINT_UPDATE () 
			: bKillPoint(false)		
			, bResuPoint(false)
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_RANK_POINT_UPDATE);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_RANK_POINT_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}        
	};
	
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_MYRANK_UPDATE
	{
		NET_MSG_GENERIC			nmg;

		SCLUBWAR_RANK_EX		sMyClubWarRank;		

		SNET_CLUBWAR_MYRANK_UPDATE () 
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_MYRANK_UPDATE);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_MYRANK_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}        
	};
	
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_RANKING_UPDATE
	{
		NET_MSG_GENERIC		nmg;

		WORD				wRankNum;		
		SCLUBWAR_RANK		sClubWarRank[CLUBWAR_RANKING_NUM];

		SNET_CLUBWAR_RANKING_UPDATE () 
			: wRankNum(0)
		{
			nmg.dwSize = sizeof(NET_MSG_GENERIC) + sizeof(WORD);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_RANKING_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}

		bool ADDRANK ( const SCLUBWAR_RANK& sClubWarRank_ )
		{
			if ( CLUBWAR_RANKING_NUM==wRankNum )		return false;

			sClubWarRank[wRankNum] = sClubWarRank_;

			++wRankNum;

			nmg.dwSize = sizeof(NET_MSG_GENERIC) + sizeof(WORD) + sizeof(SCLUBWAR_RANK)*wRankNum;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
			return true;
		}
	...
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_RANKING_REQ
	{
		NET_MSG_GENERIC		nmg;    

		DWORD				dwMapID;		

		SNET_CLUBWAR_RANKING_REQ () 
			: dwMapID(UINT_MAX)		
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_RANKING_REQ);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_RANKING_REQ;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}       
	...
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_MYRANK_RESU_UPDATE
	{
		NET_MSG_GENERIC			nmg;

		SCLUBWAR_RANK_RESU_EX	sMyClubWarRankResu;		

		SNET_CLUBWAR_MYRANK_RESU_UPDATE () 
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_MYRANK_RESU_UPDATE);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_MYRANK_RESU_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}        
	};
		
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_RANKING_RESU_UPDATE
	{
		NET_MSG_GENERIC		nmg;

		WORD				wRankNum;		
		SCLUBWAR_RANK_RESU	sClubWarRankResu[CLUBWAR_RANKING_NUM];

		SNET_CLUBWAR_RANKING_RESU_UPDATE () 
			: wRankNum(0)
		{
			nmg.dwSize = sizeof(NET_MSG_GENERIC) + sizeof(WORD);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_RANKING_RESU_UPDATE;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}

		bool ADDRANK ( const SCLUBWAR_RANK_RESU& sClubWarRankResu_ )
		{
			if ( CLUBWAR_RANKING_NUM==wRankNum )		return false;

			sClubWarRankResu[wRankNum] = sClubWarRankResu_;

			++wRankNum;

			nmg.dwSize = sizeof(NET_MSG_GENERIC) + sizeof(WORD) + sizeof(SCLUBWAR_RANK_RESU)*wRankNum;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
			return true;
		}
	...
	/* Tyranny/Clubwar Mini Ranking, Montage 3-22-25 */
	struct SNET_CLUBWAR_RANKING_RESU_REQ
	{
		NET_MSG_GENERIC		nmg;    

		DWORD				dwMapID;		

		SNET_CLUBWAR_RANKING_RESU_REQ () 
			: dwMapID(UINT_MAX)		
		{
			nmg.dwSize = sizeof(SNET_CLUBWAR_RANKING_RESU_REQ);
			nmg.nType = NET_MSG_GCTRL_CLUBWAR_RANKING_RESU_REQ;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);			
		}       
