...
HRESULT GLAgentServer::Create ( DWORD dwMaxClient, DxMsgServer *pMsgServer, DxConsoleMsg *pConsoleMsg, GLDBMan *pDBMan, char* szMapList, F_SERVER_INFO* pFieldInfo )
{
	...
	/*woe Arc Development 08-06-2024*/
	GLPVPWoeAgent::GetInstance().SetMapState();

	/* pvp club death match, Juver, 2020/11/26 */
	PVPClubDeathMatchAgent::GetInstance().SetMapState();

	PVPPBG::ManagerAgent::GetInstance().SetMapState();
	...
	/*Top Resu, Mhundz */
	m_vecTopResu.clear();
	m_pDBMan->GetTopResu( m_vecTopResu );
	/*Top MMR, Mhundz */
	m_vecTopMMR.clear();
	m_pDBMan->GetTopMMR( m_vecTopMMR );
	/////////////////////////////////////////////////////////////////////////////
...
HRESULT GLAgentServer::DropOutPC ( DWORD dwGaeaID, bool bForce )
{
	...
	if ( pPChar->m_sPMarket.IsPremiumMarket() && !bForce )
	{
...
BOOL GLAgentServer::ClearReservedDropOutPC ()
{
	EnterCriticalSection(&m_CSPCLock);
	{
		...
		for ( ; iter!=iter_end; ++iter )
		{
			...
			/* pvp club death match, Juver, 2020/11/26 */
			PVPClubDeathMatchAgent::GetInstance().RequestPlayerCancelRegister( pChar->m_dwCharID, pChar->m_dwGuild );

			PVPPBG::ManagerAgent::GetInstance().RequestPlayerCancelRegister(pChar->m_dwCharID);
			...
			/*woe Arc Development 08-06-2024*/
			GLPVPWoeAgent::GetInstance().RequestPlayerCancelRegister(pChar->m_dwCharID);
		}
...
HRESULT GLAgentServer::FrameMove ( float fTime, float fElapsedTime )
{
	...
	KickOutUpdate( fTime, fElapsedTime );


	/* play time system, Juver, 2021/01/26 */
	m_fPlayTimeUpdateTimer += fElapsedTime;
	if ( m_fPlayTimeUpdateTimer >= 60.0f )
	{
		UpdatePlayTime();
		m_fPlayTimeUpdateTimer = 0.0f;
	}
	...
	m_cGameNotice.FrameMove( fTime, fElapsedTime );

#if defined( BUILD_CH ) || defined( BUILD_CHY )
	m_cWTF.FrameMove( fElapsedTime );
#endif
