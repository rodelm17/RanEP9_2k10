...
IMPLEMENT_DYNAMIC(CPieceTab, CPropertySheet)

CPieceTab::CPieceTab(CRect& rect, LOGFONT logfont, CWnd* pParentWnd)
	: CPropertySheet("SheetPiece", pParentWnd),
	m_bNoTabs(FALSE),
	m_pWndParent(NULL),
	m_iDistanceFromTop(0),
	m_pData(NULL),
	
	m_PagePIEMain(logfont),
	...
	m_pFont ( NULL )
{
	...
	m_PagePIEMain.SetSheetTab(this);
	
	AddPage(&m_PagePIEMain);
...
BEGIN_MESSAGE_MAP(CPieceTab, CPropertySheet)
	//{{AFX_MSG_MAP(CPieceTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

HBRUSH CPieceTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL CPieceTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void CPieceTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
...
void CPieceTab::ResetData()
{
	m_PagePIEMain.ResetData();

	m_pData = NULL;
	PageActive(EMPIECE_PAGE_MAIN);
}

void CPieceTab::SetData( DxPieceEdit* pData )
{
	m_pData = pData;

	m_PagePIEMain.SetData( m_pData );

	PageActive(EMPIECE_PAGE_MAIN);
}

void CPieceTab::PageActive( int nPAGE )
{
	if( nPAGE < 0 )	return;
	...
	switch( nPAGE )
	{
	case EMPIECE_PAGE_MAIN:
		{
			SetActivePage ( EMPIECE_PAGE_MAIN );
			m_PagePIEMain.SetData( m_pData );
		}break;
