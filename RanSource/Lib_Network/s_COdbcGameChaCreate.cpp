...
int COdbcManager::CreateNewCharacter(SCHARDATA2* pCharData2)
{
	...
	_snprintf( szTemp, 1024, "INSERT INTO ChaInfo(UserNum, SGNum, ChaName, ChaTribe, ChaClass, "
		...
		"ChaActivity, "
		"ChaDailyLogin, ChaCodex, ChaBattlePass, "
		"ChaResu, ChaMMR) Values ( "
		"%u,%u,'%s',%d,%d,%u,%u,%u,%d,"
...
//		Print(GetErrorString(pConn2->hStmt));
	...
	*/
	...
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "UPDATE ChaInfo SET ChaInfo.ChaDailyLogin=? WHERE (ChaNum=%d)", nChaNewNum );
		m_pGameDB->WriteImage( szTemp, nChaNewNum, (BYTE *) pBuffer, dwSize );
		pBuffer = NULL;
	}
	...
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "UPDATE ChaInfo SET ChaInfo.ChaBattlePass=? WHERE (ChaNum=%d)", nChaNewNum );
		m_pGameDB->WriteImage( szTemp, nChaNewNum, (BYTE *) pBuffer, dwSize );
		pBuffer = NULL;
	}
