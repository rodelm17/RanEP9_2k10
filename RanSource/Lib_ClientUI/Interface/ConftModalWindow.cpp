...
void	CConftModalWindow::LoadUsableHP ()
{
	...
	{		
		...
		for ( int i = 0; i < GLCONST_CHAR::EMCONFT_RC_TYPENUM; ++i )
		{
			if (RPROFILE::wCONFT_RECOVER == GLCONST_CHAR::wCONFT_RC_TYPE[i] )
			{
				bOK = true; 
				wConftRecover = RPROFILE::wCONFT_RECOVER;
				break;
			}
		...
		if ( !bOK )
		{
			RPROFILE::wCONFT_RECOVER = GLCONST_CHAR::wCONFT_RC_TYPE[0];
			wConftRecover = RPROFILE::wCONFT_RECOVER;
		}
...
void	CConftModalWindow::LoadHPStateMy ()
{
	...
	{		
		...
		for ( int i = 0; i < GLCONST_CHAR::EMCONFT_STATE_TYPENUM; ++ i ) 
		{
			if (RPROFILE::fCONFT_HP_RATE == GLCONST_CHAR::fCONFT_STATE_TYPE[i] )
			{
				bOK = true;
				break;
			}
		...
		if ( !bOK ) RPROFILE::fCONFT_HP_RATE = GLCONST_CHAR::fCONFT_STATE_TYPE[0];
		
		strTemp.Format ( "%1.1f", RPROFILE::fCONFT_HP_RATE );
...
void	CConftModalWindow::LoadHPStateHis ()
{
	...
	{	
		...
		for ( int i = 0; i < GLCONST_CHAR::EMCONFT_STATE_TYPENUM; ++ i ) 
		{
			if (RPROFILE::fCONFT_TAR_HP_RATE == GLCONST_CHAR::fCONFT_STATE_TYPE[i] )
			{
				bOK = true;
				break;
			}
		...
		if ( !bOK ) RPROFILE::fCONFT_TAR_HP_RATE = GLCONST_CHAR::fCONFT_STATE_TYPE[0];
		
		strTemp.Format ( "%1.1f", RPROFILE::fCONFT_TAR_HP_RATE );
	}
	else
	{		
		SCONFT_OPTION* pConftOption = CInnerInterface::GetInstance().GetConftOption ();
...
{	
	if ( m_bReqModal )
	{
		...
		//		대련 영역 무조건 ON
		//		의도된 코드입니다.
		RPROFILE::bCONFT_BOUND = TRUE;
		m_pRangeLimitButton->SetFlip (RPROFILE::bCONFT_BOUND );
	}
	else
	{
		SCONFT_OPTION* pConftOption = CInnerInterface::GetInstance().GetConftOption ();
