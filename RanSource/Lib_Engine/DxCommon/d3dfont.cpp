...
//-----------------------------------------------------------------------------
CD3DFont::CD3DFont( const TCHAR* strFontName, DWORD dwHeight, DWORD dwBufferMode, DWORD dwFlags, language::LANGFLAG emLang ) :
...
HRESULT CD3DFont::GetTextExtent( const TCHAR* strText, const DWORD dwFlag, SIZE &Size )
{
	if( NULL==strText )
		return E_FAIL;
	...
	while ( *strText )
	{
		...
		{
			if ( dwFlag&D3DFONTCUSTOM_MULTILINE )
			{
				fHeight += fRowHeight;
				fRowWidth = 0.0f;
			}
...
int CD3DFont::GetTextCountByWidth ( const TCHAR* strText, const float fWidth, const float fRangePer )
{
	...
	while ( *strText )
	{
		...
		const STEXUV &sTEXUV	= (*iter).second;
		float		 fCharWidth = static_cast<float>(sTEXUV.nX2-sTEXUV.nX1);
...
HRESULT CD3DFont::DrawText ( FLOAT sx, FLOAT sy, DWORD dwColor, const TCHAR* strText, DWORD dwFlags, BOOL bImmediately )
{
	...
	while( *strText )
	{
		...
		{
			if ( dwFlags&D3DFONTCUSTOM_MULTILINE )
			{
				sx = fStartX;
				sy += m_fFontHeight;
			}
		...
		GASSERT( dwTEXINDEX + 5 < _MAXVERTEX );
