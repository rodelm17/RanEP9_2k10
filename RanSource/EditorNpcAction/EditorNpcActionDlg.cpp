...
#include "../Lib_Engine/Common/charset.h"
...
BOOL CEditorNpcActionDlg::OnInitDialog()
{
	...
	//l0wgic--
	//problem -- code page was not set, DxResponseman::OneTimeSceneInit is not called for this tool
	//solution -- set code page here
	//what could go wrong

	CHARSET::SetCodePage ( CD3DFontPar::nCodePage[RPARAM::emLangSet] );
...
#ifndef _DEBUG
	SetWin_ShowWindow( this, IDC_BUTTON_NTK_DECOMPILE_DAT, FALSE );
#endif
...
void CEditorNpcActionDlg::DecompileDat()
{
	...
	CFileDialog dlg( TRUE, ".dat", CNpcDialogueSet::GetPath(), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter, this );
	...
	if ( dlg.DoModal() == IDOK )
	{
		...
		for( WORD i=0; i<_wNum; ++i )
		{
			...
			case 0x17:
				{
					BOOL bOK = mTalk.LOAD_0017( SFile );
					if ( bOK )
					{
						CString strSavePath;
						strSavePath.Format ( "%s%s", CNpcDialogueSet::GetPath(), mTalk.m_strFileName.GetString() );
