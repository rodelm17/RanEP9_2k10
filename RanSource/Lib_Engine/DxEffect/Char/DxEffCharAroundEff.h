...
struct EFFCHAR_PROPERTY_CHAR_AROUND : public EFFCHAR_PROPERTY
{
	...
	EFFCHAR_PROPERTY_CHAR_AROUND() :
		...
		m_vMinSize(1.0f, 1.0f),
		m_vMaxSize(1.0f, 1.0f),
	...
	{
		SecureZeroMemory ( m_szTexture, AROUNDEFFECT_TEXNAME_SIZE );
	}
};
...
class DxEffCharAroundEff : public DxEffChar
{
public:
	static DWORD		TYPEID;
...
private:
	...
	std::vector<SAROUNDEFF_DATA> m_vecAroundEffList;
...
public:
	...
	std::vector<SAROUNDEFF_DATA> GetAroundEffect() { return m_vecAroundEffList; }
	void RenderCountText( DWORD nCount, const D3DXVECTOR3& vPos );


public:
	DxEffCharAroundEff(void);
	~DxEffCharAroundEff(void);
};
