...
BOOL GLCrow::IsVisibleDetect ( const bool bSkillShowInvisible, const WORD wSkillShowInvisibleLevel, const WORD wSkillShowInvisibleRange, const D3DXVECTOR3& vPos )
{
...
HRESULT GLCrow::ESCAPE ( STARGETID &sTARID, const DWORD dwDamage )
{
	...
	BOOL bESCAPE = ( EMCROWACT_UP_ESCAPE == emCROWACT ) && ( dwDamage > DWORD ( m_pCrowData->m_sBasic.m_dwHP * 0.10f ) );
	...
	case GLAT_SKILL:
		{
			...
			if ( m_pAttackProp )
			{
				...
				if ( !pSkill )								break;
				if ( wLevel >= SKILL::MAX_SKILL_LEVEL )			break;
	...
	case GLAT_IDLE:
		{
			...
			NetMsg.emCrow	= m_pCrowData->m_sBasic.m_emCrow;
	...
	if ( !m_sHITARRAY.empty() )
	{
		...
		if ( wThisKey >= sStrike.wDivKey )
		{
			if ( sStrike.bHit )
			{
				...
				ProcessSkillStigmaTypeAttack( sAniAttack.m_wDivCount );
			}
			else					AvoidProc ();
...
NET_MSG_GENERIC* GLCrow::ReqNetMsg_Drop ()
{
	...
	dropCrow.m_dwTyrannyClubOwner = m_dwTyrannyClubOwner;
	dropCrow.m_dwTyrannyClubMarkVer = m_dwTyrannyClubMarkVer;
	...
	/*woe Arc Development 08-06-2024*/
	dropCrow.m_wWoeOwner = m_wWoeOwner;
...
HRESULT GLCrow::UpdateViewAround ()
{
	...
	//optimize memory
	common_vector_free( m_vecPC_VIEW );
...
HRESULT GLCrow::UpateAnimation ( float fTime, float fElapsedTime )
{
	...
	{
	case GLAT_MOVE:
		fSkinAniElap *= GLCROWLOGIC_SERVER::GETMOVEVELO();
		break;
...
float GLCrow::GetMoveVelo ()
{
	...
	return fDefaultVelo * GLCROWLOGIC_SERVER::GETMOVEVELO();
}
...
HRESULT GLCrow::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( IsValidBody() )
	{
		...
		/*dash skill logic, Juver, 2017/06/17 */
		/* skill position shift, Juver, 2020/12/15 */
		else if ( !IsACTION(GLAT_SHOCK) && !IsACTION(GLAT_PUSHPULL) && !IsACTION(GLAT_SKILL_PUSH_PULL) && !IsACTION(GLAT_SKILL_DASH) && !m_bSkillPositionShift )
		{
			...
			GLCROWLOGIC_SERVER::UPDATE_DATA ( fTime, fElapsedTime );
...
void GLCrow::ReceivePushPull ( const D3DXVECTOR3 &vMovePos, float fSpeed, bool bSkillMove, bool bReverseDir, bool bSkillDash, WORD wActionAnim )
{
	...
	if ( m_pCrowData && ( m_pCrowData->IsPosHold() || m_pCrowData->IsBlockPushPull() ) )
		return;
...
/* push skill logic, Juver, 2020/12/14 */
void GLCrow::ReceiveSkillPush ( const D3DXVECTOR3 &vMovePos, const float& fSpeed )
{
	if ( !IsValidBody () )		return;
...
/* pull skill logic, Juver, 2020/12/14 */
void GLCrow::ReceiveSkillPull ( const D3DXVECTOR3 &vMovePos, const float& fSpeed )
{
	if ( !IsValidBody () )		return;
...
void GLCrow::ReceiveSkillDash ( const D3DXVECTOR3 &vMovePos, const float& fSpeed, const DWORD& dwActionAnim )
{
	if ( !IsValidBody () )		return;
...
void GLCrow::ReceiveSkillTeleport( const D3DXVECTOR3& vtarget_pos )
{
	if ( !IsValidBody () )		return;
...
/* skill position shift, Juver, 2020/12/15 */
void GLCrow::ReceivePositionShift ( const D3DXVECTOR3 &vMovePos, const float& fSpeed )
{
	if ( !IsValidBody () )		return;
...
DWORD GLCrow::ReceiveDamage ( const EMCROW emACrow, const DWORD dwAID, const DWORD dwDamage, const BOOL bShock )
{
	...
	{
		//	Note : Damage 적용.
		DWORD dwDxHp = GLCROWLOGIC_SERVER::RECEIVE_DAMAGE ( (WORD)dwDamage );
...
/*woe Arc Development 08-06-2024*/
void GLCrow::WoeResetTower()
{
	if (!m_pCrowData)	return;
	if (m_pCrowData->m_emNPCType != EMNPC_TYPE_WOE)	return;
...
{
	if ( !m_pCrowData )	return;
	if ( m_pCrowData->m_sBasic.m_emNPCType != EMNPC_TYPE_EVENT_TOWER )	return;
	...
	netmsgowner.dwTyrannyClubOwner = m_dwTyrannyClubOwner;
	netmsgowner.dwTyrannyClubMarkVer = m_dwTyrannyClubMarkVer;
...
void GLCrow::TyrannyDamageCheck()
{
	...
	DWORD dwClubMarkVer = 0;
	if( pChar )	
	{
		if( pChar->m_dwGuild != CLUB_NULL )
		{
			GLCLUB *pGuild = GLGaeaServer::GetInstance().GetClubMan().GetClub( pChar->m_dwGuild );
			if( pGuild )						
			{
				dwClubID = pChar->m_dwGuild;
				dwClubMarkVer = pGuild->m_dwMarkVER;
			}
		}
	}
	...
	for ( int i=0; i<TYRANNY_SCHOOL_SIZE; ++i )
	{
		...
		if ( m_dwTyrannyDamage[i] >= dwTyrannyDamageMax && dwClubID != m_dwTyrannyClubOwner && dwClubID != CLUB_NULL )
			ncapture = i;
	}
	...
	if ( ncapture != TYRANNY_SCHOOL_SIZE )
	{
		...
		for ( int i=0; i<TYRANNY_SCHOOL_SIZE; ++i )
		{
			/*disable set hp to full when capture for gvg*/
			//if ( ncapture == i )
		...
		m_dwTyrannyClubOwner = dwClubID;
		m_dwTyrannyClubMarkVer = dwClubMarkVer;
		...
		if ( m_pCrowData )
		{
			GLPVPTyrannyField::GetInstance().TowerCapture( m_pCrowData->m_sBasic.sNativeID, (WORD)ncapture, dwClubID, dwClubMarkVer );
		}

		if( m_sAssault.dwID != GAEAID_NULL && m_sAssault.emCrow == CROW_PC )
		{
			SNATIVEID sidCrow = NATIVEID_NULL();
			int nIndex = 0;
			for( int i=0; i<TYRANNY_TOWER_SIZE; ++i )
			{
				sidCrow = GLPVPTyrannyField::GetInstance().m_sTowerData[i].sidCrow;
				if ( m_pCrowData->m_sBasic.sNativeID == sidCrow )	
				{
					nIndex = i;
					break;
				}
			...
			GLMSG::SNETPC_TYRANNY_F2A_GUILD_TOWER_CAPTURE netmsgagent;
			netmsgagent.m_dwGuiID = dwClubID;
			netmsgagent.m_dwMarkVer = dwClubMarkVer;
			netmsgagent.m_nIndex = nIndex;
...
void GLCrow::TyrannyDamageUpdate()
{
	TYRANNY_STATE emstate = GLPVPTyrannyField::GetInstance().m_emState;
	if ( emstate != TYRANNY_STATE_BATTLE )	return;
...
void GLCrow::LinkSkillStigmaTarget ( const EMCROW _emACrow, const DWORD _dwAID, const SNATIVEID& _sSkillID, const float& fDistance, const float& fDamage, const DWORD& dwAttackFlag, const SNATIVEID& sMapID )
{
...
/* skill stigma, Juver, 2020/12/13 */
BOOL GLCrow::CheckSkillStigmaExist( const EMCROW _emACrow, const DWORD _dwAID, const SNATIVEID& _sSkillID ) const
{
	BOOL bFOUND = FALSE;
...
void GLCrow::ReceiveSkillBuffDurationChange( const SNATIVEID& _sSkillID, const DWORD& _dwFlags, const float& fSeconds )
{
...
void GLCrow::ReactDamage( GLACTOR* pAttacker, DWORD dwDamageFlags )
{
	/* skill illusion, Juver, 2021/01/17 */
	if( dwDamageFlags & DAMAGE_TYPE_ILLUSION )
	{
		...
		if ( m_pLandMan )
		{
			STARGETID dwActorID = STARGETID(CROW_MOB,m_dwGlobID);
...
{
	if ( !m_pCrowData )	return;
	if ( m_pCrowData->m_sBasic.m_emCTFType != EMNPC_CTF_FLAG_TYPE_A && 
		m_pCrowData->m_sBasic.m_emCTFType != EMNPC_CTF_FLAG_TYPE_B )	return;
	...
	for ( ; it1_b != it1_e; )
	{
		...
		for ( ; it2_b != it2_e; )
		{
			...
			if ( pACTOR )
			{
				...
				BOOL bExist = pACTOR->CheckSkillStigmaExist( CROW_MOB, m_dwGlobID, sData.sSkillID );
				if ( !bExist )
					bDeleteTarget = TRUE;
			}
			else
