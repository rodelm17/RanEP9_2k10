...
CLargeMapWindowInfo::CLargeMapWindowInfo()
	: m_nMiniPosX(0)
	, m_nMiniPosY(0)
...
CLargeMapWindowInfo::~CLargeMapWindowInfo(void)
{
}

void CLargeMapWindowInfo::UpdatePostion ()
{
	CString strText;
...
void CLargeMapWindowInfo::SetMapPos ( float fCurX, float fCurY, int nMiniX, int nMiniY )
{
...
void CLargeMapWindowInfo::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	case MINIMAP_CLUBMARK:
		{
			...
			{				
				CInnerInterface::GetInstance().InfoDisplayShowCommonSingleLine ( m_strClubName, NS_UITEXTCOLOR::DEFAULT );
			}
		}
		break;
	}
...
void CLargeMapWindowInfo::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{	
...
HRESULT CLargeMapWindowInfo::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT( pd3dDevice );
...
void CLargeMapWindowInfo::CreateSubControl ()
{
	...
	CBasicTextBox* pMiniMapCoordinateBox = new CBasicTextBox;
	pMiniMapCoordinateBox->CreateSub(this, "MINIMAP_COORDINATE_POSITION");
	pMiniMapCoordinateBox->SetFont(pFont);
	pMiniMapCoordinateBox->SetTextAlign(TEXT_ALIGN_RIGHT);
	RegisterControl(pMiniMapCoordinateBox);
	m_pMinimapCoordinateBox = pMiniMapCoordinateBox;
}