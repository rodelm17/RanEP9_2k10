...
namespace ITEM
{
	...
	void SSUIT::Assign ( SSUIT_112 &sSuitOld )
	{
		...
		for ( int i=0; i<SSUIT_112::ADDON_SIZE; ++i )		
			sADDON[i] = sSuitOld.sADDON[i];

		bItemColor = sSuitOld.bItemColor;
		dummy_var_1 = sSuitOld.dummy_var_1;
		dummy_var_2 = sSuitOld.dummy_var_2;
	}
	...
	void SSUIT::Assign ( SSUIT_113 &sSuitOld )
	{
		...
		for ( int i=0; i<SSUIT_113::ADDON_SIZE; ++i )		
			sADDON[i] = sSuitOld.sADDON[i];
	...
	void SSUIT::Assign ( SSUIT_200 &sSuitOld )
	{
		emSuit		= sSuitOld.emSuit;
		dwHAND		= sSuitOld.dwHAND;
		emHand		= sSuitOld.emHand;
		...
		for ( int i=0; i<SSUIT_200::ADDON_SIZE; ++i )		
			sADDON[i] = sSuitOld.sADDON[i];
