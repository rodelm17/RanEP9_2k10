...
#include "../../Meshs/DxSkinEffResult.h"
...
enum EFFCHAR_TYPES
{
	...
	EMEFFCHAR_VISUALMATERIAL	= 31,
	EMEFFCHAR_SPLINESINGLEEFF	= 32,
	EMEFFCHAR_WORLDBATTLEFLAG	= 33,
	EMEFFCHAR_DISABLERENDER		= 34,
};

inline BOOL IsEffectNotSupported( DWORD dwID )
{
	//unsupported official effects
	if ( 
		dwID == EMEFFCHAR_OVERLAY ||
		dwID == EMEFFCHAR_TOTALSHADER ||
		dwID == EMEFFCHAR_VISUALMATERIAL ||
		dwID == EMEFFCHAR_SPLINESINGLEEFF ||
		dwID == EMEFFCHAR_WORLDBATTLEFLAG ||
		dwID == EMEFFCHAR_DISABLERENDER
		)
		return TRUE;
...
public:
	...
	virtual DWORD					GetMaterialsNum()				{ return 0;	}
	...
	virtual HRESULT R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice ) = 0;
