...
char		DxEffCharLevel::NAME[MAX_PATH]	= "DxEffCharLevel";
...
void	DxEffCharLevel::Render ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bPieceRender )
{
	...
	for ( DWORD i=0; i<m_dwMaterials; i++ )
	{
		...
		{
			if( strlen( m_pMaterials[i].szEffTex ) && m_pMaterials[i].bEffUse )
			{
				bAllLoad = FALSE;
...
HRESULT	DxEffCharLevel::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	else if ( dwVer == 0x0106 )
	{
		//	Note : 이팩트의 Property 를 저장.
		//
		EFFCHAR_PROPERTY_LEVEL_106	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_LEVEL_106) );

		m_dwFlag		= sProp.m_dwFlag;
		m_dwColorOP		= sProp.m_dwColorOP;
		m_dwMaterials	= sProp.m_dwMaterials;
		m_fPower		= sProp.m_fPower;
		m_cSpecular		= sProp.m_cSpecular;
	...
	else if ( dwVer == 0x0105 )
	{
		//	Note : 이팩트의 Property 를 저장.
		//
		EFFCHAR_PROPERTY_LEVEL_104_105	sProp;
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_LEVEL_104_105) );
	...
	{
		EFFCHAR_PROPERTY_LEVEL_102_103	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_LEVEL_102_103) );
	...
	{
		EFFCHAR_PROPERTY_LEVEL_101	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_LEVEL_101) );
		...
		StringCchCopy( m_pTempMaterials[3].szTexture, MAX_PATH, sProp.m_szSrcTex_4 );

		StringCchCopy( m_pTempMaterials[0].szEffTex, MAX_PATH, sProp.m_szTexture_1 );
		StringCchCopy( m_pTempMaterials[1].szEffTex, MAX_PATH, sProp.m_szTexture_2 );
		StringCchCopy( m_pTempMaterials[2].szEffTex, MAX_PATH, sProp.m_szTexture_3 );
		StringCchCopy( m_pTempMaterials[3].szEffTex, MAX_PATH, sProp.m_szTexture_4 );
	...
	{
		EFFCHAR_PROPERTY_LEVEL_100	sProp;

		//	Note : 이팩트의 Property 를 저장.
		//
		SFile.ReadBuffer ( &sProp, sizeof(EFFCHAR_PROPERTY_LEVEL_100) );
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharLevel::LoadFile", SFile.GetFileName(), dwVer );
...
HRESULT	DxEffCharLevel::R2LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	HRESULT hr;
	DWORD dwVer, dwSize;
	...
	{
		CDebugSet::ErrorVersion( "DxEffCharLevel::R2LoadFile", SFile.GetFileName(), dwVer );
