								   ...
								   SCHARDATA2* pChaData2)
{
	...
	TCHAR szTemp[2048] = {0};
	_snprintf( szTemp, 2048, "SELECT chaInfo.UserNum,chaInfo.SGNum,chaInfo.ChaName,ChaTribe,ChaClass,"
							...
							"ChaPromisePoint "
							"FROM ChaInfo as chaInfo "
							"LEFT JOIN RanUser.dbo.UserInfo as user_info "
							"ON chaInfo.UserNum = user_info.UserNum "
							"WHERE chaInfo.ChaNum=%d AND chaInfo.UserNum=%d", 
							nChaNum, nUserNumber);
...
//			strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
		...
		{		
			...
			/*contribution point, Juver, 2017/08/23 */
			::SQLGetData(pConn->hStmt,53, SQL_C_LONG, &nChaContributionPoint, 0, &cbChaContributionPoint);	
			...
			/* Gacha System, MontageDev 7/10/24 */
			::SQLGetData(pConn->hStmt, 56, SQL_C_LONG, &nGachaPromisePoint, 0, &cbGachaPromisePoint );	
