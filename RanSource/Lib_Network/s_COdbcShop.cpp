...
int COdbcManager::SetItemShop(CString strItemNum, DWORD dwUserID)
{
	...
	/* Baby011528, Add SQL Error Log, V3, 10-10-2023, Start */
	//CDebugSet::ToFileWithTime("_SQL_SetItemShop.txt","%s [Result]:%d",szTemp,nReturn);
	/* Baby011528, Add SQL Error Log, V3, 10-10-2023, End */
...
int COdbcManager::GetItemShop(std::vector<ITEMSHOP> &v)
{
	...
	_snprintf( szTemp, 256, "SELECT ProductNum, ItemMain, ItemSub , Itemstock , ItemMoney , ItemOrder , Category , ItemVisible FROM RanShop.dbo.ShopItemMap	 where Itemstock > 0 AND ItemVisible = 1");
...
/* Add ItemShop, Ssodomain, 11-09-2023, End */
int COdbcManager::SetPoints( int nChaNum, DWORD dwUserID , DWORD dwClientID , int  nAmount ,WORD wType )
{
	...
	{
	case 1://ep
		_snprintf( szTemp, 256, "UPDATE UserInfo SET UserPoint = UserPoint + %d WHERE UserNum=%d",
			nAmount,dwUserID );
		break;
	case 2://bp
		_snprintf( szTemp, 256, "UPDATE UserInfo SET UserPoint2 = UserPoint2 + %d WHERE UserNum=%d",
			nAmount,dwUserID );
		break;

	default: return DB_ERROR; break;
