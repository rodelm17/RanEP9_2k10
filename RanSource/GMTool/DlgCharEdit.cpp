...
BOOL CDlgCharEdit::DataSave()
{
	...
	BOOL bONLINE = FALSE;
	CGMToolOdbcBase::GetInstance()->CharOnlineCheck( m_pData->pCharData2->m_dwCharID, bONLINE );

	if ( bONLINE ){
		if ( CDebugSet::MsgBoxYesNo( GetSafeHwnd(), "Character: [%d]%s is Online! continue?", m_pData->pCharData2->m_dwCharID, m_pData->pCharData2->m_szName ) != IDYES ){
			return FALSE;
		}
	}
