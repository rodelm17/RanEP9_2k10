...
namespace TextureManager
{
	...
	protected:
		...
		HRESULT LoadImageData ( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bCube );
		HRESULT CopyRGBADataToSurface ();
		HRESULT Restore ( LPDIRECT3DDEVICEQ pd3dDevice );

	public:
		HRESULT LoadTexture ( const char* strName, LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwFlags, DWORD dwStage=0 );
		HRESULT LoadTexture ( LPDIRECT3DDEVICEQ pd3dDevice );

		HRESULT LoadCubeTexture ( const char* szName, LPDIRECT3DDEVICEQ pd3dDevice, DWORD dwFlags, DWORD dwStage );

		std::string GetTextureFilePath( const char* szFileName, BOOL &bEncrypted );
