...
#include "../Lib_Client/G-Logic/GLMobSchedule.h"
...
BEGIN_MESSAGE_MAP(CEditorLevelView, CView)
	...
	ON_COMMAND(ID_RENDER_PIECEUSECOLLISION, &CEditorLevelView::OnRenderPieceusecollision)
	ON_UPDATE_COMMAND_UI(ID_RENDER_PIECEUSECOLLISION, &CEditorLevelView::OnUpdateRenderPieceusecollision)
	ON_COMMAND(ID_RENDER_PIECEUSEDISTANCE, &CEditorLevelView::OnRenderPieceusedistance)
	ON_UPDATE_COMMAND_UI(ID_RENDER_PIECEUSEDISTANCE, &CEditorLevelView::OnUpdateRenderPieceusedistance)
END_MESSAGE_MAP()
...
void CEditorLevelView::OnLevelSave()
{
	...
	{
		CString szFilter = "lev|*.LEV|";
		CString strsavefile = GLLevelFile::GetLevelPath();
		strsavefile += m_strFileName.c_str();
		CFileDialog dlg ( false,".LEV", strsavefile.GetString(), OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, szFilter, this );
		dlg.m_ofn.lpstrInitialDir = GLLevelFile::GetLevelPath();
		if ( dlg.DoModal() == IDOK )
		{
			//do collision fix
			DWORD dwcollision_error = 0;
			DWORD dwcrow_error = 0;
			...
			{		
				...
				if ( !pSchdule->CheckCollision( m_pGLLandServer->GetNavi() ) )
					dwcollision_error ++;

				SCROWDATA* pcrow_data = GLCrowDataMan::GetInstance().GetCrowData( pSchdule->m_CrowID );
				if ( !pcrow_data )
					dwcrow_error ++;
			...
			if ( dwcollision_error )
				CDebugSet::MsgBox( GetSafeHwnd(), "collision error :%u", dwcollision_error );

			if ( dwcrow_error )
				CDebugSet::MsgBox( GetSafeHwnd(), "crow id error :%u", dwcrow_error );
...
void CEditorLevelView::OnRenderPieceusecollision()
{
	g_bPIECE_USE_COLLISION = !g_bPIECE_USE_COLLISION;
}

void CEditorLevelView::OnUpdateRenderPieceusecollision(CCmdUI *pCmdUI)
{
	if ( pCmdUI )	
		pCmdUI->SetCheck( g_bPIECE_USE_COLLISION );
}

void CEditorLevelView::OnRenderPieceusedistance()
{
	g_bPIECE_USE_DISTANCE = !g_bPIECE_USE_DISTANCE;
}

void CEditorLevelView::OnUpdateRenderPieceusedistance(CCmdUI *pCmdUI)
{
	if ( pCmdUI )	
