...
const DWORD OBJOCTreeLightMap::VERSION = 0x0200;

BOOL OBJOCTreeLightMap::IsLoad()
{
...
void OBJOCTreeLightMap::Save ( CSerialFile& SFile, BOOL bPiece )
{
	SFile << (DWORD)VERSION;
...
void OBJOCTreeLightMap::Load( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, BOOL bPiece )
{
	CleanUpBase();
	CleanUpDerived();
	...
	{
		CDebugSet::ErrorVersion( "OBJOCTreeLightMap::Load", SFile.GetFileName(), _dwVER );
...
void OBJOCTreeLightMap::Load_OLD( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bDynamicLoad, BOOL bPiece )
{
}

void OBJOCTreeLightMap::ThreadLoad( LOADINGDATALIST &listLoadingData, CRITICAL_SECTION &CSLockLoading )
{
	if( m_bCheck )	return;
	m_bCheck = TRUE;
...
BOOL OBJOCTreeLightMap::DynamicLoad( const LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile, const BOOL bThread )
{
	return TRUE;
}
