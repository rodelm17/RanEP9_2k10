...
BOOL CRanXMLParser::_ParseSENTENSE( STRFLAG & pData )
{
	...
	try
	{
		...
		while(SentenseElem){
			...
			STRFLAG_IT it = pData.find( std::string(szId) );
			if ( it != pData.end() )
			{
				RLOG::Log( "CRanXMLParser::_ParseSENTENSE duplicated %s", it->first );
		...
		while(WordElem){
			...
			while(ValueElem){
				...
				{	
					...
					if ( nCount2 != nIndex )
					{
						CDebugSet::MsgBox( "[%s] error! possible duplicated index:[%s]", szId, szValue );
					}
			...
			if ( it != pData.end() )
			{
				RLOG::Log( "CRanXMLParser::_ParseWORD duplicated %s", it->first );
...
BOOL CRanXMLParser::_ParseNUMBER( STRFLAG & pData )
{
	...
	try
	{
		...
		while(NumberElem){
			...
			if ( it != pData.end() )
			{
				RLOG::Log( "CRanXMLParser::_ParseNUMBER duplicated %s", it->first );
