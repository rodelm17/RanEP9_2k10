...
int CGetChaBInfo::Execute(CServer* pServer)
{
	...
	{
		CAgentServer* pTemp = reinterpret_cast<CAgentServer*> (pServer);
		if (COdbcManager::GetInstance()->GetChaBInfo(m_nUserNum, m_nChaNum, &m_msg.Data, RPARAM::nNameType) == DB_ERROR)
		{			
			return NET_ERROR;
		}
		else
	...
	DWORD dwVPoints )
{
	...
	/*dmk14 ingame web*/
	m_cCharAgData.m_dwPPoints = dwPPoints;
	m_cCharAgData.m_dwVPoints = dwVPoints;

	/* user flag verified, Juver, 2020/02/25 */
	m_cCharAgData.m_bUserFlagVerified = bUserFlagVerified;

	/* user flag restricted, Juver, 2020/04/20 */
	m_cCharAgData.m_bUserFlagRestricted = bUserFlagRestricted;

	m_cCharAgData.m_bUserFlagHWIDWatchlist = bUserFlagHWIDWatchlist;
}
...
int CGetChaInfoAndJoin::Execute( CServer* pServer )
{
	...
	nRetCode = COdbcManager::GetInstance()->GetCharacterInfo(
						...
						&m_cCharAgData,
						RPARAM::nNameType ); // Use parameter from .ini
...
#else
	std::vector<CHA_FRIEND> vecFriend;
	nRetCode = COdbcManager::GetInstance()->GetChaFriend( m_cCharAgData.m_dwCharID, vecFriend, RPARAM::nNameType );
	if ( nRetCode == DB_ERROR )
	{
...
//		m_cCharAgData.m_mapFriend[sFRIEND.szCharName] = sFRIEND;
	...
	nRetCode = COdbcManager::GetInstance()->getChaTitle( m_cCharAgData.m_dwCharID, m_cCharAgData.m_mapAwardTitle );
	if ( nRetCode == DB_ERROR )
	{
		CConsoleMessage::GetInstance()->Write( _T("ERROR:Read Award Title failed") );
		return NET_OK;
	}
	...
	DWORD dwVPoints )
{
	...
	/*dmk14 ingame web*/
	m_cCharData.m_dwPPoints = dwPPoints;
	m_cCharData.m_dwVPoints = dwVPoints;

	/* user flag verified, Juver, 2020/02/25 */
	m_cCharData.m_bUserFlagVerified	= bUserFlagVerified;

	/* user flag restricted, Juver, 2020/04/20 */
	m_cCharData.m_bUserFlagRestricted	= bUserFlagRestricted;
}
